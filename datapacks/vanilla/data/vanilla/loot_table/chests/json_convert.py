...
def convert_loot_table(old_data):
    ...
    for pool in old_data.get("pools", []):  # Ensure pools exist
        new_pool = {
            "rolls": {},
            "entries": []
        }

        # Handle different roll structures
        rolls = pool.get("rolls", 1)
        if isinstance(rolls, dict) and rolls.get("type") == "minecraft:uniform":
            new_pool["rolls"] = {"min": int(rolls["min"]), "max": int(rolls["max"])}
        else:
            new_pool["rolls"] = int(rolls)

        for entry in pool.get("entries", []):
            new_entry = {
                "type": "item" if entry["type"] == "minecraft:item" else entry["type"],
                "name": entry["name"].replace("minecraft:", "mcl_") if "name" in entry else "",
                "weight": entry.get("weight", 1),
            }
            ...
            if "functions" in entry:
                ...
                for func in entry["functions"]:
                    if func["function"] == "minecraft:set_count":
                        ...
                        new_entry["functions"].append(func)
