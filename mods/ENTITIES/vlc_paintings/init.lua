...
local wood = "[combine:16x16:-192,0=vlc_paintings_paintings.png"
...
local function shrink_painting(x, y)
	...
	while not painting do
		painting = vlc_paintings.paintings[y] and vlc_paintings.paintings[y][x]
...
local function get_painting(x, y, motive)
	local painting = vlc_paintings.paintings[y] and vlc_paintings.paintings[y][x] and vlc_paintings.paintings[y][x][motive]
	if not painting then
		return nil
	end
	local px, py = -painting.cx, -painting.cy
	local sx, sy = 16*x, 16*y
	return "[combine:"..sx.."x"..sy..":"..px..","..py.."=vlc_paintings_paintings.png"
end

local function get_random_painting(x, y)
	if not vlc_paintings.paintings[y] or not vlc_paintings.paintings[y][x] then
		return nil
	end
	local max = #vlc_paintings.paintings[y][x]
	if max < 1 then
		return nil
	end
...
local function set_entity(object)
	...
	if not ent._xsize or not ent._ysize or not ent._motive then
		minetest.log("error", "[vlc_paintings] Painting loaded with missing painting values!")
		return
	end
	local painting = get_painting(xsize, ysize, ent._motive)
	if not painting then
		minetest.log("error", "[vlc_paintings] No painting found for size "
...
minetest.register_entity("vlc_paintings:painting", {
	initial_properties = {
		visual = "cube",
	...
	on_punch = function(self, puncher, time_from_last_punch, tool_capabilities, dir, damage)
		...
		if puncher and puncher:is_player() then
			...
			if not vlc_util.check_position_protection(pos, puncher) then
				...
				minetest.after(0.15, function(object)
					...
					if not minetest.is_creative_enabled(kname) then
						minetest.add_item(pos, "vlc_paintings:painting")
					end
				end, self.object)
			end
		end
...
minetest.register_craftitem("vlc_paintings:painting", {
	description = S("Painting"),
	inventory_image = "vlc_paintings_painting.png",
	on_place = function(itemstack, placer, pointed_thing)
		...
		if wallm then
			...
			if vlc_util.check_position_protection(ppos, placer) then return itemstack end
			local ppos2
			if dir.z ~= 0 then
			...
			if vlc_util.check_position_protection(ppos2, placer) then return itemstack end
			local painting, pid = get_random_painting(xsize, ysize)
			if not painting then
				minetest.log("error", "[vlc_paintings] No painting found for size "..xsize..","..ysize.."!")
				return itemstack
			end
			local staticdata = {
				_facing = wallm,
			...
			local obj = minetest.add_entity(pposa, "vlc_paintings:painting", minetest.serialize(staticdata))
			if not obj then
				return itemstack
			end
		else
...
vlc_wip.register_wip_item("vlc_paintings:painting")

minetest.register_craft({
	output = "vlc_paintings:painting",
	recipe = {
		{ "vlc_core:stick", "vlc_core:stick", "vlc_core:stick" },
		{ "vlc_core:stick", "group:wool", "vlc_core:stick" },
		{ "vlc_core:stick", "vlc_core:stick", "vlc_core:stick" },
	}
})
