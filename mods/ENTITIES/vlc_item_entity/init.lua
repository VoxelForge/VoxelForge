...
vlc_item_entity.get_gravity = get_gravity

local registered_pickup_achievement = {}

function vlc_item_entity.register_pickup_achievement(itemname, award)
	if not has_awards then
		minetest.log("warning", "[vlc_item_entity] Trying to register pickup achievement ["..award.."] for ["..itemname.."] while awards missing")
	else
...
vlc_item_entity.register_pickup_achievement("tree", "vlc:mineWood")
vlc_item_entity.register_pickup_achievement("vlc_mobitems:blaze_rod", "vlc:blazeRod")
vlc_item_entity.register_pickup_achievement("vlc_mobitems:leather", "vlc:killCow")
vlc_item_entity.register_pickup_achievement("vlc_core:diamond", "vlc:diamonds")
vlc_item_entity.register_pickup_achievement("vlc_core:crying_obsidian", "vlc:whosCuttingOnions")
vlc_item_entity.register_pickup_achievement("vlc_nether:ancient_debris", "vlc:hiddenInTheDepths")
vlc_item_entity.register_pickup_achievement("vlc_end:dragon_egg", "vlc:PickUpDragonEgg")
vlc_item_entity.register_pickup_achievement("vlc_armor:elytra", "vlc:skysTheLimit")

vlc_player.register_globalstep(function(player)
		...
		for _,object in pairs(minetest.get_objects_inside_radius(checkpos, item_drop_settings.xp_radius_magnet)) do
			if not object:is_player() then
				...
				le._magnet_timer and (le._insta_collect or (le.age > item_drop_settings.age)) then
					le:pickup(player)
				elseif le and le.name == "vlc_experience:orb" and not le.collected then
...
local function get_drops(drop, toolname, param2, paramtype2)
	local tmp_node_name = "vlc_item_entity:TMP_NODE"
...
local doTileDrops = minetest.settings:get_bool("vlc_doTileDrops", true)
...
function minetest.handle_node_drops(pos, drops, digger)
	...
	if digger and digger:is_player() then
		...
		if not vlc_autogroup.can_harvest(dug_node.name, tool:get_name(), digger) then
			return
		end
	end

	local diggroups = tooldef and tooldef._vlc_diggroups
	local shearsy_level = diggroups and diggroups.shearsy and diggroups.shearsy.level

	--[[ Special node drops when dug by shears by reading _vlc_shears_drop or with a silk touch tool reading _vlc_silk_touch_drop
	from the node definition.
	Definition of _vlc_shears_drop / _vlc_silk_touch_drop:
	* true: Drop itself when dug by shears / silk touch tool
	* table: Drop every itemstring in this table when dug by shears _vlc_silk_touch_drop
	]]

	local enchantments = tool and vlc_enchanting.get_enchantments(tool, "silk_touch")

	local silk_touch_drop = false
	local nodedef = minetest.registered_nodes[dug_node.name]
	if not nodedef then return end

	if shearsy_level and shearsy_level > 0 and nodedef._vlc_shears_drop then
		if nodedef._vlc_shears_drop == true then
			drops = { dug_node.name }
		else
			drops = nodedef._vlc_shears_drop
		end
	elseif tool and enchantments.silk_touch and nodedef._vlc_silk_touch_drop then
		silk_touch_drop = true
		if nodedef._vlc_silk_touch_drop == true then
			drops = { dug_node.name }
		else
			drops = nodedef._vlc_silk_touch_drop
		end
	end

	if tool and nodedef._vlc_fortune_drop and enchantments.fortune then
		local fortune_level = enchantments.fortune
		local fortune_drop = nodedef._vlc_fortune_drop
	...
	if digger and vlc_experience.throw_xp and not silk_touch_drop then
		local experience_amount = minetest.get_item_group(dug_node.name,"xp")
		if experience_amount > 0 then
			vlc_experience.throw_xp(pos, experience_amount)
		end
	end
...
minetest.register_entity(":__builtin:item", {
	...
	set_item = function(self, itemstring)
		...
		if minetest.get_item_group(stack:get_name(), "compass") > 0 then
			...
			if string.find(stack:get_name(), "_lodestone") then
				stack:set_name("vlc_compass:18_lodestone")
			else
				stack:set_name("vlc_compass:18")
			end
		end
	...
	on_step = function(self, dtime, moveresult)
		...
		if self.is_clock then
			self.object:set_properties({
				wield_item = "vlc_clock:clock_" .. (vlc_worlds.clock_works(p) and vlc_clock.old_time or vlc_clock.random_frame)
			})
		end
	...
	apply_physics = function(self, dtime, moveresult)
		...
		if moveresult and moveresult.collides then
			for _, collision in pairs(moveresult.collisions) do
				...
				if collision.type == "node" and minetest.get_node(pos).name == "vlc_core:cactus" then
					self._removed = true
					self.object:remove()
					return
				end
