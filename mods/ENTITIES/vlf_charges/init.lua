...
vlf_charges.wind_burst_spawner = {
...
for i=1,2 do
	table.insert(vlf_charges.wind_burst_spawner.texpool, {
...
function vlf_charges.wind_burst(pos, radius)
	for obj in minetest.objects_inside_radius(pos, radius) do
		...
		if obj:is_player() then
			obj:add_velocity(vector.multiply(vector.normalize(vector.subtract(obj_pos, pos)), vlf_util.float_random(1.8, 2.0) / dist * RADIUS))
		else
...
function vlf_charges.register_charge(name, descr, def)
