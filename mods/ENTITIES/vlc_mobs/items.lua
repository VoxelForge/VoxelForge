...
local function get_armor_texture(armor_name)
	...
	return "vlc_armor_"..string.sub(armor_name, seperator+1, -1)..".png^"
end
...
function mob_class:set_armor_texture()
	if self.armor_list then
		...
		for _,item in pairs(self.armor_list) do
			...
			if type(minetest.get_item_group(item, "vlc_armor_points")) == "number" then
				armor_.fleshy=armor_.fleshy-(minetest.get_item_group(item, "vlc_armor_points")*3.5)
			end
		end
	...
	if self.pick_up and #self.pick_up > 0 or self.wears_armor then
		...
		for _,o in pairs(minetest.get_objects_inside_radius(p,2)) do
			...
			if l and l.name == "__builtin:item" then
				if not player_near(p) and l.itemstring:find("vlc_armor") and self.wears_armor then
					local armor_type
