...
function mob_class:is_at_water_danger()
	...
	if not vlc_mobs.check_vector(pos) or not vlc_mobs.check_vector(p1) or not vlc_mobs.check_vector(p2) then return false end
...
function mob_class:is_object_in_view(object_list, object_range, node_range, turn_around)
	...
	for n = 1, #objs do
		...
		if object:is_player() then
			if not (vlc_mobs.invis[ object:get_player_name() ]
...
function mob_class:follow_holding(clicker)
	...
	if vlc_mobs.invis[clicker:get_player_name()] then
		return false
	end
...
function mob_class:follow_flop()
	...
	and self.state ~= "runaway" then
		...
		for n = 1, #players do
			...
			and not vlc_mobs.invis[ players[n]:get_player_name() ] then
