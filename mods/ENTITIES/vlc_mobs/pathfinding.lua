local mob_class = vlc_mobs.mob_class
...
local LOGGING_ON = minetest.settings:get_bool("vlc_logging_mobs_pathfinding",false)
...
function output_table (wp)
	...
	vlc_log("wp items: ".. tostring(#wp))
	for a,b in pairs(wp) do
		vlc_log(a.. ": ".. tostring(b))
	end
end

function append_paths (wp1, wp2)
	--vlc_log("Start append")
	if not wp1 or not wp2 then
		vlc_log("Cannot append wp's")
		return
	end
	output_table(wp1)
	output_table(wp2)
...
local function output_enriched (wp_out)
	vlc_log("Output enriched path")
	local i = 0
	for _,outy in pairs (wp_out) do
		i = i + 1
		vlc_log("Pos ".. i ..":" .. minetest.pos_to_string(outy["pos"]))

		local action =  outy["action"]
		if action then
			--vlc_log("Pos ".. i ..":" .. minetest.pos_to_string(outy["pos"]))
			vlc_log("type: " .. action["type"])
			vlc_log("action: " .. action["action"])
			vlc_log("target: " .. minetest.pos_to_string(action["target"]))
		end
		--vlc_log("failed attempts: " .. outy["failed_attempts"])
	end
end
...
function mob_class:ready_to_path(prioritised)
	...
	if self._pf_last_failed and (os.time() - self._pf_last_failed) < (PATHFINDING_FAIL_WAIT + math.random(1, PATHFINDING_FAIL_WAIT)) then
		vlc_log("Not ready to path as last fail is less than threshold: " .. (os.time() - self._pf_last_failed))
		return false
	else
		local time_since_path_start = os.time() - last_pathing_time
		vlc_log("time_since_path_start: " .. tostring(time_since_path_start))
		if prioritised or (time_since_path_start) > PATHING_START_DELAY then
			vlc_log("We are ready to pathfind, no previous fail or we are past threshold")
			return true
		end
	end
end
...
local function calculate_path_through_door (p, cur_door_pos, t)
	if t then
		vlc_log("Plot route through door from pos: " .. minetest.pos_to_string(p) .. ", to target: " .. minetest.pos_to_string(t))
	else
		vlc_log("Plot route through door from pos: " .. minetest.pos_to_string(p))
	end
	...
	if cur_door_pos then
		vlc_log("Found a door near: " .. minetest.pos_to_string(cur_door_pos))
		...
		for _,v in pairs(plane_adjacents) do
			...
			if n.name == "air" then
				vlc_log("We have air space next to door at: " .. minetest.pos_to_string(pos_closest_to_door))

				prospective_wp = minetest.find_path(p, pos_closest_to_door, PATHFINDING_SEARCH_DISTANCE, 1, 4)

				if prospective_wp then
					vlc_log("Found a path to next to door".. minetest.pos_to_string(pos_closest_to_door))
					vlc_log("Opposite is: ".. minetest.pos_to_string(other_side_of_door))

					table.insert(prospective_wp, cur_door_pos)

					if t then
						vlc_log("We have t, lets go from door to target")
						...
						if wp_otherside_door_to_target and #wp_otherside_door_to_target > 0 then
							...
							wp = prospective_wp
							vlc_log("We have a path from outside door to target")
						else
							vlc_log("We cannot path from outside door to target")
						end
					else
						vlc_log("No t, just add other side of door")
					...
					vlc_log("Cannot path to this air block next to door.")
				end
			end
		end
	else
		vlc_log("No door found")
	end

	if wp and not enriched_path then
		vlc_log("Wp but not enriched")
...
function mob_class:gopath(target, callback_arrived, prioritised)
	if self.state == PATHFINDING then vlc_log("Already pathfinding, don't set another until done.") return end
	...
	if not wp then
		vlc_log("### No direct path. Path through door closest to target.")
		local door_near_target = minetest.find_node_near(target, 16, {"group:door"})
		wp = calculate_path_through_door(p, door_near_target, t)

		if not wp then
			vlc_log("### No path though door closest to target. Try door closest to origin.")
			...
			if not wp then
				vlc_log("### Still not wp. Need to path through 2 doors.")
				...
				if path_through_closest_door and #path_through_closest_door > 0 then
					vlc_log("We have path through first door")
					vlc_log("Number of pos in path through door: " .. tostring(#path_through_closest_door))
					...
					if pos_after_door_entry then
						...
						vlc_log("pos_after_door: " .. minetest.pos_to_string(pos_after_door))
						...
						if path_after_door and #path_after_door > 1 then
							vlc_log("We have path after first door")
							table.remove(path_after_door, 1) -- Remove duplicate
							wp = path_through_closest_door
							append_paths (wp, path_after_door)
						else
							vlc_log("Path after door is not good")
						end
					else
						vlc_log("No pos after door")
					end
				else
					vlc_log("Path through closest door empty or null")
				end
			else
				vlc_log("ok, we have a path through 1 door")
			end
		end
	else
		wp = generate_enriched_path(wp)
		vlc_log("We have a direct route")
	end

	if not wp then
		vlc_log("Could not calculate path")
	...
	if wp and #wp > 0 then
		...
		if current_location and current_location["pos"] then
			vlc_log("Removing first co-ord? " .. tostring(current_location["pos"]))
		else
			vlc_log("Nil pos")
		end
...
function mob_class:interact_with_door(action, target)
	if target then
		vlc_log("Door target is: ".. minetest.pos_to_string(target))
		...
		vlc_log("no target. cannot try and open or close door")
	end
	--end
end
...
function mob_class:do_pathfind_action(action)
	if action then
		vlc_log("Action present")
		local type = action["type"]
		local action_val = action["action"]
		local target = action["target"]
		if target then
			vlc_log("Target: ".. minetest.pos_to_string(target))
		end
		if type and type == "door" then
			vlc_log("Type is door")
...
function mob_class:check_gowp(dtime)
	...
	if not p or not self._target then
		vlc_log("p: ".. tostring(p))
		vlc_log("self._target: ".. tostring(self._target))
		return
	end

	-- arrived at location, finish gowp
	local distance_to_targ = vector.distance(p,self._target)
	--vlc_log("Distance to targ: ".. tostring(distance_to_targ))
	if distance_to_targ < 1.8 then
		vlc_log("Arrived at _target")
		self.waypoints = nil
		self._target = nil
		self.current_target = nil
	...
	elseif not self.current_target then
		vlc_log("Not close enough to targ: ".. tostring(distance_to_targ))
	end
	...
	if self.waypoints and #self.waypoints > 0 and ( not self.current_target or not self.current_target["pos"] or distance_to_current_target < 0.9 ) then
		...
		vlc_log("There after " .. failed_attempts .. " failed attempts. current target:".. minetest.pos_to_string(self.current_target["pos"]) .. ". Distance: " ..  distance_to_current_target)
	...
	elseif self.current_target and self.current_target["pos"] then
		...
		if failed_attempts >= PATHFINDING_FAIL_THRESHOLD then
			vlc_log("Failed to reach position (" .. minetest.pos_to_string(self.current_target["pos"]) .. ") too many times. Abandon route. Times tried: " .. failed_attempts)
			self:set_state("stand")
			self.current_target = nil
			self.waypoints = nil
			self._target = nil
		...
		--vlc_log("Not at pos with failed attempts ".. failed_attempts ..": ".. minetest.pos_to_string(p) .. "self.current_target: ".. minetest.pos_to_string(self.current_target["pos"]) .. ". Distance: ".. distance_to_current_target)
		self:go_to_pos(self.current_target["pos"])
		...
		vlc_log("We don't have waypoints or a current target. Let's try to path to target")

		if self.waypoints then
			vlc_log("WP: " .. tostring(self.waypoints))
			vlc_log("WP num: " .. tostring(#self.waypoints))
		else
			vlc_log("No wp set")
		end
		if self.current_target then
			vlc_log("Current target: " .. tostring(self.current_target))
		else
			vlc_log("No current target")
		end

		local final_wp = minetest.find_path(p, self._target, PATHFINDING_SEARCH_DISTANCE, 1, 4)
		if final_wp then
			vlc_log("We can get to target here.")
		--	self.waypoints = final_wp
			self:go_to_pos(self._target)
		else
			-- Abandon route?
			vlc_log("Cannot plot final route to target")
		end
	end
	...
	if self.current_target and self.current_target["pos"] and (self.waypoints and #self.waypoints == 0) then
		...
		vlc_log("Distance to current target: ".. tostring(distance_to_cur_targ))
		vlc_log("Current p: ".. minetest.pos_to_string(updated_p))
		...
		if distance_to_cur_targ > 1.9 then
			vlc_log("not close to current target: ".. minetest.pos_to_string(self.current_target["pos"]))
			self:go_to_pos(self._current_target)
		else
			vlc_log("close to current target: ".. minetest.pos_to_string(self.current_target["pos"]))
			vlc_log("target is: ".. minetest.pos_to_string(self._target))
			self.current_target = nil
		end
		return
	end
