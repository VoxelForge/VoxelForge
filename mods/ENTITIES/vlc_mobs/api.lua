...
local spawn_logging = minetest.settings:get_bool("vlc_logging_mobs_spawn", false)
local peaceful_mode = minetest.settings:get_bool("only_peaceful_mobs", false)

function mob_class:set_properties(prop)
	vlc_util.set_properties(self.object, prop)
end
...
function mob_class:get_staticdata()
	...
	if not vlc_mobs.check_vector(pos) then
		self.object:remove()
		return
	end
	...
	and self.lifetimer <= 20 then
		if spawn_logging then
			minetest.log("action", "[vlc_mobs] Mob "..tostring(self.name).." despawns at "..minetest.pos_to_string(vector.round(pos)) .. " - out of range")
		end
...
function mob_class:update_textures()
	local def = vlc_mobs.registered_mobs[self.name]
...
function mob_class:on_step(dtime)
	local pos = self.object:get_pos()
	if not vlc_mobs.check_vector(pos) or self.removed then
		self:safe_remove()
		return
	end
