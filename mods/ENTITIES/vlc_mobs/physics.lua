...
function mob_class:object_in_range(object)
	...
	if object:is_player() then
		local factors = vlc_armor.player_view_range_factors[object]
	...
	for _,object in pairs(minetest.get_objects_inside_radius(pos, width)) do
		...
		if (self.pushable and object:is_player()) or
		   (self.mob_pushable and ent and ent.is_mob and object ~= self.object) then
			...
			if object:is_player() and vlc_burning.is_burning(self.object) then
				vlc_burning.set_on_fire(object, 4)
			end
...
function mob_class:set_yaw(yaw, delay, dtime)
	...
	--vlc_log("Yaw is: \t\t" .. tostring(math.deg(yaw)))
	--vlc_log("self.object:get_yaw() is: \t" .. tostring(math.deg(self.object:get_yaw())))
	...
	if math.abs(target_shortest_path_nums) > 10 then
		...
		if self.acc and vlc_mobs.check_vector(self.acc) then
...
function vlc_mobs.yaw(self, yaw, delay, dtime)
...
function mob_class:check_for_death(cause, cmi_cause)
	...
	local function death_handle(self)
		...
		if cause == "lava" or cause == "fire" then
			...
			local cooked = vlc_burning.is_burning(self.object) or vlc_enchanting.has_enchantment(wielditem, "fire_aspect")
			local looting = vlc_enchanting.get_enchantment(wielditem, "looting")
			...
			if killed_by_player then
				if self.type == "monster" or self.name == "mobs_mc:zombified_piglin" and self.last_player_hit_name then
					awards.unlock(self.last_player_hit_name, "vlc:monsterHunter")
				end
				if ((not self.child) or self.type ~= "animal") and (minetest.get_us_time() - self.xp_timestamp <= math.huge) then
					...
					if not minetest.is_creative_enabled(self.last_player_hit_name) and not vlc_sculk.handle_death(pos, xp_amount) then
						vlc_experience.throw_xp(pos, xp_amount)
					end
				end
			end
		end
	...
	local kill = function(self)
		...
		self:safe_remove()
		vlc_mobs.death_effect(dpos, yaw, cbox, not self.instant_death)
	end
	if length <= 0 then
		kill(self)
	else
...
function mob_class:deal_light_damage(pos, damage)
	if not ((vlc_weather.rain.raining or vlc_weather.state == "snow") and vlc_weather.is_outdoor(pos)) then
		self:damage_mob("light", damage)

		vlc_mobs.effect(pos, 5, "vlc_particles_smoke.png")
...
function mob_class:do_env_damage()
	...
	if (self.sunlight_damage ~= 0 or self.ignited_by_sunlight) and (sunlight or 0) >= minetest.LIGHT_MAX and dim == "overworld" then
		if self.armor_list and not self.armor_list.helmet or not self.armor_list or self.armor_list and self.armor_list.helmet and self.armor_list.helmet == "" then
			if self.ignited_by_sunlight then
				vlc_burning.set_on_fire(self.object, 10)
			else
	...
	-- wither rose effect
	elseif self.standing_in == "vlc_flowers:wither_rose" then
		vlc_potions.withering_func(self.object, 1, 2)
	end
	...
	if self.rain_damage > 0 then
		if vlc_weather.rain.raining and vlc_weather.is_outdoor(pos) then
	...
	and nodef.groups.water then
		...
		vlc_mobs.effect(pos, 5, "vlc_particles_smoke.png", nil, nil, 1, nil)
	...
	and self:is_in_node("group:lava") then
		...
		if self.lava_damage ~= 0 then
			...
			vlc_mobs.effect(pos, 5, "fire_basic_flame.png", nil, nil, 1, nil)
			vlc_burning.set_on_fire(self.object, 10)
	...
	and self:is_in_node("group:fire") then
		...
		if self.fire_damage ~= 0 then
			...
			vlc_mobs.effect(pos, 5, "fire_basic_flame.png", nil, nil, 1, nil)
			vlc_burning.set_on_fire(self.object, 5)
	...
	if self.object:get_properties().breath_max ~= -1 then
		...
		if drowning then
			...
			vlc_mobs.effect(pos, 2, "bubble.png", nil, nil, 1, nil)
	...
	-- FIXME: Redundant with vlc_playerplus
...
function mob_class:damage_mob(reason, damage)
	...
	if damage > 0 then
		local vlc_reason = { type = reason }
		vlc_damage.finish_reason(vlc_reason)
		vlc_util.deal_damage(self.object, damage, vlc_reason)

		vlc_mobs.effect(self.object:get_pos(), 5, "vlc_particles_smoke.png", 1, 2, 2, nil)
...
function mob_class:check_water_flow()
	-- Add water flowing for mobs from vlc_item_entity
	local p, node, nn, def
