vlc_mobs = {}
vlc_mobs.registered_mobs = {}
local modname = minetest.get_current_modname()
local path = minetest.get_modpath(modname)
local S = minetest.get_translator(modname)

local old_spawn_icons = minetest.settings:get_bool("vlc_old_spawn_icons",false)
local extended_pet_control = minetest.settings:get_bool("vlc_extended_pet_control",false)
...
vlc_mobs.mob_class = {
	...
	player_active_range = tonumber(minetest.settings:get("vlc_mob_active_range")) or 48,
...
vlc_mobs.mob_class_meta = {__index = vlc_mobs.mob_class}
vlc_mobs.fallback_node = minetest.registered_aliases["mapgen_dirt"] or "vlc_core:dirt"

function vlc_mobs.check_vector(v)
...
local node_ok = function(pos, fallback)
	fallback = fallback or vlc_mobs.fallback_node
...
function vlc_mobs.mob_class:set_nametag(name)
	if name ~= "" then
...
local on_rightclick_prefix = function(self, clicker)
	...
	if not self.ignores_nametag and item_name == "vlc_mobitems:nametag" then
...
local function within_limits(pos, radius)
	...
	if vlc_vars then
		if vlc_vars.mapgen_edge_min and vlc_vars.mapgen_edge_max then
			wmin, wmax = vlc_vars.mapgen_edge_min, vlc_vars.mapgen_edge_max
		end
	end
	if radius then
		wmin = wmin - radius
...
vlc_mobs.spawning_mobs = {}
function vlc_mobs.register_mob(name, def)
	local def = table.copy(def)
	if not def.description then
		minetest.log("warning","[vlc_mobs] Mob "..name.." registered without description field. This is needed for proper death messages.")
	end

	vlc_mobs.spawning_mobs[name] = true
	vlc_mobs.registered_mobs[name] = def
	...
	init_props.collisionbox = init_props.collisionbox or vlc_mobs.mob_class.initial_properties.collisionbox
	init_props.selectionbox = init_props.selectionbox or init_props.collisionbox or vlc_mobs.mob_class.initial_properties.selectionbox

	local final_def = setmetatable(table.merge(def,{
		initial_properties = table.merge(vlc_mobs.mob_class.initial_properties,init_props),
	...
	}),vlc_mobs.mob_class_meta)

	vlc_mobs.registered_mobs[name] = final_def
...
function vlc_mobs.get_arrow_damage_func(damage, typ, shooter)
	local typ = vlc_damage.types[typ] and typ or "arrow"
	return function(projectile, object)
		return vlc_util.deal_damage(object, damage, {type = typ, source = shooter or projectile._shooter})
	end
end

vlc_mobs.arrow_class = {
	initial_properties = {
		physical = false,
...
vlc_mobs.arrow_class_meta = {__index = vlc_mobs.arrow_class}

-- register arrow for shoot attack
function vlc_mobs.register_arrow(name, def)
	if not name or not def then return end -- errorcheck

	local init_props = vlc_mobs.arrow_class.initial_properties
	for _,k in pairs(object_properties) do
		if def[k] ~= nil then
			if type(def[k]) == "table" then
				init_props[k] = table.copy(def[k])
	...
	}, def), vlc_mobs.arrow_class_meta))
end

-- Register spawn eggs

function vlc_mobs.register_egg(mob, desc, background_color, overlay_color, addegg, no_creative)
	...
	if old_spawn_icons then
		...
		if vlc_util.file_exists(minetest.get_modpath("mobs_mc").."/textures/"..fn) then
			invimg = fn
		end
	end
	if addegg == 1 then
