...
local enable_blood = minetest.settings:get_bool("vlc_damage_particles", true)
...
function vlc_mobs.effect(pos, amount, texture, min_size, max_size, radius, gravity, glow, go_down)
...
function vlc_mobs.death_effect(pos, yaw, collisionbox, rotate)
	local min, max
...
function mob_class:damage_effect(damage)
	...
	if enable_blood and damage > 0 then
		...
		if amount_large > 0 then
			vlc_mobs.effect(pos, amount_large, texture, 2, 2, 1.75, 0, nil, true)
		end
		-- half heart damage (one additional particle if damage is an odd number)
		if amount_small > 0 then
			-- TODO: Use "half heart"
			vlc_mobs.effect(pos, amount_small, texture, 1, 1, 1.75, 0, nil, true)
		end
	end
end
...
function vlc_mobs.set_animation(self, anim)
...
function mob_class:check_head_swivel(dtime)
	...
	vlc_util.set_bone_position(self.object,self.head_swivel, vector.new(0,self.bone_eye_height,self.horizontal_head_height), final_rotation)
end
