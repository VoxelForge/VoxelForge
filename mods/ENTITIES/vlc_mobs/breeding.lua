...
local LOGGING_ON = minetest.settings:get_bool("vlc_logging_mobs_villager",false)

local LOG_MODULE = "[vlc_mobs]"
...
function vlc_mobs.capture_mob(self, clicker, chance_hand, chance_net, chance_lasso, force_take, replacewith)
	return false
end


-- No-op in MCL2 (protecting mobs is not possible).
function vlc_mobs.protect(self, clicker)
	return false
end
...
function mob_class:use_shears(new_textures, shears_stack)
	if minetest.get_item_group(shears_stack:get_name(), "shears") > 0 then
		...
		self.gotten = true
		minetest.sound_play("vlc_tools_shears_cut", { pos = self.object:get_pos() }, true)
		local shears_def = shears_stack:get_definition()
		shears_stack:add_wear(65535 / shears_def._vlc_diggroups.shearsy.uses)
	end
	return shears_stack
end
...
function vlc_mobs.spawn_child(pos, mob_type)
	...
	vlc_mobs.effect(pos, 15, "vlc_particles_smoke.png", 1, 2, 2, 15, 5)
...
function mob_class:check_breeding()
	...
	and self.hornytimer <= HORNY_TIME then
		...
		vlc_log("In breed function. All good. Do the magic.")

		local pos = self.object:get_pos()

		vlc_mobs.effect({x = pos.x, y = pos.y + 1, z = pos.z}, 8, "heart.png", 3, 4, 1, 0.1)
		...
		for n = 1, #objs do
			...
			if canmate then vlc_log("In breed function. Can mate.") end
			...
			if num > 1 then
				...
				minetest.after(5, function(parent1, parent2, pos)
					...
					vlc_experience.throw_xp(pos, math.random(1, 7))
