...
local modern_lighting = minetest.settings:get_bool("vlc_mobs_modern_lighting", true)
...
local mob_cap = {
	monster = tonumber(minetest.settings:get("vlc_mob_cap_monster")) or 70,
	animal = tonumber(minetest.settings:get("vlc_mob_cap_animal")) or 10,
	ambient = tonumber(minetest.settings:get("vlc_mob_cap_ambient")) or 15,
	water = tonumber(minetest.settings:get("vlc_mob_cap_water")) or 5,
	water_ambient = tonumber(minetest.settings:get("vlc_mob_cap_water_ambient")) or 20,
	player = tonumber(minetest.settings:get("vlc_mob_cap_player")) or 75,
	total = tonumber(minetest.settings:get("vlc_mob_cap_total")) or 500,
}

--do mobs spawn?
local mobs_spawn = minetest.settings:get_bool("mobs_spawn", true) ~= false
local spawn_protected = minetest.settings:get_bool("mobs_spawn_protected") ~= false
local logging = minetest.settings:get_bool("vlc_logging_mobs_spawn", false)
...
function vlc_mobs.spawn_setup(def)
	...
	local min_height       = def.min_height or vlc_vars["mg_"..dimension.."_min"] or -31000
	local max_height       = def.max_height or vlc_vars["mg_"..dimension.."_max"] or 31000
	...
	if numbers then
		...
		if chance == 0 then
			minetest.log("warning", string.format("[vlc_mobs] %s has spawning disabled", name))
			return
		end
		minetest.log("action", string.format("[vlc_mobs] Chance setting for %s changed to %s (total: %s)", name, chance, aoc))
	end
...
function vlc_mobs.get_mob_light_level(mob,dim)
...
local function has_room(self,pos)
	...
	if r > n then
		minetest.log("info","[vlc_mobs] No room for mob "..self.name.." at "..minetest.pos_to_string(vector.round(pos)))
		return false
	end
	return true
end
...
local function spawn_check(pos,spawn_def,ignore_caps)
	...
	local dimension = vlc_worlds.pos_to_dimension(pos)
	...
	local is_bedrock  = gotten_node == "vlc_core:bedrock"
...
function vlc_mobs.spawn(pos,id, staticdata)
...
vlc_mobs.spawn_group = spawn_group

local S = minetest.get_translator("vlc_mobs")
...
local function can_spawn(spawn_def,spawning_position)
	if spawn_def.type_of_spawning == "water" then
		...
		if not spawning_position then
			minetest.log("warning","[vlc_mobs] no water spawn for mob "..spawn_def.name.." found at "..minetest.pos_to_string(vector.round(spawning_position)))
			return
		end
	end
	if minetest.registered_entities[spawn_def.name].can_spawn and not minetest.registered_entities[spawn_def.name].can_spawn(spawning_position) then
		minetest.log("warning","[vlc_mobs] mob "..spawn_def.name.." refused to spawn at "..minetest.pos_to_string(vector.round(spawning_position)))
		return false
	end
	return true
end
...
if mobs_spawn then
				...
				if spawn_check(spawning_position,spawn_def) then
					...
					if can_spawn(spawn_def,spawning_position) and check_timer(spawn_def, dtime) then
						...
						if spawn_in_group and ( mob_type ~= "monster" or math.random(5) == 1 ) then
							if logging then
								minetest.log("action", "[vlc_mobs] A group of mob " .. spawn_def.name .. " spawns on " ..minetest.get_node(vector.offset(spawning_position,0,-1,0)).name .." at " .. minetest.pos_to_string(spawning_position, 1))
							end
							...
							if logging then
								minetest.log("action", "[vlc_mobs] Mob " .. spawn_def.name .. " spawns on " ..minetest.get_node(vector.offset(spawning_position,0,-1,0)).name .." at ".. minetest.pos_to_string(spawning_position, 1))
							end
							vlc_mobs.spawn(spawning_position, spawn_def.name)
						end
					end
				end
			end
	...
	minetest.register_globalstep(function(dtime)
		...
		if total_mobs > mob_cap.total or total_mobs > #players * mob_cap.player then
			minetest.log("action","[vlc_mobs] global mob cap reached. no cycle spawning.")
			return
...
function mob_class:check_despawn(pos, dtime)
	...
	if remove_far and self:despawn_allowed() then
		if self.despawn_immediately or self.lifetimer <= 0 then
			if logging then
				minetest.log("action", "[vlc_mobs] Mob "..self.name.." despawns at "..minetest.pos_to_string(pos, 1) .. " lifetimer ran out")
			end
			vlc_burning.extinguish(self.object)
			self:safe_remove()
			return true
...
minetest.register_chatcommand("spawn_mob",{
	...
	func = function(n,param)
		...
		local mob = vlc_mobs.spawn(pos, mobname, minetest.serialize({ persist_in_peaceful = true }))
