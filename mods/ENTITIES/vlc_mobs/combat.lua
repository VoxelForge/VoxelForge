				...
				if s.y < p1.y then
					...
					if not minetest.is_protected(s, "") then
						...
						if ndef1 and (ndef1.buildable_to or ndef1.groups.liquid) then
								...
								minetest.set_node(s, {name = vlc_mobs.fallback_node})
						end
					end
...
function mob_class:safe_boom(pos, strength, no_remove)
	...
	vlc_mobs.effect(pos, 32, "vlc_particles_smoke.png", radius * 3, radius * 5, radius, 1, 0)
...
function mob_class:boom(pos, strength, fire, no_remove)
	if mobs_griefing and not minetest.is_protected(pos, "") then
		vlc_explosions.explode(pos, strength, { fire = fire }, self.object)
	else
		vlc_mobs.mob_class.safe_boom(self, pos, strength, no_remove) --need to call it this way bc self can be the "arrow" object here
	end
	if not no_remove then
		if self.is_mob then
			self:safe_remove()
...
function mob_class:on_punch(hitter, tflp, tool_capabilities, dir)
	...
	-- exhaust attacker
	if is_player then
		vlc_hunger.exhaust(hitter:get_player_name(), vlc_hunger.EXHAUST_ATTACK)
	end
	...
	if weapon then
		local fire_aspect_level = vlc_enchanting.get_enchantment(weapon, "fire_aspect")
		if fire_aspect_level > 0 then
			vlc_burning.set_on_fire(self.object, fire_aspect_level * 4)
		end
	end
	...
	if damage >= 0 then
		...
		if damage >= 0.1 then
			...
			-- do damage
			local vlc_reason = {}
			vlc_damage.from_punch(vlc_reason, hitter)
		...
		and tflp >= punch_interval then
			...
			if hitter and is_player then
				...
				kb = kb + 3 * vlc_enchanting.get_enchantment(wielditem, "knockback")
	...
	and not vlc_mobs.invis[ name ] then
		if not die then
...
function mob_class:do_states_attack (dtime)
	...
	if not self.attack
			...
			or (self.attack:is_player() and vlc_mobs.invis[ self.attack:get_player_name() ]) then
	...
	if self.attack_type == "explode" then
		...
		if self.v_start then
			...
			if self.timer > self.explosion_timer then
				...
				if mobs_griefing and not minetest.is_protected(pos, "") then
					vlc_explosions.explode(vlc_util.get_object_center(self.object), self.explosion_strength, {}, self.object)
				else
					...
					vlc_mobs.effect(pos, 32, "vlc_particles_smoke.png", nil, nil, node_break_radius, 1, 0)
				end
				...
				if self.timer > self.dogfight_interval then
					...
					if self:target_visible(self.object:get_pos()) then
						...
						if self.dealt_effect then
							vlc_mobs.effect_functions[self.dealt_effect.name](
