...
function vlc_burning.get_storage(obj)
	return obj:is_player() and vlc_burning.storage[obj] or obj:get_luaentity()
end

function vlc_burning.is_burning(obj)
	local storage = vlc_burning.get_storage(obj)
	if storage then
		return vlc_burning.get_storage(obj).burn_time
	else
		return false
	end
end

function vlc_burning.is_affected_by_rain(obj)
	return vlc_weather.get_weather() == "rain" and vlc_weather.is_outdoor(obj:get_pos())
end

function vlc_burning.get_collisionbox(obj, smaller, storage)
...
function vlc_burning.get_touching_nodes(obj, nodenames, storage)
	local pos = obj:get_pos()
	local minp, maxp = vlc_burning.get_collisionbox(obj, true, storage)
...
--
function vlc_burning.update_hud(player)
	local animation_frames = tonumber(minetest.settings:get("fire_animation_frames")) or 8
	local hud_flame_animated = "vlc_burning_hud_flame_animated.png^[opacity:180^[verticalframe:" .. animation_frames .. ":"

	local storage = vlc_burning.get_storage(player)
...
--
function vlc_burning.set_on_fire(obj, burn_time)
	...
	if obj:is_player() and not enable_damage then
		...
		if armor_list then
			for _, stack in pairs(armor_list) do
				local fire_prot_lvl = vlc_enchanting.get_enchantment(stack, "fire_protection")
	...
	local minp, maxp = vlc_burning.get_collisionbox(obj, false, storage)
	...
	local fire_entity = minetest.add_entity(obj:get_pos(), "vlc_burning:fire")
	...
	if obj:is_player() then
		vlc_burning.update_hud(obj)
	end
end

function vlc_burning.extinguish(obj)
	if not obj:get_pos() then return end
	if vlc_burning.is_burning(obj) then
		local storage = vlc_burning.get_storage(obj)
		if obj:is_player() then
			if storage.fire_hud_id then
				obj:hud_remove(storage.fire_hud_id)
			end
			vlc_burning.storage[obj] = {}
		else
...
function vlc_burning.tick(obj, dtime, storage)
	if storage.burn_time then
		storage.burn_time = storage.burn_time - dtime

		if storage.burn_time <= 0 or vlc_burning.is_affected_by_rain(obj) or #vlc_burning.get_touching_nodes(obj, "group:puts_out_fire", storage) > 0 then
			vlc_burning.extinguish(obj)
			return true
		else
			...
			if storage.fire_damage_timer >= 1 then
				...
				if not luaentity or not luaentity.fire_damage_resistant then
					local vlc_reason =  { type = "on_fire" }
					vlc_damage.finish_reason(vlc_reason)
					vlc_util.deal_damage(obj, 1, vlc_reason)
				end
			end
		end
	end
