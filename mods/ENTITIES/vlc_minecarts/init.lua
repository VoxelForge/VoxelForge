...
vlc_minecarts = {}
vlc_minecarts.modpath = minetest.get_modpath(modname)
vlc_minecarts.speed_max = 10
vlc_minecarts.check_float_time = 15

dofile(vlc_minecarts.modpath.."/functions.lua")
dofile(vlc_minecarts.modpath.."/rails.lua")
...
local function detach_driver(self)
	...
	self._start_pos = nil
	if player then
		vlc_player.players[player].attached = nil
		player:set_detach()
		player:set_eye_offset({x=0, y=0, z=0},{x=0, y=0, z=0})
		vlc_player.player_set_animation(player, "stand" , 30)
	end
end
...
local function hopper_take_item(self, dtime)
	...
	if not self or self.name ~= "vlc_minecarts:hopper_minecart" then return end
	...
	for k, v in pairs(minetest.get_objects_inside_radius(above_pos, 1.25)) do
		...
		if ent and not ent._removed and ent.itemstring and ent.itemstring ~= "" then
			local inv = vlc_entity_invs.load_inv(self, 5)
...
local function register_entity(entity_id, mesh, textures, drop, on_rightclick, on_activate_by_rail)
	...
	function cart:on_activate(staticdata, dtime_s)
		...
		if self.on_activate_by_rail then
			...
			if node.name == "vlc_minecarts:activator_rail_on" then
	...
	function cart:on_punch(puncher, time_from_last_punch, tool_capabilities, direction)
		...
		if not puncher or not puncher:is_player() then
			local cart_dir = vlc_minecarts:get_rail_direction(pos, {x=1, y=0, z=0}, nil, nil, self._railtype)
			if vector.equals(cart_dir, {x=0, y=0, z=0}) then
				return
			end
			vlc_minecarts:set_velocity(self, cart_dir)
			return
		end
		...
		if puncher:get_player_control().sneak and not self._boomtimer then
			...
			if node.name == "vlc_minecarts:detector_rail_on" then
				local newnode = {name="vlc_minecarts:detector_rail", param2 = node.param2}
		...
		local punch_dir = vlc_minecarts:velocity_to_dir(puncher:get_look_dir())
		punch_dir.y = 0
		local cart_dir = vlc_minecarts:get_rail_direction(pos, punch_dir, nil, nil, self._railtype)
		if vector.equals(cart_dir, {x=0, y=0, z=0}) then
			return
		end
		...
		vlc_minecarts:set_velocity(self, cart_dir, f)
	end
	...
	function cart:on_step(dtime)
		...
		for _, node_pos in pairs({{r, 0}, {0, r}, {-r, 0}, {0, -r}}) do
			if minetest.get_node(vector.offset(pos, node_pos[1], 0, node_pos[2])).name == "vlc_core:cactus" then
				detach_driver(self)
				for d = 1, #drop do
		...
		if math.random(1,20) > 15 and not self._passenger then
			if self.name == "vlc_minecarts:minecart" then
		...
		if self._last_float_check >= vlc_minecarts.check_float_time then
			...
			if g ~= self._railtype and self._railtype then
				...
				if self._boomtimer then
					vlc_explosions.explode(pos, 4, {}, self.object)
					self.object:remove()
					return
				end
		...
		if self._old_pos and not self._punched then
			...
			if node.name == "vlc_minecarts:detector_rail" then
				local newnode = {name="vlc_minecarts:detector_rail_on", param2 = node.param2}
				minetest.swap_node(rou_pos, newnode)
				mesecon.receptor_on(rou_pos)
			end
			if node.name == "vlc_minecarts:golden_rail_on" then
				restart_pos = rou_pos
			end
			if node_old.name == "vlc_minecarts:detector_rail_on" then
				local newnode = {name="vlc_minecarts:detector_rail", param2 = node_old.param2}
				minetest.swap_node(rou_old, newnode)
				mesecon.receptor_off(rou_old)
			end
			-- Activate minecart if on activator rail
			if node_old.name == "vlc_minecarts:activator_rail_on" and self.on_activate_by_rail then
				self:on_activate_by_rail()
			end
		end
		...
		if self._old_pos then
			...
			for _,v in ipairs({"x","y","z"}) do
				if math.abs(diff[v]) > 1.1 then
					...
					dir, last_switch = vlc_minecarts:get_rail_direction(pos, self._old_dir, ctrl, self._old_switch, self._railtype)
		...
		local cart_dir = vlc_minecarts:velocity_to_dir(vel)
		local max_vel = vlc_minecarts.speed_max
		if not dir then
			dir, last_switch = vlc_minecarts:get_rail_direction(pos, cart_dir, ctrl, self._old_switch, self._railtype)
		end
		...
		for _,v in ipairs({"x","y","z"}) do
			if math.abs(vel[v]) > max_vel then
				vel[v] = vlc_minecarts:get_sign(vel[v]) * max_vel
				new_acc[v] = 0
				update.vel = true
			end
		end
		...
		if self._driver and (vector.distance(self._start_pos, pos) >= 1000) then
			awards.unlock(self._driver, "vlc:onARail")
		end
		...
		-- stopped on "vlc_minecarts:golden_rail_on"
		if vector.equals(vel, {x=0, y=0, z=0}) and restart_pos then
			local dir = vlc_minecarts:get_start_direction(restart_pos)
			if dir then
				vlc_minecarts:set_velocity(self, dir)
			end
		end
	end
...
function vlc_minecarts.place_minecart(itemstack, pointed_thing, placer)
	if pointed_thing.type ~= "node" then
		return
	end

	local railpos, node
	if vlc_minecarts:is_rail(pointed_thing.under) then
		railpos = pointed_thing.under
		node = minetest.get_node(pointed_thing.under)
	elseif vlc_minecarts:is_rail(pointed_thing.above) then
	...
	local cart_dir
	if node.name == "vlc_minecarts:golden_rail_on" then
		cart_dir = vlc_minecarts:get_start_direction(railpos)
	end
	if cart_dir then
		vlc_minecarts:set_velocity(le, cart_dir)
	else
		cart_dir = vlc_minecarts:get_rail_direction(railpos, {x=1, y=0, z=0}, nil, nil, railtype)
	end
...
local function register_craftitem(itemstring, entity_id, description, tt_help, longdesc, usagehelp, icon, creative)
	...
	local def = {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			return vlc_minecarts.place_minecart(itemstack, pointed_thing, placer)
		end,
		_on_dispense = function(stack, pos, droppos, dropnode, dropdir)
			...
			if minetest.get_item_group(dropnode.name, "rail") ~= 0 then
				...
				placed = vlc_minecarts.place_minecart(stack, pointed_thing)
			end
			if placed == nil then
				-- Drop item
...
register_minecart(
	...
	"vlc_minecarts_minecart.b3d",
	{"vlc_minecarts_minecart.png"},
	"vlc_minecarts_minecart_normal.png",
	{"vlc_minecarts:minecart"},
	function(self, clicker)
		...
		elseif not self._driver then
			...
			vlc_player.players[clicker].attached = true
			clicker:set_attach(self.object, "", {x=0, y=-1.75, z=-2}, {x=0, y=0, z=0})
			minetest.after(0.2, function(name)
				local player = minetest.get_player_by_name(name)
				if player then
					vlc_player.player_set_animation(player, "sit" , 30)
					player:set_eye_offset({x=0, y=-5.5, z=0},{x=0, y=-4, z=0})
					vlc_title.set(clicker, "actionbar", {text=S("Sneak to dismount"), color="white", stay=60})
				end
			end, name)
		end
...
-- Minecart with Chest
register_minecart(
	"vlc_minecarts:chest_minecart",
	"vlc_minecarts:chest_minecart",
	S("Minecart with Chest"),
	nil, nil, nil,
	"vlc_minecarts_minecart_chest.b3d",
	{ "vlc_chests_normal.png", "vlc_minecarts_minecart.png" },
	"vlc_minecarts_minecart_chest.png",
	{"vlc_minecarts:minecart", "vlc_chests:chest"},
	nil, nil, true)
vlc_entity_invs.register_inv("vlc_minecarts:chest_minecart","Minecart",27,false,true)

-- Minecart with Furnace
register_minecart(
	"vlc_minecarts:furnace_minecart",
	"vlc_minecarts:furnace_minecart",
	...
	"vlc_minecarts_minecart_block.b3d",
	{
		"default_furnace_top.png",
		"default_furnace_top.png",
		"default_furnace_front.png",
	...
	"vlc_minecarts_minecart_furnace.png",
	{"vlc_minecarts:minecart", "vlc_furnaces:furnace"},
...
register_minecart(
	"vlc_minecarts:command_block_minecart",
	"vlc_minecarts:command_block_minecart",
	...
	"vlc_minecarts_minecart_command_block.png",
	{"vlc_minecarts:minecart"},
	nil, nil, false
)
...
register_minecart(
	"vlc_minecarts:hopper_minecart",
	"vlc_minecarts:hopper_minecart",
	S("Minecart with Hopper"),
	nil, nil, nil,
	"vlc_minecarts_minecart_hopper.b3d",
	{
		"vlc_hoppers_hopper_inside.png",
		"vlc_minecarts_minecart.png",
		"vlc_hoppers_hopper_outside.png",
		"vlc_hoppers_hopper_top.png",
	},
	"vlc_minecarts_minecart_hopper.png",
	{"vlc_minecarts:minecart", "vlc_hoppers:hopper"},
	nil, nil, true
)
vlc_entity_invs.register_inv("vlc_minecarts:hopper_minecart", "Hopper Minecart", 5, false, true)

-- Minecart with TNT
register_minecart(
	"vlc_minecarts:tnt_minecart",
	"vlc_minecarts:tnt_minecart",
	S("Minecart with TNT"),
	...
	"vlc_minecarts_minecart_tnt.png",
	{"vlc_minecarts:minecart", "vlc_tnt:tnt"},
	-- Ingite
	function(self, clicker)
		if not clicker or not clicker:is_player() then
			return
...
minetest.register_craft({
	output = "vlc_minecarts:tnt_minecart",
	recipe = {
		{"vlc_tnt:tnt"},
		{"vlc_minecarts:minecart"},
	},
...
minetest.register_craft({
	output = "vlc_minecarts:furnace_minecart",
	recipe = {
		{"vlc_furnaces:furnace"},
		{"vlc_minecarts:minecart"},
	},
})

minetest.register_craft({
	output = "vlc_minecarts:hopper_minecart",
	recipe = {
		{"vlc_hoppers:hopper"},
		{"vlc_minecarts:minecart"},
	},
...
minetest.register_craft({
	output = "vlc_minecarts:chest_minecart",
	recipe = {
		{"vlc_chests:chest"},
		{"vlc_minecarts:minecart"},
	},
...
if has_vlc_wip then
	vlc_wip.register_wip_item("vlc_minecarts:chest_minecart")
	vlc_wip.register_wip_item("vlc_minecarts:furnace_minecart")
	vlc_wip.register_wip_item("vlc_minecarts:command_block_minecart")
end
