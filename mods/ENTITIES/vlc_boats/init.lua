...
local function is_river_water(p)
	...
	if n == "vlcx_core:river_water_source" or n == "vlcx_core:river_water_flowing" then
		return true
	end
end
...
local function attach_object(self, obj)
	...
	if obj:is_player() then
		...
		vlc_player.players[obj].attached = true
		...
		vlc_title.set(obj, "actionbar", {text=S("Sneak to dismount"), color="white", stay=60})
	else
...
local function detach_object(obj, change_pos)
	...
	if obj:is_player() then
		vlc_player.players[obj].attached = nil
		obj:set_eye_offset({x=0, y=0, z=0},{x=0, y=0, z=0})
		vlc_player.player_set_animation(obj, "stand" , 30)
	else
...
local boat = {
	...
	_itemstring = "vlc_boats:boat", -- Itemstring of the boat item (implies boat type)
...
function boat.on_rightclick(self, clicker)
	if self._passenger or not clicker or clicker:get_attach() or (self.name == "vlc_boats:chest_boat" and self._driver) then
		return
	end
	...
	if type(data) == "table" then
		...
		if #data.textures ~= 2 then
			...
			data.textures = {
				...
				has_chest and "vlc_chests_normal.png" or "blank.png"
			}
		end
...
function boat.on_step(self, dtime, moveresult)
	vlc_burning.tick(self.object, dtime, self)
	-- vlc_burning.tick may remove object immediately
...
minetest.register_entity("vlc_boats:boat", boat)

local cboat = table.copy(boat)
cboat._itemstring = "vlc_boats:chest_boat"
cboat.initial_properties.textures = { "oak_chest_boat.png", "vlc_chests_normal.png" }
...
minetest.register_entity("vlc_boats:chest_boat", cboat)
vlc_entity_invs.register_inv("vlc_boats:chest_boat","Boat",27)

local doc_itemstring_boat
local doc_itemstring_chest_boat

function vlc_boats.register_boat(name,item_def,object_properties,entity_overrides)
	local itemstring = "vlc_boats:boat_"..name
	local id = name.."_boat"
	...
	if not doc_itemstring_boat then
		...
		doc.sub.identifier.register_object("vlc_boats:boat", "craftitems", itemstring)
	else
	...
	if id:find("chest") then
		...
		if not doc_itemstring_chest_boat then
			...
			doc.sub.identifier.register_object("vlc_boats:chest_boat", "craftitems", doc_itemstring_chest_boat)
		else
	...
	minetest.register_craftitem(":"..itemstring, table.merge({
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			local boat_ent = "vlc_boats:boat"
			local chest_tex = "blank.png"
			if itemstring:find("chest") then
				boat_ent = "vlc_boats:chest_boat"
				chest_tex = "vlc_chests_normal.png"
			end
		...
		_on_dispense = function(stack, pos, droppos, dropnode, dropdir)
			...
			if minetest.get_item_group(dropnode.name, "water") ~= 0 or (dropnode.name == "air" and minetest.get_item_group(belownode.name, "water") ~= 0) then
				minetest.add_entity(droppos, "vlc_boats:boat")
			else
	...
	local c = "vlc_trees:wood_"..name
	if itemstring:find("chest") then
		minetest.register_craft({
			output = itemstring,
			recipe = {
				{"vlc_chests:chest"},
				{"vlc_boats:boat_"..name:gsub("_chest","")},
			},
		})
