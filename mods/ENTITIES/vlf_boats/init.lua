...
local function is_river_water(p)
	...
	if n == "vlfx_core:river_water_source" or n == "vlfx_core:river_water_flowing" then
		return true
	end
end
...
local function attach_object(self, obj)
	...
	if obj:is_player() then
		...
		vlf_title.set(obj, "actionbar", {text=S("Sneak to dismount"), color="white", stay=60})
	else
...
local boat = {
	...
	_itemstring = "vlf_boats:boat", -- Itemstring of the boat item (implies boat type)
...
function boat.on_rightclick(self, clicker)
	if self._passenger or not clicker or clicker:get_attach() or (self.name == "vlf_boats:chest_boat" and self._driver) then
		return
	end
...
function boat:on_activate(staticdata)
	...
	if type(data) == "table" then
		...
		if not data.textures then
			local tx = { "vlf_boats_texture_oak_boat.png", "blank.png" }
			if self.name == "vlf_boats:chest_boat" then
				tx = { "vlf_boats_texture_oak_chest_boat.png", "vlf_chests_normal.png" }
			end
			data.textures = tx
		end
		...
		if #data.textures ~= 2 then
			...
			data.textures = {
				...
				has_chest and "vlf_chests_normal.png" or "blank.png"
			}
		end
...
minetest.register_entity("vlf_boats:boat", boat)
...
function vlf_boats.register_boat(name,item_def,object_properties,entity_overrides)
	local itemstring = "vlf_boats:boat_"..name
	local id = name.."_boat"
	...
	if not doc_itemstring_boat then
		...
		doc.sub.identifier.register_object("vlf_boats:boat", "craftitems", itemstring)
	else
	...
	if id:find("chest") then
		...
		if not doc_itemstring_chest_boat then
			...
			doc.sub.identifier.register_object("vlf_boats:chest_boat", "craftitems", doc_itemstring_chest_boat)
		else
	...
	minetest.register_craftitem(":"..itemstring, table.merge({
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			local boat_ent = "vlf_boats:boat"
			local chest_tex = "blank.png"
			if itemstring:find("chest") then
				boat_ent = "vlf_boats:chest_boat"
				chest_tex = "vlf_chests_normal.png"
			end
		...
		_on_dispense = function(stack, pos, droppos, dropnode, dropdir)
			...
			if minetest.get_item_group(dropnode.name, "water") ~= 0 or (dropnode.name == "air" and minetest.get_item_group(belownode.name, "water") ~= 0) then
				minetest.add_entity(droppos, "vlf_boats:boat")
			else
	...
	local c = "vlf_trees:wood_"..name
	if itemstring:find("chest") then
		minetest.register_craft({
			output = itemstring,
			recipe = {
				{"vlf_chests:chest"},
				{"vlf_boats:boat_"..name:gsub("_chest","")},
			},
		})
