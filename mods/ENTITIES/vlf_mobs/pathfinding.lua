...
vlf_mobs.gwp_longhash = longhash
...
local function is_water_source (name)
	return name == "vlf_core:water_source"
		or name == "vlfx_core:river_water_source"
end
...
function mob_class:gwp_safe_fall_distance ()
	...
	if self.attack then
		...
		sacrifice = sacrifice - (3 - vlf_vars.difficulty) * 4
...
vlf_mobs.gwp_nodevalue_to_name = gwp_nodevalue_to_name
vlf_mobs.gwp_name_to_nodevalue = gwp_name_to_nodevalue
...
vlf_mobs.gwp_get_node = gwp_get_node
...
local function get_partial_type (nodedef)
	if nodedef.groups._vlf_partial == 3 then
		return "SLAB"
	elseif nodedef.groups._vlf_partial == 2 then
		return "BLOCKED"
	elseif nodedef.groups._vlf_partial == 1 then
		return "OPEN"
	end
...
vlf_mobs.gwp_basic_node_classes = gwp_basic_node_classes
vlf_mobs.gwp_fixed_ground_height = gwp_fixed_ground_height
...
vlf_mobs.gwp_basic_classify = gwp_basic_classify
...
vlf_mobs.gwp_classify_node_1 = gwp_classify_node_1
...
vlf_mobs.mobs_being_tested = {}
vlf_mobs.players_selecting_mob = {}
...
function vlf_mobs.maybe_test_pathfinding (mob, clicker)
	local name = clicker:get_player_name ()
	if vlf_mobs.players_selecting_mob[name] then
		vlf_mobs.players_selecting_mob[name] = mob.object
...
local cdef = {
	...
	func = function (playername, param)
		...
		local mobs = vlf_mobs.mobs_being_tested
		...
		if param == "cancel" then
			...
			if vlf_mobs.players_selecting_mob[playername] then
				vlf_mobs.players_selecting_mob[playername] = nil
			end
		...
		elseif param == "choose" then
			...
			vlf_mobs.players_selecting_mob[playername] = true
		elseif param == "start" then
			local mob = vlf_mobs.players_selecting_mob[playername]
...
local function print_node_classification (itemstack, user, pointed_thing)
	...
	elseif pointed_thing.type == "object" then
		...
		if entity and entity.is_mob then
			vlf_mobs.players_selecting_mob[playername] = mob
...
minetest.register_tool ("vlf_mobs:pathfinder_stick", {
...
minetest.register_tool ("vlf_mobs:pathfinder_liquid_stick", {
...
minetest.register_tool ("vlf_mobs:pathfinder_edge_stick", {
...
vlf_mobs.last_dbg_entity = nil

minetest.register_tool ("vlf_mobs:pathfinder_dump_stick", {
	...
	on_use = function (itemstack, user, pointed_thing)
		...
		vlf_mobs.last_dbg_entity = pointed_thing.ref:get_luaentity ()
	end,
})
	...
	for _, door in pairs (self.doors_to_close) do
		...
		elseif not is_door_in_waypoints (self, door)
			...
			if minetest.get_item_group (node.name, "door") ~= 0 then
				if vlf_doors.is_open (door) then
...
function mob_class:gwp_open_and_memorize_door (door, dtime)
	...
	if minetest.get_item_group (node.name, "door") ~= 0
		...
		local door_node = vlf_util.get_nodepos (door)
		if not vlf_doors.is_open (door_node) then
