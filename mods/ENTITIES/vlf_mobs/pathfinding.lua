...
local LOGGING_ON = minetest.settings:get_bool("vlf_logging_mobs_pathfinding",false)
...
function output_table (wp)
	...
	vlf_log("wp items: ".. tostring(#wp))
	for a,b in pairs(wp) do
		vlf_log(a.. ": ".. tostring(b))
	end
end

function append_paths (wp1, wp2)
	--vlf_log("Start append")
	if not wp1 or not wp2 then
		vlf_log("Cannot append wp's")
		return
	end
	output_table(wp1)
	output_table(wp2)
...
local function output_enriched (wp_out)
	vlf_log("Output enriched path")
	local i = 0
	for _,outy in pairs (wp_out) do
		i = i + 1
		vlf_log("Pos ".. i ..":" .. minetest.pos_to_string(outy["pos"]))

		local action =  outy["action"]
		if action then
			--vlf_log("Pos ".. i ..":" .. minetest.pos_to_string(outy["pos"]))
			vlf_log("type: " .. action["type"])
			vlf_log("action: " .. action["action"])
			vlf_log("target: " .. minetest.pos_to_string(action["target"]))
		end
		--vlf_log("failed attempts: " .. outy["failed_attempts"])
	end
end
...
function mob_class:ready_to_path(prioritised)
	...
	if self._pf_last_failed and (os.time() - self._pf_last_failed) < (PATHFINDING_FAIL_WAIT + math.random(1, PATHFINDING_FAIL_WAIT)) then
		vlf_log("Not ready to path as last fail is less than threshold: " .. (os.time() - self._pf_last_failed))
		return false
	else
		local time_since_path_start = os.time() - last_pathing_time
		vlf_log("time_since_path_start: " .. tostring(time_since_path_start))
		if prioritised or (time_since_path_start) > PATHING_START_DELAY then
			vlf_log("We are ready to pathfind, no previous fail or we are past threshold")
			return true
		end
	end
end
...
local function calculate_path_through_door (p, cur_door_pos, t)
	if t then
		vlf_log("Plot route through door from pos: " .. minetest.pos_to_string(p) .. ", to target: " .. minetest.pos_to_string(t))
	else
		vlf_log("Plot route through door from pos: " .. minetest.pos_to_string(p))
	end
	...
	if cur_door_pos then
		vlf_log("Found a door near: " .. minetest.pos_to_string(cur_door_pos))
		...
		for _,v in pairs(plane_adjacents) do
			...
			if n.name == "air" then
				vlf_log("We have air space next to door at: " .. minetest.pos_to_string(pos_closest_to_door))

				prospective_wp = minetest.find_path(p, pos_closest_to_door, PATHFINDING_SEARCH_DISTANCE, 1, 4)

				if prospective_wp then
					vlf_log("Found a path to next to door".. minetest.pos_to_string(pos_closest_to_door))
					vlf_log("Opposite is: ".. minetest.pos_to_string(other_side_of_door))

					table.insert(prospective_wp, cur_door_pos)

					if t then
						vlf_log("We have t, lets go from door to target")
						...
						if wp_otherside_door_to_target and #wp_otherside_door_to_target > 0 then
							...
							wp = prospective_wp
							vlf_log("We have a path from outside door to target")
						else
							vlf_log("We cannot path from outside door to target")
						end
					else
						vlf_log("No t, just add other side of door")
					...
					vlf_log("Cannot path to this air block next to door.")
				end
			end
		end
	else
		vlf_log("No door found")
	end

	if wp and not enriched_path then
		vlf_log("Wp but not enriched")
...
function mob_class:gopath(target, callback_arrived, prioritised)
	if self.state == PATHFINDING then vlf_log("Already pathfinding, don't set another until done.") return end
	...
	if not wp then
		vlf_log("### No direct path. Path through door closest to target.")
		local door_near_target = minetest.find_node_near(target, 16, {"group:door"})
		wp = calculate_path_through_door(p, door_near_target, t)

		if not wp then
			vlf_log("### No path though door closest to target. Try door closest to origin.")
			...
			if not wp then
				vlf_log("### Still not wp. Need to path through 2 doors.")
				...
				if path_through_closest_door and #path_through_closest_door > 0 then
					vlf_log("We have path through first door")
					vlf_log("Number of pos in path through door: " .. tostring(#path_through_closest_door))
					...
					if pos_after_door_entry then
						...
						vlf_log("pos_after_door: " .. minetest.pos_to_string(pos_after_door))
						...
						if path_after_door and #path_after_door > 1 then
							vlf_log("We have path after first door")
							table.remove(path_after_door, 1) -- Remove duplicate
							wp = path_through_closest_door
							append_paths (wp, path_after_door)
						else
							vlf_log("Path after door is not good")
						end
					else
						vlf_log("No pos after door")
					end
				else
					vlf_log("Path through closest door empty or null")
				end
			else
				vlf_log("ok, we have a path through 1 door")
			end
		end
	else
		wp = generate_enriched_path(wp)
		vlf_log("We have a direct route")
	end

	if not wp then
		vlf_log("Could not calculate path")
	...
	if wp and #wp > 0 then
		...
		if current_location and current_location["pos"] then
			vlf_log("Removing first co-ord? " .. tostring(current_location["pos"]))
		else
			vlf_log("Nil pos")
		end
...
function mob_class:interact_with_door(action, target)
	if target then
		vlf_log("Door target is: ".. minetest.pos_to_string(target))
		...
		vlf_log("no target. cannot try and open or close door")
	end
	--end
end
...
function mob_class:do_pathfind_action(action)
	if action then
		vlf_log("Action present")
		local type = action["type"]
		local action_val = action["action"]
		local target = action["target"]
		if target then
			vlf_log("Target: ".. minetest.pos_to_string(target))
		end
		if type and type == "door" then
			vlf_log("Type is door")
...
function mob_class:check_gowp(dtime)
	...
	if not p or not self._target then
		vlf_log("p: ".. tostring(p))
		vlf_log("self._target: ".. tostring(self._target))
		return
	end

	-- arrived at location, finish gowp
	local distance_to_targ = vector.distance(p,self._target)
	--vlf_log("Distance to targ: ".. tostring(distance_to_targ))
	if distance_to_targ < 1.8 then
		vlf_log("Arrived at _target")
		self.waypoints = nil
		self._target = nil
		self.current_target = nil
	...
	elseif not self.current_target then
		vlf_log("Not close enough to targ: ".. tostring(distance_to_targ))
	end
	...
	if self.waypoints and #self.waypoints > 0 and ( not self.current_target or not self.current_target["pos"] or distance_to_current_target < 0.9 ) then
		...
		vlf_log("There after " .. failed_attempts .. " failed attempts. current target:".. minetest.pos_to_string(self.current_target["pos"]) .. ". Distance: " ..  distance_to_current_target)
	...
	elseif self.current_target and self.current_target["pos"] then
		...
		if failed_attempts >= PATHFINDING_FAIL_THRESHOLD then
			vlf_log("Failed to reach position (" .. minetest.pos_to_string(self.current_target["pos"]) .. ") too many times. Abandon route. Times tried: " .. failed_attempts)
			self:set_state("stand")
			self.current_target = nil
			self.waypoints = nil
			self._target = nil
		...
		--vlf_log("Not at pos with failed attempts ".. failed_attempts ..": ".. minetest.pos_to_string(p) .. "self.current_target: ".. minetest.pos_to_string(self.current_target["pos"]) .. ". Distance: ".. distance_to_current_target)
		self:go_to_pos(self.current_target["pos"])
		...
		vlf_log("We don't have waypoints or a current target. Let's try to path to target")

		if self.waypoints then
			vlf_log("WP: " .. tostring(self.waypoints))
			vlf_log("WP num: " .. tostring(#self.waypoints))
		else
			vlf_log("No wp set")
		end
		if self.current_target then
			vlf_log("Current target: " .. tostring(self.current_target))
		else
			vlf_log("No current target")
		end

		local final_wp = minetest.find_path(p, self._target, PATHFINDING_SEARCH_DISTANCE, 1, 4)
		if final_wp then
			vlf_log("We can get to target here.")
		--	self.waypoints = final_wp
			self:go_to_pos(self._target)
		else
			-- Abandon route?
			vlf_log("Cannot plot final route to target")
		end
	end
	...
	if self.current_target and self.current_target["pos"] and (self.waypoints and #self.waypoints == 0) then
		...
		vlf_log("Distance to current target: ".. tostring(distance_to_cur_targ))
		vlf_log("Current p: ".. minetest.pos_to_string(updated_p))
		...
		if distance_to_cur_targ > 1.9 then
			vlf_log("not close to current target: ".. minetest.pos_to_string(self.current_target["pos"]))
			self:go_to_pos(self._current_target)
		else
			vlf_log("close to current target: ".. minetest.pos_to_string(self.current_target["pos"]))
			vlf_log("target is: ".. minetest.pos_to_string(self._target))
			self.current_target = nil
		end
		return
	end
