...
function vlf_mobs.register_mob(self,name,def)
...
function vlf_mobs.register_egg(self, mob, desc, background_color, overlay_color, addegg, no_creative)
	if type(self) == "string" then
		no_creative = addegg
...
local oldregarrow = vlf_mobs.register_mob
function vlf_mobs.register_mob(self,name,def)
	if type(self) == "string" then
		def = name
		name = self
...
function vlf_mobs.spawn_specific(name, dimension, type_of_spawning, biomes, min_light, max_light, interval, chance, aoc, min_height, max_height, day_toggle, on_spawn)
