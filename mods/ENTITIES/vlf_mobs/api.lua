	...
	for field, factors in pairs (tmp._physics_factors) do
		...
		for id, factor in pairs (factors) do
			if id ~= "base"
				...
				and not vlf_mobs.persistent_physics_factors[id] then
				factors[id] = nil
			end
		end
	end

	if self._vlf_potions then
		tmp._vlf_potions = self._vlf_potions
		for name_raw, data in pairs(tmp._vlf_potions) do
			data.spawner = nil
			local def = vlf_potions.registered_effects[name_raw:match("^_EF_(.+)$")]
...
function mob_class:mob_activate (staticdata, dtime)
	...
	if vlf_vars.difficulty <= 0 and not self.persist_in_peaceful then
		vlf_burning.extinguish(self.object)
		self.object:remove()
		return false
	end
	...
	if not self._vlf_potions then
		self._vlf_potions = {}
	end
	vlf_potions._load_entity_effects(self)
...
function mob_class:on_step (dtime, moveresult)
	...
	if bbase - feet.y <= 1.0e-2 then
		self.standing_in = vlf_mobs.node_ok (feet, "air").name
		feet.y = feet.y - 1
		local node = vlf_mobs.node_ok (feet, "air")
