...
local function get_armor_texture (obj, stack)
	...
	local t = def._vlf_armor_texture or "blank.png"
...
function mob_class:armor_better_than (stack, current)
	...
	if minetest.get_item_group (curname, "vlf_armor_points")
		< minetest.get_item_group (itemname, "vlf_armor_points") then
		return true
	elseif minetest.get_item_group (curname, "vlf_armor_toughness")
		< minetest.get_item_group (itemname, "vlf_armor_toughness") then
		return true
	else
		-- TODO: the MC Wiki states that Minecraft also
		-- replaces items without "NBT values" with those
		-- which have them.
		local dur_old = vlf_util.calculate_durability (current)
		local dur_new = vlf_util.calculate_durability (stack)
...
function mob_class:wielditem_better_than (stack, current)
	...
	if minetest.get_item_group (itemname, "tool") ~= 0
		...
		local dur_old = vlf_util.calculate_durability (stack)
		local dur_new = vlf_util.calculate_durability (current)
		if dur_old < dur_new then
			return true
		end
		-- Prefer enchanted to non-enchanted items.
...
function mob_class:evaluate_new_item (item)
	...
	if self.wears_armor
		and def._vlf_armor_element
		and minetest.get_item_group (itemname, "armor") > 0 then
		local slot = def._vlf_armor_element
...
function mob_class:try_equip_item (stack, def, itemname)
	if self.wears_armor
		...
		and def._vlf_armor_element then
