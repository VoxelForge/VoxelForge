...
function vlf_mobs.register_mob(name, def)
	...
	if not def.description then
		minetest.log("warning","[vlf_mobs] Mob "..name.." registered without description field. This is needed for proper death messages.")
	end

	vlf_mobs.spawning_mobs[name] = true
	vlf_mobs.registered_mobs[name] = def
...
function vlf_mobs.register_arrow(name, def)
	...
	local init_props = vlf_mobs.arrow_class.initial_properties
	for _,k in pairs(object_properties) do
		if def[k] ~= nil then
			if type(def[k]) == "table" then
				init_props[k] = table.copy(def[k])
...
function vlf_mobs.register_egg(mob, desc, background_color, overlay_color, addegg, no_creative)
	...
	if old_spawn_icons then
		...
		if vlf_util.file_exists(minetest.get_modpath("mobs_mc").."/textures/"..fn) then
			invimg = fn
		end
	end
	if addegg == 1 then
