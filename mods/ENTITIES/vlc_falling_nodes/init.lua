...
local function deal_falling_damage(self, dtime)
	...
	for _, obj in ipairs(minetest.get_objects_inside_radius(pos, 1)) do
		...
		if entity and entity.name == "__builtin:item" then
			obj:remove()
		elseif vlc_util.get_hp(obj) > 0 and not self._hit[obj] then
			self._hit[obj] = true
			...
			if damage >= 1 then
				...
				if inv then
					...
					if minetest.get_item_group(helmet:get_name(), "combat_armor") > 0 then
						damage = damage / 4 * 3
						vlc_util.use_item_durability(helmet, 1)
				...
				vlc_util.deal_damage(obj, damage, {type = dmg_type})
			end
		end
	end
end
...
minetest.register_entity(":__builtin:falling_node", {
	...
	set_node = function(self, node, meta)
		...
		if def and def._vlc_falling_node_alternative then
			node.name = def._vlc_falling_node_alternative
		end
		local glow
		self.node = node
		self.meta = meta or {}
	...
	on_step = function(self, dtime)
		...
		if n2.name == "vlc_portals:portal_end" then
		...
		if bcn and (not bcd or bcd.walkable or
			...
			if bcd and bcd.leveled and
				...
				if minetest.add_node_level(bcp, addlevel) == 0 then
					if minetest.registered_nodes[self.node.name]._vlc_after_falling then
						minetest.registered_nodes[self.node.name]._vlc_after_falling(bcp, get_falling_depth(self))
					end
            ...
            if (nd and nd.buildable_to == true) or minetest.get_item_group(self.node.name, "crush_after_fall") ~= 0 then
				...
				if def then
					...
					if def._vlc_after_falling then
						def._vlc_after_falling(np, get_falling_depth(self))
					end
					if self.meta then
		...
		if vector.equals(vel, {x = 0, y = 0, z = 0}) then
			...
			if minetest.get_item_group(lownode.name, "fence") == 1 or minetest.get_item_group(lownode.name, "wall") == 1 then
				...
				if def then
					if def._vlc_after_falling then
						def._vlc_after_falling(npos3, get_falling_depth(self))
					end
					if def.sounds and def.sounds.place and def.sounds.place.name then
						minetest.sound_play(def.sounds.place, {pos = np}, true)
					end
