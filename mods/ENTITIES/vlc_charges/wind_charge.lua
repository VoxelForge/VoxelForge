...
register_charge("wind_charge", "Wind Charge", {
	hit_player = vlc_mobs.get_arrow_damage_func(0, "fireball"),
	hit_mob = vlc_mobs.get_arrow_damage_func(6, "fireball"),
	hit_node = function(self, pos, node)
		vlc_charges.wind_burst(pos, damage_radius)
...
-- Bell, Chorus flower, and Decorated Pot:
				if node.name == "vlc_bells:bell" then
					vlc_bells.ring_once(pos)
				end
				if node.name == "vlc_end:chorus_flower" then
					minetest.dig_node(pos)
					vlc_charges.chorus_flower_effects(pos, radius)
				end
				if node.name == "vlc_end:chorus_flower_dead" then
					minetest.swap_node(pos, {name = "air"})
					minetest.add_item(pos, {name = "vlc_end:chorus_flower"})
					vlc_charges.chorus_flower_effects(pos, radius)
				end
				if node.name == "vlc_pottery_sherds:pot" then
					minetest.swap_node(pos, {name = "air"})
					minetest.add_item(pos, {name = "vlc_core:brick"})
					minetest.add_item(pos, {name = "vlc_core:brick"})
					minetest.add_item(pos, {name = "vlc_core:brick"})
					minetest.add_item(pos, {name = "vlc_core:brick"})
					vlc_charges.pot_effects(pos, radius)
				end
	end,
