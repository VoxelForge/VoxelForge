...
function vlf_burning.get_storage(obj)
	return obj:is_player() and vlf_burning.storage[obj] or obj:get_luaentity()
end

function vlf_burning.is_burning(obj)
	local storage = vlf_burning.get_storage(obj)
	if storage then
		return vlf_burning.get_storage(obj).burn_time
	else
		return false
	end
end
...
--
function vlf_burning.update_hud(player)
	local animation_frames = tonumber(minetest.settings:get("fire_animation_frames")) or 8
	local hud_flame_animated = "vlf_burning_hud_flame_animated.png^[opacity:180^[verticalframe:" .. animation_frames .. ":"

	local storage = vlf_burning.get_storage(player)
...
--
function vlf_burning.set_on_fire(obj, burn_time)
	...
	if obj:is_player() and not enable_damage then
		...
		if armor_list then
			for _, stack in pairs(armor_list) do
				local fire_prot_lvl = vlf_enchanting.get_enchantment(stack, "fire_protection")
	...
	local minp, maxp = vlf_burning.get_collisionbox(obj, false, storage)
	...
	local fire_entity = minetest.add_entity(obj:get_pos(), "vlf_burning:fire")
	...
	if obj:is_player() then
		vlf_burning.update_hud(obj)
	end
end

function vlf_burning.extinguish(obj)
...
function vlf_burning.tick(obj, dtime, storage)
	if storage.burn_time then
		storage.burn_time = storage.burn_time - dtime

		if storage.burn_time <= 0 or vlf_burning.is_affected_by_rain(obj) or #vlf_burning.get_touching_nodes(obj, "group:puts_out_fire", storage) > 0 then
			...
			if storage.fire_damage_timer >= 1 then
				...
				if not luaentity or not luaentity.fire_damage_resistant then
					local vlf_reason =  { type = "on_fire" }
					vlf_damage.finish_reason(vlf_reason)
					vlf_util.deal_damage(obj, 1, vlf_reason)
				end
			end
		end
	end
