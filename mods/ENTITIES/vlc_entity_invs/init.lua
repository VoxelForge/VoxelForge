...
function vlc_entity_invs.load_inv(ent,size)
...
function vlc_entity_invs.save_inv(ent)
	if ent._inv then
		ent._items = {}
...
local function load_default_formspec (ent, text)
	...
	local formspec = "size[9,8.75]"
			...
			.. vlc_formspec.get_itemslot_bg(spacing,0.5,cols,rows)
			...
			.. vlc_formspec.get_itemslot_bg(0,7.74,9,1)
...
function vlc_entity_invs.show_inv_form(ent,player,text)
	if not ent._inv_id then return end
	if not open_invs[ent] then
		open_invs[ent] = 0
	end
	ent._inv = vlc_entity_invs.load_inv(ent,ent._inv_size)
...
function vlc_entity_invs.register_inv(entity_name,show_name,size,no_on_righclick,no_sneak)
	assert(minetest.registered_entities[entity_name],"vlc_entity_invs.register_inv called with invalid entity: "..tostring(entity_name))
	...
	minetest.registered_entities[entity_name].get_staticdata  = function(self)
		...
		assert(type(d) == "table","vlc_entity_invs currently only works with entities that return a (serialized) table in get_staticdata. "..tostring(self.name).." returned: "..tostring(old_sd))
