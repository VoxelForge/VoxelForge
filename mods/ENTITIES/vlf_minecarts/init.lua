...
vlf_minecarts = {}
vlf_minecarts.modpath = minetest.get_modpath(modname)
vlf_minecarts.speed_max = 10
vlf_minecarts.check_float_time = 15

dofile(vlf_minecarts.modpath.."/functions.lua")
dofile(vlf_minecarts.modpath.."/rails.lua")
...
local function detach_driver(self)
	...
	self._start_pos = nil
	if player then
		vlf_player.players[player].attached = nil
		player:set_detach()
...
local function hopper_take_item(self, dtime)
	...
	if not self or self.name ~= "vlf_minecarts:hopper_minecart" then return end
	...
	for k, v in pairs(minetest.get_objects_inside_radius(above_pos, 1.25)) do
		...
		if ent and not ent._removed and ent.itemstring and ent.itemstring ~= "" then
			local inv = vlf_entity_invs.load_inv(self, 5)
...
local function register_entity(entity_id, mesh, textures, drop, on_rightclick, on_activate_by_rail)
	...
	function cart:on_activate(staticdata, dtime_s)
		...
		if self.on_activate_by_rail then
			...
			if node.name == "vlf_minecarts:activator_rail_on" then
	...
	function cart:on_punch(puncher, time_from_last_punch, tool_capabilities, direction)
		...
		if not puncher or not puncher:is_player() then
			local cart_dir = vlf_minecarts:get_rail_direction(pos, {x=1, y=0, z=0}, nil, nil, self._railtype)
			if vector.equals(cart_dir, {x=0, y=0, z=0}) then
				return
			end
			vlf_minecarts:set_velocity(self, cart_dir)
			return
		end
		...
		local punch_dir = vlf_minecarts:velocity_to_dir(puncher:get_look_dir())
		punch_dir.y = 0
		local cart_dir = vlf_minecarts:get_rail_direction(pos, punch_dir, nil, nil, self._railtype)
		if vector.equals(cart_dir, {x=0, y=0, z=0}) then
			return
		end
		...
		vlf_minecarts:set_velocity(self, cart_dir, f)
	end
	...
	function cart:on_step(dtime)
		...
		for _, node_pos in pairs({{r, 0}, {0, r}, {-r, 0}, {0, -r}}) do
			if minetest.get_node(vector.offset(pos, node_pos[1], 0, node_pos[2])).name == "vlf_core:cactus" then
				detach_driver(self)
				for d = 1, #drop do
		...
		if self._last_float_check >= vlf_minecarts.check_float_time then
			...
			if g ~= self._railtype and self._railtype then
				...
				if self._boomtimer then
					vlf_explosions.explode(pos, 4, {}, self.object)
					self.object:remove()
					return
				end
		...
		if self._old_pos then
			...
			for _,v in ipairs({"x","y","z"}) do
				if math.abs(diff[v]) > 1.1 then
					...
					dir, last_switch = vlf_minecarts:get_rail_direction(pos, self._old_dir, ctrl, self._old_switch, self._railtype)
		...
		local cart_dir = vlf_minecarts:velocity_to_dir(vel)
		local max_vel = vlf_minecarts.speed_max
		if not dir then
			dir, last_switch = vlf_minecarts:get_rail_direction(pos, cart_dir, ctrl, self._old_switch, self._railtype)
		end
		...
		for _,v in ipairs({"x","y","z"}) do
			if math.abs(vel[v]) > max_vel then
				vel[v] = vlf_minecarts:get_sign(vel[v]) * max_vel
				new_acc[v] = 0
				update.vel = true
			end
		end
		...
		-- stopped on "vlf_minecarts:golden_rail_on"
		if vector.equals(vel, {x=0, y=0, z=0}) and restart_pos then
			local dir = vlf_minecarts:get_start_direction(restart_pos)
			if dir then
				vlf_minecarts:set_velocity(self, dir)
			end
		end
	end
...
function vlf_minecarts.place_minecart(itemstack, pointed_thing, placer)
	if pointed_thing.type ~= "node" then
		return
	end

	local railpos, node
	if vlf_minecarts:is_rail(pointed_thing.under) then
		railpos = pointed_thing.under
		node = minetest.get_node(pointed_thing.under)
	elseif vlf_minecarts:is_rail(pointed_thing.above) then
	...
	local cart_dir
	if node.name == "vlf_minecarts:golden_rail_on" then
		cart_dir = vlf_minecarts:get_start_direction(railpos)
	end
	if cart_dir then
		vlf_minecarts:set_velocity(le, cart_dir)
	else
		cart_dir = vlf_minecarts:get_rail_direction(railpos, {x=1, y=0, z=0}, nil, nil, railtype)
	end
	...
	local def = {
		...
		_on_dispense = function(stack, pos, droppos, dropnode, dropdir)
			...
			if minetest.get_item_group(dropnode.name, "rail") ~= 0 then
				...
				placed = vlf_minecarts.place_minecart(stack, pointed_thing)
			end
			if placed == nil then
				-- Drop item
...
register_minecart(
	...
	"vlf_minecarts_minecart.b3d",
	{"vlf_minecarts_minecart.png"},
	"vlf_minecarts_minecart_normal.png",
	{"vlf_minecarts:minecart"},
...
register_minecart(
	...
	nil, nil, nil,
	"vlf_minecarts_minecart_chest.b3d",
	{ "vlf_chests_normal.png", "vlf_minecarts_minecart.png" },
	"vlf_minecarts_minecart_chest.png",
...
register_minecart(
	"vlf_minecarts:furnace_minecart",
	"vlf_minecarts:furnace_minecart",
	...
	"vlf_minecarts_minecart_block.b3d",
	{
		"default_furnace_top.png",
		"default_furnace_top.png",
		"default_furnace_front.png",
	...
	"vlf_minecarts_minecart_furnace.png",
	{"vlf_minecarts:minecart", "vlf_furnaces:furnace"},
...
register_minecart(
	"vlf_minecarts:command_block_minecart",
	"vlf_minecarts:command_block_minecart",
	...
	"vlf_minecarts_minecart_command_block.png",
	{"vlf_minecarts:minecart"},
	nil, nil, false
)
...
register_minecart(
	"vlf_minecarts:hopper_minecart",
	"vlf_minecarts:hopper_minecart",
	S("Minecart with Hopper"),
	nil, nil, nil,
	"vlf_minecarts_minecart_hopper.b3d",
	{
		"vlf_hoppers_hopper_inside.png",
		"vlf_minecarts_minecart.png",
		"vlf_hoppers_hopper_outside.png",
		"vlf_hoppers_hopper_top.png",
	},
	"vlf_minecarts_minecart_hopper.png",
	{"vlf_minecarts:minecart", "vlf_hoppers:hopper"},
	nil, nil, true
)
vlf_entity_invs.register_inv("vlf_minecarts:hopper_minecart", "Hopper Minecart", 5, false, true)

-- Minecart with TNT
register_minecart(
	"vlf_minecarts:tnt_minecart",
	"vlf_minecarts:tnt_minecart",
	S("Minecart with TNT"),
	...
	"vlf_minecarts_minecart_tnt.png",
	{"vlf_minecarts:minecart", "vlf_tnt:tnt"},
	-- Ingite
	function(self, clicker)
		if not clicker or not clicker:is_player() then
			return
...
minetest.register_craft({
	output = "vlf_minecarts:tnt_minecart",
	recipe = {
		{"vlf_tnt:tnt"},
		{"vlf_minecarts:minecart"},
	},
...
minetest.register_craft({
	output = "vlf_minecarts:furnace_minecart",
	recipe = {
...
minetest.register_craft({
	output = "vlf_minecarts:hopper_minecart",
	recipe = {
		{"vlf_hoppers:hopper"},
		{"vlf_minecarts:minecart"},
	},
...
minetest.register_craft({
	output = "vlf_minecarts:chest_minecart",
	recipe = {
		{"vlf_chests:chest"},
		{"vlf_minecarts:minecart"},
	},
...
if has_vlf_wip then
	vlf_wip.register_wip_item("vlf_minecarts:chest_minecart")
	vlf_wip.register_wip_item("vlf_minecarts:furnace_minecart")
	vlf_wip.register_wip_item("vlf_minecarts:command_block_minecart")
end
