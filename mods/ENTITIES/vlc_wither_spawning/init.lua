			...
			if check_schem(p, schem) then
				...
				for _, players in ipairs(objects) do
					if players:is_player() then
						awards.unlock(players:get_player_name(), "vlc:witheringHeights")
					end
				end
			end
		end
...
local wither_head = minetest.registered_nodes["vlc_heads:wither_skeleton"]
local old_on_place = wither_head.on_place
minetest.override_item("vlc_heads:wither_skeleton",{
	on_place = function(itemstack, placer, pointed)
		local n = minetest.get_node(vector.offset(pointed.above,0,-1,0))
		if n and n.name  == "vlc_nether:soul_sand" then
