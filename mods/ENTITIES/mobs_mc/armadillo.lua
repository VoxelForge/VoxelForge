...
function armadillo:on_rightclick (clicker)
	if not clicker:is_player() then
		return
	end
	local item = clicker:get_wielded_item()
	local item_name = item:get_name()
	if item_name == "mcl_sus_nodes:brush" then
		minetest.add_item(self.object:get_pos(), "mcl_mobitems:armadillo_scute")
		if awards and awards.unlock and clicker then
			awards.unlock(clicker:get_player_name(), "mcl:brush_armadillo")
		end
...
function armadillo:do_custom(dtime)
		...
		self.egg_timer = nil
		local pos = self.object:get_pos()
		minetest.add_item(pos, "mcl_mobitems:armadillo_scute")
end

function armadillo:receive_damage(mcl_reason, damage)
...
mcl_mobs.register_mob("mobs_mc:armadillo", armadillo)
...
-- spawn eggs
mcl_mobs.register_egg("mobs_mc:armadillo", S("Armadillo"), "#A56C68", "#663939", 0)
