...
vlf_mobs.register_mob("mobs_mc:copper_golem", {
	...
	deal_damage = function (self, damage, vlf_reason)
		if vlf_reason.type == "lightning_bolt" then
...
function mobs_mc.check_copper_golem_summon(pos, player)
	local checks = {
	...
	for c=1, #checks do
		...
		if node1.name ~= "vlf_copper:copper" then
			ok = false
		end
		...
		if node2.name ~= "vlf_farming:pumpkin_face" then
			ok = false
		end
		...
		if node3.name ~= "vlf_lightning_rods:rod" then
			ok = false
		end
		...
		if ok then
			...
			place.y = place.y + 0.5
			local o = minetest.add_entity(place, "mobs_mc:copper_golem")
			if o then
		...
		for x = minp.x, maxp.x do
			for y = minp.y, maxp.y do
				for z = minp.z, maxp.z do
					...
					mobs_mc.check_copper_golem_summon(pos, player)
				end
			end
		end
	end
