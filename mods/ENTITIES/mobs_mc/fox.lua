...
local followitem = "vlf_farming:sweet_berry"
...
local fox = {
	...
	on_spawn = function(self)
		if minetest.find_node_near(self.object:get_pos(), 4, "vlf_core:snow") ~= nil
		or minetest.find_node_near(self.object:get_pos(), 4, "vlf_core:dirt_with_grass_snow") ~= nil then
			self.object:set_properties({textures={"mobs_mc_artic_fox.png", "blank.png"}})
		end
	end,
	do_custom = function(self)
		...
		for _,object in pairs(minetest.get_objects_inside_radius(self.object:get_pos(), 8)) do
			...
			and self.state ~= "attack" and math.random(1, 2000) == 1 then
				 self.horny = true
			end
...
vlf_mobs.register_mob("mobs_mc:fox", fox)
...
-- spawn eggs
vlf_mobs.register_egg("mobs_mc:fox", S("Fox"), "#FFDDCC", "#FFaa99", 0)
