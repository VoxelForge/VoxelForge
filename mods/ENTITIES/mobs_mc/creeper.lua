...
local creeper_defs = {
	...
	on_attack = function (self)
	    ...
	    for name, val in pairs (vlf_entity_effects.all_effects (self.object)) do
		local level = vlf_entity_effects.get_effect_level (self.object,
							    name)
		vlf_entity_effects.add_lingering_effect (pos, name, val.dur / 2,
						  level, 2.5)
	    end
	end,
}
