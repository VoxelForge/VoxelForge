...
local function player_setting(p,s)
	...
	if check_setting(s) then
		p:get_meta():set_string("vlf_info_show",s)
		player_dbg[name] = tonumber(s)
	end
	if not player_dbg[name] then
		local r = p:get_meta():get_string("vlf_info_show")
...
local fields_keyset = {}
function vlf_info.register_debug_field(name,def)
	table.insert(fields_keyset,name)
	vlf_info.registered_debug_fields[name]=def
end
...
local function get_text(player, bits)
	...
	for _,key in ipairs(fields_keyset) do
		local def = vlf_info.registered_debug_fields[key]
		if def then
...
vlf_info.register_debug_field("Location", {
	...
	func = function(_, pos)
		...
		-- nether
		if (pos.y >= vlf_vars.mg_nether_min) and (pos.y <= vlf_vars.mg_nether_max) then
			local report_y = pos.y - vlf_vars.mg_nether_min
			return string.format("Nether: x:%.1f y:%.1f z:%.1f", pos.x, report_y, pos.z)
		end

		-- end
		if (pos.y >= vlf_vars.mg_end_min) and (pos.y <= vlf_vars.mg_end_max) then
			local report_y = pos.y - vlf_vars.mg_end_min
