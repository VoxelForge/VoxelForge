...
local function get_item_killer_message(obj, messages, reason)
	if messages.item then
		...
		if itemname ~= "" then
			itemname = "[" .. itemname .. "]"
			if vlf_enchanting.is_enchanted(wielded:get_name()) then
				itemname = minetest.colorize(vlf_colors.AQUA, itemname)
			end
			return S(messages.item, vlf_util.get_object_name(obj), vlf_util.get_object_name(reason.source), itemname)
		end
	end
end

local function get_plain_killer_message(obj, messages, reason)
	return messages.killer and S(messages.killer, vlf_util.get_object_name(obj), vlf_util.get_object_name(reason.source))
end
...
vlf_damage.register_on_death(function(obj, reason)
	if not minetest.settings:get_bool("vlf_showDeathMessages", true) then
		return
	end
	...
	if send_to then
		local messages = vlf_death_messages.messages[reason.type] or {}
...
vlf_damage.register_on_damage(function(obj, damage, reason)
	if obj:get_hp() - damage > 0 then
		if reason.source then
			...
			vlf_death_messages.assist[obj] = nil
		end
	end
end)

minetest.register_globalstep(function(dtime)
	for obj, tbl in pairs(vlf_death_messages.assist) do
