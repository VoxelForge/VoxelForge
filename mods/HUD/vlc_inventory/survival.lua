local S = minetest.get_translator("vlc_inventory")
local F = minetest.formspec_escape

vlc_inventory.registered_survival_inventory_tabs = {}

function vlc_inventory.register_survival_inventory_tab(def)
	if #vlc_inventory.registered_survival_inventory_tabs == 7 then
	...
	for _, d in ipairs(vlc_inventory.registered_survival_inventory_tabs) do
	...
	table.insert(vlc_inventory.registered_survival_inventory_tabs, def)
end
...
local function build_page(player, content, inventory, tabname)
	...
	if #vlc_inventory.registered_survival_inventory_tabs ~= 1 then
		for i, d in ipairs(vlc_inventory.registered_survival_inventory_tabs) do
	...
	return table.concat({
		...
		inventory and table.concat({
			--Main inventory
			vlc_formspec.get_itemslot_bg_v4(0.375, 5.575, 9, 3),
			"list[current_player;main;0.375,5.575;9,3;9]",

			--Hotbar
			vlc_formspec.get_itemslot_bg_v4(0.375, 9.525, 9, 1),
...
local main_page_static = table.concat({
	--Armor slots
	vlc_formspec.get_itemslot_bg_v4(0.375, 0.375, 1, 4),
	...
	"image[1.57,0.343;3.62,4.85;vlc_inventory_background9.png;2]",

	--Offhand
	vlc_formspec.get_itemslot_bg_v4(5.375, 4.125, 1, 1),
	"list[current_player;offhand;5.375,4.125;1,1]",

	--Craft grid
	"label[6.61,0.5;" .. F(minetest.colorize(vlc_formspec.label_color, S("Crafting"))) .. "]",

	vlc_formspec.get_itemslot_bg_v4(6.625, 0.875, 2, 2),
	"list[current_player;craft;6.625,0.875;2,2]",

	"image[9.125,1;1,1;crafting_formspec_arrow.png]",

	vlc_formspec.get_itemslot_bg_v4(10.375, 1, 1, 1),
	"list[current_player;craftpreview;10.375,1;1,1;]",

	"image_button[9.125,2.125;1,1;vlc_crafting_table_inv_fill.png;__vlc_crafting_fillgrid;]",
	"tooltip[__vlc_crafting_fillgrid;" .. F(S("Fill Craft Grid")) .. "]",

	--Crafting guide button
	"image_button[6.575,4.075;1.1,1.1;craftguide_book.png;__vlc_craftguide;]",
	"tooltip[__vlc_craftguide;" .. F(S("Recipe book")) .. "]",

	--Help button
	"image_button[7.825,4.075;1.1,1.1;doc_button_icon_lores.png;__vlc_doc;]",
	"tooltip[__vlc_doc;" .. F(S("Help")) .. "]",

	--Skins button
	"image_button[9.075,4.075;1.1,1.1;vlc_skins_button.png;__vlc_skins;]",
	"tooltip[__vlc_skins;" .. F(S("Select player skin")) .. "]",

	--Achievements button
	"image_button[10.325,4.075;1.1,1.1;vlc_achievements_button.png;__vlc_achievements;]",
	"tooltip[__vlc_achievements;" .. F(S("Achievements")) .. "]",
...
vlc_inventory.register_survival_inventory_tab({
	id = "main",
	description = "Main Inventory",
	item_icon = "vlc_crafting_table:crafting_table",
	show_inventory = true,
		...
		for a = 1, 4 do
			if inv:get_stack("armor", a + 1):is_empty() then
				armor_slot_imgs = armor_slot_imgs ..
					"image[0.375," .. (0.375 + (a - 1) * 1.25) .. ";1,1;vlc_inventory_empty_armor_slot_" .. armor_slots[a] .. ".png]"
			end
		end

		if inv:get_stack("offhand", 1):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[5.375,4.125;1,1;vlc_inventory_empty_armor_slot_shield.png]"
		end
		return main_page_static .. armor_slot_imgs .. vlc_player.get_player_formspec_model(player, 1.57, 0.4, 3.62, 4.85, "")
	end,
	handle = function() end,
})
...
function vlc_inventory.build_survival_formspec(player)
	local inv = player:get_inventory()
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname == "" and #vlc_inventory.registered_survival_inventory_tabs ~= 1 and
		...
		for _, d in ipairs(vlc_inventory.registered_survival_inventory_tabs) do
			if fields["tab_" .. d.id] and d.access(player) then
				...
				vlc_inventory.update_inventory(player)
				break
			end
		end
...
minetest.register_on_player_inventory_action(function(player, action, inv, info)
	if action == "move" and info.to_list == "sorter" then
		...
		if trg == "armor" then
			local newstack = vlc_armor.equip(stack, player, true)
