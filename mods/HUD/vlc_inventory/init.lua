...
local function set_inventory(player, armor_change_only)
	if minetest.is_creative_enabled(player:get_player_name()) then
		if armor_change_only then
			...
			vlc_inventory.set_creative_formspec(player, 0, 0, nil, nil, "inv")
		else
			vlc_inventory.set_creative_formspec(player, 0, 1)
		end
		return
	end

	player:set_inventory_formspec(vlc_inventory.build_survival_formspec(player))
end

function vlc_inventory.get_recipe_groups(pinv, craft)
...
function vlc_inventory.to_craft_grid(player, craft)
	return_fields(player, "craft")
	local pinv = player:get_inventory()
	if craft.type == "normal" then
		local recipe = vlc_inventory.get_recipe_groups(pinv, craft)
		if recipe then
...
function vlc_inventory.fill_grid(player)
	local inv = player:get_inventory()
	local itcounts = {}
	local invcounts = {}
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if fields.__vlc_crafting_fillgrid then
		vlc_inventory.fill_grid(player)
	end
end)
...
function vlc_inventory.reset_craft_grid(player)
	local inv = player:get_inventory()
	if inv and inv:get_size("craft") > 4 and not vlc_crafting_table.has_crafting_table(player) then
		return_fields(player, "craft")
...
function vlc_inventory.update_inventory_formspec(player)
	set_inventory(player)
end
...
minetest.register_on_joinplayer(function(player)
	...
	--add hotbar images
	player:hud_set_hotbar_image("vlc_inventory_hotbar.png")
	player:hud_set_hotbar_selected_image("vlc_inventory_hotbar_selected.png")
...
function vlc_inventory.update_inventory(player)
	...
	vlc_meshhand.update_player(player)
end

vlc_player.register_on_visual_change(vlc_inventory.update_inventory_formspec)
