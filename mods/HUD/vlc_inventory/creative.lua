...
local show_nici = minetest.settings:get_bool("vlc_creative_show_nici_tab", false)
...
local function replace_enchanted_books(tbl)
	for k, item in ipairs(tbl) do
		if item:find("vlc_enchanting:book_enchanted") == 1 then
			local _, enchantment, level = item:match("(%a+) ([_%w]+) (%d+)")
			level = level and tonumber(level)
			if enchantment and level then
				tbl[k] = vlc_enchanting.enchant(ItemStack("vlc_enchanting:book_enchanted"), enchantment, level)
			end
		end
	end
end
...
minetest.register_on_mods_loaded(function()
	...
	for ench, def in pairs(vlc_enchanting.enchantments) do
		local str = "vlc_enchanting:book_enchanted " .. ench .. " " .. def.max_level
...
local function set_inv_search(filter, player)
	...
	for ench, def in pairs(vlc_enchanting.enchantments) do
		for i = 1, def.max_level do
			local stack = vlc_enchanting.enchant(ItemStack("vlc_enchanting:book_enchanted"), ench, i)
			if filter_item("vlc_enchanting:book_enchanted", minetest.strip_colors(stack:get_description()), lang, filter) then
				table.insert(creative_list, "vlc_enchanting:book_enchanted " .. ench .. " " .. i)
			end
		end
	end
...
local tab_icon = {
	blocks = "vlc_core:brick_block",
	deco = "vlc_flowers:peony",
	redstone = "mesecons:redstone",
	rail = "vlc_minecarts:golden_rail",
	misc = "vlc_buckets:bucket_lava",
	nix = "vlc_compass:compass",
	food = "vlc_core:apple",
	tools = "vlc_core:axe_iron",
	combat = "vlc_core:sword_gold",
	mobs = "mobs_mc:cow",
	brew = "vlc_potions:dragon_breath",
	matr = "vlc_core:stick",
	inv = "vlc_chests:chest",
	nici = "vlc_core:barrier",
}

-- Get the player configured stack size when taking items from creative inventory
local function get_stack_size(player)
	return player:get_meta():get_int("vlc_inventory:switch_stack")
end

-- Set the player configured stack size when taking items from creative inventory
local function set_stack_size(player, n)
	player:get_meta():set_int("vlc_inventory:switch_stack", n)
end
...
function vlc_inventory.set_creative_formspec(player)
	local playername = player:get_player_name()
	...
	if name == "inv" then
		...
		if inv:get_stack("armor", 2):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[3.5,0.375;1,1;vlc_inventory_empty_armor_slot_helmet.png]"
		end
		if inv:get_stack("armor", 3):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[3.5,2.125;1,1;vlc_inventory_empty_armor_slot_chestplate.png]"
		end
		if inv:get_stack("armor", 4):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[7.25,0.375;1,1;vlc_inventory_empty_armor_slot_leggings.png]"
		end
		if inv:get_stack("armor", 5):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[7.25,2.125;1,1;vlc_inventory_empty_armor_slot_boots.png]"
		end

		if inv:get_stack("offhand", 1):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[2.25,1.25;1,1;vlc_inventory_empty_armor_slot_shield.png]"
		end
		...
		main_list = table.concat({
			vlc_formspec.get_itemslot_bg_v4(0.375, 3.375, 9, 3),
			"list[current_player;main;0.375,3.375;9,3;9]",

			-- Armor
			vlc_formspec.get_itemslot_bg_v4(3.5, 0.375, 1, 1),
			vlc_formspec.get_itemslot_bg_v4(3.5, 2.125, 1, 1),
			vlc_formspec.get_itemslot_bg_v4(7.25, 0.375, 1, 1),
			vlc_formspec.get_itemslot_bg_v4(7.25, 2.125, 1, 1),
			...
			-- Offhand
			vlc_formspec.get_itemslot_bg_v4(2.25, 1.25, 1, 1),
			"list[current_player;offhand;2.25,1.25;1,1]",

			armor_slot_imgs,

			-- Player preview
			"image[4.75,0.33;2.25,2.83;vlc_inventory_background9.png;2]",
			vlc_player.get_player_formspec_model(player, 4.75, 0.45, 2.25, 2.75, ""),

			-- Crafting guide button
			"image_button[11.575,0.825;1.1,1.1;craftguide_book.png;__vlc_craftguide;]",
			"tooltip[__vlc_craftguide;" .. F(S("Recipe book")) .. "]",

			-- Help button
			"image_button[11.575,2.075;1.1,1.1;doc_button_icon_lores.png;__vlc_doc;]",
			"tooltip[__vlc_doc;" .. F(S("Help")) .. "]",

			-- Advancements button
			"image_button[11.575,3.325;1.1,1.1;vlc_achievements_button.png;__vlc_achievements;]",
			--"style_type[image_button;border=;bgimg=;bgimg_pressed=]",
			"tooltip[__vlc_achievements;" .. F(S("Advancements")) .. "]",
			...
			-- Skins button
			"image_button[11.575,5.825;1.1,1.1;vlc_skins_button.png;__vlc_skins;]",
			"tooltip[__vlc_skins;" .. F(S("Select player skin")) .. "]",
		})
		...
		main_list = table.concat({
			vlc_formspec.get_itemslot_bg_v4(0.375, 0.875, 9, 5),
	...
	if name ~= "inv" and filtername[name] then
		caption = "label[0.375,0.375;" .. F(C(vlc_formspec.label_color, filtername[name])) .. "]"
	end
	...
	local formspec = table.concat({
		...
		-- Hotbar
		vlc_formspec.get_itemslot_bg_v4(0.375, 7.375, 9, 1),
		"list[current_player;main;0.375,7.375;9,1;]",

		-- Trash
		vlc_formspec.get_itemslot_bg_v4(11.625, 7.375, 1, 1, nil, "crafting_creative_trash.png"),
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	vlc_inventory.set_creative_formspec(player)
end)
