...
local function player_setting(p,s)
	...
	if check_setting(s) then
		p:get_meta():set_string("vlc_info_show",s)
		player_dbg[name] = tonumber(s)
	end
	if not player_dbg[name] then
		local r = p:get_meta():get_string("vlc_info_show")
...
vlc_info.registered_debug_fields = {}
local fields_keyset = {}
function vlc_info.register_debug_field(name,def)
	table.insert(fields_keyset,name)
	vlc_info.registered_debug_fields[name]=def
end
...
local function get_text(player, bits)
	...
	for _,key in ipairs(fields_keyset) do
		local def = vlc_info.registered_debug_fields[key]
		if def then
...
vlc_info.register_debug_field("Node feet",{
	level = 4,
	func = function(pl,pos)
		return nodeinfo(pos)
	end
})
vlc_info.register_debug_field("Node below",{
	level = 4,
	func = function(pl,pos)
		return nodeinfo(vector.offset(pos,0,-1,0))
	end
})
vlc_info.register_debug_field("Biome",{
	level = 3,
	func = function(pl,pos)
...
vlc_info.register_debug_field("Coords", {
	level = 2,
	func = function(pl, pos)
...
vlc_info.register_debug_field("Location", {
	level = 1,
	func = function(pl, pos)
		-- overworld
		if (pos.y >= vlc_vars.mg_overworld_min) and (pos.y <= vlc_vars.mg_overworld_max) then
			return string.format("Overworld: x:%.1f y:%.1f z:%.1f", pos.x, pos.y, pos.z)
		end

		-- nether
		if (pos.y >= vlc_vars.mg_nether_min) and (pos.y <= vlc_vars.mg_nether_max) then
			local report_y = pos.y - vlc_vars.mg_nether_min
			return string.format("Nether: x:%.1f y:%.1f z:%.1f", pos.x, report_y, pos.z)
		end

		-- end
		if (pos.y >= vlc_vars.mg_end_min) and (pos.y <= vlc_vars.mg_end_max) then
			local report_y = pos.y - vlc_vars.mg_end_min
