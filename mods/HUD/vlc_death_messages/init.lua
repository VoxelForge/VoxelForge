...
local function get_item_killer_message(obj, messages, reason)
	if messages.item then
		local wielded = vlc_util.get_wielded_item(reason.source)
		local itemname = wielded:get_meta():get_string("name")
		if itemname ~= "" then
			itemname = "[" .. itemname .. "]"
			if vlc_enchanting.is_enchanted(wielded:get_name()) then
				itemname = minetest.colorize(vlc_colors.AQUA, itemname)
			end
			return S(messages.item, vlc_util.get_object_name(obj), vlc_util.get_object_name(reason.source), itemname)
		end
	end
end

local function get_plain_killer_message(obj, messages, reason)
	return messages.killer and S(messages.killer, vlc_util.get_object_name(obj), vlc_util.get_object_name(reason.source))
end
...
local function get_assist_message(obj, messages, reason)
	if messages.assist and vlc_death_messages.assist[obj] then
		return S(messages.assist, vlc_util.get_object_name(obj), vlc_death_messages.assist[obj].name)
	end
end

local function get_plain_message(obj, messages, reason)
	if messages.plain then
		return S(messages.plain, vlc_util.get_object_name(obj))
	end
end

local function get_fallback_message(obj, messages, reason)
	return "vlc_death_messages.messages." .. reason.type .. " " .. vlc_util.get_object_name(obj)
end

vlc_damage.register_on_death(function(obj, reason)
	if not minetest.settings:get_bool("vlc_showDeathMessages", true) then
		return
	end
	...
	if send_to then
		local messages = vlc_death_messages.messages[reason.type] or {}
...
vlc_damage.register_on_damage(function(obj, damage, reason)
	if obj:get_hp() - damage > 0 then
		if reason.source then
			vlc_death_messages.assist[obj] = {name = vlc_util.get_object_name(reason.source), timeout = 5}
		else
			vlc_death_messages.assist[obj] = nil
		end
	end
end)

minetest.register_globalstep(function(dtime)
	for obj, tbl in pairs(vlc_death_messages.assist) do
