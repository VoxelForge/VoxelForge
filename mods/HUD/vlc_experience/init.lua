...
function vlc_experience.get_level(player)
	return caches[player].level
end

function vlc_experience.set_level(player, level)
	local cache = caches[player]

	if level ~= cache.level then
		vlc_experience.set_xp(player, math.floor(bar_to_xp(xp_to_bar(vlc_experience.get_xp(player), cache.level), level)))
	end
end

function vlc_experience.get_xp(player)
	return player:get_meta():get_int("xp")
end

function vlc_experience.set_xp(player, xp)
	player:get_meta():set_int("xp", xp)

	vlc_experience.update(player)
end

function vlc_experience.add_xp(player, xp)
	for _, cb in ipairs(vlc_experience.on_add_xp) do
	...
	vlc_experience.set_xp(player, vlc_experience.get_xp(player) + xp)
	...
	if current_time - cache.last_time > 0.01 then
		...
		if old_level == cache.level then
			minetest.sound_play("vlc_experience", {
				to_player = name,
				gain = 0.1,
			...
			minetest.sound_play("vlc_experience_level_up", {
				to_player = name,
				gain = 0.2,
			})
...
function vlc_experience.remove_hud(player)
...
function vlc_experience.setup_hud(player)
	if hud_bars[player] and hud_levels[player] then return end
	vlc_experience.remove_hud(player)
	caches[player] = {
		last_time = get_time(),
	}
...
function vlc_experience.update(player)
	local xp = vlc_experience.get_xp(player)
	local cache = caches[player]
	...
	if not minetest.is_creative_enabled(player:get_player_name()) then
		if not hud_bars[player] then
			vlc_experience.setup_hud(player)
		end

		player:hud_change(hud_bars[player], "text", "(vlc_experience_bar_background.png^[lowpart:"
			.. math.floor(math.floor(xp_to_bar(xp, cache.level) * 18) / 18 * 100)
			.. ":vlc_experience_bar.png)^[resize:40x1456^[transformR270"
		)
...
function vlc_experience.register_on_add_xp(func, priority)
	table.insert(vlc_experience.on_add_xp, {func = func, priority = priority or 0})
end
...
minetest.register_on_dieplayer(function(player)
	if not minetest.settings:get_bool("vlc_keepInventory", false) then
		vlc_experience.throw_xp(player:get_pos(), vlc_experience.get_xp(player))
		vlc_experience.set_xp(player, 0)
	end
end)

minetest.register_on_mods_loaded(function()
	table.sort(vlc_experience.on_add_xp, function(a, b) return a.priority < b.priority end)
end)

vlc_gamemode.register_on_gamemode_change(function(p, old_gm, gm)
	if gm == "survival" then
		 vlc_experience.setup_hud(p)
		 vlc_experience.update(p)
	elseif gm == "creative" then
		 vlc_experience.remove_hud(p)
	end
end)
...
minetest.register_chatcommand("set_xp", {
	...
	func = function(pn,param)
		...
		local rt = false
		if num then
			vlc_experience.set_xp(player, num)
		end
		return rt, "XP for player "..pn..": "..tostring(vlc_experience.get_xp(player))
	end,
})
