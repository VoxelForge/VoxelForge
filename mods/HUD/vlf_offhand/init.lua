...
function vlf_offhand.get_offhand(player)
	return player:get_inventory():get_stack("offhand", 1)
end

function vlf_offhand.set_offhand(player, itemstack)
	return player:get_inventory():set_stack("offhand", 1, itemstack)
end

function vlf_offhand.place(placer, pointed_thing)
	local offhand = vlf_offhand.get_offhand(placer)
	if offhand and minetest.get_item_group(offhand:get_name(), "offhand_placeable") ~= 0 and pointed_thing.above then
		...
		if not new_stack then
			...
			vlf_offhand.set_offhand(placer, new_stack)
		end
		return true
	end
...
minetest.override_item("", {
	on_place = function(itemstack, placer, pointed_thing)
		...
		if not vlf_offhand.place(placer, pointed_thing) and old_hand_op then
...
local function offhand_get_wear(player)
	return vlf_offhand.get_offhand(player):get_wear()
end

local function offhand_get_count(player)
	return vlf_offhand.get_offhand(player):get_count()
end
...
local function remove_hud(player, hud)
	local offhand_hud = vlf_offhand[player].hud[hud]
	if offhand_hud then
		player:hud_remove(offhand_hud)
		vlf_offhand[player].hud[hud] = nil
	end
end
...
local function update_wear_bar(player, itemstack)
	...
	local wear_bar = vlf_offhand[player].hud.wear_bar
	player:hud_change(wear_bar, "text", "vlf_wear_bar.png^[colorize:#" .. rgb_to_hex(color[1], color[2], color[3]))
...
vlf_player.register_globalstep(function(player)
	...
	local offhand_hud = vlf_offhand[player].hud
	...
	if offhand_item ~= "" and item then
		...
		if not offhand_hud.slot then
			offhand_hud.slot = player:hud_add({
				...
				text = "vlf_offhand_slot.png" .. "^[resize:" .. max_offhand_px .. "x" .. max_offhand_px,
				z_index = 0,
			})
		end
		...
		if not offhand_hud.wear_bar_bg and minetest.registered_tools[offhand_item] then
			if offhand_get_wear(player) > 0 then
				local texture = "vlf_wear_bar.png^[colorize:#000000"
	...
	elseif offhand_hud.slot then
		for index, _ in pairs(vlf_offhand[player].hud) do
...
minetest.register_on_player_inventory_action(function(player, action, _, inventory_info)
	...
	if action == "move" and from_offhand or to_offhand then
		vlf_inventory.update_inventory_formspec(player)
	end
end)
