...
function vlc_bossbars.recalculate_colors()
	local sorted = {}
	local colors = vlc_bossbars.colors
	local color_count = #colors
	local frame_count = color_count * 2
	for i, color in ipairs(colors) do
		local idx = i * 2 - 1
		local image = "(vlc_bossbars.png"
			.. "^[transformR270"
			.. "^[verticalframe:" .. frame_count .. ":" .. (idx - 1)
			.. "^(vlc_bossbars_empty.png"
			.. "^[lowpart:%d:vlc_bossbars.png"
			.. "^[transformR270"
			.. "^[verticalframe:" .. frame_count .. ":" .. idx .. "))^[resize:1456x40"
		local _, hex = vlc_util.get_color(color)
		sorted[color] = {
			image = image,
			hex = hex,
		}
	end
	vlc_bossbars.colors_sorted = sorted
end

local function get_color_info(color, percentage)
	local cdef = vlc_bossbars.colors_sorted[color]
	return cdef.hex, string.format(cdef.image, percentage)
end

local last_id = 0

function vlc_bossbars.add_bar(player, def, dynamic, priority)
	local name = player:get_player_name()
	local bars = vlc_bossbars.bars[name]
	...
	if not dynamic then
		...
		bar.id = last_id + 1
		last_id = bar.id
		vlc_bossbars.static[bar.id] = bar
		return bar.id
	end
end

function vlc_bossbars.remove_bar(id)
	vlc_bossbars.static[id].id = nil
	vlc_bossbars.static[id] = nil
end

function vlc_bossbars.update_bar(id, def, priority)
	local old = vlc_bossbars.static[id]
...
function vlc_bossbars.update_boss(object, name, color)
...
minetest.register_on_joinplayer(function(player)
	local name = player:get_player_name()
	vlc_bossbars.huds[name] = {}
	vlc_bossbars.bars[name] = {}
end)

minetest.register_on_leaveplayer(function(player)
	local name = player:get_player_name()
	vlc_bossbars.huds[name] = nil
	for _, bar in pairs(vlc_bossbars.bars[name]) do
		if bar.id then
			vlc_bossbars.static[bar.id] = nil
		end
	end
	vlc_bossbars.bars[name] = nil
end)
...
minetest.register_globalstep(function(dtime)
	for _, player in pairs(minetest.get_connected_players()) do
		local name = player:get_player_name()
		local bars = vlc_bossbars.bars[name]
		local huds = vlc_bossbars.huds[name]
		...
		while #huds > 0 or #bars > 0 do
			...
			if bar and not hud then
				if i < vlc_bossbars.max_bars then
					hud = {
						color = bar.color,
						image = bar.image,
						text = bar.text,
		...
		vlc_bossbars.huds[name] = huds_new
		vlc_bossbars.bars[name] = bars_new
	end
end)

vlc_bossbars.recalculate_colors()
