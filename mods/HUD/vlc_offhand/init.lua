...
function vlc_offhand.get_offhand(player)
	return player:get_inventory():get_stack("offhand", 1)
end

function vlc_offhand.set_offhand(player, itemstack)
	return player:get_inventory():set_stack("offhand", 1, itemstack)
end

function vlc_offhand.place(placer, pointed_thing)
	local offhand = vlc_offhand.get_offhand(placer)
	if offhand and minetest.get_item_group(offhand:get_name(), "offhand_placeable") ~= 0 and pointed_thing.above then
		...
		if not new_stack then
			...
			vlc_offhand.set_offhand(placer, new_stack)
		end
		return true
	end
...
local function offhand_get_wear(player)
	return vlc_offhand.get_offhand(player):get_wear()
end

local function offhand_get_count(player)
	return vlc_offhand.get_offhand(player):get_count()
end
...
local function remove_hud(player, hud)
	local offhand_hud = vlc_offhand[player].hud[hud]
	if offhand_hud then
		player:hud_remove(offhand_hud)
		vlc_offhand[player].hud[hud] = nil
	end
end
...
local function update_wear_bar(player, itemstack)
	...
	local wear_bar = vlc_offhand[player].hud.wear_bar
	player:hud_change(wear_bar, "text", "vlc_wear_bar.png^[colorize:#" .. rgb_to_hex(color[1], color[2], color[3]))
...
minetest.register_globalstep(function(dtime)
	for _, player in pairs(minetest.get_connected_players()) do
		local itemstack = vlc_offhand.get_offhand(player)
		local offhand_item = itemstack:get_name()
		local offhand_hud = vlc_offhand[player].hud
		...
		if offhand_item ~= "" and item then
			...
			if not offhand_hud.slot then
				offhand_hud.slot = player:hud_add({
					...
					text = "vlc_offhand_slot.png" .. "^[resize:" .. max_offhand_px .. "x" .. max_offhand_px,
					z_index = 0,
				})
			end
			...
			if not offhand_hud.wear_bar_bg and minetest.registered_tools[offhand_item] then
				if offhand_get_wear(player) > 0 then
					local texture = "vlc_wear_bar.png^[colorize:#000000"
		...
		elseif offhand_hud.slot then
			for index, _ in pairs(vlc_offhand[player].hud) do
...
minetest.register_on_player_inventory_action(function(player, action, inventory, inventory_info)
	...
	if action == "move" and from_offhand or to_offhand then
		vlc_inventory.update_inventory_formspec(player)
	end
end)
