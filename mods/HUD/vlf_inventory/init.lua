...
local function set_inventory(player, armor_change_only)
	if minetest.is_creative_enabled(player:get_player_name()) then
		if armor_change_only then
			...
			vlf_inventory.set_creative_formspec(player, 0, 0, nil, nil, "inv")
		else
			vlf_inventory.set_creative_formspec(player, 0, 1)
		end
		return
	end

	player:set_inventory_formspec(vlf_inventory.build_survival_formspec(player))
end

function vlf_inventory.get_recipe_groups(pinv, craft)
...
function vlf_inventory.reset_craft_grid(player)
	local inv = player:get_inventory()
