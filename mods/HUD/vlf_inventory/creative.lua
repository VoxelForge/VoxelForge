...
local show_nici = minetest.settings:get_bool("vlf_creative_show_nici_tab", false)
		...
		if (not def.groups.not_in_creative_inventory or def.groups.not_in_creative_inventory == 0) and def.description and
			...
			if def.groups._vlf_potion == 1 then
				if def.has_potent then
					...
					stack:get_meta():set_int("vlf_potions:potion_potent", potency)
					tt.reload_itemstack_description(stack)
				...
				if def.has_plus then
					...
					stack:get_meta():set_int("vlf_potions:potion_plus", extend)
					tt.reload_itemstack_description(stack)
					table.insert(inventory_lists["brew"], stack:to_string())
				end
			end
...
function vlf_inventory.set_creative_formspec(player)
	...
	if name == "inv" then
		...
		if inv:get_stack("armor", 2):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[3.5,0.375;1,1;vlf_inventory_empty_armor_slot_helmet.png]"
		end
		if inv:get_stack("armor", 3):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[3.5,2.125;1,1;vlf_inventory_empty_armor_slot_chestplate.png]"
		end
		if inv:get_stack("armor", 4):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[7.25,0.375;1,1;vlf_inventory_empty_armor_slot_leggings.png]"
		end
		if inv:get_stack("armor", 5):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[7.25,2.125;1,1;vlf_inventory_empty_armor_slot_boots.png]"
		end

		if inv:get_stack("offhand", 1):is_empty() then
			armor_slot_imgs = armor_slot_imgs .. "image[2.25,1.25;1,1;vlf_inventory_empty_armor_slot_shield.png]"
		end
		...
		main_list = table.concat({
			vlf_formspec.get_itemslot_bg_v4(0.375, 3.375, 9, 3),
			"list[current_player;main;0.375,3.375;9,3;9]",

			-- Armor
			vlf_formspec.get_itemslot_bg_v4(3.5, 0.375, 1, 1),
			vlf_formspec.get_itemslot_bg_v4(3.5, 2.125, 1, 1),
			vlf_formspec.get_itemslot_bg_v4(7.25, 0.375, 1, 1),
			vlf_formspec.get_itemslot_bg_v4(7.25, 2.125, 1, 1),
			...
			-- Offhand
			vlf_formspec.get_itemslot_bg_v4(2.25, 1.25, 1, 1),
			"list[current_player;offhand;2.25,1.25;1,1]",

			armor_slot_imgs,

			-- Player preview
			"image[4.75,0.33;2.25,2.83;vlf_inventory_background9.png;2]",
			vlf_player.get_player_formspec_model(player, 4.75, 0.45, 2.25, 2.75, ""),

			-- Crafting guide button
			"image_button[11.575,0.825;1.1,1.1;craftguide_book.png;__vlf_craftguide;]",
			"tooltip[__vlf_craftguide;" .. F(S("Recipe book")) .. "]",

			-- Help button
			"image_button[11.575,2.075;1.1,1.1;doc_button_icon_lores.png;__vlf_doc;]",
			"tooltip[__vlf_doc;" .. F(S("Help")) .. "]",

			-- Advancements button
			"image_button[11.575,3.325;1.1,1.1;vlf_achievements_button.png;__vlf_achievements;]",
			--"style_type[image_button;border=;bgimg=;bgimg_pressed=]",
			"tooltip[__vlf_achievements;" .. F(S("Advancements")) .. "]",
			...
			-- Skins button
			"image_button[11.575,5.825;1.1,1.1;vlf_skins_button.png;__vlf_skins;]",
	...
	if name ~= "inv" and filtername[name] then
		caption = "label[0.375,0.375;" .. F(C(vlf_formspec.label_color, filtername[name])) .. "]"
	end
	...
	local formspec = table.concat({
		...
		-- Trash
		vlf_formspec.get_itemslot_bg_v4(11.625, 7.375, 1, 1, nil, "crafting_creative_trash.png"),
