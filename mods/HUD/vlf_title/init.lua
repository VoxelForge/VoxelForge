...
vlf_title = {}
vlf_title.defaults = { fadein = 10, stay = 70, fadeout = 20 }
vlf_title.layout = {}
vlf_title.layout.title = { position = { x = 0.5, y = 0.5 }, alignment = { x = 0, y = -1.3 }, size = 7 }
vlf_title.layout.subtitle = { position = { x = 0.5, y = 0.5 }, alignment = { x = 0, y = 1.7 }, size = 4 }
vlf_title.layout.actionbar = { position = { x = 0.5, y = 1 }, alignment = { x = 0, y = 0 }, size = 1 }
...
minetest.register_on_joinplayer(function(player)
	...
	player_params[player] = {
		stay = vlf_title.defaults.stay,
		--fadeIn = vlf_title.defaults.fadein,
		--fadeOut = vlf_title.defaults.fadeout,
	}
	local _, hex_color = vlf_util.get_color("white")
	huds_idx.title[player] = player:hud_add({
		[hud_elem_type_field] = "text",
		position      = vlf_title.layout.title.position,
		alignment     = vlf_title.layout.title.alignment,
		text          = "",
		style         = no_style,
		size          = { x = vlf_title.layout.title.size },
		number        = hex_color,
		z_index       = 100,
	})
	huds_idx.subtitle[player] = player:hud_add({
		[hud_elem_type_field] = "text",
		position      = vlf_title.layout.subtitle.position,
		alignment     = vlf_title.layout.subtitle.alignment,
		text          = "",
		style         = no_style,
		size          = { x = vlf_title.layout.subtitle.size },
		number        = hex_color,
		z_index       = 100,
	})
	huds_idx.actionbar[player] = player:hud_add({
		[hud_elem_type_field] = "text",
		position      = vlf_title.layout.actionbar.position,
		offset        = { x = 0, y = -210 },
		alignment     = vlf_title.layout.actionbar.alignment,
		style         = no_style,
		text          = "",
		size          = { x = vlf_title.layout.actionbar.size },
		number        = hex_color,
		z_index       = 100,
	})
end)
...
function vlf_title.params_set(player, data)
	player_params[player] = {
		stay = data.stay or vlf_title.defaults.stay,
		--fadeIn = data.fadeIn or vlf_title.defaults.fadein,
		--fadeOut = data.fadeOut or vlf_title.defaults.fadeout,
	}
end

function vlf_title.params_get(player)
	return player_params[player]
end

--API FUNCTIONS
function vlf_title.set(player, type, data)
	if not data.color then
		data.color = "white"
	end
	local _, hex_color = vlf_util.get_color(data.color)
	...
	hud_hide_timeouts[type][player:get_player_name()] = gametick_to_secondes(data.stay) or
		gametick_to_secondes(vlf_title.params_get(player).stay)

	return true
end

function vlf_title.remove(player, type)
	if player then
...
function vlf_title.clear(player)
	vlf_title.remove(player, "title")
	vlf_title.remove(player, "subtitle")
	vlf_title.remove(player, "actionbar")
end
	...
	for element, content in pairs(hud_hide_timeouts) do
		for name, timeout in pairs(content) do
			...
			if timeout <= 0 then
				...
				vlf_title.remove(player, element)
			else
...
minetest.register_chatcommand("title", {
	...
	func = function(name, param)
		local player = minetest.get_player_by_name(name)
		if player then
			vlf_title.set(player, "title", { text = param, color = "gold", bold = true, italic = true })
			return true, dbg_msg
		else
...
minetest.register_chatcommand("subtitle", {
	...
	func = function(name, param)
		local player = minetest.get_player_by_name(name)
		if player then
			vlf_title.set(player, "subtitle", { text = param, color = "gold" })
			return true, dbg_msg
		else
			return false, dbg_msg
		end
...
minetest.register_chatcommand("actionbar", {
	...
	func = function(name, param)
		local player = minetest.get_player_by_name(name)
		if player then
			vlf_title.set(player, "actionbar", { text = param, color = "gold", bold = true, italic = true })
			return true, dbg_msg
		else
			return false, dbg_msg
		end
...
minetest.register_chatcommand("title_all", {
	...
	func = function(name, param)
		...
		if player then
			vlf_title.params_set(player, { stay = 600 })
			vlf_title.set(player, "title", { text = param, color = "gold" })
			vlf_title.set(player, "subtitle", { text = param, color = "gold" })
			vlf_title.set(player, "actionbar", { text = param, color = "gold" })
			return true, dbg_msg
		else
			return false, dbg_msg
		end
...
minetest.register_chatcommand("title_all_styles", {
	...
	func = function(name, param)
		...
		if player then
			vlf_title.params_set(player, { stay = 600 })
			vlf_title.set(player, "title", { text = param, color = "gold" })
			vlf_title.set(player, "subtitle", { text = param, color = "gold", bold = true })
			vlf_title.set(player, "actionbar", { text = param, color = "gold", italic = true })
			return true, dbg_msg
		else
			return false, dbg_msg
		end
