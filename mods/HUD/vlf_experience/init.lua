...
function vlf_experience.get_level(player)
	return caches[player].level
end

function vlf_experience.set_level(player, level)
	local cache = caches[player]

	if level ~= cache.level then
		vlf_experience.set_xp(player, math.floor(bar_to_xp(xp_to_bar(vlf_experience.get_xp(player), cache.level), level)))
	end
end

function vlf_experience.get_xp(player)
	return player:get_meta():get_int("xp")
end

function vlf_experience.set_xp(player, xp)
	player:get_meta():set_int("xp", xp)

	vlf_experience.update(player)
end

function vlf_experience.add_xp(player, xp)
	for _, cb in ipairs(vlf_experience.on_add_xp) do
	...
	vlf_experience.set_xp(player, vlf_experience.get_xp(player) + xp)
	...
	if current_time - cache.last_time > 0.01 then
		...
		if old_level == cache.level then
			minetest.sound_play("vlf_experience", {
				to_player = name,
				gain = 0.1,
			...
			minetest.sound_play("vlf_experience_level_up", {
				to_player = name,
				gain = 0.2,
			})
...
function vlf_experience.remove_hud(player)
...
function vlf_experience.setup_hud(player)
	if hud_bars[player] and hud_levels[player] then return end
	vlf_experience.remove_hud(player)
	caches[player] = {
		last_time = get_time(),
	}
...
function vlf_experience.update(player)
	local xp = vlf_experience.get_xp(player)
	local cache = caches[player]
	...
	if not minetest.is_creative_enabled(player:get_player_name()) then
		if not hud_bars[player] then
			vlf_experience.setup_hud(player)
		end

		player:hud_change(hud_bars[player], "text", "(vlf_experience_bar_background.png^[lowpart:"
			.. math.floor(math.floor(xp_to_bar(xp, cache.level) * 18) / 18 * 100)
			.. ":vlf_experience_bar.png)^[resize:40x1456^[transformR270"
		)
...
function vlf_experience.register_on_add_xp(func, priority)
	table.insert(vlf_experience.on_add_xp, {func = func, priority = priority or 0})
end
...
minetest.register_on_dieplayer(function(player)
	if not minetest.settings:get_bool("vlf_keepInventory", false) then
		vlf_experience.throw_xp(player:get_pos(), vlf_experience.get_xp(player))
		vlf_experience.set_xp(player, 0)
	end
end)
...
minetest.register_chatcommand("set_xp", {
	...
	func = function(pn,param)
		...
		local rt = false
		if num then
			vlf_experience.set_xp(player, num)
		end
		return rt, "XP for player "..pn..": "..tostring(vlf_experience.get_xp(player))
	end,
})
