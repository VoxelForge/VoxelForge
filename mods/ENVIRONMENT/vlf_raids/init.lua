	...
	l.on_die = function(self, pos, cmi_cause)
		if l._banner then
			...
			if cmi_cause and cmi_cause.type == "punch" and cmi_cause.puncher:is_player() then
				...
				local lv = vlf_effects.get_effect_level(cmi_cause.puncher, "bad_omen")
				if not lv then lv = 0
				else lv = lv.factor end
				lv = math.max(5,lv + 1)
				vlf_effects.give_effect_by_level("bad_omen", cmi_cause.puncher, lv, 6000)
			end
		end
...
vlf_events.register_event("raid",{
	...
	cond_start  = function(self)
		...
		for _,p in pairs(minetest.get_connected_players()) do
			if vlf_effects.has_effect(p,"bad_omen") then
	...
	on_start = function(self)
		...
		self.health_max = 1
		self.health = 0
		local lv = vlf_effects.get_effect_level(minetest.get_player_by_name(self.player), "bad_omen")
	...
	on_complete = function(self)
		...
		vlf_effects.clear_effect(minetest.get_player_by_name(self.player),"bad_omen")
	end,
})
