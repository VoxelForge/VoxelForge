		...
		for p=1, #players do
			...
			if void_deadly then
				...
				if is_immortal or not enable_damage then
					...
					local spawn = vlc_spawn.get_player_spawn_pos(player)
					player:set_pos(spawn)
					vlc_worlds.dimension_change(player, vlc_worlds.pos_to_dimension(spawn))
				...
				elseif enable_damage and not is_immortal then
					...
					if player:get_hp() > 0 then
						vlc_util.deal_damage(player, VOID_DAMAGE, {type = "out_of_world"})
					end
				end
			end
		end
