...
local oban_def = table.copy(minetest.registered_entities["vlc_banners:standing_banner"])
...
minetest.register_entity(":vlc_raids:ominous_banner",oban_def)

function vlc_raids.drop_obanner(pos)
	local it = ItemStack("vlc_banners:banner_item_white")
...
function vlc_raids.promote_to_raidcaptain(c) -- object
	if not c or not c:get_pos() then return end
	local pos = c:get_pos()
	local l = c:get_luaentity()
	l._banner = minetest.add_entity(pos,"vlc_raids:ominous_banner")
	if not l._banner or not l._banner:get_pos() then return end
	l._banner:set_properties({textures = {vlc_banners.make_banner_texture("unicolor_white", oban_layers)}})
	...
	l.on_die = function(self, pos, cmi_cause)
		if l._banner then
			l._banner:remove()
			l._banner = nil
			vlc_raids.drop_obanner(pos)
			if cmi_cause and cmi_cause.type == "punch" and cmi_cause.puncher:is_player() then
				awards.unlock(cmi_cause.puncher:get_player_name(), "vlc:voluntary_exile")
				local lv = vlc_potions.player_get_effect(cmi_cause.puncher, "bad_omen")
				if not lv then lv = 0
				else lv = lv.factor end
				lv = math.max(5,lv + 1)
				vlc_potions.bad_omen_func(cmi_cause.puncher,lv,6000)
			end
		end
...
function vlc_raids.is_raidcaptain_near(pos)
...
function vlc_raids.register_possible_raidcaptain(mob)
	...
	table.insert(minetest.registered_entities[mob].pick_up,"vlc_banners:banner_item_white")
...
vlc_raids.register_possible_raidcaptain("mobs_mc:pillager")
vlc_raids.register_possible_raidcaptain("mobs_mc:vindicator")
vlc_raids.register_possible_raidcaptain("mobs_mc:evoker")

function vlc_raids.spawn_raid(event)
	local pos = event.pos
	local r = 32
	local n = 12
	local i = math.random(1, n)
	local raid_pos = vector.offset(pos,r * math.cos(((i-1)/n) * (2*math.pi)),0,  r * math.sin(((i-1)/n) * (2*math.pi)))
	local sn = minetest.find_nodes_in_area_under_air(vector.offset(raid_pos,-5,-50,-5), vector.offset(raid_pos,5,50,5), {"group:grass_block", "group:grass_block_snow", "group:snow_cover", "group:sand", "vlc_core:ice"})
	vlc_bells.ring_once(pos)
	if sn and #sn > 0 then
		local spawn_pos = sn[math.random(#sn)]
		if spawn_pos then
			minetest.log("action", "[vlc_raids] Raid Spawn Position chosen at " .. minetest.pos_to_string(spawn_pos) .. ".")
			event.health_max = 0
			local w
			...
			if event.stage == 1 then
				...
				vlc_raids.promote_to_raidcaptain(event.mobs[1])
			end
			minetest.log("action", "[vlc_raids] Raid Spawned. Illager Count: " .. #event.mobs .. ".")
			return #event.mobs == 0
		else
			minetest.log("action", "[vlc_raids] Raid Spawn Postion not chosen.")
		end
	elseif not sn then
		minetest.log("action", "[vlc_raids] Raid Spawn Position error, no appropriate site found.")
	end
	return true
end

function vlc_raids.find_villager(pos)
	...
	for _, objects in ipairs(obj) do
		...
		if object then
			...
			elseif object.name == "mobs_mc:villager" then
				--minetest.log("action", "[vlc_raids] Villager Found.")
				return true
			else
				--minetest.log("action", "[vlc_raids] No Villager Found.")
				return false
			end
		end
	end
...
function vlc_raids.find_bed(pos)
	return minetest.find_node_near(pos,32,{"group:bed"})
end

function vlc_raids.find_village(pos)
	local bed = vlc_raids.find_bed(pos)
	if bed and vlc_raids.find_villager(bed) then
		return bed
	end
end
...
vlc_events.register_event("raid",{
	readable_name = "Raid",
	max_stage = 5,
	health = 1,
	health_max = 1,
	...
	cond_start  = function(self)
		...
		for _,p in pairs(minetest.get_connected_players()) do
			if vlc_potions.player_has_effect(p,"bad_omen") then
				local raid_pos = vlc_raids.find_village(p:get_pos())
				if raid_pos then
	...
	on_start = function(self)
		...
		self.health_max = 1
		self.health = 0
		local lv = vlc_potions.player_get_effect(minetest.get_player_by_name(self.player), "bad_omen")
	...
	on_stage_begin = vlc_raids.spawn_raid,
	...
	on_complete = function(self)
		awards.unlock(self.player,"vlc:hero_of_the_village")
		vlc_potions.player_clear_effect(minetest.get_player_by_name(self.player),"bad_omen")
	end,
})
...
minetest.register_chatcommand("raidcap",{
	...
	func = function(pname,param)
		...
		vlc_raids.promote_to_raidcaptain(c)
	end,
})
...
minetest.register_chatcommand("dump_banner_layers",{
	...
	func = function(pname,param)
		...
		vlc_raids.drop_obanner(vector.offset(p:get_pos(),1,1,1))
		for k,v in pairs(minetest.get_objects_inside_radius(p:get_pos(),5)) do
			local l = v:get_luaentity()
			if l and l.name == "vlc_banners:standing_banner" then
