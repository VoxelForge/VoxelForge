...
local psdef= {
	amount = vlc_weather.rain.particles_count,
	time=0,
...
function vlc_weather.has_rain(pos)
	...
	if bd and bd._vlc_biome_type == "hot" then return false end
...
function vlc_weather.rain.sound_handler(player)
...
function vlc_weather.rain.set_sky_box()
	if vlc_weather.state == "rain" then
...
function vlc_weather.rain.add_rain_particles(player)
	vlc_weather.rain.last_rp_count = vlc_weather.rain.particles_count
	local l = false
	for k,v in pairs(textures) do
		psdef.texture=v
		l = l or vlc_weather.add_spawner_player(player,"rain"..k,psdef)
	end
	if l then
...
function vlc_weather.rain.add_player(player)
	if vlc_weather.players[player:get_player_name()] == nil then
		local player_meta = {}
		player_meta.origin_sky = {player:get_sky(true)}
		vlc_weather.players[player:get_player_name()] = player_meta
		update_sound[player:get_player_name()]=true
	end
end
...
function vlc_weather.rain.remove_player(player)
	local player_meta = vlc_weather.players[player:get_player_name()]
	if player_meta and player_meta.origin_sky then
		player:set_clouds({color="#FFF0F0E5"})
		vlc_weather.players[player:get_player_name()] = nil
		update_sound[player:get_player_name()]=true
	end
end
...
function vlc_weather.rain.update_sound(player)
	...
	if player_meta then
		...
		if player_meta.sound_handler then
			if vlc_weather.rain.last_rp_count == 0 then
				minetest.sound_fade(player_meta.sound_handler, -0.5, 0.0)
				player_meta.sound_handler = nil
			end
		elseif vlc_weather.rain.last_rp_count > 0 then
			player_meta.sound_handler = vlc_weather.rain.sound_handler(player)
		end
...
function vlc_weather.rain.remove_sound(player)
	local player_meta = vlc_weather.players[player:get_player_name()]
...
function vlc_weather.rain.clear()
	vlc_weather.rain.raining = false
	vlc_weather.rain.sky_last_update = -1
	vlc_weather.rain.init_done = false
	vlc_weather.rain.set_particles_mode("rain")
	vlc_weather.skycolor.remove_layer("weather-pack-rain-sky")
	for _, player in pairs(minetest.get_connected_players()) do
		vlc_weather.rain.remove_sound(player)
		vlc_weather.rain.remove_player(player)
...
function vlc_weather.rain.make_weather()
	if vlc_weather.rain.init_done == false then
		vlc_weather.rain.raining = true
		vlc_weather.rain.set_sky_box()
		vlc_weather.rain.set_particles_mode(vlc_weather.mode)
		vlc_weather.rain.init_done = true
	end

	for _, player in pairs(minetest.get_connected_players()) do
		local pos=player:get_pos()
		if vlc_weather.is_underwater(player) or not vlc_weather.has_rain(pos) then
			vlc_weather.rain.remove_sound(player)
			vlc_weather.remove_spawners_player(player)
			if vlc_worlds.has_weather(pos) then
				vlc_weather.set_sky_box_clear(player)
			end
		else
			if vlc_weather.has_snow(pos) then
				vlc_weather.rain.remove_sound(player)
				vlc_weather.snow.add_player(player)
				vlc_weather.snow.set_sky_box()
			else
				vlc_weather.rain.add_player(player)
				vlc_weather.rain.add_rain_particles(player)
				vlc_weather.rain.update_sound(player)
				vlc_weather.rain.set_sky_box()
			end
		end
	end
...
function vlc_weather.rain.set_particles_mode(mode)
	if mode == "thunder" then
		psdef.amount=PARTICLES_COUNT_THUNDER
		vlc_weather.rain.particles_count = PARTICLES_COUNT_THUNDER
	else
		psdef.amount=PARTICLES_COUNT_RAIN
		vlc_weather.rain.particles_count = PARTICLES_COUNT_RAIN
	end
end

if vlc_weather.allow_abm then
	-- ABM for extinguish fire
	minetest.register_abm({
		label = "Rain extinguishes fire",
		nodenames = {"vlc_fire:fire"},
		interval = 2.0,
		chance = 2,
		action = function(pos, node, active_object_count, active_object_count_wider)
			-- Fire is extinguished if in rain or one of 4 neighbors is in rain
			if vlc_weather.rain.raining and vlc_weather.rain.extinguish_fire then
				local around = {
					{ x = 0, y = 0, z = 0 },
					{ x = -1, y = 0, z = 0 },
					{ x = 1, y = 0, z = 0 },
				...
				for a=1, #around do
					...
					if vlc_weather.is_outdoor(apos) and vlc_weather.has_rain(apos) then
	...
	minetest.register_abm({
		...
		nodenames = {"vlc_cauldrons:cauldron", "vlc_cauldrons:cauldron_1", "vlc_cauldrons:cauldron_2"},
		interval = 56.0,
		chance = 1,
		action = function(pos, node, active_object_count, active_object_count_wider)
			-- Rain is equivalent to a water bottle
			if vlc_weather.rain.raining and vlc_weather.is_outdoor(pos) and vlc_weather.has_rain(pos) then
				if node.name == "vlc_cauldrons:cauldron" then
					minetest.swap_node(pos, {name="vlc_cauldrons:cauldron_1"})
				elseif node.name == "vlc_cauldrons:cauldron_1" then
					minetest.swap_node(pos, {name="vlc_cauldrons:cauldron_2"})
				elseif node.name == "vlc_cauldrons:cauldron_2" then
					minetest.swap_node(pos, {name="vlc_cauldrons:cauldron_3"})
				elseif node.name == "vlc_cauldrons:cauldron_1r" then
					minetest.swap_node(pos, {name="vlc_cauldrons:cauldron_2r"})
				elseif node.name == "vlc_cauldrons:cauldron_2r" then
					minetest.swap_node(pos, {name="vlc_cauldrons:cauldron_3r"})
				end
			end
		end
	})
	...
	minetest.register_abm({
		...
		nodenames = {"vlc_farming:soil"},
		interval = 22.0,
		chance = 3,
		action = function(pos, node, active_object_count, active_object_count_wider)
			if vlc_weather.rain.raining and vlc_weather.is_outdoor(pos) and vlc_weather.has_rain(pos) then
				if node.name == "vlc_farming:soil" then
					minetest.set_node(pos, {name="vlc_farming:soil_wet"})
				end
			end
		end
	})
...
if vlc_weather.reg_weathers.rain == nil then
	vlc_weather.reg_weathers.rain = {
		clear = vlc_weather.rain.clear,
		light_factor = 0.6,
		-- 10min - 20min
		min_duration = 600,
		max_duration = 1200,
