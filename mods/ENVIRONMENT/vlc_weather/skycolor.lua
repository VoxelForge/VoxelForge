...
function vlc_weather.set_sky_box_clear(player, sky, fog)
...
function vlc_weather.set_sky_color(player, def)
	local pos = player:get_pos()
...
vlc_weather.skycolor = {
	...
	add_layer = function(layer_name, layer_color, instant_update)
		vlc_weather.skycolor.colors[layer_name] = layer_color
		table.insert(vlc_weather.skycolor.layer_names, layer_name)
		vlc_weather.skycolor.force_update = true
	end,

	current_layer_name = function()
		return vlc_weather.skycolor.layer_names[#vlc_weather.skycolor.layer_names]
	end,

	-- Retrieve layer from colors table
	retrieve_layer = function()
		local last_layer = vlc_weather.skycolor.current_layer_name()
		return vlc_weather.skycolor.colors[last_layer]
	end,

	-- Remove layer from colors table
	remove_layer = function(layer_name)
		for k, name in pairs(vlc_weather.skycolor.layer_names) do
			if name == layer_name then
				table.remove(vlc_weather.skycolor.layer_names, k)
				vlc_weather.skycolor.force_update = true
				return
			end
		end
	...
	override_day_night_ratio = function(player, ratio)
		...
		local is_dark = minetest.get_timeofday() > 0.8 or minetest.get_timeofday() < 0.2 or vlc_weather.state ~= "none"
		local pos = player:get_pos()
	...
	update_sky_color = function(players)
		...
		players = vlc_weather.skycolor.utils.get_players(players)
		for _, player in ipairs(players) do
			local pos = player:get_pos()
			local dim = vlc_worlds.pos_to_dimension(pos)
			local has_weather = (vlc_worlds.has_weather(pos) and (vlc_weather.state == "snow" or vlc_weather.state =="rain" or vlc_weather.state == "thunder") and vlc_weather.has_snow(pos)) or ((vlc_weather.state =="rain" or vlc_weather.state == "thunder") and vlc_weather.has_rain(pos))
			...
			if minetest.get_item_group(checkname, "water") ~= 0 then
				...
				if biome then water_color = biome._vlc_waterfogcolor end
				if not biome then water_color = "#3F76E4" end
				if checkname == "vlcx_core:river_water_source" or checkname == "vlcx_core:river_water_flowing" then water_color = "#0084FF" end
			...
			if dim == "overworld" then
				...
				if mg_name ~= "v6" and mg_name ~= "singlenode" then
					...
					if biome then
						...
						biomesky = biome._vlc_skycolor
						biomefog = biome._vlc_fogcolor
					end
				end
				if (vlc_weather.state == "none") then
					-- Clear weather
					vlc_weather.set_sky_box_clear(player,biomesky,biomefog)
					player:set_sun({visible = true, sunrise_visible = true})
					player:set_moon({visible = true})
					player:set_stars({visible = true})
					vlc_weather.skycolor.override_day_night_ratio(player, nil)
				elseif not has_weather then
					local day_color = vlc_weather.skycolor.get_sky_layer_color(0.15)
					local dawn_color = vlc_weather.skycolor.get_sky_layer_color(0.27)
					local night_color = vlc_weather.skycolor.get_sky_layer_color(0.1)
					vlc_weather.set_sky_color(player, {
						type = "regular",
						sky_color = {
							day_sky = day_color,
				...
				elseif has_weather then
					-- Weather skies
					local day_color = vlc_weather.skycolor.get_sky_layer_color(0.5)
					local dawn_color = vlc_weather.skycolor.get_sky_layer_color(0.75)
					local night_color = vlc_weather.skycolor.get_sky_layer_color(0)
					vlc_weather.set_sky_color(player, {
						type = "regular",
						sky_color = {
							day_sky = day_color,
					...
					local lf = vlc_weather.get_current_light_factor()
					if vlc_weather.skycolor.current_layer_name() == "lightning" then
						vlc_weather.skycolor.override_day_night_ratio(player, 1)
					elseif lf then
						...
						vlc_weather.skycolor.override_day_night_ratio(player, light)
					else
						vlc_weather.skycolor.override_day_night_ratio(player, nil)
					end
				end
			elseif dim == "end" then
				...
				if mg_name ~= "v6" and mg_name ~= "singlenode" then
					...
					if biome then
						--minetest.log("action", string.format("Biome found for number: %s in biome: %s", tostring(biome_index), biome_name))
						biomesky = biome._vlc_skycolor
						--biomefog = biome._vlc_fogcolor -- The End biomes seemingly don't use the fog colour, despite having this value according to the wiki. The sky colour is seemingly used for both sky and fog?
					end
				end
				local t = "vlc_playerplus_end_sky.png"
				...
				player:set_moon({visible = false})
				player:set_stars({visible = false})
				vlc_weather.skycolor.override_day_night_ratio(player, 0.5)
			elseif dim == "nether" then
				...
				if mg_name ~= "v6" and mg_name ~= "singlenode" then
					...
					if biome then
						--minetest.log("action", string.format("Biome found for number: %s in biome: %s", tostring(biome_index), biome_name))
						--biomesky = biome._vlc_skycolor -- The Nether biomes seemingly don't use the sky colour, despite having this value according to the wiki. The fog colour is used for both sky and fog.
						biomefog = biome._vlc_fogcolor
					end
				end
				vlc_weather.set_sky_color(player, {
	...
	get_sky_layer_color = function(timeofday)
		if #vlc_weather.skycolor.layer_names == 0 then
			return nil
		end

		-- min timeofday value 0; max timeofday value 1. So sky color gradient range will be between 0 and 1 * vlc_weather.skycolor.max_val.
		local rounded_time = math.floor(timeofday * vlc_weather.skycolor.max_val)
		local color = vlc_weather.skycolor.utils.convert_to_rgb(vlc_weather.skycolor.min_val, vlc_weather.skycolor.max_val, rounded_time, vlc_weather.skycolor.retrieve_layer())
		return color
	end,
	...
	utils = {
		...
		get_current_bg_color = function()
			local players = vlc_weather.skycolor.utils.get_players(nil)
			if players[1] then
...
minetest.register_globalstep(function(dtime)
	if vlc_weather.skycolor.active ~= true or #minetest.get_connected_players() == 0 then
		return
	end

	if vlc_weather.skycolor.force_update then
		vlc_weather.skycolor.update_sky_color()
		vlc_weather.skycolor.force_update = false
		return
	end

	-- regular updates based on iterval
	timer = timer + dtime;
	if timer >= vlc_weather.skycolor.update_interval then
		vlc_weather.skycolor.update_sky_color()
		timer = 0
	end
...
local function initsky(player)
	...
	if (vlc_weather.skycolor.active) then
		vlc_weather.skycolor.force_update = true
	end

	player:set_clouds(vlc_worlds.get_cloud_parameters() or {height=vlc_worlds.layer_to_y(127), speed={x=-2, z=0}, thickness=4, color="#FFF0FEF"})
end

minetest.register_on_joinplayer(initsky)
minetest.register_on_respawnplayer(initsky)

vlc_worlds.register_on_dimension_change(function(player)
