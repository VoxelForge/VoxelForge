...
"vlc_lightning" is free software; you can redistribute it and/or modify
...
-- vlc_lightning strike API

-- See API.md
--[[
	vlc_lightning.register_on_strike(function(pos, pos2, objects)
		-- code
	end)
]]
function vlc_lightning.register_on_strike(func)
	table.insert(vlc_lightning.on_strike_functions, func)
end
...
local function choose_pos(pos)
	if not pos then
		...
		pos.x = math.floor(pos.x - (vlc_lightning.range_h / 2) + rng:next(1, vlc_lightning.range_h))
		pos.y = pos.y + (vlc_lightning.range_v / 2)
		pos.z = math.floor(pos.z - (vlc_lightning.range_h / 2) + rng:next(1, vlc_lightning.range_h))
	end

	local b, pos2 = minetest.line_of_sight(pos, { x = pos.x, y = pos.y - vlc_lightning.range_v, z = pos.z }, 1)
...
function vlc_lightning.strike_func(pos, pos2, objects)
	local particle_pos = vector.offset(pos2, 0, (vlc_lightning.size / 2) + 0.5, 0)
	local particle_size = vlc_lightning.size * 10
	local time = 0.2
	minetest.add_particlespawner({
		...
		-- to make the texture vlc_lightning bolt hit exactly in the middle of the
	...
	for _, obj in pairs(objects) do
		...
		if lua then
			if not lua.on_vlc_lightning_strike or ( lua.on_vlc_lightning_strike and lua.on_vlc_lightning_strike(lua, pos, pos2, objects) ~= true ) then
				vlc_util.deal_damage(obj, 5, { type = "lightning_bolt" })
			end
		else
			vlc_util.deal_damage(obj, 5, { type = "lightning_bolt" })
		end
	...
	for i = 1, #playerlist do
		...
		if ps[name] == nil then
			...
			vlc_weather.skycolor.add_layer("lightning", { { r = 255, g = 255, b = 255 } }, true)
	...
	if minetest.get_item_group(minetest.get_node({ x = pos2.x, y = pos2.y - 1, z = pos2.z }).name, "liquid") < 1 then
		if minetest.get_node(pos2).name == "air" then
			...
			-- Cause a fire
				...
				minetest.set_node(pos2, { name = "vlc_fire:fire" })
			end
		end
	end
end
...
function vlc_lightning.strike(pos)
	local pos2
	...
	if vlc_lightning.on_strike_functions then
		for _, func in pairs(vlc_lightning.on_strike_functions) do
	...
	if do_strike and pos and pos2 then
		vlc_lightning.strike_func(pos,pos2,minetest.get_objects_inside_radius(pos2, 3.5))
	end
end
...
minetest.register_chatcommand("lightning", {
	...
	func = function(name, param)
		...
		if pos then
			vlc_lightning.strike(pos)
		elseif player_to_strike then
			vlc_lightning.strike(player_to_strike:get_pos())
		end
		return true
	end,
})
