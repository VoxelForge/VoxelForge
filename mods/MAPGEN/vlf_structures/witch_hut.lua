...
local function hut_placement_callback(pos,def,_)
	...
	for _,leg in pairs(legs) do
		while minetest.get_item_group(vlf_vars.get_node(vector.offset(leg,0,-1,0)).name, "water") ~= 0 do
			leg = vector.offset(leg,0,-1,0)
			table.insert(tree,leg)
		end
	end
	vlf_util.bulk_swap_node(tree, {name = "vlf_trees:tree_oak", param2 = 2})
	spawn_witch(p1,p2)
end
...
vlf_structures.register_structure("witch_hut",{
	...
	filenames = { modpath.."/schematics/vlf_structures_witch_hut.mts" },
