...
local function temple_placement_callback(pos,def, pr)
	...
	local pplates = minetest.find_nodes_in_area(p1, p2, "vlf_pressureplates:pressure_plate_stone_off")
	...
	for p=1, #pplates do
		if pplates_remove > 0 and pr:next(1, 100) >= 50 then
			...
			-- Initialize plate
			minetest.registered_nodes["vlf_pressureplates:pressure_plate_stone_off"].on_construct(pplates[p])
		end
	end
...
vlf_structures.register_structure("desert_temple",{
	...
	y_min = 1,
	biomes = { "Desert" },
	filenames = { modpath.."/schematics/vlf_structures_desert_temple.mts" },
