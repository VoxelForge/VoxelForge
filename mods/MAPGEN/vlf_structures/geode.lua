...
local function set_node_no_bedrock(pos, node)
	...
	if n.name == "vlf_core:bedrock" then return end
...
local function makegeode(pos, _, pr)
	...
	minetest.emerge_area(p1, p2, function(_, _, calls_remaining)
		...
		local calcite = {}
		local nn = minetest.find_nodes_in_area(p1,p2,{"group:material_stone","group:dirt","vlf_core:gravel"})
		...
		for i=1,pr:next(1, math.max(2, #nn - math.ceil(#nn/5) )) do
			set_node_no_bedrock(nn[i],{name="vlf_amethyst:amethyst_block"})
		end

		for _, v in pairs(minetest.find_nodes_in_area(p1,p2,{"vlf_amethyst:amethyst_block"})) do
			local all_amethyst = true
			for _, vv in pairs(adjacents) do
				local pp = vector.add(v,vv)
				local an = minetest.get_node(pp)
				if an.name ~= "vlf_amethyst:amethyst_block" then
					if minetest.get_item_group(an.name,"material_stone") > 0 then
						set_node_no_bedrock(pp,{name="vlf_amethyst:calcite"})
						table.insert(calcite,pp)
						if pr:next(1,5) == 1 then
							set_node_no_bedrock(v,{name="vlf_amethyst:budding_amethyst_block"})
						end
						all_amethyst = false
					elseif an.name ~= "vlf_amethyst:amethyst_block" and an.name ~= "air" and an.name ~= "vlf_amethyst:budding_amethyst_block" then
						all_amethyst = false
					end
				end
			end
		...
		for _,v in pairs(calcite) do
			for _,vv in pairs(minetest.find_nodes_in_area(vector.offset(v,-1,-1,-1),vector.offset(v,1,1,1),{"group:material_stone"})) do
				set_node_no_bedrock(vv,{name="vlf_blackstone:basalt_smooth"})
			end
		end

		for _, v in pairs(minetest.find_nodes_in_area_under_air(p1,p2,{"vlf_amethyst:amethyst_block","vlf_amethyst:budding_amethyst_block"})) do
			local r = pr:next(1,50)
			if r < 10 then
				set_node_no_bedrock(vector.offset(v,0,1,0),{name="vlf_amethyst:amethyst_cluster",param2=1})
			end
		end
		return true
	end)
...
vlf_structures.register_structure("geode",{
