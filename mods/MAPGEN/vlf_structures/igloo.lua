...
function vlf_structures.generate_igloo_top(pos, pr)
	...
	local path = vlf_structures.schempath.."/schems/vlf_structures_igloo_top.mts"
	local rotation = tostring(pr:next(0,3)*90)
	return vlf_structures.place_schematic(newpos, path, rotation, nil, true, nil, function()
		local p1 = vector.offset(pos,-5,-5,-5)
		local p2 = vector.offset(pos,5,5,5)
		vlf_structures.construct_nodes(p1,p2,{"vlf_furnaces:furnace","vlf_books:bookshelf"})
	end), rotation
end

local function spawn_mobs(p1,p2,vi,zv)
	local mc = minetest.find_nodes_in_area_under_air(p1,p2,{"vlf_core:stonebrickmossy"})
	if #mc == 2 then
		local vp = mc[1]
		local zp = mc[2]
...
function vlf_structures.generate_igloo_basement(pos, orientation, loot, pr)
	-- TODO: Add monster eggs
	local path = vlf_structures.schempath.."/schems/vlf_structures_igloo_basement.mts"
	vlf_structures.place_schematic(pos, path, orientation, nil, true, nil, function()
		local p1 = vector.offset(pos,-5,-5,-5)
		local p2 = vector.offset(pos,5,5,5)
		vlf_structures.fill_chests(p1,p2,loot,pr)
		vlf_structures.construct_nodes(p1,p2,{"vlf_brewing:stand_000","vlf_books:bookshelf"})
		spawn_mobs(p1,p2)
	end, pr)
end

function vlf_structures.generate_igloo(pos, def, pr)
	-- Place igloo
	local success, rotation = vlf_structures.generate_igloo_top(pos, pr)
	-- Place igloo basement with 50% chance
	local r = pr:next(1,2)
	if r == 1 then
		-- Select basement depth
		local dim = vlf_worlds.pos_to_dimension(pos)
		--local buffer = pos.y - (vlf_vars.mg_lava_overworld_max + 10)
		local buffer
		if dim == "nether" then
			buffer = pos.y - (vlf_vars.mg_lava_nether_max + 10)
		elseif dim == "end" then
			buffer = pos.y - (vlf_vars.mg_end_min + 1)
		elseif dim == "overworld" then
			buffer = pos.y - (vlf_vars.mg_lava_overworld_max + 10)
		else
			return success
		end
		if buffer <= 19 then
		...
		local function set_brick(pos)
			...
			if m == 1 then
				if c == 1 then
					brick = "vlf_monster_eggs:monster_egg_stonebrickcracked"
				else
					brick = "vlf_monster_eggs:monster_egg_stonebrick"
				end
			else
				if c == 1 then
					brick = "vlf_core:stonebrickcracked"
				else
					brick = "vlf_core:stonebrick"
				end
			end
		...
		for y=1, real_depth-1 do
			...
			set_brick({x=tpos.x  ,y=tpos.y-y,z=tpos.z+1})
			minetest.set_node({x=tpos.x,y=tpos.y-y,z=tpos.z}, {name="vlf_core:ladder", param2=ladder_param2})
		end
		-- Place basement
		vlf_structures.generate_igloo_basement(bpos, rotation, def.loot, pr)
		-- Place hidden trapdoor
		minetest.after(5, function(tpos, dir)
			minetest.set_node(tpos, {name="vlf_doors:trapdoor", param2=20+minetest.dir_to_facedir(dir)}) -- TODO: more reliable param2
		end, tpos, dir)
	end
	return success
end

vlf_structures.register_structure("igloo",{
	...
	place_func = vlf_structures.generate_igloo,
	loot = {
		...
		},
		{
			...
			items = {
				...
				{ itemstring = "vlf_farming:wheat_item", weight = 10, amount_min = 2, amount_max = 3 },
				{ itemstring = "vlf_core:gold_nugget", weight = 10, amount_min = 1, amount_max = 3 },
				{ itemstring = "vlf_mobitems:rotten_flesh", weight = 10 },
				{ itemstring = "vlf_tools:axe_stone", weight = 2 },
