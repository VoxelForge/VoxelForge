local function vlc_log (message)
	vlc_util.vlc_log (message, "[Village - Foundation]")
end

local foundation_materials = {}

foundation_materials["vlc_core:sand"] = "vlc_core:sandstone"
--"vlc_core:sandstonecarved"
...
function settlements.terraform(settlement_info, pr)
	...
	for i, built_house in ipairs(settlement_info) do
		...
		vlc_log("Surface material: " .. tostring(surface_mat))
		local platform_mat = foundation_materials[surface_mat]
		vlc_log("Foundation material: " .. tostring(platform_mat))
...
--						local node = vlc_vars.get_node(p)
