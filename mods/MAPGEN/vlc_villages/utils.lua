local get_node = vlc_vars.get_node
...
function settlements.fill_chest(pos, pr)
	...
	if meta:get_string("infotext") ~= "Chest" then
		...
		-- minetest.registered_nodes["vlc_chests:chest"].on_construct(pos)
		--
		-- For MineClone2 after commit 09ab1482b5 (the new entity chests)
		minetest.registered_nodes["vlc_chests:chest_small"].on_construct(pos)
	end
	-- fill chest
	...
	local function get_treasures(prand)
		local loottable = {{
			...
			items = {
				...
				{ itemstring = "vlc_tools:pick_iron", weight = 5 },
				{ itemstring = "vlc_tools:sword_iron", weight = 5 },
				{ itemstring = "vlc_armor:chestplate_iron", weight = 5 },
				{ itemstring = "vlc_armor:helmet_iron", weight = 5 },
				{ itemstring = "vlc_armor:leggings_iron", weight = 5 },
				{ itemstring = "vlc_armor:boots_iron", weight = 5 },
				{ itemstring = "vlc_core:obsidian", weight = 5, amount_min = 3, amount_max = 7 },
				{ itemstring = "vlc_core:sapling", weight = 5, amount_min = 3, amount_max = 7 },
		...
		local items = vlc_loot.get_multi_loot(loottable, prand)
		return items
	end
...
function settlements.initialize_furnace(pos)
  ...
  then
    ...
    then
      minetest.registered_nodes["vlc_furnaces:furnace"].on_construct(furnacepos)
    end
  end
end
-------------------------------------------------------------------------------
...
function settlements.initialize_anvil(pos)
  ...
  then
    ...
    then
      minetest.registered_nodes["vlc_anvils:anvil"].on_construct(anvilpos)
    end
  end
end
-------------------------------------------------------------------------------
	...
	for j = 1 , 40, 1 do
		...
		for i = square_start, square_end, 1 do
			...
			if current_hm_entry then
				-- skip buggy heightmaps, return high value. Converted vlc5 maps can be -31007
