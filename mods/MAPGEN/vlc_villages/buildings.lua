...
function settlements.build_schematic(vm, data, va, pos, building, replace_wall, name)
  ...
  local platform_material =  vlc_vars.get_node(pos)
  ...
  if replace_wall == "y" then
    schem_lua = schem_lua:gsub("vlc_core:cobble", material)
  end
  schem_lua = schem_lua:gsub("vlc_core:dirt_with_grass",
    platform_material)

--  Disable special junglewood for now.
 -- special material for spawning npcs
 -- schem_lua = schem_lua:gsub("vlc_core:junglewood",
...
function settlements.create_site_plan(maxp, minp, pr)
	...
	local chunks = {}
	chunks[vlc_vars.get_chunk_number(center)] = true
	...
	for j = 1,20 do
		...
		for j = 0, 360, 15 do
			...
			local chunk_number = vlc_vars.get_chunk_number(pos1)
...
local function construct_node(p1, p2, name)
	...
	if r then
		...
		minetest.log("warning", "[vlc_villages] No on_construct defined for node name " .. name)
		return
	end
	minetest.log("warning", "[vlc_villages] Attempt to 'construct' inexistant nodes: " .. name)
end

local function spawn_iron_golem(pos)
	--minetest.log("action", "Attempt to spawn iron golem.")
	local p = minetest.find_node_near(pos,50,"vlc_core:grass_path")
	if p then
...
local function spawn_villagers(minp,maxp)
	...
	local beds=minetest.find_nodes_in_area(vector.offset(minp,-20,-20,-20),vector.offset(maxp,20,20,20),{"vlc_beds:bed_red_bottom"})
...
local function init_nodes(p1, p2, size, rotation, pr)
	construct_node(p1, p2, "vlc_itemframes:item_frame")
	construct_node(p1, p2, "vlc_furnaces:furnace")
	construct_node(p1, p2, "vlc_anvils:anvil")

	construct_node(p1, p2, "vlc_smoker:smoker")
	construct_node(p1, p2, "vlc_barrels:barrel_closed")
	construct_node(p1, p2, "vlc_blast_furnace:blast_furnace")
	construct_node(p1, p2, "vlc_brewing:stand_000")
	local nodes = construct_node(p1, p2, "vlc_chests:chest")
...
function settlements.place_schematics(settlement_info, pr)
	...
	for i, built_house in ipairs(settlement_info) do
		...
		local schem_lua = minetest.serialize_schematic(building,
			"lua",
			{lua_use_comments = false, lua_num_indent_spaces = 0}).." return schematic"
		schem_lua = schem_lua:gsub("vlc_core:stonebrickcarved", "vlc_villages:stonebrickcarved")
		-- replace material
		if replace_wall then
			...
			-- Example: Attempting to swap out 'vlc_core:stonebrick'; which has multiple, additional sub-variants: (carved, cracked, mossy). Will currently cause issues, so leaving disabled.
			if platform_material == "vlc_core:snow" or platform_material == "vlc_core:dirt_with_grass_snow" or platform_material == "vlc_core:podzol" then
				schem_lua = schem_lua:gsub("vlc_core:tree", "vlc_core:sprucetree")
				schem_lua = schem_lua:gsub("vlc_core:wood", "vlc_core:sprucewood")
				--schem_lua = schem_lua:gsub("vlc_fences:fence", "vlc_fences:spruce_fence")
				--schem_lua = schem_lua:gsub("vlc_stairs:slab_wood_top", "vlc_stairs:slab_sprucewood_top")
				--schem_lua = schem_lua:gsub("vlc_stairs:stair_wood", "vlc_stairs:stair_sprucewood")
			...
			elseif platform_material == "vlc_core:sand" or platform_material == "vlc_core:redsand" then
				schem_lua = schem_lua:gsub("vlc_core:tree", "vlc_core:sandstonecarved")
				schem_lua = schem_lua:gsub("vlc_core:cobble", "vlc_core:sandstone")
				schem_lua = schem_lua:gsub("vlc_core:wood", "vlc_core:sandstonesmooth")
				--schem_lua = schem_lua:gsub("vlc_fences:fence", "vlc_fences:birch_fence")
				--schem_lua = schem_lua:gsub("vlc_stairs:slab_wood_top", "vlc_stairs:slab_birchwood_top")
				--schem_lua = schem_lua:gsub("vlc_stairs:stair_wood", "vlc_stairs:stair_birchwood")
				...
				--schem_lua = schem_lua:gsub("vlc_stairs:stair_stonebrick", "vlc_stairs:stair_redsandstone")
				--schem_lua = schem_lua:gsub("vlc_core:stonebrick", "vlc_core:redsandstonesmooth")
				schem_lua = schem_lua:gsub("vlc_core:brick_block", "vlc_core:redsandstone")
			end
		end
		schem_lua = schem_lua:gsub("vlc_core:dirt_with_grass", platform_material)

		--[[ Disable special junglewood for now.
		-- special material for spawning npcs
		schem_lua = schem_lua:gsub("vlc_core:junglewood", "settlements:junglewood")
		--]]

		schem_lua = schem_lua:gsub("vlc_stairs:stair_wood_outer", "vlc_stairs:slab_wood")
		schem_lua = schem_lua:gsub("vlc_stairs:stair_stone_rough_outer", "air")
		...
		vlc_structures.place_schematic(
			pos,
			schematic,
			rotation,
			nil,
