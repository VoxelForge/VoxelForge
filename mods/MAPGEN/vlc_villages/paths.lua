  ...
  for o,p in pairs(settlement_info) do
    ...
    then
      ...
      while true do
        ...
        -- replace surface node with vlc_core:grass_path
        if surface_point
        then
          if surface_mat == "vlc_core:sand" or surface_mat == "vlc_core:redsand" then
			minetest.swap_node(surface_point,{name="vlc_core:sandstonesmooth2"})
		  else
			minetest.swap_node(surface_point,{name="vlc_core:grass_path"})
		  end
