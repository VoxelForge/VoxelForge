...
local logging = minetest.settings:get_bool("vlf_logging_mapgen",false)
...
minetest.register_on_generated(function(minp, maxp, blockseed)
	...
	vlf_vars.add_chunk(minp)
	if logging then
		minetest.log("action", "[vlf_mapgen_core] Generating chunk " .. minetest.pos_to_string(minp) .. " ... " .. minetest.pos_to_string(maxp).."..."..tostring(roundN(((os.clock() - t1)*1000),2)).."ms")
	end
end)

function minetest.register_on_generated(node_function)
	vlf_mapgen_core.register_generator("mod_"..minetest.get_current_modname().."_"..tostring(#registered_generators+1), nil, node_function)
end

function vlf_mapgen_core.register_generator(id, lvm_function, node_function, priority, needs_param2)
...
function vlf_mapgen_core.unregister_generator(id)
	local index
