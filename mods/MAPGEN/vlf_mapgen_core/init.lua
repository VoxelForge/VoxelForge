...
minetest.register_alias("mapgen_water_source", "vlf_core:water_source")
minetest.register_alias("mapgen_lava_source", "air") -- Built-in lava generator is too unpredictable, we generate lava on our own
minetest.register_alias("mapgen_cobble", "vlf_core:cobble")

if minetest.get_modpath("vlfx_core") then
	minetest.register_alias("mapgen_river_water_source", "vlfx_core:river_water_source")
else
	minetest.register_alias("mapgen_river_water_source", "vlf_core:water_source")
end
...
local enable_mt_dungeons = minetest.settings:get_bool("vlf_enable_mt_dungeons",false)

-- Content IDs
local c_bedrock = minetest.get_content_id("vlf_core:bedrock")
local c_void = minetest.get_content_id("vlf_core:void")
local c_lava = minetest.get_content_id("vlf_core:lava_source")
local c_nether_lava = minetest.get_content_id("vlf_nether:nether_lava_source")
local c_water = minetest.get_content_id("vlf_core:water_source")
local c_realm_barrier = minetest.get_content_id("vlf_core:realm_barrier")
...
local bedrock_check
if vlf_vars.mg_bedrock_is_rough then
	function bedrock_check(_, y, _, _, pr)
		-- Bedrock layers with increasing levels of roughness, until a perfecly flat bedrock later at the bottom layer
		-- This code assumes a bedrock height of 5 layers.

		local diff = vlf_vars.mg_bedrock_overworld_max - y -- Overworld bedrock
		local ndiff1 = vlf_vars.mg_bedrock_nether_bottom_max - y -- Nether bedrock, bottom
		local ndiff2 = vlf_vars.mg_bedrock_nether_top_max - y -- Nether bedrock, ceiling
...
local function world_structure(vm, data, data2, emin, emax, area, minp, maxp, blockseed)
	...
	lvm_used = set_layers(data, area, c_void         , nil, vlf_vars.mapgen_edge_min                     , vlf_vars.mg_nether_min                     -1, minp, maxp, lvm_used, pr)

	-- [[ THE NETHER:					vlf_vars.mg_nether_min			       vlf_vars.mg_nether_max							]]

	-- The Air on the Nether roof, https://git.minetest.land/MineClone2/MineClone2/issues/1186
	lvm_used = set_layers(data, area, c_air		 , nil, vlf_vars.mg_nether_max			   +1, vlf_vars.mg_nether_max + 128                 , minp, maxp, lvm_used, pr)
	-- The Void above the Nether below the End:
	lvm_used = set_layers(data, area, c_void         , nil, vlf_vars.mg_nether_max + 128               +1, vlf_vars.mg_end_min                        -1, minp, maxp, lvm_used, pr)

	-- [[ THE END:						vlf_vars.mg_end_min			       vlf_vars.mg_end_max							]]

	-- The Void above the End below the Realm barrier:
	lvm_used = set_layers(data, area, c_void         , nil, vlf_vars.mg_end_max                        +1, vlf_vars.mg_realm_barrier_overworld_end_min-1, minp, maxp, lvm_used, pr)
	-- Realm barrier between the Overworld void and the End
	lvm_used = set_layers(data, area, c_realm_barrier, nil, vlf_vars.mg_realm_barrier_overworld_end_min  , vlf_vars.mg_realm_barrier_overworld_end_max  , minp, maxp, lvm_used, pr)
	...
	if mg_name ~= "singlenode" then
		...
		lvm_used = set_layers(data, area, c_bedrock, bedrock_check, vlf_vars.mg_bedrock_nether_top_min, vlf_vars.mg_bedrock_nether_top_max, minp, maxp, lvm_used, pr)

		-- Flat Nether
		if mg_name == "flat" then
			lvm_used = set_layers(data, area, c_air, nil, vlf_vars.mg_flat_nether_floor, vlf_vars.mg_flat_nether_ceiling, minp, maxp, lvm_used, pr)
		end

		-- Big lava seas by replacing air below a certain height
		if vlf_vars.mg_lava then
			lvm_used = set_layers(data, area, c_lava, c_air, vlf_vars.mg_overworld_min, vlf_vars.mg_lava_overworld_max, minp, maxp, lvm_used, pr)
			lvm_used = set_layers(data, area, c_nether_lava, c_air, vlf_vars.mg_nether_min, vlf_vars.mg_lava_nether_max, minp, maxp, lvm_used, pr)
		end
	end
	local deco = false
	local ores = false
...
minetest.register_on_mods_loaded(function()
	...
	for k, v in pairs(minetest.registered_biomes) do
		biome_id_p2[minetest.get_biome_id(k)] = v._vlf_palette_index or 255
	end
end)
