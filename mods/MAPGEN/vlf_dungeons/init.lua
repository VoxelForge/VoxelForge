...
local min_y = math.max(vlf_vars.mg_overworld_min, vlf_vars.mg_bedrock_overworld_max) + 1
local max_y = vlf_vars.mg_overworld_max - 1
-- Calculate the number of dungeon spawn attempts
-- In Minecraft, there 8 dungeon spawn attempts Minecraft chunk (16*256*16 = 65536 blocks).
-- Minetest chunks don't have this size, so scale the number accordingly.
local attempts = math.ceil(((vlf_vars.chunksize * vlf_vars.MAP_BLOCKSIZE) ^ 3) / 8192) -- 63 = 80*80*80/8192
...
{
	{
		...
		stacks_max = 3,
		items = {
			{ itemstring = "vlf_mobitems:nametag", weight = 20 },
			{ itemstring = "vlf_mobitems:saddle", weight = 20 },
			{ itemstring = "vlf_jukebox:record_1", weight = 15 },
			{ itemstring = "vlf_jukebox:record_4", weight = 15 },
			{ itemstring = "vlf_jukebox:record_5", weight = 3 },
	...
	},
	{
		...
		items = {
			...
			{ itemstring = "vlf_buckets:bucket_empty", weight = 10 },
...
local function ecb_spawn_dungeon(blockpos, action, calls_remaining, param)
	...
	if check then
		...
		for tz = z+1, z+dim.z do
			local fdef = minetest.registered_nodes[vlf_vars.get_node({x = tx, y = y_floor  , z = tz}).name]
			local cdef = minetest.registered_nodes[vlf_vars.get_node({x = tx, y = y_ceiling, z = tz}).name]
	...
	if vlf_vars.get_node({x=x, y=y+1, z=z}).name == "air" and vlf_vars.get_node({x=x, y=y+2, z=z}).name == "air" then
		openings_counter = openings_counter + 1
		if not openings[x] then openings[x]={} end
		openings[x][z] = true
		table.insert(corners, {x=x, z=z})
	end
	if vlf_vars.get_node({x=x2, y=y+1, z=z}).name == "air" and vlf_vars.get_node({x=x2, y=y+2, z=z}).name == "air" then
		openings_counter = openings_counter + 1
		if not openings[x2] then openings[x2]={} end
		openings[x2][z] = true
		table.insert(corners, {x=x2, z=z})
	end
	if vlf_vars.get_node({x=x, y=y+1, z=z2}).name == "air" and vlf_vars.get_node({x=x, y=y+2, z=z2}).name == "air" then
		openings_counter = openings_counter + 1
		if not openings[x] then openings[x]={} end
		openings[x][z2] = true
		table.insert(corners, {x=x, z=z2})
	end
	if vlf_vars.get_node({x=x2, y=y+1, z=z2}).name == "air" and vlf_vars.get_node({x=x2, y=y+2, z=z2}).name == "air" then
		openings_counter = openings_counter + 1
		if not openings[x2] then openings[x2]={} end
		openings[x2][z2] = true
	...
	for wx = x+1, x+dim.x do
		if vlf_vars.get_node({x=wx, y=y+1, z=z}).name == "air" and vlf_vars.get_node({x=wx, y=y+2, z=z}).name == "air" then
			openings_counter = openings_counter + 1
			if check and openings_counter > 5 then return end
			if not openings[wx] then openings[wx]={} end
			openings[wx][z] = true
		end
		if vlf_vars.get_node({x=wx, y=y+1, z=z2}).name == "air" and vlf_vars.get_node({x=wx, y=y+2, z=z2}).name == "air" then
			openings_counter = openings_counter + 1
			if check and openings_counter > 5 then return end
			if not openings[wx] then openings[wx]={} end
			openings[wx][z2] = true
	...
	for wz = z+1, z+dim.z do
		if vlf_vars.get_node({x=x, y=y+1, z=wz}).name == "air" and vlf_vars.get_node({x=x, y=y+2, z=wz}).name == "air" then
			openings_counter = openings_counter + 1
			if check and openings_counter > 5 then return end
			if not openings[x] then openings[x]={} end
			openings[x][wz] = true
		end
		if vlf_vars.get_node({x=x2, y=y+1, z=wz}).name == "air" and vlf_vars.get_node({x=x2, y=y+2, z=wz}).name == "air" then
			openings_counter = openings_counter + 1
			if check and openings_counter > 5 then return end
			if not openings[x2] then openings[x2]={} end
			openings[x2][wz] = true
	...
	minetest.log("info","[vlf_dungeons] Placing new dungeon at "..minetest.pos_to_string({x=x,y=y,z=z}))
	...
	local rn = minetest.registered_nodes[vlf_vars.get_node(sp).name]
	...
	for ty = y, maxy do
		...
		local name = vlf_vars.get_node(p).name
		local rn = minetest.registered_nodes[name]
		if rn and rn.is_ground_content or name == "vlf_core:cobble" or name == "vlf_core:mossycobble" then
			-- Floor
			if ty == y then
				if pr:next(1,4) == 1 then
					minetest.swap_node(p, {name = "vlf_core:cobble"})
				else
					minetest.swap_node(p, {name = "vlf_core:mossycobble"})
				end
	...
	for c=#chests, 1, -1 do
		...
		for s=1, #surround_vectors do
			...
			local nodename = vlf_vars.get_node(spos).name
			local nodename2 = vlf_vars.get_node(wpos).name
			...
			if nodedef and nodedef.walkable == false and nodedef2 and nodedef2.walkable == true and nodename2 ~= "vlf_chests:chest" then
	...
	for s=#spawner_posses, 1, -1 do
		...
		vlf_mobspawners.setup_spawner(sp, spawner_mob, 0, 7)
	end
end
...
local function dungeons_nodes(minp, maxp, blockseed)
	...
	for a=1, attempts do
		...
		if ymin <= ymax - dim.y - 1 then
			...
			minetest.log("verbose","[vlf_dungeons] size=" ..minetest.pos_to_string(dim) .. ", emerge from "..minetest.pos_to_string(p1) .. " to " .. minetest.pos_to_string(p2))
			minetest.emerge_area(p1, p2, ecb_spawn_dungeon, {p1=p1, p2=p2, dim=dim, pr=pr})
		end
	end
end

function vlf_dungeons.spawn_dungeon(p1, _, pr)
...
vlf_mapgen_core.register_generator("dungeons", nil, dungeons_nodes, 999999)
