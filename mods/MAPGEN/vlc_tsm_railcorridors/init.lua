...
if vlc_vars.mg_lava then
	height_min = vlc_vars.mg_lava_overworld_max + 2
else
	height_min = vlc_vars.mg_bedrock_overworld_max + 2
end
local height_max = vlc_worlds.layer_to_y(60)
...
vlc_structures.register_structure("mineshaft",{
	place_on = {"group:sand","group:grass_block","vlc_core:water_source","group:dirt","vlc_core:dirt_with_grass","vlc_core:gravel","group:material_stone","vlc_core:snow"},
	fill_ratio = 0.0001,
	...
	place_func = function(pos,def,pr,blockseed)
		...
		if p.y < vlc_vars.mg_overworld_min + 5 then
			p.y = vlc_vars.mg_overworld_min + 5
		end
...
vlc_mapgen_core.register_generator("railcorridors", nil, function(minp, maxp, blockseed, _pr)
