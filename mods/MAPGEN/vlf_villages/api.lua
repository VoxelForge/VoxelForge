vlf_villages.schematic_houses = {}
vlf_villages.schematic_jobs = {}
vlf_villages.schematic_lamps = {}
vlf_villages.schematic_bells = {}
vlf_villages.schematic_wells = {}
vlf_villages.on_village_placed = {}
vlf_villages.on_villager_placed = {}
vlf_villages.mandatory_buildings = {}
vlf_villages.forced_blocks = {}
	...
	for _, n in pairs(mobs_mc.jobsites) do
		if string.find(n, "^group:") then
			if n == "group:cauldron" then
				count = count + select(2, string.gsub(str, '"vlf_cauldrons:cauldron', ""))
			else
				...
				if num then
					minetest.log(
						"info",
						string.format("[vlf_villages] Guessing how to handle %s counting it as %d job sites", name, num)
					)
					count = count + num
				else
					minetest.log(
						"warning",
						string.format("[vlf_villages] Don't know how to handle group %s counting it as 1 job site", n)
					)
					count = count + 1
				end
			end
...
local function set_mandatory(record, type)
	if record['is_mandatory'] then
		if not vlf_villages.mandatory_buildings[type] then
			vlf_villages.mandatory_buildings[type] = {}
		end

		table.insert(vlf_villages.mandatory_buildings[type], record["name"])
	end
end

function vlf_villages.register_lamp(record)
	local data = load_schema(record["name"], record["mts"])
	set_all_optional(record, data)
	table.insert(vlf_villages.schematic_lamps, data)
	set_mandatory(record, 'lamps')
end

function vlf_villages.register_bell(record)
	local data = load_schema(record["name"], record["mts"])
	set_all_optional(record, data)
	table.insert(vlf_villages.schematic_bells, data)
	set_mandatory(record, 'bells')
end

function vlf_villages.register_well(record)
	local data = load_schema(record["name"], record["mts"])
	set_all_optional(record, data)
	table.insert(vlf_villages.schematic_wells, data)
	set_mandatory(record, 'wells')
end

local optional_fields = { "min_jobs", "max_jobs", "num_others", "is_mandatory" }

function vlf_villages.register_building(record)
	local data = load_schema(record["name"], record["mts"])
	...
	local num_beds = select(2, string.gsub(str, '"vlf_beds:bed_[^"]+_bottom"', ""))
	...
	if job_count > 0 then
		...
		table.insert(vlf_villages.schematic_jobs, data)
		set_mandatory(record, 'jobs')
	else
		table.insert(vlf_villages.schematic_houses, data)
...
function vlf_villages.default_crop()
	return "vlf_farming:wheat_1"
end
...
function vlf_villages.get_crop_types()
	return table.copy(supported_crop_types)
end

function vlf_villages.get_crops()
	return table.copy(crop_list)
end

function vlf_villages.get_weighted_crop(biome, crop_type, pr)
	if weighted_crops[biome] == nil then
		biome = "plains"
	end
...
function vlf_villages.register_crop(crop_def)
...
function vlf_villages.register_on_village_placed(func)
	table.insert(vlf_villages.on_village_placed, func)
end

function vlf_villages.register_on_villager_spawned(func)
	table.insert(vlf_villages.on_villager_placed, func)
end
