...
minetest.register_node("vlf_villages:structblock", {drawtype="airlike",groups = {not_in_creative_inventory=1},})
...
if minetest.get_modpath("mobs_mc") then
  vlf_mobs:register_spawn("mobs_mc:villager", --name
    {"vlf_core:stonebrickcarved"}, --nodes
    15, --max_light
    0, --min_light
    20, --chance
...
if mg_name ~= "singlenode" then
	vlf_mapgen_core.register_generator("villages", nil, function(minp, maxp, blockseed)
		...
		if n and n.name == "vlf_villages:structblock" then return end
		...
		minetest.set_node(minp,{name="vlf_villages:structblock"})
...
minetest.register_lbm({
	name = "vlf_villages:structblock",
	run_at_every_load = true,
	nodenames = {"vlf_villages:structblock"},
...
if minetest.is_creative_enabled("") then
	minetest.register_craftitem("vlf_villages:tool", {
		description = S("vlf_villages build tool"),
	...
	vlf_wip.register_experimental_item("vlf_villages:tool")
end
