vlf_villages = {}
vlf_villages.modpath = minetest.get_modpath(minetest.get_current_modname())

local village_chance = tonumber(minetest.settings:get("vlf_villages_village_chance")) or 100
local generate_in_singlenode = false

dofile(vlf_villages.modpath.."/const.lua")
dofile(vlf_villages.modpath.."/utils.lua")
dofile(vlf_villages.modpath.."/foundation.lua")
dofile(vlf_villages.modpath.."/buildings.lua")
dofile(vlf_villages.modpath.."/paths.lua")
dofile(vlf_villages.modpath.."/pois.lua")

dofile(vlf_villages.modpath .. "/api.lua")

--
-- load settlements on server
--
vlf_villages.grundstellungen()

local S = minetest.get_translator(minetest.get_current_modname())

minetest.register_alias("vlf_villages:stonebrickcarved", "vlf_core:stonebrickcarved")
minetest.register_alias("vlf_villages:structblock", "air")

--
-- on map generation, try to build a settlement
--
local function build_a_settlement(minp, maxp, blockseed)
	if vlf_villages.village_exists(blockseed) then
		return
	end

	local pr = PseudoRandom(blockseed)

	local settlement_info, grid = vlf_villages.create_site_plan_new(minp, maxp, pr)

	if not settlement_info then
		return
	end

	vlf_villages.terraform_new(settlement_info, grid)
	vlf_villages.place_schematics_new(settlement_info, pr, blockseed)

	-- TODO when run here minetest.find_path regularly fails :(
	--vlf_villages.paths_new(blockseed)
	--minetest.log("info", "Completed village for " .. minetest.pos_to_string(minp))

	vlf_villages.add_village(blockseed, settlement_info)

	for _, on_village_placed_callback in pairs(vlf_villages.on_village_placed) do
...
if mg_name ~= "singlenode" or generate_in_singlenode then
	vlf_mapgen_core.register_generator("villages", nil, function(minp, maxp, blockseed)
...
minetest.register_on_mods_loaded(function()
	...
	minetest.registered_chatcommands["spawnstruct"].func = function(pn,p)
		if p == "village" then
			...
			local minp = vector.subtract(pos, vlf_villages.half_map_chunk_size)
			local maxp = vector.add(pos, vlf_villages.half_map_chunk_size)
...
minetest.register_node("vlf_villages:village_block", {
	drawtype = "airlike",
	...
	on_timer = function(pos, elapsed)
		...
		vlf_villages.post_process_village(blockseed)
		return false
	end,
})
...
-- This is a light source so that lamps don't get placed near it
minetest.register_node("vlf_villages:building_block", {
	drawtype = "airlike",
	groups = { not_in_creative_inventory = 1 },
	light_source = 14,
	...
	on_timer = function(pos, elapsed)
		...
		minetest.set_node(pos, { name = node_type })
		vlf_villages.post_process_building(minp, maxp, blockseed, has_beds, has_jobs, is_belltower, bell_pos)
		return false
	end,
})
...
minetest.register_lbm({
	name = "vlf_villages:clear_remains",
	run_at_every_load = true,
	nodenames = { "vlf_villages:no_paths" },
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if
			vlf_villages.forced_blocks[bell_pos]
			and vlf_villages.forced_blocks[bell_pos] < minetest.get_us_time() - 10000000
		then
			minetest.forceload_free_block(pos, true)
			vlf_villages.forced_blocks[bell_pos] = nil
		end
	end,
})
...
minetest.register_node("vlf_villages:no_paths", {
	description = S(
		"Prevent paths from being placed during villager generation. Replaced by air after village path generation"
	),
	paramtype = "light",
	drawtype = drawtype,
	inventory_image = "vlf_core_barrier.png",
	wield_image = "vlf_core_barrier.png",
	tiles = { "vlf_core_barrier.png" },
	is_ground_content = false,
	groups = { creative_breakable = 1, not_solid = 1, not_in_creative_inventory = 1 },
})

minetest.register_node("vlf_villages:path_endpoint", {
	...
	sounds = vlf_sounds.node_sound_wool_defaults(),
	paramtype = "light",
...
local schem_path = vlf_villages.modpath .. "/schematics/"

vlf_villages.register_bell({ name = "belltower", mts = schem_path .. "new_villages/belltower.mts", yadjust = 1 })

vlf_villages.register_well({
	name = "well",
...
for _, crop_type in pairs(vlf_villages.get_crop_types()) do
	for count = 1, 8 do
		local tile = crop_type .. "_" .. count .. ".png"
		minetest.register_node("vlf_villages:crop_" .. crop_type .. "_" .. count, {
...
for name, def in pairs(minetest.registered_nodes) do
	if def.groups["flower"] and not def.groups["double_plant"] and name ~= "vlf_flowers:wither_rose" then
		vlf_villages.register_crop({
			type = "flower",
			node = name,
			biomes = {
