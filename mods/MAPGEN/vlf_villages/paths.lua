...
local light_threshold = tonumber(minetest.settings:get("vlf_villages_light_threshold")) or 6

function vlf_villages.paths(settlement_info)
	...
	for o,p in pairs(settlement_info) do
		...
		if starting_point ~= end_point then
			...
			while true do
				...
				local surface_point, surface_mat = vlf_villages.find_surface(starting_point)
				-- replace surface node with vlf_core:grass_path
				if surface_point then
					if surface_mat == "vlf_core:sand" or surface_mat == "vlf_core:redsand" then
						minetest.swap_node(surface_point,{name="vlf_core:sandstonesmooth2"})
					else
						minetest.swap_node(surface_point,{name="vlf_core:grass_path"})
					end
...
function vlf_villages.dump_path_ends()
	minetest.log("info", "[vlf_villages] " .. dump(path_ends))
end

-- Insert end points in to the nested tables
function vlf_villages.store_path_ends(minp, maxp, pos, size, blockseed, bell_pos)
	local path_end_nodes = minetest.find_nodes_in_area(
		vector.offset(minp, -4, -4, -4),
		vector.offset(maxp, 4, 4, 4),
		"vlf_villages:path_endpoint"
	)
...
local function place_lamp(pos, pr)
	local lamp_index = pr:next(1, #vlf_villages.schematic_lamps)
	local building_all_info = vlf_villages.schematic_lamps[lamp_index]
	local schem_lua = vlf_villages.substitue_materials(pos, building_all_info["schem_lua"], pr)
...
local function place_path(path, pr, stair, slab)
	...
	for i = 2, #path - 1 do
		...
		elseif y < prev_y and y < next_y then
			...
			minetest.swap_node(path[i], { name = "vlf_core:dirt" })
	...
	for i, pos in ipairs(path) do
		...
		if not done then
			...
			elseif n.name == "vlf_core:sand" or n.name == "vlf_core:redsand" then
				minetest.swap_node(under_pos, { name = "vlf_core:sandstonesmooth2" })
			elseif minetest.get_item_group(n.name, "soil") > 0
				and minetest.get_item_group(n.name, "dirtifies_below_solid") == 0
				then
					minetest.swap_node(under_pos, { name = "vlf_core:grass_path" })
				end
			end
	...
	for _, pos in ipairs(path) do
		if minetest.get_node_light(pos, 0) < light_threshold then
			...
			for _, npos in ipairs(nn) do
				...
				if node.name ~= "vlf_core:grass_path" and minetest.get_item_group(node.name, "stair") == 0 then
					if minetest.get_item_group(node.name, "wood_slab") ~= 0 then
						local over_pos = vector.offset(npos, 0, 1, 0)
						minetest.add_node(over_pos, { name = "vlf_torches:torch", param2 = 1 })
					else
						place_lamp(npos, pr)
					end
					break
...
function vlf_villages.paths_new(blockseed, biome_name)
	local pr = PseudoRandom(blockseed)

	if path_ends["block_" .. blockseed] == nil then
		minetest.log("warning", string.format("[vlf_villages] Tried to set paths for block seed that doesn't exist %d", blockseed))
		return
	end

	-- Use the same stair and slab throughout the entire village
	local stair = '"vlf_stairs:stair_oak"'
	local slab = '"vlf_stairs:slab_oak_top"'
				...
				if closest_pos then
					...
					if path and #path > 0 then
						...
						minetest.log(
							"warning",
							string.format(
								"[vlf_villages] No path from %s to %s, distance %d",
