...
minetest.register_alias("mapgen_stone", "vlc_core:stone")
minetest.register_alias("mapgen_water_source", "vlc_core:water_source")
minetest.register_alias("mapgen_lava_source", "air") -- Built-in lava generator is too unpredictable, we generate lava on our own
minetest.register_alias("mapgen_cobble", "vlc_core:cobble")

if minetest.get_modpath("vlcx_core") then
	minetest.register_alias("mapgen_river_water_source", "vlcx_core:river_water_source")
else
	minetest.register_alias("mapgen_river_water_source", "vlc_core:water_source")
end
...
local enable_mt_dungeons = minetest.settings:get_bool("vlc_enable_mt_dungeons",false)

-- Content IDs
local c_bedrock = minetest.get_content_id("vlc_core:bedrock")
local c_void = minetest.get_content_id("vlc_core:void")
local c_lava = minetest.get_content_id("vlc_core:lava_source")
local c_nether_lava = minetest.get_content_id("vlc_nether:nether_lava_source")
local c_water = minetest.get_content_id("vlc_core:water_source")
local c_realm_barrier = minetest.get_content_id("vlc_core:realm_barrier")
...
local bedrock_check
if vlc_vars.mg_bedrock_is_rough then
	function bedrock_check(_, y, _, _, pr)
		-- Bedrock layers with increasing levels of roughness, until a perfecly flat bedrock later at the bottom layer
		-- This code assumes a bedrock height of 5 layers.

		local diff = vlc_vars.mg_bedrock_overworld_max - y -- Overworld bedrock
		local ndiff1 = vlc_vars.mg_bedrock_nether_bottom_max - y -- Nether bedrock, bottom
		local ndiff2 = vlc_vars.mg_bedrock_nether_top_max - y -- Nether bedrock, ceiling
...
local function world_structure(vm, data, data2, emin, emax, area, minp, maxp, blockseed)
	...
	lvm_used = set_layers(data, area, c_void         , nil, vlc_vars.mapgen_edge_min                     , vlc_vars.mg_nether_min                     -1, minp, maxp, lvm_used, pr)

	-- [[ THE NETHER:					vlc_vars.mg_nether_min			       vlc_vars.mg_nether_max							]]

	-- The Air on the Nether roof, https://git.minetest.land/MineClone2/MineClone2/issues/1186
	lvm_used = set_layers(data, area, c_air		 , nil, vlc_vars.mg_nether_max			   +1, vlc_vars.mg_nether_max + 128                 , minp, maxp, lvm_used, pr)
	-- The Void above the Nether below the End:
	lvm_used = set_layers(data, area, c_void         , nil, vlc_vars.mg_nether_max + 128               +1, vlc_vars.mg_end_min                        -1, minp, maxp, lvm_used, pr)

	-- [[ THE END:						vlc_vars.mg_end_min			       vlc_vars.mg_end_max							]]

	-- The Void above the End below the Realm barrier:
	lvm_used = set_layers(data, area, c_void         , nil, vlc_vars.mg_end_max                        +1, vlc_vars.mg_realm_barrier_overworld_end_min-1, minp, maxp, lvm_used, pr)
	-- Realm barrier between the Overworld void and the End
	lvm_used = set_layers(data, area, c_realm_barrier, nil, vlc_vars.mg_realm_barrier_overworld_end_min  , vlc_vars.mg_realm_barrier_overworld_end_max  , minp, maxp, lvm_used, pr)
	-- The Void above Realm barrier below the Overworld:
	lvm_used = set_layers(data, area, c_void         , nil, vlc_vars.mg_realm_barrier_overworld_end_max+1, vlc_vars.mg_overworld_min                  -1, minp, maxp, lvm_used, pr)


	if mg_name ~= "singlenode" then
		-- Bedrock
		lvm_used = set_layers(data, area, c_bedrock, bedrock_check, vlc_vars.mg_bedrock_overworld_min, vlc_vars.mg_bedrock_overworld_max, minp, maxp, lvm_used, pr)
		lvm_used = set_layers(data, area, c_bedrock, bedrock_check, vlc_vars.mg_bedrock_nether_bottom_min, vlc_vars.mg_bedrock_nether_bottom_max, minp, maxp, lvm_used, pr)
		lvm_used = set_layers(data, area, c_bedrock, bedrock_check, vlc_vars.mg_bedrock_nether_top_min, vlc_vars.mg_bedrock_nether_top_max, minp, maxp, lvm_used, pr)

		-- Flat Nether
		if mg_name == "flat" then
			lvm_used = set_layers(data, area, c_air, nil, vlc_vars.mg_flat_nether_floor, vlc_vars.mg_flat_nether_ceiling, minp, maxp, lvm_used, pr)
		end

		-- Big lava seas by replacing air below a certain height
		if vlc_vars.mg_lava then
			lvm_used = set_layers(data, area, c_lava, c_air, vlc_vars.mg_overworld_min, vlc_vars.mg_lava_overworld_max, minp, maxp, lvm_used, pr)
			lvm_used = set_layers(data, area, c_nether_lava, c_air, vlc_vars.mg_nether_min, vlc_vars.mg_lava_nether_max, minp, maxp, lvm_used, pr)
		end
	end
	local deco = false
	local ores = false
	if minp.y >  vlc_vars.mg_nether_deco_max - 64 and maxp.y <  vlc_vars.mg_nether_max + 128 then
		deco = {min=vlc_vars.mg_nether_deco_max,max=vlc_vars.mg_nether_max}
	end
	if minp.y <  vlc_vars.mg_nether_min + 10 or maxp.y <  vlc_vars.mg_nether_min + 60 then
		deco = {min=vlc_vars.mg_nether_min - 10,max=vlc_vars.mg_nether_min + 20}
		ores = {min=vlc_vars.mg_nether_min - 10,max=vlc_vars.mg_nether_min + 20}
	end
...
minetest.register_on_mods_loaded(function()
	...
	for k, v in pairs(minetest.registered_biomes) do
		biome_id_p2[minetest.get_biome_id(k)] = v._vlc_palette_index or 255
	end
end)

local function set_param2_nodes(vm, data, data2, emin, emax, area, minp, maxp, blockseed)
	if emin.y > vlc_vars.mg_overworld_max or emax.y < vlc_vars.mg_overworld_min then return end
...
local function end_basic(vm, data, data2, emin, emax, area, minp, maxp, blockseed)
	if maxp.y < vlc_vars.mg_end_min or minp.y > vlc_vars.mg_end_max then return end
	for z = minp.z, maxp.z do
...
vlc_mapgen_core.register_generator("world_structure", world_structure, nil, 1, false)

if mg_name ~= "singlenode" then
	vlc_mapgen_core.register_generator("end_fixes", end_basic,nil, 9999, false)
	vlc_mapgen_core.register_generator("set_param2_nodes", set_param2_nodes, nil, 9999, true)
end

-- This should be moved to vlc_structures eventually if the dependencies can be sorted out.
vlc_mapgen_core.register_generator("structures",nil, function(minp, maxp, blockseed)
	local gennotify = minetest.get_mapgen_object("gennotify")
	local has = false
	for _,struct in pairs(vlc_structures.registered_structures) do
		...
		if struct.deco_id then
			for _, pos in pairs(gennotify["decoration#"..struct.deco_id] or {}) do
				...
				if struct.chunk_probability == nil or (not has and pr:next(1,struct.chunk_probability) == 1 ) then
					vlc_structures.place_structure(realpos,struct,pr,blockseed)
					has=true
				end
			end
		elseif struct.static_pos then
			for _,p in pairs(struct.static_pos) do
				if vlc_util.in_cube(p,minp,maxp) then
					vlc_structures.place_structure(p,struct,pr,blockseed)
				end
			end
		end
	end
