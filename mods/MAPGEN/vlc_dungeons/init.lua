...
if vlc_vars.mg_dungeons == false or mg_name == "singlenode" then
	return
end

local min_y = math.max(vlc_vars.mg_overworld_min, vlc_vars.mg_bedrock_overworld_max) + 1
local max_y = vlc_vars.mg_overworld_max - 1
-- Calculate the number of dungeon spawn attempts
-- In Minecraft, there 8 dungeon spawn attempts Minecraft chunk (16*256*16 = 65536 blocks).
-- Minetest chunks don't have this size, so scale the number accordingly.
local attempts = math.ceil(((vlc_vars.chunksize * vlc_vars.MAP_BLOCKSIZE) ^ 3) / 8192) -- 63 = 80*80*80/8192
...
{
	{
		...
		stacks_max = 3,
		items = {
			{ itemstring = "vlc_mobitems:nametag", weight = 20 },
			{ itemstring = "vlc_mobitems:saddle", weight = 20 },
			{ itemstring = "vlc_jukebox:record_1", weight = 15 },
			{ itemstring = "vlc_jukebox:record_4", weight = 15 },
			{ itemstring = "vlc_jukebox:record_5", weight = 3 },
	...
	},
	{
		...
		items = {
			...
			{ itemstring = "vlc_farming:bread", weight = 20 },
			...
			{ itemstring = "vlc_buckets:bucket_empty", weight = 10 },
...
local function ecb_spawn_dungeon(blockpos, action, calls_remaining, param)
	...
	if check then
		...
		for tz = z+1, z+dim.z do
			local fdef = minetest.registered_nodes[vlc_vars.get_node({x = tx, y = y_floor  , z = tz}).name]
			local cdef = minetest.registered_nodes[vlc_vars.get_node({x = tx, y = y_ceiling, z = tz}).name]
	...
	if vlc_vars.get_node({x=x, y=y+1, z=z}).name == "air" and vlc_vars.get_node({x=x, y=y+2, z=z}).name == "air" then
		openings_counter = openings_counter + 1
		if not openings[x] then openings[x]={} end
		openings[x][z] = true
		table.insert(corners, {x=x, z=z})
	end
	if vlc_vars.get_node({x=x2, y=y+1, z=z}).name == "air" and vlc_vars.get_node({x=x2, y=y+2, z=z}).name == "air" then
		openings_counter = openings_counter + 1
		if not openings[x2] then openings[x2]={} end
		openings[x2][z] = true
		table.insert(corners, {x=x2, z=z})
	end
	if vlc_vars.get_node({x=x, y=y+1, z=z2}).name == "air" and vlc_vars.get_node({x=x, y=y+2, z=z2}).name == "air" then
		openings_counter = openings_counter + 1
		if not openings[x] then openings[x]={} end
		openings[x][z2] = true
		table.insert(corners, {x=x, z=z2})
	end
	if vlc_vars.get_node({x=x2, y=y+1, z=z2}).name == "air" and vlc_vars.get_node({x=x2, y=y+2, z=z2}).name == "air" then
		openings_counter = openings_counter + 1
		if not openings[x2] then openings[x2]={} end
		openings[x2][z2] = true
	...
	for wx = x+1, x+dim.x do
		if vlc_vars.get_node({x=wx, y=y+1, z=z}).name == "air" and vlc_vars.get_node({x=wx, y=y+2, z=z}).name == "air" then
			openings_counter = openings_counter + 1
			if check and openings_counter > 5 then return end
			if not openings[wx] then openings[wx]={} end
			openings[wx][z] = true
		end
		if vlc_vars.get_node({x=wx, y=y+1, z=z2}).name == "air" and vlc_vars.get_node({x=wx, y=y+2, z=z2}).name == "air" then
			openings_counter = openings_counter + 1
			if check and openings_counter > 5 then return end
			if not openings[wx] then openings[wx]={} end
			openings[wx][z2] = true
	...
	for wz = z+1, z+dim.z do
		if vlc_vars.get_node({x=x, y=y+1, z=wz}).name == "air" and vlc_vars.get_node({x=x, y=y+2, z=wz}).name == "air" then
			openings_counter = openings_counter + 1
			if check and openings_counter > 5 then return end
			if not openings[x] then openings[x]={} end
			openings[x][wz] = true
		end
		if vlc_vars.get_node({x=x2, y=y+1, z=wz}).name == "air" and vlc_vars.get_node({x=x2, y=y+2, z=wz}).name == "air" then
			openings_counter = openings_counter + 1
			if check and openings_counter > 5 then return end
			if not openings[x2] then openings[x2]={} end
			openings[x2][wz] = true
	...
	minetest.log("info","[vlc_dungeons] Placing new dungeon at "..minetest.pos_to_string({x=x,y=y,z=z}))
	...
	local rn = minetest.registered_nodes[vlc_vars.get_node(sp).name]
	...
	for ty = y, maxy do
		...
		local name = vlc_vars.get_node(p).name
		local rn = minetest.registered_nodes[name]
		if rn and rn.is_ground_content or name == "vlc_core:cobble" or name == "vlc_core:mossycobble" then
			-- Floor
			if ty == y then
				if pr:next(1,4) == 1 then
					minetest.swap_node(p, {name = "vlc_core:cobble"})
				else
					minetest.swap_node(p, {name = "vlc_core:mossycobble"})
				end
	...
	for c=#chests, 1, -1 do
		...
		for s=1, #surround_vectors do
			...
			local nodename = vlc_vars.get_node(spos).name
			local nodename2 = vlc_vars.get_node(wpos).name
			...
			if nodedef and nodedef.walkable == false and nodedef2 and nodedef2.walkable == true and nodename2 ~= "vlc_chests:chest" then
		...
		minetest.set_node(pos, {name="vlc_chests:chest", param2=facedir})
		local meta = minetest.get_meta(pos)
		minetest.log("info", "[vlc_dungeons] Filling chest " .. tostring(c) .. " at " .. minetest.pos_to_string(pos))
		vlc_loot.fill_inventory(meta:get_inventory(), "main", vlc_loot.get_multi_loot(loottable, pr), pr)
	end
	...
	for s=#spawner_posses, 1, -1 do
		...
		minetest.set_node(sp, {name = "vlc_mobspawners:spawner"})
		local mobs = {
			"mobs_mc:zombie",
			"mobs_mc:zombie",
			"mobs_mc:spider",
		...
		vlc_mobspawners.setup_spawner(sp, spawner_mob, 0, 7)
	end
end
...
local function dungeons_nodes(minp, maxp, blockseed)
	...
	for a=1, attempts do
		...
		if ymin <= ymax - dim.y - 1 then
			...
			minetest.log("verbose","[vlc_dungeons] size=" ..minetest.pos_to_string(dim) .. ", emerge from "..minetest.pos_to_string(p1) .. " to " .. minetest.pos_to_string(p2))
			minetest.emerge_area(p1, p2, ecb_spawn_dungeon, {p1=p1, p2=p2, dim=dim, pr=pr})
		end
	end
end

function vlc_dungeons.spawn_dungeon(p1, _, pr)
...
vlc_mapgen_core.register_generator("dungeons", nil, dungeons_nodes, 999999)
