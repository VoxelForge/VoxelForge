...
local function spawn_witch(p1,p2)
	local c = minetest.find_node_near(p1,15,{"vlc_cauldrons:cauldron"})
	if c then
		local nn = minetest.find_nodes_in_area_under_air(vector.new(p1.x,c.y-1,p1.z),vector.new(p2.x,c.y-1,p2.z),{"vlc_core:sprucewood"})
		local witch
...
local function hut_placement_callback(pos,def,pr)
	...
	local legs = minetest.find_nodes_in_area(vector.offset(pos,-hl,0,-hl),vector.offset(pos,hl,0,hl), {"vlc_core:tree","vlc_trees:tree_oak"})
	local tree = {}
	for _,leg in pairs(legs) do
		while minetest.get_item_group(vlc_vars.get_node(vector.offset(leg,0,-1,0), true, 333333).name, "water") ~= 0 do
			leg = vector.offset(leg,0,-1,0)
			table.insert(tree,leg)
		end
	end
	minetest.bulk_set_node(tree, {name = "vlc_trees:tree_oak", param2 = 2})
	spawn_witch(p1,p2)
end

vlc_structures.register_structure("witch_hut",{
	place_on = {"group:sand","group:grass_block","vlc_core:water_source","group:dirt"},
	fill_ratio = 0.01,
	...
	filenames = { vlc_structures.schempath.."/schems/vlc_structures_witch_hut.mts" },
