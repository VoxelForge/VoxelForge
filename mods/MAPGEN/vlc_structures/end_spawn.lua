...
vlc_structures.register_structure("end_spawn_obsidian_platform",{
	static_pos ={vlc_vars.mg_end_platform_pos},
	place_func = function(pos,def,pr)
		local obby = minetest.find_nodes_in_area(vector.offset(pos,-2,0,-2),vector.offset(pos,2,0,2),{"air","vlc_end:end_stone"})
		local air = minetest.find_nodes_in_area(vector.offset(pos,-2,1,-2),vector.offset(pos,2,3,2),{"air","vlc_end:end_stone"})
		minetest.bulk_set_node(obby,{name="vlc_core:obsidian"})
		minetest.bulk_set_node(air,{name="air"})
		return true
	end,
})

vlc_structures.register_structure("end_exit_portal",{
	static_pos = { vlc_vars.mg_end_exit_portal_pos },
	filenames = {
		vlc_structures.schempath.."/schems/vlc_structures_end_exit_portal.mts"
	},
	after_place = function(pos,def,pr,blockseed)
		...
		minetest.emerge_area(p1,p2,function(blockpos, action, calls_remaining, param)
			...
			minetest.bulk_set_node(minetest.find_nodes_in_area(p1,p2,{"vlc_portals:portal_end"}),{name="air"})
			...
			if obj then
				...
				minetest.log("error", "[vlc_mapgen_core] ERROR! Ender dragon doesn't want to spawn")
			end
			minetest.fix_light(p1,p2)
		end)
	end
})
vlc_structures.register_structure("end_exit_portal_open",{
	filenames = {
		vlc_structures.schempath.."/schems/vlc_structures_end_exit_portal.mts"
	},
...
vlc_structures.register_structure("end_gateway_portal",{
	filenames = {
		vlc_structures.schempath.."/schems/vlc_structures_end_gateway_portal.mts"
	},
})
...
local function make_endspike(pos,width,height)
	...
	minetest.bulk_set_node(nodes,{ name="vlc_core:obsidian"} )
...
function make_cage(pos,width)
	...
	minetest.bulk_set_node(nodes,{ name="vlc_panes:bar_flat"} )
	for _,p in pairs(nodes) do
		vlc_panes.update_pane(p)
	end
end
...
vlc_structures.register_structure("end_spike",{
	static_pos =get_points_on_circle(vector.offset(vlc_vars.mg_end_exit_portal_pos,0,-20,0),43,10),
	place_func = function(pos,def,pr)
		...
		minetest.emerge_area(p1, p2, function(blockpos, action, calls_remaining, param)
			...
			minetest.set_node(vector.offset(s,0,1,0),{name="vlc_core:bedrock"})
			minetest.add_entity(vector.offset(s,0,2,0),"vlc_end:crystal")
