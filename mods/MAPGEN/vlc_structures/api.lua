vlc_structures.registered_structures = {}
local modname = minetest.get_current_modname()
local modpath = minetest.get_modpath(modname)

--local place_queue = {}
local disabled_structures = minetest.settings:get("vlc_disabled_structures")
if disabled_structures then	disabled_structures = disabled_structures:split(",")
else disabled_structures = {} end

local peaceful = minetest.settings:get_bool("only_peaceful_mobs", false)
local mob_cap_player = tonumber(minetest.settings:get("vlc_mob_cap_player")) or 75
local mob_cap_animal = tonumber(minetest.settings:get("vlc_mob_cap_animal")) or 10

local logging = minetest.settings:get_bool("vlc_logging_structures",true)
...
function vlc_structures.is_disabled(structname)
...
function vlc_structures.place_schematic(pos, schematic, rotation, replacements, force_placement, flags, after_placement_callback, pr, callback_param)
	if type(schematic) ~= "table" and not vlc_util.file_exists(schematic) then
		minetest.log("warning","[vlc_structures] schematic file "..tostring(schematic).." does not exist.")
		return end
	...
	if s and s.size then
		...
		minetest.log("verbose", "[vlc_structures] size=" ..minetest.pos_to_string(s.size) .. ", rotation=" .. tostring(rotation) .. ", emerge from "..minetest.pos_to_string(p1) .. " to " .. minetest.pos_to_string(p2))
...
function vlc_structures.get_struct(file)
	local localfile = vlc_structures.schempath.."/schems/"..file
	local file, errorload = io.open(localfile, "rb")
	if errorload then
		minetest.log("error", "[vlc_structures] Could not open this struct: "..localfile)
		return nil
	end
...
vlc_structures.init_node_construct = init_node_construct

function vlc_structures.fill_chests(p1,p2,loot,pr)
	for it,lt in pairs(loot) do
		if it ~= "SUS" then --don't try to generate loot for "sus nodes" here, this happens when a player brushes a suspicious node
			...
			for _,p in pairs(nodes) do
				local lootitems = vlc_loot.get_multi_loot(lt, pr)
				vlc_structures.init_node_construct(p)
				local meta = minetest.get_meta(p)
				local inv = meta:get_inventory()
				vlc_loot.fill_inventory(inv, "main", lootitems, pr)
			end
		end
	end
end
...
local function generate_loot(pos, def, pr)
	...
	if def.loot then vlc_structures.fill_chests(p1,p2,def.loot,pr) end
end

function vlc_structures.construct_nodes(p1,p2,nodes)
...
local function construct_nodes(pos,def,pr)
	return vlc_structures.construct_nodes(vector.offset(pos,-def.sidelen/2,0,-def.sidelen/2),vector.offset(pos,def.sidelen/2,def.sidelen,def.sidelen/2),def.construct_nodes)
end


function vlc_structures.find_lowest_y(pp)
	local y = 31000
...
function vlc_structures.find_highest_y(pp)
	local y = -31000
	for _,p in pairs(pp) do
		if p.y > y then y = p.y end
	end
...
local function find_ground(pos,nn,gn)
	...
	for _,v in pairs(nn) do
		...
		until not n or n == "vlc_core:bedrock" or n == "ignore" or n == gn
...
local function get_foundation_nodes(ground_p1,ground_p2,pos,sidelen,node_stone)
	local replace = {"air","group:liquid","vlc_core:snow","group:tree","group:leaves","group:plant","grass_block","group:dirt"}
...
local function foundation(ground_p1,ground_p2,pos,sidelen)
	local node_stone = "vlc_core:stone"
	local node_filler = "vlc_core:dirt"
	local node_top = "vlc_core:dirt_with_grass" or minetest.get_node(ground_p1).name
	local node_dust = nil
...
local function process_queue()
	...
	vlc_structures.place_schematic(s.pos, s.file, s.rot, nil, true, "place_center_x,place_center_z",function(s)
...
function vlc_structures.spawn_mobs(mob,spawnon,p1,p2,pr,n,water)
	n = n or 1
	local sp = {}
	if water then
...
function vlc_structures.place_structure(pos, def, pr, blockseed, rot)
	...
	if def.solid_ground and def.sidelen then
		...
		if #solid < ( def.sidelen * def.sidelen ) then
			if def.make_foundation then
				vlc_util.create_ground_turnip(vector.offset(pos, 0, -1, 0), def.sidelen, def.sidelen)
			else
				if log_enabled then
					minetest.log("warning","[vlc_structures] "..def.name.." at "..minetest.pos_to_string(pp).." not placed. No solid ground.")
				end
				return false
			end
		end
	...
	if def.on_place and not def.on_place(pos,def,pr,blockseed) then
		if log_enabled then
			minetest.log("warning","[vlc_structures] "..def.name.." at "..minetest.pos_to_string(pp).." not placed. Conditions not satisfied.")
		end
		return false
	end
	if def.filenames then
		...
		if file then
			...
			if def.daughters then
				ap = function(pos,def,pr,blockseed)
					for _,d in pairs(def.daughters) do
						...
						vlc_structures.place_schematic(p, d.files[pr:next(1,#d.files)], rot, nil, true, "place_center_x,place_center_z",function()
			...
			vlc_structures.place_schematic(pp, file, rot,  def.replacements, true, "place_center_x,place_center_z",function(p1, p2, size, rotation)
			...
			if log_enabled then
				minetest.log("info","[vlc_structures] "..def.name.." placed at "..minetest.pos_to_string(pp))
			end
			return true
		end
	...
	if log_enabled then
		minetest.log("warning","[vlc_structures] placing "..def.name.." failed at "..minetest.pos_to_string(pos))
	end
end

function vlc_structures.register_structure(name,def,nospawn) --nospawn means it will be placed by another (non-nospawn) structure that contains it's structblock i.e. it will not be placed by mapgen directly
	if vlc_structures.is_disabled(name) then return end
	local structblock = "vlc_structures:structblock_"..name
		...
		if def.place_on then
			minetest.register_on_mods_loaded(function() --make sure all previous decorations and biomes have been registered
				def.deco = minetest.register_decoration({
					name = "vlc_structures:deco_"..name,
				...
				def.deco_id = minetest.get_decoration_id("vlc_structures:deco_"..name)
				minetest.set_gen_notify({decoration=true}, { def.deco_id })
				--catching of gennotify happens in vlc_mapgen_core

			end)
		end
	end
	vlc_structures.registered_structures[name] = def
end

function vlc_structures.register_structure_spawn(def)
...
minetest.register_lbm({
	name = "vlc_structures:struct_lbm",
	run_at_every_load = true,
	nodenames = {"group:structblock_lbm"},
	action = function(pos, node)
		minetest.remove_node(pos)
		local name = node.name:gsub("vlc_structures:structblock_","")
		local def = vlc_structures.registered_structures[name]
		if not def then return end
		vlc_structures.place_structure(pos)
	end
})
