	...
	for s=1, #stronghold_rings do
		...
		for a=1, ring.amount do
			...
			if vlc_vars.superflat then
				y = vlc_vars.mg_bedrock_overworld_max + 3
			else
				y = pr:next(vlc_vars.mg_bedrock_overworld_max+1, vlc_vars.mg_overworld_min+48)
			end
...
vlc_structures.register_structure("end_shrine",{
	static_pos = init_strongholds(),
	filenames = {
		minetest.get_modpath("vlc_structures").."/schematics/vlc_structures_end_portal_room_simple.mts"
	},
	after_place = function(pos,def,pr,blockseed,p1,p2,size,rotation)
		...
		local spawners = minetest.find_nodes_in_area(p1, p2, "vlc_mobspawners:spawner")
		for s=1, #spawners do
			--local meta = minetest.get_meta(spawners[s])
			vlc_mobspawners.setup_spawner(spawners[s], "mobs_mc:silverfish")
		end

		-- Shuffle stone brick types
		local bricks = minetest.find_nodes_in_area(p1, p2, "vlc_core:stonebrick")
		for b=1, #bricks do
			...
			if r_infested <= 5 then
				if r_bricktype <= 30 then -- 30%
					bricktype = "vlc_monster_eggs:monster_egg_stonebrickmossy"
				elseif r_bricktype <= 50 then -- 20%
					bricktype = "vlc_monster_eggs:monster_egg_stonebrickcracked"
				else -- 50%
					bricktype = "vlc_monster_eggs:monster_egg_stonebrick"
				end
			else
				if r_bricktype <= 30 then -- 30%
					bricktype = "vlc_core:stonebrickmossy"
				elseif r_bricktype <= 50 then -- 20%
					bricktype = "vlc_core:stonebrickcracked"
				end
		...
		-- Also replace stairs
		local stairs = minetest.find_nodes_in_area(p1, p2, {"vlc_stairs:stair_stonebrick", "vlc_stairs:stair_stonebrick_outer", "vlc_stairs:stair_stonebrick_inner"})
		for s=1, #stairs do
			local stair = minetest.get_node(stairs[s])
			local r_type = pr:next(1, 100)
			if r_type <= 30 then -- 30% mossy
				if stair.name == "vlc_stairs:stair_stonebrick" then
					stair.name = "vlc_stairs:stair_stonebrickmossy"
				elseif stair.name == "vlc_stairs:stair_stonebrick_outer" then
					stair.name = "vlc_stairs:stair_stonebrickmossy_outer"
				elseif stair.name == "vlc_stairs:stair_stonebrick_inner" then
					stair.name = "vlc_stairs:stair_stonebrickmossy_inner"
				end
				minetest.set_node(stairs[s], stair)
			elseif r_type <= 50 then -- 20% cracky
				if stair.name == "vlc_stairs:stair_stonebrick" then
					stair.name = "vlc_stairs:stair_stonebrickcracked"
				elseif stair.name == "vlc_stairs:stair_stonebrick_outer" then
					stair.name = "vlc_stairs:stair_stonebrickcracked_outer"
				elseif stair.name == "vlc_stairs:stair_stonebrick_inner" then
					stair.name = "vlc_stairs:stair_stonebrickcracked_inner"
				end
				minetest.set_node(stairs[s], stair)
			end
			-- 50% no change
		...
		local frames = minetest.find_nodes_in_area(p1, p2, "vlc_portals:end_portal_frame")
		local eyes = 0
		for f=1, #frames do
			...
			if r_eye == 1 then
				...
				if eyes < #frames then
					...
					frame_node.name = "vlc_portals:end_portal_frame_eye"
