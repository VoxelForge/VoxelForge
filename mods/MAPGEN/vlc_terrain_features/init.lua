			...
			for kk,vv in pairs(adjacents) do
				...
				if not border then
					...
					elseif minetest.get_item_group(minetest.get_node(nn[1]).name,"solid") > 0 then
						...
						border = "vlc_core:stone"
					end
					if border == nil or border == "vlc_core:dirt" then border = "vlc_core:dirt_with_grass" end
				end
...
local mushrooms = {"vlc_mushrooms:mushroom_brown","vlc_mushrooms:mushroom_red"}
...
local function get_fallen_tree_schematic(pos,pr)
	...
	for i = 1,len do
		table.insert(schem.data,{name = "vlc_core:vine",param2=4, prob=vprob})
	end
	...
	table.insert(schem.data,{name = "air", prob=0})
	for i = 1,len do
		table.insert(schem.data,{name = "vlc_core:vine",param2=5, prob=vprob})
	end
	table.insert(schem.data,{name = "air", prob=0})
	table.insert(schem.data,{name = "air", prob=0})
	for i = 1,len do
...
vlc_structures.register_structure("fallen_tree",{
...
vlc_structures.register_structure("lavapool",{
	...
	y_min = minetest.get_mapgen_setting("water_level"),
	place_func = function(pos,def,pr)
		return makelake(pos,5,"vlc_core:lava_source",{"group:material_stone", "group:sand", "group:dirt"},"vlc_core:stone",pr)
	end
})

vlc_structures.register_structure("water_lake",{
	...
	y_min = minetest.get_mapgen_setting("water_level"),
	place_func = function(pos,def,pr)
		return makelake(pos,5,"vlc_core:water_source",{"group:material_stone", "group:sand", "group:dirt","group:grass_block"},"vlc_core:dirt_with_grass",pr)
	end
})

vlc_structures.register_structure("water_lake_mangrove_swamp",{
	place_on = {"vlc_mud:mud"},
	...
	y_min = minetest.get_mapgen_setting("water_level"),
	place_func = function(pos,def,pr)
		return makelake(pos,3,"vlc_core:water_source",{"group:material_stone", "group:sand", "group:dirt","group:grass_block","vlc_mud:mud"},"vlc_mud:mud",pr,true)
	end
})

vlc_structures.register_structure("basalt_column",{
	place_on = {"vlc_blackstone:blackstone","vlc_blackstone:basalt"},
	terrain_feature = true,
	spawn_by = {"air"},
	num_spawn_by = 2,
	noise_params = {
	...
	flags = "all_floors",
	y_max = vlc_vars.mg_nether_max - 20,
	y_min = vlc_vars.mg_lava_nether_max + 1,
	biomes = { "BasaltDelta" },
	place_func = function(pos,def,pr)
		local nn = minetest.find_nodes_in_area(vector.offset(pos,-5,-1,-5),vector.offset(pos,5,-1,5),{"air","vlc_blackstone:basalt","vlc_blackstone:blackstone"})
		table.sort(nn,function(a, b)
		   return vector.distance(vector.new(pos.x,0,pos.z), a) < vector.distance(vector.new(pos.x,0,pos.z), b)
		end)
		...
		minetest.bulk_set_node(magma,{name="vlc_nether:magma"})
		minetest.bulk_set_node(basalt,{name="vlc_blackstone:basalt"})
		return true
	end
})
vlc_structures.register_structure("basalt_pillar",{
	place_on = {"vlc_blackstone:blackstone","vlc_blackstone:basalt"},
	terrain_feature = true,
	noise_params = {
		offset = 0,
	...
	flags = "all_floors",
	y_max = vlc_vars.mg_nether_max-40,
	y_min = vlc_vars.mg_lava_nether_max + 1,
	biomes = { "BasaltDelta" },
	place_func = function(pos,def,pr)
		local nn = minetest.find_nodes_in_area(vector.offset(pos,-2,-1,-2),vector.offset(pos,2,-1,2),{"air","vlc_blackstone:basalt","vlc_blackstone:blackstone"})
		table.sort(nn,function(a, b)
		   return vector.distance(vector.new(pos.x,0,pos.z), a) < vector.distance(vector.new(pos.x,0,pos.z), b)
		end)
		if #nn < 1 then return false end
...
vlc_structures.register_structure("lavadelta",{
	place_on = {"vlc_blackstone:blackstone","vlc_blackstone:basalt"},
	spawn_by = {"vlc_blackstone:basalt","vlc_blackstone:blackstone"},
	num_spawn_by = 2,
	terrain_feature = true,
	noise_params = {
		offset = 0,
	...
	biomes = { "BasaltDelta" },
	place_func = function(pos,def,pr)
		local nn = minetest.find_nodes_in_area_under_air(vector.offset(pos,-10,-1,-10),vector.offset(pos,10,-2,10),{"vlc_blackstone:basalt","vlc_blackstone:blackstone","vlc_nether:netherrack"})
		table.sort(nn,function(a, b)
		   return vector.distance(vector.new(pos.x,0,pos.z), a) < vector.distance(vector.new(pos.x,0,pos.z), b)
		end)
		if #nn < 1 then return false end
		...
		minetest.bulk_set_node(lava,{name="vlc_nether:nether_lava_source"})
		local basalt = {}
		local magma = {}
		for _,v in pairs(lava) do
			for _,vv in pairs(adjacents) do
				local p = vector.add(v,vv)
				if minetest.get_node(p).name ~= "vlc_nether:nether_lava_source" then
