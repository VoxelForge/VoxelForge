...
function vlc_signs.upgrade_sign_meta(pos)
		...
		vlc_signs.get_text_entity (pos, true) -- the 2nd "true" arg means deleting the entity for respawn
end

function vlc_signs.upgrade_sign_rot(pos,node)
	local numsign = false
	...
	vlc_signs.upgrade_sign_meta(pos)
	vlc_signs.update_sign(pos)
end

minetest.register_lbm({
	nodenames = {"group:sign"},
	name = ":vlc_signs:update_old_signs",
	label = "Update old signs",
	run_at_every_load = false,
	action = vlc_signs.upgrade_sign_rot,
})

for k,_ in pairs(mcl2rotsigns) do table.insert(vlc_signs.old_rotnames, k) end
for k,_ in pairs(mcl2standingsigns) do table.insert(vlc_signs.old_rotnames, k) end
minetest.register_lbm({
	nodenames = vlc_signs.old_rotnames,
	name = ":vlc_signs:update_old_rotated_standing",
