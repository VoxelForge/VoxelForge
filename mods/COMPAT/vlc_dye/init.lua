...
mcl_dye.colors = vlc_dyes.colors
mcl_dye.unicolor_to_dye = vlc_dyes.unicolor_to_dye
mcl_dye.add_bone_meal_particle = vlc_bone_meal.add_bone_meal_particle

mcl_dye.mcl2dyes_translate = {}
for k, v in pairs(vlc_dyes.colors) do
	mcl_dye.mcl2dyes_translate["mcl_dye:"..(v.mcl2 or k)] = "vlc_dyes:"..k
end
...
-- using that itemstring are rewritten to use "vlc_dyes:blue" so they still work the same.
...
function mcl_dye.register_on_bone_meal_apply(func)
	minetest.log("warning", "[mcl_dye] A mod "..(minetest.get_current_modname() or "").."is using the function mcl_dye.register_on_bone_meal_apply - this is deprecated. Use the node defintion callbacks as documented in mods/vlc_bone_meal/API.md instead!")
	table.insert(mcl_dye.bone_meal_callbacks, func)
end

local old_on_place = minetest.registered_items["vlc_bone_meal:bone_meal"].on_place
minetest.override_item("vlc_bone_meal:bone_meal", {
