...
mcl_dye.colors = vlf_dyes.colors
mcl_dye.unicolor_to_dye = vlf_dyes.unicolor_to_dye
mcl_dye.add_bone_meal_particle = vlf_bone_meal.add_bone_meal_particle

mcl_dye.mcl2dyes_translate = {}
for k, v in pairs(vlf_dyes.colors) do
	mcl_dye.mcl2dyes_translate["mcl_dye:"..(v.mcl2 or k)] = "vlf_dyes:"..k
end
...
function mcl_dye.register_on_bone_meal_apply(func)
	minetest.log("warning", "[mcl_dye] A mod "..(minetest.get_current_modname() or "").."is using the function mcl_dye.register_on_bone_meal_apply - this is deprecated. Use the node defintion callbacks as documented in mods/vlf_bone_meal/API.md instead!")
