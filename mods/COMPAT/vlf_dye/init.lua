...
vlf_dye.vlf2dyes_translate = {}
for k, v in pairs(vlf_dyes.colors) do
	vlf_dye.vlf2dyes_translate["vlf_dye:"..(v.vlf2 or k)] = "vlf_dyes:"..k
end

-- Override of minetest.register_craft rewrites crafing recipes that use vlf2 item names to use the vlfa eqivalents.
		...
		for k,v in pairs(recipe.recipe) do
			if type(v) == "table" then
				for l,w in pairs(v) do
					if vlf_dye.vlf2dyes_translate[w] then
						recipe.recipe[k][l] = vlf_dye.vlf2dyes_translate[w]
					end
				end
			elseif type(v) == "string"then
				if vlf_dye.vlf2dyes_translate[v] then
					recipe.recipe[k] = vlf_dye.vlf2dyes_translate[v]
				end
			end
		end
	end
