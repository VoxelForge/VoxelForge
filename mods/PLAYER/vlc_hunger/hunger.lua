...
function minetest.do_item_eat(hp_change, replace_with_item, itemstack, user, pointed_thing)
	...
	if no_eat_delay or (vlc_hunger.last_eat[name] < 0) or (os.difftime(os.time(), vlc_hunger.last_eat[name]) >= 2) then
		local can_eat_when_full = creative or (vlc_hunger.active == false)
		...
		if can_eat_when_full or (vlc_hunger.get_hunger(user) < 20) then
			itemstack = vlc_hunger.eat(hp_change, replace_with_item, itemstack, user, pointed_thing)
			...
			vlc_hunger.last_eat[name] = os.time()
		end
	end
...
function vlc_hunger.eat(hp_change, replace_with_item, itemstack, user, pointed_thing)
	local item = itemstack:get_name()
	local def = vlc_hunger.registered_foods[item]
	if not def then
		def = {}
	...
	local func = vlc_hunger.item_eat(def.saturation, def.replace, def.poisontime,
...
function vlc_hunger.reset_bars_poison_hunger(player)
	hb.change_hudbar(player, "hunger", nil, nil, "hbhunger_icon.png", nil, "hbhunger_bar.png")
	if vlc_hunger.debug then
		hb.change_hudbar(player, "exhaustion", nil, nil, nil, nil, "vlc_hunger_bar_exhaustion.png")
	end
end
...
local function poisonp(tick, time, time_left, damage, exhaustion, name)
	...
	if vlc_hunger.poison_hunger[name] == 0 then
		return
	end
	...
	if time_left < time then
		...
		if exhaustion > 0 then
			vlc_hunger.poison_hunger [name] = vlc_hunger.poison_hunger[name] - 1
		end
		if vlc_hunger.poison_hunger[name] <= 0 then
	...
	if player:get_hp()-damage > 0 then
		vlc_util.deal_damage(player, damage, {type = "hunger"})
	end

	vlc_hunger.exhaust(name, exhaustion)

end

local poisonrandomizer = PseudoRandom(os.time())

function vlc_hunger.item_eat(hunger_change, replace_with_item, poisontime, poison, exhaust, poisonchance, sound)
	return function(itemstack, user, pointed_thing)
		...
		if itemstack:peek_item() and user then
			...
			if foodtype == 3 then
				...
				minetest.sound_play("vlc_hunger_bite", {
					max_hear_distance = 12,
					gain = 1.0,
					pitch = 1 + math.random(-10, 10)*0.005,
					object = user,
			...
			if vlc_hunger.active and hunger_change then
				-- Add saturation (must be defined in item table)
				local _vlc_saturation = minetest.registered_items[itemname]._vlc_saturation
				local saturation
				if not _vlc_saturation then
					saturation = 0
				else
					saturation = minetest.registered_items[itemname]._vlc_saturation
				end
				vlc_hunger.saturate(name, saturation, false)

				-- Add food points
				local h = vlc_hunger.get_hunger(user)
				if h < 20 and hunger_change then
					h = h + hunger_change
					if h > 20 then h = 20 end
					vlc_hunger.set_hunger(user, h, false)
				end

				hb.change_hudbar(user, "hunger", h)
				vlc_hunger.update_saturation_hud(user, vlc_hunger.get_saturation(user), h)
			elseif not vlc_hunger.active and hunger_change then
				user:set_hp(math.min(user:get_properties().hp_max or 20, user:get_hp() + hunger_change))
			end
			-- Poison
			if vlc_hunger.active and poisontime then
				...
				if do_poison then
					...
					if exhaust and exhaust > 0 then
						hb.change_hudbar(user, "hunger", nil, nil, "vlc_hunger_icon_foodpoison.png", nil, "vlc_hunger_bar_foodpoison.png")
						if vlc_hunger.debug then
							hb.change_hudbar(user, "exhaustion", nil, nil, nil, nil, "vlc_hunger_bar_foodpoison.png")
						end
						vlc_hunger.poison_hunger[name] = vlc_hunger.poison_hunger[name] + 1
					end
...
if vlc_hunger.active then
	...
	minetest.register_on_dignode(function(pos, oldnode, player)
		...
		-- dig event
		vlc_hunger.exhaust(name, vlc_hunger.EXHAUST_DIG)
	end)
end
