...
vlc_player.node_offsets = nodeinfo_pos
...
minetest.register_on_joinplayer(function(player)
	vlc_player.players[player] = table.copy(tpl_playerinfo)
...
function vlc_player.register_globalstep(func)
	table.insert(vlc_player.registered_globalsteps, func)
end

function vlc_player.register_globalstep_slow(func)
	table.insert(vlc_player.registered_globalsteps_slow, func)
end

-- Check each player and run callbacks
minetest.register_globalstep(function(dtime)
	for _, player in pairs(minetest.get_connected_players()) do
		for _, func in pairs(vlc_player.registered_globalsteps) do
			if vlc_player.players[player] then
				func(player, dtime)
			end
		end
	end
	...
	slow_gs_timer = 0.5
	for _, player in pairs(minetest.get_connected_players()) do
		for _, func in pairs(vlc_player.registered_globalsteps_slow) do
			if vlc_player.players[player] then
				func(player, dtime)
			end
		end
		vlc_player.players[player].lastPos = player:get_pos()
	end
end)
...
vlc_player.register_globalstep_slow(function(player, dtime)
	for k, v in pairs(nodeinfo_pos) do
		vlc_player.players[player].nodes[k] = node_ok(vector.add(player:get_pos(), v))
	end
end)
...
vlc_player.register_globalstep_slow(function(player, dtime)
	...
	local ndef = minetest.registered_nodes[vlc_player.players[player].nodes.head]
	if vlc_player.players[player].is_swimming then
		ndef = minetest.registered_nodes[vlc_player.players[player].nodes.feet]
	end
	...
	and (vlc_player.players[player].nodes.head ~= "ignore")
	-- Check privilege, too
	and (not minetest.check_player_privs(player:get_player_name(), {noclip = true})) then
		if player:get_hp() > 0 then
			vlc_util.deal_damage(player, 1, {type = "in_wall"})
		end
	end
end)
	...
	if reason.type == "fall" then
		...
		for i = 1, math.ceil(v_axis_max/5)+1 do -- trace at least 1/5 of the way per second
			...
			if node then
				...
				if node.name == "vlc_portals:portal_end" then
					if vlc_portals and vlc_portals.end_teleport then
						vlc_portals.end_teleport(obj)
					end
					return 0
				end
				if node.name == "vlc_core:cobweb" then
					return 0
				end
				if node.name == "vlc_core:vine" then
					return 0
				end
			end
