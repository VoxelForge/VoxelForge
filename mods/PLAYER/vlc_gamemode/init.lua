local S = minetest.get_translator("vlc_gamemode")
...
function vlc_gamemode.register_on_gamemode_change(func)
	table.insert(vlc_gamemode.registered_on_gamemode_change, func)
end
...
function vlc_gamemode.get_gamemode(p)
	return minetest.is_creative_enabled(p:get_player_name()) and "creative" or "survival"
end

function vlc_gamemode.set_gamemode(p, gm)
	if table.indexof(vlc_gamemode.gamemodes, gm) == -1 then return false end
	local old_gm = vlc_gamemode.get_gamemode(p)
	p:get_meta():set_string("gamemode", gm)
	for _, func in ipairs(vlc_gamemode.registered_on_gamemode_change) do
		func(p, old_gm, gm)
	end
	return true
end
...
minetest.register_chatcommand("gamemode",{
	...
	func = function(n,param)
		...
		if args[1] and vlc_gamemode.set_gamemode(p, args[1]) == false then
			return false, S("Failed to set Gamemode @1 for player @2", args[1], p:get_player_name())
		end
		--Result message - show effective game mode
		return true, S("Gamemode for player @1: @2", n, vlc_gamemode.get_gamemode(p))
	end
})
