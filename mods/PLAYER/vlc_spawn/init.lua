...
local function vlc_log (message)
	vlc_util.vlc_log (message, "[Spawn]")
end
...
local success = storage:get_int("vlc_spawn_success")==1
local searched = (storage:get_int("vlc_spawn_searched")==1) or mg_name == "singlenode" or minetest.settings:get("static_spawnpoint")
local wsp = minetest.string_to_pos(storage:get_string("vlc_spawn_world_spawn_point")) or {} -- world spawn position
local check = storage:get_int("vlc_spawn_check") or 0
local cp = minetest.string_to_pos(storage:get_string("vlc_spawn_cp")) or {x=start_pos.x, y=start_pos.y, z=start_pos.z}
local edge_len = storage:get_int("vlc_spawn_edge_len") or 1
local edge_dist = storage:get_int("vlc_spawn_edge_dist") or 0
local dir_step = storage:get_int("vlc_spawn_dir_step") or 0
local dir_ind = storage:get_int("vlc_spawn_dir_ind") or 1
local emerge_pos1, emerge_pos2

local spawn_limit = vlc_vars.mapgen_edge_max
...
local function next_biome()
	if #biome_ids < 1 then
		...
		if #biome_ids < 1 then
			...
			if math.abs(cp.x) > spawn_limit or math.abs(cp.z) > spawn_limit then
				check = checks + 1
				minetest.log("warning", "[vlc_spawn] No white-listed biomes found - search stopped by overlimit")
				return false
			end
			check = check + 1
			...
			if cp.y then
				...
				minetest.log("warning", "[vlc_spawn] No white-listed biomes found - using current")
				return true
			else
				minetest.log("warning", "[vlc_spawn] No white-listed biomes found and spawn level is nil - please define start_pos to continue")
				return false
			end
		end
		minetest.log("action", "[vlc_spawn] Suitable biomes found: "..tostring(#biome_ids))
	end
	while check <= checks do
		...
		if biome then
			minetest.log("verbose", "[vlc_spawn] Search white-listed biome at "..minetest.pos_to_string(cp)..": "..minetest.get_biome_name(biome))
...
local function ecb_search_continue(blockpos, action, calls_remaining, param)
	if calls_remaining <= 0 then
		...
		minetest.log("verbose", "[vlc_spawn] Data emerge callback: "..minetest.pos_to_string(wsp).." - "..tostring(nodes and #nodes) .. " node(s) found under air")
		if nodes then
			if no_trees_area_counter >= 0 then
				...
				if trees and #trees > 0 then
					...
					if attempts_to_find_pos * 3 < #nodes then
						...
						for i=1, attempts_to_find_pos do
							...
							if wsp then
								...
								if good_for_respawn(wsp) and can_find_tree(wsp, trees) then
									minetest.log("action", "[vlc_spawn] Dynamic world spawn randomly determined to be "..minetest.pos_to_string(wsp))
									searched = true
									success = true
									return
								end
						...
						for i=1, math.min(#nodes, attempts_to_find_pos) do
							...
							if wsp then
								wsp.y = wsp.y + 1
								if good_for_respawn(wsp) and can_find_tree(wsp, trees) then
									minetest.log("action", "[vlc_spawn] Dynamic world spawn determined to be "..minetest.pos_to_string(wsp))
									searched = true
									success = true
									return
								end
					...
					if no_trees_area_counter > 10 then
						minetest.log("verbose", "[vlc_spawn] More than 10 times no trees at all! Won't search trees next 200 calls")
				...
				if attempts_to_find_pos * 3 < #nodes then
					...
					for i=1, attempts_to_find_pos do
						...
						if wsp then
							...
							if good_for_respawn(wsp) then
								minetest.log("action", "[vlc_spawn] Dynamic world spawn randomly determined to be "..minetest.pos_to_string(wsp) .. " (no trees)")
								searched = true
								success = true
								return
							end
					...
					for i=1, math.min(#nodes, attempts_to_find_pos) do
						...
						if wsp then
							wsp.y = wsp.y + 1
							if good_for_respawn(wsp) then
								minetest.log("action", "[vlc_spawn] Dynamic world spawn determined to be "..minetest.pos_to_string(wsp) .. " (no trees)")
								searched = true
								success = true
								return
							end
...
function vlc_spawn.get_world_spawn_pos()
	...
	minetest.log("action", "[vlc_spawn] Failed to determine dynamic world spawn!")
	return start_pos
end
...
-- false otherwise.
function vlc_spawn.get_bed_spawn_pos(player)
	local spawn, custom_spawn = nil, false
	if player and player:is_player() then
		local attr = player:get_meta():get_string("vlc_beds:spawn")
	...
	if not spawn or spawn == "" then
		spawn = vlc_spawn.get_world_spawn_pos()
		custom_spawn = false
	end
...
-- changed.
function vlc_spawn.set_spawn_pos(player, pos, message)
	local spawn_changed = false
	local meta = player:get_meta()
	if pos == nil then
		if meta:get_string("vlc_beds:spawn") ~= "" then
			spawn_changed = true
			if message then
				minetest.chat_send_player(player:get_player_name(), S("Respawn position cleared!"))
			end
		end
		meta:set_string("vlc_beds:spawn", "")
	else
		local oldpos = minetest.string_to_pos(meta:get_string("vlc_beds:spawn"))
		meta:set_string("vlc_beds:spawn", minetest.pos_to_string(pos))

		-- Set player ownership on bed
		local bed_node = minetest.get_node(pos)
		local bed_meta = minetest.get_meta(pos)

		local bed_bottom = vlc_beds.get_bed_bottom (pos)
		local bed_bottom_meta = minetest.get_meta(bed_bottom)

		if bed_meta then
			if bed_node then
				vlc_log("Bed name: " .. bed_node.name)
			end

			vlc_log("Setting bed meta: " .. player:get_player_name())
			...
			if bed_bottom_meta then
				vlc_log("Removing villager from bed bottom meta")
				bed_bottom_meta:set_string("villager", "")
				bed_bottom_meta:set_string("infotext", "")
			else
				vlc_log("Cannot remove villager from bed bottom meta")
			end
			...
			if oldpos and oldpos ~= pos then
				...
				if old_bed_meta then
					vlc_log("Removing old bed meta")
					old_bed_meta:set_string("player", "")
				end
			else
				vlc_log("No old bed meta to remove or same as current")
			end
		end
...
function vlc_spawn.get_player_spawn_pos(player)
	local pos, custom_spawn = vlc_spawn.get_bed_spawn_pos(player)
	if pos and custom_spawn then
		...
		if bgroup ~= 1 and bgroup ~= 2 then
			-- Bed is destroyed:
			if player and player:is_player() then
				local checkpos = minetest.string_to_pos(player:get_meta():get_string("vlc_beds:spawn"))
				local checknode = minetest.get_node(checkpos)

				if(string.match(checknode.name, "vlc_beds:respawn_anchor_charged_")) then
					...
					if not charge_level then
						...
						player:get_meta():set_string("vlc_beds:spawn", "")
						minetest.chat_send_player(player:get_player_name(), S("Couldn't get level of your respawn anchor!"))
						return vlc_spawn.get_world_spawn_pos(), false
					elseif charge_level ~= 1 then
						minetest.set_node(checkpos, {name="vlc_beds:respawn_anchor_charged_".. charge_level-1})
						return checkpos, false
					else
						minetest.set_node(checkpos, {name="vlc_beds:respawn_anchor"})
						return checkpos, false
					end
				else
					player:get_meta():set_string("vlc_beds:spawn", "")
...
function vlc_spawn.spawn(player)
	local pos, in_bed = vlc_spawn.get_player_spawn_pos(player)
	player:set_pos(pos)
	return in_bed or success
end

-- Respawn player at specified respawn position
minetest.register_on_respawnplayer(vlc_spawn.spawn)

function vlc_spawn.shadow_worker()
	if not searched then
		searched = true
		vlc_spawn.search()
		minetest.log("action", "[vlc_spawn] Started world spawn point search")
	end
	...
	if success then
		...
		and ((not good_for_respawn(wsp)) or ((no_trees_area_counter >= 0) and not can_find_tree(wsp))) then
			success = false
			minetest.log("action", "[vlc_spawn] World spawn position isn't safe anymore: "..minetest.pos_to_string(wsp))
			vlc_spawn.search()
		end
	end

	minetest.after(respawn_search_interval, vlc_spawn.shadow_worker)
end

minetest.after(respawn_search_initial_delay, function()
	vlc_spawn.shadow_worker()

	minetest.register_on_shutdown(function()
		storage:set_int("vlc_spawn_success", success and 1 or 0)
		if wsp and wsp.x then
			storage:set_string("vlc_spawn_world_spawn_point", minetest.pos_to_string(wsp))
		end
		storage:set_int("vlc_spawn_searched", searched and 1 or 0)
		storage:set_int("vlc_spawn_check", check)
		storage:set_string("vlc_spawn_cp", minetest.pos_to_string(cp))
		storage:set_int("vlc_spawn_edge_len", edge_len)
		storage:set_int("vlc_spawn_edge_dist", edge_dist)
		storage:set_int("vlc_spawn_dir_step", dir_step)
		storage:set_int("vlc_spawn_dir_ind", dir_ind)
	end)
end)
