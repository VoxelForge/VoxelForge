...
local function vlf_log (message)
	vlf_util.vlf_log (message, "[Spawn]")
end
...
local success = storage:get_int("vlf_spawn_success")==1
local searched = (storage:get_int("vlf_spawn_searched")==1) or mg_name == "singlenode" or minetest.settings:get("static_spawnpoint")
local wsp = minetest.string_to_pos(storage:get_string("vlf_spawn_world_spawn_point")) or {} -- world spawn position
local check = storage:get_int("vlf_spawn_check") or 0
local cp = minetest.string_to_pos(storage:get_string("vlf_spawn_cp")) or {x=start_pos.x, y=start_pos.y, z=start_pos.z}
local edge_len = storage:get_int("vlf_spawn_edge_len") or 1
local edge_dist = storage:get_int("vlf_spawn_edge_dist") or 0
local dir_step = storage:get_int("vlf_spawn_dir_step") or 0
local dir_ind = storage:get_int("vlf_spawn_dir_ind") or 1
local emerge_pos1, emerge_pos2

local spawn_limit = vlf_vars.mapgen_edge_max
...
local function next_biome()
	if #biome_ids < 1 then
		...
		if #biome_ids < 1 then
			...
			if math.abs(cp.x) > spawn_limit or math.abs(cp.z) > spawn_limit then
				check = checks + 1
				minetest.log("warning", "[vlf_spawn] No white-listed biomes found - search stopped by overlimit")
				return false
			end
			check = check + 1
			...
			if cp.y then
				...
				minetest.log("warning", "[vlf_spawn] No white-listed biomes found - using current")
				return true
			else
				minetest.log("warning", "[vlf_spawn] No white-listed biomes found and spawn level is nil - please define start_pos to continue")
				return false
			end
		end
		minetest.log("action", "[vlf_spawn] Suitable biomes found: "..tostring(#biome_ids))
	end
	while check <= checks do
		...
		if biome then
			minetest.log("verbose", "[vlf_spawn] Search white-listed biome at "..minetest.pos_to_string(cp)..": "..minetest.get_biome_name(biome))
...
local function ecb_search_continue(blockpos, action, calls_remaining, param)
	if calls_remaining <= 0 then
		...
		minetest.log("verbose", "[vlf_spawn] Data emerge callback: "..minetest.pos_to_string(wsp).." - "..tostring(nodes and #nodes) .. " node(s) found under air")
		if nodes then
			if no_trees_area_counter >= 0 then
				...
				if trees and #trees > 0 then
					...
					if attempts_to_find_pos * 3 < #nodes then
						...
						for i=1, attempts_to_find_pos do
							...
							if wsp then
								...
								if good_for_respawn(wsp) and can_find_tree(wsp, trees) then
									minetest.log("action", "[vlf_spawn] Dynamic world spawn randomly determined to be "..minetest.pos_to_string(wsp))
									searched = true
									success = true
									return
								end
						...
						for i=1, math.min(#nodes, attempts_to_find_pos) do
							...
							if wsp then
								wsp.y = wsp.y + 1
								if good_for_respawn(wsp) and can_find_tree(wsp, trees) then
									minetest.log("action", "[vlf_spawn] Dynamic world spawn determined to be "..minetest.pos_to_string(wsp))
									searched = true
									success = true
									return
								end
					...
					if no_trees_area_counter > 10 then
						minetest.log("verbose", "[vlf_spawn] More than 10 times no trees at all! Won't search trees next 200 calls")
				...
				if attempts_to_find_pos * 3 < #nodes then
					...
					for i=1, attempts_to_find_pos do
						...
						if wsp then
							...
							if good_for_respawn(wsp) then
								minetest.log("action", "[vlf_spawn] Dynamic world spawn randomly determined to be "..minetest.pos_to_string(wsp) .. " (no trees)")
								searched = true
								success = true
								return
							end
					...
					for i=1, math.min(#nodes, attempts_to_find_pos) do
						...
						if wsp then
							wsp.y = wsp.y + 1
							if good_for_respawn(wsp) then
								minetest.log("action", "[vlf_spawn] Dynamic world spawn determined to be "..minetest.pos_to_string(wsp) .. " (no trees)")
								searched = true
								success = true
								return
							end
...
function vlf_spawn.get_world_spawn_pos()
	...
	minetest.log("action", "[vlf_spawn] Failed to determine dynamic world spawn!")
	return start_pos
end
...
function vlf_spawn.set_spawn_pos(player, pos, message)
	...
	if pos == nil then
		...
		if bed_meta then
			if bed_node then
				vlf_log("Bed name: " .. bed_node.name)
			end

			vlf_log("Setting bed meta: " .. player:get_player_name())
			...
			if bed_bottom_meta then
				vlf_log("Removing villager from bed bottom meta")
				bed_bottom_meta:set_string("villager", "")
				bed_bottom_meta:set_string("infotext", "")
			else
				vlf_log("Cannot remove villager from bed bottom meta")
			end
			...
			if oldpos and oldpos ~= pos then
				...
				if old_bed_meta then
					vlf_log("Removing old bed meta")
					old_bed_meta:set_string("player", "")
				end
			else
				vlf_log("No old bed meta to remove or same as current")
			end
		end
...
minetest.register_on_respawnplayer(vlf_spawn.spawn)

function vlf_spawn.shadow_worker()
	if not searched then
		searched = true
		vlf_spawn.search()
		minetest.log("action", "[vlf_spawn] Started world spawn point search")
	end
	...
	if success then
		...
		and ((not good_for_respawn(wsp)) or ((no_trees_area_counter >= 0) and not can_find_tree(wsp))) then
			success = false
			minetest.log("action", "[vlf_spawn] World spawn position isn't safe anymore: "..minetest.pos_to_string(wsp))
			vlf_spawn.search()
		end
	end

	minetest.after(respawn_search_interval, vlf_spawn.shadow_worker)
end

minetest.after(respawn_search_initial_delay, function()
	vlf_spawn.shadow_worker()

	minetest.register_on_shutdown(function()
		storage:set_int("vlf_spawn_success", success and 1 or 0)
		if wsp and wsp.x then
			storage:set_string("vlf_spawn_world_spawn_point", minetest.pos_to_string(wsp))
		end
		storage:set_int("vlf_spawn_searched", searched and 1 or 0)
		storage:set_int("vlf_spawn_check", check)
		storage:set_string("vlf_spawn_cp", minetest.pos_to_string(cp))
		storage:set_int("vlf_spawn_edge_len", edge_len)
		storage:set_int("vlf_spawn_edge_dist", edge_dist)
		storage:set_int("vlf_spawn_dir_step", dir_step)
		storage:set_int("vlf_spawn_dir_ind", dir_ind)
	end)
end)
