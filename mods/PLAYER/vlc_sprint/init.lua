...
minetest.register_on_joinplayer(function(player)
	...
	players[playerName] = {
		...
		sprintDistance = 0,
		fov = 1.0,
		channel = minetest.mod_channel_join("vlc_sprint:" .. playerName),
	}
end)
...
local function setSprinting(playerName, sprinting) --Sets the state of a player (0=stopped/moving, 1=sprinting)
	if not sprinting and not vlc_sprint.is_sprinting(playerName) then return end
	...
	if players[playerName] then
		...
		if sprinting == true
		or controls.RMB
		and string.find(player:get_wielded_item():get_name(), "vlc_bows:bow")
		and player:get_wielded_item():get_name() ~= "vlc_bows:bow" then
			...
			if sprinting == true then
				playerphysics.add_physics_factor(player, "speed", "vlc_sprint:sprint", vlc_sprint.SPEED)
			end
		elseif sprinting == false
		and player:get_wielded_item():get_name() ~= "vlc_bows:bow_0"
		and player:get_wielded_item():get_name() ~= "vlc_bows:bow_1"
		and player:get_wielded_item():get_name() ~= "vlc_bows:bow_2" then
			fov_new = math.max(players[playerName].fov - 0.05, 1.0)
			if sprinting == false then
				playerphysics.remove_physics_factor(player, "speed", "vlc_sprint:sprint")
			end
		end
...
minetest.register_on_modchannel_message(function(channel_name, sender, message)
	if channel_name == "vlc_sprint:" .. sender then
		...
		if player then
			...
			if playerInfo["sprinting"] == true and not player:get_attach() and gameTime % 0.1 == 0 then
				...
				if players[playerName].sprintDistance >= 1 then
					...
					vlc_hunger.exhaust(playerName, vlc_hunger.EXHAUST_SPRINT * superficial)
			...
			if players[playerName]["shouldSprint"] == true then --Stopped
				...
				if (vlc_hunger.active and vlc_hunger.get_hunger(player) <= 6)
				or (player:get_meta():get_string("vlc_beds:sleeping") == "true") then
					sprinting = false
