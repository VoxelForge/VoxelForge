local function make_texture(base, colorspec)
	local output = ""
	if vlc_skins.masks[base] then
		output = vlc_skins.masks[base] ..
...
function vlc_skins.get_skin_list()
	local list = {}
	for _, game_mode in pairs({"_crea", "_surv"}) do
		for _, base in pairs(vlc_skins.base) do
			for _, base_color in pairs(vlc_skins.base_color) do
		...
		for _, skin in pairs(vlc_skins.simple_skins) do
			table.insert(list, {
...
function vlc_skins.get_node_id_by_player(player)
	local skin = vlc_skins.player_skins[player]
	local simple_skin = skin.simple_skins_id
	if simple_skin then
		skin = vlc_skins.texture_to_simple_skin[skin.simple_skins_id]
	end
