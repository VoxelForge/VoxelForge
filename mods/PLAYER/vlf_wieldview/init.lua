minetest.register_entity("vlf_wieldview:wieldview", {
	initial_properties = {
		hp_max           = 1,
		visual           = "wielditem",
		physical         = false,
...
local function update_offhand_hud(player, texture)
	...
	local stack = vlf_offhand.get_offhand(player)
...
local function update_wieldview_entity(player, bone, position, rotation, get_item)
	...
	if luaentity and luaentity.object:get_yaw() then
		...
		if def and def._vlf_wieldview_item then
			item = def._vlf_wieldview_item
		end
		...
		local obj_ref = minetest.add_entity(player:get_pos(), "vlf_wieldview:wieldview")
...
minetest.register_globalstep(function(dtime)
	...
	for i, player in pairs(players) do
		...
		update_wieldview_entity(player, "Arm_Left", vector.new(0, 4.5, 2), vector.new(120, 0, 0), vlf_offhand.get_offhand)
	end
end)
