...
vlf_potions.registered_entity_effects = {}
-- shorthand
local registered_entity_effects = vlf_potions.registered_entity_effects

local function entity_effect_image(colorstring, opacity)
	if not opacity then
		opacity = 127
	end
	return "vlf_potions_entity_effect_overlay.png^[colorize:"..colorstring..":"..tostring(opacity).."^vlf_potions_entity_effect_bottle.png"
end
...
local function generate_on_use(effects, color, on_use, custom_effect)
	return function(itemstack, user, pointed_thing)
		...
		local potency = itemstack:get_meta():get_int("vlf_potions:entity_effect_potent")
		local plus = itemstack:get_meta():get_int("vlf_potions:entity_effect_plus")
		local ef_level
		local dur
		...
		if itemstack then vlf_potions._use_entity_effect(user, color) end
...
--   - must contain _vlf_potions=1 for tooltip to include dynamic_tt and effects
--   - defaults to {brewitem=1, food=3, can_eat_when_full=1, _vlf_potions=1}
...
function vlf_potions.register_entity_effect(def)
	...
	pdef.groups = def.groups or {brewitem=1, food=3, can_eat_when_full=1, _vlf_potions=1}
	...
	vlf_potions.registered_entity_effects[modname..":"..name] = internal_def
end

vlf_potions.register_entity_effect({
	name = "trolling",
...
vlf_potions.register_entity_effect({
	...
	image = table.concat({
		"(vlf_potions_entity_effect_overlay.png^[colorize:red:100)",
		"^vlf_potions_splash_overlay.png^[colorize:black:100",
		"^vlf_potions_entity_effect_bottle.png",
	}),
	_effect_list = {
		bad_omen = {dur = 6000},
	},
...
local function replace_legacy_entity_effect(itemstack)
	...
	if new_name then
		...
		new_stack:get_meta():set_int("vlf_potions:entity_effect_plus",
	...
	if new_name then
		new_stack = ItemStack(new_name..suffix)
		new_stack:get_meta():set_int("vlf_potions:entity_effect_potent",
...
local compat = "vlf_potions:compat_entity_effect"
...
minetest.register_craftitem(compat, {
	...
	image = "vlf_potions_entity_effect_overlay.png^[colorize:#00F:127^vlf_potions_entity_effect_bottle.png^vl_unknown.png",
