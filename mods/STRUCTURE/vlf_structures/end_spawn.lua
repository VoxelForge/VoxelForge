vlf_structures.register_structure("end_spawn_obsidian_platform",{
	...
	place_func = function(pos,def,pr)
		...
		minetest.bulk_set_node(obby,{name="vlf_core:obsidian"})
...
vlf_structures.register_structure("end_exit_portal",{
	...
	filenames = {
		vlf_structures.schempath.."/schems/vlf_structures_end_exit_portal.mts"
	},
	after_place = function(pos,def,pr,blockseed)
		...
		minetest.emerge_area(p1,p2,function(blockpos, action, calls_remaining, param)
			...
			minetest.bulk_set_node(minetest.find_nodes_in_area(p1,p2,{"vlf_portals:portal_end"}),{name="air"})
...
vlf_structures.register_structure("end_gateway_portal",{
	filenames = {
		vlf_structures.schempath.."/schems/vlf_structures_end_gateway_portal.mts"
	},
})
...
local function make_endspike(pos,width,height)
	...
	minetest.bulk_set_node(nodes,{ name="vlf_core:obsidian"} )
...
function make_cage(pos,width)
	...
	minetest.bulk_set_node(nodes,{ name="vlf_panes:bar_flat"} )
...
vlf_structures.register_structure("end_spike",{
	...
	place_func = function(pos,def,pr)
		...
		minetest.emerge_area(p1, p2, function(blockpos, action, calls_remaining, param)
			...
			minetest.set_node(vector.offset(s,0,1,0),{name="vlf_core:bedrock"})
