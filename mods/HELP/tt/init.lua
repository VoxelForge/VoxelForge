...
-- Apply item description updates and generate formspec tooltip
local function apply_snippets(desc, itemstring, toolcaps, itemstack)
    local first = true
    local formspec_tooltip = ""
    ...
    for s=1, #tt.registered_snippets do
        ...
        if str then
            ...
            if snippet_color then
                desc = desc .. minetest.colorize(snippet_color, str)
                formspec_tooltip = formspec_tooltip .. minetest.colorize(snippet_color, str) .. "\n"
            else
                desc = desc .. str
                formspec_tooltip = formspec_tooltip .. str .. "\n"
            end
        end
    end

    -- Ensure formspec_tooltip is not empty
    if formspec_tooltip ~= "" then
        formspec_tooltip = formspec_tooltip:sub(1, -2)  -- Remove the last newline character
    end

    return desc, formspec_tooltip
end
...
local function append_snippets()
    for itemstring, def in pairs(minetest.registered_items) do
        if should_change(itemstring, def) then
            local orig_desc = def.description
            local desc, formspec_tooltip = apply_snippets(orig_desc, itemstring, def.tool_capabilities, nil)
            if desc ~= orig_desc then
                minetest.override_item(itemstring, { description = desc, _tt_original_description = orig_desc })
            end
        end
...
function tt.reload_itemstack_description(itemstack)
    ...
    elseif should_change(itemstring, def) then
        ...
        local desc, formspec_tooltip = apply_snippets(orig_desc, itemstring, toolcaps or def.tool_capabilities, itemstack)
        meta:set_string("description", desc)

        -- Set the formspec tooltip
        if formspec_tooltip ~= "" then
            meta:set_string("formspec_tooltip", formspec_tooltip)
        end
    end
end
