...
doc.sub.items.register_factoid(nil, "groups", function(_, def)
	...
	if def._vlf_saturation and def._vlf_saturation > 0 then
		s = s .. "\n" .. S("Saturation points restored: @1%", string.format("%.1f", def._vlf_saturation))
	end
	return s
end)
...
doc.sub.items.register_factoid("nodes", "mining", function(_, def)
	...
	local hardness = def._vlf_hardness
	...
	local blast = def._vlf_blast_resistance
	if not blast then
		blast = 0
	end
...
doc.sub.items.register_factoid("nodes", "drops", function(_, def)
	...
	elseif type(def._vlf_shears_drop) == "table" then
		local drops = {}
		for d=1, #def._vlf_shears_drop do
			local item = ItemStack(def._vlf_shears_drop[d])
