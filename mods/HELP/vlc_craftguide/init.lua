...
local progressive_mode = minetest.settings:get_bool("vlc_craftguide_progressive_mode", true)

local C = minetest.colorize
local F = minetest.formspec_escape
local S = minetest.get_translator("vlc_craftguide")
...
local group_stereotypes = {
	wood         = "vlc_trees:wood_oak",
	stone        = "vlc_core:stone",
	sand         = "vlc_core:sand",
	wool         = "vlc_wool:white",
	carpet       = "vlc_wool:white_carpet",
	dye          = "vlc_dyes:red",
	water_bucket = "vlc_buckets:bucket_water",
	flower	     = "vlc_flowers:dandelion",
	mushroom     = "vlc_mushrooms:mushroom_brown",
	wood_slab    = "vlc_stairs:slab_wood",
	wood_stairs  = "vlc_stairs:stairs_wood",
	coal         = "vlc_core:coal_lump",
	shulker_box  = "vlc_chests:violet_shulker_box",
	quartz_block = "vlc_nether:quartz_block",
	banner       = "vlc_banners:banner_item_white",
	mesecon_conductor_craftable = "mesecons:wire_00000000_off",
	purpur_block = "vlc_end:purpur_block",
	normal_sandstone = "vlc_core:sandstone",
	red_sandstone = "vlc_core:redsandstone",
	compass      = vlc_compass.stereotype,
	clock        = vlc_clock.sterotype,
}
...
function vlc_craftguide.register_craft_type(name, def)
	local func = "vlc_craftguide.register_craft_type(): "
...
function vlc_craftguide.register_craft(def)
	local func = "vlc_craftguide.register_craft(): "
...
function vlc_craftguide.add_recipe_filter(name, f)
	local func = "vlc_craftguide.add_recipe_filter(): "
...
function vlc_craftguide.remove_recipe_filter(name)
	recipe_filters[name] = nil
end

function vlc_craftguide.set_recipe_filter(name, f)
	local func = "vlc_craftguide.set_recipe_filter(): "
	assert(name, func .. "filter name missing")
	assert(f and type(f) == "function", func .. "filter function missing")
...
function vlc_craftguide.get_recipe_filters()
...
function vlc_craftguide.add_search_filter(name, f)
	local func = "vlc_craftguide.add_search_filter(): "
	assert(name, func .. "filter name missing")
	assert(f and type(f) == "function", func .. "filter function missing")
...
function vlc_craftguide.remove_search_filter(name)
	search_filters[name] = nil
end

function vlc_craftguide.get_search_filters()
	return search_filters
end

local formspec_elements = {}

function vlc_craftguide.add_formspec_element(name, def)
	local func = "vlc_craftguide.add_formspec_element(): "
...
function vlc_craftguide.remove_formspec_element(name)
	formspec_elements[name] = nil
end

function vlc_craftguide.get_formspec_elements()
...
local function get_tooltip(item, groups, cooktime, burntime)
	...
	if groups then
		local gcol = vlc_colors.LIGHT_PURPLE
		if #groups == 1 then
	...
	if not groups and cooktime then
		tooltip = tooltip .. "\n" ..
			S("Cooking time: @1", C(vlc_colors.YELLOW, cooktime))
	end

	if not groups and burntime then
		tooltip = tooltip .. "\n" ..
			S("Burning time: @1", C(vlc_colors.YELLOW, burntime))
	end
...
local function get_recipe_fs(data, iY, player)
	vlc_inventory.reset_craft_grid(player)
	local fs = {}
	...
	if recipe.type == "fuel" then
		...
		if vlc_inventory.get_recipe_groups(pinv, recipe) and
			( vlc_crafting_table.has_crafting_table(player) or
...
local function make_formspec(name)
	vlc_inventory.reset_craft_grid(minetest.get_player_by_name(name))
	...
	fs[#fs + 1] = "background9[1,1;1,1;vlc_base_textures_background9.png;true;7]"
...
vlc_craftguide.make_formspec = make_formspec

local function show_fs(player, name)
	minetest.show_formspec(name, "vlc_craftguide", make_formspec(name))
end

vlc_craftguide.add_search_filter("groups", function(item, groups)
...
local function on_receive_fields(player, fields)
	...
	elseif fields.craft_inv and fields.craft_inv == "craft" then
		local pinv = player:get_inventory()
		if not vlc_inventory.get_recipe_groups(pinv, data.recipes[data.rnum]) then return end
		if vlc_crafting_table.has_crafting_table(player) then
		...
		vlc_inventory.to_craft_grid(player, data.recipes[data.rnum])
	else
		local item
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname == "vlc_craftguide" then
		on_receive_fields(player, fields)
	elseif fields.__vlc_craftguide then
		vlc_craftguide.show(player:get_player_name())
	end
end)
...
if progressive_mode then
	...
	vlc_craftguide.add_recipe_filter("Default progressive filter", progressive_filter)
...
function vlc_craftguide.show(name)
