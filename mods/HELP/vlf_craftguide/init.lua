...
local S = minetest.get_translator("vlf_craftguide")
...
local group_stereotypes = {
	wood         = "vlf_trees:wood_oak",
	stone        = "vlf_core:stone",
	sand         = "vlf_core:sand",
	wool         = "vlf_wool:white",
	carpet       = "vlf_wool:white_carpet",
	dye          = "vlf_dyes:red",
	water_bucket = "vlf_buckets:bucket_water",
	flower	     = "vlf_flowers:dandelion",
	mushroom     = "vlf_mushrooms:mushroom_brown",
	wood_slab    = "vlf_stairs:slab_wood",
	wood_stairs  = "vlf_stairs:stairs_wood",
	coal         = "vlf_core:coal_lump",
	shulker_box  = "vlf_chests:violet_shulker_box",
	quartz_block = "vlf_nether:quartz_block",
	banner       = "vlf_banners:banner_item_white",
	mesecon_conductor_craftable = "mesecons:wire_00000000_off",
	purpur_block = "vlf_end:purpur_block",
	normal_sandstone = "vlf_core:sandstone",
	red_sandstone = "vlf_core:redsandstone",
	compass      = vlf_compass.stereotype,
	clock        = vlf_clock.sterotype,
}
...
function vlf_craftguide.register_craft_type(name, def)
	local func = "vlf_craftguide.register_craft_type(): "
...
function vlf_craftguide.register_craft(def)
	local func = "vlf_craftguide.register_craft(): "
...
function vlf_craftguide.add_recipe_filter(name, f)
	local func = "vlf_craftguide.add_recipe_filter(): "
...
function vlf_craftguide.remove_recipe_filter(name)
	recipe_filters[name] = nil
end

function vlf_craftguide.set_recipe_filter(name, f)
	local func = "vlf_craftguide.set_recipe_filter(): "
	assert(name, func .. "filter name missing")
	assert(f and type(f) == "function", func .. "filter function missing")
...
function vlf_craftguide.get_recipe_filters()
...
function vlf_craftguide.add_search_filter(name, f)
	local func = "vlf_craftguide.add_search_filter(): "
	assert(name, func .. "filter name missing")
	assert(f and type(f) == "function", func .. "filter function missing")
...
function vlf_craftguide.remove_search_filter(name)
	search_filters[name] = nil
end

function vlf_craftguide.get_search_filters()
	return search_filters
end

local formspec_elements = {}

function vlf_craftguide.add_formspec_element(name, def)
	local func = "vlf_craftguide.add_formspec_element(): "
...
function vlf_craftguide.remove_formspec_element(name)
	formspec_elements[name] = nil
end

function vlf_craftguide.get_formspec_elements()
...
local function get_tooltip(item, groups, cooktime, burntime)
	...
	if groups then
		local gcol = vlf_colors.LIGHT_PURPLE
		if #groups == 1 then
	...
	if not groups and cooktime then
		tooltip = tooltip .. "\n" ..
			S("Cooking time: @1", C(vlf_colors.YELLOW, cooktime))
	end

	if not groups and burntime then
		tooltip = tooltip .. "\n" ..
			S("Burning time: @1", C(vlf_colors.YELLOW, burntime))
	end
...
local function make_formspec(name)
	...
	fs[#fs + 1] = "background9[1,1;1,1;vlf_base_textures_background9.png;true;7]"
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	elseif fields.__vlf_craftguide then
		vlf_craftguide.show(player:get_player_name())
	end
end)
...
if progressive_mode then
	...
	vlf_craftguide.add_recipe_filter("Default progressive filter", progressive_filter)
...
function vlf_craftguide.show(name)
