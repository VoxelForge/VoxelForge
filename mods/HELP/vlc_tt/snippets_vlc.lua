...
tt.register_snippet(function(itemstring, _, itemstack)
	--local def = minetest.registered_items[itemstring]
	local s = ""
	local use = minetest.get_item_group(itemstring, "vlc_armor_uses")
	local pts = minetest.get_item_group(itemstring, "vlc_armor_points")
	if pts > 0 then
...
tt.register_snippet(function(itemstring)
	...
	if def._vlc_saturation and def._vlc_saturation > 0 then
		if s ~= "" then
			s = s .. "\n"
		end
		s = s .. S("Saturation points: +@1", string.format("%.1f", def._vlc_saturation))
	end
	if s == "" then
		s = nil
	end
...
tt.register_snippet(function(itemstring)
	...
	if minetest.get_item_group(itemstring, "crush_after_fall") == 1 then
		return S("Deals damage when falling"), vlc_colors.YELLOW
	end
end)
...
tt.register_snippet(function(itemstring)
	if itemstring == "vlc_heads:zombie" then
		return S("Zombie view range: -50%")
	elseif itemstring == "vlc_heads:skeleton" then
		return S("Skeleton view range: -50%")
	elseif itemstring == "vlc_heads:creeper" then
		return S("Creeper view range: -50%")
	end
end)

tt.register_snippet(function(itemstring, _, itemstack)
	if itemstring:sub(1, 23) == "vlc_fishing:fishing_rod" or itemstring:sub(1, 12) == "vlc_bows:bow" then
		return S("Durability: @1", S("@1 uses", vlc_util.calculate_durability(itemstack or ItemStack(itemstring))))
	end
end)
