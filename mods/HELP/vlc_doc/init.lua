...
doc.sub.items.register_factoid(nil, "groups", function(itemstring, def)
	...
	if def._vlc_saturation and def._vlc_saturation > 0 then
		s = s .. "\n" .. S("Saturation points restored: @1%", string.format("%.1f", def._vlc_saturation))
	end
	return s
end)
...
doc.sub.items.register_factoid("nodes", "mining", function(itemstring, def)
	...
	local hardness = def._vlc_hardness
	...
	local blast = def._vlc_blast_resistance
	if not blast then
		blast = 0
	end
...
doc.sub.items.register_factoid("nodes", "drops", function(itemstring, def)
	if def._vlc_shears_drop == true then
		return S("This block drops itself when mined by shears.")
	elseif type(def._vlc_shears_drop) == "table" then
		local drops = {}
		for d=1, #def._vlc_shears_drop do
			local item = ItemStack(def._vlc_shears_drop[d])
