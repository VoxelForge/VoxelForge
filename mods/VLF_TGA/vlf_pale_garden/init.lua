	...
	action = function(pos, node)
		...
		if (time_of_day >= 0.8 or time_of_day <= 0.2) and not has_spawned then
			...
			if spawn_pos and not minetest.is_protected(spawn_pos, "") then
				local mob = vlf_mobs.spawn(spawn_pos, "vlf_pale_garden:creaking_transient")
				if mob then
...
vlf_flowerpots.register_potted_flower("vlf_pale_garden:closed_eyeblossom", {
...
vlf_flowerpots.register_potted_flower("vlf_pale_garden:open_eyeblossom", {
...
minetest.register_abm({
    ...
    nodenames = {"vlf_flowerpots:flower_pot_open_eyeblossom"},
...
minetest.register_entity("vlf_pale_garden:eyeblossom_pot_emissive", {
	initial_properties = {
        ...
        on_step = function(self, dtime)
            local pos = self.object:get_pos()
            local node = minetest.get_node(pos)
            if node.name ~= "vlf_flowerpots:flower_pot_open_eyeblossom" then
                self.object:remove()
                return
            end
        end,
