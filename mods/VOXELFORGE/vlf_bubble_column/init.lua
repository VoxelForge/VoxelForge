...
minetest.register_abm({
    ...
    action = function(pos, node)
        ...
        if above.name ~= "mcl_core:water_source" and above_pos_node.name ~= "air"  then return end
    ...
    for hash, dir in pairs(positions) do
        if math.random(1, 17) == 1 then
            ...
            if check_bubble_column(pos, node) then
                ...
                if particle_node.name == "mcl_core:water_source" then
            ...
            while true do
                ...
                if below.name == "mcl_nether:soul_sand" or below.name == "mcl_nether:magma" then
                    if node.name == "mcl_core:water_source" then
                        local dir = below.name == "mcl_nether:soul_sand" and 1 or -1
                        local speed = below.name == "mcl_nether:soul_sand" and 1.2 or 1.5
                        ...
                        if head_node.name == "mcl_core:water_source" and entity:is_player() then
                            entity:add_velocity({x = 0, y = dir * (speed*1.7), z = 0})
                        elseif head_node.name == "mcl_core:water_source" and not entity:is_player() then
                    ...
                    break
                elseif below.name ~= "mcl_core:water_source" then
                    break
                end
