...
function mcl_credits.show(player)
	local name = player:get_player_name()
	if mcl_credits.players[name] then return end
	...
	table.insert(huds.ids, player:hud_add({
		type = "image",
		text = "credits_bg.png",
		position = {x = 0, y = 0},
		alignment = {x = 1, y = 1},
	...
	local y = 300
	add_hud_element({ type = "image", text = "voxelforge_logo.png", scale = {x = 1, y = 1} }, huds, y)
	add_hud_element({ type = "text", text = mcl_credits.description, number = 0x757575, scale = {x = 5, y = 5} }, huds, y + 50)
	y = 450
	for _, group in ipairs(mcl_credits.people) do
		add_hud_element({ type = "text", text = group[1], number = group[2], scale = {x = 3, y = 3} }, huds, y)
		y = y + 25
		for _, name in ipairs(group[3]) do
			y = y + 25
			add_hud_element({ type = "text", text = name, number = 0xFFFFFF, scale = {x = 1, y = 1} }, huds, y)
		end
		y = y + 200
	end

	huds.icon = add_hud_element({ type = "image", text = "voxelforge_icon.png", scale = {x = 1, y = 1} }, huds, y)
	mcl_credits.players[name] = huds
end
...
minetest.register_globalstep(function(_)
	for name, huds in pairs(mcl_credits.players) do
		local player = huds.player
		local control = player:get_player_control()
