...
local function create_mesh_node(nodeDef, param2)
...
local function create_node(idx, area, vContent, vParam2)
	...
	elseif nodeDrawtype == "mesh" then
		faces = create_mesh_node(nodeDef, vParam2[idx])
...
function meshport.create_mesh(p1, p2, path, name)
	...
	for idx in vArea:iterp(p1, p2) do
		...
		local faces = create_node(idx, vArea, vContent, vParam2)
