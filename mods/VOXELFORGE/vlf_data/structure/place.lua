...
dofile (modpath.."/structure/processors.lua")
vlf_structure_block = {}
...
local wallmounted_table = {2, 5, 3, 4}

-- Function to rotate param2 based on its table and rotation angle
local function rotate_param2(node, param2, rotation)
    local table_to_use
    if minetest.registered_nodes[node].paramtype2 ~= "wallmounted" and (param2 == 6 or param2 == 15 or param2 == 8 or param2 == 17) then
        table_to_use = up_orientation_table
    elseif minetest.registered_nodes[node].paramtype2 ~= "wallmounted" and (param2 == 4 or param2 == 13 or param2 == 10 or param2 == 19) then
        table_to_use = down_orientation_table
    elseif minetest.registered_nodes[node].paramtype2 ~= "wallmounted" and (param2 == 0 or param2 == 1 or param2 == 2 or param2 == 3) then
        table_to_use = direction_table
    else
    	table_to_use = wallmounted_table
    end
...
--[[function vlf_structure_block.place_schematic(pos, file_name, rotation, rotation_origin, binary, worldpath, include_entities, terrain_setting, processor)
    ...
    if terrain_setting == "terrain_matching" then
        ...
        if highest_pos then
            pos.y = highest_pos.y
            minp = vector.add(pos, vector.new(-schem_size.x, -schem_size.y, -schem_size.z))
			maxp = vector.add(pos, vector.new(2 * schem_size.x, 2 * schem_size.y, 2 * schem_size.z))
        end
    elseif terrain_setting == "rigid" then
        minp = vector.add(pos, vector.new(-schem_size.x, -schem_size.y, -schem_size.z))
        maxp = vector.add(pos, vector.new(2 * schem_size.x, 2 * schem_size.y, 2 * schem_size.z))
        if pos.y < 0 then
    ...
    for _, node in ipairs(schematic.nodes) do
        ...
        local rotated_param2 = rotate_param2(node.name, node.param2 or 0, rotation)
...
--[[function vlf_structure_block.place_schematic(pos, file_name, rotation, rotation_origin, binary, worldpath, include_entities, terrain_setting, processor)
	...
	elseif terrain_setting == "terrain_matching" then
...
-- Define the function to call after emergence
local function place_foundation(vm, pos, schematic_size, node_top, node_filler, node_stone, pr, pmin, pmax, center_pos)
                if not vm then
                    minetest.log("error", "VoxelManipulator is nil, cannot place foundation.")
                    return
                end
                vl_terraforming.clearance_vm(pos.x, pos.y, pos.z, schematic_size.x * 1 + 2, schematic_size.y + 4, schematic_size.z * 1 + 2, 0, node_top, {name = "air"}, pr)
                vl_terraforming.foundation_vm(vm, pos.x, pos.y - 1, pos.z, 
                    schematic_size.x * 1 + 3, -5, schematic_size.z * 1 + 3, 0, 
                    node_top, node_filler, node_stone, nil, pr, pmin, pmax)
                minetest.log("error", "Placing foundation at " .. minetest.pos_to_string(center_pos))
            end

function vlf_structure_block.place_schematic(pos, file_name, rotation, rotation_origin, binary, worldpath, include_entities, terrain_setting, processor, data)
    rotation = rotation or 0
    rotation_origin = rotation_origin or pos
    processor = processor or nil
    local b = mg_name ~= "v6" and minetest.registered_biomes[minetest.get_biome_name(minetest.get_biome_data(pos).biome)]
	local node_top    = b and b.node_top and { name = b.node_top } or { name = "mcl_core:dirt_with_grass" }
	...
	local pr = PcgRandom(minetest.hash_node_position(pos) + blockseed + 959)
    ...
    --local vm2 = VoxelManip()
    local vm2 = VoxelManip(minp, maxp)
    vm2:read_from_map(minp, maxp)
...
-- Ensure the foundation generation call is correctly made
--[[elseif terrain_setting == "rigid" then
    if pos.y > 0 then
        local center_pos = vector.add(pos, vector.new(schematic.size.x / 2, -1, schematic.size.z / 2))
        
        -- Check if 'vl_terraforming.foundation_vm' is available and call it with the correct parameters
        if vl_terraforming.foundation_vm then
            local pmin = vector.offset(pos, -math.floor((schematic_size.x-1)*0.5), -1, -math.floor((schematic_size.z-1)*0.5))
            local pmax = vector.offset(pmin, schematic_size.x-1, schematic_size.y-1, schematic_size.z-1)
            local gp = vector.offset(pmin, -1, -1, -1) -- base level
            
            -- Call the foundation creation function
            --vl_terraforming.foundation_vm(vm2, gp.x, gp.y - 1, gp.z, schematic_size.x * 2, -5, schematic_size.z * 2, 1, surface_mat, platform_mat, stone_mat, nil, pr)
            vl_terraforming.foundation_vm(vm, pos.x, pos.y - 1, pos.z, schematic_size.x * 2, -5, schematic_size.z * 2, 1, node_top, node_filler, node_stone, nil, pr, minp, maxp)
            minetest.log("error", "Placing foundation at " .. minetest.pos_to_string(center_pos))
        else
            minetest.log("error", "vl_terraforming.foundation_vm function not found.")
        end
    end
end]]
...
elseif terrain_setting == "rigid" then
    if pos.y > 0 then
        local center_pos = vector.add(pos, -vector.new(schem_size.x / 5, -0, schem_size.z / 5))
        
        if vl_terraforming.foundation_vm then
            local pmin = vector.offset(pos, -math.floor((schematic_size.x-1)*0.5), -1, -math.floor((schematic_size.z-1)*0.5))
            local pmax = vector.offset(pmin, schematic_size.x-1, schematic_size.y-1, schematic_size.z-1)

            -- Ensure the area is loaded before calling the function
            --[[minetest.emerge_area(pmin, pmax, function(blockpos, action, calls_remaining)
                if action == minetest.EMERGE_ERRORED then
                    minetest.log("error", "Error emerging block at " .. minetest.pos_to_string(blockpos))
                elseif calls_remaining == 0 then
                    place_foundation(vm2, center_pos, schematic_size, node_top, node_filler, node_stone, pr, pmin, pmax, center_pos)
                end
            end)]]
            place_foundation(vm2, center_pos, schem_size, node_top, node_filler, node_stone, pr, pmin, pmax, center_pos)
        else
            minetest.log("error", "vl_terraforming.foundation_vm function not found.")
        end
    end
    ...
    -- Set all node metas in the "terrain_matching" setting
    if terrain_setting == "terrain_matching" then
        for _, node_group in pairs(nodes_by_type) do
            local positions = node_group.positions
            local total_positions = #positions
    ...
    -- Ensure that the generate meta is set to true for "voxelforge:jigsaw" nodes
    for node_pos, meta_data in pairs(metadata) do
        local meta = minetest.get_meta(node_pos)
        if minetest.get_node(node_pos).name == "voxelforge:jigsaw" then
            meta:set_string("generate", "true")
    ...
    -- Set metadata for all other nodes
    for _, node_pos in ipairs(constructed_nodes) do
        local node = minetest.get_node(node_pos)
        if minetest.registered_nodes[node.name] and minetest.registered_nodes[node.name].on_construct then
            minetest.registered_nodes[node.name].on_construct(node_pos)
