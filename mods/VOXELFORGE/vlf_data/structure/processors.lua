...
function processors.generic_processor(json_path, pos, node)
    ...
    if not data then
        -- Load JSON from file
        local fpath = minetest.get_modpath("vlf_data") .. "/data/voxelforge/worldgen/processor_list/" .. json_path .. ".json"
    ...
    for _, processor in ipairs(data.processors) do
		if processor.processor_type == "minecraft:block_rot" and processor.integrity then
            local integrity = processor.integrity

            -- Prevent removing jigsaw blocks
            if node.name ~= "voxelforge:jigsaw" then
                local rand_value = rand:random()
                if rand_value < integrity then
                    -- Block is removed (return nil to keep original terrain)
                    return "rotted"
                end
            end
        elseif processor.processor_type == "minecraft:rule" and processor.rules then
