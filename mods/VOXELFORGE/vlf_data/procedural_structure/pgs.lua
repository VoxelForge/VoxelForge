local mpath = minetest.get_modpath("vlf_data")
...
function vlf_procedural_structures.spawn_struct(pos)
    ...
    for _, element_entry in ipairs(elements) do
        ...
        if element.type == "minecraft:empty" then
            minetest.log("error", "Skipping empty element.")
            jigsaw_type = "minecraft:empty"
            return
        end
        if element.type == "minecraft:legacy_single_pool_element" then
			jigsaw_type = "minecraft:legacy_single_pool_element"
        end
		if element.type == "minecraft:single_pool_element" then
			jigsaw_type = "minecraft:single_pool_element"
        end

        -- Handle single_pool_element and legacy_single_pool_element
        if not element.location then
            ...
            if found_matching_node then
                table.insert(valid_schematics, {schematic = selecting_schematic, weight = weight, type = element.type})
                total_weight = total_weight + weight
            end
        end
    end
    ...
    while #valid_schematics > 0 do
        ...
        local valid_target_positions = {}
        for _, node in ipairs(schematic_data.nodes) do
            if node.metadata and node.metadata.name == target then
                table.insert(valid_target_positions, {pos = node.pos, param2 = node.param2})
            end
        end

        if #valid_target_positions == 0 then
            return
        end

        -- Use RNG to randomly pick a target block
        local random_index = rng:random(1, #valid_target_positions)
        local selected_target = valid_target_positions[random_index]
        target_pos = selected_target.pos
        target_param2 = selected_target.param2
        ...
        if vlf_structure_block.get_bounding_box(placement_pos, selected_schematic, rot, target_pos, "true", false) == "good" then
            vlf_structure_block.place_schematic(placement_pos, selected_schematic, rot, target_pos, "true", false, true, projection, processor, jigsaw_type)
            --minetest.log("error", "placed schematic: " .. selected_schematic .. "projection: " .. projection)
            minetest.set_node(pos, {name = final_state})
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	vlf_procedural_structures.spawn_struct(pos)
    end
end)
