...
--[[minetest.register_on_generated(function(minp, maxp, seed)
    ...
    for x = minp.x, maxp.x do
        for z = minp.z, maxp.z do
            ...
            local continentalness_value = perlin:noise2d(x, z, 2, 1, 200)
...
minetest.register_chatcommand("generate_terrain", {
    params = "",
    description = "Generate terrain from X 0-128, Z 0-128, across all Y levels",
    func = function(name)
        local minp = {x = 0, y = 0, z = 0} -- Full Y range
        local maxp = {x = 128, y = 300, z = 128} 
        ...
        for x = minp.x, maxp.x do
            for z = minp.z, maxp.z do
                local continentalness_value = perlin:noise2d(x, z, 1, 3)
                --local erosion_value = erosion_perlin:noise2d(x, z, 5, 0.1, 100)
                --local peaks_and_valleys_value = peaks_perlin:noise2d(x, z, 6, 1, 200)
                --local continentalness_value = perlin.fbm(x, 100, z, 9, 0.6, 1, 300)
                ...
                for y = minp.y, maxp.y do
                    --local weirdness_value = weirdness_perlin:fractal3d(x, y, z, 2, 0.5, 10, {1,1,1}, 9)
                    --weirdness_value = math.max(-1, math.min(1, weirdness_value))
                    --local density = weirdness_value + (y * density_adjustment)
                    local vi = area:index(x, y, z)
        ...
        vm:set_data(data)
        vm:write_to_map()
        minetest.chat_send_player(name, "Terrain generation complete!")
    end
})
...
--[[minetest.register_globalstep(function()
    for _, player in ipairs(minetest.get_connected_players()) do
        ...
        -- Get the continentalness, erosion, and peaks values at the player's position
            ...
            --local continentalness_value = perlin:fractal2d(x, z, 1, 1, 50)
            --local continentalness_value = perlin.fbm(x, 0, z, -9, 100, 100, 100)
            local continentalness_value = perlin.fbm(x, 100, z, 9, 0.6, 1, 300)
            local erosion_value = perlin.fbm(x, 0, z, 1, 1, 1, 1)
            local peaks_and_valleys_value = perlin.fbm(x, 0, z, 1, 1, 1, 1)
            --local erosion_value = erosion_perlin:fractal2d(x, z, 5, 0.1, 100)
            --local peaks_and_valleys_value = peaks_perlin:fractal2d(x, z, 6, 1, 200)
