...
local S = minetest.get_translator("vlf_potato_cutter")
...
function vlf_potato_cutter.refresh_recipes()
	recipes = {}
	yields = {}
	for result,resultdef in pairs(minetest.registered_items) do
		if resultdef._vlf_potato_cutter_recipes then
			local yield = 1
			for k,v in pairs(recipe_yield) do if minetest.get_item_group(result,k) > 0 then yield = yield * v end end
			for _,recipe in pairs(resultdef._vlf_potato_cutter_recipes) do
...
minetest.register_on_mods_loaded(vlf_potato_cutter.refresh_recipes)
...
local function show_stonecutter_formspec(input)
	...
	"size[11.75,10.425]"..
	"label[0.375,0.375;" .. F(C(vlf_formspec.label_color, S("Stonecutter"))) .. "]"..

	vlf_formspec.get_itemslot_bg_v4(1.375,1.5,1,1,0)..
...
minetest.register_node(":voxelforge:potato_cutter", {
	description = S("Potato Cutter"),
	tiles = {
		"potato_cutter_top.png",
		"potato_cutter_bottom.png",
		"potato_cutter_side.png",
		"potato_cutter_side.png",
		{name="potato_cutter_saw.png",
		animation={
		...
		if fields then
			for k, _ in pairs(fields) do
				if tostring(k) then
					...
					if def and def._vlf_potato_cutter_recipes then
