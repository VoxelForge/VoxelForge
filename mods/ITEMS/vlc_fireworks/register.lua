...
local function register_rocket(n, duration, force)
	minetest.register_craftitem("vlc_fireworks:rocket_" .. n, {
		description = description,
		_tt_help = tt_help .. " " .. duration,
		inventory_image = "vlc_fireworks_rocket.png",
		on_use = function(itemstack, user, pointed_thing)
			local elytra = vlc_player.players[user].elytra
			if elytra.active and elytra.rocketing <= 0 then
				elytra.rocketing = duration
				if not minetest.is_creative_enabled(user:get_player_name()) then
					itemstack:take_item()
				end
				minetest.sound_play("vlc_fireworks_rocket", {pos = user:get_pos()})
			end
			return itemstack
		end,
	})
...
minetest.register_alias("vlc_bows:rocket", "vlc_fireworks:rocket_2")
