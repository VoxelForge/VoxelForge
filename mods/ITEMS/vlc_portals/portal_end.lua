...
vlc_portals.registered_on_beat_game = {}
function vlc_portals.register_on_beat_game(func)
	table.insert(vlc_portals.registered_on_beat_game, func)
end
-- Parameters
--local SPAWN_MIN = vlc_vars.mg_end_min+70
--local SPAWN_MAX = vlc_vars.mg_end_min+98
...
local function destroy_portal(pos)
	...
	for n=1, #neighbors do
		...
		if minetest.get_node(npos).name == "vlc_portals:portal_end" then
...
-- End portal
minetest.register_node("vlc_portals:portal_end", {
	...
	tiles = {
		{
			name = "vlc_portals_end_portal.png",
			animation = {
...
local function check_end_portal_frame(pos)
	for i = 1, 12 do
		...
		for j = 1, 12 do
			...
			if not node or node.name ~= "vlc_portals:end_portal_frame_eye" or node.param2 ~= ep_scheme[j].p then
				portal = false
				break
			end
		end
...
local function show_credits(player)
	...
	if completed_end == 0 then
		...
		for _, func in ipairs(vlc_portals.registered_on_beat_game) do
			func(player)
		end
		vlc_credits.show(player)
	end
end

function vlc_portals.end_teleport(obj, pos)
	...
	if dim == "end" then
		...
		if obj:is_player() then
			target = vlc_spawn.get_player_spawn_pos(obj)
		end

		target = target or vlc_spawn.get_world_spawn_pos(obj)
	else
		-- End portal in any other dimension:
		-- Teleport to the End at a fixed position.
		-- The destination is built by vlc_structures.

		local platform_pos = vlc_vars.mg_end_platform_pos
	...
	if obj:is_player() then
		...
		vlc_worlds.dimension_change(obj, vlc_worlds.pos_to_dimension(target))
		minetest.sound_play("vlc_portals_teleport", {pos=target, gain=0.5, max_hear_distance = 16}, true)
	else
...
function vlc_portals.end_portal_teleport(pos, node)
	for _,obj in pairs(minetest.get_objects_inside_radius(pos, 1)) do
		...
		if obj:is_player() or lua_entity then
			...
			if minetest.get_node(objpos).name ~= "vlc_portals:portal_end" then
				return
			end

			vlc_portals.end_teleport(obj, objpos)
			awards.unlock(obj:get_player_name(), "vlc:enterEndPortal")
		end
	end
end
...
minetest.register_abm({
	...
	nodenames = {"vlc_portals:portal_end"},
	interval = 0.1,
	chance = 1,
	action = vlc_portals.end_portal_teleport,
})
...
local function after_place_node(pos, placer, itemstack, pointed_thing)
	...
	if node then
		...
		if ok then
			...
			minetest.sound_play("vlc_portals_open_end_portal", {gain=0.8}, true)
			end_portal_area(ppos)
		end
	end
end

minetest.register_node("vlc_portals:end_portal_frame", {
	...
	tiles = { "vlc_portals_endframe_top.png", "vlc_portals_endframe_bottom.png", "vlc_portals_endframe_side.png" },
	...
	_vlc_blast_resistance = 36000000,
	_vlc_hardness = -1,
})

minetest.register_node("vlc_portals:end_portal_frame_eye", {
	...
	tiles = { "vlc_portals_endframe_top.png^[lowpart:75:vlc_portals_endframe_eye.png", "vlc_portals_endframe_bottom.png", "vlc_portals_endframe_eye.png^vlc_portals_endframe_side.png" },
	use_texture_alpha = minetest.features.use_texture_alpha_string_modes and "opaque" or false,
	paramtype2 = "facedir",
	drawtype = "nodebox",
	node_box = {
...
if has_doc then
	doc.add_entry_alias("nodes", "vlc_portals:end_portal_frame", "nodes", "vlc_portals:end_portal_frame_eye")
end
...
-- Portal opener
local old_on_place = minetest.registered_items["vlc_end:ender_eye"].on_place
minetest.override_item("vlc_end:ender_eye", {
	on_place = function(itemstack, user, pointed_thing)
		...
		if pointed_thing.under and node.name == "vlc_portals:end_portal_frame" then
			...
			minetest.set_node(pointed_thing.under, { name = "vlc_portals:end_portal_frame_eye", param2 = node.param2 })

			if has_doc then
				doc.mark_entry_as_revealed(user:get_player_name(), "nodes", "vlc_portals:end_portal_frame")
			end
			minetest.sound_play(
			...
			if ok then
				...
				end_portal_area(ppos)
				if has_doc then
					doc.mark_entry_as_revealed(user:get_player_name(), "nodes", "vlc_portals:portal_end")
				end
			end
