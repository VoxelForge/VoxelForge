...
local storage = vlc_portals.storage
...
local path_gateway_portal = minetest.get_modpath("vlc_schematics").."/schems/vlc_structures_end_gateway_portal.mts"

local function spawn_gateway_portal(pos, dest_str)
	return vlc_structures.place_schematic(vector.add(pos, vector.new(-1, -2, -1)), path_gateway_portal, "0", nil, true, nil, dest_str and function()
		minetest.get_meta(pos):set_string("vlc_portals:gateway_destination", dest_str)
	end)
end

function vlc_portals.spawn_gateway_portal()
...
local gateway_def = table.copy(minetest.registered_nodes["vlc_portals:portal_end"])
...
minetest.register_node("vlc_portals:portal_gateway", gateway_def)
	...
	for y = maxp.y, minp.y, -1 do
		for x = maxp.x, minp.x, -1 do
			for z = maxp.z, minp.z, -1 do
				...
				if nn ~= "ignore" and nn ~= "vlc_portals:portal_gateway" and nn ~= "vlc_core:bedrock" then
...
local function teleport(pos, obj)
	...
	local dest_portal
	local dest_str = meta:get_string("vlc_portals:gateway_destination")
	...
	if dest_str == "" then
		...
		meta:set_string("vlc_portals:gateway_destination", minetest.pos_to_string(dest_portal))
	else
...
minetest.register_abm({
	...
	nodenames = {"vlc_portals:portal_gateway"},
	interval = 0.1,
	chance = 1,
	action = function(pos)
		...
		for _, obj in pairs(minetest.get_objects_inside_radius(pos, 1)) do
			if obj:get_hp() > 0 then
				...
				if luaentity and luaentity.name == "vlc_throwing:ender_pearl" then
					obj:remove()
