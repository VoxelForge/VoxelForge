...
local overlay = vlf_enchanting.overlay
local hud = "vlf_shield_hud.png"

minetest.register_tool("vlf_shields:shield", {
	description = S("Shield"),
	_doc_items_longdesc = S("A shield is a tool used for protecting the player against attacks."),
	inventory_image = "vlf_shield.png",
	stack_max = 1,
	groups = {
		shield = 1,
		weapon = 1,
...
function vlf_shields.wielding_shield(obj, i)
	return wielded_item(obj, i):find("vlf_shields:shield")
end

local function shield_is_enchanted(obj, i)
	return vlf_enchanting.is_enchanted(wielded_item(obj, i))
end

minetest.register_entity("vlf_shields:shield_entity", {
	initial_properties = {
		visual = "mesh",
		mesh = "vlf_shield.obj",
		physical = false,
		...
		textures = {"vlf_shield_base_nopattern.png"},
	...
	on_step = function(self, dtime, moveresult)
		...
		local shield_texture = "vlf_shield_base_nopattern.png"
		local i = self._shield_number
		local item = wielded_item(player, i)

		if item ~= "vlf_shields:shield" and item ~= "vlf_shields:shield_enchanted" then
			local itemstack = player:get_wielded_item()
			if i == 1 then
				itemstack = player:get_inventory():get_stack("offhand", 1)
			end
			local meta_texture = itemstack:get_meta():get_string("vlf_shields:shield_custom_pattern_texture")
			if meta_texture ~= "" then
				shield_texture = meta_texture
			else
				local color = minetest.registered_items[item]._shield_color
				if color then
					shield_texture = "vlf_shield_base_nopattern.png^(vlf_shield_pattern_base.png^[colorize:" .. color .. ")"
				end
			end
		end
...
for _, e in pairs(vlf_shields.enchantments) do
	vlf_enchanting.enchantments[e].secondary.shield = true
end

function vlf_shields.is_blocking(obj)
	if not obj:is_player() then return end
	if vlf_shields.players[obj] then
		local blocking = vlf_shields.players[obj].blocking
		if blocking <= 0 then
			return
		end
...
vlf_damage.register_modifier(function(obj, damage, reason)
	...
	local blocking, shieldstack = vlf_shields.is_blocking(obj)

	if not (obj:is_player() and blocking and vlf_shields.types[type] and damager) then
		return
	end
	...
	local unbreaking = vlf_enchanting.get_enchantment(shieldstack, vlf_shields.enchantments[2])
	...
	minetest.sound_play({name = "vlf_block"}, {pos = obj:get_pos(), max_hear_distance = 16})
	return 0
end)
...
local function modify_shield(player, vpos, vrot, i)
	...
	local shield = vlf_shields.players[player].shields[i]
	if shield then
...
local function set_interact(player, interact)
	...
	if interact and meta:get_int("vlf_shields:interact_revoked") ~= 0 then
		meta:set_int("vlf_shields:interact_revoked", 0)
		privs.interact = true
	elseif not interact then
		meta:set_int("vlf_shields:interact_revoked", privs.interact and 1 or 0)
		privs.interact = nil
	end
...
local function add_shield_entity(player, i)
	local shield = minetest.add_entity(player:get_pos(), "vlf_shields:shield_entity")
	if shield and shield:get_pos() then
		shield:get_luaentity()._shield_number = i
		vlf_shields.players[player].shields[i] = shield
		set_shield(player, false, i)
	end
end

local function remove_shield_entity(player, i)
	local shields = vlf_shields.players[player].shields
	if shields[i] then
		shields[i]:remove()
		shields[i] = nil
	end
...
local function handle_blocking(player)
	local player_shield = vlf_shields.players[player]
	...
	local shield_in_offhand = vlf_shields.wielding_shield(player, 1)
	local shield_in_hand = vlf_shields.wielding_shield(player)
	...
	elseif shield_in_offhand then
		local pointed_thing = vlf_util.get_pointed_thing(player, true)
...
local function update_shield_entity(player, blocking, i)
	local shield = vlf_shields.players[player].shields[i]
	if vlf_shields.wielding_shield(player, i) then
		if not shield then
...
local function add_shield_hud(shieldstack, player, blocking)
	local texture = hud
	if vlf_enchanting.is_enchanted(shieldstack:get_name()) then
...
minetest.register_globalstep(function(dtime)
	for _, player in pairs(minetest.get_connected_players()) do
		...
		local blocking, shieldstack = vlf_shields.is_blocking(player)
...
minetest.register_on_leaveplayer(function(player)
	shield_hud[player] = nil
	vlf_shields.players[player] = nil
end)

minetest.register_craft({
	output = "vlf_shields:shield",
	recipe = {
		{"group:wood", "vlf_core:iron_ingot", "group:wood"},
...
for colorkey, colortab in pairs(vlf_banners.colors) do
	local color = colortab[1]
	minetest.register_tool("vlf_shields:shield_" .. color, {
		description = shield_colors[colorkey],
		_doc_items_longdesc = S("A shield is a tool used for protecting the player against attacks."),
		inventory_image = "vlf_shield.png^(vlf_shield_item_overlay.png^[colorize:" .. colortab[4] ..")",
		stack_max = 1,
		groups = {
			shield = 1,
			weapon = 1,
	...
	local banner = "vlf_banners:banner_item_" .. color
	minetest.register_craft({
		type = "shapeless",
		output = "vlf_shields:shield_" .. color,
		recipe = {"vlf_shields:shield", banner},
	})
	minetest.register_craft({
		type = "shapeless",
		output = "vlf_shields:shield_" .. color .. "_enchanted",
		recipe = {"vlf_shields:shield_enchanted", banner},
	})
end

local function to_shield_texture(banner_texture)
	return banner_texture
	:gsub("vlf_banners_base_inverted.png", "vlf_shield_base_nopattern.png^vlf_shield_pattern_base.png")
	:gsub("vlf_banners_banner_base.png", "vlf_shield_base_nopattern.png^vlf_shield_pattern_base.png")
	:gsub("vlf_banners_base", "vlf_shield_pattern_base")
...
local function craft_banner_on_shield(itemstack, player, old_craft_grid, craft_inv)
	...
	for i = 1, player:get_inventory():get_size("craft") do
		...
		if string.find(banner_name, "vlf_banners:banner") and shield_stack then
			...
			if layers_meta ~= "" then
				local color = vlf_banners.color_reverse(banner_name)
				local layers = minetest.deserialize(layers_meta)
				local texture = vlf_banners.make_banner_texture(color, layers)
				new_shield_meta:set_string("description", vlf_banners.make_advanced_banner_description(itemstack:get_description(), layers))
				new_shield_meta:set_string("vlf_shields:shield_custom_pattern_texture", to_shield_texture(texture))
			end
