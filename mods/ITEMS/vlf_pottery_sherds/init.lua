...
for name, def in pairs(vlf_pottery_sherds.defs) do
	minetest.register_craftitem("vlf_pottery_sherds:"..name, {
		description = S("@1 Pottery Sherd", def.description),
		_tt_help = S("Used for crafting decorated pots"),
		_doc_items_create_entry = false,
		inventory_image = "vlf_pottery_sherds_"..name..".png",
		wield_image = "vlf_pottery_sherds_"..name..".png",
		groups = { pottery_sherd = 1, decorated_pot_recipe = 1 },
		_vlf_pottery_sherd_name = name,
	})
end

local brick_groups = table.copy(minetest.registered_items["vlf_core:brick"].groups)
brick_groups["decorated_pot_recipe"] = 1
minetest.override_item("vlf_core:brick", { groups = brick_groups })
...
local function update_entities(pos,rm)
	...
	for k,v in pairs(pot_face_positions) do
		...
		if face then
			local o = minetest.add_entity(pos + v, "vlf_pottery_sherds:pot_face")
			local e = o:get_luaentity()
			e.texture = "vlf_pottery_sherds_pattern_"..face..".png"
			o:set_properties({
				textures = { "vlf_pottery_sherds_pattern_"..face..".png" },
			})
...
minetest.register_entity("vlf_pottery_sherds:pot_face",{
	initial_properties = {
		physical = false,
	...
	on_step = function(self)
		if minetest.get_node(self.object:get_pos()).name ~= "vlf_pottery_sherds:pot" then
			self.object:remove()
		end
	end
})
...
local function get_sherd_desc(face)
	if face == nil then
		return minetest.registered_items["vlf_core:brick"].description
	end
...
minetest.register_node("vlf_pottery_sherds:pot", {
	...
	tiles = {
		...
		{ name = "vlf_pottery_sherds_pot_side.png", align_style = "world" },
	},
...
local function get_sherd_name(itemstack)
	...
	local r = nil
	if def and def._vlf_pottery_sherd_name then
		r = def._vlf_pottery_sherd_name
	end
	return r
end
