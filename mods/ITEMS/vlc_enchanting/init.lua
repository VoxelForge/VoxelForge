...
minetest.register_chatcommand("enchant", {
	...
	func = function(_, param)
		...
		local can_enchant, errorstring, extra_info = vlc_enchanting.can_enchant(itemstack, enchantment, level)
		if not can_enchant then
			...
			elseif errorstring == "incompatible" then
				return false, S("@1 can't be combined with @2.", vlc_enchanting.get_enchantment_description(enchantment, level), extra_info)
			end
		else
			target:set_wielded_item(vlc_enchanting.enchant(itemstack, enchantment, level))
...
minetest.register_chatcommand("forceenchant", {
	...
	func = function(_, param)
		...
		local itemstack = target:get_wielded_item()
		local _, errorstring = vlc_enchanting.can_enchant(itemstack, enchantment, level)
		if errorstring == "enchantment invalid" then
			return false, S("There is no such enchantment '@1'.", enchantment)
		elseif errorstring == "item missing" then
			return false, S("The target doesn't hold an item.")
		elseif errorstring == "item not supported" and not vlc_enchanting.is_enchantable(itemstack:get_name()) then
...
minetest.register_craftitem("vlc_enchanting:book_enchanted", {
	description = S("Enchanted Book"),
	inventory_image = "vlc_enchanting_book_enchanted.png" .. vlc_enchanting.overlay,
	groups = {enchanted = 1, not_in_creative_inventory = 1, enchantability = 1},
	_vlc_enchanting_enchanted_tool = "vlc_enchanting:book_enchanted",
	stack_max = 1,
})

minetest.register_alias("vlc_books:book_enchanted", "vlc_enchanting:book_enchanted")
...
local function spawn_book_entity(pos, respawn)
	if respawn then
		...
		for o=1, #objs do
			...
			if lua and lua.name == "vlc_enchanting:book" then
	...
	local obj = minetest.add_entity(vector.add(pos, vlc_enchanting.book_offset), "vlc_enchanting:book")
	if obj then
		local lua = obj:get_luaentity()
		if lua then
...
minetest.register_entity("vlc_enchanting:book", {
	initial_properties = {
		visual = "mesh",
		mesh = "vlc_enchanting_book.b3d",
		visual_size = {x = 12.5, y = 12.5},
		collisionbox = {0, 0, 0},
		pointable = false,
		physical = false,
		textures = {"vlc_enchanting_book_entity.png", "vlc_enchanting_book_entity.png", "vlc_enchanting_book_entity.png", "vlc_enchanting_book_entity.png", "vlc_enchanting_book_entity.png"},
		static_save = false,
	},
	_player_near = false,
	_table_pos = nil,
	on_activate = function(self, staticdata)
		...
		vlc_enchanting.set_book_animation(self, "close")
	end,
	on_step = function(self, dtime)
		local old_player_near = self._player_near
		local player_near = false
		local player
		for _, obj in pairs(minetest.get_objects_inside_radius(vector.subtract(self.object:get_pos(), vlc_enchanting.book_offset), 2.5)) do
		...
		if player_near and not old_player_near then
			vlc_enchanting.set_book_animation(self, "opening")
			vlc_enchanting.schedule_book_animation(self, "open")
		elseif old_player_near and not player_near then
			vlc_enchanting.set_book_animation(self, "closing")
			vlc_enchanting.schedule_book_animation(self, "close")
		end
		if player then
			vlc_enchanting.look_at(self, player:get_pos())
		end
		self._player_near = player_near
		vlc_enchanting.check_animation_schedule(self, dtime)
	end,
})
...
minetest.register_node("vlc_enchanting:table", {
	...
	drawtype = "nodebox",
	tiles = {"vlc_enchanting_table_top.png",  "vlc_enchanting_table_bottom.png", "vlc_enchanting_table_side.png", "vlc_enchanting_table_side.png", "vlc_enchanting_table_side.png", "vlc_enchanting_table_side.png"},
	...
	on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
		...
		--local num_bookshelves = table_meta:get_int("vlc_enchanting:num_bookshelves")
		local table_name = minetest.get_meta(pos):get_string("name")
		if table_name == "" then
			table_name = S("Enchant")
		end
		local bookshelves = vlc_enchanting.get_bookshelves(pos)
		player_meta:set_int("vlc_enchanting:num_bookshelves", math.min(15, #bookshelves))
		player_meta:set_string("vlc_enchanting:table_name", table_name)
		vlc_enchanting.show_enchanting_formspec(clicker)
	...
	after_dig_node = function(pos, oldnode, oldmetadata, digger)
		...
		local itemstack = ItemStack("vlc_enchanting:table")
...
minetest.register_craft({
	output = "vlc_enchanting:table",
	recipe = {
		{"", "vlc_books:book", ""},
		{"vlc_core:diamond", "vlc_core:obsidian", "vlc_core:diamond"},
...
minetest.register_abm({
	...
	interval = 1,
	chance = 1,
	nodenames = "vlc_enchanting:table",
	action = function(pos)
		local playernames = {}
		...
		local absolute, relative = vlc_enchanting.get_bookshelves(pos)
		for i, ap in ipairs(absolute) do
			if math.random(5) == 1 then
				...
				local tx = "vlc_enchanting_glyph_" .. math.random(18) .. ".png"
...
minetest.register_lbm({
	...
	name = "vlc_enchanting:spawn_book_entity",
	nodenames = {"vlc_enchanting:table"},
...
minetest.register_on_mods_loaded(vlc_enchanting.initialize)
minetest.register_on_joinplayer(vlc_enchanting.initialize_player)
minetest.register_on_player_receive_fields(vlc_enchanting.handle_formspec_fields)
minetest.register_allow_player_inventory_action(vlc_enchanting.allow_inventory_action)
minetest.register_on_player_inventory_action(vlc_enchanting.on_inventory_action)
tt.register_priority_snippet(vlc_enchanting.enchantments_snippet)
