...
vlc_enchanting.enchantments.bane_of_arthropods = {
...
--[[vlc_enchanting.enchantments.channeling = {
	name = S("Channeling"),
	max_level = 1,
...
-- implemented in vlc_death_drop
vlc_enchanting.enchantments.curse_of_vanishing = {
...
-- implemented below
vlc_enchanting.enchantments.depth_strider = {
...
vlc_player.register_globalstep_slow(function(player, dtime)
	if minetest.get_item_group(vlc_player.players[player].nodes.feet, "liquid") ~= 0 and vlc_enchanting.get_enchantment(player:get_inventory():get_stack("armor", 5), "depth_strider") then
		local boots = player:get_inventory():get_stack("armor", 5)
		local depth_strider = vlc_enchanting.get_enchantment(boots, "depth_strider")

		if depth_strider > 0 then
			playerphysics.add_physics_factor(player, "speed", "vlc_playerplus:depth_strider", (depth_strider / 3) + 0.75)
		end
	else
		playerphysics.remove_physics_factor(player, "speed", "vlc_playerplus:depth_strider")
	end
end)

-- implemented via on_enchant
vlc_enchanting.enchantments.efficiency = {
	name = S("Efficiency"),
	max_level = 5,
...
-- implemented in vlc_mobs and via register_on_punchplayer callback
vlc_enchanting.enchantments.fire_aspect = {
	name = S("Fire Aspect"),
	max_level = 2,
	primary = {sword = true},
	secondary = {},
...
minetest.register_on_punchplayer(function(player, hitter, time_from_last_punch, tool_capabilities, dir, damage)
	if hitter and hitter:is_player() then
		...
		if wielditem then
			local fire_aspect_level = vlc_enchanting.get_enchantment(wielditem, "fire_aspect")
			if fire_aspect_level > 0 then
				vlc_burning.set_on_fire(player, fire_aspect_level * 4)
			end
		end
	end
end)

vlc_enchanting.enchantments.flame = {
	name = S("Flame"),
	max_level = 1,
	primary = {bow = true},
	secondary = {},
...
-- implemented in vlc_item_entity
vlc_enchanting.enchantments.fortune = {
	name = S("Fortune"),
	max_level = 3,
	primary = {pickaxe = true, shovel = true, axe = true, hoe = true},
	secondary = {},
...
vlc_enchanting.enchantments.frost_walker = {
	name = S("Frost Walker"),
	max_level = 2,
	primary = {},
	secondary = {armor_feet = true},
...
walkover.register_global(function(pos, _, player)
	local boots = player:get_inventory():get_stack("armor", 5)
	local frost_walker = vlc_enchanting.get_enchantment(boots, "frost_walker")
	...
	local positions = minetest.find_nodes_in_area_under_air(minp, maxp, "vlc_core:water_source")
	for _, p in ipairs(positions) do
		if vector.distance(pos, p) <= radius then
			minetest.set_node(p, {name = "vlc_core:frosted_ice_0"})
		end
	end
end)

-- requires missing MineClone2 feature
--[[vlc_enchanting.enchantments.impaling = {
	name = S("Impaling"),
	max_level = 5,
	primary = {trident = true},
	secondary = {},
...
-- implemented in vlc_bows
vlc_enchanting.enchantments.infinity = {
	name = S("Infinity"),
	max_level = 1,
	primary = {bow = true},
	secondary = {},
...
vlc_enchanting.enchantments.knockback = {
	name = S("Knockback"),
	max_level = 2,
	primary = {sword = true},
	secondary = {},
...
function minetest.calculate_knockback(player, hitter, time_from_last_punch, tool_capabilities, dir, distance, damage)
	...
	if hitter and hitter:is_player() then
		local wielditem = hitter:get_wielded_item()
		knockback = knockback + 3 * vlc_enchanting.get_enchantment(wielditem, "knockback")
...
-- implemented in vlc_mobs and mobs_mc
vlc_enchanting.enchantments.looting = {
	name = S("Looting"),
	max_level = 3,
	primary = {sword = true},
	secondary = {},
...
-- requires missing MineClone2 feature
--[[vlc_enchanting.enchantments.loyalty = {
	name = S("Loyalty"),
	max_level = 3,
	primary = {trident = true},
	secondary = {},
...
-- implemented in vlc_fishing
vlc_enchanting.enchantments.luck_of_the_sea = {
...
-- implemented in vlc_fishing
vlc_enchanting.enchantments.lure = {
	name = S("Lure"),
	max_level = 3,
	primary = {fishing_rod = true},
	secondary = {},
...
-- implemented in vlc_experience
vlc_enchanting.enchantments.mending = {
	name = S("Mending"),
	max_level = 1,
	primary = {},
	secondary = {armor_head = true, armor_torso = true, armor_legs = true, armor_feet = true, tool = true, weapon = true},
...
vlc_experience.register_on_add_xp(function(player, xp)
	...
	for _, can in ipairs(candidates) do
		...
		if vlc_enchanting.has_enchantment(stack, "mending") and wear > 0 then
			can.stack = stack
			can.wear = wear
	...
	if #final_candidates > 0 then
		...
		local uses = vlc_util.calculate_durability(stack)
...
vlc_enchanting.enchantments.multishot = {
	name = S("Multishot"),
	max_level = 1,
...
-- requires missing MineClone2 feature
vlc_enchanting.enchantments.piercing = {
	name = S("Piercing"),
	max_level = 4,
	primary = {crossbow = true},
	secondary = {},
...
-- implemented in vlc_bows
vlc_enchanting.enchantments.power = {
	name = S("Power"),
	max_level = 5,
	primary = {bow = true},
	secondary = {},
...
-- implemented via minetest.calculate_knockback (together with the Knockback enchantment) and vlc_bows
vlc_enchanting.enchantments.punch = {
	name = S("Punch"),
	max_level = 2,
	primary = {},
	secondary = {bow = true},
...
-- requires missing MineClone2 feature
vlc_enchanting.enchantments.quick_charge = {
	name = S("Quick Charge"),
	max_level = 3,
	primary = {crossbow = true},
	secondary = {},
...
-- unimplemented
--[[vlc_enchanting.enchantments.respiration = {
	name = S("Respiration"),
	max_level = 3,
...
-- requires missing MineClone2 feature
--[[vlc_enchanting.enchantments.riptide = {
	name = S("Riptide"),
	max_level = 3,
	primary = {trident = true},
	secondary = {},
...
-- implemented via on_enchant
vlc_enchanting.enchantments.sharpness = {
	name = S("Sharpness"),
	max_level = 5,
	primary = {sword = true},
	secondary = {axe = true},
...
-- implemented in vlc_item_entity
vlc_enchanting.enchantments.silk_touch = {
	name = S("Silk Touch"),
	max_level = 1,
	primary = {pickaxe = true, shovel = true, axe = true, hoe = true},
	secondary = {shears = true},
...
vlc_enchanting.enchantments.smite = {
	name = S("Smite"),
	max_level = 5,
	primary = {sword = true},
	secondary = {axe = true},
...
-- implemented in vlc_playerplus
vlc_enchanting.enchantments.soul_speed = {
	name = S("Soul Speed"),
	max_level = 3,
	primary = {},
	secondary = {armor_feet = true},
...
-- requires missing MineClone2 feature
--[[vlc_enchanting.enchantments.sweeping_edge = {
...
-- for tools & weapons implemented via on_enchant; for bows implemented in vlc_bows; for armor implemented in vlc_armor and vlc_tt; for fishing rods implemented in vlc_fishing
vlc_enchanting.enchantments.unbreaking = {
	name = S("Unbreaking"),
	max_level = 3,
