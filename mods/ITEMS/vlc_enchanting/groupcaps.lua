...
local function get_efficiency_groupcaps(toolname, level)
	...
	if not levelcache then
		levelcache = {}
		levelcache.values = vlc_autogroup.get_groupcaps(toolname, level)
...
function vlc_enchanting.update_groupcaps(itemstack)
	local name = itemstack:get_name()
	if not minetest.registered_tools[name] or not minetest.registered_tools[name].tool_capabilities then
		return
	end

	local efficiency = vlc_enchanting.get_enchantment(itemstack, "efficiency")
