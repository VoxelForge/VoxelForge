...
local has_vlf_portals = minetest.get_modpath("vlf_portals")
...
local function spawn_fire(pos, age)
	minetest.set_node(pos, {name="vlf_fire:fire", param2 = age})
	minetest.check_single_for_falling({x=pos.x, y=pos.y+1, z=pos.z})
end

minetest.register_node("vlf_fire:fire", {
	description = S("Fire"),
	...
	on_construct = function(pos)
		...
		local dim = vlf_worlds.pos_to_dimension(bpos)
		if under == "vlf_nether:magma" or under == "vlf_nether:netherrack" or (under == "vlf_core:bedrock" and dim == "end") then
			minetest.swap_node(pos, {name = "vlf_fire:eternal_fire"})
		end

		if has_vlf_portals then
...
minetest.register_node("vlf_fire:eternal_fire", {
	...
	on_construct = function(pos)
		if has_vlf_portals then --Calling directly minetest.get_modpath consumes 4x more compute time
			vlf_portals.light_nether_portal(pos)
		end
	end,
	sounds = {},
...
function vlf_fire.update_player_sound(player)
	...
	local fpos, num = minetest.find_nodes_in_area(
		areamin,
		areamax,
		{"vlf_fire:fire", "vlf_fire:eternal_fire"}
	)
	-- Total number of flames in radius
	local flames = (num["vlf_fire:fire"] or 0) +
		(num["vlf_fire:eternal_fire"] or 0)
	-- Stop previous sound
...
minetest.register_abm({
	...
	nodenames = {"vlf_fire:fire", "vlf_fire:eternal_fire"},
...
if not fire_enabled then
	...
	minetest.register_abm({
		...
		nodenames = {"vlf_core:lava_source","vlf_nether:nether_lava_source"},
...
function vlf_fire.set_fire(pointed_thing, player, allow_on_fire)
	local pname
	if player == nil then
		pname = ""
	else
	...
	minetest.set_node(pointed_thing.above, {name="vlf_fire:fire"})
end

minetest.register_alias("vlf_fire:basic_flame", "vlf_fire:fire")
minetest.register_alias("fire:basic_flame", "vlf_fire:fire")
minetest.register_alias("fire:permanent_flame", "vlf_fire:eternal_fire")
