...
local function on_bone_meal(itemstack,placer,pointed_thing,pos,node)
	return vlc_farming.on_bone_meal(itemstack,placer,pointed_thing,pos,node,"plant_potato")
end
...
for i=1, 7 do
	...
	if i < 3 then
		texture = "vlc_farming_potatoes_stage_0.png"
		selbox = { -5/16, -0.5 ,-5/16, 5/16, -0.5+(3/16) ,5/16 }
	elseif i < 5 then
		texture = "vlc_farming_potatoes_stage_1.png"
		selbox = { -6/16, -0.5 ,-6/16, 6/16, -0.5+(4/16) ,6/16 }
	else
		texture = "vlc_farming_potatoes_stage_2.png"
	...
	if i==1 then
		...
		if minetest.get_modpath("doc") then
			doc.add_entry_alias("nodes", "vlc_farming:potato_1", "nodes", "vlc_farming:potato_"..i)
		end
	end

	minetest.register_node("vlc_farming:potato_"..i, {
...
-- Mature plant
minetest.register_node("vlc_farming:potato", {
	...
	walkable = false,
	drawtype = "plantlike",
	tiles = {"vlc_farming_potatoes_stage_3.png"},
	wield_image = "vlc_farming_potatoes_stage_3.png",
	inventory_image = "vlc_farming_potatoes_stage_3.png",
	drop = {
		items = {
			{ items = {"vlc_farming:potato_item 1"} },
			{ items = {"vlc_farming:potato_item 1"}, rarity = 2 },
			{ items = {"vlc_farming:potato_item 1"}, rarity = 2 },
			{ items = {"vlc_farming:potato_item 1"}, rarity = 2 },
			{ items = {"vlc_farming:potato_item_poison 1"}, rarity = 50 }
		}
	},
	selection_box = {
		type = "fixed",
...
minetest.register_craftitem("vlc_farming:potato_item", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		local new = vlc_farming:place_seed(itemstack, placer, pointed_thing, "vlc_farming:potato_1")
		if new then
			return new
		else
...
minetest.register_craftitem("vlc_farming:potato_item_baked", {
...
minetest.register_craftitem("vlc_farming:potato_item_poison", {
	description = S("Poisonous Potato"),
	_tt_help = minetest.colorize(vlc_colors.YELLOW, S("60% chance of poisoning")),
...
minetest.register_craft({
	type = "cooking",
	output = "vlc_farming:potato_item_baked",
	recipe = "vlc_farming:potato_item",
	cooktime = 10,
})

vlc_farming:add_plant("plant_potato", "vlc_farming:potato", {"vlc_farming:potato_1", "vlc_farming:potato_2", "vlc_farming:potato_3", "vlc_farming:potato_4", "vlc_farming:potato_5", "vlc_farming:potato_6", "vlc_farming:potato_7"}, 19.75, 20)

minetest.register_on_item_eat(function (hp_change, replace_with_item, itemstack, user, pointed_thing)

	-- 60% chance of poisoning with poisonous potato
	if itemstack:get_name() == "vlc_farming:potato_item_poison" then
		if math.random(1,10) >= 6 then
			vlc_potions.poison_func(user, 1, 5)
		end
	end
