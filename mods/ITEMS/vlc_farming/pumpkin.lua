...
local function on_bone_meal(itemstack,placer,pointed_thing,pos,node)
	return vlc_farming.on_bone_meal(itemstack,placer,pointed_thing,pos,node,"plant_pumpkin_stem")
end
...
local function carve_pumpkin(itemstack, placer, pointed_thing)
	...
	if not minetest.is_creative_enabled(placer:get_player_name()) then
		...
		local wear = vlc_autogroup.get_wear(toolname, "shearsy")
		itemstack:add_wear(wear)
	end
	minetest.sound_play({name="default_grass_footstep", gain=1}, {pos = pointed_thing.above}, true)
	local dir = vector.subtract(pointed_thing.under, pointed_thing.above)
	local param2 = minetest.dir_to_facedir(dir)
	minetest.set_node(pointed_thing.under, {name="vlc_farming:pumpkin_face", param2 = param2})
	minetest.add_item(pointed_thing.above, "vlc_farming:pumpkin_seeds 4")
	return itemstack, true
end

-- Seeds
minetest.register_craftitem("vlc_farming:pumpkin_seeds", {
	...
	inventory_image = "vlc_farming_pumpkin_seeds.png",
	groups = {craftitem=1, compostability = 30},
	on_place = function(itemstack, placer, pointed_thing)
		return vlc_farming:place_seed(itemstack, placer, pointed_thing, "vlc_farming:pumpkin_1")
	end
})
...
local stem_drop = {
	...
	items = {
		...
		{ items = {"vlc_farming:pumpkin_seeds 1"}, rarity = 6 },

		-- 2 seeds: Approximation to 4/125 chance
		-- 4/125 = 0.032
		-- Approximation: 1/31 = ca. 0.032258064516129
		{ items = {"vlc_farming:pumpkin_seeds 2"}, rarity = 31 },

		-- 3 seeds: 1/125 chance
		{ items = {"vlc_farming:pumpkin_seeds 3"}, rarity = 125 },
	},
}
...
for s=1,7 do
	...
	local texture = "([combine:16x16:0,"..((8-s)*2).."=vlc_farming_pumpkin_stem_disconnected.png)^[colorize:"..colorstring..":127"
	minetest.register_node("vlc_farming:pumpkin_"..s, {
...
local stem_def = {
	...
	tiles = {"vlc_farming_pumpkin_stem_disconnected.png^[colorize:#FFA800:127"},
	wield_image = "vlc_farming_pumpkin_stem_disconnected.png^[colorize:#FFA800:127",
	inventory_image = "vlc_farming_pumpkin_stem_disconnected.png^[colorize:#FFA800:127",
}
...
pumpkin_face_base_def._vlc_armor_mob_range_factor = 0
pumpkin_face_base_def._vlc_armor_mob_range_mob = "mobs_mc:enderman"

pumpkin_face_base_def._vlc_armor_element = "head"
pumpkin_face_base_def._vlc_armor_texture = "vlc_farming_pumpkin_face.png"
...
if minetest.get_modpath("vlc_armor") then
	local pumpkin_hud = {}
	local function add_pumpkin_hud(player)
		pumpkin_hud[player] = {
			pumpkin_blur = player:hud_add({
				...
				scale = {x = -101, y = -101},
				text = "vlc_farming_pumpkin_hud.png",
				z_index = -200
			}),
	...
	pumpkin_face_base_def.on_secondary_use = vlc_armor.equip_on_use
	pumpkin_face_base_def._on_equip = add_pumpkin_hud
	pumpkin_face_base_def._on_unequip = remove_pumpkin_hud

	minetest.register_on_joinplayer(function(player)
		if player:get_inventory():get_stack("armor", 2):get_name() == "vlc_farming:pumpkin_face" then
...
vlc_farming:add_plant("plant_pumpkin_stem", "vlc_farming:pumpkintige_unconnect", {"vlc_farming:pumpkin_1", "vlc_farming:pumpkin_2", "vlc_farming:pumpkin_3", "vlc_farming:pumpkin_4", "vlc_farming:pumpkin_5", "vlc_farming:pumpkin_6", "vlc_farming:pumpkin_7"}, 30, 5)

-- Register actual pumpkin, connected stems and stem-to-pumpkin growth
vlc_farming:add_gourd("vlc_farming:pumpkintige_unconnect", "vlc_farming:pumpkintige_linked", "vlc_farming:pumpkintige_unconnect", stem_def, stem_drop, "vlc_farming:pumpkin", pumpkin_base_def, 30, 15, "vlc_farming_pumpkin_stem_connected.png^[colorize:#FFA800:127")

-- Steal function to properly disconnect a carved pumpkin
pumpkin_face_base_def.after_destruct = minetest.registered_nodes["vlc_farming:pumpkin"].after_destruct
minetest.register_node("vlc_farming:pumpkin_face", pumpkin_face_base_def)

-- Jack o'Lantern
minetest.register_node("vlc_farming:pumpkin_face_light", {
...
minetest.register_craft({
	output = "vlc_farming:pumpkin_face_light",
	recipe = {{"vlc_farming:pumpkin_face"},
	{"vlc_torches:torch"}}
})

minetest.register_craft({
	output = "vlc_farming:pumpkin_seeds 4",
	recipe = {{"vlc_farming:pumpkin"}}
})

minetest.register_craftitem("vlc_farming:pumpkin_pie", {
	description = S("Pumpkin Pie"),
	_doc_items_longdesc = S("A pumpkin pie is a tasty food item which can be eaten."),
	inventory_image = "vlc_farming_pumpkin_pie.png",
	wield_image = "vlc_farming_pumpkin_pie.png",
...
minetest.register_craft({
	type = "shapeless",
	output = "vlc_farming:pumpkin_pie",
	recipe = {"vlc_farming:pumpkin", "vlc_core:sugar", "vlc_throwing:egg"},
})


if minetest.get_modpath("doc") then
	for i=2,8 do
		doc.add_entry_alias("nodes", "vlc_farming:pumpkin_1", "nodes", "vlc_farming:pumpkin_"..i)
	end
end
