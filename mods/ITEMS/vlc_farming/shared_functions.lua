...
function vlc_farming:add_plant(identifier, full_grown, names, interval, chance)
	vlc_farming.plant_lists[identifier] = {}
	vlc_farming.plant_lists[identifier].full_grown = full_grown
	vlc_farming.plant_lists[identifier].names = names
	vlc_farming.plant_lists[identifier].interval = interval
	vlc_farming.plant_lists[identifier].chance = chance
	plant_lists = vlc_farming.plant_lists --provide local copy of plant lists (performances)
	minetest.register_abm({
		...
		action = function(pos, node)
			local low_speed = minetest.get_node({ x = pos.x, y = pos.y - 1, z = pos.z }).name ~= "vlc_farming:soil_wet"
			vlc_farming:grow_plant(identifier, pos, node, false, false, low_speed)
		end,
	})
...
-- identifier: Identifier of plant as defined by vlc_farming:add_plant
-- pos: Position
-- node: Node table
...
function vlc_farming:grow_plant(identifier, pos, node, stages, ignore_light, low_speed)
...
function vlc_farming:place_seed(itemstack, placer, pointed_thing, plantname)
	...
	if string.find(farmland.name, "vlc_farming:soil") and string.find(place_s.name, "air") then
...
function vlc_farming:add_gourd(full_unconnected_stem, connected_stem_basename, stem_itemstring, stem_def, stem_drop, gourd_itemstring, gourd_def, grow_interval, grow_chance, connected_stem_texture, gourd_on_construct_extra)
	...
	if stem_def.sounds == nil then
		stem_def.sounds = vlc_sounds.node_sound_leaves_defaults()
	end
		...
		action = function(stempos)
			...
			if light and light > 10 then
				...
				for n = #neighbors, 1, -1 do
					...
					if not ((minetest.get_item_group(floor.name, "grass_block") == 1 or floor.name == "vlc_core:dirt" or soilgroup == 2 or soilgroup == 3) and block.name == "air") then
				...
				if #neighbors > 0 then
					...
					if minetest.get_item_group(floor.name, "dirtifies_below_solid") == 1 then
						minetest.set_node(floorpos, { name = "vlc_core:dirt" })
					end
				end
			end
		end,
...
function vlc_farming:stem_color(startcolor, endcolor, step, step_count)
	local color = {}
...
function vlc_farming:get_seed_or_eat_callback(plantname, hp_change)
	return function(itemstack, placer, pointed_thing)
		local new = vlc_farming:place_seed(itemstack, placer, pointed_thing, plantname)
		if new then
			return new
		else
...
function vlc_farming.on_bone_meal(itemstack,placer,pointed_thing,pos,n,plant,stages)
	local stages = stages or math.random(2, 5)
	return vlc_farming:grow_plant(plant, pos, n, stages, true)
end

minetest.register_lbm({
	label = "Add growth for unloaded farming plants",
	name = "vlc_farming:growth",
