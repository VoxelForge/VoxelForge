...
minetest.register_node("vlc_farming:soil", {
	tiles = {"vlc_farming_farmland_dry.png", "default_dirt.png"},
	...
	groups = {handy=1, shovely=1, dirtifies_below_solid=1, dirtifier=1, soil=2, soil_sapling=1, soil_fungus=1, deco_block=1 },
	sounds = vlc_sounds.node_sound_dirt_defaults(),
	_vlc_blast_resistance = 0.6,
	_vlc_hardness = 0.6,
})

minetest.register_node("vlc_farming:soil_wet", {
	tiles = {"vlc_farming_farmland_wet.png", "default_dirt.png"},
	...
	groups = {handy=1,shovely=1, not_in_creative_inventory=1, dirtifies_below_solid=1, dirtifier=1, soil=3, soil_sapling=1, soil_fungus=1 },
	sounds = vlc_sounds.node_sound_dirt_defaults(),
...
minetest.register_abm({
	...
	nodenames = {"vlc_farming:soil", "vlc_farming:soil_wet"},
	interval = 15,
	chance = 4,
	action = function(pos, node)
		...
		if check_surroundings(pos, "group:water") then
			if node.name ~= "vlc_farming:soil_wet" then
				-- Make it wet
				node.name = "vlc_farming:soil_wet"
				minetest.set_node(pos, node)
			end
			...
			if vlc_weather.rain.raining then
				if vlc_weather.is_outdoor(pos) then
					return
				end
			end
