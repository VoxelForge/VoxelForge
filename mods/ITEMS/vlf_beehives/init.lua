...
local abm_nodes = { "vlf_beehives:beehive", "vlf_beehives:bee_nest" }
...
local honey_harvest = function(pos, node, player, itemstack)
	...
	if node.name == "vlf_beehives:beehive_5" then
		original_block = "vlf_beehives:beehive"
	end
	...
	if bottle or shears then
		...
		if bottle then
			local honey = "vlf_honey:honey_bottle"
			if inv:room_for_item("main", honey) then
				inv:add_item("main", "vlf_honey:honey_bottle")
				if not is_creative then
					itemstack:take_item()
				end
				if campfire[1] then
					awards.unlock(player:get_player_name(), "vlf:bee_our_guest")
				end
			end
		else --Must be shears
			minetest.add_item(pos, "vlf_honey:honeycomb 3")
		end
		...
		if not campfire[1] then vlf_util.deal_damage(player, 10, {type = "mob"}) end
...
local dig_hive = function(pos, node, _, digger)
	...
	local beehive = string.find(node.name, "vlf_beehives:beehive")
	local beenest = string.find(node.name, "vlf_beehives:bee_nest")
	local silk_touch = vlf_enchanting.has_enchantment(wield_item, "silk_touch")
	local is_creative = minetest.is_creative_enabled(digger:get_player_name())
	local inv = digger:get_inventory()

	if beehive then
		if not is_creative then
			minetest.add_item(pos, "vlf_beehives:beehive")
			if not silk_touch then vlf_util.deal_damage(digger, 10, {type = "mob"}) end
		elseif is_creative and inv:room_for_item("main", "vlf_beehives:beehive") and not inv:contains_item("main", "vlf_beehives:beehive") then
			inv:add_item("main", "vlf_beehives:beehive")
		end
	elseif beenest then
		if not is_creative then
			if silk_touch and wield_item:get_name() ~= "vlf_enchanting:book_enchanted" then
				minetest.add_item(pos, "vlf_beehives:bee_nest")
				awards.unlock(digger:get_player_name(), "vlf:total_beelocation")
			else
				vlf_util.deal_damage(digger, 10, {type = "mob"})
			end
		elseif is_creative and inv:room_for_item("main", "vlf_beehives:bee_nest") and not inv:contains_item("main", "vlf_beehives:bee_nest") then
			inv:add_item("main", "vlf_beehives:bee_nest")
		end
	end
end

-- Beehive
minetest.register_node("vlf_beehives:beehive", {
	description = S("Beehive"),
	_doc_items_longdesc = S("Artificial bee nest."),
	tiles = {
		"vlf_beehives_beehive_end.png", "vlf_beehives_beehive_end.png",
		"vlf_beehives_beehive_side.png", "vlf_beehives_beehive_side.png",
		"vlf_beehives_beehive_side.png", "vlf_beehives_beehive_front.png",
	},
	paramtype2 = "facedir",
...
for l = 1, 4 do
	local name = "vlf_beehives:beehive_" .. l
	...
	minetest.register_node(name, {
		...
		_vlf_blast_resistance = 0.6,
		_vlf_hardness = 0.6,
		_vlf_baseitem = "vlf_beehives:beehive",
		drop = "",
		after_dig_node = dig_hive,
	})
end

minetest.register_node("vlf_beehives:beehive_5", {
	description = S("Beehive"),
	_doc_items_longdesc = S("Artificial bee nest."),
	tiles = {
		"vlf_beehives_beehive_end.png", "vlf_beehives_beehive_end.png",
		"vlf_beehives_beehive_side.png", "vlf_beehives_beehive_side.png",
		"vlf_beehives_beehive_side.png", "vlf_beehives_beehive_front_honey.png",
	},
	paramtype2 = "facedir",
...
-- Bee Nest
minetest.register_node("vlf_beehives:bee_nest", {
	...
	tiles = {
		"vlf_beehives_bee_nest_top.png", "vlf_beehives_bee_nest_bottom.png",
		"vlf_beehives_bee_nest_side.png", "vlf_beehives_bee_nest_side.png",
		"vlf_beehives_bee_nest_side.png", "vlf_beehives_bee_nest_front.png",
	},
	paramtype2 = "facedir",
...
for i = 1, 4 do
	local name = "vlf_beehives:bee_nest_"..i
	table.insert(abm_nodes, name)
	minetest.register_node(name, {
		description = S("Bee Nest"),
		_doc_items_longdesc = S("A naturally generating block that houses bees and a tasty treat...if you can get it."),
		...
		_vlf_blast_resistance = 0.3,
		_vlf_hardness = 0.3,
		_vlf_baseitem = "vlf_beehives:bee_nest",
		drop = "",
		after_dig_node = dig_hive,
	})
end

minetest.register_node("vlf_beehives:bee_nest_5", {
	description = S("Bee Nest"),
	_doc_items_longdesc = S("A naturally generating block that houses bees and a tasty treat...if you can get it."),
	tiles = {
		"vlf_beehives_bee_nest_top.png", "vlf_beehives_bee_nest_bottom.png",
		"vlf_beehives_bee_nest_side.png", "vlf_beehives_bee_nest_side.png",
		"vlf_beehives_bee_nest_side.png", "vlf_beehives_bee_nest_front_honey.png",
	},
	paramtype2 = "facedir",
...
minetest.register_craft({
	output = "vlf_beehives:beehive",
	recipe = {
		{ "group:wood", "group:wood", "group:wood" },
		{ "vlf_honey:honeycomb", "vlf_honey:honeycomb", "vlf_honey:honeycomb" },
		{ "group:wood", "group:wood", "group:wood" },
	},
})
...
minetest.register_abm({
	...
	action = function(pos, node)
		...
		if tod > 6000 and tod < 18000 and flower and vlf_weather.get_weather() ~= "rain" then
