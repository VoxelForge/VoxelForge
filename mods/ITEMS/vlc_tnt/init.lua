...
local explosions_griefing = minetest.settings:get_bool("vlc_explosions_griefing", true)
...
function tnt.ignite(pos)
	...
	local e = spawn_tnt(pos, "vlc_tnt:tnt")
...
minetest.register_node("vlc_tnt:tnt", {
	tiles = {
		"default_tnt_top.png",
...
function TNT:on_activate(_, _)
	...
	self.object:set_texture_mod("^vlc_tnt_blink.png")
end
...
--[[local function add_effects(pos, radius, drops)
	...
	local texture = "vlc_particles_smoke.png" --fallback texture
	local most = 0
...
function TNT:on_step(dtime, _)
	...
	if self.timer > tnt.BOOMTIMER then
		vlc_explosions.explode(self.object:get_pos(), 4, {}, self.object)
		self.object:remove()
	end
end

minetest.register_entity("vlc_tnt:tnt", TNT)

if minetest.get_modpath("vlc_mobitems") then
	minetest.register_craft({
		output = "vlc_tnt:tnt",
		recipe = {
			{ "vlc_mobitems:gunpowder", "group:sand", "vlc_mobitems:gunpowder" },
			{ "group:sand", "vlc_mobitems:gunpowder", "group:sand" },
			{ "vlc_mobitems:gunpowder", "group:sand", "vlc_mobitems:gunpowder" }
		},
	})
end
...
if minetest.get_modpath("doc_identifier") then
	doc.sub.identifier.register_object("vlc_tnt:tnt", "nodes", "vlc_tnt:tnt")
end
