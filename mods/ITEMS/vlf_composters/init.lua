...
minetest.register_craft({
	output = "vlf_composters:composter",
	recipe = {
...
local function composter_ready(pos)
	minetest.swap_node(pos, {name = "vlf_composters:composter_ready"})
...
local function composter_harvest(pos, _, player, itemstack, _)
	...
	minetest.swap_node(pos, {name="vlf_composters:composter"})
	minetest.add_item(pos, "vlf_bone_meal:bone_meal")
	return itemstack
end
...
function vlf_composters.test_composter (nn)
	...
	elseif nn == "vlf_composters:composter_1" then
		return 1
	elseif nn == "vlf_composters:composter_2" then
		return 2
	elseif nn == "vlf_composters:composter_3" then
		return 3
	elseif nn == "vlf_composters:composter_4" then
		return 4
	elseif nn == "vlf_composters:composter_5" then
		return 5
	elseif nn == "vlf_composters:composter_6" then
		return 6
...
for i = 1, 7 do
	assert(composter_level({name = "vlf_composters:composter_" .. i}) == i)
end

assert(composter_level({name = "vlf_composters:composter"}) == 0)
assert(composter_level({name = "vlf_composters:some_other_node"}) == nil)

local function on_hopper_out(uppos, pos)
	-- Get bonemeal from composter above
	local upnode = minetest.get_node(uppos)
	if upnode.name == "vlf_composters:composter_ready" then
		local meta = minetest.get_meta(pos)
		local inv = meta:get_inventory()

		minetest.swap_node(uppos, {name = "vlf_composters:composter"})

		inv:add_item("main", "vlf_bone_meal:bone_meal")
	end
end
		...
		for i = 1, 5 do
			...
			if compchance > 0 then
				...
				if compchance >= math.random(0, 100) then
					vlf_bone_meal.add_bone_meal_particle(vector.offset(downpos, 0, level / 8, 0))
					if level < 7 then
						level = level + 1
					else
						level = "ready" ---@diagnostic disable-line: cast-local-type
					end
					minetest.swap_node(downpos, {name = "vlf_composters:composter_" .. level})
				end
				break
			end
		end
...
--
minetest.register_node("vlf_composters:composter", {
	...
	tiles = {
		"vlf_composter_bottom.png^vlf_composter_top.png",
		"vlf_composter_bottom.png",
		"vlf_composter_side.png"
	},
...
local function register_filled_composter(level)
	local id = "vlf_composters:composter_"..level
	minetest.register_node(id, {
		...
		selection_box = {type = "regular"},
		tiles = {
			"vlf_composter_compost.png^vlf_composter_top.png",
			"vlf_composter_bottom.png",
			"vlf_composter_side.png"
		},
		is_ground_content = false,
		...
		_vlf_hardness = 0.6,
		_vlf_blast_resistance = 0.6,
		_vlf_compost_level = level,
		on_rightclick = composter_add_item,
		on_timer = composter_ready,
		_on_hopper_in = on_hopper_in,
		_vlf_baseitem = "vlf_composters:composter",
	})

	-- Add entry aliases for the Help
	if minetest.get_modpath("doc") then
		doc.add_entry_alias("nodes", "vlf_composters:composter", "nodes", id)
	end
end
...
--
minetest.register_node("vlf_composters:composter_ready", {
	...
	selection_box = {type = "regular"},
	tiles = {
		"vlf_composter_ready.png^vlf_composter_top.png",
		"vlf_composter_bottom.png",
		"vlf_composter_side.png"
	},
	is_ground_content = false,
...
-- Add entry aliases for the Help
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlf_composters:composter",
		"nodes", "vlf_composters:composter_ready" )
end
