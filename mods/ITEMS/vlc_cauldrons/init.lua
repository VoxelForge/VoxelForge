...
-- Empty cauldron
minetest.register_node("vlc_cauldrons:cauldron", {
	...
	wield_image = "vlc_cauldrons_cauldron.png",
	inventory_image = "vlc_cauldrons_cauldron.png",
	...
	tiles = {
		"vlc_cauldrons_cauldron_inner.png^vlc_cauldrons_cauldron_top.png",
		"vlc_cauldrons_cauldron_inner.png^vlc_cauldrons_cauldron_bottom.png",
		"vlc_cauldrons_cauldron_side.png"
	},
	...
	_on_bucket_place = function(itemstack,placer,pointed_thing)
		...
		local s
		if n == "water" then
			n = "vlc_cauldrons:cauldron_3"
			s = "vlc_core:water_source"
		elseif n == "river_water" then
			n = "vlc_cauldrons:cauldron_3r"
			s = "vlcx_core:river_water_source"
		elseif n == "lava" then
			n = "vlc_cauldrons:cauldron_3_lava"
			s = "vlc_core:lava_source"
		end
...
local function register_filled_cauldron(water_level, description, liquid)
	local id = "vlc_cauldrons:cauldron_"..water_level
	local water_tex
	...
	minetest.register_node(id, {
		...
		selection_box = { type = "regular" },
		tiles = {
			"("..water_tex..")^vlc_cauldrons_cauldron_top.png",
			"vlc_cauldrons_cauldron_inner.png^vlc_cauldrons_cauldron_bottom.png",
			"vlc_cauldrons_cauldron_side.png"
		},
		sounds = vlc_sounds.node_sound_metal_defaults(),
		drop = "vlc_cauldrons:cauldron",
		_vlc_hardness = 2,
		_vlc_blast_resistance = 2,
		_on_bucket_place_empty  = function(itemstack,placer,pointed_thing)
			local n,s
			if id == "vlc_cauldrons:cauldron_3" then
				n = "vlc_buckets:bucket_water"
				s = "vlc_core:water_source"
			elseif id == "vlc_cauldrons:cauldron_3r" then
				n = "vlc_buckets:bucket_river_water"
				s = "vlcx_core:river_water_source"
			elseif id == "vlc_cauldrons:cauldron_3_lava" then
				n = "vlc_buckets:bucket_lava"
				s = "vlc_core:lava_source"
			end
		...
		on_rightclick = function(pos, node, player, itemstack)
			local outcome = vlc_armor.wash_leather_armor(itemstack)
			if outcome then
				minetest.sound_play("vlc_potions_bottle_pour",
	...
	if minetest.get_modpath("doc") then
		doc.add_entry_alias("nodes", "vlc_cauldrons:cauldron", "nodes", id)
	end
end
...
local function cauldron_extinguish(obj,pos)
	...
	if vlc_burning.is_burning(obj) then
		...
		minetest.swap_node(pos, {name = "vlc_cauldrons:cauldron" .. (new_group == 0 and "" or "_" .. new_group)})
	end
end
...
minetest.register_globalstep(function(dtime)
	...
	for _,ent in pairs(minetest.luaentities) do
		if ent.object:get_pos() and ent.is_mob then
			...
			elseif n and minetest.get_node(n).name:find("lava") then
				vlc_burning.set_on_fire(ent.object, 5)
			end
		end
	end
end)
