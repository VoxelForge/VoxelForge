...
vlc_damage.register_modifier(function(obj, damage, reason)
	if obj:is_player() and not reason.flags.bypasses_totem then
		...
		if hp - damage <= 0 then
			...
			if wield:get_name() ~= "vlc_totems:totem_of_undying" then
			...
			if wield:get_name() == "vlc_totems:totem" then
				...
				awards.unlock(obj:get_player_name(), "vlc:postMortal")

				-- Effects
				minetest.sound_play({name = "vlc_totems_totem", gain = 1}, {pos=ppos, max_hear_distance = 16}, true)
				...
				for i = 1, 4 do
					for c = 1, #particle_colors do
						minetest.add_particlespawner({
								...
								vertical = false,
								texture = "vlc_particles_totem" .. i .. ".png^[colorize:#" .. particle_colors[c],
								glow = 10,
							})
					end
				end
