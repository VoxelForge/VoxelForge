...
local liquid_nodes = {
	water = "vlf_core:water_source",
	river_water = "vlfx_core:river_water_source",
	lava = "vlf_core:lava_source",
}

local buckets = {
	water = "vlf_buckets:bucket_water",
	river_water = "vlf_buckets:bucket_river_water",
	lava = "vlf_buckets:bucket_lava",
}

function vlf_cauldrons.get_cauldron_name(level, liquid)
	level = math.min(3, level)
	level = math.max(0, level)
	if level == 0 then return "vlf_cauldrons:cauldron" end
	return "vlf_cauldrons:cauldron_"..level..cauldron_ids[liquid or "water"]
end

function vlf_cauldrons.add_level(pos, amount, liquid)
	...
	local liquid = def and def._vlf_cauldrons_liquid or liquid
	if amount ~= 0 and liquid then
		...
		node.name = vlf_cauldrons.get_cauldron_name(water_level + amount, liquid)
...
local function bucket_place(itemstack,placer,pointed_thing)
	...
	local l = itemstack:get_definition()._vlf_buckets_liquid
	if def and l and ( def._vlf_cauldrons_liquid == l or core.get_item_group(name, "cauldron") == 1 ) then
		vlf_cauldrons.add_level(pointed_thing.under, 3, l)
...
-- Empty cauldron
core.register_node("vlf_cauldrons:cauldron", {
	...
	groups = {pickaxey=1, deco_block=1, cauldron=1, _vlf_partial=2},
...
local function register_filled_cauldron(water_level, description, liquid)
	local id = vlf_cauldrons.get_cauldron_name(water_level, liquid)
	local water_tex
	local light_level = 0
	...
	core.register_node(id, {
		...
		is_ground_content = false,
		groups = {pickaxey=1, not_in_creative_inventory=1, cauldron=(1+water_level), cauldron_filled=water_level, comparator_signal=water_level, cauldron_water = cauldron_water, _vlf_partial = 2},
		...
		_vlf_hardness = 2,
		_vlf_blast_resistance = 2,
		_vlf_cauldrons_liquid = liquid or "water",
		_vlf_baseitem = "vlf_cauldrons:cauldron",
		_on_bucket_place = bucket_place,
		_on_bucket_place_empty  = function(itemstack,placer,pointed_thing)
			local name = core.get_node(pointed_thing.under).name
			if core.get_item_group(name, "cauldron_filled") < 3 then return itemstack end
			vlf_cauldrons.add_level(pointed_thing.under, -3)
			if not core.is_creative_enabled(placer:get_player_name()) then
				local def = core.registered_nodes[name]
				if def and def._vlf_cauldrons_liquid and buckets[def._vlf_cauldrons_liquid] then
					itemstack:take_item()
					local inv = placer:get_inventory()
					inv:add_item("main", buckets[def._vlf_cauldrons_liquid])
				end
			end
			return itemstack
		end,
...
for i=1,3 do
	...
	if core.get_modpath("vlfx_core") then
...
local function cauldron_extinguish(obj,pos)
	...
	if vlf_burning.is_burning(obj) then
		...
		core.swap_node(pos, {name = "vlf_cauldrons:cauldron" .. (new_group == 0 and "" or "_" .. new_group)})
	end
end
