...
local absorb = function(pos)
		...
		for j=-3,3 do
			for k=-3,3 do
				...
				if minetest.get_item_group(n.name, "water") ~= 0 then
					...
					change = true
					if n.name == "vlcx_core:river_water_source" or n.name == "vlcx_core:river_water_flowing" then
	...
	if river_water > non_river_water then
		sponge_type = "vlc_sponges:sponge_wet_river_water"
	else
		sponge_type = "vlc_sponges:sponge_wet"
	end
	return change, sponge_type
end

minetest.register_node("vlc_sponges:sponge", {
...
function place_wet_sponge(itemstack, placer, pointed_thing)
	...
	if vlc_worlds.pos_to_dimension(pointed_thing.above) == "nether" then
		minetest.item_place_node(ItemStack("vlc_sponges:sponge"), placer, pointed_thing)
		local pos = pointed_thing.above
		...
		for n = 1, 5 do
			minetest.add_particlespawner({
				...
				vertical = false,
				texture = "vlc_particles_sponge" .. n .. ".png",
			})
		end
...
minetest.register_node("vlc_sponges:sponge_wet", {
	...
	drawtype = "normal",
	is_ground_content = false,
	tiles = {"vlc_sponges_sponge_wet.png"},
	sounds = vlc_sounds.node_sound_dirt_defaults(),
	groups = {handy=1, hoey=1, building_block=1},
...
if minetest.get_modpath("vlcx_core") then
	minetest.register_node("vlc_sponges:sponge_wet_river_water", {
		...
		drawtype = "normal",
		is_ground_content = false,
		tiles = {"vlc_sponges_sponge_wet_river_water.png"},
		sounds = vlc_sounds.node_sound_dirt_defaults(),
	...
	minetest.register_craft({
		type = "cooking",
		output = "vlc_sponges:sponge",
		recipe = "vlc_sponges:sponge_wet_river_water",
		cooktime = 10,
	})
end
...
minetest.register_craft({
	type = "cooking",
	output = "vlc_sponges:sponge",
	recipe = "vlc_sponges:sponge_wet",
	cooktime = 10,
})

minetest.register_abm({
	label = "Sponge water absorbtion",
	nodenames = { "vlc_sponges:sponge" },
