...
local function campfire_drops(pos, digger, drops, nodename)
	...
	if not minetest.is_creative_enabled(digger:get_player_name()) then
		if vlc_enchanting.has_enchantment(wield_item, "silk_touch") then
...
function vlc_campfires.light_campfire(pos)
...
function vlc_campfires.take_item(pos, node, player, itemstack)
	if minetest.get_item_group(itemstack:get_name(), "campfire_cookable") ~= 0 then
		...
		if cookable then
			...
			local o = minetest.add_entity(pos + campfire_spots[spot], "vlc_campfires:food_entity")
			o:set_properties({
...
function vlc_campfires.register_campfire(name, def)
	-- Define Campfire
	minetest.register_node(name, {
		...
		drawtype = "mesh",
		mesh = "vlc_campfires_campfire.obj",
		tiles = {{name="vlc_campfires_log.png"},},
		use_texture_alpha = "clip",
		groups = { handy=1, axey=1, material_wood=1, not_in_creative_inventory=1, campfire=1, },
		paramtype = "light",
		paramtype2 = "4dir",
		_on_ignite = function(player, node)
			vlc_campfires.light_campfire(node.under)
			return true
		end,
		_on_arrow_hit = function(pos, arrowent)
			if vlc_burning.is_burning(arrowent.object) then
				vlc_campfires.light_campfire(pos)
			end
		end,
		drop = "",
	...
	minetest.register_node(name.."_lit", {
		...
		on_rightclick = function (pos, node, player, itemstack, pointed_thing)
			if minetest.get_item_group(itemstack:get_name(), "shovel") ~= 0 then
				local protected = vlc_util.check_position_protection(pos, player)
			...
			elseif minetest.get_item_group(itemstack:get_name(), "campfire_cookable") ~= 0 then
				vlc_campfires.take_item(pos, node, player, itemstack)
		...
		_vlc_campfires_smothered_form = name,
	})
end
...
minetest.register_globalstep(function(dtime)
	...
	for _,pl in pairs(minetest.get_connected_players()) do
		...
		if pl and not pl:get_player_control().sneak and not vlc_enchanting.has_enchantment(armor_feet, "frost_walker") then
			burn_in_campfire(pl)
		end
	end
...
function vlc_campfires.generate_smoke(pos)
	local smoke_timer

	if minetest.get_node(vector.offset(pos, 0, -1, 0)).name == "vlc_farming:hay_block" then
		smoke_timer = 8
	else
		smoke_timer = 4.75
	end
	...
	for _,pl in pairs(minetest.get_connected_players()) do
		...
		if not player_particlespawners[pl][ph] and vector.distance(pos, pl:get_pos()) < PARTICLE_DISTANCE then
			player_particlespawners[pl][ph] = minetest.add_particlespawner({
				...
				vertical = true,
				texture = "vlc_campfires_particle_9.png",
				playername = pl:get_player_name(),
				texpool = {
					{ name = "vlc_campfires_particle_1.png" },
					{ name = "vlc_campfires_particle_2.png" },
					{ name = "vlc_campfires_particle_3.png" },
					{ name = "vlc_campfires_particle_4.png" },
					{ name = "vlc_campfires_particle_5.png" },
					{ name = "vlc_campfires_particle_6.png" },
					{ name = "vlc_campfires_particle_7.png" },
					{ name = "vlc_campfires_particle_8.png" },
					{ name = "vlc_campfires_particle_9.png" },
					{ name = "vlc_campfires_particle_10.png" },
					{ name = "vlc_campfires_particle_11.png" },
					{ name = "vlc_campfires_particle_11.png" },
					{ name = "vlc_campfires_particle_12.png" },
				}
			})
		end
	end
...
minetest.register_entity("vlc_campfires:food_entity", {
	initial_properties = {
		physical = false,
		visual = "wielditem",
...
minetest.register_abm({
	...
	interval = 2,
	chance = 2,
	action = vlc_campfires.generate_smoke,
})
