...
local function get_far_node(pos, itemstack) --code from minetest dev wiki: https://dev.minetest.net/minetest.get_node, some edits have been made to add a cooldown for force loads
	...
	if node.name == "ignore" then
		...
		if tonumber(os.time(os.date("!*t"))) - tstamp > 180 then --current time in secounds - old time in secounds, if it is over 180 (3 mins): forceload
			...
			node = {name="vlc_compass:lodestone"} --cooldown not over yet, pretend like there is something...
		end
	end
	return node
end
...
local function get_compass_frame(pos, dir, itemstack)
	if not string.find(itemstack:get_name(), "_lodestone") then -- normal compass
		...
		if vlc_worlds.compass_works(pos) then
		...
		if not lpos then
			minetest.log("warning", "vlc_compass: invalid lodestone position!")
			return random_frame
		end
		local _, l_dim = vlc_worlds.y_to_layer(lpos.y)
		local _, p_dim = vlc_worlds.y_to_layer(pos.y)
		-- compass and lodestone must be in the same dimension
		if l_dim == p_dim then
			--check if lodestone still exists
			if get_far_node(lpos, itemstack).name == "vlc_compass:lodestone" then
...
vlc_compass.stereotype = "vlc_compass:" .. stereotype_frame
...
--
function vlc_compass.get_compass_image(pos, dir)
	minetest.log("warning", "vlc_compass: deprecated function " ..
		"get_compass_image() called, use get_compass_itemname().")
	local itemstack = ItemStack(vlc_compass.stereotype)
...
-- itemstack: the compass including its optional lodestone metadata.
--
function vlc_compass.get_compass_itemname(pos, dir, itemstack)
	if not itemstack then
		minetest.log("warning", "vlc_compass.get_compass_image called without itemstack!")
		return "vlc_compass:" .. stereotype_frame
	end
	local frame = get_compass_frame(pos, dir, itemstack)
	if itemstack:get_meta():get_string("pointsto") ~= "" then
		return "vlc_compass:" .. frame .. "_lodestone"
	else
		return "vlc_compass:" .. frame
	end
end
			...
			if compass_nr ~= 0 and not string.find(stack:get_name(), "_recovery") then
				...
				if compass_nr - 1 ~= compass_frame then
					...
					if string.find(stack:get_name(), "_lodestone") then
						stack:set_name("vlc_compass:" .. compass_frame .. "_lodestone")
						awards.unlock(player:get_player_name(), "vlc:countryLode")
					else
						stack:set_name("vlc_compass:" .. compass_frame)
					end
					inv:set_stack("main", j, stack)
				end
			elseif compass_nr ~= 0 then
				local meta = player:get_meta()
				local posstring =  meta:get_string("vlc_compass:recovery_pos")
				if not posstring or posstring == "" then
					stack:set_name("vlc_compass:"..random_frame .. "_recovery")
				else
					local targetpos = minetest.string_to_pos(posstring)
					local _, target_dim = vlc_worlds.y_to_layer(targetpos.y)
					local _, p_dim = vlc_worlds.y_to_layer(pos.y)
					if p_dim ~= target_dim then
						stack:set_name("vlc_compass:"..random_frame.."_recovery")
					else
						stack:set_name("vlc_compass:"..get_compass_angle(pos,targetpos,dir).."_recovery")
					end
				end
				inv:set_stack("main",j,stack)
			end
...
for _, item in pairs(compass_types) do
	...
	if item.name == "compass" then
		name_fmt = "vlc_compass:%d"
		img_fmt = "vlc_compass_compass_%02d.png"
	elseif item.name == "compass_lodestone" then
		name_fmt = "vlc_compass:%d_lodestone"
		img_fmt = "vlc_compass_compass_%02d.png^[colorize:purple:50"
	elseif item.name == "compass_recovery" then
		name_fmt = "vlc_compass:%d_recovery"
		img_fmt = "vlc_compass_recovery_compass_%02d.png"
	end
	for i = 0, compass_frames - 1 do
		...
		if doc_mod and i ~= stereotype_frame then
			doc.add_entry_alias("craftitems", "vlc_compass:"..(stereotype_frame), "craftitems", itemstring)
		end
	end
end

minetest.register_craft({
	output = "vlc_compass:" .. stereotype_frame,
	recipe = {
		{"", "vlc_core:iron_ingot", ""},
...
minetest.register_craft({ --TODO: update once echo shards are a thing
	output = "vlc_compass:" .. random_frame .. "_recovery",
	recipe = {
		{"","vlc_nether:netherite_ingot",""},
		{"vlc_core:diamondblock","vlc_compass:" .. stereotype_frame ,"vlc_core:diamondblock"},
		{"vlc_core:diamondblock","vlc_core:diamondblock","vlc_core:diamondblock"}

	}
})

minetest.register_alias("vlc_compass:compass", "vlc_compass:" .. stereotype_frame)


minetest.register_node("vlc_compass:lodestone",{
	description=S("Lodestone"),
	on_rightclick = function(pos, node, player, itemstack)
		local name = itemstack.get_name(itemstack)
		if string.find(name,"vlc_compass:") then
			if name ~= "vlc_compass:lodestone" then
				itemstack:get_meta():set_string("pointsto", minetest.pos_to_string(pos))
				local dir = player:get_look_horizontal()
				local frame = get_compass_frame(pos, dir, itemstack)
				itemstack:set_name("vlc_compass:" .. frame .. "_lodestone")
			end
		end
	end,
	tiles = {
...
minetest.register_craft({
	output = "vlc_compass:lodestone",
	recipe = {
		{"vlc_core:stonebrickcarved","vlc_core:stonebrickcarved","vlc_core:stonebrickcarved"},
		{"vlc_core:stonebrickcarved", "vlc_nether:netherite_ingot", "vlc_core:stonebrickcarved"},
		{"vlc_core:stonebrickcarved", "vlc_core:stonebrickcarved", "vlc_core:stonebrickcarved"}
	}
})
...
minetest.register_on_dieplayer(function(player)
	local meta = player:get_meta();
	meta:set_string("vlc_compass:recovery_pos",minetest.pos_to_string(player:get_pos()))
end)
