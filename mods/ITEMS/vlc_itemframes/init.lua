vlc_itemframes = {}
vlc_itemframes.registered_nodes = {}
vlc_itemframes.registered_itemframes = {}
...
local function find_entity(pos)
	for _,o in pairs(minetest.get_objects_inside_radius(pos, 0.45)) do
		...
		if l and l.name == "vlc_itemframes:item" then
			return l
		end
	end
end
...
local function find_or_create_entity(pos)
	...
	if not l then
		l = minetest.add_entity(pos, "vlc_itemframes:item"):get_luaentity()
	end
	return l
end
...
vlc_itemframes.remove_entity = remove_entity
...
local function get_map_id(itemstack)
	local map_id = itemstack:get_meta():get_string("vlc_maps:id")
...
vlc_itemframes.update_entity = update_entity

--Node functions
function vlc_itemframes.tpl_node.on_rightclick(pos, node, clicker, ostack, pointed_thing)
...
vlc_itemframes.tpl_node.on_destruct = remove_entity

function vlc_itemframes.tpl_node.on_construct(pos)
...
-- Entity functions
function vlc_itemframes.tpl_entity:set_item(itemstack, pos)
	...
	if not ndef._vlc_itemframe then
		self.object:remove()
		update_entity()
		return
	end
	local def = vlc_itemframes.registered_itemframes[ndef._vlc_itemframe]
	...
	if self._map_id then
		vlc_maps.load_map(self._map_id, function(texture)
...
function vlc_itemframes.tpl_entity:get_staticdata()
...
function vlc_itemframes.tpl_entity:on_activate(staticdata, dtime_s)
...
function vlc_itemframes.tpl_entity:on_step(dtime)
	...
	if minetest.get_item_group(self._item, "clock") > 0 then
		self:set_item(ItemStack("vlc_clock:clock_"..vlc_clock.get_clock_frame()))
	end
end

function vlc_itemframes.register_itemframe(name, def)
	if not def.node then return end
	local nodename = "vlc_itemframes:"..name
	table.insert(vlc_itemframes.registered_nodes, nodename)
	vlc_itemframes.registered_itemframes[name] = def
	minetest.register_node(":"..nodename, table.merge(vlc_itemframes.tpl_node, def.node, {
		_vlc_itemframe = name,
		groups = table.merge({ dig_immediate = 3, deco_block = 1, dig_by_piston = 1, handy = 1, axey = 1, itemframe = 1 }, def.node.groups),
	}))
end

minetest.register_entity("vlc_itemframes:item", vlc_itemframes.tpl_entity)

vlc_itemframes.register_itemframe("frame", {
	node = {
		...
		tiles = { "vlc_itemframes_item_frame.png" },
		inventory_image = "vlc_itemframes_item_frame.png",
		wield_image = "vlc_itemframes_item_frame.png",
	},
})

vlc_itemframes.register_itemframe("glow_frame", {
	node = {
		description = S("Glow Item Frame"),
		_tt_help = S("Can hold an item and glows"),
		_doc_items_longdesc = S("Item frames are decorative blocks in which items can be placed."),
		_doc_items_usagehelp = S("Just place any item on the item frame. Use the item frame again to retrieve the item."),
		tiles = { "vlc_itemframes_glow_item_frame.png" },
		inventory_image = "vlc_itemframes_glow_item_frame.png",
		wield_image = "vlc_itemframes_glow_item_frame.png",
	},
	object_properties = { glow = 15 },
})

awards.register_achievement("vlc_itemframes:glowframe", {
	title = S("Glow and Behold!"),
	description = S("Craft a glow item frame."),
	icon = "vlc_itemframes_glow_item_frame.png",
	trigger = {
		type = "craft",
		item = "vlc_itemframes:glow_item_frame",
		target = 1
	},
	type = "Advancement",
	group = "Overworld",
...
minetest.register_lbm({
	...
	name = "vlc_itemframes:respawn_entities",
...
minetest.register_craft({
	output = "vlc_itemframes:frame",
	recipe = {
		{ "vlc_core:stick", "vlc_core:stick", "vlc_core:stick" },
		{ "vlc_core:stick", "vlc_mobitems:leather", "vlc_core:stick" },
		{ "vlc_core:stick", "vlc_core:stick", "vlc_core:stick" },
	}
})
...
minetest.register_craft({
	type = "shapeless",
	output = 'vlc_itemframes:glow_frame',
	recipe = { 'vlc_mobitems:glow_ink_sac', 'vlc_itemframes:item_frame' },
})
