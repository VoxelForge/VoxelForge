...
local vlc_hoppers_formspec = table.concat({
	"formspec_version[4]",
	"size[11.75,8.175]",

	"label[0.375,0.375;" .. F(C(vlc_formspec.label_color, S("Hopper"))) .. "]",

	vlc_formspec.get_itemslot_bg_v4(2.875, 0.75, 5, 1),
	"list[context;main;2.875,0.75;5,1;]",

	"label[0.375,2.45;" .. F(C(vlc_formspec.label_color, S("Inventory"))) .. "]",

	vlc_formspec.get_itemslot_bg_v4(0.375, 2.85, 9, 3),
	"list[current_player;main;0.375,2.85;9,3;9]",

	vlc_formspec.get_itemslot_bg_v4(0.375, 6.8, 9, 1),
...
local def_hopper = {
	inventory_image = "vlc_hoppers_item.png",
	wield_image = "vlc_hoppers_item.png",
	groups = { pickaxey = 1, container = 2, deco_block = 1, hopper = 1 },
	drawtype = "nodebox",
	paramtype = "light",
	-- FIXME: vlc_hoppers_hopper_inside.png is unused by hoppers.
	tiles = {"vlc_hoppers_hopper_inside.png^vlc_hoppers_hopper_top.png", "vlc_hoppers_hopper_outside.png", "vlc_hoppers_hopper_outside.png", "vlc_hoppers_hopper_outside.png", "vlc_hoppers_hopper_outside.png", "vlc_hoppers_hopper_outside.png"},
	node_box = {
		type = "fixed",
		fixed = {
			--funnel walls
	...
	on_construct = function(pos)
		...
		meta:set_string("formspec", vlc_hoppers_formspec)
		local inv = meta:get_inventory()
		inv:set_size("main", 5)
	end,
	after_dig_node = vlc_util.drop_items_from_meta_container({"main"}),
	...
	on_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, count, player)
		minetest.log("action", player:get_player_name()..
				" moves stuff in vlc_hoppers at "..minetest.pos_to_string(pos))
	end,
	on_metadata_inventory_put = function(pos, listname, index, stack, player)
		minetest.log("action", player:get_player_name()..
				" moves stuff to vlc_hoppers at "..minetest.pos_to_string(pos))
	end,
	on_metadata_inventory_take = function(pos, listname, index, stack, player)
		minetest.log("action", player:get_player_name()..
				" takes stuff from vlc_hoppers at "..minetest.pos_to_string(pos))
	end,
	sounds = vlc_sounds.node_sound_metal_defaults(),

	_vlc_blast_resistance = 4.8,
	_vlc_hardness = 3,
}
...
def_hopper_enabled.on_place = function(itemstack, placer, pointed_thing)
	...
	local param2
	if x == -1 then
		fake_itemstack:set_name("vlc_hoppers:hopper_side")
		param2 = 0
	elseif x == 1 then
		fake_itemstack:set_name("vlc_hoppers:hopper_side")
		param2 = 2
	...
	itemstack:set_name("vlc_hoppers:hopper")
	return itemstack
end
def_hopper_enabled.mesecons = {
	effector = {
		action_on = function(pos, node)
			minetest.swap_node(pos, {name="vlc_hoppers:hopper_disabled", param2=node.param2})
		end,
	},
}

minetest.register_node("vlc_hoppers:hopper", def_hopper_enabled)
...
def_hopper_disabled.drop = "vlc_hoppers:hopper"
def_hopper_disabled.mesecons = {
	effector = {
		action_off = function(pos, node)
			minetest.swap_node(pos, {name="vlc_hoppers:hopper", param2=node.param2})
		end,
	},
}

minetest.register_node("vlc_hoppers:hopper_disabled", def_hopper_disabled)
...
def_hopper_side_enabled.mesecons = {
	effector = {
		action_on = function(pos, node)
			minetest.swap_node(pos, {name="vlc_hoppers:hopper_side_disabled", param2=node.param2})
		end,
	},
}
minetest.register_node("vlc_hoppers:hopper_side", def_hopper_side_enabled)
...
def_hopper_side_disabled.mesecons = {
	effector = {
		action_off = function(pos, node)
			minetest.swap_node(pos, {name="vlc_hoppers:hopper_side", param2=node.param2})
		end,
	},
}
minetest.register_node("vlc_hoppers:hopper_side_disabled", def_hopper_side_disabled)

--[[ END OF NODE DEFINITIONS ]]

local function hopper_pull_from_mc (mc_ent, dest_pos, inv_size)
	local inv = vlc_entity_invs.load_inv(mc_ent, inv_size)
	if not inv then
		return false
	end
...
local function hopper_push_to_mc(mc_ent, dest_pos, inv_size)
	local dest_inv = vlc_entity_invs.load_inv(mc_ent, inv_size)
	if not dest_inv then
		return false
	end
...
minetest.register_abm({
	...
	nodenames = {"vlc_hoppers:hopper","vlc_hoppers:hopper_side"},
	interval = 0.5,
	chance = 1,
		...
		if objs and #objs > 0 then
			for k,v in pairs(objs) do
				...
				if entity and entity.name then
					if entity.name == "vlc_minecarts:hopper_minecart" or entity.name == "vlc_minecarts:chest_minecart"then
						...
						if (hm_pos.y == pos.y + 1)
							...
							if entity.name == "vlc_minecarts:hopper_minecart" then
								hopper_pull_from_mc(entity, pos, 5)
							elseif entity.name == "vlc_minecarts:chest_minecart" then
...
local function hopper_suck(pos)
	...
	if updef then
		...
		if not success then
			success = vlc_util.move_item_container(uppos, pos)
		end
	end
	return success
end
...
local function hopper_push(pos, to_pos)
	...
	if to_def then
		...
		if not success and cgroup >= 2 and cgroup <= 6 then
			success = vlc_util.move_item_container(pos, to_pos)
		end
	end
	return success
end
...
minetest.register_abm({
	...
	nodenames = { "vlc_hoppers:hopper_side" },
	neighbors = { "group:container" },
	interval = 1.0,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
...
minetest.register_craft({
	output = "vlc_hoppers:hopper",
	recipe = {
		{"vlc_core:iron_ingot","","vlc_core:iron_ingot"},
		{"vlc_core:iron_ingot","vlc_chests:chest","vlc_core:iron_ingot"},
		{"","vlc_core:iron_ingot",""},
	}
})
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlc_hoppers:hopper", "nodes", "vlc_hoppers:hopper_side")
end

-- Legacy
minetest.register_alias("vlc_hoppers:hopper_item", "vlc_hoppers:hopper")

minetest.register_lbm({
	label = "Update hopper formspecs (0.60.0",
	name = "vlc_hoppers:update_formspec_0_60_0",
