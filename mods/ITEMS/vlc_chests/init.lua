...
local tiles_chest_normal_small = { "vlc_chests_normal.png" }
local tiles_chest_normal_double = { "vlc_chests_normal_double.png" }

if it_is_christmas then
	tiles_chest_normal_small = { "vlc_chests_normal_present.png^vlc_chests_noise.png" }
	tiles_chest_normal_double = { "vlc_chests_normal_double_present.png^vlc_chests_noise_double.png" }
end

local tiles_chest_trapped_small = { "vlc_chests_trapped.png" }
local tiles_chest_trapped_double = { "vlc_chests_trapped_double.png" }

if it_is_christmas then
	tiles_chest_trapped_small = { "vlc_chests_trapped_present.png^vlc_chests_noise.png" }
	tiles_chest_trapped_double = { "vlc_chests_trapped_double_present.png^vlc_chests_noise_double.png" }
end

local tiles_chest_ender_small = { "vlc_chests_ender.png" }

local ender_chest_texture = { "vlc_chests_ender.png" }
if it_is_christmas then
	tiles_chest_ender_small = { "vlc_chests_ender_present.png^vlc_chests_noise.png" }
	ender_chest_texture = { "vlc_chests_ender_present.png" }
end
...
minetest.register_entity("vlc_chests:chest", {
	initial_properties = {
		visual = "mesh",
		pointable = false,
		physical = false,
	...
	on_activate = function(self, initialization_data)
		if initialization_data and initialization_data:find("\"###vlc_chests:chest###\"") then
			self:initialize(unpack(minetest.deserialize(initialization_data)))
		else
			minetest.log("warning", "[vlc_chests] on_activate called without proper initialization_data ... removing entity")
			self.object:remove()
		end
	end,
...
local function find_entity(pos)
	for _, obj in pairs(minetest.get_objects_inside_radius(pos, 0)) do
		...
		if luaentity and luaentity.name == "vlc_chests:chest" then
			return luaentity
		end
	end
end
...
local function create_entity(pos, node_name, textures, param2, double, sound_prefix, mesh_prefix, animation_type, dir, entity_pos)
	if animate_chests or double then
		...
		local initialization_data = minetest.serialize({pos, node_name, textures, dir, double, sound_prefix, mesh_prefix, animation_type, "###vlc_chests:chest###"})
		local obj = minetest.add_entity(entity_pos, "vlc_chests:chest", initialization_data)
		if obj and obj:get_pos() then
			local luaentity = obj:get_luaentity()
			return luaentity
		else
			minetest.log("warning", "[vlc_chests] Failed to create entity at " .. (entity_pos and minetest.pos_to_string(entity_pos, 1) or "nil"))
		end
	end
end
...
local function player_chest_open(player, pos, node_name, textures, param2, double, sound, mesh, shulker)
	...
	if animate_chests then
		...
		local blocked = not shulker and (back_is_blocked(pos, dir) or double and back_is_blocked(vlc_util.get_double_container_neighbor_pos(pos, param2, node_name:sub(-4)), dir))
...
local function chest_update_after_close(pos)
	...
	if animate_chests then
		if node.name == "vlc_chests:trapped_chest_on_small" then
			minetest.swap_node(pos, {name="vlc_chests:trapped_chest_small", param2 = node.param2})
			find_or_create_entity(pos, "vlc_chests:trapped_chest_small", {"vlc_chests_trapped.png"}, node.param2, false, "default_chest", "vlc_chests_chest", "chest"):reinitialize("vlc_chests:trapped_chest_small")
			mesecon.receptor_off(pos, trapped_chest_mesecons_rules)
		elseif node.name == "vlc_chests:trapped_chest_on_left" then
			minetest.swap_node(pos, { name = "vlc_chests:trapped_chest_left", param2 = node.param2 })
			find_or_create_entity(pos, "vlc_chests:trapped_chest_left", tiles_chest_trapped_double, node.param2, true,
				"default_chest", "vlc_chests_chest", "chest"):reinitialize("vlc_chests:trapped_chest_left")
			mesecon.receptor_off(pos, trapped_chest_mesecons_rules)

			local pos_other = vlc_util.get_double_container_neighbor_pos(pos, node.param2, "left")
			minetest.swap_node(pos_other, { name = "vlc_chests:trapped_chest_right", param2 = node.param2 })
			mesecon.receptor_off(pos_other, trapped_chest_mesecons_rules)
		elseif node.name == "vlc_chests:trapped_chest_on_right" then
			minetest.swap_node(pos, { name = "vlc_chests:trapped_chest_right", param2 = node.param2 })
			mesecon.receptor_off(pos, trapped_chest_mesecons_rules)

			local pos_other = vlc_util.get_double_container_neighbor_pos(pos, node.param2, "right")
			minetest.swap_node(pos_other, { name = "vlc_chests:trapped_chest_left", param2 = node.param2 })
			find_or_create_entity(pos_other, "vlc_chests:trapped_chest_left", tiles_chest_trapped_double, node.param2, true,
				"default_chest", "vlc_chests_chest", "chest"):reinitialize("vlc_chests:trapped_chest_left")
			mesecon.receptor_off(pos_other, trapped_chest_mesecons_rules)
		end
	end
...
local function register_chest(basename, desc, longdesc, usagehelp, tt_help, tiles_table, hidden, mesecons,
	...
	if not drop then
		drop = "vlc_chests:" .. basename
	else
		drop = "vlc_chests:" .. drop
	end
	...
	local drop_items_chest = vlc_util.drop_items_from_meta_container("main")
	...
	local small_name = "vlc_chests:" .. basename .. "_small"
	local small_textures = tiles_table.small
	local left_name = "vlc_chests:" .. basename .. "_left"
	local left_textures = tiles_table.double

	minetest.register_node("vlc_chests:" .. basename, {
		description = desc,
		_tt_help = tt_help,
		...
		for p = 1, #players do
			if vector.distance(players[p]:get_pos(), pos) <= 30 then
				minetest.close_formspec(players[p]:get_player_name(),
					"vlc_chests:" .. canonical_basename .. "_" .. pos.x .. "_" .. pos.y .. "_" .. pos.z)
			end
		end
	end
	...
	minetest.register_node(small_name, {
		...
		mesh = not animate_chests and "vlc_chests_chest.obj" or nil,
		...
		_chest_entity_mesh = "vlc_chests_chest",
		...
		on_construct = function(pos)
			...
			if minetest.get_node(vlc_util.get_double_container_neighbor_pos(pos, param2, "right")).name ==
				"vlc_chests:" .. canonical_basename .. "_small" then
				minetest.swap_node(pos, { name = "vlc_chests:" .. canonical_basename .. "_right", param2 = param2 })
				local p = vlc_util.get_double_container_neighbor_pos(pos, param2, "right")
				minetest.swap_node(p, { name = "vlc_chests:" .. canonical_basename .. "_left", param2 = param2 })
				create_entity(p, "vlc_chests:" .. canonical_basename .. "_left", left_textures, param2, true,
					"default_chest",
					"vlc_chests_chest", "chest")
			elseif minetest.get_node(vlc_util.get_double_container_neighbor_pos(pos, param2, "left")).name ==
				"vlc_chests:" .. canonical_basename .. "_small" then
				minetest.swap_node(pos, { name = "vlc_chests:" .. canonical_basename .. "_left", param2 = param2 })
				create_entity(pos, "vlc_chests:" .. canonical_basename .. "_left", left_textures, param2, true,
					"default_chest",
					"vlc_chests_chest", "chest")
				local p = vlc_util.get_double_container_neighbor_pos(pos, param2, "left")
				minetest.swap_node(p, { name = "vlc_chests:" .. canonical_basename .. "_right", param2 = param2 })
			else
				minetest.swap_node(pos, { name = "vlc_chests:" .. canonical_basename .. "_small", param2 = param2 })
				create_entity(pos, small_name, small_textures, param2, false, "default_chest", "vlc_chests_chest",
					"chest")
			end
		end,
		after_place_node = function(pos, placer, itemstack, pointed_thing)
		...
		on_rightclick = function(pos, node, clicker)
			...
			minetest.show_formspec(clicker:get_player_name(),
				sf("vlc_chests:%s_%s_%s_%s", canonical_basename, pos.x, pos.y, pos.z),
				table.concat({
					"formspec_version[4]",
					"size[11.75,10.425]",

					"label[0.375,0.375;" .. F(C(vlc_formspec.label_color, name)) .. "]",
					vlc_formspec.get_itemslot_bg_v4(0.375, 0.75, 9, 3),
	...
	minetest.register_node(left_name, {
		...
		on_construct = function(pos)
			...
			local p = vlc_util.get_double_container_neighbor_pos(pos, param2, "left")
			if not p or minetest.get_node(p).name ~= "vlc_chests:" .. canonical_basename .. "_right" then
				n.name = "vlc_chests:" .. canonical_basename .. "_small"
				minetest.swap_node(pos, n)
			end
			create_entity(pos, left_name, left_textures, param2, true, "default_chest", "vlc_chests_chest", "chest")
		end,
		after_place_node = function(pos, placer, itemstack, pointed_thing)
			minetest.get_meta(pos):set_string("name", itemstack:get_meta():get_string("name"))
		end,
		on_destruct = function(pos)
			...
			local param2 = n.param2
			local p = vlc_util.get_double_container_neighbor_pos(pos, param2, "left")
			if not p or minetest.get_node(p).name ~= "vlc_chests:" .. basename .. "_right" then
				return
			end
			close_forms(canonical_basename, p)

			minetest.swap_node(p, { name = small_name, param2 = param2 })
			create_entity(p, small_name, small_textures, param2, false, "default_chest", "vlc_chests_chest", "chest")
		end,
		after_dig_node = drop_items_chest,
		on_blast = on_chest_blast,
		allow_metadata_inventory_move = protection_check_move,
		...
		allow_metadata_inventory_put = function(pos, listname, index, stack, player)
			...
			elseif listname == "input" then
				local inv = minetest.get_inventory({ type = "node", pos = pos })
				local other_pos = vlc_util.get_double_container_neighbor_pos(pos, minetest.get_node(pos).param2, "left")
		...
		on_rightclick = function(pos, node, clicker)
			...
			minetest.show_formspec(clicker:get_player_name(),
				...
				table.concat({
					...
					vlc_formspec.get_itemslot_bg_v4(0.375, 0.75, 9, 3),
					sf("list[nodemeta:%s,%s,%s;main;0.375,0.75;9,3;]", pos.x, pos.y, pos.z),
					vlc_formspec.get_itemslot_bg_v4(0.375, 4.5, 9, 3),
					sf("list[nodemeta:%s,%s,%s;main;0.375,4.5;9,3;]", pos_other.x, pos_other.y, pos_other.z),
					"label[0.375,8.45;" .. F(C(vlc_formspec.label_color, S("Inventory"))) .. "]",
					vlc_formspec.get_itemslot_bg_v4(0.375, 8.825, 9, 3),
					"list[current_player;main;0.375,8.825;9,3;9]",

					vlc_formspec.get_itemslot_bg_v4(0.375, 12.775, 9, 1),
	...
	minetest.register_node("vlc_chests:" .. basename .. "_right", {
		drawtype = "nodebox",
		paramtype = "light",
		paramtype2 = "facedir",
		node_box = {
		...
		on_construct = function(pos)
			...
			local param2 = n.param2
			local p = vlc_util.get_double_container_neighbor_pos(pos, param2, "right")
			if not p or minetest.get_node(p).name ~= "vlc_chests:" .. canonical_basename .. "_left" then
				n.name = "vlc_chests:" .. canonical_basename .. "_small"
				minetest.swap_node(pos, n)
			end
		end,
		...
		on_destruct = function(pos)
			...
			local param2 = n.param2
			local p = vlc_util.get_double_container_neighbor_pos(pos, param2, "right")
			if not p or minetest.get_node(p).name ~= "vlc_chests:" .. basename .. "_left" then
				return
			end
			close_forms(canonical_basename, p)
		...
		allow_metadata_inventory_put = function(pos, listname, index, stack, player)
			...
			if minetest.is_protected(pos, name) then
				...
				-- BEGIN OF LISTRING WORKAROUND
			elseif listname == "input" then
				local other_pos = vlc_util.get_double_container_neighbor_pos(pos, minetest.get_node(pos).param2, "right")
				local other_inv = minetest.get_inventory({ type = "node", pos = other_pos })
				local inv = minetest.get_inventory({ type = "node", pos = pos })
				--[[if other_inv:room_for_item("main", stack) then
					return -1
	...
	if mod_doc then
		doc.add_entry_alias("nodes", small_name, "nodes", "vlc_chests:" .. basename .. "_left")
		doc.add_entry_alias("nodes", small_name, "nodes", "vlc_chests:" .. basename .. "_right")
	end
...
register_chest("chest",
	...
	S("27 inventory slots") .. "\n" .. S("Can be combined to a large chest"),
	{
		...
		inv = { "default_chest_top.png", "vlc_chests_chest_bottom.png",
			"vlc_chests_chest_right.png", "vlc_chests_chest_left.png",
			"vlc_chests_chest_back.png", "default_chest_front.png" },
...
register_chest("trapped_chest",
	...
	function(pos, node, clicker)
		minetest.swap_node(pos, {name="vlc_chests:trapped_chest_on_small", param2 = node.param2})
		if animate_chests then
			find_or_create_entity(pos, "vlc_chests:trapped_chest_on_small", {"vlc_chests_trapped.png"}, node.param2, false, "default_chest", "vlc_chests_chest", "chest"):reinitialize("vlc_chests:trapped_chest_on_small")
		end
	...
	function(pos, node, clicker)
		...
		minetest.swap_node(pos, { name = "vlc_chests:trapped_chest_on_left", param2 = node.param2 })
		if animate_chests then
			find_or_create_entity(pos, "vlc_chests:trapped_chest_on_left", tiles_chest_trapped_double, node.param2, true,
				"default_chest", "vlc_chests_chest", "chest"):reinitialize("vlc_chests:trapped_chest_on_left")
		end
		mesecon.receptor_on(pos, trapped_chest_mesecons_rules)

		local pos_other = vlc_util.get_double_container_neighbor_pos(pos, node.param2, "left")
		minetest.swap_node(pos_other, { name = "vlc_chests:trapped_chest_on_right", param2 = node.param2 })
		mesecon.receptor_on(pos_other, trapped_chest_mesecons_rules)
	end,
	function(pos, node, clicker)
		local pos_other = vlc_util.get_double_container_neighbor_pos(pos, node.param2, "right")

		minetest.swap_node(pos, { name = "vlc_chests:trapped_chest_on_right", param2 = node.param2 })
		mesecon.receptor_on(pos, trapped_chest_mesecons_rules)

		minetest.swap_node(pos_other, { name = "vlc_chests:trapped_chest_on_left", param2 = node.param2 })
		if animate_chests then
			find_or_create_entity(pos_other, "vlc_chests:trapped_chest_on_left", tiles_chest_trapped_double, node.param2,
				true,
				"default_chest", "vlc_chests_chest", "chest"):reinitialize("vlc_chests:trapped_chest_on_left")
		end
		mesecon.receptor_on(pos_other, trapped_chest_mesecons_rules)
...
--[[local function close_if_trapped_chest(pos, player)
	...
	if animate_chests then
		...
		elseif node.name == "vlc_chests:trapped_chest_on_left" then
			minetest.swap_node(pos, {name="vlc_chests:trapped_chest_left", param2 = node.param2})
			find_or_create_entity(pos, "vlc_chests:trapped_chest_left", tiles_chest_trapped_double, node.param2, true, "default_chest", "vlc_chests_chest", "chest"):reinitialize("vlc_chests:trapped_chest_left")
			mesecon.receptor_off(pos, trapped_chest_mesecons_rules)
		...
		elseif node.name == "vlc_chests:trapped_chest_on_right" then
			...
			local pos_other = vlc_util.get_double_container_neighbor_pos(pos, node.param2, "right")
			minetest.swap_node(pos_other, {name="vlc_chests:trapped_chest_left", param2 = node.param2})
			find_or_create_entity(pos_other, "vlc_chests:trapped_chest_left", tiles_chest_trapped_double, node.param2, true, "default_chest", "vlc_chests_chest", "chest"):reinitialize("vlc_chests:trapped_chest_left")
			mesecon.receptor_off(pos_other, trapped_chest_mesecons_rules)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname:find("vlc_chests:") == 1 then
		if fields.quit then
			player_chest_close(player)
		end
	end
...
minetest.register_craft({
	type = "fuel",
	recipe = "vlc_chests:chest",
	burntime = 15,
})

minetest.register_craft({
	type = "fuel",
	recipe = "vlc_chests:trapped_chest",
	burntime = 15,
})

minetest.register_node("vlc_chests:ender_chest", {
	...
	on_construct = function(pos)
		local node = minetest.get_node(pos)
		node.name = "vlc_chests:ender_chest_small"
		minetest.set_node(pos, node)
	end,
})
...
local formspec_ender_chest = table.concat({
	...
	"label[0.375,0.375;" .. F(C(vlc_formspec.label_color, S("Ender Chest"))) .. "]",
	vlc_formspec.get_itemslot_bg_v4(0.375, 0.75, 9, 3),
...
minetest.register_node("vlc_chests:ender_chest_small", {
	description = S("Ender Chest"),
	_tt_help = S("27 interdimensional inventory slots") ..
		"\n" .. S("Put items inside, retrieve them from any ender chest"),
	_doc_items_longdesc = S(
	...
	tiles = animate_chests and {"blank.png"} or {"vlc_chests_ender.png"},
	_chest_entity_textures = {"vlc_chests_ender.png"},
	_chest_entity_sound = "vlc_chests_enderchest",
	_chest_entity_mesh = "vlc_chests_chest",
	_chest_entity_animation_type = "chest",
	use_texture_alpha = minetest.features.use_texture_alpha_string_modes and "clip" or true,
	--[[{"vlc_chests_ender_chest_top.png", "vlc_chests_ender_chest_bottom.png",
		"vlc_chests_ender_chest_right.png", "vlc_chests_ender_chest_left.png",
		"vlc_chests_ender_chest_back.png", "vlc_chests_ender_chest_front.png"},]]--
	...
	paramtype2 = "facedir",
	sounds = vlc_sounds.node_sound_stone_defaults(),
	drop = "vlc_core:obsidian 8",
	on_construct = function(pos)
		local meta = minetest.get_meta(pos)
		meta:set_string("formspec", formspec_ender_chest)
		create_entity(pos, "vlc_chests:ender_chest_small", {"vlc_chests_ender.png"}, minetest.get_node(pos).param2, false, "vlc_chests_enderchest", "vlc_chests_chest", "chest")
	end,
	on_rightclick = function(pos, node, clicker)
		local def = minetest.registered_nodes[minetest.get_node(vector.offset(pos, 0, 1, 0)).name]
		if not def or def.groups.opaque == 1 then
		...
		minetest.show_formspec(clicker:get_player_name(), "vlc_chests:ender_chest_" .. clicker:get_player_name(),
			formspec_ender_chest)
		player_chest_open(clicker, pos, "vlc_chests:ender_chest_small", ender_chest_texture, node.param2, false,
			"vlc_chests_enderchest", "vlc_chests_chest")
	end,
	...
	_vlc_blast_resistance = 3000,
	_vlc_hardness = 22.5,
	_vlc_silk_touch_drop = { "vlc_chests:ender_chest" },
	on_rotate = simple_rotate,
})
...
minetest.register_allow_player_inventory_action(function(player, action, inv, info)
	if inv:get_location().type == "player" and (
		...
		if not minetest.find_node_near(player:get_pos(), def and def.range or ItemStack():get_definition().range or tonumber(minetest.settings:get("vlc_hand_range")) or 4.5, "vlc_chests:ender_chest_small", true) then
			return 0
		end
	end
end)
...
minetest.register_craft({
	output = "vlc_chests:ender_chest",
	recipe = {
		{ "vlc_core:obsidian", "vlc_core:obsidian", "vlc_core:obsidian" },
		{ "vlc_core:obsidian", "vlc_end:ender_eye", "vlc_core:obsidian" },
		{ "vlc_core:obsidian", "vlc_core:obsidian", "vlc_core:obsidian" },
	},
})
...
for color, desc in pairs(boxtypes) do
	...
	local small_name = "vlc_chests:" .. color .. "_shulker_box_small"

	minetest.register_node("vlc_chests:" .. color .. "_shulker_box", {
		description = desc,
	...
	minetest.register_node(small_name, {
		...
		_doc_items_usagehelp = usagehelp,
		drawtype = animate_chests and "nodebox" or "mesh",
		mesh = not animate_chests and "vlc_chests_shulker.obj" or nil,
		tiles = animate_chests and {"blank.png"} or {mob_texture},
		use_texture_alpha = minetest.features.use_texture_alpha_string_modes and "clip" or true,
		_chest_entity_textures = {mob_texture},
		_chest_entity_sound = "vlc_chests_shulker",
		_chest_entity_mesh = "vlc_chests_shulker",
		...
		on_construct = function(pos)
			...
			local inv = meta:get_inventory()
			inv:set_size("main", 9*3)
			create_entity(pos, small_name, {mob_texture}, minetest.get_node(pos).param2, false, "vlc_chests_shulker", "vlc_chests_shulker", "shulker")
		end,
		after_place_node = function(pos, placer, itemstack, pointed_thing)
		...
		on_rightclick = function(pos, node, clicker)
			player_chest_open(clicker, pos, small_name, { mob_texture }, node.param2, false, "vlc_chests_shulker",
				"vlc_chests_shulker", true)
		end,
		on_receive_fields = function(pos, formname, fields, sender)
			if fields.quit then
				player_chest_close(sender)
		...
		on_destruct = function(pos)
			...
			local boxitem = ItemStack("vlc_chests:" .. color .. "_shulker_box")
	...
	if mod_doc and not is_canonical then
		doc.add_entry_alias("nodes", "vlc_chests:" .. canonical_shulker_color .. "_shulker_box", "nodes",
			"vlc_chests:" .. color .. "_shulker_box")
		doc.add_entry_alias("nodes", "vlc_chests:" .. canonical_shulker_color .. "_shulker_box_small", "nodes",
			"vlc_chests:" .. color .. "_shulker_box_small")
	end

	minetest.register_craft({
		type = "shapeless",
		output = "vlc_chests:"..color.."_shulker_box",
		recipe = { "group:shulker_box", "vlc_dyes:"..color }
	})
end

minetest.register_craft({
	output = "vlc_chests:violet_shulker_box",
	recipe = {
		{ "vlc_mobitems:shulker_shell" },
		{ "vlc_chests:chest" },
		{ "vlc_mobitems:shulker_shell" },
	},
})
...
minetest.register_lbm({
	...
	name = "vlc_chests:spawn_chest_entities",
...
minetest.register_lbm({
	...
	name = "vlc_chests:replace_old",
	nodenames = { "vlc_chests:chest", "vlc_chests:trapped_chest", "vlc_chests:trapped_chest_on",
		"vlc_chests:ender_chest",
	...
	action = function(pos, node)
		...
		if node_name == "vlc_chests:trapped_chest_on" then
			minetest.log("action", "[vlc_chests] Disabled active trapped chest on load: " .. minetest.pos_to_string(pos))
			chest_update_after_close(pos)
		elseif node_name == "vlc_chests:ender_chest" then
			local meta = minetest.get_meta(pos)
			meta:set_string("formspec", formspec_ender_chest)
		end
	end
...
minetest.register_lbm({
	...
	name = "vlc_chests:reset_trapped_chests",
	nodenames = { "vlc_chests:trapped_chest_on_small", "vlc_chests:trapped_chest_on_left",
		"vlc_chests:trapped_chest_on_right" },
	run_at_every_load = true,
	action = function(pos, node)
		minetest.log("action", "[vlc_chests] Disabled active trapped chest on load: " .. minetest.pos_to_string(pos))
		chest_update_after_close(pos)
...
minetest.register_lbm({
	...
	name = "vlc_chests:update_shulker_box_formspecs_0_72_0",
...
minetest.register_lbm({
	...
	name = "vlc_chests:replace_old_ender_form",
	nodenames = { "vlc_chests:ender_chest_small" },
	run_at_every_load = false,
	action = function(pos, node)
