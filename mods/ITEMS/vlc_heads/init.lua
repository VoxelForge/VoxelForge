...
if minetest.get_modpath("vlc_armor") then
	equip_armor = vlc_armor.equip_on_use
end

vlc_heads = {}
vlc_heads.FLOOR_BOX = { -0.25, -0.5, -0.25, 0.25, 0.0, 0.25, }
vlc_heads.CEILING_BOX = { -0.25, 0, -0.25, 0.25, 0.5, 0.25, }

vlc_heads.deftemplate = {
	drawtype = "mesh",
	mesh = "vlc_heads_floor.obj",
	paramtype = "light",
	paramtype2 = "degrotate",
	selection_box = {
		type = "fixed",
		fixed = vlc_heads.FLOOR_BOX,
	},
	collision_box = {
		type = "fixed",
		fixed = vlc_heads.FLOOR_BOX,
...
function vlc_heads.deftemplate.on_rotate(pos, node, user, mode, new_param2)
...
function vlc_heads.deftemplate.on_place(itemstack, placer, pointed_thing)
...
function vlc_heads.register_head(head_def)
	local name = "vlc_heads:" ..head_def.name

	-- register the floor head node
	minetest.register_node(":"..name, table.update(table.copy(vlc_heads.deftemplate), {
		description = head_def.description,
		_doc_items_longdesc = head_def.longdesc,
		tiles = { head_def.texture },

		_vlc_armor_mob_range_mob = head_def.range_mob,
		_vlc_armor_mob_range_factor = head_def.range_factor,
		_vlc_armor_texture = "[combine:64x32:32,0=" ..head_def.texture,
	}))

	minetest.register_node(":"..name.."_ceiling", table.update(table.copy(vlc_heads.deftemplate), {
		mesh = "vlc_heads_ceiling.obj",
		groups = {
			handy = 1,
			head = 1,
			deco_block = 1,
	...
	minetest.register_node(":"..name .."_wall", table.update(table.copy(vlc_heads.deftemplate), {
		drawtype = "nodebox",
		paramtype = "light",
...
vlc_heads.register_head{
	name = "zombie",
	texture = "vlc_heads_zombie.png",
...
vlc_heads.register_head{
	name = "skeleton",
	texture = "vlc_heads_skeleton.png",
...
vlc_heads.register_head{
	...
	texture = "vlc_heads_wither_skeleton.png",
...
local old_bheads = {
	...
	"vlc_heads:skeleton",
	"vlc_heads:creeper",
	"vlc_heads:wither_skeleton",
}

local old_rheads = {
	"vlc_heads:creeper22_5",
	"vlc_heads:creeper45",
	"vlc_heads:creeper67_5",

	"vlc_heads:wither_skeleton22_5",
	"vlc_heads:wither_skeleton45",
	"vlc_heads:wither_skeleton67_5",

	"vlc_heads:skeleton45",
	"vlc_heads:skeleton22_5",
	"vlc_heads:skeleton67_5",

	"vlc_heads:steve22_5",
	"vlc_heads:steve45",
	"vlc_heads:steve67_5",

	"vlc_heads:zombie22_5",
	"vlc_heads:zombie45",
	"vlc_heads:zombie67_5",
}

minetest.register_lbm({
	name = "vlc_heads:convert_old_angled_heads",
...
minetest.register_lbm({
	name = "vlc_heads:convert_old_ceiling_heads",
