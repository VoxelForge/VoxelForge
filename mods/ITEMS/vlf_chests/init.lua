...
local tiles_chest_normal_small = { "vlf_chests_normal.png" }
local tiles_chest_normal_double = { "vlf_chests_normal_double.png" }

if it_is_christmas then
	tiles_chest_normal_small = { "vlf_chests_normal_present.png^vlf_chests_noise.png" }
	tiles_chest_normal_double = { "vlf_chests_normal_double_present.png^vlf_chests_noise_double.png" }
end

local tiles_chest_trapped_small = { "vlf_chests_trapped.png" }
local tiles_chest_trapped_double = { "vlf_chests_trapped_double.png" }

if it_is_christmas then
	tiles_chest_trapped_small = { "vlf_chests_trapped_present.png^vlf_chests_noise.png" }
	tiles_chest_trapped_double = { "vlf_chests_trapped_double_present.png^vlf_chests_noise_double.png" }
end

local tiles_chest_ender_small = { "vlf_chests_ender.png" }

local ender_chest_texture = { "vlf_chests_ender.png" }
if it_is_christmas then
	tiles_chest_ender_small = { "vlf_chests_ender_present.png^vlf_chests_noise.png" }
	ender_chest_texture = { "vlf_chests_ender_present.png" }
end
...
minetest.register_entity("vlf_chests:chest", {
	...
	on_activate = function(self, initialization_data)
		if initialization_data and initialization_data:find("\"###vlf_chests:chest###\"") then
			...
			minetest.log("warning", "[vlf_chests] on_activate called without proper initialization_data ... removing entity")
			self.object:remove()
		end
	end,
...
local function create_entity(pos, node_name, textures, param2, double, sound_prefix, mesh_prefix, animation_type, dir, entity_pos)
	if animate_chests or double then
		...
		local initialization_data = minetest.serialize({pos, node_name, textures, dir, double, sound_prefix, mesh_prefix, animation_type, "###vlf_chests:chest###"})
...
local function player_chest_open(player, pos, node_name, textures, param2, double, sound, mesh, shulker)
	...
	if animate_chests then
		...
		local blocked = not shulker and (back_is_blocked(pos, dir) or double and back_is_blocked(vlf_util.get_double_container_neighbor_pos(pos, param2, node_name:sub(-4)), dir))
...
local function chest_update_after_close(pos)
	...
	if animate_chests then
		if node.name == "vlf_chests:trapped_chest_on_small" then
			vlf_redstone.swap_node(pos, {name="vlf_chests:trapped_chest_small", param2 = node.param2})
			find_or_create_entity(pos, "vlf_chests:trapped_chest_small", {"vlf_chests_trapped.png"}, node.param2, false, "default_chest", "vlf_chests_chest", "chest"):reinitialize("vlf_chests:trapped_chest_small")
		elseif node.name == "vlf_chests:trapped_chest_on_left" then
			vlf_redstone.swap_node(pos, { name = "vlf_chests:trapped_chest_left", param2 = node.param2 })
			find_or_create_entity(pos, "vlf_chests:trapped_chest_left", tiles_chest_trapped_double, node.param2, true,
		...
		elseif node.name == "vlf_chests:trapped_chest_on_right" then
			vlf_redstone.swap_node(pos, { name = "vlf_chests:trapped_chest_right", param2 = node.param2 })

			local pos_other = vlf_util.get_double_container_neighbor_pos(pos, node.param2, "right")
			vlf_redstone.swap_node(pos_other, { name = "vlf_chests:trapped_chest_left", param2 = node.param2 })
			find_or_create_entity(pos_other, "vlf_chests:trapped_chest_left", tiles_chest_trapped_double, node.param2, true,
				"default_chest", "vlf_chests_chest", "chest"):reinitialize("vlf_chests:trapped_chest_left")
		end
	end
end
...
local function register_chest(basename, desc, longdesc, usagehelp, tt_help, tiles_table, hidden, redstone,
	...
	if not drop then
		drop = "vlf_chests:" .. basename
	else
		drop = "vlf_chests:" .. drop
	end
	...
	local function close_forms(canonical_basename, pos)
		for pl in vlf_util.connected_players(pos, 30) do
			minetest.close_formspec(pl:get_player_name(), "vlf_chests:" .. canonical_basename .. "_" .. pos.x .. "_" .. pos.y .. "_" .. pos.z)
		end
	end
	...
	minetest.register_node(small_name, {
		...
		mesh = not animate_chests and "vlf_chests_chest.obj" or nil,
		...
		paramtype2 = "facedir",
		drop = drop,
		_vlf_baseitem = "vlf_chests:"..basename,
		groups = {
			handy = 1,
			axey = 1,
			container = 2,
		...
		on_construct = function(pos)
			...
			if minetest.get_node(vlf_util.get_double_container_neighbor_pos(pos, param2, "right")).name ==
				"vlf_chests:" .. canonical_basename .. "_small" then
				minetest.swap_node(pos, { name = "vlf_chests:" .. canonical_basename .. "_right", param2 = param2 })
				local p = vlf_util.get_double_container_neighbor_pos(pos, param2, "right")
				minetest.swap_node(p, { name = "vlf_chests:" .. canonical_basename .. "_left", param2 = param2 })
				create_entity(p, "vlf_chests:" .. canonical_basename .. "_left", left_textures, param2, true,
					"default_chest",
					"vlf_chests_chest", "chest")
			elseif minetest.get_node(vlf_util.get_double_container_neighbor_pos(pos, param2, "left")).name ==
				"vlf_chests:" .. canonical_basename .. "_small" then
				minetest.swap_node(pos, { name = "vlf_chests:" .. canonical_basename .. "_left", param2 = param2 })
				create_entity(pos, "vlf_chests:" .. canonical_basename .. "_left", left_textures, param2, true,
					"default_chest",
					"vlf_chests_chest", "chest")
				local p = vlf_util.get_double_container_neighbor_pos(pos, param2, "left")
				minetest.swap_node(p, { name = "vlf_chests:" .. canonical_basename .. "_right", param2 = param2 })
			else
				minetest.swap_node(pos, { name = "vlf_chests:" .. canonical_basename .. "_small", param2 = param2 })
				create_entity(pos, small_name, small_textures, param2, false, "default_chest", "vlf_chests_chest",
					"chest")
			end
		end,
		after_place_node = function(pos, _, itemstack,  _)
		...
		on_rightclick = function(pos, node, clicker)
			...
			minetest.show_formspec(clicker:get_player_name(),
				sf("vlf_chests:%s_%s_%s_%s", canonical_basename, pos.x, pos.y, pos.z),
				table.concat({
					"formspec_version[4]",
					"size[11.75,10.425]",
	...
	minetest.register_node(left_name, {
		...
		on_construct = function(pos)
			...
			local p = vlf_util.get_double_container_neighbor_pos(pos, param2, "left")
			if not p or minetest.get_node(p).name ~= "vlf_chests:" .. canonical_basename .. "_right" then
				n.name = "vlf_chests:" .. canonical_basename .. "_small"
				minetest.swap_node(pos, n)
			end
			create_entity(pos, left_name, left_textures, param2, true, "default_chest", "vlf_chests_chest", "chest")
		end,
		after_place_node = function(pos, _, itemstack, _)
			minetest.get_meta(pos):set_string("name", itemstack:get_meta():get_string("name"))
		end,
		...
		allow_metadata_inventory_put = function(pos, listname, _, stack, player)
			...
			elseif listname == "input" then
				local inv = minetest.get_inventory({ type = "node", pos = pos })
				local other_pos = vlf_util.get_double_container_neighbor_pos(pos, minetest.get_node(pos).param2, "left")
	...
	minetest.register_node("vlf_chests:" .. basename .. "_right", {
		drawtype = "nodebox",
		paramtype = "light",
		paramtype2 = "facedir",
		_vlf_baseitem = "vlf_chests:"..basename,
		...
		on_construct = function(pos)
			...
			local param2 = n.param2
			local p = vlf_util.get_double_container_neighbor_pos(pos, param2, "right")
			if not p or minetest.get_node(p).name ~= "vlf_chests:" .. canonical_basename .. "_left" then
				n.name = "vlf_chests:" .. canonical_basename .. "_small"
				minetest.swap_node(pos, n)
			end
		end,
		...
		on_destruct = function(pos)
			...
			local param2 = n.param2
			local p = vlf_util.get_double_container_neighbor_pos(pos, param2, "right")
			if not p or minetest.get_node(p).name ~= "vlf_chests:" .. basename .. "_left" then
				return
			end
			close_forms(canonical_basename, p)
		...
		allow_metadata_inventory_put = function(pos, listname, _, stack, player)
			...
			if minetest.is_protected(pos, name) then
				...
				-- BEGIN OF LISTRING WORKAROUND
			elseif listname == "input" then
				local other_pos = vlf_util.get_double_container_neighbor_pos(pos, minetest.get_node(pos).param2, "right")
				local other_inv = minetest.get_inventory({ type = "node", pos = other_pos })
				local inv = minetest.get_inventory({ type = "node", pos = pos })
				--[[if other_inv:room_for_item("main", stack) then
					return -1
	...
	if mod_doc then
		doc.add_entry_alias("nodes", small_name, "nodes", "vlf_chests:" .. basename .. "_left")
		doc.add_entry_alias("nodes", small_name, "nodes", "vlf_chests:" .. basename .. "_right")
	end
...
register_chest("trapped_chest",
	...
	function(pos, node, _)
		vlf_redstone.swap_node(pos, {name="vlf_chests:trapped_chest_on_small", param2 = node.param2})
		if animate_chests then
			find_or_create_entity(pos, "vlf_chests:trapped_chest_on_small", {"vlf_chests_trapped.png"}, node.param2, false, "default_chest", "vlf_chests_chest", "chest"):reinitialize("vlf_chests:trapped_chest_on_small")
		end
	end,
	function(pos, node, _)
		local meta = minetest.get_meta(pos)
		...
		vlf_redstone.swap_node(pos, { name = "vlf_chests:trapped_chest_on_left", param2 = node.param2 })
		if animate_chests then
			find_or_create_entity(pos, "vlf_chests:trapped_chest_on_left", tiles_chest_trapped_double, node.param2, true,
				"default_chest", "vlf_chests_chest", "chest"):reinitialize("vlf_chests:trapped_chest_on_left")
		end

		local pos_other = vlf_util.get_double_container_neighbor_pos(pos, node.param2, "left")
		vlf_redstone.swap_node(pos_other, { name = "vlf_chests:trapped_chest_on_right", param2 = node.param2 })
	end,
	function(pos, node, _)
		local pos_other = vlf_util.get_double_container_neighbor_pos(pos, node.param2, "right")

		vlf_redstone.swap_node(pos, { name = "vlf_chests:trapped_chest_on_right", param2 = node.param2 })
		vlf_redstone.swap_node(pos_other, { name = "vlf_chests:trapped_chest_on_left", param2 = node.param2 })
		if animate_chests then
			find_or_create_entity(pos_other, "vlf_chests:trapped_chest_on_left", tiles_chest_trapped_double, node.param2,
				true,
				"default_chest", "vlf_chests_chest", "chest"):reinitialize("vlf_chests:trapped_chest_on_left")
		end
	end
...
minetest.register_node("vlf_chests:ender_chest", {
	...
	on_construct = function(pos)
		local node = minetest.get_node(pos)
		node.name = "vlf_chests:ender_chest_small"
		minetest.set_node(pos, node)
	end,
})
...
minetest.register_node("vlf_chests:ender_chest_small", {
	description = S("Ender Chest"),
	_tt_help = S("27 interdimensional inventory slots") ..
		"\n" .. S("Put items inside, retrieve them from any ender chest"),
	_doc_items_longdesc = S(
	...
	drawtype = animate_chests and "nodebox" or "mesh",
	mesh = not animate_chests and "vlf_chests_chest.obj" or nil,
	_vlf_baseitem = "vlf_chests:ender_chest",
	node_box = animate_chests and {
		type = "fixed",
        fixed = {-0.4375, -0.5, -0.4375, 0.4375, 0.375, 0.4375},
	} or nil,
	...
	tiles = animate_chests and {"blank.png"} or {"vlf_chests_ender.png"},
	_chest_entity_textures = {"vlf_chests_ender.png"},
	_chest_entity_sound = "vlf_chests_enderchest",
	_chest_entity_mesh = "vlf_chests_chest",
	_chest_entity_animation_type = "chest",
	use_texture_alpha = minetest.features.use_texture_alpha_string_modes and "clip" or true,
	--[[{"vlf_chests_ender_chest_top.png", "vlf_chests_ender_chest_bottom.png",
		"vlf_chests_ender_chest_right.png", "vlf_chests_ender_chest_left.png",
		"vlf_chests_ender_chest_back.png", "vlf_chests_ender_chest_front.png"},]]--
	...
	paramtype2 = "facedir",
	sounds = vlf_sounds.node_sound_stone_defaults(),
	drop = "vlf_core:obsidian 8",
	on_construct = function(pos)
		local meta = minetest.get_meta(pos)
		meta:set_string("formspec", formspec_ender_chest)
		create_entity(pos, "vlf_chests:ender_chest_small", {"vlf_chests_ender.png"}, minetest.get_node(pos).param2, false, "vlf_chests_enderchest", "vlf_chests_chest", "chest")
	end,
	on_rightclick = function(pos, node, clicker)
		local def = minetest.registered_nodes[minetest.get_node(vector.offset(pos, 0, 1, 0)).name]
		if not def or def.groups.opaque == 1 then
		...
		minetest.show_formspec(clicker:get_player_name(), "vlf_chests:ender_chest_" .. clicker:get_player_name(),
			formspec_ender_chest)
		player_chest_open(clicker, pos, "vlf_chests:ender_chest_small", ender_chest_texture, node.param2, false,
			"vlf_chests_enderchest", "vlf_chests_chest")
	end,
	...
	_vlf_blast_resistance = 3000,
	_vlf_hardness = 22.5,
	_vlf_silk_touch_drop = { "vlf_chests:ender_chest" },
	on_rotate = simple_rotate,
})
...
minetest.register_allow_player_inventory_action(function(player, action, inv, info)
	if inv:get_location().type == "player" and (
		...
		if not minetest.find_node_near(player:get_pos(), def and def.range or ItemStack():get_definition().range or tonumber(minetest.settings:get("vlf_hand_range")) or 4.5, "vlf_chests:ender_chest_small", true) then
			return 0
		end
	end
end)
...
minetest.register_craft({
	output = "vlf_chests:ender_chest",
	recipe = {
		{ "vlf_core:obsidian", "vlf_core:obsidian", "vlf_core:obsidian" },
		{ "vlf_core:obsidian", "vlf_end:ender_eye", "vlf_core:obsidian" },
		{ "vlf_core:obsidian", "vlf_core:obsidian", "vlf_core:obsidian" },
	},
})
...
tt.register_snippet(function(itemstring, _ , itemstack)
	if minetest.get_item_group(itemstring, "shulker_box") > 0 then
		if itemstack then
			...
			if i > 0 then
				return d, vlf_colors.GREEN
			end
		end
		return shulker_tt_empty, vlf_colors.GREEN
	end
end)
...
for color, desc in pairs(boxtypes) do
	...
	local small_name = "vlf_chests:" .. color .. "_shulker_box_small"
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if minetest.get_item_group(minetest.get_node(pointed_thing.under).name, "cauldron_water") > 0 then
				if vlf_cauldrons.add_level(pointed_thing.under, -1) then
					itemstack:set_name("vlf_chests:"..canonical_shulker_color.."_shulker_box")
					return itemstack
				end
			end
	...
	minetest.register_node("vlf_chests:" .. color .. "_shulker_box", {
		description = desc,
	...
	minetest.register_node(small_name, {
		...
		_doc_items_usagehelp = usagehelp,
		drawtype = animate_chests and "nodebox" or "mesh",
		mesh = not animate_chests and "vlf_chests_shulker.obj" or nil,
		...
		_vlf_baseitem = get_shulker_stack,
		groups = {
			handy = 1,
			pickaxey = 1,
			container = 3,
		...
		on_construct = function(pos)
			create_entity(pos, small_name, {mob_texture}, minetest.get_node(pos).param2, false, "vlf_chests_shulker", "vlf_chests_shulker", "shulker")
		end,
		after_place_node = function(pos, placer, itemstack, _)
		...
		on_rightclick = function(pos, node, clicker)
			player_chest_open(clicker, pos, small_name, { mob_texture }, node.param2, false, "vlf_chests_shulker",
				"vlf_chests_shulker", true)
		end,
		on_receive_fields = function(_, _, fields, sender)
			if fields.quit then
				player_chest_close(sender)
	...
	if mod_doc and not is_canonical then
		doc.add_entry_alias("nodes", "vlf_chests:" .. canonical_shulker_color .. "_shulker_box", "nodes",
			"vlf_chests:" .. color .. "_shulker_box")
		doc.add_entry_alias("nodes", "vlf_chests:" .. canonical_shulker_color .. "_shulker_box_small", "nodes",
			"vlf_chests:" .. color .. "_shulker_box_small")
	end
	minetest.register_craft({
		type = "shapeless",
		output = "vlf_chests:"..color.."_shulker_box",
		recipe = { "group:shulker_box", "vlf_dyes:"..vlf_dyes.vlf2_to_color(color) }
	})
end
...
function vlf_chests.is_opened (chest)
