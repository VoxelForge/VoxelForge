...
--[[local function close_if_trapped_chest(pos, player)
	...
	if animate_chests then
		...
		elseif node.name == "vlf_chests:trapped_chest_on_left" then
			minetest.swap_node(pos, {name="vlf_chests:trapped_chest_left", param2 = node.param2})
			find_or_create_entity(pos, "vlf_chests:trapped_chest_left", tiles_chest_trapped_double, node.param2, true, "default_chest", "vlf_chests_chest", "chest"):reinitialize("vlf_chests:trapped_chest_left")
			mesecon.receptor_off(pos, trapped_chest_mesecons_rules)
		...
		elseif node.name == "vlf_chests:trapped_chest_on_right" then
			...
			local pos_other = vlf_util.get_double_container_neighbor_pos(pos, node.param2, "right")
			minetest.swap_node(pos_other, {name="vlf_chests:trapped_chest_left", param2 = node.param2})
			find_or_create_entity(pos_other, "vlf_chests:trapped_chest_left", tiles_chest_trapped_double, node.param2, true, "default_chest", "vlf_chests_chest", "chest"):reinitialize("vlf_chests:trapped_chest_left")
			mesecon.receptor_off(pos_other, trapped_chest_mesecons_rules)
...
for color, desc in pairs(boxtypes) do
	...
	minetest.register_craft({
		...
		recipe = { "group:shulker_box", "vlf_dyes:"..color }
	})
end
