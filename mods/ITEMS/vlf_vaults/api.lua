...
local RINGBUFFER_SIZE = tonumber(minetest.settings:get("vlf_vaults_looter_list_length")) or 128

local function can_open(pos, player)
	if not player or not player:is_player() then return false end
	local rb = vlf_util.ringbuffer.get_from_node_meta(pos, VISITED_KEY, RINGBUFFER_SIZE)
...
local function get_vault_def(pos)
	...
	return def and def._vlf_vault_name and vlf_vaults.registered_vaults[def._vlf_vault_name], node
end
...
local function generate_loot(pos)
	...
	if def then
		local loot = {}
		for _, stack in pairs(vlf_loot.get_multi_loot(def.loot, PcgRandom(os.time()))) do
...
local function get_next_loot(pos)
	...
	if type(loot) ~= "table" then
		...
		if loot_string ~= "" then
			minetest.log("warning", "[vlf_vaults]: cleaning invalid loot data at pos " .. minetest.pos_to_string(pos, 0) .. ": " .. dump(loot_string))
		end
...
local tpl = {
	...
	drop = "",
	_vlf_hardness = 50,
	_vlf_blast_resitance = 50,
}

minetest.register_entity("vlf_vaults:item_entity", {
	initial_properties = {
		physical = false,
		visual = "wielditem",
		...
		if self._timer < 0 then
			...
			if node.name == self._vault_on_name then
				if get_eligible_player_near(self._pos, 5) then
					...
					local item = vlf_loot.get_multi_loot(self._loot, self._pr)[1]:get_name()
	...
	on_activate = function(self)
		...
		if def then
			self._vault_name = "vlf_vaults:" .. def.name
...
local function activate_item_entity(pos)
	...
	for o in minetest.objects_inside_radius(pos, 0.1) do
		...
		if lua_entity.name == "vlf_vaults:item_entity" then
			count = count + 1
			if count == 1 then
				entity = o
				...
				minetest.log("warning", "[vlf_vaults] more than one item entity found at " .. minetest.pos_to_string(pos, 0))
				o:remove()
			end
		end
	end

	return entity or minetest.add_entity(pos, "vlf_vaults:item_entity")
end

-- Activate node at position `pos`.
-- Creates an entity inside the vault that displays potential loot.
function vlf_vaults.activate(pos, player)
	local def, node = get_vault_def(pos)
	if def and node.name == "vlf_vaults:"..def.name and can_open(pos, player) then
...
--
-- The code currently assumes that `#vlf_loot.get_multi_loot(loot, pr) > 0`,
-- i.e. that at least some loot components have `stacks_min > 0`.
function vlf_vaults.register_vault(name, def)
	assert(type(name) == "string", "[vlf_vaults] trying to register vault without a valid (string) name")
	assert(def.loot, "[vlf_vaults] vault "..tostring(name).." does not define a loot table.")
	assert(def.keyitem or type(def.key) == "table", "[vlf_vaults] vault "..tostring(name).." does not define a key item.")
	def.name = name
	vlf_vaults.registered_vaults[name] = def

	local keyitem = def.keyitem or ("vlf_vaults:"..def.key.name)
	if not def.keyitem then
		minetest.register_craftitem(keyitem, def.key)
	end

	minetest.register_node(":vlf_vaults:"..name, table.merge(tpl, {
		...
		on_rightclick = function(pos, _, clicker)
			...
			vlf_vaults.activate(pos, clicker)
		end,
		...
		on_timer = function(pos)
			...
			return not player or not vlf_vaults.activate(pos, player)
		end,
	}, def.node_off))

	minetest.register_node(":vlf_vaults:"..name.."_ejecting", table.merge(tpl, {
		_vlf_vault_name = name,
		groups = table.merge(tpl.groups, { vault = 3 }),
	}, def.node_ejecting))

	minetest.register_node(":vlf_vaults:"..name.."_on", table.merge(tpl, {
		_vlf_vault_name = name,
		...
		on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
			...
			if itemstack:get_name() == keyitem and try_open(pos, clicker) then
				node.name = "vlf_vaults:"..name.."_ejecting"
				minetest.swap_node(pos, node)
	...
	minetest.register_lbm({
		name = "vlf_vaults:" .. name,
		label = "Activate vault item entity",
		nodenames = {
			"vlf_vaults:" .. name .. "_on",
			"vlf_vaults:" .. name .. "_ejecting",
		},
