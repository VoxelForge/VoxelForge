...
local function find_doll(pos)
	for  _,obj in pairs(minetest.get_objects_inside_radius(pos, 0.5)) do
		if not obj:is_player() then
			if obj and obj:get_luaentity().name == "vlc_mobspawners:doll" then
				return obj
			end
		end
	end
...
local function spawn_doll(pos)
	return minetest.add_entity({x=pos.x, y=pos.y-0.3, z=pos.z}, "vlc_mobspawners:doll")
end
...
function vlc_mobspawners.setup_spawner(pos, Mob, MinLight, MaxLight, MaxMobsInArea, PlayerDistance, YOffset)
	...
	local mn,mx = vlc_mobs.get_mob_light_level(Mob,dim)
...
local function spawn_mobs(pos, elapsed)
	...
	if not vlc_mobs.spawning_mobs[mob] then
		minetest.log("error", "[vlc_mobspawners] Mob Spawner: Mob doesn't exist: "..mob)
		return
	end
...
-- MUST call vlc_mobspawners.setup_spawner right after the spawner has been placed.
minetest.register_node("vlc_mobspawners:spawner", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if success then
			...
			vlc_mobspawners.setup_spawner(placepos)
		end
	...
	on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
		...
		vlc_mobspawners.setup_spawner(pos, itemstack:get_name())
	...
	on_destruct = function(pos)
		...
		vlc_experience.throw_xp(pos, math.random(15, 43))
	end,
...
doll_def.on_step = function(self, dtime)
	...
	if self.timer > 1 then
		if n and n.name and n.name ~= "vlc_mobspawners:spawner" then
			self.object:remove()
		end
	end
end
...
minetest.register_entity("vlc_mobspawners:doll", doll_def)

-- FIXME: Doll can get destroyed by /clearobjects
minetest.register_lbm({
	label = "Respawn mob spawner dolls",
	name = "vlc_mobspawners:respawn_entities",
	nodenames = { "vlc_mobspawners:spawner" },
