if minetest.settings:get_bool('bundles', true) then
    ...
    local function on_rightclick(itemstack, placer, pointed_thing)
        ...
        local most_recent_item = items[#items]

        if not most_recent_item then
            return itemstack
        end
        ...
        -- Drop all collected items of the same type
