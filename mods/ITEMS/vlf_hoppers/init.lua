...
local vlf_hoppers_formspec = table.concat({
	"formspec_version[4]",
	"size[11.75,8.175]",

	"label[0.375,0.375;" .. F(C(vlf_formspec.label_color, S("Hopper"))) .. "]",

	vlf_formspec.get_itemslot_bg_v4(2.875, 0.75, 5, 1),
	"list[context;main;2.875,0.75;5,1;]",

	"label[0.375,2.45;" .. F(C(vlf_formspec.label_color, S("Inventory"))) .. "]",

	vlf_formspec.get_itemslot_bg_v4(0.375, 2.85, 9, 3),
	"list[current_player;main;0.375,2.85;9,3;9]",

	vlf_formspec.get_itemslot_bg_v4(0.375, 6.8, 9, 1),
...
local def_hopper = {
	...
	tiles = {"vlf_hoppers_hopper_inside.png^vlf_hoppers_hopper_top.png", "vlf_hoppers_hopper_outside.png", "vlf_hoppers_hopper_outside.png", "vlf_hoppers_hopper_outside.png", "vlf_hoppers_hopper_outside.png", "vlf_hoppers_hopper_outside.png"},
	node_box = {
		type = "fixed",
		fixed = {
			--funnel walls
...
def_hopper_enabled.on_place = function(itemstack, placer, pointed_thing)
	...
	local param2
	if x == -1 then
		fake_itemstack:set_name("vlf_hoppers:hopper_side")
		param2 = 0
	elseif x == 1 then
		fake_itemstack:set_name("vlf_hoppers:hopper_side")
		param2 = 2
...
local function hopper_pull_from_mc (mc_ent, dest_pos, inv_size)
	local inv = vlf_entity_invs.load_inv(mc_ent, inv_size)
	if not inv then
		return false
	end
...
local function hopper_push_to_mc(mc_ent, dest_pos, inv_size)
	local dest_inv = vlf_entity_invs.load_inv(mc_ent, inv_size)
	if not dest_inv then
		return false
	end
...
minetest.register_abm({
	...
	nodenames = { "vlf_hoppers:hopper_side" },
	neighbors = { "group:container" },
	interval = 1.0,
	chance = 1,
	action = function(pos)
...
minetest.register_craft({
	output = "vlf_hoppers:hopper",
	recipe = {
		{"vlf_core:iron_ingot","","vlf_core:iron_ingot"},
		{"vlf_core:iron_ingot","vlf_chests:chest","vlf_core:iron_ingot"},
		{"","vlf_core:iron_ingot",""},
	}
})
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlf_hoppers:hopper", "nodes", "vlf_hoppers:hopper_side")
end

-- Legacy
minetest.register_alias("vlf_hoppers:hopper_item", "vlf_hoppers:hopper")
