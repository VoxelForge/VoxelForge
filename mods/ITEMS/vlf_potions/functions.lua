...
local function potions_set_hudbar(player)
	if EF.withering[player] and EF.regenerating[player] then
		hb.change_hudbar(player, "health", nil, nil, "vlf_potions_icon_regen_wither.png", nil, "hudbars_bar_health.png")
	elseif EF.withering[player] then
		hb.change_hudbar(player, "health", nil, nil, "vlf_potions_icon_wither.png", nil, "hudbars_bar_health.png")
...
local function potions_set_icons(player)
	...
	for i=1, EFFECT_TYPES do
		...
		if effect_name == nil then
			...
			player:hud_change(icon, "text", "vlf_potions_effect_"..effect_name..".png^[resize:128x128")
		end
	end
...
minetest.register_globalstep(function(dtime)
	...
	for player, vals in pairs(EF.invisible) do
		...
		if player:get_pos() then vlf_potions._add_spawner(player, "#7F8392") end

		if EF.invisible[player].timer >= EF.invisible[player].dur then
			vlf_potions.make_invisible(player, false)
	...
	for player, vals in pairs(EF.withering) do
		...
		if player:get_pos() then vlf_potions._add_spawner(player, "#000000") end

		if EF.withering[player].hit_timer >= EF.withering[player].step then
			if is_player or entity then vlf_util.deal_damage(player, 1, {type = "magic"}) end
	...
	for player, vals in pairs(EF.poisoned) do
		...
		if player:get_pos() then vlf_potions._add_spawner(player, "#4E9331") end

		if EF.poisoned[player].hit_timer >= EF.poisoned[player].step then
			if vlf_util.get_hp(player) - 1 > 0 then
				vlf_util.deal_damage(player, 1, {type = "magic"})
			end
	...
	for player, vals in pairs(EF.regenerating) do
		...
		if player:get_pos() then vlf_potions._add_spawner(player, "#CD5CAB") end
	...
	for player, vals in pairs(EF.water_breathing) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlf_potions._add_spawner(player, "#2E5299") end
	...
	for player, vals in pairs(EF.leaping) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlf_potions._add_spawner(player, "#22FF4C") end

			if EF.leaping[player].timer >= EF.leaping[player].dur then
				playerphysics.remove_physics_factor(player, "jump", "vlf_potions:leaping")
	...
	for player, vals in pairs(EF.swift) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlf_potions._add_spawner(player, "#7CAFC6") end

			if EF.swift[player].timer >= EF.swift[player].dur then
				playerphysics.remove_physics_factor(player, "speed", "vlf_potions:swiftness")
				EF.swift[player] = nil
				meta = player:get_meta()
	...
	for player, vals in pairs(EF.night_vision) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlf_potions._add_spawner(player, "#1F1FA1") end
	...
	for player, vals in pairs(EF.fire_proof) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlf_potions._add_spawner(player, "#E49A3A") end
	...
	for player, vals in pairs(EF.weak) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlf_potions._add_spawner(player, "#484D48") end
	...
	for player, vals in pairs(EF.strong) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlf_potions._add_spawner(player, "#932423") end
	...
	for player, vals in pairs(EF.bad_omen) do
		...
		if player:get_pos() then vlf_potions._add_spawner(player, "#0b6138") end
...
function vlf_potions._clear_cached_player_data(player)
	EF.invisible[player] = nil
	EF.poisoned[player] = nil
	EF.regenerating[player] = nil
	EF.strong[player] = nil
...
function vlf_potions._reset_player_effects(player, set_hud)
	...
	vlf_potions._clear_cached_player_data(player)
...
function vlf_potions._load_player_effects(player)
	...
	if minetest.deserialize(meta:get_string("_is_invisible")) then
		...
		vlf_potions.make_invisible(player, true)
	end
...
function vlf_potions.player_has_effect(player, effect_name)
...
function vlf_potions.player_get_effect(player, effect_name)
...
function vlf_potions.player_clear_effect(player,effect)
...
minetest.register_on_leaveplayer( function(player)
	...
	vlf_potions._clear_cached_player_data(player) -- clearout the buffer to prevent looking for a player not there
	icon_ids[player:get_player_name()] = nil
end)

minetest.register_on_dieplayer( function(player)
	vlf_potions._reset_player_effects(player)
	potions_set_hud(player)
end)

minetest.register_on_joinplayer( function(player)
	vlf_potions._reset_player_effects(player, false) -- make sure there are no wierd holdover effects
...
function vlf_potions._use_potion(item, obj, color)
	local d = 0.1
...
function vlf_potions.healing_func(player, hp)
	...
	elseif hp < 0 then
		...
		vlf_util.deal_damage(player, -hp, {type = "magic"})
	end

end

function vlf_potions.swiftness_func(player, factor, duration)
	...
	if not EF.swift[player] then
		...
		playerphysics.add_physics_factor(player, "speed", "vlf_potions:swiftness", factor)
...
function vlf_potions.leaping_func(player, factor, duration)
	...
	if not EF.leaping[player] then
		...
		playerphysics.add_physics_factor(player, "jump", "vlf_potions:leaping", factor)
...
function vlf_potions.weakness_func(player, factor, duration)
...
function vlf_potions.strength_func(player, factor, duration)
...
function vlf_potions.withering_func(player, factor, duration)
...
function vlf_potions.poison_func(player, factor, duration)
...
function vlf_potions.regeneration_func(player, factor, duration)
...
function vlf_potions.invisiblility_func(player, null, duration)
...
function vlf_potions.water_breathing_func(player, null, duration)
...
function vlf_potions.fire_resistance_func(player, null, duration)
...
function vlf_potions.night_vision_func(player, null, duration)
...
function vlf_potions.bad_omen_func(player, factor, duration)
