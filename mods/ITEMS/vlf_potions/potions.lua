...
local function generate_on_use(vanish, effects, _, on_use, custom_effect)
	return function(itemstack, user, pointed_thing)
		...
		if vanish then
			...
			replacement = "vlf_potions:glass_bottle"
		end
		itemstack = minetest.do_item_eat(0, replacement, itemstack,
						 user, pointed_thing)
		if vanish or itemstack then vlf_potions._use_potion(user) end

		return itemstack
	end
end

function vlf_potions.consume_potion (mob, id, potency, plus)
	...
	dur = vlf_potions.duration_from_details (details, potency,
						 plus, 1.0)
	vlf_potions.give_effect_by_level (name, mob, ef_level, dur)
	end
...
function vlf_potions.register_potion(def)
	...
	pdef._vlf_filter_description = vlf_potions.filter_potion_description
	...
	if def.has_splash or def.has_splash == nil then
		...
		sdef._get_all_virtual_items = generate_get_all_virtual_items_func("vlf_potions:" .. name .. "_splash", sdef)
	...
	if def.has_lingering or def.has_lingering == nil then
		...
		ldef._get_all_virtual_items = generate_get_all_virtual_items_func("vlf_potions:" .. name .. "_lingering", ldef)
	...
	if def.has_arrow then
		...
		adef._get_all_virtual_items = generate_get_all_virtual_items_func("vlf_potions:" .. name .. "_arrow", adef)
...
function vlf_potions.filter_potion_description (itemstack, orig_desc)
...
vlf_potions.register_potion({
	...
	custom_effect = function(object, level, _, user)
		return vlf_potions.healing_func(object, 4 * level, user)
	end,
})
...
vlf_potions.register_potion({
	...
	has_arrow = true,
	custom_effect = function(object, level, _, user)
		return vlf_potions.healing_func(object, -6 * level, user)
	end,
})
...
--[[vlf_potions.register_potion({
	...
	image = "vlf_potions_ominous_potion.png",
	_effect_list = {
...
local compat_def = {
	...
	image = "vlf_potions_potion_overlay.png^[colorize:#00F:127^vlf_potions_potion_bottle.png^vlf_unknown.png",
...
local compat_arrow_def = {
	...
	image = "vlf_bows_arrow_inv.png^(vlf_potions_arrow_inv.png^[colorize:#FFF:100)^vlf_unknown.png",
	groups = {not_in_creative_inventory = 1},
	on_secondary_use = replace_legacy_potion,
	on_place = replace_legacy_potion,
}
...
for _, name in pairs(old_potions_2) do
	minetest.register_craftitem("vlf_potions:" .. name .. "_2", compat_def)
	minetest.register_craftitem("vlf_potions:" .. name .. "_2_splash", compat_def)
	minetest.register_craftitem("vlf_potions:" .. name .. "_2_lingering", compat_def)
	minetest.register_craftitem("vlf_potions:" .. name .. "_2_arrow", compat_arrow_def)
end
for _, name in pairs(old_potions_plus) do
	minetest.register_craftitem("vlf_potions:" .. name .. "_plus", compat_def)
	minetest.register_craftitem("vlf_potions:" .. name .. "_plus_splash", compat_def)
	minetest.register_craftitem("vlf_potions:" .. name .. "_plus_lingering", compat_def)
	minetest.register_craftitem("vlf_potions:" .. name .. "_plus_arrow", compat_arrow_def)
end
