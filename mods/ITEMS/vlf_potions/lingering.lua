...
local function lingering_image(colorstring, opacity)
	...
	return "vlf_potions_splash_overlay.png^[colorize:"..colorstring..":"..tostring(opacity).."^vlf_potions_lingering_bottle.png"
end

local lingering_effects_at = {}

local function potency_to_level (potency)
	return (potency + 1) * vlf_potions.LINGERING_FACTOR
end
...
local function def_to_effect_list (def, potency, plus)
	...
	effects[name] = {}
	effects[name].level = vlf_potions.level_from_details (details,
								  potency)
	effects[name].dur
		= vlf_potions.duration_from_details (details, potency, plus,
						 vlf_potions.LINGERING_FACTOR)
	end
	return effects
end
...
function vlf_potions.add_lingering_effect (pos, name, duration, level,
					   initial_radius)
	local def = vlf_potions.registered_effects[name]
...
minetest.register_globalstep(function(dtime)
	...
	if lingering_timer >= 1 then
		...
		for i, vals in ipairs (lists) do
			...
			if vlf_potions._water_effect (pos, d) then
				deduct_time = true
			end
			end
				...
				if obj:is_player() or entity and entity.is_mob then
					if vals.is_water then
						...
						for name, effect in pairs (vals.effects) do
							vlf_potions.give_effect_by_level (name, obj, effect.level,
											  effect.dur)
							deduct_time = true
						end
...
function vlf_potions.register_lingering(name, descr, color, def)
	local id = "vlf_potions:"..name.."_lingering"
	...
	local groups = {brewitem=1, not_in_creative_inventory = 0,
			bottle=1, ling_potion=1, _vlf_potion=1}
	...
	minetest.register_entity(id.."_flying",{
		...
		on_step = function(self, dtime, moveresult)
			...
			local d, val = 4, vlf_potions.detect_hit (self.object,
								  pos,
								  moveresult,
								  velocity)
			if val then
	...
	minetest.register_entity(id.."_ominous_item_flying",{
		...
		on_step = function(self, dtime, moveresult)
			local pos = self.object:get_pos()
			local velocity = self.object:get_velocity ()
			local d, val = 4, vlf_potions.detect_hit (self.object, pos, moveresult, velocity)
			if val then
				if val.target and mod_target then
				vlf_target.hit (val.target, 0.4) --4 redstone ticks
				end
