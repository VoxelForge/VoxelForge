...
local overlay = vlc_enchanting.overlay
local hud = "vlc_shield_hud.png"

minetest.register_tool("vlc_shields:shield", {
	description = S("Shield"),
	_doc_items_longdesc = S("A shield is a tool used for protecting the player against attacks."),
	inventory_image = "vlc_shield.png",
	stack_max = 1,
	groups = {
		shield = 1,
		weapon = 1,
...
function vlc_shields.wielding_shield(obj, i)
	return wielded_item(obj, i):find("vlc_shields:shield")
end

local function shield_is_enchanted(obj, i)
	return vlc_enchanting.is_enchanted(wielded_item(obj, i))
end

minetest.register_entity("vlc_shields:shield_entity", {
	initial_properties = {
		visual = "mesh",
		mesh = "vlc_shield.obj",
		physical = false,
		...
		textures = {"vlc_shield_base_nopattern.png"},
	...
	on_step = function(self, dtime, moveresult)
		...
		local shield_texture = "vlc_shield_base_nopattern.png"
		local i = self._shield_number
		local item = wielded_item(player, i)

		if item ~= "vlc_shields:shield" and item ~= "vlc_shields:shield_enchanted" then
			local itemstack = player:get_wielded_item()
			if i == 1 then
				itemstack = player:get_inventory():get_stack("offhand", 1)
			end
			local meta_texture = itemstack:get_meta():get_string("vlc_shields:shield_custom_pattern_texture")
			if meta_texture ~= "" then
				shield_texture = meta_texture
			else
				local color = minetest.registered_items[item]._shield_color
				if color then
					shield_texture = "vlc_shield_base_nopattern.png^(vlc_shield_pattern_base.png^[colorize:" .. color .. ")"
				end
			end
		end
...
for _, e in pairs(vlc_shields.enchantments) do
	vlc_enchanting.enchantments[e].secondary.shield = true
end

function vlc_shields.is_blocking(obj)
	if not obj:is_player() then return end
	if vlc_shields.players[obj] then
		local blocking = vlc_shields.players[obj].blocking
		if blocking <= 0 then
			return
		end
...
vlc_damage.register_modifier(function(obj, damage, reason)
	...
	local blocking, shieldstack = vlc_shields.is_blocking(obj)

	if not (obj:is_player() and blocking and vlc_shields.types[type] and damager) then
		return
	end
	...
	local unbreaking = vlc_enchanting.get_enchantment(shieldstack, vlc_shields.enchantments[2])
	...
	if not minetest.is_creative_enabled(obj:get_player_name()) and damage >= 3 then
		...
		if blocking == 2 then
			...
			vlc_inventory.update_inventory_formspec(obj)
		end
	end
	minetest.sound_play({name = "vlc_block"}, {pos = obj:get_pos(), max_hear_distance = 16})
	return 0
end)
...
local function modify_shield(player, vpos, vrot, i)
	...
	local shield = vlc_shields.players[player].shields[i]
	if shield then
...
local function set_interact(player, interact)
	...
	if interact and meta:get_int("vlc_shields:interact_revoked") ~= 0 then
		meta:set_int("vlc_shields:interact_revoked", 0)
		privs.interact = true
	elseif not interact then
		meta:set_int("vlc_shields:interact_revoked", privs.interact and 1 or 0)
		privs.interact = nil
	end
...
local function add_shield_entity(player, i)
	local shield = minetest.add_entity(player:get_pos(), "vlc_shields:shield_entity")
	if shield and shield:get_pos() then
		shield:get_luaentity()._shield_number = i
		vlc_shields.players[player].shields[i] = shield
		set_shield(player, false, i)
	end
end

local function remove_shield_entity(player, i)
	local shields = vlc_shields.players[player].shields
	if shields[i] then
		shields[i]:remove()
		shields[i] = nil
	end
...
local function handle_blocking(player)
	local player_shield = vlc_shields.players[player]
	...
	local shield_in_offhand = vlc_shields.wielding_shield(player, 1)
	local shield_in_hand = vlc_shields.wielding_shield(player)
	...
	elseif shield_in_offhand then
		local pointed_thing = vlc_util.get_pointed_thing(player, true)
...
local function update_shield_entity(player, blocking, i)
	local shield = vlc_shields.players[player].shields[i]
	if vlc_shields.wielding_shield(player, i) then
		if not shield then
...
local function add_shield_hud(shieldstack, player, blocking)
	local texture = hud
	if vlc_enchanting.is_enchanted(shieldstack:get_name()) then
...
minetest.register_globalstep(function(dtime)
	for _, player in pairs(minetest.get_connected_players()) do
		...
		local blocking, shieldstack = vlc_shields.is_blocking(player)
...
minetest.register_on_dieplayer(function(player)
	remove_shield_hud(player)
	if not minetest.settings:get_bool("vlc_keepInventory") then
...
minetest.register_on_leaveplayer(function(player)
	shield_hud[player] = nil
	vlc_shields.players[player] = nil
end)

minetest.register_craft({
	output = "vlc_shields:shield",
	recipe = {
		{"group:wood", "vlc_core:iron_ingot", "group:wood"},
...
for colorkey, colortab in pairs(vlc_banners.colors) do
	local color = colortab[1]
	minetest.register_tool("vlc_shields:shield_" .. color, {
		description = shield_colors[colorkey],
		_doc_items_longdesc = S("A shield is a tool used for protecting the player against attacks."),
		inventory_image = "vlc_shield.png^(vlc_shield_item_overlay.png^[colorize:" .. colortab[4] ..")",
		stack_max = 1,
		groups = {
			shield = 1,
			weapon = 1,
	...
	local banner = "vlc_banners:banner_item_" .. color
	minetest.register_craft({
		type = "shapeless",
		output = "vlc_shields:shield_" .. color,
		recipe = {"vlc_shields:shield", banner},
	})
	minetest.register_craft({
		type = "shapeless",
		output = "vlc_shields:shield_" .. color .. "_enchanted",
		recipe = {"vlc_shields:shield_enchanted", banner},
	})
end

local function to_shield_texture(banner_texture)
	return banner_texture
	:gsub("vlc_banners_base_inverted.png", "vlc_shield_base_nopattern.png^vlc_shield_pattern_base.png")
	:gsub("vlc_banners_banner_base.png", "vlc_shield_base_nopattern.png^vlc_shield_pattern_base.png")
	:gsub("vlc_banners_base", "vlc_shield_pattern_base")
	:gsub("vlc_banners", "vlc_shield_pattern")
end

local function craft_banner_on_shield(itemstack, player, old_craft_grid, craft_inv)
	if not string.find(itemstack:get_name(), "vlc_shields:shield_") then
		return itemstack
	end
	...
	for i = 1, player:get_inventory():get_size("craft") do
		...
		if string.find(banner_name, "vlc_banners:banner") and shield_stack then
			...
			if layers_meta ~= "" then
				local color = vlc_banners.color_reverse(banner_name)
				local layers = minetest.deserialize(layers_meta)
				local texture = vlc_banners.make_banner_texture(color, layers)
				new_shield_meta:set_string("description", vlc_banners.make_advanced_banner_description(itemstack:get_description(), layers))
				new_shield_meta:set_string("vlc_shields:shield_custom_pattern_texture", to_shield_texture(texture))
			end
