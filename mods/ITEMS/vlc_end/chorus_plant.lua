...
-- at start_pos.
function vlc_end.detach_chorus_plant(start_pos, digger)
	...
	for t=1, #tree_start_posses do
		...
		while #check_posses > 0 do
			...
			if not touched_nodes_hashes[minetest.hash_node_position(pos)] then
				...
				if node.name == "vlc_end:end_stone" then
		...
		if break_tree then
			...
			for c=1, #chorus_nodes do
				...
				if digger then
					minetest.node_dig(chorus_nodes[c], { name = "vlc_end:chorus_plant" }, digger)
				else
...
function vlc_end.check_detach_chorus_plant(pos, oldnode, oldmetadata, digger)
	vlc_end.detach_chorus_plant(pos, digger)
end

function vlc_end.check_blast_chorus_plant(pos)
	minetest.remove_node(pos)
	vlc_end.detach_chorus_plant(pos)
end

minetest.register_node("vlc_end:chorus_flower", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		-- Condition 1
		if node_below.name == "vlc_end:chorus_plant" or node_below.name == "vlc_end:end_stone" then
			plant_ok = true
		-- Condition 2
		elseif node_below.name == "air" then
			...
			for a=1, #around do
				...
				if node_side.name == "vlc_end:chorus_plant" then
		...
		if plant_ok then
			...
			if suc then
				minetest.sound_play(vlc_sounds.node_sound_wood_defaults().place, {pos = pos}, true)
			end
			return it
		else
			return itemstack
	...
	after_dig_node = vlc_end.check_detach_chorus_plant,
	on_blast = vlc_end.check_blast_chorus_plant,
	_vlc_blast_resistance = 0.4,
	_vlc_hardness = 0.4,
})

minetest.register_node("vlc_end:chorus_flower_dead", {
	...
	tiles = {
		"vlc_end_chorus_flower_dead.png",
		"vlc_end_chorus_flower_dead.png",
		"vlc_end_chorus_flower_dead.png",
		"vlc_end_chorus_flower_dead.png",
		"vlc_end_chorus_flower_dead.png",
...
minetest.register_node("vlc_end:chorus_plant", {
	...
	connects_to = {"group:chorus_plant", "vlc_end:end_stone"},
	sounds = vlc_sounds.node_sound_wood_defaults(),
	drop = {
		items = {
			{ items = { "vlc_end:chorus_fruit"}, rarity = 2 },
		}
	},
	groups = {handy=1,axey=1, deco_block = 1, dig_by_piston = 1, destroy_by_lava_flow = 1, chorus_plant = 1 },
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		local plant_ok = false
		if node_check.name == "vlc_end:end_stone" or minetest.get_item_group(node_check.name, "chorus_plant") > 0 then
			plant_ok = true
		end
		if plant_ok then
			-- Placement OK! Proceed normally
			local it, suc = minetest.item_place_node(itemstack, placer, pointed_thing)
			if suc then
				minetest.sound_play(vlc_sounds.node_sound_wood_defaults().place, {pos = pos_place}, true)
			end
			return it
		else
			return itemstack
...
function vlc_end.grow_chorus_plant(pos, node, pr)
	local flowers = { pos }
	-- Plant initial flower (if it isn't there already)
	if not node then
		node = minetest.get_node(pos)
	end
	if node.name ~= "vlc_end:chorus_flower" then
		minetest.set_node(pos, { name = "vlc_end:chorus_flower" })
	end
	while true do
		local new_flowers_list = {}
		for f=1, #flowers do
			local new_flowers = vlc_end.grow_chorus_plant_step(flowers[f], minetest.get_node(flowers[f]), pr)
...
function vlc_end.grow_chorus_plant_step(pos, node, pr)
	...
	if node_above.name == "air" and air_around then
		...
		for y=1, 4 do
			...
			if node.name == "vlc_end:chorus_plant" then
				h = y
		...
		if grow_chance then
			...
			for _, f in pairs(new_flowers) do
				if age >= MAX_FLOWER_AGE then
					...
					if nn ~= "vlc_end:chorus_flower" and nn ~= "vlc_end:chorus_flower_dead" then
						minetest.set_node(f, {name="vlc_end:chorus_flower_dead"})
						grown = true
					end
				else
					local nn = minetest.get_node(f).name
					if nn ~= "vlc_end:chorus_flower" and nn ~= "vlc_end:chorus_flower_dead" then
						minetest.set_node(f, {name="vlc_end:chorus_flower", param2 = age})
			...
			if #new_flowers >= 1 then
				minetest.set_node(pos, {name="vlc_end:chorus_plant"})
				grown = true
			end
		end
	end
	if not grown then
		...
		minetest.set_node(pos, {name = "vlc_end:chorus_flower_dead"})
	end
...
minetest.register_abm({
	...
	nodenames = { "vlc_end:chorus_flower" },
	interval = 35.0,
	chance = 4.0,
	action = function(pos, node, active_object_count, active_object_count_wider)
		vlc_end.grow_chorus_plant_step(pos, node, pr)
	end,
})
...
local function random_teleport(player)
	...
	for a=1, 16 do
		...
		for t=0, 16 do
			...
			if tnode.name == "vlc_core:void" or tnode.name == "ignore" then
				break
			end
		...
		if ground_level and #node_cache >= 1 then
			...
			for c=#node_cache, 1, -1 do
				...
				if streak >= 2 then
					...
					player:set_pos(goal)
					minetest.sound_play({name="vlc_end_teleport", gain=0.8}, {pos=goal, max_hear_distance=16}, true)
					return true
				end
			end
		end
...
local eat_chorus_fruit = function(itemstack, player, pointed_thing)
	...
	if count ~= new_count or new_itemstack:get_name() ~= "vlc_end:chorus_fruit" or (minetest.is_creative_enabled(player:get_player_name()) == true) then
		random_teleport(player)
	end
	return new_itemstack
end

minetest.register_craftitem("vlc_end:chorus_fruit", {
	...
	wield_image = "vlc_end_chorus_fruit.png",
	inventory_image = "vlc_end_chorus_fruit.png",
...
minetest.register_craftitem("vlc_end:chorus_fruit_popped", {
	description = S("Popped Chorus Fruit"),
	_doc_items_longdesc = doc.sub.items.temp.craftitem,
	wield_image = "vlc_end_chorus_fruit_popped.png",
	inventory_image = "vlc_end_chorus_fruit_popped.png",
	groups = { craftitem = 1 },
})
...
minetest.register_craft({
	type = "cooking",
	output = "vlc_end:chorus_fruit_popped",
	recipe = "vlc_end:chorus_fruit",
	cooktime = 10,
})
