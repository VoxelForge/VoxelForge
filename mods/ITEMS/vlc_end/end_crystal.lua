...
local function crystal_explode(self, puncher)
	...
	if puncher then
		...
		local reason = {}
		vlc_damage.from_punch(reason, puncher)
		vlc_damage.finish_reason(reason)
		source = reason.source
	end
	minetest.after(0, self.object.remove, self.object)
	vlc_explosions.explode(vector.add(self.object:get_pos(), {x = 0, y = 1.5, z = 0}), strength, {}, self.object, source)
end
...
local function spawn_crystal(pos)
	minetest.add_entity(pos, "vlc_end:crystal")
	if not vector.equals(pos, vector.floor(pos)) then return end
	if vlc_worlds.pos_to_dimension(pos) ~= "end" then return end
	local portal_center
	for _, dir in pairs(directions) do
	...
	for _,o in pairs(minetest.get_objects_inside_radius(pos,64)) do
		...
		if not peaceful then
			if o:is_player() then
				awards.unlock(o:get_player_name(), "vlc:theEndAgain")
			end
		end
	end
	...
	vlc_structures.place_structure(portal_pos,vlc_structures.registered_structures["end_exit_portal"],PseudoRandom(minetest.get_mapgen_setting("seed")),-1)
end

minetest.register_entity("vlc_end:crystal", {
	initial_properties = {
		physical = true,
		visual = "mesh",
		visual_size = {x = 6, y = 6},
...
minetest.register_entity("vlc_end:crystal_beam", {
	initial_properties = {
		physical = false,
		visual = "cube",
		visual_size = {x = 1, y = 1, z = 1},
		textures = {
			"vlc_end_crystal_beam.png^[transformR90",
			"vlc_end_crystal_beam.png^[transformR90",
			"vlc_end_crystal_beam.png",
			"vlc_end_crystal_beam.png",
			"blank.png",
			"blank.png",
		},
...
minetest.register_craftitem("vlc_end:crystal", {
	inventory_image = "vlc_end_crystal_item.png",
	...
	on_place = function(itemstack, placer, pointed_thing)
		if pointed_thing.type == "node" then
			...
			if node_name == "vlc_core:obsidian" or node_name == "vlc_core:bedrock" then
...
minetest.register_craft({
	output = "vlc_end:crystal",
	recipe = {
		{"vlc_core:glass", "vlc_core:glass", "vlc_core:glass"},
		{"vlc_core:glass", "vlc_end:ender_eye", "vlc_core:glass"},
		{"vlc_core:glass", "vlc_mobitems:ghast_tear", "vlc_core:glass"},
	}
})

minetest.register_alias("vlc_end_crystal:end_crystal", "vlc_end:crystal")
