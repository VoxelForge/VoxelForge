...
minetest.register_entity("vlc_end:ender_eye", {
	initial_properties = {
		physical = false,
		textures = {"vlc_end_ender_eye.png"},
	...
	on_step = function(self, dtime)
		...
		if self._age >= 3 then
			...
			if r == 1 then
				...
				self.object:remove()
				local item = minetest.add_item(pos, "vlc_end:ender_eye")
				item:set_velocity(v)
				return
			end
		elseif self._age >= 2 then
			...
			minetest.add_particle({
				pos = pos,
				texture = "vlc_particles_teleport.png",
				expirationtime = 1,
...
local function throw_eye(itemstack, user, pointed_thing)
	...
	local strongholds = vlc_structures.registered_structures["end_shrine"].static_pos
	local dim = vlc_worlds.pos_to_dimension(origin)
	...
	if #strongholds <= 0 or dim ~= "overworld" then
		if not is_creative then
			minetest.item_drop(ItemStack("vlc_end:ender_eye"), user, user:get_pos())
			itemstack:take_item()
		end
		return itemstack
	end
	...
	-- Throw it!
	local obj = minetest.add_entity(origin, "vlc_end:ender_eye")
...
minetest.register_craftitem("vlc_end:ender_eye", {
	...
	wield_image = "vlc_end_ender_eye.png",
	inventory_image = "vlc_end_ender_eye.png",
	on_place = throw_eye,
...
minetest.register_craft({
	type = "shapeless",
	output = "vlc_end:ender_eye",
	recipe = {"vlc_mobitems:blaze_powder", "vlc_throwing:ender_pearl"},
})
