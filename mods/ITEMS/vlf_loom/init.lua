...
local dyerecipes = {}
local preview_item_prefix = "vlf_banners:banner_preview_"

for name,pattern in pairs(vlf_banners.patterns) do
...
local function get_formspec(pos)
	...
	"size[11.75,10.425]"..
	"label[0.375,0.375;" .. F(C(vlf_formspec.label_color, S("Loom"))) .. "]"..

	vlf_formspec.get_itemslot_bg_v4(0.5,1,1,1,0)..
	vlf_formspec.get_itemslot_bg_v4(0.5,1,1,1,0,"vlf_loom_itemslot_bg_banner.png")..
	"list[context;banner;0.5,1;1,1;]"..
	vlf_formspec.get_itemslot_bg_v4(1.75,1,1,1)..
	vlf_formspec.get_itemslot_bg_v4(1.75,1,1,1,0,"vlf_loom_itemslot_bg_dye.png")..
	"list[context;dye;1.75,1;1,1;]"..
	vlf_formspec.get_itemslot_bg_v4(0.5,2.25,1,1)..
	vlf_formspec.get_itemslot_bg_v4(0.5,2.25,1,1,0,"vlf_loom_itemslot_bg_pattern.png")..
	"list[context;pattern;0.5,2.25;1,1;]"..

	"box[3.275,0.75;5.2,3.5;"..vlf_colors.DARK_GRAY.."]"..
	...
	vlf_formspec.get_itemslot_bg_v4(9.5,1.5,1,1)..
...
local function create_banner(stack, pattern, color)
	...
	table.insert(layers,{
		pattern = pattern,
		color = "unicolor_"..vlf_dyes.colors[color].unicolor
	})
	im:set_string("description", vlf_banners.make_advanced_banner_description(stack:get_definition().description, layers))
...
minetest.register_node("vlf_loom:loom", {
	description = S("Loom"),
		...
		if fields then
			...
			for k, _ in pairs(fields) do
				...
				not inv:is_empty("banner") and not inv:is_empty("dye") and inv:is_empty("output") then
					...
					elseif not vlf_dyes.colors[cdef._color] or table.indexof(dyerecipes,pattern) == -1 then
						pattern = nil
					end
					if pattern then
