...
minetest.register_node("vlc_lectern:lectern", {
	...
	paramtype2 = "facedir",
	drawtype = "mesh",
	mesh = "vlc_lectern_lectern.obj",
	tiles = {"vlc_lectern_lectern.png", },
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if minetest.dir_to_wallmounted(vector.subtract(pointed_thing.under,  pointed_thing.above)) == 1 then
			...
			minetest.sound_play(vlc_sounds.node_sound_wood_defaults().place, {pos=pointed_thing.above, gain=1}, true)
		end
		return itemstack
	end,
})

vlc_wip.register_wip_item("vlc_lectern:lectern")

-- April Fools setup
local date = os.date("*t")
if (date.month == 4 and date.day == 1) then
	minetest.override_item("vlc_lectern:lectern", {waving = 2})
end

minetest.register_craft({
	output = "vlc_lectern:lectern",
	recipe = {
