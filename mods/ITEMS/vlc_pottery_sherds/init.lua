...
for name, def in pairs(vlc_pottery_sherds.defs) do
	minetest.register_craftitem("vlc_pottery_sherds:"..name, {
		description = S("@1 Pottery Sherd", def.description),
		_tt_help = S("Used for crafting decorated pots"),
		_doc_items_create_entry = false,
		inventory_image = "vlc_pottery_sherds_"..name..".png",
		wield_image = "vlc_pottery_sherds_"..name..".png",
		groups = { pottery_sherd = 1, decorated_pot_recipe = 1 },
		_vlc_pottery_sherd_name = name,
	})
end

local brick_groups = table.copy(minetest.registered_items["vlc_core:brick"].groups)
brick_groups["decorated_pot_recipe"] = 1
minetest.override_item("vlc_core:brick", { groups = brick_groups })

local function update_entities(pos,rm)
	pos = vector.round(pos)
	for _,v in pairs(minetest.get_objects_inside_radius(pos, 0.5, true)) do
		local ent = v:get_luaentity()
		if ent and ent.name == "vlc_pottery_sherds:pot_face" then
			v:remove()
		end
	end
	...
	for k,v in pairs(pot_face_positions) do
		...
		if face then
			local o = minetest.add_entity(pos + v, "vlc_pottery_sherds:pot_face")
			local e = o:get_luaentity()
			e.texture = "vlc_pottery_sherds_pattern_"..face..".png"
			o:set_properties({
				textures = { "vlc_pottery_sherds_pattern_"..face..".png" },
			})
...
minetest.register_entity("vlc_pottery_sherds:pot_face",{
	initial_properties = {
		physical = false,
	...
	on_step = function(self)
		if minetest.get_node(self.object:get_pos()).name ~= "vlc_pottery_sherds:pot" then
			self.object:remove()
		end
	end
})
...
local function get_sherd_desc(face)
	if face == nil then
		return minetest.registered_items["vlc_core:brick"].description
	end
	return S("@1 Pottery Sherd", vlc_pottery_sherds.defs[face].description)
end
...
minetest.register_node("vlc_pottery_sherds:pot", {
	...
	tiles = {
		{ name = "vlc_pottery_sherds_pot_top.png", align_style = "world" },
		{ name = "vlc_pottery_sherds_pot_bottom.png", align_style = "world" },
		{ name = "vlc_pottery_sherds_pot_side.png", align_style = "world" },
	},
	...
	on_destruct = function(pos)
		local meta = minetest.get_meta(pos)
		local it = ItemStack("vlc_pottery_sherds:pot")
...
local function get_sherd_name(itemstack)
	...
	local r = nil
	if def and def._vlc_pottery_sherd_name then
		r = def._vlc_pottery_sherd_name
	end
	return r
end

local function get_craft(itemstack, player, old_craft_grid, craft_inv)
	if itemstack:get_name() ~= "vlc_pottery_sherds:pot" then return end
	if old_craft_grid[1][2] == "vlc_core:brick" then return end
