...
local bark_stairs = minetest.settings:get_bool("vlc_extra_nodes",true)
...
local wood_sounds = vlc_sounds.node_sound_wood_defaults()
...
local function update_far_away_leaves(pos)
	...
	for _, lpos in pairs(leaves) do
		if not log_in_range(lpos) then
			...
			if math.floor(node.param2 / 32) ~= 1 and ndef._vlc_leaves then
				minetest.swap_node(lpos, {
					name = ndef._vlc_orphan_leaves,
					param2 = node.param2,
				})
			end
		end
...
minetest.register_on_mods_loaded(function()
	for name, ndef in pairs(minetest.registered_nodes) do
		...
		if minetest.get_item_group(name, "leaves") ~= 0 and ndef._vlc_leaves then
			local def = {
				c_leaves = minetest.get_content_id(ndef._vlc_leaves),
				c_orphan_leaves = minetest.get_content_id(ndef._vlc_orphan_leaves),
			}
			leaves_tab[cid] = def
...
local function set_placed_leaves_p2(pos)
	...
	if minetest.get_item_group(n.name, "biomecolor") ~= 0 then
		palette_index = vlc_util.get_pos_p2(pos)
	end
...
local tpl_log = {
	...
	_on_axe_place = vlc_trees.strip_tree,
...
local tpl_leaves = {
	...
	paramtype = "light",
	paramtype2 = "color",
	palette = "vlc_core_palette_leaves.png",
	groups = {
...
local tpl_sapling = {
	...
	_on_bone_meal = function(itemstack,placer,pointed_thing,pos,node)
		...
		return vlc_trees.grow_tree(pos,node)
	end,
...
function vlc_trees.generate_leaves_def(modname, subname, def, sapling, drop_apples, sapling_chances)
	...
	local function get_drops(fortune_level)
		local drop = {
			...
			items = {
				{
					items = {"vlc_core:stick 1"},
					rarity = stick_chances[fortune_level + 1]
				},
				{
					items = {"vlc_core:stick 2"},
					rarity = stick_chances[fortune_level + 1]
				},
			}
		}
	...
	local basedef = table.merge(tpl_leaves, {
		...
		_vlc_fortune_drop = { get_drops(1), get_drops(2), get_drops(3), get_drops(4) },
		_vlc_leaves = leaves_id,
		_vlc_orphan_leaves = orphan_leaves_id,
	...
	local o_def = table.merge(basedef, {
		...
		_vlc_shears_drop = {leaves_id},
		_vlc_silk_touch_drop = {leaves_id},
	})
...
local function register_leaves(subname, def, sapling, drop_apples, sapling_chances)
	local d = vlc_trees.generate_leaves_def("vlc_trees:", subname, def, sapling, drop_apples, sapling_chances)
	minetest.register_node(":" .. d["leaves_id"], d["leaves_def"])
	minetest.register_node(":" .. d["orphan_leaves_id"], d["orphan_leaves_def"])
end

function vlc_trees.register_wood(name, p)
	if not p then p = {} end
	local rname = p.readable_name or name
	if vlc_trees.woods[name] == nil then
		vlc_trees.woods[name] = p
	end
	if p.tree == nil or type(p.tree) == "table" then
		minetest.register_node(":vlc_trees:".."tree_"..name,table.merge(tpl_log,{
			description = S("@1 Log", rname),
			_doc_items_longdesc = S("The trunk of a @1 tree.", rname),
			tiles = { minetest.get_current_modname().."_log_"..name.."_top.png",  "vlc_core_log_"..name.."_top.png", "vlc_core_log_"..name..".png"},
			_vlc_stripped_variant = "vlc_trees:stripped_"..name,
		},p.tree or {}))
	end

	if p.wood == nil or type(p.wood) == "table" then
		minetest.register_node(":vlc_trees:wood_"..name, table.merge(tpl_wood,{
			description =  S("@1 Planks", rname),
			_doc_items_longdesc = doc.sub.items.temp.build,
			tiles = { minetest.get_current_modname().."_planks_"..name..".png"},
		},p.wood or {}))
		minetest.register_craft({
			output = "vlc_trees:wood_"..name.." 4",
			recipe = {
				{ "vlc_trees:tree_"..name },
			}
		})
	end

	if p.bark == nil or type(p.bark) == "table" then
		minetest.register_node(":vlc_trees:bark_"..name,table.merge(tpl_log, {
			...
			is_ground_content = false,
			_vlc_stripped_variant = "vlc_trees:bark_stripped_"..name,
		}, p.bark or {}))
		minetest.register_craft({
			output = "vlc_trees:bark_"..name.." 3",
			recipe = {
				{ "vlc_trees:tree_"..name, "vlc_trees:tree_"..name },
				{ "vlc_trees:tree_"..name, "vlc_trees:tree_"..name },
			}
		})
	end
	...
	if p.stripped == nil or type(p.stripped) == "table" then
		minetest.register_node(":vlc_trees:stripped_"..name, table.merge(tpl_log, {
			description = S("Stripped @1 Log", rname),
			_doc_items_longdesc = S("The stripped trunk of an @1 tree.", rname),
			_doc_items_hidden = false,
			tiles = { minetest.get_current_modname().."_stripped_"..name.."_top.png",  "vlc_core_stripped_"..name.."_top.png", "vlc_core_stripped_"..name.."_side.png"},
		}, p.stripped or {}))
	end

	if p.stripped_bark == nil or type(p.stripped_bark) == "table" then
		minetest.register_node(":vlc_trees:bark_stripped_"..name, table.merge(tpl_log, {
	...
	if p.sapling == nil or type(p.sapling) == "table" then
		minetest.register_node(":vlc_trees:sapling_"..name, table.merge(tpl_sapling, {
	...
	if p.leaves == nil or type(p.leaves) == "table" then
		register_leaves("leaves_"..name,
			...
			p.saplingdrop or "vlc_trees:sapling_"..name,
	...
	if p.fence == nil or type(p.fence) == "table" then
		p.fence = p.fence or {}
		vlc_fences.register_fence(name.."_fence", p.fence.description or S("@1 Fence", rname), p.fence.tiles and p.fence.tiles[1] or "vlc_fences_fence_"..name..".png", p.fence.groups or table.merge(wood_groups,{fence_wood = 1}), p.fence._vlc_blast_hardness or 2, p.fence._vlc_blast_resistance or 3 , p.fence.connects_to or {"group:fence_wood","group:fence"}, p.fence.sounds or wood_sounds)
		minetest.register_craft({
			output = "vlc_fences:"..name.."_fence 3",
			recipe = {
				{"vlc_trees:wood_"..name, "vlc_core:stick", "vlc_trees:wood_"..name},
				{"vlc_trees:wood_"..name, "vlc_core:stick", "vlc_trees:wood_"..name},
			}
		})
	end
	if p.fence_gate == nil or type(p.fence_gate) == "table" then
		...
		vlc_fences.register_fence_gate(name.."_fence", p.fence_gate.description or S("@1 Fence", rname), p.fence_gate.tiles and p.fence_gate.tiles[1] or "vlc_fences_fence_"..name..".png", p.fence_gate.groups or table.merge(wood_groups,{fence_wood = 1}), p.fence_gate._vlc_blast_hardness or 2, p.fence_gate._vlc_blast_resistance or 3,  p.fence_gate.sounds or wood_sounds, p.fence_gate.sound_open, p.fence_gate.sound_close, p.fence_gate.sound_gain_open, p.fence_gate.sound_gain_close)
		minetest.register_craft({
			output = "vlc_fences:"..name.."_fence_gate",
			recipe = {
				{"vlc_core:stick", "vlc_trees:wood_"..name, "vlc_core:stick"},
				{"vlc_core:stick", "vlc_trees:wood_"..name, "vlc_core:stick"},
			}
		})
	end
	if p.door == nil or type(p.door) == "table" then
		vlc_doors:register_door("vlc_doors:door_"..name,table.merge(tpl_door, {
			description = S("@1 Door", rname),
			inventory_image = "vlc_doors_door_"..name..".png",
			tiles_bottom = {"vlc_doors_door_"..name.."_lower.png", "vlc_doors_door_"..name.."_side_lower.png"},
			tiles_top = {"vlc_doors_door_"..name.."_upper.png", "vlc_doors_door_"..name.."_side_upper.png"}
		}, p.door or {}))
		minetest.register_craft({
			output = "vlc_doors:door_"..name.." 3",
			recipe = {
				{"vlc_trees:wood_"..name, "vlc_trees:wood_"..name},
				{"vlc_trees:wood_"..name, "vlc_trees:wood_"..name},
				{"vlc_trees:wood_"..name, "vlc_trees:wood_"..name}
			}
		})
		minetest.register_craft({
			type = "fuel",
			recipe = "vlc_doors:door_"..name,
			burntime = 10,
		})
	end
	if p.trapdoor == nil or type(p.trapdoor) == "table" then
		vlc_doors:register_trapdoor("vlc_doors:trapdoor_"..name,table.merge(tpl_trapdoor, {
			description = S("@1 Trapdoor", rname),
			tile_front = "vlc_doors_trapdoor_"..name..".png",
			tile_side = "vlc_doors_trapdoor_"..name.."_side.png",
			wield_image = "vlc_doors_trapdoor_"..name..".png",
		}, p.trapdoor or {}))
		minetest.register_craft({
			output = "vlc_doors:trapdoor_"..name.." 2",
			recipe = {
				{"vlc_trees:wood_"..name,"vlc_trees:wood_"..name,"vlc_trees:wood_"..name,},
				{"vlc_trees:wood_"..name,"vlc_trees:wood_"..name,"vlc_trees:wood_"..name,},
			}
		})
		minetest.register_craft({
			type = "fuel",
			recipe = "vlc_doors:trapdoor_"..name,
			burntime = 15,
		})
	end
	...
	if p.stairs == nil or type(p.stairs) == "table" then
		...
		vlc_stairs.register_stair(name, {
			baseitem="vlc_trees:wood_"..name,
			description = S("@1 Stairs", rname),
			groups = { wood_stairs = 1 },
		})
		vlc_stairs.register_stair(name.."_bark", {
			baseitem="vlc_trees:bark_"..name,
			description = S("@1 Bark Stairs", rname),
			groups = { bark_stairs = 1 },
			recipeitem=bark_stairs and "vlc_trees:bark_"..name or ""
		})
	end

	if p.slab == nil or type(p.slab) == "table" then
		p.slab = p.slab or {}
		vlc_stairs.register_slab(name, {
			baseitem="vlc_trees:wood_"..name,
			description = S("@1 Slab", rname),
			groups = { wood_slab = 1 },
			register_stair_and_slab = false
		})
		vlc_stairs.register_slab(name.."_bark", {
			baseitem="vlc_trees:bark_"..name,
	...
	if p.sign_color and ( p.sign == nil or type(p.sign) == "table" ) then
		vlc_signs.register_sign(name,p.sign_color,table.merge({
			description = S("@1 Sign", rname),
		}, p.sign or {}))
		minetest.register_craft({
			output = "vlc_signs:wall_sign_"..name.." 3",
			recipe = {
				{"vlc_trees:wood_"..name,"vlc_trees:wood_"..name,"vlc_trees:wood_"..name,},
				{"vlc_trees:wood_"..name,"vlc_trees:wood_"..name,"vlc_trees:wood_"..name,},
				{"","vlc_core:stick",""},
		...
		minetest.register_craft({
			type = "fuel",
			recipe = "vlc_signs:wall_sign_"..name,
			burntime = 10,
		})
	end
	...
	if p.pressure_plate == nil or type(p.pressure_plate) == "table" then
		mesecon.register_pressure_plate(
			...
			p.wood and p.wood.tiles or { minetest.get_current_modname().."_planks_"..name..".png"},
			p.wood and p.wood.tiles[1] or "vlc_core_planks_"..name..".png",
			nil,
			{{"vlc_trees:wood_"..name, "vlc_trees:wood_"..name}},
			vlc_sounds.node_sound_wood_defaults(),
	...
	if (p.sapling == nil or type(p.sapling) == "table") and (p.potted_sapling == nil or type(p.potted_sapling) == "table") then
		vlc_flowerpots.register_potted_flower("vlc_trees:sapling_"..name, table.merge({
			name = "sapling_"..name,
	...
	if p.boat == nil or type(p.boat) == "table" then
		p.boat = p.boat or {}
		vlc_boats.register_boat(name,table.merge({
	...
	if p.chest_boat == nil or type(p.chest_boat) == "table" then
		...
		vlc_boats.register_boat(name.."_chest",table.merge({
...
minetest.register_on_mods_loaded(function()
	vlc_structures.register_structure("wood_test", {
	filenames = {
...
minetest.register_on_mods_loaded(function()
	vlc_structures.register_structure("stairs_test", {
