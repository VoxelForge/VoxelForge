...
function vlc_trees.check_growth_simple(pos, height)
	for y = 1, height - 1 do
...
function vlc_trees.check_growth_giant(pos, height)
	for x = -3, 2 do
		for z = -3, 2 do
			for y = 0, height - 1 do
...
local function check_schem_growth(pos, file, giant)
	if file then
		...
		if schem then
			...
			if giant then
				return vlc_trees.check_growth_giant(pos, h)
			else
				return vlc_trees.check_growth_simple(pos, h)
			end
		end
	end
...
function vlc_trees.grow_tree(pos, node)
	local name = node.name:gsub("vlc_trees:sapling_", "")
	if node.name:find("propagule") then
		name = "mangrove"
	end
	if not vlc_trees.woods[name] or ( not vlc_trees.woods[name].tree_schems and not vlc_trees.woods[name].tree_schems_2x2 ) then
		return
	end
	...
	if vlc_trees.woods[name].tree_schems_2x2  then
		tbt, ne = vlc_trees.check_2by2_saps(pos, node)
		if tbt then
			table.shuffle(vlc_trees.woods[name].tree_schems_2x2)
			schem = vlc_trees.woods[name].tree_schems_2x2[1]
			can_grow = check_schem_growth(ne, schem.file, true)
			place_at = ne
		end
	end

	if not tbt and vlc_trees.woods[name].tree_schems then
		table.shuffle(vlc_trees.woods[name].tree_schems)
		schem = vlc_trees.woods[name].tree_schems[1]
