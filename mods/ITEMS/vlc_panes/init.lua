...
vlc_panes = {}
vlc_panes.update_pane = update_pane
function vlc_panes.register_pane(name, def)
	for i = 1, 15 do
		minetest.register_alias("vlc_panes:" .. name .. "_" .. i, "vlc_panes:" .. name .. "_flat")
	end

	local flatgroups = table.copy(def.groups)
	local drop = def.drop
	if not drop then
		drop = "vlc_panes:" .. name .. "_flat"
	end
	flatgroups.pane = 1
	flatgroups.deco_block = 1
	minetest.register_node(":vlc_panes:" .. name .. "_flat", {
		...
		_vlc_blast_resistance = def._vlc_blast_resistance,
		_vlc_hardness = def._vlc_hardness,
		_vlc_silk_touch_drop = def._vlc_silk_touch_drop and {"vlc_panes:" .. name .. "_flat"},
	})

	local groups = table.copy(def.groups)
	groups.pane = 1
	groups.not_in_creative_inventory = 1
	minetest.register_node(":vlc_panes:" .. name, {
		drawtype = "nodebox",
		paramtype = "light",
		is_ground_content = false,
		sunlight_propagates = true,
	...
	minetest.register_craft({
		output = "vlc_panes:" .. name .. "_flat 16",
		recipe = def.recipe
	})

	if mod_doc and def._doc_items_create_entry ~= false then
		doc.add_entry_alias("nodes", "vlc_panes:" .. name .. "_flat", "nodes", "vlc_panes:" .. name)
	end
end
...
local function pane(description, node, append)
	...
	if append == "_natural" then
		...
		texture1 = "vlc_core_glass"..txappend..".png"
	end
	vlc_panes.register_pane("pane"..append, {
	...
	if mod_doc and not is_canonical then
		doc.add_entry_alias("nodes", "vlc_panes:pane_".. canonical_color .. "_flat", "nodes", "vlc_panes:pane"..append)
		doc.add_entry_alias("nodes", "vlc_panes:pane_".. canonical_color .. "_flat", "nodes", "vlc_panes:pane"..append.."_flat")
	end
end

-- Iron Bars
vlc_panes.register_pane("bar", {
...
-- Glass Pane
pane(S("Glass Pane"), "vlc_core:glass", "_natural") -- triggers special case

-- Stained Glass Panes
for k,v in pairs(vlc_dyes.colors) do
	pane(S("@1 Glass Pane", v.readable_name), "vlc_core:glass_"..k, "_"..k)
end
