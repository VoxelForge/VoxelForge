...
function vlf_throwing.register_throwable_object(name, entity, velocity)
	entity_mapping[name] = entity
	velocities[name] = velocity
end

function vlf_throwing.throw(throw_item, pos, dir, velocity, thrower)
...
function vlf_throwing.get_staticdata(self)
	local thrower
