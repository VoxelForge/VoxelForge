...
local snowball_ENTITY={
	initial_properties = {
		physical = false,
		textures = {"vlf_throwing_snowball.png"},
...
local pearl_ENTITY={
	initial_properties = {
		physical = false,
		textures = {"vlf_throwing_ender_pearl.png"},
...
local function snowball_on_step(self, dtime)
	...
	if self._lastpos.x~=nil then
		if (def and def.walkable) or not def then
			minetest.sound_play("vlf_throwing_snowball_impact_hard", { pos = pos, max_hear_distance=16, gain=0.7 }, true)
			snowball_particles(self._lastpos, vel)
			self.object:remove()
			if mod_target and node.name == "vlf_target:target_off" then
	...
	if check_object_hit(self, pos, {snowball_vulnerable = 3}) then
		minetest.sound_play("vlf_throwing_snowball_impact_soft", { pos = pos, max_hear_distance=16, gain=0.7 }, true)
...
local function egg_on_step(self, dtime)
	...
	if self._lastpos.x then
		if (def and def.walkable) or not def then
			...
			if r == 1 then
				vlf_mobs.spawn_child(self._lastpos, "mobs_mc:chicken")
				...
				if r == 1 then
					...
					for o=1, 3 do
						...
						vlf_mobs.spawn_child(pos, "mobs_mc:chicken")
					end
				end
			end
...
local function pearl_on_step(self, dtime)
	...
	if self._lastpos.x~=nil then
		...
		elseif walkable or nn == "vlf_core:vine" or nn == "vlf_core:deadbush" or minetest.get_item_group(nn, "flower") ~= 0 or minetest.get_item_group(nn, "sapling") ~= 0 or minetest.get_item_group(nn, "plant") ~= 0 or minetest.get_item_group(nn, "mushroom") ~= 0 or not def then
...
minetest.register_entity("vlf_throwing:snowball_entity", snowball_ENTITY)
minetest.register_entity("vlf_throwing:egg_entity", egg_ENTITY)
minetest.register_entity("vlf_throwing:ender_pearl_entity", pearl_ENTITY)


local how_to_throw = S("Use the punch key to throw.")

-- Snowball
minetest.register_craftitem("vlf_throwing:snowball", {
	...
	inventory_image = "vlf_throwing_snowball.png",
	stack_max = 16,
	groups = { weapon_ranged = 1 },
	on_use = vlf_throwing.get_player_throw_function("vlf_throwing:snowball_entity"),
	_on_dispense = vlf_throwing.dispense_function,
})

-- Egg
minetest.register_craftitem("vlf_throwing:egg", {
	description = S("Egg"),
	...
	_doc_items_usagehelp = how_to_throw,
	inventory_image = "vlf_throwing_egg.png",
	stack_max = 16,
	on_use = vlf_throwing.get_player_throw_function("vlf_throwing:egg_entity"),
	_on_dispense = vlf_throwing.dispense_function,
...
-- Ender Pearl
minetest.register_craftitem("vlf_throwing:ender_pearl", {
	description = S("Ender Pearl"),
	_tt_help = S("Throwable").."\n"..minetest.colorize(vlf_colors.YELLOW, S("Teleports you on impact for cost of 5 HP")),
	...
	wield_image = "vlf_throwing_ender_pearl.png",
	inventory_image = "vlf_throwing_ender_pearl.png",
	stack_max = 16,
	on_place = vlf_throwing.get_player_throw_function("vlf_throwing:ender_pearl_entity"),
	on_secondary_use = vlf_throwing.get_player_throw_function("vlf_throwing:ender_pearl_entity"),
	groups = { transport = 1 },
})

vlf_throwing.register_throwable_object("vlf_throwing:snowball", "vlf_throwing:snowball_entity", 22)
vlf_throwing.register_throwable_object("vlf_throwing:egg", "vlf_throwing:egg_entity", 22)
vlf_throwing.register_throwable_object("vlf_throwing:ender_pearl", "vlf_throwing:ender_pearl_entity", 22)
