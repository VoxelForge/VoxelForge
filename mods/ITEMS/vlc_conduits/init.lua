...
local conduit_nodes = { "vlc_ocean:prismarine",  "vlc_ocean:prismarine_brick", "vlc_ocean:prismarine_dark", "vlc_ocean:sea_lantern" }
...
local function check_conduit(pos)
	...
	if #water < 26 or ( cname ~= "vlc_conduits:conduit" and #water < 27 ) then return false end
	local pn = 0
...
function vlc_conduits.player_effect(player)
	if minetest.get_item_group(vlc_player.players[player].nodes.feet, "water") == 0 then return end
	vlc_potions.water_breathing_func(player, 2, 13)
	vlc_potions.swiftness_func(player, 2, 13)
end

function vlc_conduits.conduit_damage(ent)
	if minetest.get_item_group(minetest.get_node(ent.object:get_pos()).name, "water") == 0 then return end
	vlc_util.deal_damage(ent.object, 4, {type = "magic"})
end

minetest.register_entity("vlc_conduits:conduit", {
	initial_properties = {
		physical = true,
		visual = "mesh",
		visual_size = {x = 4, y = 4},
		collisionbox = {-0.5, -0.5, -0.5, 0.5, 0.5, 0.5},
		mesh = "vlc_end_crystal.b3d",
		textures = {"vlc_conduit_conduit.png"},
	...
	on_step = function(self, dtime)
		...
		if not lvl then
			minetest.set_node(self._pos, {name = "vlc_conduits:conduit"})
			self.object:remove()
			return
		end
		local dst = lvl * 2
		...
		for _, ent in pairs(minetest.luaentities) do
			if ent.is_mob and ent.type == "monster" and ent.object and ent.object:get_pos() and vector.distance(self._pos, ent.object:get_pos()) < 9 then
				vlc_conduits.conduit_damage(ent)
			end
		end
	end
})
local conduit_box = { -0.25, -0.25, -0.25, 0.25, 0.25, 0.25, }
minetest.register_node("vlc_conduits:conduit", {
	...
	groups = { pickaxey = 1 },
	light_source = 14,
	tiles = { "vlc_conduit_conduit_node.png", },
	_vlc_hardness = 3,
...
minetest.register_abm({
	...
	nodenames = { "vlc_conduits:conduit" },
	interval = check_interval,
	chance = 1,
	action = function(pos, node)
		for _, v in pairs(minetest.get_objects_inside_radius(vector.subtract(pos, entity_pos_offset), 0.5)) do
			if v.name == "vlc_conduits:conduit" then return end
		end
		if check_conduit(pos) then
			minetest.remove_node(pos)
			local o = minetest.add_entity(vector.add(pos, entity_pos_offset) , "vlc_conduits:conduit")
			if o then
...
minetest.register_craft({
	output = "vlc_conduits:conduit",
	recipe = {
		{"vlc_mobitems:nautilus_shell", "vlc_mobitems:nautilus_shell", "vlc_mobitems:nautilus_shell"},
		{"vlc_mobitems:nautilus_shell", "vlc_mobitems:heart_of_the_sea", "vlc_mobitems:nautilus_shell"},
		{"vlc_mobitems:nautilus_shell", "vlc_mobitems:nautilus_shell", "vlc_mobitems:nautilus_shell"},
	},
})
