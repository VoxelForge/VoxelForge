...
local function entity_effect_player(entity_effect, pos, power_level, entity_effect_level,player)
	local distance =  vector.distance(player:get_pos(), pos)
	if distance > (power_level+1)*10 then return end
	vlf_entity_effects.give_entity_effect_by_level (entity_effect, player, entity_effect_level, 16)
end

local function apply_entity_effects_to_all_players(pos)
    local meta = minetest.get_meta(pos)
    local entity_effect_string = meta:get_string("entity_effect")
    local entity_effect_level = meta:get_int("entity_effect_level")
    local secondary = meta:get_string ("secondary_entity_effect")

    local power_level = beacon_blockcheck(pos)

    if entity_effect_level == 2 and power_level < 4 then --no need to run loops when beacon is in an invalid setup :P
	return
    end
    ...
    for _, player in pairs(minetest.get_connected_players()) do
        if vector.distance(pos, player:get_pos()) <= beacon_distance then
            if not clear_obstructed_beam (pos) then
                entity_effect_player (entity_effect_string, pos, power_level,
			       entity_effect_level, player)
		if secondary and secondary ~= "" and power_level == 4 then
		    entity_effect_player (secondary, pos, power_level,
				   1, player)
		end
            end
        end
...
local function upgrade_entity_effect_level_button (oldmeta)
    local entity_effect = oldmeta:get_string ("entity_effect")
    if entity_effect and entity_effect ~= "" then
	local pdef = vlf_entity_effects.registered_entity_effects[entity_effect] or { }
...
local function generate_beacon_formspec (pos, meta)
    return ("size[11,14]"
	    ...
	    .. "image_button[2.5,1.5;1,1;vlf_entity_effects_entity_effect_swift.png;swiftness;]"
	    .. "image_button[3.5,1.5;1,1;vlf_entity_effects_entity_effect_haste.png;haste;]"
	    .. "image_button[2.5,3;1,1;vlf_entity_effects_entity_effect_resistance.png;resistance;]"
	    .. "image_button[3.5,3;1,1;vlf_entity_effects_entity_effect_leaping.png;leaping;]"
	    .. "image_button[3.0,4.5;1,1;vlf_entity_effects_entity_effect_strong.png;strength;]"
	    .. "image_button[7.5,3.5;1,1;vlf_entity_effects_entity_effect_regenerating.png;regeneration;]"
	    .. upgrade_entity_effect_level_button (meta)
...
minetest.register_abm{
	label="apply beacon entity_effects to players",
	nodenames = {"vlf_beacons:beacon"},
	interval = 3,
	chance = 1,
	action = function(pos)
        apply_entity_effects_to_all_players(pos)
	end,
}
...
local function upgrade_old_data (pos, node, dtime_s)
    -- Clear the primary entity_effect if it is Regeneration, or substitute
    -- `strength' for the old value `strenght'.
    local meta = minetest.get_meta (pos)
    if meta:get_string ("entity_effect") == "regeneration" then
	meta:set_string ("entity_effect", "")
	meta:set_string ("secondary_entity_effect", "regeneration")
	meta:set_string ("entity_effect_level", 1)
    elseif meta:get_string ("entity_effect") == "strenght" then
	meta:set_string ("entity_effect", "strength")
    end
...
local function apply_beacon_formspec (sender, formname, fields)
	...
	if fields.swiftness then
	    meta:set_string ("entity_effect", "swiftness")
	    if minetest.get_meta (pos):get_int ("entity_effect_level") < 1 then
		meta:set_int ("entity_effect_level", 1)
	    end
	    successful = true
	elseif fields.haste then
	    meta:set_string ("entity_effect", "haste")
	    if minetest.get_meta (pos):get_int ("entity_effect_level") < 1 then
		meta:set_int ("entity_effect_level", 1)
	    end
	    successful = true
	elseif fields.leaping and power_level >= 2 then
	    meta:set_string ("entity_effect", "leaping")
	    if minetest.get_meta (pos):get_int ("entity_effect_level") < 1 then
		meta:set_int ("entity_effect_level", 1)
	    end
	    successful = true
	elseif fields.resistance and power_level >= 2 then
	    meta:set_string ("entity_effect", "resistance")
	    if minetest.get_meta (pos):get_int ("entity_effect_level") < 1 then
		meta:set_int ("entity_effect_level", 1)
	    end
	    successful = true
	...
	elseif fields.regeneration and power_level == 4 then
	    -- If a secondary entity_effect is enabled, the entity_effect level must
	    -- be reset to 1.
	    meta:set_int ("entity_effect_level", 1)
	    meta:set_string ("secondary_entity_effect", "regeneration")
	    successful = true
	elseif fields.upgrade_ii and power_level == 4 then
	    -- Upgrade the primary entity_effect to II but cancel the
	    -- secondary one.  Also verify that there is an entity_effect to
	    -- upgrade.
	    if minetest.get_meta (pos):get_string ("entity_effect")
		and minetest.get_meta (pos):get_int ("entity_effect_level") < 2 then
		minetest.get_meta (pos):set_int ("entity_effect_level", 2)
		minetest.get_meta (pos):set_string ("secondary_entity_effect", "")
		successful = true
	    end
	end
	if successful then
	    ...
	    apply_entity_effects_to_all_players(pos) --call it once outside the globalstep so the player gets the entity_effect right after selecting it
