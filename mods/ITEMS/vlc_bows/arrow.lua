...
minetest.register_craftitem("vlc_bows:arrow", {
	description = S("Arrow"),
...
S("Arrows might get stuck on solid blocks and can be retrieved again. They are also capable of pushing wooden buttons."),
	...
	inventory_image = "vlc_bows_arrow_inv.png",
	...
	_on_dispense = function(itemstack, dispenserpos, droppos, dropnode, dropdir)
		...
		vlc_bows.shoot_arrow(itemstack:get_name(), shootpos, dropdir, yaw, nil, 19, 3)
	end,
})
...
local ARROW_ENTITY={
	initial_properties = {
		...
		pointable = false,
		visual = "mesh",
		mesh = "vlc_bows_arrow.obj",
		visual_size = {x=-1, y=1},
		textures = {"vlc_bows_arrow.png"},
...
local function spawn_item(self, pos)
	if not minetest.is_creative_enabled("") then
		local itemstring = "vlc_bows:arrow"
...
local function damage_particles(pos, is_critical)
	if is_critical then
		minetest.add_particlespawner({
			...
			vertical = false,
			texture = "vlc_particles_crit.png^[colorize:#bc7a57:127",
		})
	end
end
		...
		for _,obj in ipairs(objects) do
			if obj:is_player() then
				if self._collectable and not minetest.is_creative_enabled(obj:get_player_name()) then
					if obj:get_inventory():room_for_item("main", "vlc_bows:arrow") then
						obj:get_inventory():add_item("main", "vlc_bows:arrow")
			...
			if obj == self._shooter and self._lifetime > 0.5 or obj ~= self._shooter and (is_player or (lua and (lua.is_mob or lua._hittable_by_projectile))) then
				if obj:get_hp() > 0 then
					...
					if not lua or lua.name ~= "mobs_mc:enderman" then
						...
						if vlc_burning.is_burning(self.object) then
							vlc_burning.set_on_fire(obj, 5)
						end
						if not self._in_player and not self._blocked then
							vlc_util.deal_damage(obj, self._damage, {type = "arrow", source = self._shooter, direct = self.object})
							if self._extra_hit_func then
								self._extra_hit_func(obj)
							end
							if obj:is_player() then
								if not vlc_shields.is_blocking(obj) then
									local placement
					...
					if is_player then
						if self._shooter and self._shooter:is_player() and not self._in_player and not self._blocked then
							...
							minetest.sound_play({name="vlc_bows_hit_player", gain=0.1}, {to_player=self._shooter:get_player_name()}, true)
						end
					end

					if not self._in_player and not self._blocked then
						minetest.sound_play({name="vlc_bows_hit_other", gain=0.3}, {pos=self.object:get_pos(), max_hear_distance=16}, true)
					end
				end
...
minetest.register_on_respawnplayer(function(player)
	for _, obj in pairs(player:get_children()) do
		...
		if ent and ent.name and string.find(ent.name, "vlc_bows:arrow_entity") then
			obj:remove()
		end
	end
end)

minetest.register_entity("vlc_bows:arrow_entity", ARROW_ENTITY)

if minetest.get_modpath("vlc_core") and minetest.get_modpath("vlc_mobitems") then
	minetest.register_craft({
		output = "vlc_bows:arrow 4",
		recipe = {
			{"vlc_core:flint"},
			{"vlc_core:stick"},
