...
function vlc_bows.shoot_arrow(arrow_item, pos, dir, yaw, shooter, power, damage, is_critical, bow_stack, collectable)
	...
	local knockback
	if bow_stack then
		local enchantments = vlc_enchanting.get_enchantments(bow_stack)
		...
		if enchantments.flame then
			vlc_burning.set_on_fire(obj, math.huge)
		end
	end
	...
	minetest.sound_play("vlc_bows_bow_shoot", {pos=pos, max_hear_distance=16}, true)
...
local function player_shoot_arrow(itemstack, player, power, damage, is_critical)
	...
	local has_infinity_enchantment = vlc_enchanting.has_enchantment(player:get_wielded_item(), "infinity")
	...
	vlc_bows.shoot_arrow(arrow_itemstring, {x=playerpos.x,y=playerpos.y+1.5,z=playerpos.z}, dir, yaw, player, power, damage, is_critical, player:get_wielded_item(), not infinity_used)
	return true
end

-- Bow item, uncharged state
minetest.register_tool("vlc_bows:bow", {
	description = S("Bow"),
...
S("The speed and damage of the arrow increases the longer you charge. The regular damage of the arrow is between 1 and 9. At full charge, there's also a 20% of a critical hit, dealing 10 damage instead."),
	...
	inventory_image = "vlc_bows_bow.png",
...
local function reset_bows(player)
	...
	for place, stack in pairs(list) do
		if stack:get_name() == "vlc_bows:bow" or stack:get_name() == "vlc_bows:bow_enchanted" then
			stack:get_meta():set_string("active", "")
		elseif stack:get_name()=="vlc_bows:bow_0" or stack:get_name()=="vlc_bows:bow_1" or stack:get_name()=="vlc_bows:bow_2" then
			stack:set_name("vlc_bows:bow")
			stack:get_meta():set_string("active", "")
			list[place] = stack
		elseif stack:get_name()=="vlc_bows:bow_0_enchanted" or stack:get_name()=="vlc_bows:bow_1_enchanted" or stack:get_name()=="vlc_bows:bow_2_enchanted" then
			stack:set_name("vlc_bows:bow_enchanted")
			stack:get_meta():set_string("active", "")
			list[place] = stack
		end
	end
...
local function reset_bow_state(player, also_reset_bows)
	...
	if minetest.get_modpath("playerphysics") then
		playerphysics.remove_physics_factor(player, "speed", "vlc_bows:use_bow")
	end
...
for level=0, 2 do
	minetest.register_tool("vlc_bows:bow_"..level, {
		description = S("Bow"),
		_doc_items_create_entry = false,
		inventory_image = "vlc_bows_bow_"..level..".png",
		wield_scale = vlc_vars.tool_wield_scale,
		stack_max = 1,
		...
		on_drop = function(itemstack, dropper, pos)
			...
			if vlc_enchanting.is_enchanted(itemstack:get_name()) then
				itemstack:set_name("vlc_bows:bow_enchanted")
			else
				itemstack:set_name("vlc_bows:bow")
			end
...
controls.register_on_release(function(player, key, time)
	...
	if (wielditem:get_name()=="vlc_bows:bow_0" or wielditem:get_name()=="vlc_bows:bow_1" or wielditem:get_name()=="vlc_bows:bow_2" or
		wielditem:get_name()=="vlc_bows:bow_0_enchanted" or wielditem:get_name()=="vlc_bows:bow_1_enchanted" or wielditem:get_name()=="vlc_bows:bow_2_enchanted") then
		...
		if enchanted then
			wielditem:set_name("vlc_bows:bow_enchanted")
		else
			wielditem:set_name("vlc_bows:bow")
		end
...
controls.register_on_hold(function(player, key, time)
	...
	local wielditem = player:get_wielded_item()
		...
		and (wielditem:get_name()=="vlc_bows:bow" or wielditem:get_name()=="vlc_bows:bow_enchanted")
		and (wielditem:get_meta():get("active") or key == "zoom") and (creative or get_arrow(player)) then
		local enchanted = vlc_enchanting.is_enchanted(wielditem:get_name())
		if enchanted then
			wielditem:set_name("vlc_bows:bow_0_enchanted")
		else
			wielditem:set_name("vlc_bows:bow_0")
		end
		player:set_wielded_item(wielditem)
		if minetest.get_modpath("playerphysics") then
			-- Slow player down when using bow
			playerphysics.add_physics_factor(player, "speed", "vlc_bows:use_bow", PLAYER_USE_BOW_SPEED)
		end
		...
		if player:get_wield_index() == bow_index[name] then
			if type(bow_load[name]) == "number" then
				if wielditem:get_name() == "vlc_bows:bow_0" and minetest.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_HALF then
					wielditem:set_name("vlc_bows:bow_1")
				elseif wielditem:get_name() == "vlc_bows:bow_0_enchanted" and minetest.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_HALF then
					wielditem:set_name("vlc_bows:bow_1_enchanted")
				elseif wielditem:get_name() == "vlc_bows:bow_1" and minetest.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_FULL then
					wielditem:set_name("vlc_bows:bow_2")
				elseif wielditem:get_name() == "vlc_bows:bow_1_enchanted" and minetest.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_FULL then
					wielditem:set_name("vlc_bows:bow_2_enchanted")
				end
			else
				if wielditem:get_name() == "vlc_bows:bow_0" or wielditem:get_name() == "vlc_bows:bow_1" or wielditem:get_name() == "vlc_bows:bow_2" then
					wielditem:set_name("vlc_bows:bow")
				elseif wielditem:get_name() == "vlc_bows:bow_0_enchanted" or wielditem:get_name() == "vlc_bows:bow_1_enchanted" or wielditem:get_name() == "vlc_bows:bow_2_enchanted" then
					wielditem:set_name("vlc_bows:bow_enchanted")
				end
			end
			player:set_wielded_item(wielditem)
...
minetest.register_globalstep(function(dtime)
	for _, player in pairs(minetest.get_connected_players()) do
		...
		if type(bow_load[name]) == "number" and ((wielditem:get_name()~="vlc_bows:bow_0" and wielditem:get_name()~="vlc_bows:bow_1" and wielditem:get_name()~="vlc_bows:bow_2" and wielditem:get_name()~="vlc_bows:bow_0_enchanted" and wielditem:get_name()~="vlc_bows:bow_1_enchanted" and wielditem:get_name()~="vlc_bows:bow_2_enchanted") or wieldindex ~= bow_index[name]) then
			reset_bow_state(player, true)
		end
	end
end)
...
if minetest.get_modpath("vlc_core") and minetest.get_modpath("vlc_mobitems") then
	minetest.register_craft({
		output = "vlc_bows:bow",
		recipe = {
			{"", "vlc_core:stick", "vlc_mobitems:string"},
			{"vlc_core:stick", "", "vlc_mobitems:string"},
			{"", "vlc_core:stick", "vlc_mobitems:string"},
		}
	})
	minetest.register_craft({
		output = "vlc_bows:bow",
		recipe = {
			{"vlc_mobitems:string", "vlc_core:stick", ""},
			{"vlc_mobitems:string", "", "vlc_core:stick"},
			{"vlc_mobitems:string", "vlc_core:stick", ""},
		}
	})
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("tools", "vlc_bows:bow", "tools", "vlc_bows:bow_0")
	doc.add_entry_alias("tools", "vlc_bows:bow", "tools", "vlc_bows:bow_1")
	doc.add_entry_alias("tools", "vlc_bows:bow", "tools", "vlc_bows:bow_2")
end
