...
-- Flint and Steel
minetest.register_tool("vlc_fire:flint_and_steel", {
	...
	inventory_image = "vlc_fire_flint_and_steel.png",
	stack_max = 1,
		...
		if pointed_thing.type == "node" then
			...
			if nodedef and nodedef._on_ignite then
				...
				if not overwrite then
					vlc_fire.set_fire(pointed_thing, user, false)
				end
			else
				vlc_fire.set_fire(pointed_thing, user, false)
			end
	...
	_on_dispense = function(stack, pos, droppos, dropnode, dropdir)
		...
		if dropnode.name == "air" then
			minetest.set_node(droppos, {name="vlc_fire:fire"})
			if not minetest.is_creative_enabled("") then
				stack:add_wear(65535/65) -- 65 uses
			end
		-- Ignite TNT
		elseif dropnode.name == "vlc_tnt:tnt" then
			tnt.ignite(droppos)
			if not minetest.is_creative_enabled("") then
				stack:add_wear(65535/65) -- 65 uses
			end
...
minetest.register_craft({
	type = "shapeless",
	output = "vlc_fire:flint_and_steel",
	recipe = { "vlc_core:iron_ingot", "vlc_core:flint"},
})
