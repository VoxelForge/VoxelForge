...
local has_vlc_portals = minetest.get_modpath("vlc_portals")
...
local function spawn_fire(pos, age)
	minetest.set_node(pos, {name="vlc_fire:fire", param2 = age})
	minetest.check_single_for_falling({x=pos.x, y=pos.y+1, z=pos.z})
end

minetest.register_node("vlc_fire:fire", {
	description = S("Fire"),
	...
	on_construct = function(pos)
		...
		local dim = vlc_worlds.pos_to_dimension(bpos)
		if under == "vlc_nether:magma" or under == "vlc_nether:netherrack" or (under == "vlc_core:bedrock" and dim == "end") then
			minetest.swap_node(pos, {name = "vlc_fire:eternal_fire"})
		end

		if has_vlc_portals then
			vlc_portals.light_nether_portal(pos)
		end
	end,
	_vlc_blast_resistance = 0,
})

minetest.register_node("vlc_fire:eternal_fire", {
	...
	on_construct = function(pos)
		if has_vlc_portals then --Calling directly minetest.get_modpath consumes 4x more compute time
			vlc_portals.light_nether_portal(pos)
		end
	end,
	sounds = {},
...
function vlc_fire.update_player_sound(player)
	...
	local fpos, num = minetest.find_nodes_in_area(
		areamin,
		areamax,
		{"vlc_fire:fire", "vlc_fire:eternal_fire"}
	)
	-- Total number of flames in radius
	local flames = (num["vlc_fire:fire"] or 0) +
		(num["vlc_fire:eternal_fire"] or 0)
	-- Stop previous sound
...
minetest.register_globalstep(function(dtime)
	...
	for n = 1, #players do
		vlc_fire.update_player_sound(players[n])
	end
end)
...
minetest.register_abm({
	...
	nodenames = {"vlc_fire:fire", "vlc_fire:eternal_fire"},
...
if not fire_enabled then
	...
	minetest.register_abm({
		...
		nodenames = {"vlc_core:lava_source","vlc_nether:nether_lava_source"},
...
function vlc_fire.set_fire(pointed_thing, player, allow_on_fire)
	local pname
	if player == nil then
		pname = ""
	else
	...
	minetest.set_node(pointed_thing.above, {name="vlc_fire:fire"})
end

minetest.register_alias("vlc_fire:basic_flame", "vlc_fire:fire")
minetest.register_alias("fire:basic_flame", "vlc_fire:fire")
minetest.register_alias("fire:permanent_flame", "vlc_fire:eternal_fire")
