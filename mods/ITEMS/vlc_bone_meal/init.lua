...
function vlc_bone_meal.add_bone_meal_particle(pos, def)
	if not def then
		def = {}
	end
	minetest.add_particlespawner({
		...
		texture = "vlc_particles_bonemeal.png^[colorize:#00EE00:125", -- TODO: real MC color
		glow = def.glow or 1,
	})
end
...
local function bone_meal(itemstack, user, pointed_thing)
	...
	if udef and udef._on_bone_meal then
		...
		if udef._on_bone_meal(itemstack,user,pointed_thing, pointed_thing.under,unode) ~= false then
			vlc_bone_meal.add_bone_meal_particle(pointed_thing.under)
			vlc_bone_meal.add_bone_meal_particle(pointed_thing.above)
...
minetest.register_craftitem("vlc_bone_meal:bone_meal", {
	inventory_image = "vlc_bone_meal_bone_meal.png",
...
minetest.register_craft({
	output = "vlc_bone_meal:bone_meal 3",
	recipe = {{"vlc_mobitems:bone"}},
})
