...
local function get_anvil_formspec(set_name, player, cost)
	...
	if player and not minetest.is_creative_enabled(player:get_player_name()) and cost and cost > 0 then
		...
		cost_label = c..F(C(vlc_formspec.label_color, st)).."]"
		if player and vlc_experience.get_level(player) < cost then
			cost_label = c..F(C(vlc_colors.RED, st)).."]"
		end
	end
	return table.concat({
		"formspec_version[4]",
		...
		"label[4.125,0.375;" .. F(C(vlc_formspec.label_color, S("Repair and Name"))) .. "]",

		"image_button[0.875,0.375;1.75,1.75;vlc_anvils_inventory_hammer.png;anvil_set_name;]",
		...
		vlc_formspec.get_itemslot_bg_v4(1.625, 2.6, 1, 1),
		"list[context;input;1.625,2.6;1,1;]",

		"image[3.5,2.6;1,1;vlc_anvils_inventory_cross.png]",

		vlc_formspec.get_itemslot_bg_v4(5.375, 2.6, 1, 1),
		"list[context;input;5.375,2.6;1,1;1]",

		"image[6.75,2.6;2,1;vlc_anvils_inventory_arrow.png]",

		vlc_formspec.get_itemslot_bg_v4(9.125, 2.6, 1, 1),
...
local function clear_cost(meta, cost) meta:set_string("vlc_anvil:xp_cost", "") end

local function add_cost(meta, cost)
	local old = meta:get_int("vlc_anvil:xp_cost")
	meta:set_int("vlc_anvil:xp_cost", old + cost)
end
...
local function update_anvil_slots(meta, player)
	...
	if (not input1:is_empty() and not input2:is_empty()) then
		add_cost(meta, vlc_enchanting.get_prior_work_penalty(input1) + vlc_enchanting.get_prior_work_penalty(input2))
		-- Repair, if tool
		local def1 = input1:get_definition()
		local def2 = input2:get_definition()
		...
		local can_combine, enchanting_level_requirements = vlc_enchanting.combine(input1, input2)
		...
		if can_combine then
			...
			-- Example: Iron Pickaxe + Iron Ingot. `_repair_material = vlc_core:iron_ingot`
	...
	local cost = meta:get_int("vlc_anvil:xp_cost")
	local has_result = not ItemStack(new_output):is_empty()
	if new_output and vlc_experience.get_level(player) >= cost then
	...
	elseif has_result and vlc_experience.get_level(player) < cost then
		meta:set_string("formspec", get_anvil_formspec(new_name, player, cost))
	end
...
local drop_contents = vlc_util.drop_items_from_meta_container({"input"})
...
local function damage_anvil(pos)
	...
	if node.name == "vlc_anvils:anvil" then
		minetest.swap_node(pos, { name = "vlc_anvils:anvil_damage_1", param2 = node.param2 })
		damage_particles(pos, node)
		minetest.sound_play(vlc_sounds.node_sound_metal_defaults().dig, { pos = pos, max_hear_distance = 16 }, true)
		return false
	elseif node.name == "vlc_anvils:anvil_damage_1" then
		minetest.swap_node(pos, { name = "vlc_anvils:anvil_damage_2", param2 = node.param2 })
		damage_particles(pos, node)
		minetest.sound_play(vlc_sounds.node_sound_metal_defaults().dig, { pos = pos, max_hear_distance = 16 }, true)
		return false
	elseif node.name == "vlc_anvils:anvil_damage_2" then
		drop_contents(pos, node, minetest.get_meta(pos))
		minetest.sound_play(vlc_sounds.node_sound_metal_defaults().dug, { pos = pos, max_hear_distance = 16 }, true)
...
local anvildef = {
	...
	tiles = { "vlc_anvils_anvil_top_damaged_0.png^[transformR90", "vlc_anvils_anvil_base.png", "vlc_anvils_anvil_side.png" },
	...
	_vlc_hardness = 5,
	_vlc_after_falling = damage_anvil_by_falling,
	...
	allow_metadata_inventory_take = function(pos, listname, index, stack, player)
		...
		elseif listname == "output" and not minetest.is_creative_enabled(player:get_player_name()) then
			...
			local anvil_costs = meta:get_int("vlc_anvil:xp_cost")
	...
	on_metadata_inventory_take = function(pos, listname, index, stack, player)
		...
		if listname == "output" then
			...
			clear_cost(meta)
			if not minetest.is_creative_enabled(player:get_player_name()) then
				vlc_experience.set_level(player, player_level - anvil_costs)
			end
...
minetest.register_node("vlc_anvils:anvil", table.merge(anvildef, {
	description = S("Anvil"),
...
local anvildef1 = table.merge(anvildef, {
	...
	tiles = { "vlc_anvils_anvil_top_damaged_1.png^[transformR90", "vlc_anvils_anvil_base.png", "vlc_anvils_anvil_side.png" }
})
minetest.register_node("vlc_anvils:anvil_damage_1", anvildef1)

minetest.register_node("vlc_anvils:anvil_damage_2", table.merge(anvildef1, {
	description = S("Very Damaged Anvil"),
	groups = table.merge(anvildef.groups, {anvil = 3}),
	tiles = { "vlc_anvils_anvil_top_damaged_2.png^[transformR90", "vlc_anvils_anvil_base.png", "vlc_anvils_anvil_side.png" }
}))

if minetest.get_modpath("vlc_core") then
	minetest.register_craft({
		output = "vlc_anvils:anvil",
		recipe = {
			{ "vlc_core:ironblock", "vlc_core:ironblock", "vlc_core:ironblock" },
			{ "", "vlc_core:iron_ingot", "" },
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlc_anvils:anvil", "nodes", "vlc_anvils:anvil_damage_1")
	doc.add_entry_alias("nodes", "vlc_anvils:anvil", "nodes", "vlc_anvils:anvil_damage_2")
end

-- Legacy
minetest.register_lbm({
	label = "Update anvil formspecs (0.60.0)",
	name = "vlc_anvils:update_formspec_0_60_0",
