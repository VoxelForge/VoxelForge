function vlf_armor.play_equip_sound(stack, obj, pos, unequip)
	local def = stack:get_definition()
	local estr = "equip"
	if unequip then
		estr = "unequip"
	end
	local snd = def.sounds and def.sounds["_vlf_armor_" .. estr]
	if not snd then
		-- Fallback sound
		snd = { name = "vlf_armor_" .. estr .. "_generic" }
	end
	if snd then
		local dist = 8
		if pos then
...
function vlf_armor.on_equip(itemstack, obj)
	local def = itemstack:get_definition()
	vlf_armor.play_equip_sound(itemstack, obj)
...
function vlf_armor.on_unequip(itemstack, obj)
	local def = itemstack:get_definition()
	vlf_armor.play_equip_sound(itemstack, obj, nil, true)
...
function vlf_armor.equip(itemstack, obj, swap)
	local def = itemstack:get_definition()
	...
	local inv = vlf_util.get_inventory(obj, true)

	if not inv or inv:get_size("armor") == 0 then
		return itemstack
	end

	local element = vlf_armor.elements[def._vlf_armor_element or ""]
	...
	if element then
		...
		if swap or old_stack:is_empty() then
			...
			vlf_armor.on_equip(new_stack, obj)
		end
	end
...
function vlf_armor.equip_on_use(itemstack, player, pointed_thing)
	if not player or not player:is_player() then
		return itemstack
	end

	local new_stack = vlf_util.call_on_rightclick(itemstack, player, pointed_thing)
	if new_stack then
		return new_stack
	end

	return vlf_armor.equip(itemstack, player, true)
end
...
local function get_armor_texture(textures, name, modname, itemname, itemstring)
	...
	local func = function(obj, itemstack)
		...
		if vlf_enchanting.is_enchanted(itemstack:get_name()) then -- working with the original stack to know wether to apply enchanting overlay or not
			...
			core_armor_texture = core_armor_texture .. vlf_enchanting.overlay
		end
...
function vlf_armor.register_set(def)
	...
	if def.description then
		minetest.log("warning", "[vlf_armor] using the description field of armor set definitions is deprecated, please provide the localized strings in def.descriptions instead. Currently processing " .. def.name)
	...
	for name, element in pairs(vlf_armor.elements) do
		...
		groups.vlf_armor_points = def.points[name]
		groups.vlf_armor_toughness = def.toughness
		groups.vlf_armor_uses = (durabilities[name] or math.floor(def.durability * element.durability)) + 1
		...
		if def._vlf_upgradable and def._vlf_upgrade_item_material then
...
vlf_armor.protection_enchantments = {
	flags = {},
	types = {},
	wildcard = {},
}

function vlf_armor.register_protection_enchantment(def)
	local prot_def = {id = def.id, factor = def.factor}
	if def.damage_flag then
		local tbl = vlf_armor.protection_enchantments.flags[def.damage_flag] or {}
		table.insert(tbl, prot_def)
		vlf_armor.protection_enchantments.flags = tbl
	elseif def.damage_type then
		local tbl = vlf_armor.protection_enchantments.types[def.damage_type] or {}
		table.insert(tbl, prot_def)
		vlf_armor.protection_enchantments.types = tbl
	else
		table.insert(vlf_armor.protection_enchantments.wildcard, prot_def)
	end
	vlf_enchanting.enchantments[def.id] = {
		name = def.name,
...
function vlf_armor.update(obj)
	...
	if inv then
		for i = 2, 5 do
			...
			if not itemstack:is_empty() then
				...
				info.points = info.points + minetest.get_item_group(itemname, "vlf_armor_points")

				local mob_range_mob = def._vlf_armor_mob_range_mob
				...
				if mob_range_mob then
					...
					if factor then
						if factor > 0 then
							info.view_range_factors[mob_range_mob] = factor * def._vlf_armor_mob_range_factor
						end
					else
						info.view_range_factors[mob_range_mob] = def._vlf_armor_mob_range_factor
					end
				end
			end
		end
	...
	if obj:is_player() then
		vlf_armor.update_player(obj, info)
	else
...
function vlf_armor.trim(itemstack, overlay, color_string)
	local def = itemstack:get_definition()
	if not def._vlf_armor_texture and not vlf_armor.trims.blacklisted[itemstack:get_name()] then return end
	local meta = itemstack:get_meta()

	local piece_overlay = overlay
	local inv_overlay = ""
	local piece_type = def._vlf_armor_element
	...
	local color = vlf_armor.trims.colors[color_string]
	inv_overlay = inv_overlay .. "^[colorize:" .. color .. ":150)"
	piece_overlay = piece_overlay .. ".png"

	piece_overlay = "^(" .. piece_overlay .. "^[colorize:" .. color .. ":150)"

	meta:set_string("vlf_armor:trim_overlay" , piece_overlay) -- set textures to render on the player, will work for clients below 5.8 as well
	meta:set_string("vlf_armor:inv", inv_overlay) -- make 5.8+ clients display the fancy inv image, older ones will see no change in the *inventory* image
...
function vlf_armor.reload_trim_inv_image(itemstack)
	local meta = itemstack:get_meta()
	local inv_overlay = meta:get_string("vlf_armor:inv")
	local def = itemstack:get_definition()
...
tt.register_snippet(function(itemstring, toolcaps, stack)
	...
	local full_overlay = meta:get_string("vlf_armor:trim_overlay")
