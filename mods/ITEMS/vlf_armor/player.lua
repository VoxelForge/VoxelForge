vlf_player.player_register_model("vlf_armor_character.b3d", {
	animation_speed = 30,
	textures = {
		"character.png",
		"blank.png",
...
vlf_player.player_register_model("vlf_armor_character_female.b3d", {
	animation_speed = 30,
	textures = {
		"character.png",
		"blank.png",
...
function vlf_armor.update_player(player, info)
	vlf_player.player_set_armor(player, info.texture)

	local meta = player:get_meta()
	meta:set_int("vlf_armor:armor_points", info.points)

	vlf_armor.player_view_range_factors[player] = info.view_range_factors
end
...
local function limit_put(_, inventory, index, stack, count)
	...
	local element = def._vlf_armor_element

	if not element then
		return 0
	end

	local element_index = vlf_armor.elements[element].index
...
local function on_put(player, inventory, index, stack)
	if index == 1 then
		vlf_armor.equip(stack, player)
		inventory:set_stack("armor", 1, nil)
	else
		vlf_armor.on_equip(stack, player)
	end
end
...
minetest.register_on_player_inventory_action(function(player, action, inventory, inventory_info)
	if is_armor_action(inventory_info) then
		...
		elseif action == "take" then
			vlf_armor.on_unequip(inventory_info.stack, player)
		else
			...
			elseif inventory_info.from_list == "armor" then
				vlf_armor.on_unequip(stack, player)
			end
		end
	end
end)
...
minetest.register_on_joinplayer(function(player)
	...
	if not minetest.global_exists("vlf_skins") then
		vlf_player.player_set_model(player, "vlf_armor_character.b3d")
	end
...
minetest.register_on_leaveplayer(function(player)
	vlf_armor.player_view_range_factors[player] = nil
end)
