...
local function get_texture_function(texture)
	return function(_, itemstack)
		local out
		local color = itemstack:get_meta():get_string("vlf_armor:color")
		...
		if vlf_enchanting.is_enchanted(itemstack:get_name()) then
			return out..vlf_enchanting.overlay
		else
			return out
		end
	end
...
function vlf_armor.colorize_leather_armor(itemstack, colorstring)
	...
	meta:set_string("vlf_armor:color", colorstring)
...
function vlf_armor.wash_leather_armor(itemstack)
	if not itemstack or itemstack:get_definition().groups.armor_leather ~= 1 then
		return
	end
	local meta = itemstack:get_meta()
	meta:set_string("vlf_armor:color", "")
	meta:set_string("inventory_image", "")
	tt.reload_itemstack_description(itemstack)
	return itemstack
end
...
vlf_armor.register_set({
	...
	textures = {
		head = get_texture_function("vlf_armor_helmet_leather.png"),
		torso = get_texture_function("vlf_armor_chestplate_leather.png"),
		legs = get_texture_function("vlf_armor_leggings_leather.png"),
...
tt.register_priority_snippet(function(_, _, itemstack)
	...
	if color and color ~= "" then
		local text = C(vlf_colors.GRAY, "Dyed: "..color)
		return text, false
	end
end)
	...
	for _, item in pairs(old_craft_grid) do
		...
		if name ~= "" then
			...
			elseif minetest.get_item_group(name, "dye") > 0 then
				...
				dye_color = vlf_dyes.colors[minetest.registered_items[name]._color].rgb
			else return end
		end
	end
	return vlf_armor.colorize_leather_armor(found_la, dye_color) or ItemStack()
end
...
minetest.register_chatcommand("color_leather", {
	...
	func = function(name, param)
		...
		if player then
			...
			if param == "wash" then
				player:set_wielded_item(vlf_armor.wash_leather_armor(item))
			...
			player:set_wielded_item(vlf_armor.colorize_leather_armor(item, colorstring))
