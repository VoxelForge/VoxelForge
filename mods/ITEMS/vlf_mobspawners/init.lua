...
local function spawn_doll(pos)
	return minetest.add_entity({x=pos.x, y=pos.y-0.3, z=pos.z}, "vlf_mobspawners:doll")
end
...
function vlf_mobspawners.setup_spawner(pos, Mob, MinLight, MaxLight, MaxMobsInArea, PlayerDistance, YOffset)
...
local function spawn_mobs(pos, elapsed)
	...
	if not vlf_mobs.spawning_mobs[mob] then
		minetest.log("error", "[vlf_mobspawners] Mob Spawner: Mob doesn't exist: "..mob)
		return
	end
...
-- MUST call vlf_mobspawners.setup_spawner right after the spawner has been placed.
minetest.register_node("vlf_mobspawners:spawner", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if success then
			...
			vlf_mobspawners.setup_spawner(placepos)
		end
	...
	on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
		...
		vlf_mobspawners.setup_spawner(pos, itemstack:get_name())
...
doll_def.on_step = function(self, dtime)
	...
	if self.timer > 1 then
		if n and n.name and n.name ~= "vlf_mobspawners:spawner" then
			self.object:remove()
		end
	end
end
