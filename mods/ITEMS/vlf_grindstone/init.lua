...
local grindstone_formspec = table.concat({
	...
	"label[0.375,0.375;" .. F(C(vlf_formspec.label_color, S("Repair & Disenchant"))) .. "]",

	vlf_formspec.get_itemslot_bg_v4(2.875, 1.25, 1, 1),
	"list[context;input;2.875,1.25;1,1;]",

	vlf_formspec.get_itemslot_bg_v4(2.875, 2.625, 1, 1),
	"list[context;input;2.875,2.625;1,1;1]",

	"image[5.125,1.95;1.5,1;gui_crafting_arrow.png]",

	vlf_formspec.get_itemslot_bg_v4(7.875, 1.9375, 1, 1),
...
function vlf_grindstone.remove_enchant_name(stack)
...
function vlf_grindstone.disenchant(stack)
	local def = stack:get_definition()
	local meta = stack:get_meta()
	local new_item = ""
	if def.type == "tool" and vlf_enchanting.is_enchanted(stack:get_name()) then
		new_item = create_new_item(vlf_grindstone.remove_enchant_name(stack), meta, stack:get_wear())
		new_item = transfer_curse(stack, new_item)
	elseif stack:get_name() == "vlf_enchanting:book_enchanted" then
		new_item = create_new_item("vlf_books:book", meta, nil)
		new_item = transfer_curse(stack, new_item)
	end
	return new_item
end
...
local function update_grindstone_slots(meta)
	...
	if not input1:is_empty() and not input2:is_empty() then
		...
		local name1 = vlf_grindstone.remove_enchant_name(input1)
		local name2 = vlf_grindstone.remove_enchant_name(input2)
		...
		if input2:is_empty() and not input1:is_empty() then
			new_output = vlf_grindstone.disenchant(input1)
		elseif input1:is_empty() and not input2:is_empty() then
			new_output = vlf_grindstone.disenchant(input2)
		end
	end
...
minetest.register_node("vlf_grindstone:grindstone", {
	...
	groups = { pickaxey = 1, deco_block = 1, _vlf_partial = 2, },
	...
	on_metadata_inventory_take = function(pos, listname, _, stack)
		...
		if listname == "output" then
			...
			if vlf_experience.throw_xp and xp_earnt > 0 then
				vlf_experience.throw_xp(pos, xp_earnt)
			end
...
minetest.register_craft({
	output = "vlf_grindstone:grindstone",
	recipe = {
		{ "vlf_core:stick", "vlf_stairs:slab_stone_rough", "vlf_core:stick" },
