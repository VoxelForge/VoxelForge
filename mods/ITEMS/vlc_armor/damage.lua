local function use_durability(obj, inv, index, stack, uses)
	...
	vlc_util.use_item_durability(stack, uses)
...
vlc_damage.register_modifier(function(obj, damage, reason)
		...
		for name, element in pairs(vlc_armor.elements) do
			...
			if not itemstack:is_empty() then
				...
				if not flags.bypasses_armor and minetest.get_item_group(itemname, "non_combat_armor") == 0 then
					points = points + minetest.get_item_group(itemname, "vlc_armor_points")
					toughness = toughness + minetest.get_item_group(itemname, "vlc_armor_toughness")
				...
				if not flags.bypasses_magic then
					...
					add_enchantments(vlc_armor.protection_enchantments.wildcard)
					add_enchantments(vlc_armor.protection_enchantments.types[reason.type])

					for flag, value in pairs(flags) do
						if value then
							add_enchantments(vlc_armor.protection_enchantments.flags[flag])
						end
					end
				end
	...
	if thorns_damage > 0 and reason.type ~= "thorns" and reason.source ~= obj then
		vlc_util.deal_damage(reason.source, thorns_damage, {type = "thorns", direct = obj})
		-- vlc_util.deal_damage may remove object immediately
