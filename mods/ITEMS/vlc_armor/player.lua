vlc_player.player_register_model("vlc_armor_character.b3d", {
	animation_speed = 30,
	textures = {
		"character.png",
		"blank.png",
...
vlc_player.player_register_model("vlc_armor_character_female.b3d", {
	animation_speed = 30,
	textures = {
		"character.png",
		"blank.png",
...
function vlc_armor.update_player(player, info)
	vlc_player.player_set_armor(player, info.texture)

	local meta = player:get_meta()
	meta:set_int("vlc_armor:armor_points", info.points)

	vlc_armor.player_view_range_factors[player] = info.view_range_factors
end
...
local function limit_put(player, inventory, index, stack, count)
	...
	local element = def._vlc_armor_element

	if not element then
		return 0
	end

	local element_index = vlc_armor.elements[element].index
...
local function limit_take(player, inventory, index, stack, count)
	if vlc_enchanting.has_enchantment(stack, "curse_of_binding") and not minetest.is_creative_enabled(player:get_player_name()) then
		return 0
	end
...
local function on_put(player, inventory, index, stack)
	if index == 1 then
		vlc_armor.equip(stack, player)
		inventory:set_stack("armor", 1, nil)
	else
		vlc_armor.on_equip(stack, player)
	end
end
...
minetest.register_on_player_inventory_action(function(player, action, inventory, inventory_info)
	if is_armor_action(inventory_info) then
		...
		elseif action == "take" then
			vlc_armor.on_unequip(inventory_info.stack, player)
		else
			...
			elseif inventory_info.from_list == "armor" then
				vlc_armor.on_unequip(stack, player)
			end
		end
	end
end)
...
minetest.register_on_joinplayer(function(player)
	...
	if not minetest.global_exists("vlc_skins") then
		vlc_player.player_set_model(player, "vlc_armor_character.b3d")
	end
...
minetest.register_on_leaveplayer(function(player)
	vlc_armor.player_view_range_factors[player] = nil
end)
