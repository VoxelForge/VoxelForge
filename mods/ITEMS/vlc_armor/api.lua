function vlc_armor.play_equip_sound(stack, obj, pos, unequip)
	local def = stack:get_definition()
	local estr = "equip"
	if unequip then
		estr = "unequip"
	end
	local snd = def.sounds and def.sounds["_vlc_armor_" .. estr]
	if not snd then
		-- Fallback sound
		snd = { name = "vlc_armor_" .. estr .. "_generic" }
	end
	if snd then
		local dist = 8
		if pos then
...
function vlc_armor.on_equip(itemstack, obj)
	local def = itemstack:get_definition()
	vlc_armor.play_equip_sound(itemstack, obj)
	if def._on_equip then
		def._on_equip(obj, itemstack)
	end
	vlc_armor.update(obj)
end

function vlc_armor.on_unequip(itemstack, obj)
	local def = itemstack:get_definition()
	vlc_armor.play_equip_sound(itemstack, obj, nil, true)
...
function vlc_armor.equip(itemstack, obj, swap)
	local def = itemstack:get_definition()
	...
	local inv = vlc_util.get_inventory(obj, true)

	if not inv or inv:get_size("armor") == 0 then
		return itemstack
	end

	local element = vlc_armor.elements[def._vlc_armor_element or ""]
	...
	if element then
		...
		if swap or old_stack:is_empty() then
			...
			vlc_armor.on_equip(new_stack, obj)
		end
	end
...
function vlc_armor.equip_on_use(itemstack, player, pointed_thing)
	if not player or not player:is_player() then
		return itemstack
	end

	local new_stack = vlc_util.call_on_rightclick(itemstack, player, pointed_thing)
	if new_stack then
		return new_stack
	end

	return vlc_armor.equip(itemstack, player, true)
end
...
local function get_armor_texture(textures, name, modname, itemname, itemstring)
	...
	vlc_armor.trims.core_textures[itemstring] = core_texture
	local func = function(obj, itemstack)
		local overlay = itemstack:get_meta():get_string("vlc_armor:trim_overlay")
		local stack_name = vlc_grindstone.remove_enchant_name(itemstack) -- gets original itemstring if enchanted, no need to store (nearly) identical values
		local core_armor_texture = vlc_armor.trims.core_textures[stack_name]

		if vlc_enchanting.is_enchanted(itemstack:get_name()) then -- working with the original stack to know wether to apply enchanting overlay or not
			...
			core_armor_texture = core_armor_texture .. vlc_enchanting.overlay
		end
...
function vlc_armor.register_set(def)
	...
	if def.description then
		minetest.log("warning", "[vlc_armor] using the description field of armor set definitions is deprecated, please provide the localized strings in def.descriptions instead. Currently processing " .. def.name)
		local S = minetest.get_translator(modname)
		for name, element in pairs(vlc_armor.elements) do
	...
	for name, element in pairs(vlc_armor.elements) do
		...
		groups.vlc_armor_points = def.points[name]
		groups.vlc_armor_toughness = def.toughness
		groups.vlc_armor_uses = (durabilities[name] or math.floor(def.durability * element.durability)) + 1
		...
		if def._vlc_upgradable and def._vlc_upgrade_item_material then
			upgrade_item = itemstring:gsub("_[%l%d]*$",def._vlc_upgrade_item_material)
		end

		minetest.register_tool(itemstring, {
			description = descriptions[name],
			_doc_items_longdesc = vlc_armor.longdesc,
			_doc_items_usagehelp = vlc_armor.usage,
			inventory_image = modname .. "_inv_" .. itemname .. ".png",
			_repair_material = def.repair_material or def.craft_material,
			groups = groups,
			sounds = {
				_vlc_armor_equip = def.sound_equip or modname .. "_equip_" .. def.name,
				_vlc_armor_unequip = def.sound_unequip or modname .. "_unequip_" .. def.name,
			},
			on_place = vlc_armor.equip_on_use,
			on_secondary_use = vlc_armor.equip_on_use,
			...
			_vlc_armor_element = name,
			_vlc_armor_texture = get_armor_texture(textures, name, modname, itemname, itemstring),
			_vlc_upgradable = def._vlc_upgradable,
			_vlc_upgrade_item = upgrade_item
		})
...
vlc_armor.protection_enchantments = {
	flags = {},
	types = {},
	wildcard = {},
}

function vlc_armor.register_protection_enchantment(def)
	local prot_def = {id = def.id, factor = def.factor}
	if def.damage_flag then
		local tbl = vlc_armor.protection_enchantments.flags[def.damage_flag] or {}
		table.insert(tbl, prot_def)
		vlc_armor.protection_enchantments.flags = tbl
	elseif def.damage_type then
		local tbl = vlc_armor.protection_enchantments.types[def.damage_type] or {}
		table.insert(tbl, prot_def)
		vlc_armor.protection_enchantments.types = tbl
	else
		table.insert(vlc_armor.protection_enchantments.wildcard, prot_def)
	end
	vlc_enchanting.enchantments[def.id] = {
		name = def.name,
...
function vlc_armor.update(obj)
	local info = {points = 0, view_range_factors = {}}

	local inv = vlc_util.get_inventory(obj)
	...
	if inv then
		for i = 2, 5 do
			...
			if not itemstack:is_empty() then
				...
				local texture = def._vlc_armor_texture
				...
				info.points = info.points + minetest.get_item_group(itemname, "vlc_armor_points")

				local mob_range_mob = def._vlc_armor_mob_range_mob
				...
				if mob_range_mob then
					...
					if factor then
						if factor > 0 then
							info.view_range_factors[mob_range_mob] = factor * def._vlc_armor_mob_range_factor
						end
					else
						info.view_range_factors[mob_range_mob] = def._vlc_armor_mob_range_factor
					end
				end
			end
		end
	...
	if obj:is_player() then
		vlc_armor.update_player(obj, info)
	else
...
function vlc_armor.trim(itemstack, overlay, color_string)
	local def = itemstack:get_definition()
	if not def._vlc_armor_texture and not vlc_armor.trims.blacklisted[itemstack:get_name()] then return end
	local meta = itemstack:get_meta()

	local piece_overlay = overlay
	local inv_overlay = ""
	local piece_type = def._vlc_armor_element
	...
	local color = vlc_armor.trims.colors[color_string]
	inv_overlay = inv_overlay .. "^[colorize:" .. color .. ":150)"
	piece_overlay = piece_overlay .. ".png"

	piece_overlay = "^(" .. piece_overlay .. "^[colorize:" .. color .. ":150)"

	meta:set_string("vlc_armor:trim_overlay" , piece_overlay) -- set textures to render on the player, will work for clients below 5.8 as well
	meta:set_string("vlc_armor:inv", inv_overlay) -- make 5.8+ clients display the fancy inv image, older ones will see no change in the *inventory* image
...
function vlc_armor.reload_trim_inv_image(itemstack)
	local meta = itemstack:get_meta()
	local inv_overlay = meta:get_string("vlc_armor:inv")
	local def = itemstack:get_definition()
...
tt.register_snippet(function(itemstring, toolcaps, stack)
	...
	if not vlc_armor.is_trimmed(stack) then return nil end
	...
	local full_overlay = meta:get_string("vlc_armor:trim_overlay")
	local trim_name = full_overlay:match("%((.-)%_")
	return "Upgrade:\n " .. trim_name:gsub("^%l", string.upper) .. " Armor Trim"
end)

function vlc_armor.is_trimmed(itemstack)
	return itemstack:get_meta():get_string("vlc_armor:trim_overlay") ~= ""
end
