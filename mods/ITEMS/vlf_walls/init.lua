...
function vlf_walls.update_wall(pos)
...
local function update_wall_global(pos)
	for i = 1,5 do
		local dir = directions[i]
		vlf_walls.update_wall({x = pos.x + dir.x, y = pos.y + dir.y, z = pos.z + dir.z})
	end
end
...
function vlf_walls.register_wall(nodename, description, source, tiles, inventory_image, groups, sounds, overrides)
	...
	minetest.register_node(nodename, table.merge({
		...
		on_construct = vlf_walls.update_wall,
		sounds = sounds,
		_vlf_blast_resistance = 6,
		_vlf_hardness = 2,
	}, overrides or {}))
...
function vlf_walls.register_wall_def(name,def)
	local source = def.source
	def.source = nil
	vlf_walls.register_wall(name, nil, source, nil, nil, nil, nil, def)
end
