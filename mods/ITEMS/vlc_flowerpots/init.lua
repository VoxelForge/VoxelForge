...
vlc_flowerpots.registered_pots = {}
...
minetest.register_node("vlc_flowerpots:flower_pot", {
	...
	mesh = "flowerpot.obj",
	tiles = {
		"vlc_flowerpots_flowerpot.png",
	},
	use_texture_alpha = "clip",
	visual_scale = 0.5,
	wield_image = "vlc_flowerpots_flowerpot_inventory.png",
	paramtype = "light",
	sunlight_propagates = true,
	selection_box = pot_box,
	collision_box = pot_box,
	is_ground_content = false,
	inventory_image = "vlc_flowerpots_flowerpot_inventory.png",
	...
	on_rightclick = function(pos, node, clicker, itemstack)
		...
		if vlc_flowerpots.registered_pots[item] then
			minetest.swap_node(pos, { name = "vlc_flowerpots:flower_pot_" .. vlc_flowerpots.registered_pots[item] })
...
minetest.register_craft({
	output = "vlc_flowerpots:flower_pot",
	recipe = {
...
function vlc_flowerpots.register_potted_flower(name, def)
	vlc_flowerpots.registered_pots[name] = def.name
	minetest.register_node(":vlc_flowerpots:flower_pot_" .. def.name, {
		description = def.desc .. " " .. S("Flower Pot"),
		_doc_items_create_entry = false,
		drawtype = "mesh",
		mesh = "flowerpot.obj",
		tiles = {
			"[combine:32x32:0,0=vlc_flowerpots_flowerpot.png:0,0=" .. def.image,
		},
		use_texture_alpha = "clip",
		visual_scale = 0.5,
		paramtype = "light",
		...
		on_rightclick = function(pos, item, clicker)
			...
			minetest.set_node(pos, { name = "vlc_flowerpots:flower_pot" })
		end,
		drop = {
			items = {
				{ items = { "vlc_flowerpots:flower_pot", name } },
			},
		},
	})
	-- Add entry alias for the Help
	if has_doc then
		doc.add_entry_alias("nodes", "vlc_flowerpots:flower_pot", "nodes", "vlc_flowerpots:flower_pot_" .. name)
	end
end

function vlc_flowerpots.register_potted_cube(name, def)
	vlc_flowerpots.registered_pots[name] = def.name
	minetest.register_node(":vlc_flowerpots:flower_pot_" .. def.name, {
		description = def.desc .. " " .. S("Flower Pot"),
		_doc_items_create_entry = false,
	...
	-- Add entry alias for the Help
	if has_doc then
		doc.add_entry_alias("nodes", "vlc_flowerpots:flower_pot", "nodes", "vlc_flowerpots:flower_pot_" .. def.name)
	end
end
