...
vlc_beacons = {
	blocks ={"vlc_core:diamondblock","vlc_core:ironblock","vlc_core:goldblock","vlc_core:emeraldblock","vlc_nether:netheriteblock"},
	fuel = {"vlc_core:diamond","vlc_core:emerald","vlc_core:iron_ingot","vlc_core:gold_ingot","vlc_nether:netherite_ingot"}
}
...
minetest.register_node("vlc_beacons:beacon_beam", {
...
mesecon.register_mvps_stopper("vlc_beacons:beacon_beam")
...
local formspec_string=
	...
	"image_button[5.2,1.5;1,1;vlc_potions_effect_swift.png;swiftness;]"..
	"image_button[5.2,3;1,1;vlc_potions_effect_leaping.png;leaping;]"..
	"image_button[5.2,4.5;1,1;vlc_potions_effect_strong.png;strenght;]"..
	"image_button[5.2,6;1,1;vlc_potions_effect_regenerating.png;regeneration;]"..

	"item_image[1,7;1,1;vlc_core:diamond]"..
	"item_image[2.2,7;1,1;vlc_core:emerald]"..
	"item_image[3.4,7;1,1;vlc_core:iron_ingot]"..
	"item_image[4.6,7;1,1;vlc_core:gold_ingot]"..
	"item_image[5.8,7;1,1;vlc_nether:netherite_ingot]"..

	vlc_formspec.get_itemslot_bg(7.2,7,1,1)..
	"list[context;input;7.2,7;1,1;]"..
	vlc_formspec.get_itemslot_bg(1,9,9,3)..
	"list[current_player;main;1,9;9,3;9]"..
	vlc_formspec.get_itemslot_bg(1,12.5,9,1)..
...
local function remove_beacon_beam(pos)
	for y=pos.y, pos.y+301 do
		...
		if node.name ~= "air" and node.name ~= "vlc_core:bedrock" and node.name ~= "vlc_core:void" then
			...
			if node.name == "vlc_beacons:beacon_beam" then
	...
	for y_offset = 1,4 do
		...
		for block_x = (pos.x-y_offset),(pos.x+y_offset) do
			for block_z = (pos.z-y_offset),(pos.z+y_offset) do
				...
				for _, beacon_block in pairs(vlc_beacons.blocks) do
...
local function clear_obstructed_beam(pos)
	for y=pos.y+1, pos.y+100 do
		...
		if nodename ~= "vlc_core:bedrock" and nodename ~= "air" and nodename ~= "vlc_core:void" and nodename ~= "ignore" then --ignore means not loaded, let's just assume that's air
			if nodename ~="vlc_beacons:beacon_beam" then
...
local function effect_player(effect,pos,power_level, effect_level,player)
	...
	if effect == "swiftness" then
		vlc_potions.swiftness_func(player,effect_level,16)
	elseif effect == "leaping" then
		vlc_potions.leaping_func(player, effect_level, 16)
	elseif effect == "strenght" then
		vlc_potions.strength_func(player, effect_level, 16)
	elseif effect == "regeneration" then
		vlc_potions.regeneration_func(player, effect_level, 16)
	end
end
...
minetest.register_node("vlc_beacons:beacon", {
	...
	on_destruct = function(pos)
		...
		if not input:is_empty() then
			local p = {x=pos.x+math.random(0, 10)/10-0.5, y=pos.y, z=pos.z+math.random(0, 10)/10-0.5} --from vlc_anvils
	...
	on_receive_fields = function(pos, formname, fields, sender)
		if fields.swiftness or fields.regeneration or fields.leaping or fields.strenght then
			...
			for _, item in ipairs(vlc_beacons.fuel) do
			...
			if successful then
				if power_level == 4 then
					awards.unlock(sender:get_player_name(),"vlc:maxed_beacon")
				end
				awards.unlock(sender:get_player_name(),"vlc:beacon")
				input:take_item()
				...
				for y = pos.y +1, pos.y + 201 do
					...
					if node.name == "air" then
						minetest.set_node({x=pos.x,y=y,z=pos.z},{name="vlc_beacons:beacon_beam",param2=beam_palette_index})
					end
				end
	...
	light_source = 14,
	groups = {handy=1},
	drop = "vlc_beacons:beacon",
	sounds = vlc_sounds.node_sound_glass_defaults(),
	_vlc_hardness = 3,
})

mesecon.register_mvps_stopper("vlc_beacons:beacon")
vlc_wip.register_wip_item("vlc_beacons:beacon")

function vlc_beacons.register_beaconblock (itemstring)--API function for other mods
	table.insert(vlc_beacons.blocks, itemstring)
end

function vlc_beacons.register_beaconfuel(itemstring)
	table.insert(vlc_beacons.fuel, itemstring)
end

minetest.register_abm{
	label="update beacon beam",
	nodenames = {"vlc_beacons:beacon_beam"},
	interval = 1,
	chance = 1,
	action = function(pos)
		...
		if node_below.name ~= "vlc_beacons:beacon" and minetest.get_item_group(node_below.name,"material_glass") == 0 and node_below.name ~= "vlc_beacons:beacon_beam" then
			if minetest.get_node({x=pos.x,y=pos.y-2,z=pos.z}).name == "vlc_beacons:beacon" then
				set_node_if_clear({x=pos.x,y=pos.y-1,z=pos.z},{name="vlc_beacons:beacon_beam",param2=0})
			end
			remove_beacon_beam(pos)
		elseif node_above.name == "air" or (node_above.name == "vlc_beacons:beacon_beam" and node_above.param2 ~= node_current.param2) then
			set_node_if_clear({x=pos.x,y=pos.y+1,z=pos.z},{name="vlc_beacons:beacon_beam",param2=node_current.param2})
		elseif minetest.get_item_group(node_above.name, "glass") ~= 0 or minetest.get_item_group(node_above.name,"material_glass") ~= 0 then
			set_node_if_clear({x=pos.x,y=pos.y+2,z=pos.z},{name="vlc_beacons:beacon_beam",param2=get_beacon_beam(node_above.name)})
		end
	end,
}
...
minetest.register_craft({
	output = "vlc_beacons:beacon",
	recipe = {
		{"vlc_core:glass", "vlc_core:glass", "vlc_core:glass"},
		{"vlc_core:glass", "vlc_mobitems:nether_star", "vlc_core:glass"},
