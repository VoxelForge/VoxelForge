...
local grindstone_formspec = table.concat({
	...
	"label[0.375,0.375;" .. F(C(vlc_formspec.label_color, S("Repair & Disenchant"))) .. "]",

	vlc_formspec.get_itemslot_bg_v4(2.875, 1.25, 1, 1),
	"list[context;input;2.875,1.25;1,1;]",

	vlc_formspec.get_itemslot_bg_v4(2.875, 2.625, 1, 1),
	"list[context;input;2.875,2.625;1,1;1]",

	"image[5.125,1.95;1.5,1;gui_crafting_arrow.png]",

	vlc_formspec.get_itemslot_bg_v4(7.875, 1.9375, 1, 1),
...
local function transfer_curse(old_itemstack, new_itemstack)
	local enchants = vlc_enchanting.get_enchantments(old_itemstack)
	for enchant, level in pairs(enchants) do
		if vlc_enchanting.enchantments[enchant].curse == true then
			new_itemstack = vlc_enchanting.enchant(new_itemstack, enchant, level)
		end
	end
...
local function calculate_xp(stack)
	local xp = 0
	local enchants = vlc_enchanting.get_enchantments(stack)
	for enchant, level in pairs(enchants) do
		if level > 0 and vlc_enchanting.enchantments[enchant].curse == false then
...
function vlc_grindstone.remove_enchant_name(stack)
	local name = stack:get_name()
	if vlc_enchanting.is_enchanted(name) then
...
function vlc_grindstone.disenchant(stack)
	local def = stack:get_definition()
	local meta = stack:get_meta()
	local new_item = ""
	if def.type == "tool" and vlc_enchanting.is_enchanted(stack:get_name()) then
		new_item = create_new_item(vlc_grindstone.remove_enchant_name(stack), meta, stack:get_wear())
		new_item = transfer_curse(stack, new_item)
	elseif stack:get_name() == "vlc_enchanting:book_enchanted" then
		new_item = create_new_item("vlc_books:book", meta, nil)
		new_item = transfer_curse(stack, new_item)
	end
	return new_item
end
...
local function update_grindstone_slots(meta)
	...
	if not input1:is_empty() and not input2:is_empty() then
		...
		local name1 = vlc_grindstone.remove_enchant_name(input1)
		local name2 = vlc_grindstone.remove_enchant_name(input2)
		...
		if input2:is_empty() and not input1:is_empty() then
			new_output = vlc_grindstone.disenchant(input1)
		elseif input1:is_empty() and not input2:is_empty() then
			new_output = vlc_grindstone.disenchant(input2)
		end
	end
...
minetest.register_node("vlc_grindstone:grindstone", {
	...
	after_dig_node = vlc_util.drop_items_from_meta_container({"input"}),
	...
	on_metadata_inventory_take = function(pos, listname, index, stack, player)
		...
		if listname == "output" then
			...
			if vlc_experience.throw_xp and xp_earnt > 0 then
				vlc_experience.throw_xp(pos, xp_earnt)
			end
...
minetest.register_craft({
	output = "vlc_grindstone:grindstone",
	recipe = {
		{ "vlc_core:stick", "vlc_stairs:slab_stone_rough", "vlc_core:stick" },
