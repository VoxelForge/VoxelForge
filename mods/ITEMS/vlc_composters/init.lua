...
minetest.register_craft({
	output = "vlc_composters:composter",
	recipe = {
...
minetest.register_craft({
	type = "fuel",
	recipe = "vlc_composters:composter",
	burntime = 15,
})
...
local function composter_add_item(pos, node, player, itemstack, pointed_thing)
	...
	if chance > 0 then
		...
		if chance >= rand then
			...
			local level = minetest.registered_nodes[node.name]["_vlc_compost_level"]
			-- spawn green particles above new layer
			vlc_bone_meal.add_bone_meal_particle(vector.offset(pos, 0, level/8, 0))
			...
			minetest.swap_node(pos, {name = "vlc_composters:composter_" .. level})
...
local function composter_ready(pos)
	minetest.swap_node(pos, {name = "vlc_composters:composter_ready"})
...
local function composter_harvest(pos, node, player, itemstack, pointed_thing)
	...
	minetest.swap_node(pos, {name="vlc_composters:composter"})
	minetest.add_item(pos, "vlc_bone_meal:bone_meal")
	return itemstack
end
...
local function composter_level(node)
	local nn = node.name
	if nn == "vlc_composters:composter" then
		return 0
	elseif nn == "vlc_composters:composter_1" then
		return 1
	elseif nn == "vlc_composters:composter_2" then
		return 2
	elseif nn == "vlc_composters:composter_3" then
		return 3
	elseif nn == "vlc_composters:composter_4" then
		return 4
	elseif nn == "vlc_composters:composter_5" then
		return 5
	elseif nn == "vlc_composters:composter_6" then
		return 6
	elseif nn == "vlc_composters:composter_7" then
		return 7
	else
		return nil
	end
...
for i = 1, 7 do
	assert(composter_level({name = "vlc_composters:composter_" .. i}) == i)
end

assert(composter_level({name = "vlc_composters:composter"}) == 0)
assert(composter_level({name = "vlc_composters:some_other_node"}) == nil)

local function on_hopper_out(uppos, pos)
	-- Get bonemeal from composter above
	local upnode = minetest.get_node(uppos)
	if upnode.name == "vlc_composters:composter_ready" then
		local meta = minetest.get_meta(pos)
		local inv = meta:get_inventory()

		minetest.swap_node(uppos, {name = "vlc_composters:composter"})

		inv:add_item("main", "vlc_bone_meal:bone_meal")
	end
end
		...
		for i = 1, 5 do
			...
			if compchance > 0 then
				...
				if compchance >= math.random(0, 100) then
					vlc_bone_meal.add_bone_meal_particle(vector.offset(downpos, 0, level / 8, 0))
					if level < 7 then
						level = level + 1
					else
						level = "ready"
					end
					minetest.swap_node(downpos, {name = "vlc_composters:composter_" .. level})
				end
				break
			end
		end
...
--
minetest.register_node("vlc_composters:composter", {
	...
	tiles = {
		"vlc_composter_bottom.png^vlc_composter_top.png",
		"vlc_composter_bottom.png",
		"vlc_composter_side.png"
	},
...
local function register_filled_composter(level)
	local id = "vlc_composters:composter_"..level
	minetest.register_node(id, {
		...
		selection_box = {type = "regular"},
		tiles = {
			"vlc_composter_compost.png^vlc_composter_top.png",
			"vlc_composter_bottom.png",
			"vlc_composter_side.png"
		},
		is_ground_content = false,
		...
		sounds = vlc_sounds.node_sound_wood_defaults(),
		drop = "vlc_composters:composter",
		_vlc_hardness = 0.6,
		_vlc_blast_resistance = 0.6,
		_vlc_compost_level = level,
		on_rightclick = composter_add_item,
	...
	if minetest.get_modpath("doc") then
		doc.add_entry_alias("nodes", "vlc_composters:composter", "nodes", id)
	end
end
...
--
minetest.register_node("vlc_composters:composter_ready", {
	...
	selection_box = {type = "regular"},
	tiles = {
		"vlc_composter_ready.png^vlc_composter_top.png",
		"vlc_composter_bottom.png",
		"vlc_composter_side.png"
	},
	is_ground_content = false,
...
-- Add entry aliases for the Help
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlc_composters:composter",
		"nodes", "vlc_composters:composter_ready" )
end
