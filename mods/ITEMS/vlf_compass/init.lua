...
local function get_far_node(pos, itemstack) --code from minetest dev wiki: https://dev.minetest.net/minetest.get_node, some edits have been made to add a cooldown for force loads
	...
	if node.name == "ignore" then
		...
		if tonumber(os.time(os.date("!*t"))) - tstamp > 180 then --current time in secounds - old time in secounds, if it is over 180 (3 mins): forceload
			...
			node = {name="vlf_compass:lodestone"} --cooldown not over yet, pretend like there is something...
		end
	end
	return node
end
...
local function get_compass_frame(pos, dir, itemstack)
	if not string.find(itemstack:get_name(), "_lodestone") then -- normal compass
		...
		if vlf_worlds.compass_works(pos) then
		...
		if not lpos then
			minetest.log("warning", "vlf_compass: invalid lodestone position!")
			return random_frame
		end
		local _, l_dim = vlf_worlds.y_to_layer(lpos.y)
		...
		if l_dim == p_dim then
			...
			if get_far_node(lpos, itemstack).name == "vlf_compass:lodestone" then
...
--
function vlf_compass.get_compass_image(pos, dir)
	minetest.log("warning", "vlf_compass: deprecated function " ..
		"get_compass_image() called, use get_compass_itemname().")
	local itemstack = ItemStack(vlf_compass.stereotype)
...
minetest.register_craft({ --TODO: update once echo shards are a thing
	output = "vlf_compass:" .. random_frame .. "_recovery",
	recipe = {
		{"","vlf_nether:netherite_ingot",""},
		{"vlf_core:diamondblock","vlf_compass:" .. stereotype_frame ,"vlf_core:diamondblock"},
		{"vlf_core:diamondblock","vlf_core:diamondblock","vlf_core:diamondblock"}
...
minetest.register_craft({
	output = "vlf_compass:lodestone",
	recipe = {
		{"vlf_core:stonebrickcarved","vlf_core:stonebrickcarved","vlf_core:stonebrickcarved"},
		{"vlf_core:stonebrickcarved", "vlf_nether:netherite_ingot", "vlf_core:stonebrickcarved"},
		{"vlf_core:stonebrickcarved", "vlf_core:stonebrickcarved", "vlf_core:stonebrickcarved"}
	}
})
...
minetest.register_on_dieplayer(function(player)
	local meta = player:get_meta();
	meta:set_string("vlf_compass:recovery_pos",minetest.pos_to_string(player:get_pos()))
end)
