...
local function particles(pointed_thing, texture)
	...
	minetest.add_particlespawner({
		...
		vertical = false,
		texture = texture or "vlc_copper_anti_oxidation_particle.png",
		glow = 5,
	})
end
...
-- Waxing Function
function vlc_honey.wax_block(pos, node, player, itemstack, pointed_thing)
	-- prevent modification of protected nodes.
	if vlc_util.check_position_protection(pos, player) then
		return
	end
	local def = minetest.registered_nodes[node.name]

	if player:get_player_control().sneak then
		if def and def._vlc_waxed_variant then
			if def.groups.door == 1 then
				if node.name:find("_b_") then
					local top_pos = { x = pos.x, y = pos.y + 1, z = pos.z }
					minetest.swap_node(top_pos, { name = def._vlc_waxed_variant:gsub("_b_", "_t_"), param2 = node.param2 })
				elseif node.name:find("_t_") then
					local bot_pos = { x = pos.x,  y = pos.y - 1, z = pos.z }
					minetest.swap_node(bot_pos, { name = def._vlc_waxed_variant:gsub("_t_", "_b_"), param2 = node.param2 })
				end
			end
		else
			return
	...
	if def._vlc_waxed_variant then
		node.name = def._vlc_waxed_variant
		minetest.swap_node(pos, node)
		particles(pointed_thing, "vlc_copper_anti_oxidation_particle.png^[colorize:#d1d553:125")
		awards.unlock(player:get_player_name(), "vlc:wax_on")
...
-- Honeycomb
minetest.register_craftitem("vlc_honey:honeycomb", {
	...
	inventory_image = "vlc_honey_honeycomb.png",
	groups = { craftitem = 1 },
	on_place = function(itemstack, placer, pointed_thing)
		...
		return vlc_honey.wax_block(pos, node, placer, itemstack, pointed_thing)
	end,
})

minetest.register_node("vlc_honey:honeycomb_block", {
	description = S("Honeycomb Block"),
	_doc_items_longdesc = S("Honeycomb Block. Used as a decoration."),
	tiles = {
		"vlc_honey_honeycomb_block.png"
	},
...
-- Honey
minetest.register_craftitem("vlc_honey:honey_bottle", {
	...
	inventory_image = "vlc_honey_honey_bottle.png",
	groups = { craftitem = 1, food = 3, eatable = 6, can_eat_when_full=1 },
	on_place = minetest.item_eat(6, "vlc_potions:glass_bottle"),
	on_secondary_use = minetest.item_eat(6, "vlc_potions:glass_bottle"),
	_vlc_saturation = 1.2,
	stack_max = 16,
})

minetest.register_node("vlc_honey:honey_block", {
	description = S("Honey Block"),
	_doc_items_longdesc = S("Honey Block. Used as a decoration and in redstone. Is sticky on some sides."),
	tiles = {"vlc_honey_block_side.png"},
	is_ground_content = false,
	...
	mvps_sticky = function(pos, node, piston_pos)
		...
		for n, v in ipairs(alldirs) do
			...
			if neighbor_node then
				...
				if name ~= "air" and name ~= "ignore" and name ~= "vlc_core:slimeblock" and not mesecon.mvps_unsticky[name] then
...
minetest.register_craft({
	output = "vlc_honey:honeycomb_block",
	recipe = {
		{ "vlc_honey:honeycomb", "vlc_honey:honeycomb" },
		{ "vlc_honey:honeycomb", "vlc_honey:honeycomb" },
	},
})

minetest.register_craft({
	output = "vlc_honey:honey_block",
	recipe = {
		{ "vlc_honey:honey_bottle", "vlc_honey:honey_bottle" },
		{ "vlc_honey:honey_bottle", "vlc_honey:honey_bottle" },
	},
	replacements = {
		{ "vlc_honey:honey_bottle", "vlc_potions:glass_bottle" },
		{ "vlc_honey:honey_bottle", "vlc_potions:glass_bottle" },
		{ "vlc_honey:honey_bottle", "vlc_potions:glass_bottle" },
		{ "vlc_honey:honey_bottle", "vlc_potions:glass_bottle" },
	},
...
minetest.register_craft({
	output = "vlc_honey:honey_bottle 4",
	recipe = {
		{ "vlc_potions:glass_bottle", "vlc_potions:glass_bottle", "vlc_honey:honey_block" },
		{ "vlc_potions:glass_bottle", "vlc_potions:glass_bottle", "" },
	},
})

minetest.register_craft({
	type = "shapeless",
	output = "vlc_core:sugar 3",
	recipe = { "vlc_honey:honey_bottle" },
	replacements = {
		{ "vlc_honey:honey_bottle", "vlc_potions:glass_bottle" },
	},
})
