...
local function check_placement_allowed(node, wdir)
	...
	elseif not def.buildable_to then
		if node.name ~= "vlf_core:ice" and node.name ~= "vlf_nether:soul_sand" and node.name ~= "vlf_mobspawners:spawner" and node.name ~= "vlf_core:barrier" and node.name ~= "vlf_end:chorus_flower" and node.name ~= "vlf_end:chorus_flower_dead" and (not def.groups.glass) and
				((not def.groups.solid) or (not def.groups.opaque)) then

			-- Only allow top placement on these nodes
			if node.name == "vlf_end:dragon_egg" or node.name == "vlf_portals:end_portal_frame_eye" or def.groups.fence == 1 or def.groups.wall or def.groups.slab_top == 1 or def.groups.anvil or def.groups.pane or (def.groups.stair == 1 and minetest.facedir_to_dir(node.param2).y ~= 0) then
				if wdir ~= 1 then
					return false
				end
			else
...
function vlf_torches.register_torch(def)
