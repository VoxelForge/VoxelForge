...
local function sea_pickle_on_place(itemstack, placer, pointed_thing, level)
	...
	-- Place
	if node_under.name == "vlc_ocean:dead_brain_coral_block" then
		-- Place on suitable coral block
		if submerged then
			node_under.name = "vlc_ocean:sea_pickle_"..level.."_dead_brain_coral_block"
		else
			node_under.name = "vlc_ocean:sea_pickle_" .. level .. "_off_dead_brain_coral_block"
		end
	...
	elseif minetest.get_item_group(node_under.name, "sea_pickle") ~= 0 then
		...
		if def and def._vlc_sea_pickle_next then
			node_under.name = def._vlc_sea_pickle_next
			minetest.set_node(pos_under, node_under)
		else
			return itemstack
		end
...
local canonical = "vlc_ocean:sea_pickle_1_"..ontop
...
for s=1,4 do
	...
	if s == 1 then
		...
		img = "vlc_ocean_sea_pickle_item.png"
		on_place = sea_pickle_on_place
	else
		doc_create = false
		nici = 1
		img = "vlc_ocean_"..ontop..".png^(vlc_ocean_sea_pickle_"..s.."_anim.png^[verticalframe:2:1)"
	end
	img_off = "vlc_ocean_"..ontop..".png^vlc_ocean_sea_pickle_"..s.."_off.png"
	local next_on, next_off
	if s < 4 then
		next_on = "vlc_ocean:sea_pickle_" .. tostring(s + 1) .. "_" .. ontop
		next_off = "vlc_ocean:sea_pickle_" .. tostring(s + 1) .. "_off_" .. ontop
	end
	...
	local function spread_sea_pickle(pos, placer)
		...
		for k, v in pairs(possible_position) do
			sea_pickle_on_place(
				ItemStack("vlc_ocean:sea_pickle"),
				placer,
	...
	local function on_bone_meal(itemstack, placer, pointed_thing, pos, node)
		...
		if 4 ~= s then
			node.name = "vlc_ocean:sea_pickle_" .. (s + 1) .. "_" .. ontop
			minetest.swap_node(pos, node)
		end
		spread_sea_pickle(pos, placer)
	end

	minetest.register_node("vlc_ocean:sea_pickle_"..s.."_"..ontop, {
		description = desc,
		_tt_help = tt_help,
		...
		tiles = { "vlc_ocean_"..ontop..".png" },
		special_tiles = {
			{
			image = "vlc_ocean_sea_pickle_"..s.."_anim.png",
		...
		node_dig_prediction = "vlc_ocean:"..ontop,
		after_dig_node = function(pos)
			local node = minetest.get_node(pos)
			if minetest.get_item_group(node.name, "sea_pickle") == 0 then
				minetest.set_node(pos, {name="vlc_ocean:"..ontop})
			end
		end,
		on_place = on_place,
		_vlc_sea_pickle_off = "vlc_ocean:sea_pickle_"..s.."_off_"..ontop,
		_vlc_sea_pickle_next = next_on,
		_vlc_hardness = 0,
		_vlc_blast_resistance = 0,
		_on_bone_meal = on_bone_meal,
	})

	minetest.register_node("vlc_ocean:sea_pickle_"..s.."_off_"..ontop, {
		drawtype = "plantlike_rooted",
		paramtype = "light",
		paramtype2 = "meshoptions",
		tiles = { "vlc_ocean_"..ontop..".png" },
		special_tiles = { "vlc_ocean_sea_pickle_"..s.."_off.png", },
		...
		_vlc_sea_pickle_on = "vlc_ocean:sea_pickle_"..s.."_"..ontop,
		_vlc_sea_pickle_next = next_off,
		_vlc_hardness = 0,
		_vlc_blast_resistance = 0,
	})
	...
	if mod_doc then
		if s == 1 then
			doc.add_entry_alias("nodes", "vlc_ocean:sea_pickle_1_dead_brain_coral_block", "nodes", "vlc_ocean:sea_pickle_1_off_"..ontop)
		else
			doc.add_entry_alias("nodes", "vlc_ocean:sea_pickle_1_dead_brain_coral_block", "nodes", "vlc_ocean:sea_pickle_"..s.."_off_"..ontop)
			doc.add_entry_alias("nodes", "vlc_ocean:sea_pickle_1_dead_brain_coral_block", "nodes", "vlc_ocean:sea_pickle_"..s.."_"..ontop)
		end
	end
end
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if minetest.get_item_group(checknode.name, "water") ~= 0 then
			-- Sea pickle is unlit
			if state == 2 then
				node.name = def._vlc_sea_pickle_on
			...
			-- Sea pickle is lit
			if state == 1 then
				node.name = def._vlc_sea_pickle_off
				minetest.set_node(pos, node)
				return
			end
		end
