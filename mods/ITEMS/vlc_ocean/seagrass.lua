...
local surfaces = {
	{ "dirt", "vlc_core:dirt" },
	{ "sand", "vlc_core:sand", 1 },
	{ "redsand", "vlc_core:redsand", 1 },
	{ "gravel", "vlc_core:gravel", 1 },
}
...
local function seagrass_on_place(itemstack, placer, pointed_thing)
	...
	if node_under.name == "vlc_core:dirt" then
		node_under.name = "vlc_ocean:seagrass_dirt"
	elseif node_under.name == "vlc_core:sand" then
		node_under.name = "vlc_ocean:seagrass_sand"
	elseif node_under.name == "vlc_core:redsand" then
		node_under.name = "vlc_ocean:seagrass_redsand"
	elseif node_under.name == "vlc_core:gravel" then
		node_under.name = "vlc_ocean:seagrass_gravel"
	else
		return itemstack
	end
...
minetest.register_craftitem("vlc_ocean:seagrass", {
	description = S("Seagrass"),
	_tt_help = S("Grows in water on dirt, sand, gravel"),
	_doc_items_create_entry = false,
	inventory_image = "vlc_ocean_seagrass_item.png",
	wield_image = "vlc_ocean_seagrass_item.png",
...
for s=1, #surfaces do
	...
	local leaf_sounds = vlc_sounds.node_sound_leaves_defaults()
	...
	if surfaces[s][1] == "dirt" then
		...
		desc = S("Seagrass")
		doc_create = true
		doc_img = "vlc_ocean_seagrass_item.png"
	else
		doc_create = false
	end
	minetest.register_node("vlc_ocean:seagrass_"..surfaces[s][1], {
		...
		drop = "",
		_vlc_falling_node_alternative = alt,
		_vlc_shears_drop = { "vlc_ocean:seagrass" },
		_vlc_hardness = 0,
		_vlc_blast_resistance = 0,
	})
	if mod_doc and surfaces[s][1] ~= "dirt" then
		doc.add_entry_alias("nodes", "vlc_ocean:seagrass_dirt", "nodes", "vlc_ocean:seagrass_"..surfaces[s][1])
	end
end

if mod_doc then
	doc.add_entry_alias("nodes", "vlc_ocean:seagrass_dirt", "craftitems", "vlc_ocean:seagrass")
end

minetest.register_lbm({
	label = "Fix incorrect seagrass",
	name = "vlc_ocean:fix_incorrect_seagrass",
