local S = minetest.get_translator("vlc_candles")
...
local tpl_candle = {
	...
	paramtype = "light",
	paramtype2 = "color",
	palette = "vlc_dyes_palette.png",
	inventory_image = "vlc_candles_item.png",
	wield_image = "vlc_candles_item.png",
	tiles = { "vlc_candles_candle.png" },
	...
	_on_dye_place = function(pos,color)
		...
		node.param2 = vlc_dyes.colors[color].palette_index
	...
	_on_ignite = function(player, pointed_thing)
		...
		if g > 0 then
			n.name = "vlc_candles:candle_lit_"..tostring(g)
...
function tpl_candle.on_place(itemstack, placer, pointed_thing)
	...
	if vlc_util.check_position_protection(pointed_thing.under, placer) then return end
	...
	if g > 0 then
		if g < #candleboxes then
			unode.name = "vlc_candles:candle_"..tostring(math.min(4, g + 1))
...
function extinguish(pos, node, clicker, itemstack, pointed_thing)
	...
	if vlc_util.check_position_protection(pos, clicker) then
		return
	end

	local g = minetest.get_item_group(node.name, "lit_candles")
	if g > 0 then
		node.name = "vlc_candles:candle_"..tostring(g)
		minetest.swap_node(pos, node)
	end
end
...
for i=1,4 do
	local candle_n = {
		mesh = "vlc_candles_candle_"..tostring(i)..".obj",
		drop = "vlc_candles:candle_1".." "..tostring(i),
	...
	--[[minetest.register_node("vlc_candles:candle_"..i,table.merge(tpl_candle, candle_n,{
		groups = table.merge(tpl_candle.groups, { candles = i, unlit_candles = i }, creative_group),
	}))
	minetest.register_node("vlc_candles:candle_lit_"..i,table.merge(tpl_candle, tpl_lit_candle, candle_n,{
		light_source = 3 * i,
...
local function candle_craft(itemstack, player, old_craft_grid, craft_inv)
	...
	if dye and candle and i == 2 then
		local cdef = vlc_dyes.colors[dye:get_definition()._color]
