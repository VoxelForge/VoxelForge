...
function vlc_furnaces.receive_fields(pos, formname, fields, sender)
	if fields.craftguide then
		vlc_craftguide.show(sender:get_player_name())
	end
end

function vlc_furnaces.give_xp(pos, player)
	local meta = minetest.get_meta(pos)
	local dir = vector.divide(minetest.facedir_to_dir(minetest.get_node(pos).param2), -1.95)
	local xp = meta:get_int("xp")
	if xp > 0 then
		if player then
			vlc_experience.add_xp(player, xp)
		else
			vlc_experience.throw_xp(vector.add(pos, dir), xp)
		end
		meta:set_int("xp", 0)
	end
end
...
--
function vlc_furnaces.is_cookable(stack, pos)
	if pos then
		local def = minetest.registered_nodes[minetest.get_node(pos).name]
		if def and def._vlc_furnaces_cook_group and minetest.get_item_group(stack:get_name(), def._vlc_furnaces_cook_group) == 0 then return false end
	end
	return minetest.get_craft_result({method = "cooking", width = 1, items = {stack}}).time ~= 0
end

local function sort_stack(stack, pos)
	if vlc_furnaces.is_cookable(stack, pos) then
		if vlc_util.is_fuel(stack) and not minetest.get_meta(pos):get_inventory():room_for_item("src", stack) then
			return "fuel"
		end
		return "src"
	elseif vlc_util.is_fuel(stack) or ( vlc_furnaces.is_cookable(ItemStack("vlc_sponges:sponge_wet"), pos) and stack:get_name() == "vlc_buckets:bucket_empty" ) then
		return "fuel"
	end
end

function vlc_furnaces.allow_metadata_inventory_put(pos, listname, index, stack, player)
	...
	if listname == "fuel" then
		...
		if stack:get_name() == "vlc_buckets:bucket_empty" then
...
function vlc_furnaces.allow_metadata_inventory_move(pos, from_list, from_index, to_list, to_index, count, player)
	if from_list == "sorter" or to_list == "sorter" then return 0 end
	local meta = minetest.get_meta(pos)
	local inv = meta:get_inventory()
	local stack = inv:get_stack(from_list, from_index)
	return vlc_furnaces.allow_metadata_inventory_put(pos, to_list, to_index, stack, player)
end

function vlc_furnaces.allow_metadata_inventory_take(pos, listname, index, stack, player)
...
function vlc_furnaces.on_metadata_inventory_take(pos, listname, index, stack, player)
	-- Award smelting achievements
	if listname == "dst" then
		if stack:get_name() == "vlc_core:iron_ingot" then
			awards.unlock(player:get_player_name(), "vlc:acquireIron")
		elseif stack:get_name() == "vlc_fishing:fish_cooked" then
			awards.unlock(player:get_player_name(), "vlc:cookFish")
		end
		vlc_furnaces.give_xp(pos, player)
	end
end

function vlc_furnaces.on_metadata_inventory_move(pos, from_list, from_index, to_list, to_index, count, player)
	if from_list == "dst" then
		vlc_furnaces.give_xp(pos, player)
	end
end

function vlc_furnaces.on_metadata_inventory_put(pos, listname, index, stack, player)
...
function vlc_furnaces.swap_node(pos, name)
...
function vlc_furnaces.furnace_reset_delta_time(pos)
	local meta = minetest.get_meta(pos)
...
function vlc_furnaces.furnace_get_delta_time(pos, elapsed)
	local meta = minetest.get_meta(pos)
	local time_speed = tonumber(minetest.settings:get("time_speed") or 72)
...
function vlc_furnaces.get_timer_function(node_normal, node_active, factor, group)
	return function(pos, elapsed)
		local meta, elapsed_game_time = vlc_furnaces.furnace_get_delta_time(pos, elapsed)
		...
		while elapsed_game_time > 0.00001 and update do
			...
			if cookable and active then
				...
				if src_time >= cooked.time then
					...
					if inv:get_stack("fuel", 1):get_name() == "vlc_buckets:bucket_empty" then
						if srclist[1]:get_name() == "vlc_sponges:sponge_wet" then
							inv:set_stack("fuel", 1, "vlc_buckets:bucket_water")
							fuellist = inv:get_list("fuel")
							-- Also for river water
						elseif srclist[1]:get_name() == "vlc_sponges:sponge_wet_river_water" then
							inv:set_stack("fuel", 1, "vlc_buckets:bucket_river_water")
							fuellist = inv:get_list("fuel")
						end
					end
		...
		local formspec
		if def._vlc_furnaces_get_inactive_formspec then
			formspec = def._vlc_furnaces_get_inactive_formspec(name)
		else
		...
		if active then
			...
			if def._vlc_furnaces_get_active_formspec then
				formspec = def._vlc_furnaces_get_active_formspec(fuel_percent, item_percent, name)
			else
				formspec = get_active_formspec(fuel_percent, item_percent, name)
			end
			vlc_furnaces.swap_node(pos, node_active)
			-- make sure timer restarts automatically
			result = true
		else
			vlc_furnaces.swap_node(pos, node_normal)
...
vlc_furnaces.furnace_node_timer = vlc_furnaces.get_timer_function()

vlc_furnaces.on_rotate = screwdriver.rotate_simple

-- Returns true if itemstack is fuel, but not for lava bucket if destination already has one
function vlc_furnaces.is_transferrable_fuel(itemstack, src_inventory, src_list, dst_inventory, dst_list)
	if vlc_util.is_fuel(itemstack) then
		if itemstack:get_name() == "vlc_buckets:bucket_lava" then
...
function vlc_furnaces.on_hopper_out(uppos, pos)
	local sucked = vlc_util.move_item_container(uppos, pos)

	-- Also suck in non-fuel items from furnace fuel slot
	if not sucked then
		local finv = minetest.get_inventory({type="node", pos=uppos})
		if finv and not vlc_util.is_fuel(finv:get_stack("fuel", 1)) then
			sucked = vlc_util.move_item_container(uppos, pos, "fuel")
		end
	end
	return sucked
end

function vlc_furnaces.on_hopper_in(pos, to_pos)
	if pos.y == to_pos.y then
		...
		local slot_id,_ = vlc_util.get_eligible_transfer_item_slot(sinv, "main", dinv, "fuel", vlc_furnaces.is_transferrable_fuel)
		if slot_id then
			vlc_util.move_item_container(pos, to_pos, nil, slot_id, "fuel")
		end
		return true
	end
end

vlc_furnaces.tpl_furnace_node = {
	paramtype2 = "facedir",
	paramtype = "light",
	groups = { pickaxey = 1, container = 4, deco_block = 1, material_stone = 1, furnace = 1 },
	is_ground_content = false,
	sounds = vlc_sounds.node_sound_stone_defaults(),
	_vlc_blast_resistance = 3.5,
	_vlc_hardness = 3.5,

	after_dig_node = vlc_util.drop_items_from_meta_container({"src","dst","fuel","sorter"}),
	on_destruct = function(pos)
		vlc_furnaces.give_xp(pos)
	end,

	allow_metadata_inventory_put = vlc_furnaces.allow_metadata_inventory_put,
	allow_metadata_inventory_move = vlc_furnaces.allow_metadata_inventory_move,
	allow_metadata_inventory_take = vlc_furnaces.allow_metadata_inventory_take,
	on_metadata_inventory_move = vlc_furnaces.on_metadata_inventory_move,
	on_metadata_inventory_take = vlc_furnaces.on_metadata_inventory_take,
	on_metadata_inventory_put = vlc_furnaces.on_metadata_inventory_put,
	on_receive_fields = vlc_furnaces.receive_fields,
	_on_hopper_in = vlc_furnaces.on_hopper_in,
	_on_hopper_out = vlc_furnaces.on_hopper_out,
	on_rotate = vlc_furnaces.on_rotate,
}

vlc_furnaces.tpl_furnace_node_normal = table.merge(vlc_furnaces.tpl_furnace_node,{
	_doc_items_hidden = false,

	on_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, count, player)
		-- Reset accumulated game time when player works with furnace:
		vlc_furnaces.furnace_reset_delta_time(pos)
		minetest.get_node_timer(pos):start(1.0)

		vlc_furnaces.on_metadata_inventory_move(pos, from_list, from_index, to_list, to_index, count, player)
	end,
	on_metadata_inventory_put = function(pos, listname, index, stack, player)
		...
		vlc_furnaces.on_metadata_inventory_put(pos, listname, index, stack, player)
	end,
	on_metadata_inventory_take = function(pos, listname, index, stack, player)
		...
		vlc_furnaces.on_metadata_inventory_take(pos, listname, index, stack, player)
	end,
})

vlc_furnaces.tpl_furnace_node_active = table.merge(vlc_furnaces.tpl_furnace_node,{
...
function vlc_furnaces.register_furnace(nodename, def)
	local timer_func = vlc_furnaces.get_timer_function(nodename, nodename.."_active", (def.factor or 1), def.cook_group)
	minetest.register_node(nodename, table.merge(vlc_furnaces.tpl_furnace_node_normal,{
		...
		on_timer = timer_func,
		_vlc_furnaces_cook_group = def.cook_group,
		_vlc_furnaces_get_active_formspec = def.get_active_formspec,
		_vlc_furnaces_get_inactive_formspec = def.get_inactive_formspec,
	},def.node_normal))
	minetest.register_node(nodename.."_active", table.merge(vlc_furnaces.tpl_furnace_node_active,{
		on_timer = timer_func,
		drop = nodename,
	},def.node_active))
end

vlc_furnaces.register_furnace("vlc_furnaces:furnace",{
	node_normal = {
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlc_furnaces:furnace", "nodes", "vlc_furnaces:furnace_active")
end

minetest.register_lbm({
	label = "Update Furnace formspecs and invs to allow new sneak+click behavior",
	name = "vlc_furnaces:update_coolsneak",
	...
	action = function(pos, node)
		...
		if minetest.get_item_group(node.name, "furnace_active") == 0 then --active furnaces update the formspec every second by themselves
			...
			if def._vlc_furnaces_get_inactive_formspec then
				m:set_string("formspec", def._vlc_furnaces_get_inactive_formspec(name))
			else
