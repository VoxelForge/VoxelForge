...
function vlf_trees.strip_tree(itemstack, placer, pointed_thing)
...
function vlf_trees.rotate_climbable(pos, node, _, mode)
...
function vlf_trees.add_bee_nest(pos)
	...
	for i = 2, 8 do
		...
		if minetest.get_node(nestpos).name == "air" and
			...
			minetest.set_node(nestpos, {name = "vlf_beehives:bee_nest"})
