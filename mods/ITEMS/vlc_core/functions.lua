	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		for w=1, #water do
			...
			if water[w].y < pos.y and water[w].x == pos.x and water[w].z == pos.z then
				minetest.set_node(water[w], {name="vlc_core:stone"})
			...
			elseif lavatype == "flowing" and water[w].y == pos.y and (water[w].x == pos.x or water[w].z == pos.z) then
				minetest.set_node(pos, {name="vlc_core:cobble"})
			...
			elseif lavatype == "source" and
				...
				minetest.set_node(pos, {name="vlc_core:obsidian"})
				minetest.sound_play("fire_extinguish_flame", {pos = pos, gain = 0.25, max_hear_distance = 16}, true)
...
-- Functions
function vlc_core.grow_cactus(pos, node)
	pos.y = pos.y-1
	local name = minetest.get_node(pos).name
	if minetest.get_item_group(name, "sand") ~= 0 then
		pos.y = pos.y+1
		local height = 0
		while minetest.get_node(pos).name == "vlc_core:cactus" and height < 4 do
			height = height+1
			pos.y = pos.y+1
		end
		if height < 3 then
			if minetest.get_node(pos).name == "air" then
				minetest.set_node(pos, {name="vlc_core:cactus"})
			end
		end
	end
end

function vlc_core.grow_reeds(pos, node)
	pos.y = pos.y-1
	local name = minetest.get_node(pos).name
	if minetest.get_item_group(name, "soil_sugarcane") ~= 0 then
		...
		pos.y = pos.y+1
		local height = 0
		while minetest.get_node(pos).name == "vlc_core:reeds" and height < 3 do
			height = height+1
			pos.y = pos.y+1
		end
		if height < 3 then
			if minetest.get_node(pos).name == "air" then
				minetest.set_node(pos, {name="vlc_core:reeds"})
			end
		end
	end
end
...
minetest.register_abm({
	label = "Cactus growth",
	nodenames = {"vlc_core:cactus"},
	neighbors = {"group:sand"},
	interval = 25,
	chance = 10,
	action = function(pos)
		vlc_core.grow_cactus(pos)
	end,
})
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		for _, object in pairs(minetest.get_objects_inside_radius(pos, 1.1)) do
			...
			elseif entity and entity.is_mob then
				vlc_util.deal_damage(object, 1, {type = "cactus"})
			end
		end
		...
		for _, p in pairs(posses) do
			...
			if ndef and ndef.walkable then
				local posy = pos.y
				while minetest.get_node(vector.new(pos.x, posy, pos.z)).name == "vlc_core:cactus" do
					local pos = vector.new(pos.x, posy, pos.z)
					minetest.remove_node(pos)
					minetest.add_item(vector.offset(pos, math.random(-0.5, 0.5), 0, math.random(-0.5, 0.5)), "vlc_core:cactus")
					posy = posy + 1
				end
				break
			end
...
function vlc_core.get_grass_palette_index(pos)
	...
	if biome_data then
		...
		if reg_biome then
			index = reg_biome._vlc_palette_index
		end
	end
	return index
end
...
function vlc_core.get_grass_block_type(pos)
	return {name = "vlc_core:dirt_with_grass", param2 = vlc_core.get_grass_palette_index(pos)}
end
...
minetest.register_abm({
	...
	nodenames = {"vlc_core:dirt"},
	neighbors = {"air", "group:grass_block_no_snow", "vlc_core:mycelium"},
	interval = 30,
	chance = 20,
	catch_up = false,
	action = function(pos)
...
minetest.register_abm({
	...
	action = function(pos, node)
		...
		if name ~= "ignore" and (minetest.get_item_group(name, "opaque") == 1 or minetest.get_item_group(name, "liquid") ~= 0) then
			minetest.set_node(pos, {name = "vlc_core:dirt"})
		end
	end
})
...
minetest.register_on_placenode(function(pos, newnode, placer, oldnode, itemstack, pointed_thing)
	if minetest.get_item_group(newnode.name, "solid") ~= 0 or
		...
		if minetest.get_item_group(belownode.name, "dirtifies_below_solid") == 1 then
			minetest.set_node(below, {name="vlc_core:dirt"})
		end
	end
end)
...
minetest.register_lbm({
	...
	name = "vlc_core:replace_legacy_dry_grass_0_65_0",
	nodenames = {"vlc_core:dirt_with_dry_grass", "vlc_core:dirt_with_dry_grass_snow"},
	action = function(pos, node)
		local biome_data = minetest.get_biome_data(pos)
		if biome_data then
			local biome = biome_data.biome
			...
			local reg_biome = minetest.registered_biomes[biome_name]
			if reg_biome then
				if node.name == "vlc_core:dirt_with_dry_grass_snow" then
					node.name = "vlc_core:dirt_with_grass_snow"
				else
					node.name = "vlc_core:dirt_with_grass"
				end
				node.param2 = reg_biome._vlc_palette_index
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		local function spread_down(origin, target, dir, node)
			if math.random(1, 2) == 1 then
				if minetest.get_node(target).name == "air" then
					minetest.add_node(target, {name = "vlc_core:vine", param2 = node.param2})
				end
			end
		end
		...
		local function spread_up(origin, target, dir, node)
			local vines_in_area = minetest.find_nodes_in_area({x=origin.x-4, y=origin.y-1, z=origin.z-4}, {x=origin.x+4, y=origin.y+1, z=origin.z+4}, "vlc_core:vine")
			...
			if #vines_in_area < 5 then
				if math.random(1, 2) == 1 then
					if minetest.get_node(target).name == "air" then
						...
						if vlc_core.supports_vines(backupnodename) then
							minetest.add_node(target, {name = "vlc_core:vine", param2 = node.param2})
						end
					end
				end
...
function vlc_core.supports_vines(nodename)
...
minetest.register_abm({
	...
	nodenames = {"vlc_core:snow", "vlc_core:ice"},
	interval = 16,
	chance = 8,
	action = function(pos, node)
		if minetest.get_node_light(pos, 0) >= 12 then
			if node.name == "vlc_core:ice" then
...
minetest.register_abm({
	...
	nodenames = {"vlc_core:water_source", "vlcx_core:river_water_source"},
	interval = 32,
	chance = 8,
	action = function(pos, node)
		if vlc_weather.has_snow(pos) and minetest.get_natural_light(vector.offset(pos,0,1,0), 0.5) == minetest.LIGHT_MAX + 1 and minetest.get_node_light(pos) < 10 then
			node.name = "vlc_core:ice"
			minetest.swap_node(pos, node)
		end
	end
})

-- Melt ice at pos. vlc_core:ice MUST be at pos if you call this!
function vlc_core.melt_ice(pos)
	-- Create a water source if ice is destroyed and there was something below it
	local below = {x=pos.x, y=pos.y-1, z=pos.z}
	local belownode = minetest.get_node(below)
	local dim = vlc_worlds.pos_to_dimension(below)
	if dim ~= "nether" and belownode.name ~= "air" and belownode.name ~= "ignore" and belownode.name ~= "vlc_core:void" then
...
--
-- The snowable nodes also MUST have _vlc_snowed defined to contain the name
-- of the snowed node.
function vlc_core.register_snowed_node(itemstring_snowed, itemstring_clear, tiles, sounds, clear_colorization, desc)
	...
	def._vlc_snowless = itemstring_clear

	-- Note: _vlc_snowed must be added to the clear node manually!

	if not tiles then
		def.tiles = {"default_snow.png", "default_dirt.png", {name="vlc_core_grass_side_snowed.png", tileable_vertical=false}}
	else
		def.tiles = tiles
	end
	...
	if not sounds then
		def.sounds = vlc_sounds.node_sound_dirt_defaults({
			footstep = vlc_sounds.node_sound_snow_defaults().footstep,
		})
	else
		def.sounds = sounds
	end

	def._vlc_silk_touch_drop = {itemstring_clear}
...
function vlc_core.clear_snow_dirt(pos, node)
	local def = minetest.registered_nodes[node.name]
	if def and def._vlc_snowless then
		minetest.swap_node(pos, {name = def._vlc_snowless, param2=node.param2})
	end
end
...
function vlc_core.on_snowable_construct(pos)
	-- Myself
	...
	if minetest.get_item_group(anode.name, "snow_cover") == 1 then
		local def = minetest.registered_nodes[node.name]
		if def and def._vlc_snowed then
			minetest.swap_node(pos, {name = def._vlc_snowed, param2=node.param2})
		end
	end
end
...
-- vlc2's snow cover nodes are Top Snow (vlc_core:snow) and Snow (vlc_core:snowblock).

-- Always add the following functions to snow cover nodes:

-- on_construct
-- Makes snowable node below snowed.
function vlc_core.on_snow_construct(pos)
	local npos = {x=pos.x, y=pos.y-1, z=pos.z}
	local node = minetest.get_node(npos)
	local def = minetest.registered_nodes[node.name]
	if def and def._vlc_snowed then
		minetest.swap_node(npos, {name = def._vlc_snowed, param2=node.param2})
	end
end
-- after_destruct
-- Clears snowed dirtlike node below.
function vlc_core.after_snow_destruct(pos)
...
local crobby_particle = {
	...
	texture = "vlc_core_crying_obsidian_tear.png",
...
minetest.register_abm({
	...
	nodenames = {"vlc_core:crying_obsidian"},
	interval = 5,
	chance = 10,
	action = function(pos, node)
...
function vlc_core.make_dirtpath(itemstack, placer, pointed_thing)
	...
	if minetest.get_node(above).name == "air" then
		if not minetest.is_creative_enabled(placer:get_player_name()) then
			...
			local wear = vlc_autogroup.get_wear(toolname, "shovely")
			itemstack:add_wear(wear)
		end
		minetest.sound_play({name="default_grass_footstep", gain=1}, {pos = above}, true)
		minetest.swap_node(pointed_thing.under, {name="vlc_core:grass_path"})
	end
	return itemstack,true
end

function vlc_core.strip_tree(itemstack, placer, pointed_thing)
	if pointed_thing.type ~= "node" then return end

	local node = minetest.get_node(pointed_thing.under)
	local noddef = minetest.registered_nodes[node.name]

	if noddef._vlc_stripped_variant and minetest.registered_nodes[noddef._vlc_stripped_variant] then
		minetest.swap_node(pointed_thing.under, {name=noddef._vlc_stripped_variant, param2=node.param2})
		if not minetest.is_creative_enabled(placer:get_player_name()) then
			-- Add wear (as if digging a axey node)
			local toolname = itemstack:get_name()
			local wear = vlc_autogroup.get_wear(toolname, "axey")
			itemstack:add_wear(wear)
		end
	end
	return itemstack,true
...
function vlc_core.bone_meal_grass(itemstack,placer,pointed_thing)
	local flowers_table_plains = {
		"vlc_flowers:dandelion",
		"vlc_flowers:dandelion",
		"vlc_flowers:poppy",

		"vlc_flowers:oxeye_daisy",
		"vlc_flowers:tulip_orange",
		"vlc_flowers:tulip_red",
		"vlc_flowers:tulip_white",
		"vlc_flowers:tulip_pink",
		"vlc_flowers:azure_bluet",
	}
				...
				if n.name ~= "" and n.name == "air" and (minetest.get_item_group(n2.name, "grass_block_no_snow") == 1) then
					...
					if math.random(1, 100) <= 90 / ((math.abs(i) + math.abs(j)) / 2)then
						...
						if math.random(1,100) <= 90 then
							local col = n2.param2
							minetest.add_node(pos, {name="vlc_flowers:tallgrass", param2=col})
						else
							local flowers_table
...
vlc_player.register_globalstep_slow(function(player, dtime)
	...
	if wi == "vlc_core:barrier" or wi == "vlc_core:realm_barrier" or minetest.get_item_group(wi, "light_block") ~= 0 then
		...
		for z=pos.z-r, pos.z+r do
			...
			local tex
			if nodename == "vlc_core:barrier" then
				tex = "vlc_core_barrier.png"
			elseif nodename == "vlc_core:realm_barrier" then
				tex = "vlc_core_barrier.png^[colorize:#FF00FF:127^[transformFX"
			elseif light_block_group ~= 0 then
				tex = "vlc_core_light_" .. (light_block_group - 1) .. ".png"
			end
			if tex then
