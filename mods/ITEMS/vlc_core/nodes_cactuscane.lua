...
minetest.register_node("vlc_core:cactus", {
	description = S("Cactus"),
	_tt_help = S("Grows on sand").."\n"..minetest.colorize(vlc_colors.YELLOW, S("Contact damage: @1 per half second", 1)),
	...
	tiles = {"vlc_core_cactus_top.png", "vlc_core_cactus_bottom.png", "vlc_core_cactus_side.png"},
	groups = {
	...
	on_place = vlc_util.generate_on_place_plant_function(function(pos, node)
		...
		return (node_below.name == "vlc_core:cactus" or minetest.get_item_group(node_below.name, "sand") == 1)
	end),
	_vlc_blast_resistance = 0.4,
	_vlc_hardness = 0.4,
})

vlc_flowerpots.register_potted_cube("vlc_core:cactus", {
	name = "cactus",
	desc = S("Cactus"),
	image = "vlc_flowerpots_cactus.png",
})

vlc_player.register_globalstep_slow(function(player, dtime)
	-- Am I near a cactus?
	local pos = player:get_pos()
	local near = minetest.find_node_near(pos, 1, "vlc_core:cactus")
	if not near then
		near = minetest.find_node_near({x=pos.x, y=pos.y-1, z=pos.z}, 1, "vlc_core:cactus")
	end
	if near then
		...
		if dist < 1.1 or dist_feet < 1.1 then
			if player:get_hp() > 0 then
				vlc_util.deal_damage(player, 1, {type = "cactus"})
			end
		end
	end
end)

minetest.register_node("vlc_core:reeds", {
	...
	drawtype = "plantlike",
	paramtype2 = "color",
	tiles = {"vlc_core_papyrus.png"},
	palette = "vlc_core_palette_grass.png",
	palette_index = 0,
	inventory_image = "vlc_core_reeds.png",
	wield_image = "vlc_core_reeds.png",
	paramtype = "light",
	walkable = false,
	node_box = {
		type = "fixed",
	...
	node_placement_prediction = "",
	drop = "vlc_core:reeds", -- to prevent color inheritation
	on_place = vlc_util.generate_on_place_plant_function(function(place_pos, place_node)
	...
	on_construct = function(pos)
		...
		if node.param2 == 0 then
			node.param2 = vlc_core.get_grass_palette_index(pos)
			if node.param2 ~= 0 then
