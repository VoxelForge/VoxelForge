...
local sus_drops_default = {
	...
	"vlc_dyes:light_blue",
	"vlc_core:coal_lump",
	"vlc_flowerpots:flower_pot",
}

function vlc_sus_nodes.get_random_item(pos)
	local meta = minetest.get_meta(pos)
	local struct = meta:get_string("structure")
	local structdef = vlc_structures.registered_structures[struct]
	...
	if struct ~= "" and structdef and structdef.loot and structdef.loot["SUS"] then
		local lootitems = vlc_loot.get_multi_loot(structdef.loot["SUS"], pr)
		if #lootitems > 0 then
			return lootitems[1]
		end
	else
...
local function brush_node(itemstack, user, pointed_thing)
	if pointed_thing and pointed_thing.type == "node" then
		...
		if not item_entities[ph] then
			local o = minetest.add_entity(pos + (dir * 0.38),"vlc_sus_nodes:item_entity")
			local l = o:get_luaentity()
			l._item = vlc_sus_nodes.get_random_item(pos)
			if not l._item then
				o:remove()
				return
			end
		...
		if item_entities[ph]._stage >= 4 then
			...
			minetest.swap_node(pos,{name = def._vlc_sus_nodes_parent})
		elseif item_entities[ph]._stage <= 0 then
			minetest.swap_node(pos,{name=def._vlc_sus_nodes_main})
		else
			minetest.swap_node(pos,{name=def._vlc_sus_nodes_main.."_"..item_entities[ph]._stage})
		end
	end
end
...
function vlc_sus_nodes.register_sus_node(name,source,overrides)
	local sdef = minetest.registered_nodes[source]
	assert(sdef, "[vlc_sus_nodes] trying to register "..tostring(name).." but source node "..tostring(source).."doesn't exist")
	local main_itemstring = "vlc_sus_nodes:"..name
	table.shuffle(sus_drops_default)
	local def = table.merge(sdef,tpl,{
		description = S("Suspicious "..name),
		tiles = overlay_tiles(sdef.tiles,"vlc_sus_nodes_suspicious_overlay.png"),
		drop = source,
		_vlc_sus_nodes_parent = source,
		_vlc_sus_nodes_main = main_itemstring,
		_vlc_sus_nodes_drops = table.copy(sus_drops_default),
		_vlc_falling_node_alternative = source,
	},overrides or {})
	minetest.register_node(main_itemstring,def)
	for i=1,3 do
		minetest.register_node(main_itemstring.."_"..i,table.merge(def,{
			tiles = overlay_tiles(sdef.tiles,"vlc_sus_nodes_suspicious_overlay_"..i..".png"),
			groups = table.merge(tpl.groups, { suspicious_stage =i, not_in_creative_inventory = 1 }),
		}))
	end
end

minetest.register_entity("vlc_sus_nodes:item_entity", {
	initial_properties = {
		physical = false,
		visual = "wielditem",
		...
		if self._timer < 0 then
			...
			if self._hide then
				...
				if self._stage <= 0 then
					minetest.swap_node(self._nodepos, {name=def._vlc_sus_nodes_main})
				else
					minetest.swap_node(self._nodepos, {name=def._vlc_sus_nodes_main.."_"..self._stage})
				end
			end
			self._timer = 1
		end
...
minetest.register_tool("vlc_sus_nodes:brush", {
	description = S("Brush"),
	...
	inventory_image = "vlc_sus_nodes_brush.png",
...
minetest.register_craft({
	output = "vlc_sus_nodes:brush",
	recipe = {
		{ "vlc_mobitems:feather"},
		{ "vlc_copper:copper_ingot"},
		{ "vlc_core:stick"},
	}
})

vlc_sus_nodes.register_sus_node("sand","vlc_core:sand",{
	description = S("Suspicious Sand"),
})

vlc_sus_nodes.register_sus_node("gravel","vlc_core:gravel",{
