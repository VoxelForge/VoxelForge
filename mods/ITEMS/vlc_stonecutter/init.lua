...
local S = minetest.get_translator("vlc_stonecutter")
...
minetest.register_on_mods_loaded(vlc_stonecutter.refresh_recipes)
...
local function show_stonecutter_formspec(input)
	...
	"size[11.75,10.425]"..
	"label[0.375,0.375;" .. F(C(vlc_formspec.label_color, S("Stonecutter"))) .. "]"..

	vlc_formspec.get_itemslot_bg_v4(1.375,1.5,1,1,0)..
...
minetest.register_node("vlc_stonecutter:stonecutter", {
	...
	tiles = {
		"vlc_stonecutter_top.png",
		"vlc_stonecutter_bottom.png",
		"vlc_stonecutter_side.png",
		"vlc_stonecutter_side.png",
		{name="vlc_stonecutter_saw.png",
		animation={
		...
		if fields then
			for k,v in pairs(fields) do
				if tostring(k) then
					...
					if def and def._vlc_stonecutter_recipes then
...
minetest.register_craft({
	output = "vlc_stonecutter:stonecutter",
	recipe = {
		{ "", "", "" },
		{ "", "vlc_core:iron_ingot", "" },
