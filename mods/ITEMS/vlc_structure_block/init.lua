...
if minetest.get_translator then
	S = minetest.get_translator("vlc_structure_block")
else
	S = function(s) return s end
end
local F = minetest.formspec_escape

local vlc_structure_block = {}

-- Directory separator
local DIR_DELIM = "/"
-- Set to true to enable `make_vlc_structure_block_readme` command
local MAKE_README = false

local import_path_full = table.concat({minetest.get_modpath("vlc_schematics"), "schems"}, DIR_DELIM)
local export_path_full = table.concat({minetest.get_modpath("vlc_schematics"), "schems"}, DIR_DELIM)
...
local get_border_texture = function()
	local val = minetest.settings:get("vlc_structure_block_border_style")
	if val == "checkers" then
		-- checkers style
		return "vlc_structure_block_border_checkers.png"
	else
		-- edges style
		return "vlc_structure_block_border.png"
	end
end
...
local export_schematic_to_mts = function(pos1, pos2, path, schem_name, player_name, slice_list)
	local plist = vlc_structure_block.scan_metadata(pos1, pos2)
	local probability_list = {}
	for hash, i in pairs(plist) do
		local prob = vlc_structure_block.lua_prob_to_schematic_prob(i.prob)
	...
	if res then
		...
		if schematic and minetest.settings:get_bool("vlc_structure_block_export_lua") and export_schematic_to_lua then
...
function vlc_structure_block.lua_prob_to_schematic_prob(lua_prob)
	return math.floor(lua_prob / 2)
end

function vlc_structure_block.schematic_prob_to_lua_prob(schematic_prob)
	return schematic_prob * 2

end

-- [function] Add form
function vlc_structure_block.add_form(name, def)
	def.name = name
	forms[name] = def
...
function vlc_structure_block.generate_tabs(current)
...
function vlc_structure_block.handle_tabs(pos, name, fields)
	local tab = tonumber(fields.tabs)
	if tab and tabs[tab] and forms[tabs[tab]] then
		vlc_structure_block.show_formspec(pos, name, forms[tabs[tab]].name)
		return true
	end
end

-- [function] Show formspec
function vlc_structure_block.show_formspec(pos, player, tab, show, ...)
	if forms[tab] then
		...
		if show ~= false then
			...
			if forms[tab].tab then
				form = form..vlc_structure_block.generate_tabs(tab)
			end

			minetest.show_formspec(name, "vlc_structure_block:"..tab, form)
			contexts[name] = pos

			-- Update player attribute
			if forms[tab].cache_name ~= false then
				local pmeta = player:get_meta()
				pmeta:set_string("vlc_structure_block:tab", tab)
			end
		else
			minetest.close_formspec(pname, "vlc_structure_block:"..tab)
		end
	end
end
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname[1] == "vlc_structure_block" and forms[formname[2]] then
		...
		if contexts[name] then
			...
			if not vlc_structure_block.handle_tabs(contexts[name], name, fields) and handle then
...
vlc_structure_block.scan_metadata = function(pos1, pos2)
	local prob_list = {}
	...
	for z=pos1.z, pos2.z do
		...
		if node == nil or node.name == "vlc_structure_block:void" then
			prob = 0
			force_place = false
		else
			local meta = minetest.get_meta(scanpos)

			prob = tonumber(meta:get_string("vlc_structure_block_prob")) or 255
			local fp = meta:get_string("vlc_structure_block_force_place")
			if fp == "true" then
				force_place = true
			else
				force_place = false
...
local function set_item_metadata(itemstack, prob, force_place)
	...
	local prob_desc = "\n"..S("Probability: @1", prob or
			smeta:get_string("vlc_structure_block_prob") or S("Not Set"))
	-- Update probability
	if prob and prob >= 0 and prob < 255 then
		smeta:set_string("vlc_structure_block_prob", tostring(prob))
	...
	elseif prob and prob == 255 then
		...
		prob_desc = ""
		smeta:set_string("vlc_structure_block_prob", nil)
		...
		prob_desc = "\n"..S("Probability: @1", smeta:get_string("vlc_structure_block_prob") or
				S("Not Set"))
	end

	-- Update force place
	if force_place == true then
		smeta:set_string("vlc_structure_block_force_place", "true")
	elseif force_place == false then
		smeta:set_string("vlc_structure_block_force_place", nil)
	end
	...
	local force_desc = ""
	if smeta:get_string("vlc_structure_block_force_place") == "true" then
		force_desc = "\n"..S("Force placement")
	end
...
vlc_structure_block.add_form("main", {
	tab = true,
	caption = S("Main"),
	get = function(self, pos, name)
		...
		local border_button
		if meta.schem_border == "true" and vlc_structure_block.markers[hashpos] then
	...
	handle = function(self, pos, name, fields)
		if fields.doc then
			doc.show_entry(name, "nodes", "vlc_structure_block:creator", true)
			return
		end
		...
		if (fields.air2void) then
			local pos1, pos2 = vlc_structure_block.size(pos)
			pos1, pos2 = vlc_structure_block.sort_pos(pos1, pos2)
			local nodes = minetest.find_nodes_in_area(pos1, pos2, {"air"})
			minetest.bulk_set_node(nodes, {name="vlc_structure_block:void"})
			return
		elseif (fields.void2air) then
			local pos1, pos2 = vlc_structure_block.size(pos)
			pos1, pos2 = vlc_structure_block.sort_pos(pos1, pos2)
			local nodes = minetest.find_nodes_in_area(pos1, pos2, {"vlc_structure_block:void"})
		...
		if fields.border then
			if meta.schem_border == "true" and vlc_structure_block.markers[hashpos] then
				vlc_structure_block.unmark(pos)
				meta.schem_border = "false"
			else
				vlc_structure_block.mark(pos)
		...
		if fields.export and meta.schem_name and meta.schem_name ~= "" then
			...
			for _, i in pairs(slist) do
				slice_list[#slice_list + 1] = {
					ypos = i.ypos,
					prob = vlc_structure_block.lua_prob_to_schematic_prob(i.prob),
				}
			end
			...
			if schematic then
				...
				if success then
					...
					for x=0, special_x_size-1 do
						...
						if data.prob == 0 then
							minetest.set_node(pp, {name="vlc_structure_block:void"})
						else
							local meta = minetest.get_meta(pp)
							if data.prob and data.prob ~= 255 and data.prob ~= 254 then
								meta:set_string("vlc_structure_block_prob", tostring(data.prob))
							else
								meta:set_string("vlc_structure_block_prob", "")
							end
							if data.force_place then
								meta:set_string("vlc_structure_block_force_place", "true")
							else
								meta:set_string("vlc_structure_block_force_place", "")
							end
						end
						d = d + 1
					end
		...
		if not fields.quit then
			vlc_structure_block.show_formspec(pos, minetest.get_player_by_name(name), "main")
		end
	end,
})

vlc_structure_block.add_form("slice", {
	caption = S("Y Slices"),
	tab = true,
	...
	handle = function(self, pos, name, fields)
		...
		if fields.add then
			if not self.panel_add then
				self.panel_add = true
				vlc_structure_block.show_formspec(pos, player, "slice")
			else
				self.panel_add = nil
				vlc_structure_block.show_formspec(pos, player, "slice")
			end
...
vlc_structure_block.add_form("probtool", {
	cache_name = false,
	...
	get = function(self, pos, name)
		...
		if probtool:get_name() ~= "vlc_structure_block:probtool" then
			return
		end

		local meta = probtool:get_meta()
		local prob = tonumber(meta:get_string("vlc_structure_block_prob"))
		local force_place = meta:get_string("vlc_structure_block_force_place")
...
function vlc_structure_block.sort_pos(pos1, pos2)
...
function vlc_structure_block.size(pos)
...
function vlc_structure_block.mark(pos)
	vlc_structure_block.unmark(pos)
	...
	for _, z in ipairs({pos1.z - 0.5, pos2.z + 0.5}) do
		...
		local marker = minetest.add_entity({x = pos1.x + sizex - 0.5, y = pos1.y + sizey - 0.5, z = z + offset}, "vlc_structure_block:display")
		if marker ~= nil then
	...
	for _, x in ipairs({pos1.x - 0.5, pos2.x + 0.5}) do
		...
		local marker = minetest.add_entity({x = x + offset, y = pos1.y + sizey - 0.5, z = pos1.z + sizez - 0.5}, "vlc_structure_block:display")
		if marker ~= nil then
			marker:set_properties({
	...
	for _, y in ipairs({pos1.y - 0.5, pos2.y + 0.5}) do
		...
		local marker = minetest.add_entity({x = pos1.x + sizex - 0.5, y = y + offset, z = pos1.z + sizez - 0.5}, "vlc_structure_block:display")
		if marker ~= nil then
			marker:set_properties({
	...
	vlc_structure_block.markers[id] = m
	return true
end

-- [function] Unmark region
function vlc_structure_block.unmark(pos)
	local id = minetest.hash_node_position(pos)
	if vlc_structure_block.markers[id] then
		local retval
		for _, entity in ipairs(vlc_structure_block.markers[id]) do
			entity:remove()
			retval = true
		end
		return retval
...
function vlc_structure_block.display_node_prob(player, pos, prob, force_place)
	local wpstring
...
function vlc_structure_block.display_node_probs_region(player, pos1, pos2)
	...
	for x=pos1.x, pos2.x do
		for y=pos1.y, pos2.y do
			for z=pos1.z, pos2.z do
				...
				prob = meta:get_string("vlc_structure_block_prob")
				force_place = meta:get_string("vlc_structure_block_force_place") == "true"
				local hud_id = vlc_structure_block.display_node_prob(player, checkpos, prob, force_place)
				if hud_id then
...
function vlc_structure_block.clear_displayed_node_probs(player)
	local playername = player:get_player_name()
	...
	if cleartimer > 2 then
		...
		for p = 1, #players do
			...
			if displayed_waypoints[pname].display_active then
				...
				if item:get_name() ~= "vlc_structure_block:probtool" then
					vlc_structure_block.clear_displayed_node_probs(player)
				end
			end
		end
		cleartimer = 0
...
minetest.register_privilege("schematic_override", {
	description = S("Allows you to access vlc_structure_block nodes not owned by you"),
...
minetest.register_node("vlc_structure_block:creator", {
...
S("With a schematic node probability tool, you can set a probability for each node and enable them to overwrite all nodes when placed as schematic. This tool must be used prior to the file export."),
	tiles = {"vlc_structure_block_creator_top.png", "vlc_structure_block_creator_bottom.png",
			"vlc_structure_block_creator_sides.png"},
	...
	on_rightclick = function(pos, node, player)
		...
		if meta:get_string("owner") == name or
			...
			local pmeta = player:get_meta()
			local tab = pmeta:get_string("vlc_structure_block:tab")
			if not forms[tab] or not tab then
				tab = "main"
			end

			vlc_structure_block.show_formspec(pos, player, tab, true)
		end
	end,
...
minetest.register_tool("vlc_structure_block:probtool", {
...
S("The node HUD is not updated automatically and may be outdated. The node HUD only updates the HUD for nodes close to you whenever you place the tool or press the punch and sneak keys simultaneously. If you sneak-punch a schematic creator, then the node HUD is updated for all nodes within the schematic creator's region, even if this region is very big."),
	wield_image = "vlc_structure_block_probtool.png",
	inventory_image = "vlc_structure_block_probtool.png",
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if not ctrl.sneak then
			...
			vlc_structure_block.show_formspec(user:get_pos(), user, "probtool", true)
		...
		-- Use + sneak
			...
			if pointed_thing and pointed_thing.type == "node" and pointed_thing.under then
				...
				if node.name == "vlc_structure_block:creator" then
					local pos1, pos2 = vlc_structure_block.size(punchpos)
					pos1, pos2 = vlc_structure_block.sort_pos(pos1, pos2)
					vlc_structure_block.display_node_probs_region(user, pos1, pos2)
					return
				end
			end

			-- Otherwise, just display the region close to the player
			vlc_structure_block.display_node_probs_region(user)
		end
	end,
	on_secondary_use = function(itemstack, user, pointed_thing)
		...
		vlc_structure_block.clear_displayed_node_probs(user)
	end,
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if node.name == "vlc_structure_block:void" then
			return itemstack
		end
		local nmeta = minetest.get_meta(pos)
		local imeta = itemstack:get_meta()
		local prob = tonumber(imeta:get_string("vlc_structure_block_prob"))
		local force_place = imeta:get_string("vlc_structure_block_force_place")

		if not prob or prob == 255 then
			nmeta:set_string("vlc_structure_block_prob", nil)
		else
			nmeta:set_string("vlc_structure_block_prob", prob)
		end
		if force_place == "true" then
			nmeta:set_string("vlc_structure_block_force_place", "true")
		else
			nmeta:set_string("vlc_structure_block_force_place", nil)
		end

		-- Enable node probablity display
		vlc_structure_block.display_node_probs_region(placer)
...
minetest.register_node("vlc_structure_block:void", {
	...
	tiles = { "vlc_structure_block_void.png" },
...
minetest.register_entity("vlc_structure_block:display", {
	...
	on_step = function(self, dtime)
		if not self.id then
			self.object:remove()
		elseif not vlc_structure_block.markers[self.id] then
			self.object:remove()
		end
	end,
...
minetest.register_lbm({
	...
	name = "vlc_structure_block:reset_border",
	nodenames = "vlc_structure_block:creator",
	...
	func = function(name, param)
		...
		if minetest.read_schematic then
			...
			if schematic then
				...
				minetest.log("action", "[vlc_structure_block] Placing schematic '"..schem.."' at: "..minetest.pos_to_string(pos))
...
minetest.register_chatcommand("listschems", {
	...
	params = "",
	func = function(name, param)
		local files = minetest.get_dir_list(import_path_full, false)
		if not files then
			return false
		end
		local out_files = {}
...
if MAKE_README then
	dofile(minetest.get_modpath("vlc_structure_block")..DIR_DELIM.."make_readme.lua")
end
