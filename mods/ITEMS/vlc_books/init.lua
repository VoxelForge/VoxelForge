...
local bookshelf_inv = minetest.settings:get_bool("vlc_bookshelf_inventories", true)

local header = ""
if minetest.get_modpath("vlc_init") then
	header = "no_prepend[]" .. vlc_vars.gui_nonbg .. vlc_vars.gui_bg_color ..
		"style_type[button;border=false;bgimg=vlc_books_button9.png;bgimg_pressed=vlc_books_button9_pressed.png;bgimg_middle=2,2]"
end

-- Book
minetest.register_craftitem("vlc_books:book", {
	description = S("Book"),
...
minetest.register_craft({
	type = "shapeless",
	output = "vlc_books:book",
	recipe = { "vlc_core:paper", "vlc_core:paper", "vlc_core:paper", "vlc_mobitems:leather", }
})
...
local function make_description(title, author, generation)
	...
	desc = desc .. "\n" .. minetest.colorize(vlc_colors.GRAY, S("by @1", author))
	return desc
end
...
local function write(itemstack, user, pointed_thing)
	...
	local formspec = "size[8,9]" ..
		header ..
		"background[-0.5,-0.5;9,10;vlc_books_book_bg.png]" ..
	...
	minetest.show_formspec(user:get_player_name(), "vlc_books:writable_book", formspec)
end
...
local function read(itemstack, user, pointed_thing)
	...
	minetest.show_formspec(user:get_player_name(), "vlc_books:written_book", formspec)
end

-- Book and Quill
minetest.register_craftitem("vlc_books:writable_book", {
	...
	inventory_image = "vlc_books_book_writable.png",
	groups = { book = 1 },
	stack_max = 1,
	on_place = write,
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if ((formname == "vlc_books:writable_book") and fields and fields.text) then
		local stack = player:get_wielded_item()
		if (stack:get_name() and (stack:get_name() == "vlc_books:writable_book")) then
			...
			elseif fields.sign then
				...
				minetest.show_formspec(player:get_player_name(), "vlc_books:signing", formspec)
			end
		end
	elseif ((formname == "vlc_books:signing") and fields and fields.sign and fields.title) then
		local newbook = ItemStack("vlc_books:written_book")
		local book = player:get_wielded_item()
		local name = player:get_player_name()
		if book:get_name() == "vlc_books:writable_book" then
			...
			minetest.log("error", "[vlc_books] " .. name .. " failed to sign a book!")
		end
	elseif ((formname == "vlc_books:signing") and fields and fields.cancel) then
		local book = player:get_wielded_item()
		if book:get_name() == "vlc_books:writable_book" then
...
minetest.register_craft({
	type = "shapeless",
	output = "vlc_books:writable_book",
	recipe = { "vlc_books:book", "vlc_mobitems:ink_sac", "vlc_mobitems:feather" },
})

-- Written Book
minetest.register_craftitem("vlc_books:written_book", {
	...
	inventory_image = "vlc_books_book_written.png",
...
for i = 1, 8 do
	local rc = {}
	table.insert(rc, "vlc_books:written_book")
	for j = 1, i do	table.insert(rc, "vlc_books:writable_book") end

	minetest.register_craft({
		type = "shapeless",
		output = "vlc_books:written_book " .. i,
		recipe = rc,
	})
end

local function craft_copy_book(itemstack, player, old_craft_grid, craft_inv)
	if itemstack:get_name() ~= "vlc_books:written_book" then
		return
	end
	...
	for i = 1, player:get_inventory():get_size("craft") do
		if old_craft_grid[i]:get_name() == "vlc_books:written_book" then
...
-- Bookshelf GUI
local drop_content = vlc_util.drop_items_from_meta_container("main")
...
local function protection_check_put_take(pos, listname, index, stack, player)
	...
	if minetest.is_protected(pos, name) then
		minetest.record_protection_violation(pos, name)
		return 0
	elseif minetest.get_item_group(stack:get_name(), "book") ~= 0 or stack:get_name() == "vlc_enchanting:book_enchanted" then
...
local function bookshelf_gui(pos, node, clicker)
	...
	minetest.show_formspec(playername,
		"vlc_books:bookshelf_" .. pos.x .. "_" .. pos.y .. "_" .. pos.z,
		table.concat({
			"formspec_version[4]",
			"size[11.75,10.425]",
			...
			vlc_formspec.get_itemslot_bg_v4(0.375, 0.75, 9, 3, 0, "vlc_book_book_empty_slot.png"),
...
local function close_forms(pos)
	...
	local formname = "vlc_books:bookshelf_" .. pos.x .. "_" .. pos.y .. "_" .. pos.z
	for p = 1, #players do
...
-- Bookshelf
minetest.register_node("vlc_books:bookshelf", {
	description = S("Bookshelf"),
	_doc_items_longdesc = S("Bookshelves are used for decoration."),
	tiles = { "vlc_books_bookshelf_top.png", "vlc_books_bookshelf_top.png", "default_bookshelf.png" },
...
minetest.register_craft({
	output = "vlc_books:bookshelf",
	recipe = {
		{ "group:wood",     "group:wood",     "group:wood" },
		{ "vlc_books:book", "vlc_books:book", "vlc_books:book" },
		{ "group:wood",     "group:wood",     "group:wood" },
	}
})
