...
local function players_in_bed_setting()
	return tonumber(minetest.settings:get("vlc_playersSleepingPercentage")) or 100
end
...
local function players_in_overworld(players)
	return table.count(players, function(_, player)
		return vlc_worlds.pos_to_dimension(player:get_pos()) == "overworld"
	end)
end
...
function vlc_beds.is_night(tod)
...
local function lay_down(player, pos, bed_pos, state, skip)
	...
	if bed_pos then
		...
		if vlc_spawn.set_spawn_pos(player, bed_pos, nil) then
			minetest.chat_send_player(name, S("New respawn position set!"))
			awards.unlock(player:get_player_name(), "vlc:sweetDreams")
		end


		if not vlc_beds.is_night() and vlc_weather.get_weather() ~= "thunder" then
		...
		for _, other_pos in pairs(vlc_beds.bed_pos) do
	...
	if state ~= nil and not state then
		local p = vlc_beds.pos[name] or nil
		if vlc_beds.player[name] then
			vlc_beds.player[name] = nil
			player_in_bed = player_in_bed - 1
		end
		vlc_beds.pos[name] = nil
		vlc_beds.bed_pos[name] = nil
		if p then
			player:set_pos(p)
		end
		...
		vlc_player.players[player].attached = false
		playerphysics.remove_physics_factor(player, "speed", "vlc_beds:sleeping")
		playerphysics.remove_physics_factor(player, "jump", "vlc_beds:sleeping")
		player:get_meta():set_string("vlc_beds:sleeping", "false")
	...
	-- lay down
		...
		vlc_beds.player[name] = 1
		vlc_beds.pos[name] = pos
		vlc_beds.bed_pos[name] = bed_pos2
		player_in_bed = player_in_bed + 1
		-- physics, eye_offset, etc
		...
		player:get_meta():set_string("vlc_beds:sleeping", "true")
		playerphysics.add_physics_factor(player, "speed", "vlc_beds:sleeping", 0)
		playerphysics.add_physics_factor(player, "jump", "vlc_beds:sleeping", 0)
		player:set_pos(bed_center)
		vlc_player.players[player].attached = true
		hud_flags.wielditem = false
		vlc_player.player_set_animation(player, "lay" , 0)
	end
...
local function update_formspecs(finished, players)
	...
	if finished then
		for name,_ in pairs(vlc_beds.player) do
			minetest.close_formspec(name, "vlc_beds_form")
		end
		return
	...
	for name,_ in pairs(vlc_beds.player) do
		minetest.show_formspec(name, "vlc_beds_form", form_n)
	end
end
...
function vlc_beds.sleep()
	if is_night_skip_enabled() then
		if vlc_weather.get_weather() == "thunder" then
			local endtime = (vlc_weather.end_time - minetest.get_gametime()) * 72 / 24000
			minetest.set_timeofday((minetest.get_timeofday() + endtime) %1)
			if vlc_beds.is_night() then
				vlc_beds.skip_night()
				vlc_beds.kick_players()
			else
				vlc_beds.kick_players()
			end
			-- Always clear weather
			vlc_weather.change_weather("none")
		elseif vlc_beds.is_night() then
			vlc_beds.skip_night()
			vlc_beds.kick_players()
			vlc_weather.change_weather("none")
		end
...
function vlc_beds.kick_players()
	for name, _ in pairs(vlc_beds.player) do
...
function vlc_beds.kick_player(player)
	local name = player:get_player_name()
	if vlc_beds.player[name] then
		lay_down(player, nil, nil, false)
		update_formspecs(false)
...
function vlc_beds.last_skip()
	return last_skip
end

function vlc_beds.skip_night()
...
function vlc_beds.get_bed_top (pos)
...
function vlc_beds.get_bed_bottom (pos)
	local node = minetest.get_node(pos)
	local dir = minetest.facedir_to_dir(node.param2)
	vlc_log("Dir: " .. tostring(dir))
	local bed_bottom = vector.add(pos, -dir)
	vlc_log("bed_bottom: " .. tostring(bed_bottom))

	local bed_bottom_node = minetest.get_node(bed_bottom)
	if bed_bottom_node then
		vlc_log("Bed bottom node name:" .. bed_bottom_node.name)
	else
		vlc_log("Didn't get bed bottom")
	end
...
function vlc_beds.on_rightclick(pos, player, is_top)
	-- Anti-Inception: Don't allow to sleep while you're sleeping
	if player:get_meta():get_string("vlc_beds:sleeping") == "true" then
		return
	end
	...
	if dim == "nether" or dim == "end" then
		...
		vlc_explosions.explode(pos, 5, {fire = true})
		return
	end
	local name = player:get_player_name()
	...
	-- move to bed
	if not vlc_beds.player[name] then
		local message
		if is_top then
			message = select(2, lay_down(player, ppos, pos))
		else
			local other = vlc_beds.get_bed_top (pos)
			message = select(2, lay_down(player, ppos, other))
		end
		if message then
			vlc_title.set(player, "actionbar", {text=message, color="white", stay=60})
			...
			for _, player in pairs(connected_players) do
				...
				if not vlc_beds.player[player:get_player_name()] and vlc_worlds.pos_to_dimension(player:get_pos()) == "overworld" then
					-- clear, old message is still being displayed
					if vlc_title.params_get(player) then vlc_title.clear(player) end
					vlc_title.set(player, "actionbar", {text=sleep_hud_message, color="white", stay=60})
				end
			end
		end
	else
...
minetest.register_on_joinplayer(function(player)
	...
	if meta:get_string("vlc_beds:sleeping") == "true" then
		-- Make player awake on joining server
		meta:set_string("vlc_beds:sleeping", "false")
	end
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "vlc_beds_form" then
		return
	end
