...
local MAP_EDGE = math.floor(vlf_vars.mapgen_limit / (16 * 5)) * 16 * 5 - 16 * 3
...
for dim, key in pairs(mod_storage_keys) do
	for _, portal in pairs(minetest.deserialize(vlf_portals.storage:get_string(key)) or {}) do
...
-- portal.
local search_y_min = {
	overworld = vlf_vars.mg_bedrock_overworld_max + 1,
	nether = vlf_vars.mg_bedrock_nether_bottom_max + 1,
}
local search_y_max = {
	overworld = vlf_vars.mg_overworld_max_official,
	nether = vlf_vars.mg_bedrock_nether_top_min - 6,
}
...
function vlf_portals.nether_portal_cooloff(object)
...
local function update_mod_storage(dim)
	vlf_portals.storage:set_string(mod_storage_keys[dim], minetest.serialize(get_portals(dim)))
end
...
local function register_portal(pos)
	...
	if not portals[dim][hash] then
		...
		portal_count = portal_count + 1
		minetest.log("action", "[vlf_portal] Registered portal at " .. tostring(pos))
		minetest.log("action", "[vlf_portal] There are " .. portal_count .. " registered portals in total")
...
local function light_nether_portal(pos, param2)
	...
	while queue:size() > 0 do
		...
		if not checked[hash] then
			...
			elseif name ~= "vlf_core:obsidian" then
				return false
			end
...
local function get_adjacent_portal_nodes(pos)
	...
	if node.name ~= "vlf_portals:portal" then
		return {}
	end
	...
	local function check_remove(pos)
		...
		local node = minetest.get_node(pos)
		if node and node.name == "vlf_portals:portal" and (param2 == nil or node.param2 == param2) then
			table.insert(nodes, pos)
...
local function update_old_meta(portal)
	...
	if target_hash then
		...
		if vlf_vars.get_node(target_portal).name ~= "vlf_portals:portal" then
			return
		end
...
local function get_portal(pos)
	local node = vlf_vars.get_node(pos)
	if node.name == "vlf_portals:portal" then
...
local function build_portal(pos, param2, bad_spot)
	...
	minetest.bulk_set_node(obsidian, { name = "vlf_core:obsidian" })
	minetest.bulk_set_node(air, { name = "air" })
	minetest.bulk_set_node(portals, { name = "vlf_portals:portal", param2 = param2 })
	init_portal_meta(portals, pos)
	register_portal(pos)

	minetest.log("action", "[vlf_portal] Destination portal generated at " .. tostring(pos))
end
...
local function finalize_teleport(obj, pos, old_param2, new_param2)
	...
	if obj:is_player() then
		...
		minetest.log("action", "[vlf_portal] " .. obj:get_player_name() .. " teleported to " .. tostring(pos))
	else
...
local function portal_emerge_area(blockpos, action, calls_remaining, param)
	...
	minetest.sound_play("vlf_portals_teleport", {pos = obj:get_pos(), gain = 0.5, max_hear_distance = 1}, true)
	minetest.log("action", "[vlf_portal] Could not generate destination portal for " .. player_name .. " at " .. tostring(portal))
	...
	for _, obj in pairs(minetest.get_objects_inside_radius(pos, 15)) do
		if obj:is_player() then
			minetest.add_particle({
				...
				texture = "vlf_particles_nether_portal.png",
...
local longdesc = minetest.registered_nodes["vlf_core:obsidian"]._doc_items_longdesc .. "\n" .. S("Obsidian is also used as the frame of Nether portals.")
...
function vlf_portals.light_nether_portal(pos)
	for param2 = 0, 1 do
...
minetest.override_item("vlf_core:obsidian", {
	...
	_on_ignite = function(user, pointed_thing)
		...
		if portal_placed then
			minetest.log("action", "[vlf_portal] Portal activated at " .. tostring(pos))
			if minetest.get_modpath("doc") then
				doc.mark_entry_as_revealed(user:get_player_name(), "nodes", "vlf_portals:portal")

				local dim = vlf_worlds.pos_to_dimension(pos)
				if minetest.get_modpath("awards") and dim ~= "nether" and user:is_player() then
					awards.unlock(user:get_player_name(), "vlf:buildNetherPortal")
				end
			end
			return true
		else
...
minetest.register_node("vlf_portals:portal", {
