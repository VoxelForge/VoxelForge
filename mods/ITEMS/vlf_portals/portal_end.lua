...
vlf_portals.registered_on_beat_game = {}
function vlf_portals.register_on_beat_game(func)
	table.insert(vlf_portals.registered_on_beat_game, func)
end
-- Parameters
--local SPAWN_MIN = vlf_vars.mg_end_min+70
--local SPAWN_MAX = vlf_vars.mg_end_min+98
...
local function destroy_portal(pos)
	...
	for n=1, #neighbors do
		...
		if minetest.get_node(npos).name == "vlf_portals:portal_end" then
...
-- End portal
minetest.register_node("vlf_portals:portal_end", {
	...
	tiles = {
		{
			name = "vlf_portals_end_portal.png",
			animation = {
...
local function check_end_portal_frame(pos)
	for i = 1, 12 do
		...
		for j = 1, 12 do
			...
			if not node or node.name ~= "vlf_portals:end_portal_frame_eye" or node.param2 ~= ep_scheme[j].p then
				portal = false
				break
			end
		end
...
local function show_credits(player)
	...
	if completed_end == 0 then
		...
		for _, func in ipairs(vlf_portals.registered_on_beat_game) do
			func(player)
		end
		vlf_credits.show(player)
	end
end

function vlf_portals.end_teleport(obj, pos)
	...
	if dim == "end" then
		...
		-- The destination is built by vlf_structures.

		local platform_pos = vlf_vars.mg_end_platform_pos
	...
	if obj:is_player() then
		...
		vlf_worlds.dimension_change(obj, vlf_worlds.pos_to_dimension(target))
		minetest.sound_play("vlf_portals_teleport", {pos=target, gain=0.5, max_hear_distance = 16}, true)
	else
...
function vlf_portals.end_portal_teleport(pos)
	for obj in minetest.objects_inside_radius(pos, 1) do
		...
		if obj:is_player() or lua_entity then
			...
			if minetest.get_node(objpos).name ~= "vlf_portals:portal_end" then
				return
			end

			vlf_portals.end_teleport(obj, objpos)
			awards.unlock(obj:get_player_name(), "vlf:enterEndPortal")
		end
	end
end
...
minetest.register_abm({
	...
	nodenames = {"vlf_portals:portal_end"},
	interval = 0.1,
	chance = 1,
	action = vlf_portals.end_portal_teleport,
})
...
minetest.register_node("vlf_portals:end_portal_frame", {
	...
	_vlf_blast_resistance = 36000000,
	_vlf_hardness = -1,
})

minetest.register_node("vlf_portals:end_portal_frame_eye", {
...
if has_doc then
	doc.add_entry_alias("nodes", "vlf_portals:end_portal_frame", "nodes", "vlf_portals:end_portal_frame_eye")
end
...
-- Portal opener
local old_on_place = minetest.registered_items["vlf_end:ender_eye"].on_place
minetest.override_item("vlf_end:ender_eye", {
	on_place = function(itemstack, user, pointed_thing)
		...
		if pointed_thing.under and node.name == "vlf_portals:end_portal_frame" then
			...
			minetest.set_node(pointed_thing.under, { name = "vlf_portals:end_portal_frame_eye", param2 = node.param2 })

			if has_doc then
				doc.mark_entry_as_revealed(user:get_player_name(), "nodes", "vlf_portals:end_portal_frame")
			end
			minetest.sound_play(
			...
			if ok then
				...
				end_portal_area(ppos)
				if has_doc then
					doc.mark_entry_as_revealed(user:get_player_name(), "nodes", "vlf_portals:portal_end")
				end
			end
