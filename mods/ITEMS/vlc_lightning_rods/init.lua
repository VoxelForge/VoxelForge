local S = minetest.get_translator("vlc_lightning_rods")
...
local rod_def = {
	...
	tiles = { "vlc_lightning_rods_rod.png" },
	drawtype = "mesh",
	mesh = "vlc_lightning_rods_rod.obj",
...
minetest.register_node("vlc_lightning_rods:rod", rod_def)

local rod_def_a = table.copy(rod_def)

rod_def_a.tiles = { "vlc_lightning_rods_rod.png^[brighten" }
...
rod_def_a.on_timer = function(pos, elapsed)
	...
	if node.name == "vlc_lightning_rods:rod_powered" then --has not been dug
		node.name = "vlc_lightning_rods:rod"
...
minetest.register_node("vlc_lightning_rods:rod_powered", rod_def_a)
...
vlc_lightning.register_on_strike(function(pos, pos2, objects)
	...
	if lr then
		...
		if node.name == "vlc_lightning_rods:rod" then
			node.name = "vlc_lightning_rods:rod_powered"
