...
-- at start_pos.
function vlf_end.detach_chorus_plant(start_pos, digger)
	...
	for t=1, #tree_start_posses do
		...
		while #check_posses > 0 do
			...
			if not touched_nodes_hashes[minetest.hash_node_position(pos)] then
				...
				if node.name == "vlf_end:end_stone" then
		...
		if break_tree then
			...
			for c=1, #chorus_nodes do
				...
				if digger then
					minetest.node_dig(chorus_nodes[c], { name = "vlf_end:chorus_plant" }, digger)
				else
...
function vlf_end.check_blast_chorus_plant(pos)
	minetest.remove_node(pos)
	vlf_end.detach_chorus_plant(pos)
end

minetest.register_node("vlf_end:chorus_flower", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		-- Condition 1
		if node_below.name == "vlf_end:chorus_plant" or node_below.name == "vlf_end:end_stone" then
			plant_ok = true
		-- Condition 2
		elseif node_below.name == "air" then
			...
			for a=1, #around do
				...
				if node_side.name == "vlf_end:chorus_plant" then
		...
		if plant_ok then
			...
			if suc then
				minetest.sound_play(vlf_sounds.node_sound_wood_defaults().place, {pos = pos}, true)
			end
			return it
		else
			return itemstack
...
minetest.register_node("vlf_end:chorus_flower_dead", {
	...
	tiles = {
		"vlf_end_chorus_flower_dead.png",
		"vlf_end_chorus_flower_dead.png",
		"vlf_end_chorus_flower_dead.png",
		"vlf_end_chorus_flower_dead.png",
		"vlf_end_chorus_flower_dead.png",
...
minetest.register_node("vlf_end:chorus_plant", {
	...
	connects_to = {"group:chorus_plant", "vlf_end:end_stone"},
	sounds = vlf_sounds.node_sound_wood_defaults(),
	drop = {
		items = {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		local plant_ok = false
		if node_check.name == "vlf_end:end_stone" or minetest.get_item_group(node_check.name, "chorus_plant") > 0 then
			plant_ok = true
		end
		if plant_ok then
			-- Placement OK! Proceed normally
			local it, suc = minetest.item_place_node(itemstack, placer, pointed_thing)
			if suc then
				minetest.sound_play(vlf_sounds.node_sound_wood_defaults().place, {pos = pos_place}, true)
			end
			return it
		else
			return itemstack
...
function vlf_end.grow_chorus_plant(pos, node, pr)
	local flowers = { pos }
	...
	if node.name ~= "vlf_end:chorus_flower" then
		minetest.set_node(pos, { name = "vlf_end:chorus_flower" })
	end
	while true do
		local new_flowers_list = {}
		for f=1, #flowers do
			local new_flowers = vlf_end.grow_chorus_plant_step(flowers[f], minetest.get_node(flowers[f]), pr)
...
function vlf_end.grow_chorus_plant_step(pos, node, pr)
	...
	if node_above.name == "air" and air_around then
		...
		for y=1, 4 do
			...
			if node.name == "vlf_end:chorus_plant" then
				h = y
		...
		if grow_chance then
			...
			for _, f in pairs(new_flowers) do
				if age >= MAX_FLOWER_AGE then
					...
					if nn ~= "vlf_end:chorus_flower" and nn ~= "vlf_end:chorus_flower_dead" then
						minetest.set_node(f, {name="vlf_end:chorus_flower_dead"})
						grown = true
					end
				else
					local nn = minetest.get_node(f).name
					if nn ~= "vlf_end:chorus_flower" and nn ~= "vlf_end:chorus_flower_dead" then
						minetest.set_node(f, {name="vlf_end:chorus_flower", param2 = age})
			...
			if #new_flowers >= 1 then
				minetest.set_node(pos, {name="vlf_end:chorus_plant"})
				grown = true
			end
		end
	end
	if not grown then
		...
		minetest.set_node(pos, {name = "vlf_end:chorus_flower_dead"})
	end
...
local function random_teleport(player)
	...
	for _ = 1, 16 do
		...
		for t=0, 16 do
			...
			if tnode.name == "vlf_core:void" or tnode.name == "ignore" then
				break
			end
		...
		if ground_level and #node_cache >= 1 then
			...
			for c=#node_cache, 1, -1 do
				...
				if streak >= 2 then
					...
					player:set_pos(goal)
					minetest.sound_play({name="vlf_end_teleport", gain=0.8}, {pos=goal, max_hear_distance=16}, true)
					return true
				end
			end
		end
...
local eat_chorus_fruit = function(itemstack, player, pointed_thing)
	...
	if count ~= new_count or new_itemstack:get_name() ~= "vlf_end:chorus_fruit" or (minetest.is_creative_enabled(player:get_player_name()) == true) then
		random_teleport(player)
	end
	return new_itemstack
end

minetest.register_craftitem("vlf_end:chorus_fruit", {
	...
	wield_image = "vlf_end_chorus_fruit.png",
	inventory_image = "vlf_end_chorus_fruit.png",
...
minetest.register_craftitem("vlf_end:chorus_fruit_popped", {
	...
	wield_image = "vlf_end_chorus_fruit_popped.png",
