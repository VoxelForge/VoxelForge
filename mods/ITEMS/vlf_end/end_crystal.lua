...
local function crystal_explode(self, puncher)
	...
	if puncher then
		...
		local reason = {}
		vlf_damage.from_punch(reason, puncher)
...
local function spawn_crystal(pos)
	minetest.add_entity(pos, "vlf_end:crystal")
	if not vector.equals(pos, vector.floor(pos)) then return end
	if vlf_worlds.pos_to_dimension(pos) ~= "end" then return end
	local portal_center
	for _, dir in pairs(directions) do
	...
	for o in minetest.objects_inside_radius(pos, 64) do
		...
		if not peaceful then
			if o:is_player() then
				awards.unlock(o:get_player_name(), "vlf:theEndAgain")
			end
		end
	end
	...
	vlf_structures.place_structure(portal_pos,vlf_structures.registered_structures["end_exit_portal"],PseudoRandom(minetest.get_mapgen_setting("seed")),-1)
end

minetest.register_entity("vlf_end:crystal", {
	initial_properties = {
		physical = true,
		visual = "mesh",
		visual_size = {x = 6, y = 6},
...
minetest.register_entity("vlf_end:crystal_beam", {
	initial_properties = {
		...
		textures = {
			"vlf_end_crystal_beam.png^[transformR90",
			"vlf_end_crystal_beam.png^[transformR90",
			"vlf_end_crystal_beam.png",
			"vlf_end_crystal_beam.png",
			"blank.png",
			"blank.png",
		},
...
minetest.register_craftitem("vlf_end:crystal", {
	inventory_image = "vlf_end_crystal_item.png",
	...
	on_place = function(itemstack, placer, pointed_thing)
		if pointed_thing.type == "node" then
			...
			if node_name == "vlf_core:obsidian" or node_name == "vlf_core:bedrock" then
...
minetest.register_craft({
	output = "vlf_end:crystal",
	recipe = {
		{"vlf_core:glass", "vlf_core:glass", "vlf_core:glass"},
		{"vlf_core:glass", "vlf_end:ender_eye", "vlf_core:glass"},
		{"vlf_core:glass", "vlf_mobitems:ghast_tear", "vlf_core:glass"},
	}
})

minetest.register_alias("vlf_end_crystal:end_crystal", "vlf_end:crystal")
