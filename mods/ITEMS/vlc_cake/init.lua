...
minetest.register_craft({
	output = "vlc_cake:cake",
	recipe = {
		{"vlc_mobitems:milk_bucket", "vlc_mobitems:milk_bucket", "vlc_mobitems:milk_bucket"},
		{"vlc_core:sugar", "vlc_throwing:egg", "vlc_core:sugar"},
		{"vlc_farming:wheat_item", "vlc_farming:wheat_item", "vlc_farming:wheat_item"},
	},
	replacements = {
		{"vlc_mobitems:milk_bucket", "vlc_buckets:bucket_empty"},
		{"vlc_mobitems:milk_bucket", "vlc_buckets:bucket_empty"},
		{"vlc_mobitems:milk_bucket", "vlc_buckets:bucket_empty"},
	},
})

minetest.register_node("vlc_cake:cake", {
	description = S("Cake"),
	...
	on_rightclick = function(pos, node, clicker, itemstack)
		...
		local newcake = minetest.do_item_eat(2, ItemStack("vlc_cake:cake_6"), ItemStack("vlc_cake:cake"), clicker, {type="nothing"})
		-- Check if we were allowed to eat
		if newcake:get_name() ~= "vlc_cake:cake" or minetest.is_creative_enabled(clicker:get_player_name()) then
			minetest.add_node(pos,{type="node",name="vlc_cake:cake_6",param2=0})
		end
	end,
...
local register_slice = function(level, nodebox, desc)
	local this = "vlc_cake:cake_"..level
	local after_eat = "vlc_cake:cake_"..(level-1)
	local on_rightclick
	if level > 1 then
		on_rightclick = function(pos, node, clicker, itemstack)
			local name = clicker:get_player_name()
		...
		on_rightclick = function(pos, node, clicker, itemstack)
			...
			local newcake = minetest.do_item_eat(CAKE_HUNGER_POINTS, ItemStack("vlc:cake:cake 0"), ItemStack("vlc_cake:cake_1"), clicker, {type="nothing"})
			-- Check if we were allowed to eat
			if newcake:get_name() ~= this or minetest.is_creative_enabled(clicker:get_player_name()) then
	...
	if minetest.get_modpath("doc") then
		doc.add_entry_alias("nodes", "vlc_cake:cake", "nodes", "vlc_cake:cake_"..level)
	end
end
