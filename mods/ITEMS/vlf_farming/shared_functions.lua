...
function vlf_farming:add_plant(identifier, full_grown, names, interval, chance)
	vlf_farming.plant_lists[identifier] = {}
	vlf_farming.plant_lists[identifier].full_grown = full_grown
	vlf_farming.plant_lists[identifier].names = names
	vlf_farming.plant_lists[identifier].interval = interval
	vlf_farming.plant_lists[identifier].chance = chance
	plant_lists = vlf_farming.plant_lists --provide local copy of plant lists (performances)
	minetest.register_abm({
		...
		action = function(pos, node)
			local low_speed = minetest.get_node({ x = pos.x, y = pos.y - 1, z = pos.z }).name ~= "vlf_farming:soil_wet"
			vlf_farming:grow_plant(identifier, pos, node, false, false, low_speed)
		end,
	})
...
-- identifier: Identifier of plant as defined by vlf_farming:add_plant
-- pos: Position
-- node: Node table
...
function vlf_farming:grow_plant(identifier, pos, node, stages, ignore_light, low_speed)
...
function vlf_farming:place_seed(itemstack, placer, pointed_thing, plantname)
	...
	if string.find(farmland.name, "vlf_farming:soil") and string.find(place_s.name, "air") then
...
function vlf_farming:add_gourd(full_unconnected_stem, connected_stem_basename, stem_itemstring, stem_def, stem_drop, gourd_itemstring, gourd_def, grow_interval, grow_chance, connected_stem_texture, gourd_on_construct_extra)
	...
	if stem_def.sounds == nil then
		stem_def.sounds = vlf_sounds.node_sound_leaves_defaults()
	end
		...
		action = function(stempos)
			...
			if light and light > 10 then
				...
				for n = #neighbors, 1, -1 do
					...
					if not ((minetest.get_item_group(floor.name, "grass_block") == 1 or floor.name == "vlf_core:dirt" or soilgroup == 2 or soilgroup == 3) and block.name == "air") then
				...
				if #neighbors > 0 then
					...
					if minetest.get_item_group(floor.name, "dirtifies_below_solid") == 1 then
						minetest.set_node(floorpos, { name = "vlf_core:dirt" })
					end
				end
			end
		end,
...
function vlf_farming:stem_color(startcolor, endcolor, step, step_count)
	local color = {}
...
function vlf_farming:get_seed_or_eat_callback(plantname, hp_change)
	return function(itemstack, placer, pointed_thing)
		local new = vlf_farming:place_seed(itemstack, placer, pointed_thing, plantname)
		if new then
			return new
		else
