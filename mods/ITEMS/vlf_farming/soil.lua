...
minetest.register_node("vlf_farming:soil", {
	tiles = {"vlf_farming_farmland_dry.png", "default_dirt.png"},
	...
	groups = {handy=1, shovely=1, dirtifies_below_solid=1, dirtifier=1, soil=2, soil_sapling=1, soil_fungus=1, deco_block=1, _vlf_partial = 2 },
...
minetest.register_node("vlf_farming:soil_wet", {
	tiles = {"vlf_farming_farmland_wet.png", "default_dirt.png"},
	...
	groups = {handy=1,shovely=1, not_in_creative_inventory=1, dirtifies_below_solid=1, dirtifier=1, soil=3, soil_sapling=1, soil_fungus=1, _vlf_partial = 2, },
	sounds = vlf_sounds.node_sound_dirt_defaults(),
...
minetest.register_abm({
	...
	nodenames = {"vlf_farming:soil", "vlf_farming:soil_wet"},
	interval = 15,
	chance = 4,
	action = function(pos, node)
		...
		if check_surroundings(pos, "group:water") then
			if node.name ~= "vlf_farming:soil_wet" then
				-- Make it wet
				node.name = "vlf_farming:soil_wet"
				minetest.set_node(pos, node)
			end
			...
			if vlf_weather.rain.raining then
				if vlf_weather.is_outdoor(pos) then
					return
				end
			end
