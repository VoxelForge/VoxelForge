...
local function place_slab_normal(itemstack, placer, pointed_thing)
	...
	if placer
		and vlc_util.is_pointing_above_middle(placer, pointed_thing)
		and origdef and origdef._vlc_other_slab_half
	then
		place:set_name(origdef._vlc_other_slab_half)
	end
...
local function place_stair(itemstack, placer, pointed_thing)
	...
	if placer then
		...
		if vlc_util.is_pointing_above_middle(placer, pointed_thing) then
			param2 = param2 + 20
			if param2 == 21 then
				param2 = 23
...
local function register_stair(subname, stairdef)
	if stairdef.recipeitem and minetest.registered_items[stairdef.recipeitem] then
		...
		if not stairdef.hardness then
			stairdef.hardness = minetest.registered_items[stairdef.recipeitem]._vlc_hardness
		end
		if not stairdef.blast_resistance then
			stairdef.blast_resistance = minetest.registered_items[stairdef.recipeitem]._vlc_blast_resistance
		end
	end
	...
	minetest.register_node(":vlc_stairs:stair_" .. subname, table.merge({
	...
	vlc_stairs.cornerstair.add("vlc_stairs:stair_"..subname, stairdef.corner_stair_texture_override)
end

function vlc_stairs.get_base_itemstring(itemstring)
...
local function register_slab(subname, stairdef)
	local lower_slab = "vlc_stairs:slab_"..subname
	...
	local nodedef = table.merge({
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if (wield_item == unode.name or (udefs and wield_item == udefs._vlc_other_slab_half)) and
				...
				minetest.set_node(under, {name = def._vlc_stairs_double_slab or double_slab, param2 = p2})
				if not creative_enabled then
					itemstack:take_item()
				end
				return itemstack
			elseif (wield_item == anode.name or (adefs and wield_item == adefs._vlc_other_slab_half)) then
				minetest.set_node(above, {name = def._vlc_stairs_double_slab or double_slab, param2 = p2})
				if not creative_enabled then
					itemstack:take_item()
				end
				return itemstack
		...
		_vlc_hardness = stairdef.hardness,
		_vlc_blast_resistance = stairdef.blast_resistance,
		_vlc_other_slab_half = upper_slab,
		on_rotate = function(pos, node, user, mode, param2)
			-- Flip slab
			if mode == screwdriver.ROTATE_AXIS then
	...
	topdef._vlc_other_slab_half = lower_slab
...
function vlc_stairs.register_stair(subname, ...)
	if type(select(1, ...)) == "table" then
		...
		register_stair(subname, {
			...
			description = stairdef.description,
			sounds = ndef.sounds,
			blast_resistance = ndef._vlc_blast_resistance,
			hardness = ndef._vlc_hardness,
...
function vlc_stairs.register_slab(subname, ...)
	if type(select(1, ...)) == "table" then
		local stairdef = select(1, ...)
		local ndef = minetest.registered_nodes[stairdef.baseitem]
...
function vlc_stairs.register_stair_and_slab(subname, ...)
	if type(select(1, ...)) == "table" then
		local stairdef = select(1, ...)
		vlc_stairs.register_stair(subname, table.merge(stairdef, { description = stairdef.description_stair }))
		vlc_stairs.register_slab(subname, table.merge(stairdef, { description = stairdef.description_slab }))
		return
	end
...
function vlc_stairs.register_stair_and_slab_simple(subname, sourcenode, desc_stair, desc_slab, desc_double_slab, corner_stair_texture_override)
	...
	vlc_stairs.register_stair_and_slab(subname, sourcenode, groups, def.tiles, desc_stair, desc_slab, def.sounds, def._vlc_blast_resistance, def._vlc_hardness, desc_double_slab, corner_stair_texture_override)
end
