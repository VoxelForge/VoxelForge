...
local explosions_griefing = minetest.settings:get_bool("vlf_explosions_griefing", true)
...
function tnt.ignite(pos)
	...
	local e = spawn_tnt(pos, "vlf_tnt:tnt")
...
minetest.register_node("vlf_tnt:tnt", {
	tiles = {
		"default_tnt_top.png",
...
--[[local function add_effects(pos, radius, drops)
	...
	local texture = "vlf_particles_smoke.png" --fallback texture
	local most = 0
...
function TNT:on_step(dtime, _)
	...
	if self.timer > tnt.BOOMTIMER then
		vlf_explosions.explode(self.object:get_pos(), 4, {}, self.object)
		self.object:remove()
	end
end

minetest.register_entity("vlf_tnt:tnt", TNT)

if minetest.get_modpath("vlf_mobitems") then
	minetest.register_craft({
		output = "vlf_tnt:tnt",
		recipe = {
			{ "vlf_mobitems:gunpowder", "group:sand", "vlf_mobitems:gunpowder" },
			{ "group:sand", "vlf_mobitems:gunpowder", "group:sand" },
			{ "vlf_mobitems:gunpowder", "group:sand", "vlf_mobitems:gunpowder" }
		},
	})
end
...
if minetest.get_modpath("doc_identifier") then
	doc.sub.identifier.register_object("vlf_tnt:tnt", "nodes", "vlf_tnt:tnt")
end
