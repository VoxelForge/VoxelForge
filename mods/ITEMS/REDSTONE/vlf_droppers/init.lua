...
- One node for the horizontal-facing dropper (vlf_droppers:dropper)
- One node for the upwards-facing droppers (vlf_droppers:dropper_up)
- One node for the downwards-facing droppers (vlf_droppers:dropper_down)
...
local dropper_formspec = table.concat({
	...
	"label[4.125,0.375;" .. F(C(vlf_formspec.label_color, S("Dropper"))) .. "]",
...
local function orientate_dropper(pos, placer)
	...
	if pitch > 55 then
		minetest.swap_node(pos, { name = "vlf_droppers:dropper_up" })
	elseif pitch < -55 then
		minetest.swap_node(pos, { name = "vlf_droppers:dropper_down" })
	end
end
...
local dropperdef = {
	...
	mesecons = { effector = {
		...
		action_on = function(pos, node)
			...
			local inv = meta:get_inventory()
			local droppos
			if node.name == "vlf_droppers:dropper" then
				droppos = vector.subtract(pos, minetest.facedir_to_dir(node.param2))
			elseif node.name == "vlf_droppers:dropper_up" then
				droppos = vector.offset(pos, 0, 1, 0)
			elseif node.name == "vlf_droppers:dropper_down" then
				droppos = vector.offset(pos, 0, -1, 0)
			end
			...
			if #stacks >= 1 then
				...
				local dropped = vlf_util.move_item_container(pos, droppos, nil, stack_id)
				-- No container?
...
minetest.register_node("vlf_droppers:dropper", horizontal_def)
...
down_def.drop = "vlf_droppers:dropper"
minetest.register_node("vlf_droppers:dropper_down", down_def)
...
minetest.register_node("vlf_droppers:dropper_up", up_def)



-- Ladies and gentlemen, I present to you: the crafting recipe!
minetest.register_craft({
	output = "vlf_droppers:dropper",
	recipe = {
...
minetest.register_lbm({
	...
	name = "vlf_droppers:update_formspecs_0_60_0",
	nodenames = { "vlf_droppers:dropper", "vlf_droppers:dropper_down", "vlf_droppers:dropper_up" },
	action = function(pos, node)
		setup_dropper(pos)
		minetest.log("action", "[vlf_droppers] Node formspec updated at " .. minetest.pos_to_string(pos))
	end,
})
