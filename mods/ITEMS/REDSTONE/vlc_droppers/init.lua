...
- One node for the horizontal-facing dropper (vlc_droppers:dropper)
- One node for the upwards-facing droppers (vlc_droppers:dropper_up)
- One node for the downwards-facing droppers (vlc_droppers:dropper_down)
...
local dropper_formspec = table.concat({
	...
	"label[4.125,0.375;" .. F(C(vlc_formspec.label_color, S("Dropper"))) .. "]",
...
local function orientate_dropper(pos, placer)
	...
	if pitch > 55 then
		minetest.swap_node(pos, { name = "vlc_droppers:dropper_up" })
	elseif pitch < -55 then
		minetest.swap_node(pos, { name = "vlc_droppers:dropper_down" })
	end
end
...
local dropperdef = {
	...
	mesecons = { effector = {
		...
		action_on = function(pos, node)
			...
			local inv = meta:get_inventory()
			local droppos
			if node.name == "vlc_droppers:dropper" then
				droppos = vector.subtract(pos, minetest.facedir_to_dir(node.param2))
			elseif node.name == "vlc_droppers:dropper_up" then
				droppos = vector.offset(pos, 0, 1, 0)
			elseif node.name == "vlc_droppers:dropper_down" then
				droppos = vector.offset(pos, 0, -1, 0)
			end
			...
			if #stacks >= 1 then
				...
				local dropped = vlc_util.move_item_container(pos, droppos, nil, stack_id)
				-- No container?
...
horizontal_def.tiles = {
	...
	"default_furnace_side.png", "vlc_droppers_dropper_front_horizontal.png"
}
horizontal_def.paramtype2 = "facedir"
horizontal_def.groups = { pickaxey = 1, container = 2, material_stone = 1 }

minetest.register_node("vlc_droppers:dropper", horizontal_def)
...
down_def.tiles = {
	"default_furnace_top.png", "vlc_droppers_dropper_front_vertical.png",
	"default_furnace_side.png", "default_furnace_side.png",
	"default_furnace_side.png", "default_furnace_side.png"
}
...
down_def.drop = "vlc_droppers:dropper"
minetest.register_node("vlc_droppers:dropper_down", down_def)
...
up_def.tiles = {
	"vlc_droppers_dropper_front_vertical.png", "default_furnace_bottom.png",
	"default_furnace_side.png", "default_furnace_side.png",
	"default_furnace_side.png", "default_furnace_side.png"
}
minetest.register_node("vlc_droppers:dropper_up", up_def)



-- Ladies and gentlemen, I present to you: the crafting recipe!
minetest.register_craft({
	output = "vlc_droppers:dropper",
	recipe = {
		{ "vlc_core:cobble", "vlc_core:cobble", "vlc_core:cobble", },
		{ "vlc_core:cobble", "", "vlc_core:cobble", },
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlc_droppers:dropper", "nodes", "vlc_droppers:dropper_down")
	doc.add_entry_alias("nodes", "vlc_droppers:dropper", "nodes", "vlc_droppers:dropper_up")
end

-- Legacy
minetest.register_lbm({
	label = "Update dropper formspecs (0.60.0)",
	name = "vlc_droppers:update_formspecs_0_60_0",
	nodenames = { "vlc_droppers:dropper", "vlc_droppers:dropper_down", "vlc_droppers:dropper_up" },
	action = function(pos, node)
		setup_dropper(pos)
		minetest.log("action", "[vlc_droppers] Node formspec updated at " .. minetest.pos_to_string(pos))
	end,
})
