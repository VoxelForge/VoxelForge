...
vlf_pistons.registered_on_move = {}
...
function vlf_pistons.register_on_move(callback)
	table.insert(vlf_pistons.registered_on_move, callback)
end

local function run_on_vlf_piston_move(moved_nodes)
	for _, callback in ipairs(vlf_pistons.registered_on_move) do
...
function vlf_pistons.push(pos, movedir, maximum, player_name, piston_pos)
	...
	while #frontiers > 0 do
		...
		if minetest.get_item_group(nn.name, "dig_by_piston") == 1 then
			...
			if not def.buildable_to then
				...
				if def._vlf_pistons_sticky then
					...
					for _, dir in pairs(sixdirs) do
						...
						is_connected = def._vlf_pistons_sticky(nn, offset_node, dir)
	...
	for id, n in ipairs(nodes) do
		...
		if string.find(n.node.name, "vlf_observers:observer") then
			...
			vlf_observers.observer_activate(n.pos)
		end
	end
	...
	local function move_object(obj, n, is_pulled)
		...
		if (entity or player) and not (entity and minetest.registered_entities[entity.name]._vlf_pistons_unmovable) then
			...
			if n.node.name == "vlf_core:slimeblock" and not is_pulled then
	...
	run_on_vlf_piston_move(nodes)

	return true
end

vlf_pistons.register_on_move(function(moved_nodes)
	for i = 1, #moved_nodes do
		...
		if node_def and node_def._vlf_piston_on_move then
			node_def._vlf_piston_on_move(moved_node)
		end
	end
end)
