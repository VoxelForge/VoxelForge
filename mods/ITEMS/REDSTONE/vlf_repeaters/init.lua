...
local function check_locked(pos, node)
	...
	if is_repeater(right_node) and vlf_redstone.get_power(pos, right) > 0 then
		return true
	end
	if is_repeater(left_node) and vlf_redstone.get_power(pos, left) > 0 then
		return true
	end
	return false
end
...
local commdef = {
	...
	drop = "vlf_repeaters:repeater_off_1",
	...
	_vlf_redstone = {
		...
		update = function(pos, node)
			...
			local on = vlf_redstone.get_power(pos, -minetest.fourdir_to_dir(node.param2)) ~= 0
			...
			if on and locked and not was_locked then
				return {
					delay = delay,
					priority = 1,
					name = "vlf_repeaters:repeater_on_locked",
			...
			if not on and locked and not was_locked then
				return {
					delay = delay,
					priority = 1,
					name = "vlf_repeaters:repeater_off_locked",
					param2 = delay * 4 + node.param2 % 4,
				}
			end
			if on and not locked then
				return {
					delay = delay,
					priority = 1,
					name = "vlf_repeaters:repeater_on_"..delay,
			...
			if not on and not locked then
				return {
					delay = delay,
					name = "vlf_repeaters:repeater_off_"..delay,
					param2 = node.param2 % 4,
				}
			end
		end,
...
for delay = 1, 4 do
	local normaldef = table.merge(commdef, {
		...
		on_rightclick = function(pos, node, clicker)
			...
			local powered = ndef._vlf_redstone and ndef._vlf_redstone.get_power and "on" or "off"

			minetest.set_node(pos, {
				name = "vlf_repeaters:repeater_"..powered.."_"..tostring(next_setting),
				param2 = node.param2
			})
		end,
	})

	minetest.register_node("vlf_repeaters:repeater_off_"..delay, table.merge(normaldef, {
	...
	minetest.register_node("vlf_repeaters:repeater_on_"..delay, table.merge(normaldef, {
		...
		_vlf_redstone = table.merge(normaldef._vlf_redstone, {
	...
	if delay ~= 1 then
		doc.add_entry_alias("nodes", "vlf_repeaters:repeater_off_1", "nodes", "vlf_repeaters:repeater_off_"..delay)
	end
	doc.add_entry_alias("nodes", "vlf_repeaters:repeater_off_1", "nodes", "vlf_repeaters:repeater_on_"..delay)
end
...
minetest.register_node("vlf_repeaters:repeater_off_locked", table.merge(lockeddef, {
...
minetest.register_node("vlf_repeaters:repeater_on_locked", table.merge(lockeddef, {
	...
	_vlf_redstone = table.merge(lockeddef._vlf_redstone, {
		get_power = function(node, dir)
			local fourdir = minetest.dir_to_fourdir(dir)
			if not fourdir or dir.y ~= 0 then
				return 0
...
minetest.register_craft({
	output = "vlf_repeaters:repeater_off_1",
	recipe = {
		{ "",      "", ""      },
		{ "vlf_redstone_torch:redstone_torch_on", "vlf_redstone:redstone",  "vlf_redstone_torch:redstone_torch_on" },
		{ "vlf_core:stone",   "vlf_core:stone",   "vlf_core:stone"   },
	}
})

doc.add_entry_alias("nodes", "vlf_repeaters:repeater_off_1", "nodes", "vlf_repeaters:repeater_off_locked")
doc.add_entry_alias("nodes", "vlf_repeaters:repeater_off_1", "nodes", "vlf_repeaters:repeater_on_locked")
