...
- One node for the horizontal-facing dispensers (vlc_dispensers:dispenser)
- One node for the upwards-facing dispensers (vlc_dispenser:dispenser_up)
- One node for the downwards-facing dispensers (vlc_dispenser:dispenser_down)
...
local dispenser_formspec = table.concat({
	...
	"label[4.125,0.375;" .. F(C(vlc_formspec.label_color, S("Dispenser"))) .. "]",

	vlc_formspec.get_itemslot_bg_v4(4.125, 0.75, 3, 3),
...
local function orientate_dispenser(pos, placer)
	...
	if pitch > 55 then
		minetest.swap_node(pos, { name = "vlc_dispensers:dispenser_up", param2 = node.param2 })
	elseif pitch < -55 then
		minetest.swap_node(pos, { name = "vlc_dispensers:dispenser_down", param2 = node.param2 })
	end
end
					...
					if igroups.armor then
						...
						for _, objs in ipairs({ minetest.get_objects_inside_radius(droppos, 1),
							...
							for _, obj in ipairs(objs) do
								stack = vlc_armor.equip(stack, obj)
						...
						if not stack:is_empty() then
							if igroups.head or iname == "vlc_farming:pumpkin_face" then
...
horizontal_def.tiles = {
	...
	"default_furnace_side.png", "vlc_dispensers_dispenser_front_horizontal.png"
}
horizontal_def.paramtype2 = "facedir"
horizontal_def.groups = { pickaxey = 1, container = 2, material_stone = 1 }

minetest.register_node("vlc_dispensers:dispenser", horizontal_def)
...
down_def.tiles = {
	"default_furnace_top.png", "vlc_dispensers_dispenser_front_vertical.png",
	"default_furnace_side.png", "default_furnace_side.png",
	"default_furnace_side.png", "default_furnace_side.png"
}
...
down_def.drop = "vlc_dispensers:dispenser"
minetest.register_node("vlc_dispensers:dispenser_down", down_def)
...
up_def.tiles = {
	"vlc_dispensers_dispenser_front_vertical.png", "default_furnace_bottom.png",
	"default_furnace_side.png", "default_furnace_side.png",
	"default_furnace_side.png", "default_furnace_side.png"
}
minetest.register_node("vlc_dispensers:dispenser_up", up_def)


minetest.register_craft({
	output = "vlc_dispensers:dispenser",
	recipe = {
		{ "vlc_core:cobble", "vlc_core:cobble", "vlc_core:cobble", },
		{ "vlc_core:cobble", "vlc_bows:bow", "vlc_core:cobble", },
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlc_dispensers:dispenser", "nodes", "vlc_dispensers:dispenser_down")
	doc.add_entry_alias("nodes", "vlc_dispensers:dispenser", "nodes", "vlc_dispensers:dispenser_up")
end

-- Legacy
minetest.register_lbm({
	label = "Update dispenser formspecs (0.60.0)",
	name = "vlc_dispensers:update_formspecs_0_60_0",
	nodenames = { "vlc_dispensers:dispenser", "vlc_dispensers:dispenser_down", "vlc_dispensers:dispenser_up" },
	action = function(pos, node)
		setup_dispenser(pos)
		minetest.log("action", "[vlc_dispenser] Node formspec updated at " .. minetest.pos_to_string(pos))
	end,
})
