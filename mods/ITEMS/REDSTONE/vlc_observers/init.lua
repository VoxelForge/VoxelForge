...
local function observer_scan(pos, initialize)
	...
	local front
	if node.name == "vlc_observers:observer_up_off" or node.name == "vlc_observers:observer_up_on" then
		front = vector.add(pos, {x=0, y=1, z=0})
	elseif node.name == "vlc_observers:observer_down_off" or node.name == "vlc_observers:observer_down_on" then
		front = vector.add(pos, {x=0, y=-1, z=0})
	else
	...
	if oldnode ~= "" and not initialize then
		if not (frontnode.name == oldnode and tostring(frontnode.param2) == oldparam2) then
			...
			if node.name == "vlc_observers:observer_off" then
				minetest.swap_node(pos, {name = "vlc_observers:observer_on", param2 = node.param2})
				mesecon.receptor_on(pos, get_rules_flat(node))
			elseif node.name == "vlc_observers:observer_down_off" then
				minetest.swap_node(pos, {name = "vlc_observers:observer_down_on"})
				mesecon.receptor_on(pos, rules_down)
			elseif node.name == "vlc_observers:observer_up_off" then
				minetest.swap_node(pos, {name = "vlc_observers:observer_up_on"})
...
function vlc_observers.observer_activate(pos)
	minetest.after(vlc_vars.redstone_tick, function(pos)
		local node = minetest.get_node(pos)
		if not node then
			return
		end
		...
		if nn == "vlc_observers:observer_off" then
			minetest.swap_node(pos, {name = "vlc_observers:observer_on", param2 = node.param2})
			mesecon.receptor_on(pos, get_rules_flat(node))
			start_timer = true
		elseif nn == "vlc_observers:observer_down_off" then
			minetest.swap_node(pos, {name = "vlc_observers:observer_down_on"})
			mesecon.receptor_on(pos, rules_down)
			start_timer = true
		elseif nn == "vlc_observers:observer_up_off" then
			minetest.swap_node(pos, {name = "vlc_observers:observer_up_on"})
			mesecon.receptor_on(pos, rules_up)
			start_timer = true
		end
		...
		if start_timer then
			...
			if not timer:is_started() then
				...
				timer:start(vlc_vars.redstone_tick)
			end
		end
...
local function observer_orientate(pos, placer)
	...
	if pitch > 55 then -- player looking upwards
		...
		minetest.set_node(pos, {name="vlc_observers:observer_down_off"})
	elseif pitch < -55 then -- player looking downwards
		-- Observer looking upwards
		minetest.set_node(pos, {name="vlc_observers:observer_up_off"})
	end
end

mesecon.register_node("vlc_observers:observer", {
		...
		tiles = {
			"vlc_observers_observer_top.png^[transformR180", "default_furnace_bottom.png",
			"vlc_observers_observer_side.png", "vlc_observers_observer_side.png",
			"vlc_observers_observer_front.png", "vlc_observers_observer_back.png",
		},
		mesecons = {
			receptor = {
		...
		tiles = {
			"vlc_observers_observer_top.png^[transformR180", "default_furnace_bottom.png",
			"vlc_observers_observer_side.png", "vlc_observers_observer_side.png",
			"vlc_observers_observer_front.png", "vlc_observers_observer_back_lit.png",
		},
		mesecons = {
			receptor = {
		...
		on_timer = function(pos, elapsed)
			local node = minetest.get_node(pos)
			minetest.swap_node(pos, {name = "vlc_observers:observer_off", param2 = node.param2})
...
mesecon.register_node("vlc_observers:observer_down", {
		is_ground_content = false,
		sounds = vlc_sounds.node_sound_stone_defaults(),
		groups = {pickaxey=1, material_stone=1, not_opaque=1, not_in_creative_inventory=1 },
		on_rotate = false,
		_vlc_blast_resistance = 3.5,
		_vlc_hardness = 3.5,
		drop = "vlc_observers:observer_off",
	}, {
		tiles = {
			"vlc_observers_observer_back.png", "vlc_observers_observer_front.png",
			"vlc_observers_observer_side.png^[transformR90", "vlc_observers_observer_side.png^[transformR90",
			"vlc_observers_observer_top.png", "vlc_observers_observer_top.png",
		},
		mesecons = {
			receptor = {
				state = mesecon.state.off,
		...
		_doc_items_create_entry = false,
		tiles = {
			"vlc_observers_observer_back_lit.png", "vlc_observers_observer_front.png",
			"vlc_observers_observer_side.png^[transformR90", "vlc_observers_observer_side.png^[transformR90",
			"vlc_observers_observer_top.png", "vlc_observers_observer_top.png",
		},
		mesecons = {
		...
		on_timer = function(pos, elapsed)
			local node = minetest.get_node(pos)
			minetest.swap_node(pos, {name = "vlc_observers:observer_down_off", param2 = node.param2})
...
mesecon.register_node("vlc_observers:observer_up", {
		is_ground_content = false,
		sounds = vlc_sounds.node_sound_stone_defaults(),
		groups = {pickaxey=1, material_stone=1, not_opaque=1, not_in_creative_inventory=1 },
		on_rotate = false,
		...
		tiles = {
			"vlc_observers_observer_front.png", "vlc_observers_observer_back.png",
			"vlc_observers_observer_side.png^[transformR270", "vlc_observers_observer_side.png^[transformR270",
			"vlc_observers_observer_top.png^[transformR180", "vlc_observers_observer_top.png^[transformR180",
		},
		mesecons = {
			receptor = {
				state = mesecon.state.off,
		...
		on_timer = function(pos, elapsed)
			minetest.swap_node(pos, {name = "vlc_observers:observer_up_off"})
...
minetest.register_craft({
	output = "vlc_observers:observer_off",
	recipe = {
		{ "vlc_core:cobble", "vlc_core:cobble", "vlc_core:cobble" },
		{ "vlc_nether:quartz", "mesecons:redstone", "mesecons:redstone" },
		{ "vlc_core:cobble", "vlc_core:cobble", "vlc_core:cobble" },
	},
})
minetest.register_craft({
	...
	recipe = {
		{ "vlc_core:cobble", "vlc_core:cobble", "vlc_core:cobble" },
		{ "mesecons:redstone", "mesecons:redstone", "vlc_nether:quartz" },
		{ "vlc_core:cobble", "vlc_core:cobble", "vlc_core:cobble" },
	},
})
...
minetest.register_lbm({
	name = "vlc_observers:activate_lbm",
	nodenames = {
		"vlc_observers:observer_off",
		"vlc_observers:observer_down_off",
		"vlc_observers:observer_up_off",
		"vlc_observers:observer_on",
		"vlc_observers:observer_down_on",
		"vlc_observers:observer_up_on",
	},
	run_at_every_load = true,
	action = function(pos)
		minetest.after(1, vlc_observers.observer_activate, {x=pos.x, y=pos.y, z=pos.z})
	end,
})
