...
local function commandblock_action_on(pos, node)
	if node.name ~= "vlf_commandblock:commandblock_off" then
		return
	end
	...
	minetest.swap_node(pos, {name = "vlf_commandblock:commandblock_on"})
...
local function commandblock_action_off(pos, node)
	if node.name == "vlf_commandblock:commandblock_on" then
		minetest.swap_node(pos, {name = "vlf_commandblock:commandblock_off"})
	end
end
...
minetest.register_node("vlf_commandblock:commandblock_off", table.merge(commdef, {
...
minetest.register_node("vlf_commandblock:commandblock_on", table.merge(commdef, {
	tiles = {{name="jeija_commandblock_off.png", animation={type="vertical_frames", aspect_w=32, aspect_h=32, length=2}}},
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if string.sub(formname, 1, 13) == "commandblock_" then
		if fields.doc and minetest.get_modpath("doc") then
			doc.show_entry(player:get_player_name(), "nodes", "vlf_commandblock:commandblock_off", true)
			return
		end
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlf_commandblock:commandblock_off", "nodes", "vlf_commandblock:commandblock_on")
end
