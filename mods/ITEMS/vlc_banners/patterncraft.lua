...
local patterns = {
	...
	["thing"] = {
		...
		type = "shapeless",
		{ e, "vlc_banners:pattern_thing", d },
	},
	["globe"] = {
		name = NS("@1 Globe Charge"),
		type = "shapeless",
		{ e, "vlc_banners:pattern_globe", d },
	},
	["piglin"] = {
		name = NS("@1 Piglin Charge"),
		type = "shapeless",
		{ e, "vlc_banners:pattern_piglin", d },
	},
	["rhombus"] = {
		name = NS("@1 Lozenge"),
		{ e, d, e },
...
function vlc_banners.register_pattern(name,recipe)
	vlc_banners.patterns[name] = table.merge({ name = readable_name(name) }, recipe)
end
...
function vlc_banners.make_advanced_banner_description(description, layers)
	if layers == nil or #layers == 0 then
		...
		for l=1, #layers do
			...
			-- Layer text line.
			local color = vlc_banners.colors[layers[l].color][6]
		...
		description = description .. "\n" .. minetest.colorize(vlc_colors.GRAY, append)
		return description
	end
end
...
local function banner_pattern_craft(itemstack, player, old_craft_grid, craft_inv, craft_predict)
	...
	if banner2 then
		...
		imeta:set_string("description", vlc_banners.make_advanced_banner_description(src_desc, src_layers))
	...
	if mname == "" then
		...
		local description = vlc_banners.make_advanced_banner_description(odesc, layers)
	...
	if craft_predict then
		local itemid_prefix = "vlc_banners:banner_preview"
...
for pattern_name, pattern in pairs(patterns) do
	...
	if pattern.type == nil then
		for colorid, colortab in pairs(vlc_banners.colors) do
			local banner = "vlc_banners:banner_item_"..colortab[1]
