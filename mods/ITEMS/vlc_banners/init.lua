...
local node_sounds
if minetest.get_modpath("vlc_sounds") then
	node_sounds = vlc_sounds.node_sound_wood_defaults()
end
...
vlc_banners.colors = {
	...
	["unicolor_white"] =      {"white",      S("White Banner"),      "vlc_wool:white", vlc_dyes.colors.white.rgb, "vlc_dyes:white", S("White") },
	["unicolor_darkgrey"] =   {"grey",       S("Grey Banner"),       "vlc_wool:grey", vlc_dyes.colors.grey.rgb, "vlc_dyes:dark_grey", S("Grey") },
	["unicolor_grey"] =       {"silver",     S("Light Grey Banner"), "vlc_wool:silver", vlc_dyes.colors.silver.rgb, "vlc_dyes:grey", S("Light Grey") },
	["unicolor_black"] =      {"black",      S("Black Banner"),      "vlc_wool:black", vlc_dyes.colors.black.rgb, "vlc_dyes:black", S("Black") },
	["unicolor_red"] =        {"red",        S("Red Banner"),        "vlc_wool:red", vlc_dyes.colors.red.rgb, "vlc_dyes:red", S("Red") },
	["unicolor_yellow"] =     {"yellow",     S("Yellow Banner"),     "vlc_wool:yellow", vlc_dyes.colors.yellow.rgb, "vlc_dyes:yellow", S("Yellow") },
	["unicolor_dark_green"] = {"green",      S("Green Banner"),      "vlc_wool:green", vlc_dyes.colors.green.rgb, "vlc_dyes:dark_green", S("Green") },
	["unicolor_cyan"] =       {"cyan",       S("Cyan Banner"),       "vlc_wool:cyan", vlc_dyes.colors.cyan.rgb, "vlc_dyes:cyan", S("Cyan") },
	["unicolor_blue"] =       {"blue",       S("Blue Banner"),       "vlc_wool:blue", vlc_dyes.colors.blue.rgb, "vlc_dyes:blue", S("Blue") },
	["unicolor_red_violet"] = {"magenta",    S("Magenta Banner"),    "vlc_wool:magenta", vlc_dyes.colors.magenta.rgb, "vlc_dyes:magenta", S("Magenta")},
	["unicolor_orange"] =     {"orange",     S("Orange Banner"),     "vlc_wool:orange", vlc_dyes.colors.orange.rgb, "vlc_dyes:orange", S("Orange") },
	["unicolor_violet"] =     {"purple",     S("Purple Banner"),     "vlc_wool:purple", vlc_dyes.colors.purple.rgb, "vlc_dyes:violet", S("Violet") },
	["unicolor_brown"] =      {"brown",      S("Brown Banner"),      "vlc_wool:brown", vlc_dyes.colors.brown.rgb, "vlc_dyes:brown", S("Brown") },
	["unicolor_dark_orange"] ={"brown",      S("Brown Banner"),      "vlc_wool:brown", vlc_dyes.colors.brown.rgb, "vlc_dyes:brown", S("Brown") },
	["unicolor_pink"] =       {"pink",       S("Pink Banner"),       "vlc_wool:pink", vlc_dyes.colors.pink.rgb, "vlc_dyes:pink", S("Pink") },
	["unicolor_light_red"] =  {"pink",       S("Pink Banner"),       "vlc_wool:pink", vlc_dyes.colors.pink.rgb, "vlc_dyes:pink", S("Pink") },
	["unicolor_lime"] =       {"lime",       S("Lime Banner"),       "vlc_wool:lime", vlc_dyes.colors.lime.rgb, "vlc_dyes:green", S("Lime") },
	["unicolor_green"] =      {"lime",       S("Lime Banner"),       "vlc_wool:lime", vlc_dyes.colors.lime.rgb, "vlc_dyes:green", S("Lime") },
	--the duplicate lines for brown/dark_orange, lime/green and pink/light_red are needed because vlc_banners previously used the wrong unicolor color names
	["unicolor_light_blue"] = {"light_blue", S("Light Blue Banner"), "vlc_wool:light_blue", vlc_dyes.colors.light_blue.rgb, "vlc_dyes:lightblue", S("Light Blue") },
}
...
for k,v in pairs(vlc_banners.colors) do
	colors_reverse["vlc_banners:banner_item_"..v[1]] = k
end

function vlc_banners.color_reverse(itemname)
...
local function on_dig_banner(pos, node, digger)
	...
	local item_str = item:is_empty() and "vlc_banners:banner_item_white"
		or item:to_string()
...
local function on_destruct_banner(pos, hanging)
	...
	if hanging then
		...
		nodename = "vlc_banners:hanging_banner"
	else
		offset = standing_banner_entity_offset
		nodename = "vlc_banners:standing_banner"
	end
...
function vlc_banners.make_banner_texture(base_color, layers)
	local colorize
	if vlc_banners.colors[base_color] then
		colorize = vlc_banners.colors[base_color][4]
	end
	if colorize then
		-- Base texture with base color
		local base = "(vlc_banners_banner_base.png^[mask:vlc_banners_base_inverted.png)^((vlc_banners_banner_base.png^[colorize:"..colorize..":"..base_color_ratio..")^[mask:vlc_banners_base.png)"
		...
		if layers then
			...
			for l=1, #layers do
				...
				if layerinfo and layerinfo.pattern and layerinfo.color and vlc_banners.colors[layerinfo.color] then
					local pattern = "vlc_banners_" .. layerinfo.pattern .. ".png"
					local color = vlc_banners.colors[layerinfo.color][4]
		...
		return "vlc_banners_banner_base.png"
	end
end
...
local function spawn_banner_entity(pos, hanging, itemstack)
	local banner
	if hanging then
		banner = minetest.add_entity(pos, "vlc_banners:hanging_banner")
	else
		banner = minetest.add_entity(pos, "vlc_banners:standing_banner")
	end
	if banner == nil then
		return banner
	end
	...
	local colorid = vlc_banners.color_reverse(itemstack:get_name())
...
local function respawn_banner_entity(pos, node, force)
	local hanging = node.name == "vlc_banners:hanging_banner"
	local offset
	if hanging then
		offset = hanging_banner_entity_offset
	else
	...
	for _, v in ipairs(objects) do
		local ent = v:get_luaentity()
		if ent and (ent.name == "vlc_banners:standing_banner" or ent.name == "vlc_banners:hanging_banner") then
			if force then
				v:remove()
			else
				return
...
minetest.register_node("vlc_banners:standing_banner", {
	_doc_items_entry_name = S("Banner"),
	_doc_items_image = "vlc_banners_item_base.png^vlc_banners_item_overlay.png",
...
S("You can copy the pattern of a banner by placing two banners of the same color in the crafting grid—one needs to be emblazoned, the other one must be clean. Finally, you can use a banner on a cauldron with water to wash off its top-most layer."),
	...
	tiles = { "vlc_banners_fallback_wood.png" },

	inventory_image = "vlc_banners_item_base.png",
	wield_image = "vlc_banners_item_base.png",
...
-- Hanging banner node
minetest.register_node("vlc_banners:hanging_banner", {
	walkable = false,
	is_ground_content = false,
	paramtype = "light",
...
for colorid, colortab in pairs(vlc_banners.colors) do
    for i, pattern_name in ipairs(pattern_names) do
	local itemid = colortab[1]
	local desc = S("@1 Banner", vlc_dyes.colors[itemid].readable_name)
	local wool = colortab[3]
	local colorize = colortab[4]

	local itemstring
	if pattern_name == "" then
		itemstring = "vlc_banners:banner_item_" .. itemid
	else
		itemstring = "vlc_banners:banner_preview" .. "_" .. pattern_name .. "_" .. itemid
	end
	...
	if pattern_name == "" then
	    if colorize then
		-- Base texture with base color
		base = "vlc_banners_item_base.png^(vlc_banners_item_overlay.png^[colorize:"..colorize..")^[resize:32x32"
	    else
		base = "vlc_banners_item_base.png^vlc_banners_item_overlay.png^[resize:32x32"
	    end
	    finished_banner = base
	else
		-- Banner item preview background
		base = "vlc_banners_item_base.png^(vlc_banners_item_overlay.png^[colorize:#CCCCCC)^[resize:32x32"

		desc = S("Preview Banner")

		local pattern = "vlc_banners_" .. pattern_name .. ".png"
	...
	minetest.register_craftitem(itemstring, {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if placer and not placer:get_player_control().sneak then
				...
				if minetest.get_modpath("vlc_cauldrons") then
					-- Use banner on cauldron to remove the top-most layer. This reduces the water level by 1.
					local new_node
					if node_under.name == "vlc_cauldrons:cauldron_3" then
						new_node = "vlc_cauldrons:cauldron_2"
					elseif node_under.name == "vlc_cauldrons:cauldron_2" then
						new_node = "vlc_cauldrons:cauldron_1"
					elseif node_under.name == "vlc_cauldrons:cauldron_1" then
						new_node = "vlc_cauldrons:cauldron"
					elseif node_under.name == "vlc_cauldrons:cauldron_3r" then
						new_node = "vlc_cauldrons:cauldron_2r"
					elseif node_under.name == "vlc_cauldrons:cauldron_2r" then
						new_node = "vlc_cauldrons:cauldron_1r"
					elseif node_under.name == "vlc_cauldrons:cauldron_1r" then
						new_node = "vlc_cauldrons:cauldron"
					end
					if new_node then
						local imeta = itemstack:get_meta()
						...
						if type(layers) == "table" and #layers > 0 then
							...
							local newdesc = vlc_banners.make_advanced_banner_description(itemstack:get_definition().description, layers)
							local mname = imeta:get_string("name")
						...
						-- Play sound (from vlc_potions mod)
			...
			if stackmeta:get_string("name"):find("Ominous Banner") then
				...
				local description = vlc_banners.make_advanced_banner_description(banner_description, oban_layers)
			...
			local _, success = minetest.item_place_node(ItemStack("vlc_banners:standing_banner"), placer, pointed_thing)
			if not success then
				-- Forbidden on ceiling
				if pointed_thing.under.y ~= pointed_thing.above.y then
					return itemstack
				end
				_, success = minetest.item_place_node(ItemStack("vlc_banners:hanging_banner"), placer, pointed_thing)
				if not success then
					return itemstack
				end
				hanging = true
			...
			if bnode.name ~= "vlc_banners:standing_banner" and bnode.name ~= "vlc_banners:hanging_banner" then
				minetest.log("error", "[vlc_banners] The placed banner node is not what the mod expected!")
				return itemstack
			end
		...
		_vlc_generate_description = function(itemstack)
			...
			local newdesc = vlc_banners.make_advanced_banner_description(desc, layers)
			meta:set_string("description", newdesc)
			return newdesc
		end,
	})

	if mod_vlc_core and minetest.get_modpath("vlc_wool") and pattern_name == "" then
	...
	if mod_doc then
		...
		doc.add_entry_alias("nodes", "vlc_banners:standing_banner", "craftitems", itemstring)
	end
    end
end
...
if mod_doc then
	-- Add item to node alias
	doc.add_entry_alias("nodes", "vlc_banners:standing_banner", "nodes", "vlc_banners:hanging_banner")
end
...
local entity_standing = {
	initial_properties = {
		...
		textures = {vlc_banners.make_banner_texture()},
		pointable = false,
	},
	...
	on_activate = function(self, staticdata)
		if staticdata and staticdata ~= "" then
			...
			self.object:set_properties({
				textures = {vlc_banners.make_banner_texture(self._base_color, self._layers)},
			})
		end
	...
	-- * other parameters: Same meaning as in vlc_banners.make_banner_texture
	_set_textures = function(self, base_color, layers)
		...
		self.object:set_properties({textures = {vlc_banners.make_banner_texture(self._base_color, self._layers)}})
	end,
}
minetest.register_entity("vlc_banners:standing_banner", entity_standing)

local entity_hanging = table.copy(entity_standing)
entity_hanging.mesh = "amc_banner_hanging.b3d"
minetest.register_entity("vlc_banners:hanging_banner", entity_hanging)

-- FIXME: Prevent entity destruction by /clearobjects
minetest.register_lbm({
	label = "Respawn banner entities",
	name = "vlc_banners:respawn_entities",
	run_at_every_load = true,
	nodenames = {"vlc_banners:standing_banner", "vlc_banners:hanging_banner"},
