...
local function potions_set_hudbar(player)
	if EF.withering[player] and EF.regenerating[player] then
		hb.change_hudbar(player, "health", nil, nil, "vlc_potions_icon_regen_wither.png", nil, "hudbars_bar_health.png")
	elseif EF.withering[player] then
		hb.change_hudbar(player, "health", nil, nil, "vlc_potions_icon_wither.png", nil, "hudbars_bar_health.png")
...
local function potions_set_icons(player)
	...
	for i=1, EFFECT_TYPES do
		...
		if effect_name == nil then
			...
			player:hud_change(icon, "text", "vlc_potions_effect_"..effect_name..".png^[resize:128x128")
		end
	end
...
minetest.register_globalstep(function(dtime)
	...
	for player, vals in pairs(EF.invisible) do
		...
		if player:get_pos() then vlc_potions._add_spawner(player, "#7F8392") end

		if EF.invisible[player].timer >= EF.invisible[player].dur then
			vlc_potions.make_invisible(player, false)
	...
	for player, vals in pairs(EF.withering) do
		...
		if player:get_pos() then vlc_potions._add_spawner(player, "#000000") end

		if EF.withering[player].hit_timer >= EF.withering[player].step then
			if is_player or entity then vlc_util.deal_damage(player, 1, {type = "magic"}) end
	...
	for player, vals in pairs(EF.poisoned) do
		...
		if player:get_pos() then vlc_potions._add_spawner(player, "#4E9331") end

		if EF.poisoned[player].hit_timer >= EF.poisoned[player].step then
			if vlc_util.get_hp(player) - 1 > 0 then
				vlc_util.deal_damage(player, 1, {type = "magic"})
			end
	...
	for player, vals in pairs(EF.regenerating) do
		...
		if player:get_pos() then vlc_potions._add_spawner(player, "#CD5CAB") end
	...
	for player, vals in pairs(EF.water_breathing) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlc_potions._add_spawner(player, "#2E5299") end
	...
	for player, vals in pairs(EF.leaping) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlc_potions._add_spawner(player, "#22FF4C") end

			if EF.leaping[player].timer >= EF.leaping[player].dur then
				playerphysics.remove_physics_factor(player, "jump", "vlc_potions:leaping")
	...
	for player, vals in pairs(EF.swift) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlc_potions._add_spawner(player, "#7CAFC6") end

			if EF.swift[player].timer >= EF.swift[player].dur then
				playerphysics.remove_physics_factor(player, "speed", "vlc_potions:swiftness")
				EF.swift[player] = nil
				meta = player:get_meta()
	...
	for player, vals in pairs(EF.night_vision) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlc_potions._add_spawner(player, "#1F1FA1") end
			...
			vlc_weather.skycolor.update_sky_color({player})
			potions_set_hud(player)
	...
	for player, vals in pairs(EF.fire_proof) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlc_potions._add_spawner(player, "#E49A3A") end
	...
	for player, vals in pairs(EF.weak) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlc_potions._add_spawner(player, "#484D48") end
	...
	for player, vals in pairs(EF.strong) do
		...
		if player:is_player() then
			...
			if player:get_pos() then vlc_potions._add_spawner(player, "#932423") end
	...
	for player, vals in pairs(EF.bad_omen) do
		...
		if player:get_pos() then vlc_potions._add_spawner(player, "#0b6138") end
...
vlc_damage.register_modifier(function(obj, damage, reason)
...
function vlc_potions._clear_cached_player_data(player)
	EF.invisible[player] = nil
	EF.poisoned[player] = nil
	EF.regenerating[player] = nil
	EF.strong[player] = nil
...
function vlc_potions._reset_player_effects(player, set_hud)
	...
	vlc_potions._clear_cached_player_data(player)
...
function vlc_potions._save_player_effects(player)
...
function vlc_potions._load_player_effects(player)
	...
	if minetest.deserialize(meta:get_string("_is_invisible")) then
		...
		vlc_potions.make_invisible(player, true)
	end
...
function vlc_potions.player_has_effect(player, effect_name)
...
function vlc_potions.player_get_effect(player, effect_name)
...
function vlc_potions.player_clear_effect(player,effect)
	EF[effect][player] = nil
	potions_set_icons(player)
end

minetest.register_on_leaveplayer( function(player)
	vlc_potions._save_player_effects(player)
	vlc_potions._clear_cached_player_data(player) -- clearout the buffer to prevent looking for a player not there
...
minetest.register_on_joinplayer( function(player)
	vlc_potions._reset_player_effects(player, false) -- make sure there are no wierd holdover effects
	vlc_potions._load_player_effects(player)
...
function vlc_potions.is_obj_hit(self, pos)
...
function vlc_potions.make_invisible(obj_ref, hide)
	if obj_ref:is_player() then
		if hide then
			vlc_player.player_set_visibility(obj_ref, false)
			obj_ref:set_nametag_attributes({ color = { a = 0 } })
		else
			vlc_player.player_set_visibility(obj_ref, true)
...
function vlc_potions._use_potion(item, obj, color)
	local d = 0.1
	local pos = obj:get_pos()
	minetest.sound_play("vlc_potions_drinking", {pos = pos, max_hear_distance = 6, gain = 1})
	minetest.add_particlespawner({
		...
		vertical = false,
		texture = "vlc_particles_effect.png^[colorize:"..color..":127",
	})
end


function vlc_potions._add_spawner(obj, color)
	local d = 0.2
	local pos = obj:get_pos()
	minetest.add_particlespawner({
		amount = 1,
...
function vlc_potions.healing_func(player, hp)
	...
	elseif hp < 0 then
		...
		vlc_util.deal_damage(player, -hp, {type = "magic"})
	end

end

function vlc_potions.swiftness_func(player, factor, duration)
	...
	if not EF.swift[player] then
		...
		playerphysics.add_physics_factor(player, "speed", "vlc_potions:swiftness", factor)
...
function vlc_potions.leaping_func(player, factor, duration)
	...
	if not EF.leaping[player] then
		...
		playerphysics.add_physics_factor(player, "jump", "vlc_potions:leaping", factor)
...
function vlc_potions.weakness_func(player, factor, duration)
...
function vlc_potions.strength_func(player, factor, duration)
...
function vlc_potions.withering_func(player, factor, duration)
...
function vlc_potions.poison_func(player, factor, duration)
...
function vlc_potions.regeneration_func(player, factor, duration)
...
function vlc_potions.invisiblility_func(player, null, duration)
...
function vlc_potions.water_breathing_func(player, null, duration)
...
function vlc_potions.fire_resistance_func(player, null, duration)
...
function vlc_potions.night_vision_func(player, null, duration)
...
function vlc_potions._extinguish_nearby_fire(pos, radius)
	...
	if not radius then
		...
		for d=1, #dirs do
			...
			elseif minetest.get_item_group(node.name, "lit_campfire") ~= 0 then
				...
				minetest.set_node(tpos, {name = def._vlc_campfires_smothered_form, param2 = node.param2})
				exting = true
			end
		end
	-- Has radius: lingering, extinguish all nodes in area
		...
		for n=1, #nodes do
			...
			elseif minetest.get_item_group(node.name, "lit_campfire") ~= 0 then
				local def = minetest.registered_nodes[node.name]
				minetest.set_node(nodes[n], {name = def._vlc_campfires_smothered_form, param2 = node.param2})
			end
			exting = true
		end
	end
...
function vlc_potions.bad_omen_func(player, factor, duration)
