...
local function lingering_image(colorstring, opacity)
	...
	return "vlc_potions_splash_overlay.png^[colorize:"..colorstring..":"..tostring(opacity).."^vlc_potions_lingering_bottle.png"
end
	...
	if lingering_timer >= 1 then
		...
		for pos, vals in pairs(lingering_effect_at) do
			...
			if vals.is_water then
				texture = "vlc_particles_droplet_bottle.png"
			elseif vals.def.instant then
				texture = "vlc_particles_instant_effect.png"
			else
				texture = "vlc_particles_effect.png"
			end
			linger_particles(pos, d, texture, vals.color)

			-- Extinguish fire if water bottle
			if vals.is_water then
				if vlc_potions._extinguish_nearby_fire(pos, d) then
...
function vlc_potions.register_lingering(name, descr, color, def)

	local id = "vlc_potions:"..name.."_lingering"
	...
	minetest.register_craftitem(id, {
		...
		on_use = function(item, placer, pointed_thing)
			...
			minetest.sound_play("vlc_throwing_throw", {pos = pos, gain = 0.4, max_hear_distance = 16}, true)
	...
	minetest.register_entity(id.."_flying",{
		...
		on_step = function(self, dtime)
			...
			local d = 4
			if mod_target and n == "vlc_target:target_off" then
				vlc_target.hit(vector.round(pos), 0.4) --4 redstone ticks
			end
			if n ~= "air" and n ~= "vlc_portals:portal" and n ~= "vlc_portals:portal_end" and g == 0 or vlc_potions.is_obj_hit(self, pos) then
				minetest.sound_play("vlc_potions_breaking_glass", {pos = pos, max_hear_distance = 16, gain = 1})
				...
				if name == "water" then
					vlc_potions._extinguish_nearby_fire(pos, d)
				end
				self.object:remove()
			end
		end,
