...
function return_on_use(def, effect, dur)
	return function (itemstack, user, pointed_thing)
		if pointed_thing.type == "node" then
			local rc = vlc_util.call_on_rightclick(itemstack, user, pointed_thing)
		...
		itemstack = minetest.do_item_eat(0, "vlc_potions:glass_bottle", itemstack, user, pointed_thing)
		if old_name ~= itemstack:get_name() or old_count ~= itemstack:get_count() then
			vlc_potions._use_potion(itemstack, user, def.color)
		end
		return itemstack
	end
end
...
local function register_potion(def)
	...
	local dur = vlc_potions.DURATION

	if def.is_inv then
		dur = dur * vlc_potions.INV_FACTOR
	end
	...
	minetest.register_craftitem("vlc_potions:"..def.name, {
		description = desc,
	...
	local splash_dur = dur * vlc_potions.SPLASH_FACTOR
	local ling_dur = dur * vlc_potions.LINGERING_FACTOR
	...
	if def.name == "healing" or def.name == "harming" then
		ling_def = {
			tt = get_tt(def._tt, def.effect*vlc_potions.LINGERING_FACTOR, ling_dur),
			longdesc = def._longdesc,
			potion_fun = get_lingering_fun(def.effect*vlc_potions.LINGERING_FACTOR, ling_dur),
			no_effect = def.no_effect,
			instant = def.instant,
		}
	else
	...
	if def.color and not def.no_throwable then
		...
		vlc_potions.register_splash(def.name, desc, def.color, splash_def)
		if def.description_lingering then
			desc = def.description_lingering
		else
			desc = S("Lingering @1 Potion", def.description)
		end
		vlc_potions.register_lingering(def.name, desc, def.color, ling_def)
		if not def.no_arrow then
			vlc_potions.register_arrow(def.name, S("Arrow of @1", def.description), def.color, arrow_def)
		end
	end
	...
	if def.is_II then
		...
		local effect_II
		if def.name == "healing" or def.name == "harming" then
			effect_II = def.effect*vlc_potions.II_FACTOR
		...
		local dur_2 = dur / vlc_potions.II_FACTOR
		...
		minetest.register_craftitem("vlc_potions:"..def.name.."_2", {
		...
		-- Register Splash and Lingering
		local splash_dur_2 = dur_2 * vlc_potions.SPLASH_FACTOR
		local ling_dur_2 = dur_2 * vlc_potions.LINGERING_FACTOR
		...
		if def.name == "healing" or def.name == "harming" then
			ling_def_2 = {
				tt = get_tt(def._tt_2, effect_II*vlc_potions.LINGERING_FACTOR, ling_dur_2),
				longdesc = def._longdesc,
				potion_fun = get_lingering_fun(effect_II*vlc_potions.LINGERING_FACTOR, ling_dur_2),
				no_effect = def.no_effect,
				instant = def.instant,
			}
		else
		...
		if def.color and not def.no_throwable then
			vlc_potions.register_splash(def.name.."_2", S("Splash @1@2 Potion", def.description, desc_mod), def.color, splash_def_2)
			vlc_potions.register_lingering(def.name.."_2", S("Lingering @1@2 Potion", def.description, desc_mod), def.color, ling_def_2)
			if not def.no_arrow then
				vlc_potions.register_arrow(def.name.."_2", S("Arrow of @1@2", def.description, desc_mod), def.color, arrow_def_2)
			end
		end
	...
	if def.is_plus then
		...
		local dur_pl = dur * vlc_potions.PLUS_FACTOR
		if def.name == "poison" or def.name == "regeneration" or def.name == "withering" then
			dur_pl = 90
		end
		...
		minetest.register_craftitem("vlc_potions:"..def.name.."_plus", {
		...
		-- Register Splash
		local splash_dur_pl = dur_pl * vlc_potions.SPLASH_FACTOR
		local ling_dur_pl = dur_pl * vlc_potions.LINGERING_FACTOR
		...
		if def.color and not def.no_throwable then
			vlc_potions.register_splash(def.name.."_plus", S("Splash @1 + Potion", def.description), def.color, splash_def_pl)
			vlc_potions.register_lingering(def.name.."_plus", S("Lingering @1 + Potion", def.description), def.color, ling_def_pl)
			if not def.no_arrow then
				vlc_potions.register_arrow(def.name.."_plus", S("Arrow of @1 +", def.description), def.color, arrow_def_pl)
			end
		end
...
local awkward_def = {
	...
	on_use = minetest.item_eat(0, "vlc_potions:glass_bottle"),
}
...
local dragon_breath_def = {
	...
	image = "vlc_potions_dragon_breath.png",
...
local healing_def = {
	...
	effect = 4,
	instant = true,
	on_use = vlc_potions.healing_func,
	is_II = true,
}
...
local night_vision_def = {
	...
	effect = nil,
	is_dur = true,
	on_use = vlc_potions.night_vision_func,
	is_plus = true,
}
...
local swiftness_def = {
	...
	effect = 1.2,
	is_dur = true,
	on_use = vlc_potions.swiftness_func,
	is_II = true,
	is_plus = true,
}
...
local leaping_def = {
	...
	effect = 1.15,
	is_dur = true,
	on_use = vlc_potions.leaping_func,
	is_II = true,
	is_plus = true,
}
...
local withering_def = {
	...
	effect = 4,
	is_dur = true,
	on_use = vlc_potions.withering_func,
	is_II = true,
	is_plus = true,
	is_inv = true,
}
...
local poison_def = {
	...
	effect = 2.5,
	is_dur = true,
	on_use = vlc_potions.poison_func,
	is_II = true,
	is_plus = true,
	is_inv = true,
}
...
local regeneration_def = {
	...
	effect = 2.5,
	is_dur = true,
	on_use = vlc_potions.regeneration_func,
	is_II = true,
	is_plus = true,
}
...
local invisibility_def = {
	...
	color = "#7F8392",
	is_dur = true,
	on_use = vlc_potions.invisiblility_func,
	is_plus = true,
}
...
local water_breathing_def = {
	...
	color = "#2E5299",
	is_dur = true,
	on_use = vlc_potions.water_breathing_func,
	is_plus = true,
}
...
local fire_resistance_def = {
	...
	color = "#E49A3A",
	is_dur = true,
	on_use = vlc_potions.fire_resistance_func,
	is_plus = true,
}
...
-- minetest.register_craftitem("vlc_potions:weakness", {
...
-- 		vlc_potions.weakness_func(user, -4, vlc_potions.DURATION*vlc_potions.INV_FACTOR)
-- 		minetest.do_item_eat(0, "vlc_potions:glass_bottle", itemstack, user, pointed_thing)
-- 		vlc_potions._use_potion(itemstack, user, "#484D48")
-- 		return itemstack
-- 	end,
--
...
-- })
--
-- minetest.register_craftitem("vlc_potions:weakness_plus", {
-- 	description = S("Weakness +"),
-- 	_tt_help = TODO,
-- 	_doc_items_longdesc = brewhelp,
-- 	wield_image = potion_image("#484D48"),
...
--
-- 	on_place = function(itemstack, user, pointed_thing)
-- 		vlc_potions.weakness_func(user, -4, vlc_potions.DURATION_2*vlc_potions.INV_FACTOR)
-- 		minetest.do_item_eat(0, "vlc_potions:glass_bottle", itemstack, user, pointed_thing)
-- 		vlc_potions._use_potion(itemstack, user, "#484D48")
-- 		return itemstack
-- 	end,
...
-- })
--
-- minetest.register_craftitem("vlc_potions:strength", {
...
--
-- 	on_place = function(itemstack, user, pointed_thing)
-- 		vlc_potions.weakness_func(user, 3, vlc_potions.DURATION)
-- 		minetest.do_item_eat(0, "vlc_potions:glass_bottle", itemstack, user, pointed_thing)
-- 		vlc_potions._use_potion(itemstack, user, "#932423")
-- 		return itemstack
-- 	end,
--
-- 	on_secondary_use = function(itemstack, user, pointed_thing)
...
-- })
--
-- minetest.register_craftitem("vlc_potions:strength_2", {
...
--
-- 	on_place = function(itemstack, user, pointed_thing)
-- 		vlc_potions.weakness_func(user, 6, vlc_potions.DURATION_2)
-- 		minetest.do_item_eat(0, "vlc_potions:glass_bottle", itemstack, user, pointed_thing)
-- 		vlc_potions._use_potion(itemstack, user, "#932423")
-- 		return itemstack
-- 	end,
...
-- })
--
-- minetest.register_craftitem("vlc_potions:strength_plus", {
-- 	description = S("Strength +"),
-- 	_tt_help = TODO,
-- 	_doc_items_longdesc = brewhelp,
-- 	wield_image = potion_image("#932423"),
...
--
-- 	on_place = function(itemstack, user, pointed_thing)
-- 		vlc_potions.weakness_func(user, 3, vlc_potions.DURATION_PLUS)
-- 		minetest.do_item_eat(0, "vlc_potions:glass_bottle", itemstack, user, pointed_thing)
-- 		vlc_potions._use_potion(itemstack, user, "#932423")
-- 		return itemstack
-- 	end,
