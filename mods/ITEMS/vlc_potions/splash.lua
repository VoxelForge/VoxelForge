...
local function splash_image(colorstring, opacity)
	...
	return "vlc_potions_splash_overlay.png^[colorize:"..colorstring..":"..tostring(opacity).."^vlc_potions_splash_bottle.png"
end


function vlc_potions.register_splash(name, descr, color, def)
	local id = "vlc_potions:"..name.."_splash"
	...
	minetest.register_entity(id.."_flying",{
		...
		on_step = function(self, dtime)
			...
			if n ~= "air" and n ~= "vlc_portals:portal" and n ~= "vlc_portals:portal_end" and g == 0 or vlc_potions.is_obj_hit(self, pos) then
				...
				if name == "water" then
					vlc_potions._extinguish_nearby_fire(pos)
				end
				self.object:remove()
