...
local sus_drops_default = {
	...
	"vlf_dyes:light_blue",
	"vlf_core:coal_lump",
	"vlf_flowerpots:flower_pot",
}

function vlf_sus_nodes.get_random_item(pos)
	local meta = minetest.get_meta(pos)
	local struct = meta:get_string("structure")
	local structdef = vlf_structures.registered_structures[struct]
	...
	if struct ~= "" and structdef and structdef.loot and structdef.loot["SUS"] then
		local lootitems = vlf_loot.get_multi_loot(structdef.loot["SUS"], pr)
		if #lootitems > 0 then
			return lootitems[1]
		end
	else
...
local function brush_node(itemstack, user, pointed_thing)
	if pointed_thing and pointed_thing.type == "node" then
		...
		if not item_entities[ph] then
			local o = minetest.add_entity(pos + (dir * 0.38),"vlf_sus_nodes:item_entity")
			local l = o:get_luaentity()
			l._item = vlf_sus_nodes.get_random_item(pos)
			if not l._item then
				o:remove()
				return
			end
		...
		if item_entities[ph]._stage >= 4 then
			...
			minetest.swap_node(pos,{name = def._vlf_sus_nodes_parent})
		elseif item_entities[ph]._stage <= 0 then
			minetest.swap_node(pos,{name=def._vlf_sus_nodes_main})
		else
			minetest.swap_node(pos,{name=def._vlf_sus_nodes_main.."_"..item_entities[ph]._stage})
		end
	end
end
...
function vlf_sus_nodes.register_sus_node(name,source,overrides)
	local sdef = minetest.registered_nodes[source]
	assert(sdef, "[vlf_sus_nodes] trying to register "..tostring(name).." but source node "..tostring(source).."doesn't exist")
	local main_itemstring = "vlf_sus_nodes:"..name
	table.shuffle(sus_drops_default)
	local def = table.merge(sdef,tpl,{
		description = S("Suspicious "..name),
		tiles = overlay_tiles(sdef.tiles,"vlf_sus_nodes_suspicious_overlay.png"),
		drop = source,
		_vlf_sus_nodes_parent = source,
		_vlf_sus_nodes_main = main_itemstring,
		_vlf_sus_nodes_drops = table.copy(sus_drops_default),
		_vlf_falling_node_alternative = source,
	},overrides or {})
	minetest.register_node(main_itemstring,def)
	for i=1,3 do
		minetest.register_node(main_itemstring.."_"..i,table.merge(def,{
			tiles = overlay_tiles(sdef.tiles,"vlf_sus_nodes_suspicious_overlay_"..i..".png"),
			groups = table.merge(tpl.groups, { suspicious_stage =i, not_in_creative_inventory = 1 }),
		}))
	end
end

minetest.register_entity("vlf_sus_nodes:item_entity", {
	initial_properties = {
		physical = false,
		visual = "wielditem",
		...
		if self._timer < 0 then
			...
			if self._hide then
				...
				if self._stage <= 0 then
					minetest.swap_node(self._nodepos, {name=def._vlf_sus_nodes_main})
				else
					minetest.swap_node(self._nodepos, {name=def._vlf_sus_nodes_main.."_"..self._stage})
				end
			end
			self._timer = 1
		end
...
minetest.register_tool("vlf_sus_nodes:brush", {
	description = S("Brush"),
...
minetest.register_craft({
	output = "vlf_sus_nodes:brush",
	recipe = {
		{ "vlf_mobitems:feather"},
		{ "vlf_copper:copper_ingot"},
		{ "vlf_core:stick"},
	}
})

vlf_sus_nodes.register_sus_node("sand","vlf_core:sand",{
	description = S("Suspicious Sand"),
})

vlf_sus_nodes.register_sus_node("gravel","vlf_core:gravel",{
