...
local fishbucket_prefix = "vlc_buckets:bucket_"
...
local function on_place_fish(itemstack, placer, pointed_thing)
	...
	if ( def and def.buildable_to ) or n.name == "vlc_portals:portal" then
		pos = pointed_thing.under
		n = minetest.get_node(pointed_thing.under)
	end

	local fish = itemstack:get_definition()._vlc_buckets_fish
	if fish_names[fish] then
		...
		if o and o:get_pos() then
			...
			local water = "vlc_core:water_source"
			if n.name == "vlcx_core:river_water_source" then
				water = n.name
			elseif n.name == "vlcx_core:river_water_flowing" then
				water = nil
			end
			if vlc_worlds.pos_to_dimension(pos) == "nether" then
				water = nil
			...
			if not placer or not minetest.is_creative_enabled(placer:get_player_name()) then
				itemstack = ItemStack("vlc_buckets:bucket_empty")
			end
		end
	end
	return itemstack
...
for techname, fishname in pairs(fish_names) do
	...
	minetest.register_alias("vlc_fishing:bucket_" .. techname, "vlc_buckets:bucket_" .. techname)
end
