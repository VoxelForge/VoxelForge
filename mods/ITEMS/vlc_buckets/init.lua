...
minetest.register_alias("bucket:bucket_empty", "vlc_buckets:bucket_empty")
minetest.register_alias("bucket:bucket_water", "vlc_buckets:bucket_water")
minetest.register_alias("bucket:bucket_lava", "vlc_buckets:bucket_lava")

local mod_doc = minetest.get_modpath("doc")

minetest.register_craft({
	output = "vlc_buckets:bucket_empty 1",
	recipe = {
...
local function get_bucket_drop(itemstack, user, take_bucket)
	...
	if take_bucket and not minetest.is_creative_enabled(user:get_player_name()) then
		...
		local new_bucket = ItemStack("vlc_buckets:bucket_empty")
		if itemstack:get_count() == 1 then
			return new_bucket
		else
			local inv = user:get_inventory()
...
local function on_place_bucket(itemstack, user, _)
	...
	local new_stack = vlc_util.call_on_rightclick(itemstack, user, pointed_thing)
	if new_stack then
		return new_stack
	end
	...
	local bucket_def = vlc_buckets.buckets[itemstack:get_name()]
...
local function on_place_bucket_empty(itemstack, user, _)
	...
	if pointable_sources[node_name] then
		...
		local liquid_def = vlc_buckets.liquids[node_name]
		if liquid_def then
			...
			minetest.log("error", string.format("[vlc_buckets] Node [%s] has invalid group [_vlc_bucket_pointable]!", node_name))
		end
		return itemstack
	else
		if new_bucket then
...
function vlc_buckets.register_liquid(def)
	for _,source in ipairs(def.source_take) do
		vlc_buckets.liquids[source] = {
		...
		if type(def.source_place) == "string" then
			vlc_buckets.liquids[def.source_place] = vlc_buckets.liquids[source]
		end
	end

	vlc_buckets.buckets[def.bucketname] = def

	if def.bucketname == nil or def.bucketname == "" then
		error(string.format("[vlc_bucket] Invalid itemname then registering [%s]!", def.name))
	end
	...
	minetest.register_craftitem(def.bucketname, {
		...
		_on_dispense = function(stack, pos, droppos, dropnode, dropdir)
			local buildable = minetest.registered_nodes[dropnode.name].buildable_to or dropnode.name == "vlc_portals:portal"
			...
			if take_bucket then
				stack:set_name("vlc_buckets:bucket_empty")
			end
			return stack
		end,
	})
...
minetest.register_craftitem("vlc_buckets:bucket_empty", {
	...
	_on_dispense = function(stack, pos, droppos, dropnode, dropdir)
		...
		local liquiddef = vlc_buckets.liquids[dropnode.name]
		local new_bucket
