...
vlf_flowers = {}
vlf_flowers.registered_simple_flowers = {}
...
function vlf_flowers.on_bone_meal(itemstack,placer,pointed_thing,pos,n)
	if n.name == "vlf_flowers:rose_bush" or n.name == "vlf_flowers:rose_bush_top" then
		...
		return true
	elseif n.name == "vlf_flowers:peony" or n.name == "vlf_flowers:peony_top" then
		minetest.add_item(pos, "vlf_flowers:peony")
		return true
	elseif n.name == "vlf_flowers:lilac" or n.name == "vlf_flowers:lilac_top" then
		minetest.add_item(pos, "vlf_flowers:lilac")
		return true
	elseif n.name == "vlf_flowers:sunflower" or n.name == "vlf_flowers:sunflower_top" then
		minetest.add_item(pos, "vlf_flowers:sunflower")
		return true
	elseif n.name == "vlf_flowers:tallgrass" then
		-- Tall Grass: Grow into double tallgrass
		local toppos = { x=pos.x, y=pos.y+1, z=pos.z }
		local topnode = minetest.get_node(toppos)
		if minetest.registered_nodes[topnode.name].buildable_to then
			minetest.set_node(pos, { name = "vlf_flowers:double_grass", param2 = n.param2 })
			minetest.set_node(toppos, { name = "vlf_flowers:double_grass_top", param2 = n.param2 })
			return true
		end
	elseif n.name == "vlf_flowers:fern" then
		-- Fern: Grow into large fern
		local toppos = { x=pos.x, y=pos.y+1, z=pos.z }
		local topnode = minetest.get_node(toppos)
		if minetest.registered_nodes[topnode.name].buildable_to then
			minetest.set_node(pos, { name = "vlf_flowers:double_fern", param2 = n.param2 })
			minetest.set_node(toppos, { name = "vlf_flowers:double_fern_top", param2 = n.param2 })
			return true
		end
	end
	return false
...
function vlf_flowers.get_palette_color_from_pos(pos)
...
vlf_flowers.on_place_flower = vlf_util.generate_on_place_plant_function(function(pos, node, itemstack)
	...
	local colorize
	if has_palette then
		colorize = vlf_flowers.get_palette_color_from_pos(pos)
	end
...
--[[	Placement requirements:
	...
	* Light level >= 8 at any time or exposed to sunlight at day
	...
	if itemstack:get_name() == "vlf_flowers:wither_rose" and (  minetest.get_item_group(soil_node.name, "grass_block") > 0 or soil_node.name == "vlf_core:dirt" or soil_node.name == "vlf_core:coarse_dirt" or soil_node.name == "vlf_mud:mud" or soil_node.name == "vlf_lush_caves:moss" or soil_node.name == "vlf_nether:netherrack" or minetest.get_item_group(soil_node.name, "soul_block") > 0  ) then
		return true,colorize
	end
	local is_flower = minetest.get_item_group(itemstack:get_name(), "flower") == 1
	local ok = (soil_node.name == "vlf_core:dirt" or minetest.get_item_group(soil_node.name, "grass_block") == 1 or soil_node.name == "vlf_lush_caves:moss" or (not is_flower and (soil_node.name == "vlf_core:coarse_dirt" or soil_node.name == "vlf_core:podzol" or soil_node.name == "vlf_core:podzol_snow"))) and light_ok
	return ok, colorize
end)

function vlf_flowers.register_simple_flower(name, def)
	local newname = "vlf_flowers:"..name
	if not def._vlf_silk_touch_drop then def._vlf_silk_touch_drop = nil end
	if not def.drop then def.drop = newname end
	vlf_flowers.registered_simple_flowers[newname] = {
		name=name,
		desc=def.desc,
		image=def.image,
	...
	minetest.register_node(":"..newname, {
		...
		_vlf_silk_touch_drop = def._vlf_silk_touch_drop,
		_on_bone_meal = vlf_flowers.on_bone_meal_simple,
	})
	if def.potted then
		vlf_flowerpots.register_potted_flower(newname, {
			name = name,
			desc = def.desc,
			image = def.image,
		})
...
function vlf_flowers.add_large_plant(name, def)
	...
	-- Bottom
	minetest.register_node(":vlf_flowers:"..name, table.merge(tpl_large_plant_bottom,{
		description = def.desc,
	...
	-- Top
	minetest.register_node(":vlf_flowers:"..name.."_top", table.merge(tpl_large_plant_top, {
		...
		after_destruct = function(pos, oldnode)
			...
			if minetest.get_node(top).name ~= "vlf_flowers:"..name.."_top" and minetest.get_node(bottom).name == "vlf_flowers:"..name then
	...
	if def.bottom._doc_items_longdesc then
		doc.add_entry_alias("nodes", "vlf_flowers:"..name, "nodes", "vlf_flowers:"..name.."_top")
...
minetest.register_abm({
	...
	action = function(pos, node)
		...
		if (below.name ~= "vlf_core:dirt" and minetest.get_item_group(below.name, "grass_block") ~= 1 and below.name ~= "vlf_lush_caves:moss") or (minetest.get_node_light(pos, 0.5) < 8) then
...
-- Legacy support
minetest.register_alias("vlf_core:tallgrass", "vlf_flowers:tallgrass")
...
if mod_mcimport and mg_name == "singlenode" then
	...
	minetest.register_lbm({
		...
		name = "vlf_flowers:double_plant_topper",
		run_at_every_load = true,
		nodenames = { "vlf_flowers:peony", "vlf_flowers:rose_bush", "vlf_flowers:lilac", "vlf_flowers:sunflower", "vlf_flowers:double_fern", "vlf_flowers:double_grass" },
		action = function(pos, node)
			for c = 1, 6 do
				local flowername = flowernames[c]
				local bottom = pos
				local top = { x = bottom.x, y = bottom.y + 1, z = bottom.z }
				if node.name == "vlf_flowers:"..flowername then
					minetest.set_node(top, {name = "vlf_flowers:"..flowername.."_top"})
				end
			end
		end,
	})
