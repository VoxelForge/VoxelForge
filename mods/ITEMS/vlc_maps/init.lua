...
local map_textures_path = worldpath .. "/vlc_maps/"
...
function vlc_maps.create_map(pos)
	local minp = vector.multiply(vector.floor(vector.divide(pos, 128)), 128)
	local maxp = vector.add(minp, vector.new(127, 127, 127))

	local itemstack = ItemStack("vlc_maps:filled_map")
	local meta = itemstack:get_meta()
	local next_id = storage:get_int("next_id")
	storage:set_int("next_id", next_id + 1)
	local id = tostring(next_id)
	meta:set_string("vlc_maps:id", id)
	meta:set_string("vlc_maps:minp", minetest.pos_to_string(minp))
	meta:set_string("vlc_maps:maxp", minetest.pos_to_string(maxp))
	...
	minetest.emerge_area(minp, maxp, function(blockpos, action, calls_remaining)
		...
		tga_encoder.image(pixels):save(map_textures_path .. "vlc_maps_map_texture_" .. id .. ".tga")
		creating_maps[id] = nil
	end)
	return itemstack
end

function vlc_maps.load_map(id, callback)
	if not id or id == "" or creating_maps[id] then
		return false
	end

	local texture = "vlc_maps_map_texture_" .. id .. ".tga"
...
function vlc_maps.load_map_item(itemstack)
	return vlc_maps.load_map(itemstack:get_meta():get_string("vlc_maps:id"))
end
...
local function fill_map(itemstack, placer, pointed_thing)
	...
	if minetest.settings:get_bool("enable_real_maps", true) then
		local new_map = vlc_maps.create_map(placer:get_pos())
		itemstack:take_item()
...
minetest.register_craftitem("vlc_maps:empty_map", {
	...
	inventory_image = "vlc_maps_map_empty.png",
	on_place = fill_map,
...
local filled_def = {
	...
	inventory_image = "vlc_maps_map_filled.png^(vlc_maps_map_filled_markings.png^[colorize:#000000)",
	groups = { not_in_creative_inventory = 1, filled_map = 1, tool = 1 },
}

minetest.register_craftitem("vlc_maps:filled_map", filled_def)
...
filled_wield_def.on_place = vlc_util.call_on_rightclick
filled_wield_def._vlc_wieldview_item = "vlc_maps:filled_map"
...
if vlc_skins_enabled then
	...
	for _, skin in pairs(list) do
		if skin.slim_arms then
			...
			minetest.register_node("vlc_maps:filled_map_" .. skin.id, female)
		else
			local male = table.copy(filled_wield_def)
			male._vlc_hand_id = skin.id
			male.mesh = "vlc_meshhand.b3d"
			male.tiles = { skin.texture }
			minetest.register_node("vlc_maps:filled_map_" .. skin.id, male)
		end
	end
else
	filled_wield_def._vlc_hand_id = "hand"
	filled_wield_def.mesh = "vlc_meshhand.b3d"
	filled_wield_def.tiles = { "character.png" }
	minetest.register_node("vlc_maps:filled_map_hand", filled_wield_def)
end
...
function minetest.add_item(pos, stack)
	...
	if minetest.get_item_group(stack:get_name(), "filled_map") > 0 then
		stack:set_name("vlc_maps:filled_map")
	end
...
tt.register_priority_snippet(function(itemstring, _, itemstack)
	if itemstack and minetest.get_item_group(itemstring, "filled_map") > 0 then
		local id = itemstack:get_meta():get_string("vlc_maps:id")
		if id ~= "" then
			return "#" .. id, vlc_colors.GRAY
		end
	end
end)

minetest.register_craft({
	output = "vlc_maps:empty_map",
	recipe = {
		{ "vlc_core:paper", "vlc_core:paper", "vlc_core:paper" },
		{ "vlc_core:paper", "group:compass", "vlc_core:paper" },
		{ "vlc_core:paper", "vlc_core:paper", "vlc_core:paper" },
	}
})
...
minetest.register_craft({
	type = "shapeless",
	output = "vlc_maps:filled_map 2",
	recipe = { "group:filled_map", "vlc_maps:empty_map" },
})

local function on_craft(itemstack, player, old_craft_grid, craft_inv)
	if itemstack:get_name() == "vlc_maps:filled_map" then
...
minetest.register_globalstep(function(dtime)
	for _, player in pairs(minetest.get_connected_players()) do
		...
		local texture = vlc_maps.load_map_item(wield)
		...
		if texture then
			...
			if hand_def and wield_def and hand_def._vlc_hand_id ~= wield_def._vlc_hand_id then
				wield:set_name("vlc_maps:filled_map_" .. hand_def._vlc_hand_id)
				player:set_wielded_item(wield)
			end

			if texture ~= maps[player] then
				player:hud_change(hud.map, "text", "[combine:140x140:0,0=vlc_maps_map_background.png:6,6=" .. texture)
				maps[player] = texture
			end

			local pos = vector.round(player:get_pos())
			local meta = wield:get_meta()
			local minp = minetest.string_to_pos(meta:get_string("vlc_maps:minp"))
			local maxp = minetest.string_to_pos(meta:get_string("vlc_maps:maxp"))

			local marker = "vlc_maps_player_arrow.png"

			if pos.x < minp.x then
				marker = "vlc_maps_player_dot.png"
				pos.x = minp.x
			...
			if marker == "vlc_maps_player_arrow.png" then
