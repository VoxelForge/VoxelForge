...
local O_Y_MIN, O_Y_MAX			= math.max(mcl_vars.mg_overworld_min, -31), math.min(mcl_vars.mg_overworld_max, 2048)
...
local function add_exit(p)
	...
	local p = {x = x, y = y, z = z}
	if mcl_vars.get_node({x=x,y=y-1,z=z}).name ~= OBSIDIAN or mcl_vars.get_node(p).name ~= PORTAL or mcl_vars.get_node({x=x,y=y+1,z=z}).name ~= PORTAL then return end
	local k = math.floor(z/256) * 256 + math.floor(x/256)
	if not exits[k] then
		exits[k]={}
	end
	local e = exits[k]
...
local function find_exit(p, dx, dy, dz)
	...
	local k1x, k2x = math.floor(x1/256), math.floor(x2/256)
	local k1z, k2z = math.floor(z1/256), math.floor(z2/256)
	...
	if t and math.abs(t.x-p.x) <= dx and math.abs(t.y-p.y) <= dy and math.abs(t.z-p.z) <= dz then
		return t
	end
end
...
local function ping_pong(x, m, l1, l2)
	if x < 0 then
		return	 l1 + math.abs(((x*m+l1) % (l1*4)) - (l1*2)), 	math.floor(x*m/l1/2)  + ((math.ceil(x*m/l1)+1)%2) * ((x*m)%l1)/l1
	end
	return		 l2 - math.abs(((x*m+l2) % (l2*4)) - (l2*2)), 	math.floor(x*m/l2/2) + (math.floor(x*m/l2)%2) * ((x*m)%l2)/l2
end
	...
	if p and p.y and p.x and p.z then
		...
		if y then
			if d=="nether" then
				...
				y = math.floor(y * TRAVEL_Y + (o1+o2) / 16 * LIM_MAX)
				y = math.min(math.max(y + O_Y_MIN, O_Y_MIN), O_Y_MAX)
			elseif d=="overworld" then
				x, y, z = math.floor(x / TRAVEL_X + 0.5), math.floor(y / TRAVEL_Y + 0.5), math.floor(z / TRAVEL_Z + 0.5)
				y = math.min(math.max(y + N_Y_MIN, N_Y_MIN), N_Y_MAX)
			end
	...
	for x = x1 - 1 + orientation, x2 + 1 - orientation do
		...
		for z = z1 - orientation, z2 + orientation do
			...
			for y = y1 - 1, y2 + 1 do
				...
				if frame then
					minetest.set_node(pos, node_frame)
				else
...
function build_nether_portal(pos, width, height, orientation, name, clear_before_build)
	local width, height, orientation = width or W_MIN - 2, height or H_MIN - 2, orientation or math.random(0, 1)
	...
	for x = pos.x - orientation, pos.x + orientation + (width - 1) * (1 - orientation), 1 + orientation do
		for z = pos.z - 1 + orientation, pos.z + 1 - orientation + (width - 1) * orientation, 2 - orientation do
			...
			local nn = mcl_vars.get_node(pp).name
			local nn_1 = mcl_vars.get_node(pp_1).name
			if ((nn=="air" and nn_1 == "air") or not minetest.registered_nodes[nn].is_ground_content) and not minetest.is_protected(pp, name) then
				minetest.set_node(pp, {name = OBSIDIAN})
			end
		end
	end

	minetest.log("action", "[mcl_portals] Destination Nether portal generated at "..minetest.pos_to_string(pos).."!")
...
local function finalize_teleport(obj, exit)
	...
	objpos = {x = math.floor(objpos.x+0.5), y = math.ceil(objpos.y), z = math.floor(objpos.z+0.5)}
	if mcl_vars.get_node(objpos).name ~= PORTAL then return end
	...
	if is_player then
		...
		minetest.log("action", "[mcl_portals] player "..name.." teleported to Nether portal at "..minetest.pos_to_string(exit)..".")
		if dim == "nether" then
			awards.unlock(obj:get_player_name(), "mcl:theNether")
		end
	else
		minetest.log("action", "[mcl_portals] entity teleported to Nether portal at "..minetest.pos_to_string(exit)..".")
	end
end
...
local function get_lava_level(pos, pos1, pos2)
	if pos.y > -1000 then
		return math.max(math.min(mcl_vars.mg_lava_overworld_max, pos2.y-1), pos1.y+1)
	end
	return math.max(math.min(mcl_vars.mg_lava_nether_max, pos2.y-1), pos1.y+1)
end
...
local function ecb_scan_area_2(blockpos, action, calls_remaining, param)
	...
	local portals = minetest.find_nodes_in_area(pos1, pos2, {PORTAL})
	...
	local nodes = minetest.find_nodes_in_area_under_air(pos1, pos2, {"group:building_block"})
	if nodes then
		local nc = #nodes
		minetest.log("action", "[mcl_portals] Area for destination Nether portal emerged! Found " .. tostring(nc) .. " nodes under the air around "..minetest.pos_to_string(pos))
		if nc > 0 then
			for i=1,nc do
				local node = nodes[i]
				local node1 = {x=node.x,   y=node.y+1, z=node.z  }
				local node2 = {x=node.x+2, y=node.y+3, z=node.z+2}
				local nodes2 = minetest.find_nodes_in_area(node1, node2, {"air"})
				if nodes2 then
					local nc2 = #nodes2
					if nc2 == 27 and not minetest.is_area_protected(node, node2, name) then
						local distance0 = vector.distance(pos, node)
						if distance0 < 2 then
							minetest.log("action", "[mcl_portals] found space at pos "..minetest.pos_to_string(node).." - creating a portal")
						...
						if not distance or (distance0 < distance) or (distance0 < distance-1 and node.y > lava and pos0.y < lava) then
							minetest.log("verbose", "[mcl_portals] found distance "..tostring(distance0).." at pos "..minetest.pos_to_string(node))
							distance = distance0
	...
	if distance then -- several nodes of air might be better than lava lake, right?
		minetest.log("action", "[mcl_portals] using backup pos "..minetest.pos_to_string(pos0).." to create a portal")
	...
	if param.next_chunk_1 and param.next_chunk_2 and param.next_pos then
		...
		if p.x >= pos1.x and p.x <= pos2.x and p.y >= pos1.y and p.y <= pos2.y and p.z >= pos1.z and p.z <= pos2.z then
			minetest.log("action", "[mcl_portals] Making additional search in chunk below, because current one doesn't contain any air space for portal, target pos "..minetest.pos_to_string(p))
			minetest.emerge_area(pos1, pos2, ecb_scan_area_2, {pos = p, pos1 = pos1, pos2 = pos2, name=name, obj=obj})
			return
		end
	end

	minetest.log("action", "[mcl_portals] found no space, reverting to target pos "..minetest.pos_to_string(pos).." - creating a portal")
	if pos.y < lava then
		pos.y = lava + 1
	else
		pos.y = pos.y + 1
...
local function create_portal(pos, limit1, limit2, name, obj)
	...
	local pos1 = vector.add(vector.multiply(mcl_vars.pos_to_chunk(pos), mcl_vars.chunk_size_in_nodes), mcl_vars.central_chunk_offset_in_nodes)
	...
	if not SCAN_2_MAP_CHUNKS then
		if limit1 and limit1.x and limit1.y and limit1.z then
			pos1 = {x = math.max(math.min(limit1.x, pos.x), pos1.x), y = math.max(math.min(limit1.y, pos.y), pos1.y), z = math.max(math.min(limit1.z, pos.z), pos1.z)}
		end
		if limit2 and limit2.x and limit2.y and limit2.z  then
			pos2 = {x = math.min(math.max(limit2.x, pos.x), pos2.x), y = math.min(math.max(limit2.y, pos.y), pos2.y), z = math.min(math.max(limit2.z, pos.z), pos2.z)}
		end
	...
	local next_chunk_2 = vector.add(next_chunk_1, mcl_vars.chunk_size_in_nodes - 1)
	local next_pos = {x = pos.x, y=math.max(next_chunk_2.y, limit1.y), z = pos.z}
	if limit1 and limit1.x and limit1.y and limit1.z then
		pos1 = {x = math.max(math.min(limit1.x, pos.x), pos1.x), y = math.max(math.min(limit1.y, pos.y), pos1.y), z = math.max(math.min(limit1.z, pos.z), pos1.z)}
		next_chunk_1 = {x = math.max(math.min(limit1.x, next_pos.x), next_chunk_1.x), y = math.max(math.min(limit1.y, next_pos.y), next_chunk_1.y), z = math.max(math.min(limit1.z, next_pos.z), next_chunk_1.z)}
	end
	if limit2 and limit2.x and limit2.y and limit2.z  then
		pos2 = {x = math.min(math.max(limit2.x, pos.x), pos2.x), y = math.min(math.max(limit2.y, pos.y), pos2.y), z = math.min(math.max(limit2.z, pos.z), pos2.z)}
		next_chunk_2 = {x = math.min(math.max(limit2.x, next_pos.x), next_chunk_2.x), y = math.min(math.max(limit2.y, next_pos.y), next_chunk_2.y), z = math.min(math.max(limit2.z, next_pos.z), next_chunk_2.z)}
	end
...
local function available_for_nether_portal(p)
	local nn = mcl_vars.get_node(p).name
...
minetest.override_item(OBSIDIAN, {
	...
	_on_ignite = function(user, pointed_thing)
		...
		if portals_placed then
			minetest.log("action", "[mcl_portals] Nether portal activated at "..minetest.pos_to_string({x=x,y=y,z=z})..".")
