...
vlc_fishing.loot_fish = {
	{ itemstring = "vlc_fishing:fish_raw", weight = 60 },
	{ itemstring = "vlc_fishing:salmon_raw", weight = 25 },
	{ itemstring = "vlc_fishing:clownfish_raw", weight = 2 },
	{ itemstring = "vlc_fishing:pufferfish_raw", weight = 13 },
}

vlc_fishing.loot_junk = {
	{ itemstring = "vlc_core:bowl", weight = 10 },
	{ itemstring = "vlc_fishing:fishing_rod", weight = 2, wear_min = 6554, wear_max = 65535 }, -- 10%-100% damage
	{ itemstring = "vlc_mobitems:leather", weight = 10 },
	{ itemstring = "vlc_armor:boots_leather", weight = 10, wear_min = 6554, wear_max = 65535 }, -- 10%-100% damage
	{ itemstring = "vlc_mobitems:rotten_flesh", weight = 10 },
	{ itemstring = "vlc_core:stick", weight = 5 },
	{ itemstring = "vlc_mobitems:string", weight = 5 },
	{ itemstring = "vlc_potions:water", weight = 10 },
	{ itemstring = "vlc_mobitems:bone", weight = 10 },
	{ itemstring = "vlc_mobitems:ink_sac", weight = 1, amount_min = 10, amount_max = 10 },
	{ itemstring = "vlc_mobitems:string", weight = 10 }, -- TODO: Tripwire Hook
}

vlc_fishing.loot_treasure = {
	{ itemstring = "vlc_bows:bow", wear_min = 49144, wear_max = 65535, func = function(stack, pr)
		vlc_enchanting.enchant_randomly(stack, 30, true, false, false, pr)
	end }, -- 75%-100% damage
	{ itemstring = "vlc_books:book", func = function(stack, pr)
		vlc_enchanting.enchant_randomly(stack, 30, true, true, false, pr)
	end },
	{ itemstring = "vlc_fishing:fishing_rod", wear_min = 49144, wear_max = 65535, func = function(stack, pr)
		vlc_enchanting.enchant_randomly(stack, 30, true, false, false, pr)
	end }, -- 75%-100% damage
	{ itemstring = "vlc_mobitems:nametag", },
	{ itemstring = "vlc_mobitems:saddle", },
	{ itemstring = "vlc_flowers:waterlily", },
	{ itemstring = "vlc_mobitems:nautilus_shell", },
}
...
local bobber_ENTITY={
	initial_properties = {
		physical = false,
		textures = {"vlc_fishing_bobber.png"},
...
local function remove_bobber(player, object)
	if player and bobbers[player] and bobbers[player].remove then
		...
		if ent and ent._reeling and ent._hooked and ent._hooked:get_pos() then
			ent._hooked:set_acceleration(vector.new(0, -vlc_item_entity.get_gravity(), 0))
		end
...
local function fish(itemstack, player, pointed_thing)
	...
	local durability = 65
	local unbreaking = vlc_enchanting.get_enchantment(itemstack, "unbreaking")
	...
	if bobbers[player] then
		...
		if ent._hooked and ent._hooked:get_pos() then
			...
			if not ent._reeling and hent and hent._vlc_fishing_reelable then
		...
		if ent and ent._dive == true then
			...
			local luck_of_the_sea = math.min(vlc_enchanting.get_enchantment(itemstack, "luck_of_the_sea"), 3)
			...
			if r <= fish_value then
				items = vlc_loot.get_loot({ items = vlc_fishing.loot_fish, stacks_min = 1, stacks_max = 1 }, pr)
				awards.unlock(player:get_player_name(), "vlc:fishyBusiness")
			elseif r <= junk_value then
				items = vlc_loot.get_loot({ items = vlc_fishing.loot_junk, stacks_min = 1, stacks_max = 1 }, pr)
			else
				items = vlc_loot.get_loot({ items = vlc_fishing.loot_treasure, stacks_min = 1, stacks_max = 1 }, pr)
			end
			local item
			if #items >= 1 then
			...
			if inv:room_for_item("main", item) then
				...
				if item:get_name() == "vlc_mobitems:leather" then
					awards.unlock(player:get_player_name(), "vlc:killCow")
				end
			else
				minetest.add_item(pos, item)
			end

			vlc_experience.throw_xp(pos, math.random(1,6))
	...
	--If no bobber or flying_bobber exists then throw bobber.
		...
		bobbers[player] = vlc_throwing.throw("vlc_fishing:flying_bobber", vector.offset(playerpos, 0, 1.5, 0), dir, 15, player:get_player_name())
	end
end
...
function bobber_ENTITY:on_step(dtime)
	...
	if def.liquidtype == "source" and minetest.get_item_group(def.name, "water") ~= 0 then
		...
		elseif not self._waittime or self._waittime <= 0 then
			...
			local lure_enchantment = vlc_enchanting.get_enchantment(player:get_wielded_item(), "lure") or 0
...
minetest.register_entity("vlc_fishing:bobber_entity", bobber_ENTITY)

local flying_bobber_ENTITY={
	initial_properties = {
		physical = false,
		textures = {"vlc_fishing_bobber.png"}, --FIXME: Replace with correct texture.
		visual_size = {x=0.5, y=0.5},
	...
	get_staticdata = vlc_throwing.get_staticdata,
	on_activate = vlc_throwing.on_activate,
...
local function flying_bobber_on_step(self, dtime)
	...
	for _, obj in pairs(minetest.get_objects_inside_radius(pos, 0.4)) do
		...
		if ent and ent._vlc_fishing_hookable then
			bobbers[player] = minetest.add_entity(ent.object:get_pos(), "vlc_fishing:bobber_entity")
	...
	if self._lastpos.x~=nil then
		if (def and (def.walkable or def.liquidtype == "flowing" or def.liquidtype == "source")) or not def then
			if minetest.get_item_group(node.name, "water") > 0 then
				bobbers[player] = minetest.add_entity(self._lastpos, "vlc_fishing:bobber_entity")
				local ent = bobbers[player]:get_luaentity()
				if ent then
...
minetest.register_entity("vlc_fishing:flying_bobber_entity", flying_bobber_ENTITY)

vlc_throwing.register_throwable_object("vlc_fishing:flying_bobber", "vlc_fishing:flying_bobber_entity", 5)
...
-- Fishing Rod
minetest.register_tool("vlc_fishing:fishing_rod", {
	...
	inventory_image = "vlc_fishing_fishing_rod.png",
	wield_image = "vlc_fishing_fishing_rod.png^[transformFY^[transformR90",
