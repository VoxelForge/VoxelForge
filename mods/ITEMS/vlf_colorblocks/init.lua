...
minetest.register_node("vlf_colorblocks:hardened_clay", {
...
for color,colordef in pairs(vlf_dyes.colors) do
	...
	-- Node Definition
	minetest.register_node("vlf_colorblocks:hardened_clay_"..color, {
	...
	minetest.register_node("vlf_colorblocks:concrete_powder_"..color, {
		...
		tiles = {"vlf_colorblocks_concrete_powder_"..color..".png"},
		...
		on_construct  = function(pos)
			...
			if nb then
				...
				if def and def._vlf_colorblocks_harden_to then
					minetest.swap_node(pos,{name=def._vlf_colorblocks_harden_to})
				end
			end
		end,

		-- Specify the node to which this node will convert after getting in contact with water
		_vlf_colorblocks_harden_to = "vlf_colorblocks:concrete_"..color,
		_vlf_blast_resistance = 0.5,
		_vlf_hardness = 0.5,
	})

	minetest.register_node("vlf_colorblocks:concrete_"..color, {
		description = sdesc_c,
		_doc_items_longdesc = ldesc_c,
		_doc_items_create_entry = create_entry,
		_doc_items_entry_name = ename_c,
		tiles = {"vlf_colorblocks_concrete_"..color..".png"},
		groups = {handy=1,pickaxey=1, concrete=1,building_block=1, material_stone=1},
		is_ground_content = false,
		sounds = vlf_sounds.node_sound_stone_defaults(),
		_vlf_blast_resistance = 1.8,
		_vlf_hardness = 1.8,
	})

	local tex = "vlf_colorblocks_glazed_terracotta_"..color..".png"
	local texes = { tex, tex, tex.."^[transformR180", tex, tex.."^[transformR270", tex.."^[transformR90" }
	minetest.register_node("vlf_colorblocks:glazed_terracotta_"..color, {
	...
	if not is_canonical and doc_mod then
		doc.add_entry_alias("nodes", "vlf_colorblocks:hardened_clay_"..canonical_color, "nodes", "vlf_colorblocks:hardened_clay_"..color)
		doc.add_entry_alias("nodes", "vlf_colorblocks:glazed_terracotta_"..canonical_color, "nodes", "vlf_colorblocks:glazed_terracotta_"..color)
		doc.add_entry_alias("nodes", "vlf_colorblocks:concrete_"..canonical_color, "nodes", "vlf_colorblocks:concrete_"..color)
		doc.add_entry_alias("nodes", "vlf_colorblocks:concrete_powder_"..canonical_color, "nodes", "vlf_colorblocks:concrete_powder_"..color)
	end

	-- Crafting recipes
	minetest.register_craft({
		output = "vlf_colorblocks:hardened_clay_"..color.." 8",
		recipe = {
				{"vlf_colorblocks:hardened_clay", "vlf_colorblocks:hardened_clay", "vlf_colorblocks:hardened_clay"},
				{"vlf_colorblocks:hardened_clay", "vlf_dyes:"..color, "vlf_colorblocks:hardened_clay"},
				{"vlf_colorblocks:hardened_clay", "vlf_colorblocks:hardened_clay", "vlf_colorblocks:hardened_clay"},
		},
	})
	minetest.register_craft({
		type = "shapeless",
		output = "vlf_colorblocks:concrete_powder_"..color.." 8",
		recipe = {
...
minetest.register_abm({
	...
	action = function(pos, node)
		local harden_to = minetest.registered_nodes[node.name]._vlf_colorblocks_harden_to
