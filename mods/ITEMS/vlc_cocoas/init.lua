...
local function cocoa_place(itemstack, placer, pt, plantname)
	...
	local rc = vlc_util.call_on_rightclick(itemstack, placer, pt)
	if rc then return rc end

	-- Check if pointing at jungle tree
	if under.name ~= "vlc_trees:tree_jungle"
...
function vlc_cocoas.grow(pos)
	local node = minetest.get_node(pos)
	if node.name == "vlc_cocoas:cocoa_1" then
		minetest.set_node(pos, {name = "vlc_cocoas:cocoa_2", param2 = node.param2})
	elseif node.name == "vlc_cocoas:cocoa_2" then
		minetest.set_node(pos, {name = "vlc_cocoas:cocoa_3", param2 = node.param2})
		return true
	end
	return false
end
...
local crop_def = {
	...
	drawtype = "mesh",
	mesh = "vlc_cocoas_cocoa_stage_0.obj",
	tiles = {"vlc_cocoas_cocoa_stage_0.png"},
	use_texture_alpha = minetest.features.use_texture_alpha_string_modes and "clip" or true,
	paramtype = "light",
	sunlight_propagates = true,
	paramtype2 = "facedir",
	drop = "vlc_cocoas:cocoa_beans",
	collision_box = {
		type = "fixed",
		fixed = {
	...
	_on_bone_meal = function(itemstack,placer,pointed_thing,pos,n)
		if n.name == "vlc_cocoas:cocoa_1" or n.name == "vlc_cocoas:cocoa_2" then
			return vlc_cocoas.grow(pos)
		end
		return false
	end,
}
...
-- 2nd stage
minetest.register_node("vlc_cocoas:cocoa_1", table.copy(crop_def))

crop_def.description = S("Medium Cocoa Pod")
crop_def._doc_items_create_entry = false
crop_def.groups.cocoa = 2
crop_def.mesh = "vlc_cocoas_cocoa_stage_1.obj"
crop_def.tiles = {"vlc_cocoas_cocoa_stage_1.png"}
...
minetest.register_node("vlc_cocoas:cocoa_2", table.copy(crop_def))
...
crop_def.mesh = "vlc_cocoas_cocoa_stage_2.obj"
crop_def.tiles = {"vlc_cocoas_cocoa_stage_2.png"}
crop_def.collision_box = {
	type = "fixed",
	fixed = {
...
crop_def.drop = "vlc_cocoas:cocoa_beans 3"
minetest.register_node("vlc_cocoas:cocoa_3", table.copy(crop_def))

minetest.register_craftitem("vlc_cocoas:cocoa_beans", {
	...
	inventory_image = "vlc_cocoas_cocoa_beans.png",
	groups = {craftitem = 1, compostability = 65},
	on_place = function(itemstack, placer, pointed_thing)
		return cocoa_place(itemstack, placer, pointed_thing, "vlc_cocoas:cocoa_1")
	end,
})

minetest.register_abm({
		label = "Cocoa pod growth",
		nodenames = {"vlc_cocoas:cocoa_1", "vlc_cocoas:cocoa_2"},
		-- Same as potatoes
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlc_cocoas:cocoa_1", "nodes", "vlc_cocoas:cocoa_2")
end
