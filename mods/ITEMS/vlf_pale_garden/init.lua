...
vlf_trees.register_wood("pale_oak",{
	...
	sapling=true,
	tree_schems_2x2 = {
		{file = schempath.."/schems/pale_oak/pale_oak_tree_1.mts", offset = vector.new(1,0,1)},
		{file = schempath.."/schems/pale_oak/pale_oak_tree_2.mts",},
	},
	tree = { 
...
--[[local function check_creaking_heart_activation(pos)
    ...
    if left_node.name == "vlf_trees:tree_pale_oak" and right_node.name == "vlf_trees:tree_pale_oak" then
        if left_node.param2 == facedir and right_node.param2 == facedir then
            return true
        end
    end

    if above_node.name == "vlf_trees:tree_pale_oak" and below_node.name == "vlf_trees:tree_pale_oak" then
...
local function check_creaking_heart_activation(pos)
    ...
    local valid_blocks = {
        ["vlf_trees:tree_pale_oak"] = true,
        ["vlf_trees:stripped_pale_oak"] = true,
        ["vlf_trees:bark_stripped_pale_oak"] = true,
        ["vlf_trees:bark_pale_oak"] = true
    }
...
minetest.register_node("vlf_pale_garden:active_creaking_heart", {
	...
	for _, object in pairs(minetest.get_objects_inside_radius(pos, 32)) do
		...
		if lua_entity and lua_entity.name == "mobs_mc:creaking_transient" and lua_entity._heart_pos and vector.equals(lua_entity._heart_pos, pos) then
...
minetest.register_abm({
	...
	action = function(pos, node)
		...
		if (time_of_day >= 0.8 or time_of_day <= 0.2) and not has_spawned then
			...
			-- Scan directly below the heart for the highest solid block
			for dy = 0, -50, -1 do
				...
				-- Check if the node below is solid, walkable, and the node above is air
				if node_below_def and node_below_def.walkable and minetest.get_item_group(node_below.name, "solid")
					and node_above.name == "air"
					and (minetest.get_node_light(node_above_pos, 0.5) or 0) > 5 then
			...
			if spawn_pos and not minetest.is_protected(spawn_pos, "") then
				local mob = vlf_mobs.spawn(spawn_pos, "mobs_mc:creaking_transient")
				if mob then
					...
					if lua_entity then
						...
						-- Particle effect
						local amount = 6
						local name = "vlf_particles_generic.png^[colorize:#A0A0A0:255"
