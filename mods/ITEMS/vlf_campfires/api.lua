...
function vlf_campfires.light_campfire(pos)
...
function vlf_campfires.take_item(pos, node, player, itemstack)
	if minetest.get_item_group(itemstack:get_name(), "campfire_cookable") ~= 0 then
		...
		if cookable then
			...
			local o = minetest.add_entity(pos + campfire_spots[spot], "vlf_campfires:food_entity")
			o:set_properties({
...
function vlf_campfires.register_campfire(name, def)
	-- Define Campfire
	minetest.register_node(name, {
		...
		_on_arrow_hit = function(pos, arrowent)
			if vlf_burning.is_burning(arrowent.object) then
				vlf_campfires.light_campfire(pos)
			end
		end,
		drop = "",
	...
	minetest.register_node(name.."_lit", {
		...
		on_rightclick = function (pos, node, player, itemstack, pointed_thing)
			...
			elseif minetest.get_item_group(itemstack:get_name(), "campfire_cookable") ~= 0 then
				vlf_campfires.take_item(pos, node, player, itemstack)
...
minetest.register_globalstep(function(dtime)
	...
	for _,pl in pairs(minetest.get_connected_players()) do
		...
		if pl and not pl:get_player_control().sneak and not vlf_enchanting.has_enchantment(armor_feet, "frost_walker") and not vlf_effects.has_effect(pl, "fire_resistance") then
			burn_in_campfire(pl)
		end
	end
...
function vlf_campfires.generate_smoke(pos)
	local smoke_timer
...
minetest.register_entity("vlf_campfires:food_entity", {
	initial_properties = {
		physical = false,
		visual = "wielditem",
...
minetest.register_abm({
	...
	interval = 2,
	chance = 2,
	action = vlf_campfires.generate_smoke,
})
