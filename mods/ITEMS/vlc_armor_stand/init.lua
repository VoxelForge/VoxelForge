...
local function spawn_stand_entity(pos, node)
	local luaentity = minetest.add_entity(pos, "vlc_armor_stand:armor_entity"):get_luaentity()
	if luaentity then
...
local function get_stand_entity(pos, node)
	for _, obj in ipairs(minetest.get_objects_inside_radius(pos, 0)) do
		...
		if luaentity and luaentity.name == "vlc_armor_stand:armor_entity" then
			return luaentity
		end
	end
...
minetest.register_node("vlc_armor_stand:armor_stand", {
	...
	tiles = {"default_wood.png", "vlc_stairs_stone_slab_top.png"},
	paramtype = "light",
	paramtype2 = "facedir",
	walkable = false,
	...
	on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
		...
		return vlc_armor.equip(itemstack, get_stand_entity(pos, node).object, true)
	end,
...
minetest.register_entity("vlc_armor_stand:armor_entity", {
	initial_properties = {
		physical = true,
		visual = "mesh",
	...
	on_activate = function(self)
		...
		vlc_armor.update(self.object)
	end,
	on_step = function(self, dtime)
		if minetest.get_node(self.node_pos).name ~= "vlc_armor_stand:armor_stand" then
			self.object:remove()
		end
	end,
...
minetest.register_lbm({
	...
	name = "vlc_armor_stand:respawn_entities",
	nodenames = {"vlc_armor_stand:armor_stand"},
...
minetest.register_craft({
	output = "vlc_armor_stand:armor_stand",
	recipe = {
		{"vlc_core:stick", "vlc_core:stick", "vlc_core:stick"},
		{"", "vlc_core:stick", ""},
		{"vlc_core:stick", "vlc_stairs:slab_stone", "vlc_core:stick"},
	}
})

-- Legacy handling
minetest.register_alias("3d_armor_stand:armor_stand", "vlc_armor_stand:armor_stand")
minetest.register_entity(":3d_armor_stand:armor_entity", {
	on_activate = function(self)
		minetest.log("action", "[vlc_armor_stand] Removing legacy entity: 3d_armor_stand:armor_entity")
		self.object:remove()
	end,
	static_save = false,
})
