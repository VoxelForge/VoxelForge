...
minetest.register_abm({
	...
	nodenames = {"vlc_lush_caves:spore_blossom"},
	interval = 25,
	chance = 10,
		...
		for _,pl in pairs(minetest.get_connected_players()) do
			if vector.distance(pos,pl:get_pos()) < PARTICLE_DISTANCE then
				minetest.add_particlespawner({
					texture = "vlc_lush_caves_spore_blossom_particle.png",
					amount = 32,
					time = 25,
					minvel = vector.zero(),
					maxvel = vector.zero(),
...
function vlc_lush_caves.makelake(pos,def,pr)
	...
	for i=1,pr:next(1,#nn) do
		minetest.set_node(nn[i],{name="vlc_core:water_source"})
		--[[
	...
	local nnn = minetest.find_nodes_in_area(p1,p2,{"vlc_core:water_source"})
	for k,v in pairs(nnn) do
		for kk,vv in pairs(adjacents) do
			local pp = vector.add(v,vv)
			local an = minetest.get_node(pp)
			if an.name ~= "vlc_core:water_source" then
				minetest.set_node(pp,{name="vlc_core:clay"})
				if pr:next(1,20) == 1 then
					minetest.set_node(vector.offset(pp,0,1,0),{name="vlc_lush_caves:moss_carpet"})
				end
			end
		end
	end
	...
	for _,d in pairs(dripleaves) do
		if minetest.get_item_group(minetest.get_node(d).name,"water") > 0 then
			minetest.set_node(vector.offset(d,0,-1,0),{name="vlc_lush_caves:dripleaf_big_waterroot"})
			minetest.registered_nodes["vlc_lush_caves:dripleaf_big_stem"].on_construct(d)
			for ii = 1, pr:next(1,4) do
				vlc_lush_caves.dripleaf_grow(d,{name = "vlc_lush_caves:dripleaf_big_stem"})
			end
		end
	end
	--]]
...
CONVERTS_TO_ROOTED_DIRT[minetest.get_content_id("vlc_core:dirt")] = true
CONVERTS_TO_ROOTED_DIRT[minetest.get_content_id("vlc_core:coarse_dirt")] = true
local CONTENT_HANGING_ROOTS = minetest.get_content_id("vlc_lush_caves:hanging_roots")
local CONTENT_ROOTED_DIRT = minetest.get_content_id("vlc_lush_caves:rooted_dirt")
...
function vlc_lush_caves.makeazalea(pos, def, pr)
	local distance = {x = 4, y = 40, z = 4}
	local airup = minetest.find_nodes_in_area_under_air(vector.offset(pos, 0, distance.y, 0), pos, {"vlc_core:dirt_with_grass"})
	...
	vm:calc_lighting()
	vm:write_to_map()
	minetest.log("info","[vlc_lush_caves] Azalea generated at "..minetest.pos_to_string(surface_pos))
	return true
end

minetest.register_abm({
	label = "Cave vines grow",
	nodenames = {"vlc_lush_caves:cave_vines_lit","vlc_lush_caves:cave_vines"},
	interval = 180,
	chance = 5,
	action = function(pos, node, active_object_count, active_object_count_wider)
		local pd1 = vector.offset(pos,0,-1,0)
		local pd2 = vector.offset(pos,0,-2,0)
		node.name = "vlc_lush_caves:cave_vines"
		if  math.random(5) == 1 then
			node.name="vlc_lush_caves:cave_vines_lit"
		end
		if minetest.get_node(pd1).name == "air" and minetest.get_node(pd2).name == "air" then
			minetest.swap_node(pd1,node)
		else
			minetest.swap_node(pos,{name="vlc_lush_caves:cave_vines_lit"})
		end
	end
})
...
vlc_structures.register_structure("clay_pool",{
	place_on = {"group:material_stone","vlc_core:gravel","vlc_lush_caves:moss","vlc_core:clay"},
	spawn_by = {"air"},
	num_spawn_by = 1,
	fill_ratio = 0.01,
	...
	y_max = -10,
	biomes = lushcaves,
	place_func = vlc_lush_caves.makelake,
})

local azaleas = {}
local az_limit = 500
vlc_structures.register_structure("azalea_tree",{
	place_on = {"group:material_stone","vlc_core:gravel","vlc_lush_caves:moss","vlc_core:clay"},
	spawn_by = {"air"},
	num_spawn_by = 1,
	fill_ratio = 0.15,
	...
	terrain_feature = true,
	y_max =0,
	y_min = vlc_vars.mg_overworld_min + 15,
	biomes = lushcaves,
	place_func = function(pos,def,pr)
		...
		if vlc_lush_caves.makeazalea(pos,def,pr) then
...
vlc_mapgen_core.register_generator("lush_caves",nil, function(minp, maxp, blockseed)
...
vlc_flowerpots.register_potted_flower("vlc_lush_caves:azalea", {
	name = "azalea",
	desc = S("Azalea Plant"),
	image = "vlc_lush_caves_azalea_side.png",
})

vlc_flowerpots.register_potted_flower("vlc_lush_caves:azalea_flowering", {
	name = "azalea_flowering",
	desc = S("Flowering Azalea Plant"),
	image = "vlc_lush_caves_azalea_flowering_side.png",
})
