...
minetest.register_craft({
	type = "shapeless",
	output = "vlf_books:book",
	recipe = { "vlf_core:paper", "vlf_core:paper", "vlf_core:paper", "vlf_mobitems:leather", }
})
...
local function make_description(title, author, generation)
	...
	desc = desc .. "\n" .. minetest.colorize(vlf_colors.GRAY, S("by @1", author))
	return desc
end
...
local function write(itemstack, user, pointed_thing)
	...
	minetest.show_formspec(user:get_player_name(), "vlf_books:writable_book", formspec)
end
...
local function read(itemstack, user, pointed_thing)
	...
	minetest.show_formspec(user:get_player_name(), "vlf_books:written_book", formspec)
end

-- Book and Quill
minetest.register_craftitem("vlf_books:writable_book", {
	...
	inventory_image = "vlf_books_book_writable.png",
	groups = { book = 1 },
	stack_max = 1,
	on_place = write,
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if ((formname == "vlf_books:writable_book") and fields and fields.text) then
		local stack = player:get_wielded_item()
		if (stack:get_name() and (stack:get_name() == "vlf_books:writable_book")) then
			...
			elseif fields.sign then
				...
				minetest.show_formspec(player:get_player_name(), "vlf_books:signing", formspec)
			end
		end
	elseif ((formname == "vlf_books:signing") and fields and fields.sign and fields.title) then
		local newbook = ItemStack("vlf_books:written_book")
		local book = player:get_wielded_item()
		local name = player:get_player_name()
		if book:get_name() == "vlf_books:writable_book" then
			...
			minetest.log("error", "[vlf_books] " .. name .. " failed to sign a book!")
		end
	elseif ((formname == "vlf_books:signing") and fields and fields.cancel) then
		local book = player:get_wielded_item()
		if book:get_name() == "vlf_books:writable_book" then
...
minetest.register_craft({
	type = "shapeless",
	output = "vlf_books:writable_book",
	recipe = { "vlf_books:book", "vlf_mobitems:ink_sac", "vlf_mobitems:feather" },
})

-- Written Book
minetest.register_craftitem("vlf_books:written_book", {
	...
	inventory_image = "vlf_books_book_written.png",
...
for i = 1, 8 do
	...
	minetest.register_craft({
		type = "shapeless",
		output = "vlf_books:written_book " .. i,
		recipe = rc,
	})
end
...
local function craft_copy_book(itemstack, player, old_craft_grid, craft_inv)
	...
	for i = 1, player:get_inventory():get_size("craft") do
		if old_craft_grid[i]:get_name() == "vlf_books:written_book" then
...
local function protection_check_put_take(pos, listname, index, stack, player)
	...
	if minetest.is_protected(pos, name) then
		minetest.record_protection_violation(pos, name)
		return 0
	elseif minetest.get_item_group(stack:get_name(), "book") ~= 0 or stack:get_name() == "vlf_enchanting:book_enchanted" then
...
local function bookshelf_gui(pos, node, clicker)
	...
	minetest.show_formspec(playername,
		"vlf_books:bookshelf_" .. pos.x .. "_" .. pos.y .. "_" .. pos.z,
		table.concat({
			"formspec_version[4]",
			"size[11.75,10.425]",
			...
			vlf_formspec.get_itemslot_bg_v4(0.375, 0.75, 9, 3, 0, "vlf_book_book_empty_slot.png"),
...
-- Bookshelf
minetest.register_node("vlf_books:bookshelf", {
	description = S("Bookshelf"),
	_doc_items_longdesc = S("Bookshelves are used for decoration."),
	tiles = { "vlf_books_bookshelf_top.png", "vlf_books_bookshelf_top.png", "default_bookshelf.png" },
