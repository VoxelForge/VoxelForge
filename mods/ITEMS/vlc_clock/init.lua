...
--[[
  vlc_clock, renew of the renew of the vlc_clock mod
...
vlc_clock.stereotype = "vlc_clock:clock"

vlc_clock.old_time = -1
...
for frame=0, clock_frames-1 do
	...
	table.insert(vlc_clock.images, "vlc_clock_clock_"..sframe..".png")
end
...
function vlc_clock.get_clock_frame()
...
-- Register items
function vlc_clock.register_item(name, image, creative, frame)
	local g = 1
	if creative then
		g = 0
	end
	local use_doc = name == vlc_clock.stereotype
	if doc_mod and not use_doc then
		doc.add_entry_alias("craftitems", vlc_clock.stereotype, "craftitems", name)
	end
...
minetest.register_globalstep(function(dtime)
	local now = vlc_clock.get_clock_frame()
	...
	if vlc_clock.old_time == now and force_clock_update_timer < 60 then
		return
	end
	force_clock_update_timer = 0

	vlc_clock.old_time = now
	vlc_clock.random_frame = random_frame
	...
	for p, player in pairs(minetest.get_connected_players()) do
		for s, stack in pairs(player:get_inventory():get_list("main")) do
			...
			if not vlc_worlds.clock_works(player:get_pos()) then
				frame = random_frame
			else
				frame = now
			end
			...
			if stack:get_name() == vlc_clock.stereotype then
				player:get_inventory():set_stack("main", s, "vlc_clock:clock_"..frame.." "..count)
...
minetest.register_on_craft(function(itemstack)
	if itemstack:get_name() == vlc_clock.stereotype then
		itemstack:set_name("vlc_clock:clock_"..vlc_clock.get_clock_frame())
	end
end)

-- Clock recipe
minetest.register_craft({
	output = vlc_clock.stereotype,
	recipe = {
		{"", "vlc_core:gold_ingot", ""},
...
-- Clock tool
vlc_clock.register_item(vlc_clock.stereotype, vlc_clock.images[1], true, 1)
...
for a=0,clock_frames-1,1 do
	...
	vlc_clock.register_item("vlc_clock:clock_"..tostring(a), vlc_clock.images[b+1], false, a+1)
end
