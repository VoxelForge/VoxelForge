...
local function active_brewing_formspec(fuel_percent, brew_percent)
	...
	return "size[9,8.75]"..
	"background[-0.19,-0.25;9.5,9.5;vlf_brewing_inventory.png]"..
	...
	vlf_formspec.get_itemslot_bg(0,7.75,9,1)..
	"list[context;fuel;0.5,1.75;1,1;]"..
	vlf_formspec.get_itemslot_bg(0.5,1.75,1,1).."image[0.5,1.75;1,1;vlf_brewing_fuel_bg.png]"..
	"list[context;input;2.75,0.5;1,1;]"..
	vlf_formspec.get_itemslot_bg(2.75,0.5,1,1)..
	"list[context;stand;4.5,2.5;1,1;]"..
	vlf_formspec.get_itemslot_bg(4.5,2.5,1,1).."image[4.5,2.5;1,1;vlf_brewing_bottle_bg.png]"..
	"list[context;stand;6,2.8;1,1;1]"..
	vlf_formspec.get_itemslot_bg(6,2.8,1,1).."image[6,2.8;1,1;vlf_brewing_bottle_bg.png]"..
	"list[context;stand;7.5,2.5;1,1;2]"..
	vlf_formspec.get_itemslot_bg(7.5,2.5,1,1).."image[7.5,2.5;1,1;vlf_brewing_bottle_bg.png]"..

	"image[2.7,3.33;1.28,0.41;vlf_brewing_burner.png^[lowpart:"..
	(100-fuel_percent)..":vlf_brewing_burner_active.png^[transformR270]"..

	"image[2.76,1.4;1,2.15;vlf_brewing_bubbles.png^[lowpart:"..
	(brew_percent)..":vlf_brewing_bubbles_active.png]"..
...
local brewing_formspec = "size[9,8.75]"..
	...
	"image[2.7,3.33;1.28,0.41;vlf_brewing_burner.png^[transformR270]"..
	"image[2.76,1.4;1,2.15;vlf_brewing_bubbles.png]"..
...
local function brewable(inv)
	...
	for i=1,stand_size do
		...
		alchemy = vlf_effects.get_alchemy(ingredient, bottle)
			...
			if stand_timer >= BREW_TIME then
				...
				for i=1, inv:get_size("stand") do
					if brew_output[i] then
						...
						minetest.sound_play("vlf_effects_bottle_pour", {pos=pos, gain=0.6, max_hear_range=6}, true)
					end
				end
				stand_timer = 0
...
local tiles = {
	"vlf_brewing_top.png", 	--top
	"vlf_brewing_base.png", 	--bottom
	"vlf_brewing_side.png", 	--right
	"vlf_brewing_side.png", 	--left
	"vlf_brewing_side.png", 	--back
	"vlf_brewing_side.png^[transformFX",   --front
}

local function sort_stack(stack)
	if stack:get_name() == "vlf_mobitems:blaze_powder" then
		return "fuel"
	end
...
local function allow_take(pos, listname, index, stack, player)
	...
	if minetest.is_protected(pos, name) then
		...
		if listname == "stand" then
			awards.unlock(name, "vlf:localBrewery")
		end
		return stack:get_count()
	end
end
...
local function hopper_in(pos, to_pos)
	...
	if pos.y == to_pos.y then
		local slot_id,_ = vlf_util.get_eligible_transfer_item_slot(sinv, "main", dinv, "fuel", function(itemstack)
			return itemstack:get_name() == "vlf_mobitems:blaze_powder"
		end)
		if slot_id then
			vlf_util.move_item(sinv, "main", slot_id, dinv, "fuel")
	...
	local slot_id,_ = vlf_util.get_eligible_transfer_item_slot(sinv, "main", dinv, "input", function(itemstack)
		return minetest.get_item_group(itemstack:get_name(), "brewing_ingredient" ) > 0
	end)
	if slot_id then
		vlf_util.move_item(sinv, "main", slot_id, dinv, "input")
		minetest.get_node_timer(to_pos):start(1.0)
	end
	return true
end
...
minetest.register_node("vlf_brewing:stand_100", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
...
minetest.register_node("vlf_brewing:stand_010", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
...
minetest.register_node("vlf_brewing:stand_001", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_node("vlf_brewing:stand_110", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_node("vlf_brewing:stand_101", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_node("vlf_brewing:stand_011", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_node("vlf_brewing:stand_111", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_craft({
	output = "vlf_brewing:stand_000",
	recipe = {
		{ "", "vlf_mobitems:blaze_rod", "" },
		{ "group:cobble", "group:cobble", "group:cobble" },
	}
})

minetest.register_alias("vlf_brewing:stand", "vlf_brewing:stand_000")

if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlf_brewing:stand_000", "nodes", "vlf_brewing:stand_001")
	doc.add_entry_alias("nodes", "vlf_brewing:stand_000", "nodes", "vlf_brewing:stand_010")
	doc.add_entry_alias("nodes", "vlf_brewing:stand_000", "nodes", "vlf_brewing:stand_011")
	doc.add_entry_alias("nodes", "vlf_brewing:stand_000", "nodes", "vlf_brewing:stand_100")
	doc.add_entry_alias("nodes", "vlf_brewing:stand_000", "nodes", "vlf_brewing:stand_101")
	doc.add_entry_alias("nodes", "vlf_brewing:stand_000", "nodes", "vlf_brewing:stand_110")
	doc.add_entry_alias("nodes", "vlf_brewing:stand_000", "nodes", "vlf_brewing:stand_111")
end
