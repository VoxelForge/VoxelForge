...
local patterns = {
	...
	["thing"] = {
		...
		type = "shapeless",
		{ e, "vlf_banners:pattern_thing", d },
	},
	["globe"] = {
		name = NS("@1 Globe Charge"),
		type = "shapeless",
		{ e, "vlf_banners:pattern_globe", d },
	},
	["piglin"] = {
		name = NS("@1 Piglin Charge"),
		type = "shapeless",
		{ e, "vlf_banners:pattern_piglin", d },
	},
	["rhombus"] = {
		name = NS("@1 Lozenge"),
		{ e, d, e },
...
function vlf_banners.register_pattern(name,recipe)
	vlf_banners.patterns[name] = table.merge({ name = readable_name(name) }, recipe)
end
...
function vlf_banners.make_advanced_banner_description(description, layers)
	if layers == nil or #layers == 0 then
		...
		description = description .. "\n" .. minetest.colorize(vlf_colors.GRAY, append)
		return description
	end
end
...
local function banner_pattern_craft(itemstack, player, old_craft_grid, craft_inv, craft_predict)
	...
	if banner2 then
		...
		imeta:set_string("description", vlf_banners.make_advanced_banner_description(src_desc, src_layers))
	...
	if mname == "" then
		...
		local description = vlf_banners.make_advanced_banner_description(odesc, layers)
	...
	if craft_predict then
		local itemid_prefix = "vlf_banners:banner_preview"
...
for pattern_name, pattern in pairs(patterns) do
	...
	if pattern.type == nil then
		for colorid, colortab in pairs(vlf_banners.colors) do
			local banner = "vlf_banners:banner_item_"..colortab[1]
