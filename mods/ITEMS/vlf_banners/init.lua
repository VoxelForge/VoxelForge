...
local node_sounds
if minetest.get_modpath("vlf_sounds") then
	node_sounds = vlf_sounds.node_sound_wood_defaults()
end
...
vlf_banners.colors = {
	...
	["unicolor_orange"] =     {"orange",     S("Orange Banner"),     "vlf_wool:orange", vlf_dyes.colors.orange.rgb, "vlf_dyes:orange", S("Orange") },
	["unicolor_violet"] =     {"purple",     S("Purple Banner"),     "vlf_wool:purple", vlf_dyes.colors.purple.rgb, "vlf_dyes:violet", S("Violet") },
	["unicolor_brown"] =      {"brown",      S("Brown Banner"),      "vlf_wool:brown", vlf_dyes.colors.brown.rgb, "vlf_dyes:brown", S("Brown") },
...
for k,v in pairs(vlf_banners.colors) do
	colors_reverse["vlf_banners:banner_item_"..v[1]] = k
end

function vlf_banners.color_reverse(itemname)
...
local function on_dig_banner(pos, node, digger)
	...
	local item_str = item:is_empty() and "vlf_banners:banner_item_white"
		or item:to_string()
...
local function on_destruct_banner(pos, hanging)
	...
	if hanging then
		...
		nodename = "vlf_banners:hanging_banner"
	else
		offset = standing_banner_entity_offset
		nodename = "vlf_banners:standing_banner"
	end
...
function vlf_banners.make_banner_texture(base_color, layers)
	local colorize
	if vlf_banners.colors[base_color] then
		colorize = vlf_banners.colors[base_color][4]
	end
	if colorize then
		-- Base texture with base color
		local base = "(vlf_banners_banner_base.png^[mask:vlf_banners_base_inverted.png)^((vlf_banners_banner_base.png^[colorize:"..colorize..":"..base_color_ratio..")^[mask:vlf_banners_base.png)"
		...
		if layers then
			...
			for l=1, #layers do
				...
				if layerinfo and layerinfo.pattern and layerinfo.color and vlf_banners.colors[layerinfo.color] then
					local pattern = "vlf_banners_" .. layerinfo.pattern .. ".png"
					local color = vlf_banners.colors[layerinfo.color][4]
		...
		return "vlf_banners_banner_base.png"
	end
end
...
local function spawn_banner_entity(pos, hanging, itemstack)
	local banner
	if hanging then
		banner = minetest.add_entity(pos, "vlf_banners:hanging_banner")
	else
		banner = minetest.add_entity(pos, "vlf_banners:standing_banner")
	end
	if banner == nil then
		return banner
	end
	...
	local colorid = vlf_banners.color_reverse(itemstack:get_name())
...
local function respawn_banner_entity(pos, node, force)
	local hanging = node.name == "vlf_banners:hanging_banner"
	local offset
	if hanging then
		offset = hanging_banner_entity_offset
	else
...
minetest.register_node("vlf_banners:standing_banner", {
	_doc_items_entry_name = S("Banner"),
	_doc_items_image = "vlf_banners_item_base.png^vlf_banners_item_overlay.png",
...
S("You can copy the pattern of a banner by placing two banners of the same color in the crafting grid—one needs to be emblazoned, the other one must be clean. Finally, you can use a banner on a cauldron with water to wash off its top-most layer."),
	...
	tiles = { "vlf_banners_fallback_wood.png" },

	inventory_image = "vlf_banners_item_base.png",
...
-- Hanging banner node
minetest.register_node("vlf_banners:hanging_banner", {
	walkable = false,
	is_ground_content = false,
	paramtype = "light",
...
for colorid, colortab in pairs(vlf_banners.colors) do
	...
	if pattern_name == "" then
		itemstring = "vlf_banners:banner_item_" .. itemid
	else
		itemstring = "vlf_banners:banner_preview" .. "_" .. pattern_name .. "_" .. itemid
	end
	...
	if pattern_name == "" then
	    if colorize then
		-- Base texture with base color
		base = "vlf_banners_item_base.png^(vlf_banners_item_overlay.png^[colorize:"..colorize..")^[resize:32x32"
	    else
		base = "vlf_banners_item_base.png^vlf_banners_item_overlay.png^[resize:32x32"
	    end
	    finished_banner = base
	else
		-- Banner item preview background
		base = "vlf_banners_item_base.png^(vlf_banners_item_overlay.png^[colorize:#CCCCCC)^[resize:32x32"

		desc = S("Preview Banner")

		local pattern = "vlf_banners_" .. pattern_name .. ".png"
	...
	minetest.register_craftitem(itemstring, {
		...
		on_place = function(itemstack, placer, pointed_thing)
				...
				if minetest.get_modpath("vlf_cauldrons") then
					...
					if new_node then
						...
						if type(layers) == "table" and #layers > 0 then
							...
							local newdesc = vlf_banners.make_advanced_banner_description(itemstack:get_definition().description, layers)
							local mname = imeta:get_string("name")
						...
						-- Play sound (from vlf_potions mod)
			...
			local _, success = minetest.item_place_node(ItemStack("vlf_banners:standing_banner"), placer, pointed_thing)
			if not success then
				-- Forbidden on ceiling
				if pointed_thing.under.y ~= pointed_thing.above.y then
					return itemstack
				end
				_, success = minetest.item_place_node(ItemStack("vlf_banners:hanging_banner"), placer, pointed_thing)
				if not success then
					return itemstack
				end
				hanging = true
			...
			if bnode.name ~= "vlf_banners:standing_banner" and bnode.name ~= "vlf_banners:hanging_banner" then
				minetest.log("error", "[vlf_banners] The placed banner node is not what the mod expected!")
				return itemstack
			end
		...
		_vlf_generate_description = function(itemstack)
			...
			local newdesc = vlf_banners.make_advanced_banner_description(desc, layers)
			meta:set_string("description", newdesc)
			return newdesc
		end,
	})

	if mod_vlf_core and minetest.get_modpath("vlf_wool") and pattern_name == "" then
	...
	if mod_doc then
		...
		doc.add_entry_alias("nodes", "vlf_banners:standing_banner", "craftitems", itemstring)
	end
    end
end
...
if mod_doc then
	-- Add item to node alias
	doc.add_entry_alias("nodes", "vlf_banners:standing_banner", "nodes", "vlf_banners:hanging_banner")
end
...
local entity_standing = {
	initial_properties = {
		...
		textures = {vlf_banners.make_banner_texture()},
		pointable = false,
	},
	...
	on_activate = function(self, staticdata)
		if staticdata and staticdata ~= "" then
			...
			self.object:set_properties({
				textures = {vlf_banners.make_banner_texture(self._base_color, self._layers)},
			})
		end
	...
	-- * other parameters: Same meaning as in vlf_banners.make_banner_texture
...
minetest.register_lbm({
	...
	name = "vlf_banners:respawn_entities",
	run_at_every_load = true,
	nodenames = {"vlf_banners:standing_banner", "vlf_banners:hanging_banner"},
