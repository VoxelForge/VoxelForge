...
vlc_deepslate = {}
vlc_deepslate.translator = minetest.get_translator(modname)
local S = vlc_deepslate.translator

function vlc_deepslate.register_deepslate_ore(item, desc, extra, basename)
	local nodename = "vlc_deepslate:deepslate_"..item.."_ore"
	local basename = basename or "vlc_core:"..item.."_ore"
	...
	def.tiles = { "vlc_deepslate_" .. item .. "_ore.png" }
...
function vlc_deepslate.register_variants(name, defs)
	assert(name, "[vlc_deepslate] vlc_deepslate.register_variants called without a valid name, refer to API.md in vlc_deepslate.")
	assert(defs.basename, "[vlc_deepslate] vlc_deepslate.register_variants needs a basename field to work, refer to API.md in vlc_deepslate.")
	assert(defs.basetiles, "[vlc_deepslate] vlc_deepslate.register_variants needs a basetiles field to work, refer to API.md in vlc_deepslate.")

	local main_itemstring = "vlc_deepslate:"..defs.basename.."_"..name
	...
	if defs.node and defs.stair then
		vlc_stairs.register_stair(defs.basename.."_"..name, {
	...
	if defs.node and defs.slab then
		vlc_stairs.register_slab(defs.basename.."_"..name, {
	...
	if defs.node and defs.wall then
		vlc_walls.register_wall("vlc_deepslate:"..defs.basename.."_"..name.."wall", defs.wall.description, main_itemstring, nil, nil, nil, nil, defs.wall)
	end
end

function vlc_deepslate.register_polished_variant(name, defs)
	assert(name, "[vlc_deepslate] vlc_deepslate.register_variants called without a valid name, refer to API.md in vlc_deepslate.")
	assert(defs.basename, "[vlc_deepslate] vlc_deepslate.register_variants needs a basename field to work, refer to API.md in vlc_deepslate.")
	assert(defs.basetiles, "[vlc_deepslate] vlc_deepslate.register_variants needs a basetiles field to work, refer to API.md in vlc_deepslate.")

	local main_itemstring = "vlc_deepslate:"..defs.basename
	local main_def = table.merge({
		_doc_items_hidden = false,
		tiles = { defs.basetiles.."_"..name..".png" },
		is_ground_content = false,
