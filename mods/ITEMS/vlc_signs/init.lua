...
local signs_editable = minetest.settings:get_bool("vlc_signs_editable", false)
...
assert(chars_file,"[vlc_signs] characters.txt not found")
local charmap = {}
while true do
...
function vlc_signs.create_lines(text)
	local line_num = 1
	local text_table = {}
...
function vlc_signs.generate_line(s, ypos)
	local i = 1
	local parsed = {}
	local width = 0
	local chars = 0
...
function vlc_signs.generate_texture(data)
	local lines = vlc_signs.create_lines(data.text or "")
	local texture = "[combine:" .. SIGN_WIDTH .. "x" .. SIGN_WIDTH
	local ypos = 0
	local letter_color = data.color or DEFAULT_COLOR

	for i = 1, #lines do
		texture = texture .. vlc_signs.generate_line(lines[i], ypos)
...
function sign_tpl.on_place(itemstack, placer, pointed_thing)
	...
	if vlc_util.call_on_rightclick(itemstack, placer, pointed_thing) then
		return itemstack
	end
	...
	if wdir ~= 0 and wdir ~= 1 then
		placestack:set_name("vlc_signs:wall_sign_"..def._vlc_sign_wood)
		itemstack, pos = minetest.item_place(placestack, placer, pointed_thing, wdir)
	elseif wdir == 1 then -- standing, not ceiling
		placestack:set_name("vlc_signs:standing_sign_"..def._vlc_sign_wood)
	...
	vlc_signs.show_formspec(placer, pos)
	itemstack:set_name(itemstring)
	return itemstack
end

function sign_tpl.on_rightclick(pos, node, clicker, itemstack, pointed_thing)
	if itemstack:get_name() == "vlc_mobitems:glow_ink_sac" then
		...
		vlc_signs.update_sign(pos)
		if not minetest.is_creative_enabled(clicker:get_player_name()) then
			itemstack:take_item()
		end
	elseif signs_editable then
		if not vlc_util.check_position_protection(pos, clicker) then
			vlc_signs.show_formspec(clicker, pos)
		end
	end
	return itemstack
end
...
function sign_tpl.on_destruct(pos)
	vlc_signs.get_text_entity (pos, true)
end

function sign_tpl._on_dye_place(pos,color)
	set_signmeta(pos,{
		color = vlc_dyes.colors[color].rgb
	})
	vlc_signs.update_sign(pos)
end

local sign_wall = table.merge(sign_tpl,{
	mesh = "vlc_signs_signonwallmount.obj",
...
--Formspec
function vlc_signs.show_formspec(player, pos)
	if not pos then return end
	local old_text = minetest.get_meta(pos):get_string("text")
	minetest.show_formspec(player:get_player_name(),
		"vlc_signs:set_text_" .. pos.x .. "_" .. pos.y .. "_" .. pos.z,
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname:find("vlc_signs:set_text_") == 1 then
		local x, y, z = formname:match("vlc_signs:set_text_(.-)_(.-)_(.*)")
		local pos = { x = tonumber(x), y = tonumber(y), z = tonumber(z) }
		if not pos or not pos.x or not pos.y or not pos.z then
			return
		end
		if not vlc_util.check_position_protection(pos, player) and (signs_editable or minetest.get_meta(pos):get_string("text") == "") then
			set_signmeta(pos,{
				text = fields.text,
			})
			vlc_signs.update_sign(pos)
...
function vlc_signs.get_text_entity (pos, force_remove)
	local objects = minetest.get_objects_inside_radius(pos, 0.5)
	local text_entity
	for _, v in pairs(objects) do
		local ent = v:get_luaentity()
		if ent and ent.name == "vlc_signs:text" then
...
function vlc_signs.update_sign(pos)
	local data = get_signdata(pos)

	local text_entity = vlc_signs.get_text_entity(pos)
	...
	elseif not text_entity then
		text_entity = minetest.add_entity(data.text_pos, "vlc_signs:text")
	...
	text_entity:set_properties({
		textures = { vlc_signs.generate_texture(data) },
		glow = glow,
	})
...
minetest.register_lbm({
	...
	name = "vlc_signs:restore_entities",
...
minetest.register_entity("vlc_signs:text", {
	initial_properties = {
		pointable = false,
...
vlc_signs.old_rotnames = {}

function vlc_signs.register_sign(name,color,def)
	local newfields = {
		tiles = { colored_texture("vlc_signs_sign_greyscale.png", color) },
		...
		drop = "vlc_signs:wall_sign_"..name,
		_vlc_sign_wood = name,
	}

	minetest.register_node(":vlc_signs:standing_sign_"..name, table.merge(sign_tpl, newfields, def or {}))
	minetest.register_node(":vlc_signs:wall_sign_"..name,table.merge(sign_wall, newfields, def or {}))

	table.insert(vlc_signs.old_rotnames,"vlc_signs:standing_sign22_5_"..name)
	table.insert(vlc_signs.old_rotnames,"vlc_signs:standing_sign45_"..name)
	table.insert(vlc_signs.old_rotnames,"vlc_signs:standing_sign67_5_"..name)
end
