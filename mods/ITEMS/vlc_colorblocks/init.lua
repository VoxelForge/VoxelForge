...
minetest.register_node("vlc_colorblocks:hardened_clay", {
...
minetest.register_craft({
	type = "cooking",
	output = "vlc_colorblocks:hardened_clay",
	recipe = "vlc_core:clay",
	cooktime = 10,
})
...
for color,colordef in pairs(vlc_dyes.colors) do
	...
	-- Node Definition
	minetest.register_node("vlc_colorblocks:hardened_clay_"..color, {
	...
	minetest.register_node("vlc_colorblocks:concrete_powder_"..color, {
		description = sdesc_cp,
		_tt_help = ltt_cp,
		_doc_items_longdesc = ldesc_cp,
		_doc_items_create_entry = create_entry,
		_doc_items_entry_name = ename_cp,
		tiles = {"vlc_colorblocks_concrete_powder_"..color..".png"},
		...
		on_construct  = function(pos)
			...
			if nb then
				...
				if def and def._vlc_colorblocks_harden_to then
					minetest.swap_node(pos,{name=def._vlc_colorblocks_harden_to})
				end
			end
		end,

		-- Specify the node to which this node will convert after getting in contact with water
		_vlc_colorblocks_harden_to = "vlc_colorblocks:concrete_"..color,
		_vlc_blast_resistance = 0.5,
		_vlc_hardness = 0.5,
	})

	minetest.register_node("vlc_colorblocks:concrete_"..color, {
		description = sdesc_c,
		_doc_items_longdesc = ldesc_c,
		_doc_items_create_entry = create_entry,
		_doc_items_entry_name = ename_c,
		tiles = {"vlc_colorblocks_concrete_"..color..".png"},
		groups = {handy=1,pickaxey=1, concrete=1,building_block=1, material_stone=1},
		is_ground_content = false,
		sounds = vlc_sounds.node_sound_stone_defaults(),
		_vlc_blast_resistance = 1.8,
		_vlc_hardness = 1.8,
	})

	local tex = "vlc_colorblocks_glazed_terracotta_"..color..".png"
	local texes = { tex, tex, tex.."^[transformR180", tex, tex.."^[transformR270", tex.."^[transformR90" }
	minetest.register_node("vlc_colorblocks:glazed_terracotta_"..color, {
	...
	if not is_canonical and doc_mod then
		doc.add_entry_alias("nodes", "vlc_colorblocks:hardened_clay_"..canonical_color, "nodes", "vlc_colorblocks:hardened_clay_"..color)
		doc.add_entry_alias("nodes", "vlc_colorblocks:glazed_terracotta_"..canonical_color, "nodes", "vlc_colorblocks:glazed_terracotta_"..color)
		doc.add_entry_alias("nodes", "vlc_colorblocks:concrete_"..canonical_color, "nodes", "vlc_colorblocks:concrete_"..color)
		doc.add_entry_alias("nodes", "vlc_colorblocks:concrete_powder_"..canonical_color, "nodes", "vlc_colorblocks:concrete_powder_"..color)
	end

	-- Crafting recipes
	minetest.register_craft({
		output = "vlc_colorblocks:hardened_clay_"..color.." 8",
		recipe = {
				{"vlc_colorblocks:hardened_clay", "vlc_colorblocks:hardened_clay", "vlc_colorblocks:hardened_clay"},
				{"vlc_colorblocks:hardened_clay", "vlc_dyes:"..color, "vlc_colorblocks:hardened_clay"},
				{"vlc_colorblocks:hardened_clay", "vlc_colorblocks:hardened_clay", "vlc_colorblocks:hardened_clay"},
		},
	})
	minetest.register_craft({
		type = "shapeless",
		output = "vlc_colorblocks:concrete_powder_"..color.." 8",
		recipe = {
			"vlc_core:sand", "vlc_core:gravel", "vlc_core:sand",
			"vlc_core:gravel", "vlc_dyes:"..color, "vlc_core:gravel",
			"vlc_core:sand", "vlc_core:gravel", "vlc_core:sand",
		}
	})
	...
	minetest.register_craft({
		type = "cooking",
		output = "vlc_colorblocks:glazed_terracotta_"..color,
		recipe = "vlc_colorblocks:hardened_clay_"..color,
		cooktime = 10,
	})
end
...
minetest.register_abm({
	...
	action = function(pos, node)
		local harden_to = minetest.registered_nodes[node.name]._vlc_colorblocks_harden_to
