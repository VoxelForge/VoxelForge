...
local function on_place_fish(itemstack, placer, pointed_thing)
	...
	if ( def and def.buildable_to ) or n.name == "vlf_portals:portal" then
	...
	if fish_names[fish] then
		...
		if o and o:get_pos() then
			...
			local water = "vlf_core:water_source"
			...
			if not placer or not minetest.is_creative_enabled(placer:get_player_name()) then
				itemstack = ItemStack("vlf_buckets:bucket_empty")
			end
		end
	end
	return itemstack
...
for techname, fishname in pairs(fish_names) do
	...
	minetest.register_alias("vlf_fishing:bucket_" .. techname, "vlf_buckets:bucket_" .. techname)
end
