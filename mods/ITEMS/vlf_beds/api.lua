...
local function kick_player_after_destruct(destruct_pos)
	for name, player_bed_pos in pairs(vlf_beds.bed_pos) do
...
local default_sounds = vlf_sounds.node_sound_wood_defaults({
	footstep = vlf_sounds.node_sound_wool_defaults().footstep,
})

function vlf_beds.register_bed(name, def)
	local common_box = {
			type = "fixed",
