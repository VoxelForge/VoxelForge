local S = minetest.get_translator("vlc_barrels")
...
local function barrel_open(pos, node, clicker)
	...
	minetest.show_formspec(playername,
		"vlc_barrels:barrel_" .. pos.x .. "_" .. pos.y .. "_" .. pos.z,
		table.concat({
			"formspec_version[4]",
			"size[11.75,10.425]",
	...
	minetest.swap_node(pos, { name = "vlc_barrels:barrel_open", param2 = node.param2 })
	open_barrels[playername] = pos
	minetest.sound_play({ name = "vlc_barrels_default_barrel_open" }, { pos = pos, gain = 0.5, max_hear_distance = 16 }, true)
end

local function close_forms(pos)
	local players = minetest.get_connected_players()
	local formname = "vlc_barrels:barrel_" .. pos.x .. "_" .. pos.y .. "_" .. pos.z
	for p = 1, #players do
...
local function update_after_close(pos)
	...
	if node.name == "vlc_barrels:barrel_open" then
		minetest.swap_node(pos, { name = "vlc_barrels:barrel_closed", param2 = node.param2 })
		minetest.sound_play({ name = "vlc_barrels_default_barrel_close" }, { pos = pos, gain = 0.5, max_hear_distance = 16 }, true)
	end
end
...
minetest.register_node("vlc_barrels:barrel_closed", {
	...
	tiles = { "vlc_barrels_barrel_top.png^[transformR270", "vlc_barrels_barrel_bottom.png", "vlc_barrels_barrel_side.png" },
...
minetest.register_node("vlc_barrels:barrel_open", {
	description = S("Barrel Open"),
	_tt_help = S("27 inventory slots"),
	_doc_items_longdesc = S("Barrels are containers which provide 27 inventory slots."),
	_doc_items_usagehelp = S("To access its inventory, rightclick it. When broken, the items will drop out."),
	_doc_items_create_entry = false,
	tiles = { "vlc_barrels_barrel_top_open.png", "vlc_barrels_barrel_bottom.png", "vlc_barrels_barrel_side.png" },
	is_ground_content = false,
	paramtype = "light",
	paramtype2 = "facedir",
	drop = "vlc_barrels:barrel_closed",
	sounds = vlc_sounds.node_sound_wood_defaults(),
	groups = {
		handy = 1,
		axey = 1,
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname:find("vlc_barrels:") == 1 and fields.quit then
		close_barrel(player)
	end
end)
...
minetest.register_craft({
	output = "vlc_barrels:barrel_closed",
	recipe = {
...
minetest.register_craft({
	type = "fuel",
	recipe = "vlc_barrels:barrel_closed",
	burntime = 15,
})
