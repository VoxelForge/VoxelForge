...
local snowball_ENTITY={
	initial_properties = {
		physical = false,
		textures = {"vlc_throwing_snowball.png"},
...
local pearl_ENTITY={
	initial_properties = {
		physical = false,
		textures = {"vlc_throwing_ender_pearl.png"},
...
local function snowball_on_step(self, dtime)
	...
	if self._lastpos.x~=nil then
		if (def and def.walkable) or not def then
			minetest.sound_play("vlc_throwing_snowball_impact_hard", { pos = pos, max_hear_distance=16, gain=0.7 }, true)
			snowball_particles(self._lastpos, vel)
			self.object:remove()
			if mod_target and node.name == "vlc_target:target_off" then
	...
	if check_object_hit(self, pos, {snowball_vulnerable = 3}) then
		minetest.sound_play("vlc_throwing_snowball_impact_soft", { pos = pos, max_hear_distance=16, gain=0.7 }, true)
...
local function egg_on_step(self, dtime)
	...
	if self._lastpos.x then
		if (def and def.walkable) or not def then
			...
			if r == 1 then
				vlc_mobs.spawn_child(self._lastpos, "mobs_mc:chicken")
				...
				if r == 1 then
					...
					for o=1, 3 do
						...
						vlc_mobs.spawn_child(pos, "mobs_mc:chicken")
					end
				end
			end
			minetest.sound_play("vlc_throwing_egg_impact", { pos = self.object:get_pos(), max_hear_distance=10, gain=0.5 }, true)
			self.object:remove()
			if mod_target and node.name == "vlc_target:target_off" then
				vlc_target.hit(vector.round(pos), 0.4) --4 redstone ticks
			end
...
local function pearl_on_step(self, dtime)
	...
	if self._lastpos.x~=nil then
		...
		elseif walkable or nn == "vlc_core:vine" or nn == "vlc_core:deadbush" or minetest.get_item_group(nn, "flower") ~= 0 or minetest.get_item_group(nn, "sapling") ~= 0 or minetest.get_item_group(nn, "plant") ~= 0 or minetest.get_item_group(nn, "mushroom") ~= 0 or not def then
...
minetest.register_entity("vlc_throwing:snowball_entity", snowball_ENTITY)
minetest.register_entity("vlc_throwing:egg_entity", egg_ENTITY)
minetest.register_entity("vlc_throwing:ender_pearl_entity", pearl_ENTITY)


local how_to_throw = S("Use the punch key to throw.")

-- Snowball
minetest.register_craftitem("vlc_throwing:snowball", {
	...
	inventory_image = "vlc_throwing_snowball.png",
	stack_max = 16,
	groups = { weapon_ranged = 1 },
	on_use = vlc_throwing.get_player_throw_function("vlc_throwing:snowball_entity"),
	_on_dispense = vlc_throwing.dispense_function,
})

-- Egg
minetest.register_craftitem("vlc_throwing:egg", {
	description = S("Egg"),
	...
	_doc_items_usagehelp = how_to_throw,
	inventory_image = "vlc_throwing_egg.png",
	stack_max = 16,
	on_use = vlc_throwing.get_player_throw_function("vlc_throwing:egg_entity"),
	_on_dispense = vlc_throwing.dispense_function,
...
-- Ender Pearl
minetest.register_craftitem("vlc_throwing:ender_pearl", {
	description = S("Ender Pearl"),
	_tt_help = S("Throwable").."\n"..minetest.colorize(vlc_colors.YELLOW, S("Teleports you on impact for cost of 5 HP")),
	...
	wield_image = "vlc_throwing_ender_pearl.png",
	inventory_image = "vlc_throwing_ender_pearl.png",
	stack_max = 16,
	on_place = vlc_throwing.get_player_throw_function("vlc_throwing:ender_pearl_entity"),
	on_secondary_use = vlc_throwing.get_player_throw_function("vlc_throwing:ender_pearl_entity"),
	groups = { transport = 1 },
})

vlc_throwing.register_throwable_object("vlc_throwing:snowball", "vlc_throwing:snowball_entity", 22)
vlc_throwing.register_throwable_object("vlc_throwing:egg", "vlc_throwing:egg_entity", 22)
vlc_throwing.register_throwable_object("vlc_throwing:ender_pearl", "vlc_throwing:ender_pearl_entity", 22)
