...
local function sea_pickle_on_place(itemstack, placer, pointed_thing, level)
	...
	-- Place
	if node_under.name == "vlf_ocean:dead_brain_coral_block" then
		-- Place on suitable coral block
		if submerged then
			node_under.name = "vlf_ocean:sea_pickle_"..level.."_dead_brain_coral_block"
		else
			node_under.name = "vlf_ocean:sea_pickle_" .. level .. "_off_dead_brain_coral_block"
		end
	...
	elseif minetest.get_item_group(node_under.name, "sea_pickle") ~= 0 then
		...
		if def and def._vlf_sea_pickle_next then
			node_under.name = def._vlf_sea_pickle_next
			minetest.set_node(pos_under, node_under)
		else
			return itemstack
		end
...
for s=1,4 do
	...
	if s < 4 then
		next_on = "vlf_ocean:sea_pickle_" .. tostring(s + 1) .. "_" .. ontop
		next_off = "vlf_ocean:sea_pickle_" .. tostring(s + 1) .. "_off_" .. ontop
	end
	...
	minetest.register_node("vlf_ocean:sea_pickle_"..s.."_"..ontop, {
		description = desc,
		_tt_help = tt_help,
		...
		tiles = { "vlf_ocean_"..ontop..".png" },
		special_tiles = {
			{
			image = "vlf_ocean_sea_pickle_"..s.."_anim.png",
		...
		node_dig_prediction = "vlf_ocean:"..ontop,
		after_dig_node = function(pos)
			local node = minetest.get_node(pos)
			if minetest.get_item_group(node.name, "sea_pickle") == 0 then
				minetest.set_node(pos, {name="vlf_ocean:"..ontop})
			end
		end,
		on_place = on_place,
	...
	minetest.register_node("vlf_ocean:sea_pickle_"..s.."_off_"..ontop, {
		drawtype = "plantlike_rooted",
		paramtype = "light",
		paramtype2 = "meshoptions",
		tiles = { "vlf_ocean_"..ontop..".png" },
		special_tiles = { "vlf_ocean_sea_pickle_"..s.."_off.png", },
	...
	if mod_doc then
		if s == 1 then
			doc.add_entry_alias("nodes", "vlf_ocean:sea_pickle_1_dead_brain_coral_block", "nodes", "vlf_ocean:sea_pickle_1_off_"..ontop)
		else
			doc.add_entry_alias("nodes", "vlf_ocean:sea_pickle_1_dead_brain_coral_block", "nodes", "vlf_ocean:sea_pickle_"..s.."_off_"..ontop)
			doc.add_entry_alias("nodes", "vlf_ocean:sea_pickle_1_dead_brain_coral_block", "nodes", "vlf_ocean:sea_pickle_"..s.."_"..ontop)
		end
	end
end
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if minetest.get_item_group(checknode.name, "water") ~= 0 then
			-- Sea pickle is unlit
			if state == 2 then
				node.name = def._vlf_sea_pickle_on
			...
			-- Sea pickle is lit
			if state == 1 then
				node.name = def._vlf_sea_pickle_off
				minetest.set_node(pos, node)
				return
			end
		end
