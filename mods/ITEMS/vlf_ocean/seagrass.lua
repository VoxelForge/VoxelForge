...
local surfaces = {
	{ "dirt", "vlf_core:dirt" },
	{ "sand", "vlf_core:sand", 1 },
	{ "redsand", "vlf_core:redsand", 1 },
	{ "gravel", "vlf_core:gravel", 1 },
}
...
local function seagrass_on_place(itemstack, placer, pointed_thing)
	...
	if node_under.name == "vlf_core:dirt" then
		node_under.name = "vlf_ocean:seagrass_dirt"
	elseif node_under.name == "vlf_core:sand" then
		node_under.name = "vlf_ocean:seagrass_sand"
	elseif node_under.name == "vlf_core:redsand" then
		node_under.name = "vlf_ocean:seagrass_redsand"
	elseif node_under.name == "vlf_core:gravel" then
		node_under.name = "vlf_ocean:seagrass_gravel"
	else
		return itemstack
	end
...
minetest.register_craftitem("vlf_ocean:seagrass", {
	description = S("Seagrass"),
	_tt_help = S("Grows in water on dirt, sand, gravel"),
	_doc_items_create_entry = false,
	inventory_image = "vlf_ocean_seagrass_item.png",
	wield_image = "vlf_ocean_seagrass_item.png",
...
for s=1, #surfaces do
	...
	local leaf_sounds = vlf_sounds.node_sound_leaves_defaults()
	...
	if surfaces[s][1] == "dirt" then
		...
		desc = S("Seagrass")
		doc_create = true
		doc_img = "vlf_ocean_seagrass_item.png"
	else
		doc_create = false
	end
	minetest.register_node("vlf_ocean:seagrass_"..surfaces[s][1], {
		...
		drop = "",
		_vlf_falling_node_alternative = alt,
...
if mod_doc then
	doc.add_entry_alias("nodes", "vlf_ocean:seagrass_dirt", "craftitems", "vlf_ocean:seagrass")
end

minetest.register_lbm({
	label = "Fix incorrect seagrass",
	name = "vlf_ocean:fix_incorrect_seagrass",
