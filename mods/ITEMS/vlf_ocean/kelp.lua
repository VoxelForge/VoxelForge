...
kelp.leaf_sounds = vlf_sounds.node_sound_leaves_defaults()
...
local function store_age (pos, age)
	if pos then
		minetest.get_meta(pos):set_int("vlf_ocean:kelp_age", age)
	end
end

local function retrieve_age (pos)
	local meta = minetest.get_meta(pos)
	local age_set = meta:contains("vlf_ocean:kelp_age")
	if not age_set then
		return nil
	end

	local age = meta:get_int("vlf_ocean:kelp_age")
	return age
end
...
function kelp.detach_drop(pos, height)
	...
	for i=1,height do
		walk_pos.y = y+i
		minetest.add_item(walk_pos, "vlf_ocean:kelp")
	end
	return true
end
	...
	-- When placed on surface.
		...
		for _,surface in pairs(kelp.surfaces) do
			if nu_name == surface.nodename then
				node_under.name = "vlf_ocean:kelp_" ..surface.name
				node_under.param2 = 0
				new_surface = true
				break
			end
...
kelp.surfaces = {
	{ name="dirt",    nodename="vlf_core:dirt",    },
	{ name="sand",    nodename="vlf_core:sand",    },
	{ name="redsand", nodename="vlf_core:redsand", },
	{ name="gravel",  nodename="vlf_core:gravel",  },
}
...
kelp.surface_deftemplate = {
	...
	special_tiles = {
		{
		image = "vlf_ocean_kelp_plant.png",
		animation = {type="vertical_frames", aspect_w=16, aspect_h=16, length=2.0},
		tileable_vertical = true,
		}
	},
	--inventory_image = "("..def.tiles[1]..")^vlf_ocean_kelp_item.png",
...
kelp.surface_docs = {
	...
	_doc_items_image = "vlf_ocean_kelp_item.png",
}
...
function kelp.register_kelp_surface(surface, surface_deftemplate, surface_docs)
	...
	local surfacename = "vlf_ocean:kelp_"..name
	...
	surface_deftemplate._vlf_falling_node_alternative = surface_deftemplate._vlf_falling_node_alternative or (falling_node and nodename or nil)
...
minetest.register_craftitem("vlf_ocean:kelp", {
	description = S("Kelp"),
	_tt_help = S("Grows in water on dirt, sand, gravel"),
	_doc_items_create_entry = false,
	inventory_image = "vlf_ocean_kelp_item.png",
	wield_image = "vlf_ocean_kelp_item.png",
...
if mod_doc then
	doc.add_entry_alias("nodes", kelp.surface_docs.entry_id_orig, "craftitems", "vlf_ocean:kelp")
end

-- Dried kelp ------------------------------------------------------------------

-- TODO: This is supposed to be eaten very fast
minetest.register_craftitem("vlf_ocean:dried_kelp", {
	description = S("Dried Kelp"),
	_doc_items_longdesc = S("Dried kelp is a food item."),
	inventory_image = "vlf_ocean_dried_kelp.png",
	wield_image = "vlf_ocean_dried_kelp.png",
...
minetest.register_node("vlf_ocean:dried_kelp_block", {
	description = S("Dried Kelp Block"),
	_doc_items_longdesc = S("A decorative block that serves as a great furnace fuel."),
	tiles = { "vlf_ocean_dried_kelp_top.png", "vlf_ocean_dried_kelp_bottom.png", "vlf_ocean_dried_kelp_side.png" },
...
minetest.register_craft({
	...
	output = "vlf_ocean:dried_kelp_block",
})
minetest.register_craft({
	recipe = {
