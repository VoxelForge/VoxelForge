...
function vlf_bows.shoot_arrow_crossbow(arrow_item, pos, dir, yaw, shooter, power, damage, is_critical, crossbow_stack, collectable)
	...
	minetest.sound_play("vlf_bows_crossbow_shoot", {pos=pos, max_hear_distance=16}, true)
...
minetest.register_tool("vlf_bows:crossbow", {
...
S("The speed and damage of the arrow increases the longer you charge. The regular damage of the arrow is between 1 and 9. At full charge, there's also a 20% of a critical hit, dealing 10 damage instead."),
	...
	inventory_image = "vlf_bows_crossbow.png",
...
S("The speed and damage of the arrow increases the longer you charge. The regular damage of the arrow is between 1 and 9. At full charge, there's also a 20% of a critical hit, dealing 10 damage instead."),
	...
	_doc_items_durability = BOW_DURABILITY,
	inventory_image = "vlf_bows_crossbow_3.png",
	wield_scale = vlf_vars.tool_wield_scale,
	stack_max = 1,
	range = 4,
	-- Trick to disable digging as well
...
local function reset_bows(player)
	...
	for place, stack in pairs(list) do
		if stack:get_name() == "vlf_bows:crossbow" or stack:get_name() == "vlf_bows:crossbow_enchanted" then
			stack:get_meta():set_string("active", "")
		elseif stack:get_name()=="vlf_bows:crossbow_0" or stack:get_name()=="vlf_bows:crossbow_1" or stack:get_name()=="vlf_bows:crossbow_2" then
			stack:set_name("vlf_bows:crossbow")
			stack:get_meta():set_string("active", "")
			list[place] = stack
		elseif stack:get_name()=="vlf_bows:crossbow_0_enchanted" or stack:get_name()=="vlf_bows:crossbow_1_enchanted" or stack:get_name()=="vlf_bows:crossbow_2_enchanted" then
			stack:set_name("vlf_bows:crossbow_enchanted")
			stack:get_meta():set_string("active", "")
			list[place] = stack
		end
	end
...
local function reset_bow_state(player, also_reset_bows)
	...
	if minetest.get_modpath("playerphysics") then
		playerphysics.remove_physics_factor(player, "speed", "vlf_bows:use_crossbow")
	end
...
for level=0, 2 do
	minetest.register_tool("vlf_bows:crossbow_"..level, {
		description = S("Crossbow"),
		_doc_items_create_entry = false,
		inventory_image = "vlf_bows_crossbow_"..level..".png",
		wield_scale = vlf_vars.tool_wield_scale,
		stack_max = 1,
		...
		on_drop = function(itemstack, dropper, pos)
			...
			if vlf_enchanting.is_enchanted(itemstack:get_name()) then
				itemstack:set_name("vlf_bows:crossbow_enchanted")
			else
				itemstack:set_name("vlf_bows:crossbow")
			end
...
controls.register_on_release(function(player, key)
	...
	if wielditem:get_name()=="vlf_bows:crossbow_2" and get_arrow(player) or wielditem:get_name()=="vlf_bows:crossbow_2" and minetest.is_creative_enabled(player:get_player_name()) or wielditem:get_name()=="vlf_bows:crossbow_2_enchanted" and get_arrow(player) or wielditem:get_name()=="vlf_bows:crossbow_2_enchanted" and minetest.is_creative_enabled(player:get_player_name()) then
		...
		if wielditem:get_name()=="vlf_bows:crossbow_2" then
			wielditem:set_name("vlf_bows:crossbow_loaded")
		else
			wielditem:set_name("vlf_bows:crossbow_loaded_enchanted")
		end
		player:set_wielded_item(wielditem)
		minetest.sound_play("vlf_bows_crossbow_load", {pos=player:get_pos(), max_hear_distance=16}, true)
	else
...
controls.register_on_press(function(player, key)
	if key~="LMB" then return end
		...
		if enchanted then
			...
			wielditem:set_name("vlf_bows:crossbow")
		end
		...
		if has_shot and not minetest.is_creative_enabled(player:get_player_name()) then
			...
			local multishot = vlf_enchanting.get_enchantment(wielditem, "multishot")
...
controls.register_on_hold(function(player, key)
	...
	local wielditem = player:get_wielded_item()
	local enchantments = vlf_enchanting.get_enchantments(wielditem)
	...
	if bow_load[name] == nil
		and (wielditem:get_name()=="vlf_bows:crossbow" or wielditem:get_name()=="vlf_bows:crossbow_enchanted")
		and (wielditem:get_meta():get("active") or key=="zoom") and (creative or get_arrow(player)) then
			local enchanted = vlf_enchanting.is_enchanted(wielditem:get_name())
			if enchanted then
				wielditem:set_name("vlf_bows:crossbow_0_enchanted")
				play_load_sound(0, player:get_pos())
			else
				wielditem:set_name("vlf_bows:crossbow_0")
				play_load_sound(0, player:get_pos())
			end
			player:set_wielded_item(wielditem)
			if minetest.get_modpath("playerphysics") then
				-- Slow player down when using bow
				playerphysics.add_physics_factor(player, "speed", "vlf_bows:use_crossbow", PLAYER_USE_CROSSBOW_SPEED)
			end
		...
		if player:get_wield_index() == bow_index[name] then
			if type(bow_load[name]) == "number" then
				if wielditem:get_name() == "vlf_bows:crossbow_0" and minetest.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_HALF then
					wielditem:set_name("vlf_bows:crossbow_1")
					play_load_sound(1, player:get_pos())
				elseif wielditem:get_name() == "vlf_bows:crossbow_0_enchanted" and minetest.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_HALF then
					wielditem:set_name("vlf_bows:crossbow_1_enchanted")
					play_load_sound(1, player:get_pos())
				elseif wielditem:get_name() == "vlf_bows:crossbow_1" and minetest.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_FULL then
					wielditem:set_name("vlf_bows:crossbow_2")
					play_load_sound(2, player:get_pos())
				elseif wielditem:get_name() == "vlf_bows:crossbow_1_enchanted" and minetest.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_FULL then
					wielditem:set_name("vlf_bows:crossbow_2_enchanted")
					play_load_sound(2, player:get_pos())
				end
			else
				if wielditem:get_name() == "vlf_bows:crossbow_0" or wielditem:get_name() == "vlf_bows:crossbow_1" or wielditem:get_name() == "vlf_bows:crossbow_2" then
					wielditem:set_name("vlf_bows:crossbow")
					play_load_sound(1, player:get_pos())
				elseif wielditem:get_name() == "vlf_bows:crossbow_0_enchanted" or wielditem:get_name() == "vlf_bows:crossbow_1_enchanted" or wielditem:get_name() == "vlf_bows:crossbow_2_enchanted" then
					wielditem:set_name("vlf_bows:crossbow_enchanted")
					play_load_sound(1, player:get_pos())
				end
			end
...
minetest.register_globalstep(function()
	for _, player in pairs(minetest.get_connected_players()) do
		...
		if type(bow_load[name]) == "number" and ((wielditem:get_name()~="vlf_bows:crossbow_0" and wielditem:get_name()~="vlf_bows:crossbow_1" and wielditem:get_name()~="vlf_bows:crossbow_2" and wielditem:get_name()~="vlf_bows:crossbow_0_enchanted" and wielditem:get_name()~="vlf_bows:crossbow_1_enchanted" and wielditem:get_name()~="vlf_bows:crossbow_2_enchanted") or wieldindex ~= bow_index[name]) then
			reset_bow_state(player, true)
		end
	end
end)
...
if minetest.get_modpath("vlf_core") and minetest.get_modpath("vlf_mobitems") then
	minetest.register_craft({
		output = "vlf_bows:crossbow",
		recipe = {
			{"vlf_core:stick", "vlf_core:iron_ingot", "vlf_core:stick"},
			{"vlf_mobitems:string", "vlf_bows:arrow", "vlf_mobitems:string"},
			{"", "vlf_core:stick", ""},
		}
	})
end
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("tools", "vlf_bows:crossbow", "tools", "vlf_bows:crossbow_0")
	doc.add_entry_alias("tools", "vlf_bows:crossbow", "tools", "vlf_bows:crossbow_1")
	doc.add_entry_alias("tools", "vlf_bows:crossbow", "tools", "vlf_bows:crossbow_2")
end
