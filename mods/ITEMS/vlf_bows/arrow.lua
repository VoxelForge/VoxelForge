...
minetest.register_craftitem("vlf_bows:arrow", {
	description = S("Arrow"),
...
local ARROW_ENTITY={
	initial_properties = {
		...
		pointable = false,
		visual = "mesh",
		mesh = "vlf_bows_arrow.obj",
		visual_size = {x=-1, y=1},
		textures = {"vlf_bows_arrow.png"},
...
minetest.register_on_respawnplayer(function(player)
	for _, obj in pairs(player:get_children()) do
		...
		if ent and ent.name and string.find(ent.name, "vlf_bows:arrow_entity") then
			obj:remove()
		end
	end
end)

minetest.register_entity("vlf_bows:arrow_entity", ARROW_ENTITY)

if minetest.get_modpath("vlf_core") and minetest.get_modpath("vlf_mobitems") then
	minetest.register_craft({
		output = "vlf_bows:arrow 4",
		recipe = {
			{"vlf_core:flint"},
			{"vlf_core:stick"},
