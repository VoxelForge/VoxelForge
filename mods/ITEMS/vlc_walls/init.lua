...
function vlc_walls.update_wall(pos)
...
local function update_wall_global(pos)
	for i = 1,5 do
		local dir = directions[i]
		vlc_walls.update_wall({x = pos.x + dir.x, y = pos.y + dir.y, z = pos.z + dir.z})
	end
end
...
function vlc_walls.register_wall(nodename, description, source, tiles, inventory_image, groups, sounds, overrides)
	...
	for i = 0, 15 do
		...
		minetest.register_node(nodename.."_"..i, table.merge({
			...
			_vlc_hardness = 2,
			_vlc_stonecutter_recipes = {source},
		}, overrides or {}))
	...
	minetest.register_node(nodename, table.merge({
		...
		on_construct = vlc_walls.update_wall,
		sounds = sounds,
		_vlc_blast_resistance = 6,
		_vlc_hardness = 2,
	}, overrides or {}))
...
function vlc_walls.register_wall_def(name,def)
	local source = def.source
	def.source = nil
	vlc_walls.register_wall(name, nil, source, nil, nil, nil, nil, def)
end
