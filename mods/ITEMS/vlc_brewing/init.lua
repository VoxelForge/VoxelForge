...
local function active_brewing_formspec(fuel_percent, brew_percent)
	...
	return "size[9,8.75]"..
	"background[-0.19,-0.25;9.5,9.5;vlc_brewing_inventory.png]"..
	"label[4,0;"..minetest.formspec_escape(minetest.colorize("#313131", S("Brewing Stand"))).."]"..
	"label[0,4.0;"..minetest.formspec_escape(minetest.colorize("#313131", S("Inventory"))).."]"..
	"list[current_player;main;0,4.5;9,3;9]"..
	vlc_formspec.get_itemslot_bg(0,4.5,9,3)..
	"list[current_player;main;0,7.75;9,1;]"..
	vlc_formspec.get_itemslot_bg(0,7.75,9,1)..
	"list[context;fuel;0.5,1.75;1,1;]"..
	vlc_formspec.get_itemslot_bg(0.5,1.75,1,1).."image[0.5,1.75;1,1;vlc_brewing_fuel_bg.png]"..
	"list[context;input;2.75,0.5;1,1;]"..
	vlc_formspec.get_itemslot_bg(2.75,0.5,1,1)..
	"list[context;stand;4.5,2.5;1,1;]"..
	vlc_formspec.get_itemslot_bg(4.5,2.5,1,1).."image[4.5,2.5;1,1;vlc_brewing_bottle_bg.png]"..
	"list[context;stand;6,2.8;1,1;1]"..
	vlc_formspec.get_itemslot_bg(6,2.8,1,1).."image[6,2.8;1,1;vlc_brewing_bottle_bg.png]"..
	"list[context;stand;7.5,2.5;1,1;2]"..
	vlc_formspec.get_itemslot_bg(7.5,2.5,1,1).."image[7.5,2.5;1,1;vlc_brewing_bottle_bg.png]"..

	"image[2.7,3.33;1.28,0.41;vlc_brewing_burner.png^[lowpart:"..
	(100-fuel_percent)..":vlc_brewing_burner_active.png^[transformR270]"..

	"image[2.76,1.4;1,2.15;vlc_brewing_bubbles.png^[lowpart:"..
	(brew_percent)..":vlc_brewing_bubbles_active.png]"..
...
local brewing_formspec = "size[9,8.75]"..
	...
	"image[2.7,3.33;1.28,0.41;vlc_brewing_burner.png^[transformR270]"..
	"image[2.76,1.4;1,2.15;vlc_brewing_bubbles.png]"..
...
local function brewable(inv)
	...
	for i=1,stand_size do
		...
		alchemy = vlc_potions.get_alchemy(ingredient, bottle)
	...
	while update do
		...
		if fuel ~= 0 and brew_output then
			...
			minetest.add_particlespawner({
				...
				vertical = false,
				texture = "vlc_brewing_bubble_sprite.png",
			})
			...
			if stand_timer >= BREW_TIME then
				...
				for i=1, inv:get_size("stand") do
					if brew_output[i] then
						minetest.sound_play("vlc_brewing_complete", {pos=pos, gain=0.4, max_hear_range=6}, true)
						inv:set_stack("stand", i, brew_output[i])
						minetest.sound_play("vlc_potions_bottle_pour", {pos=pos, gain=0.6, max_hear_range=6}, true)
					end
				end
				stand_timer = 0
		...
		elseif fuel == 0 then  --get more fuel from fuel_list
			...
			if fuel_name == "vlc_mobitems:blaze_powder" then -- Grab another fuel
...
local drop_contents = vlc_util.drop_items_from_meta_container({"fuel", "input", "stand"})
...
local tiles = {
	"vlc_brewing_top.png", 	--top
	"vlc_brewing_base.png", 	--bottom
	"vlc_brewing_side.png", 	--right
	"vlc_brewing_side.png", 	--left
	"vlc_brewing_side.png", 	--back
	"vlc_brewing_side.png^[transformFX",   --front
}

local function sort_stack(stack)
	if stack:get_name() == "vlc_mobitems:blaze_powder" then
		return "fuel"
	end
...
local function on_put(pos, listname, index, stack, player)
	...
	minetest.swap_node(pos, {name = "vlc_brewing:stand_"..str})
...
local function allow_take(pos, listname, index, stack, player)
	...
	if minetest.is_protected(pos, name) then
		...
		if listname == "stand" then
			awards.unlock(name, "vlc:localBrewery")
		end
		return stack:get_count()
	end
end
...
local function hopper_in(pos, to_pos)
	...
	if pos.y == to_pos.y then
		local slot_id,_ = vlc_util.get_eligible_transfer_item_slot(sinv, "main", dinv, "fuel", function(itemstack)
			return itemstack:get_name() == "vlc_mobitems:blaze_powder"
		end)
		if slot_id then
			vlc_util.move_item(sinv, "main", slot_id, dinv, "fuel")
			minetest.get_node_timer(to_pos):start(1.0)
		else
			local slot_id,_ = vlc_util.get_eligible_transfer_item_slot(sinv, "main", dinv, "stand", function(itemstack)
				local n = itemstack:get_name()
				return minetest.get_item_group(n, "water_bottle") > 0
			end)
			if slot_id then
				vlc_util.move_item(sinv, "main", slot_id, dinv, "stand")
				minetest.get_node_timer(to_pos):start(1.0)
			end
		end
		return true
	end
	local slot_id,_ = vlc_util.get_eligible_transfer_item_slot(sinv, "main", dinv, "input", function(itemstack)
		return minetest.get_item_group(itemstack:get_name(), "brewing_ingredient" ) > 0
	end)
	if slot_id then
		vlc_util.move_item(sinv, "main", slot_id, dinv, "input")
		minetest.get_node_timer(to_pos):start(1.0)
	end
	return true
end
...
local function hopper_out(pos, to_pos)
	local sinv = minetest.get_inventory({type="node", pos = pos})
	local dinv = minetest.get_inventory({type="node", pos = to_pos})
	local slot_id,_ = vlc_util.get_eligible_transfer_item_slot(sinv, "stand", dinv, "main", function(itemstack)
		return true
	end)
	if slot_id then
		vlc_util.move_item(sinv, "stand", slot_id, dinv, "main")
	end
	return true
end
...
minetest.register_node("vlc_brewing:stand_000", table.merge(tpl_brewing_stand, {
...
minetest.register_node("vlc_brewing:stand_100", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
...
minetest.register_node("vlc_brewing:stand_010", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
...
minetest.register_node("vlc_brewing:stand_001", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_node("vlc_brewing:stand_110", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_node("vlc_brewing:stand_101", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_node("vlc_brewing:stand_011", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_node("vlc_brewing:stand_111", table.merge(tpl_brewing_stand, {
	node_box = {
		type = "fixed",
		fixed = {
			{-1/16, -5/16, -1/16, 1/16, 8/16, 1/16}, -- heat plume
...
minetest.register_craft({
	output = "vlc_brewing:stand_000",
	recipe = {
		{ "", "vlc_mobitems:blaze_rod", "" },
		{ "group:cobble", "group:cobble", "group:cobble" },
	}
})

minetest.register_alias("vlc_brewing:stand", "vlc_brewing:stand_000")

if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", "vlc_brewing:stand_000", "nodes", "vlc_brewing:stand_001")
	doc.add_entry_alias("nodes", "vlc_brewing:stand_000", "nodes", "vlc_brewing:stand_010")
	doc.add_entry_alias("nodes", "vlc_brewing:stand_000", "nodes", "vlc_brewing:stand_011")
	doc.add_entry_alias("nodes", "vlc_brewing:stand_000", "nodes", "vlc_brewing:stand_100")
	doc.add_entry_alias("nodes", "vlc_brewing:stand_000", "nodes", "vlc_brewing:stand_101")
	doc.add_entry_alias("nodes", "vlc_brewing:stand_000", "nodes", "vlc_brewing:stand_110")
	doc.add_entry_alias("nodes", "vlc_brewing:stand_000", "nodes", "vlc_brewing:stand_111")
end

minetest.register_lbm({
	label = "Update brewing stand formspecs and invs to allow new sneak+click behavior",
	name = "vlc_brewing:update_coolsneak",
