...
"_vlc_hardness" (0 by default).  These values are used together with digging
...
the digging group in their node definition.  "vlc_core:dirt" for example has
...
"vlc_core:stone_with_gold" for example has pickaxey=4 because it requires a
pickaxe of level 4 be mined.

For tools to be able to dig nodes of digging groups they need to use the have
the custom field "_vlc_diggroups" function to get the groupcaps.  The value of
...
"vlc_tools/init.lua" for examples on how "_vlc_diggroups" is used in practice.
...
This also means that it is very important that no mod adds _vlc_autogroup as a
dependency.
--]]

assert(minetest.get_modpath("vlc_autogroup"), "This mod requires the mod vlc_autogroup to function")

local groups_mtg2vlc = {
...
local function convert_mtg_groups(nname)
	...
	local hardness = minetest.registered_nodes[nname]._vlc_hardness

	if not hardness then --if _vlc_hardness is defined the node is clearly intended for vlc specifically, don't mess with the groups in that case
		for mtg, vlc in pairs(groups_mtg2vlc) do
			local g_mtg = minetest.get_item_group(nname, mtg)
			local g_vlc = minetest.get_item_group(nname, vlc.group)
			if g_mtg > 0 and g_vlc == 0 then
				groups[vlc.group] = g_mtg
				groups[mtg] = nil

				hardness = math.max(hardness or 0, g_mtg * vlc.hardness)
			end
		end
	end
...
for nname, ndef in pairs(minetest.registered_nodes) do
	...
	minetest.override_item(nname, {
		groups = newgroups,
		_vlc_hardness = newhardness,
	})
end

-- Returns a table containing the unique "_vlc_hardness" for nodes belonging to
-- each diggroup.
local function get_hardness_values_for_groups()
	local maps = {}
	local values = {}
	for g, _ in pairs(vlc_autogroup.registered_diggroups) do
		maps[g] = {}
		values[g] = {}
	end
	...
	for _, ndef in pairs(minetest.registered_nodes) do
		for g, _ in pairs(vlc_autogroup.registered_diggroups) do
			if ndef.groups[g] then
				maps[g][ndef._vlc_hardness or 0] = true
			end
		end
	end
...
-- Returns a table containing a table indexed by "_vlc_hardness" value to get
...
-- Add the groupcaps from a field in "_vlc_diggroups" to the groupcaps of a
-- tool.
local function add_groupcaps(toolname, groupcaps, groupcaps_def, efficiency)
	...
	for g, capsdef in pairs(groupcaps_def) do
		...
		local def = vlc_autogroup.registered_diggroups[g]
...
function vlc_autogroup.can_harvest(nodename, toolname, player)
	...
	if tdef and tdef._vlc_diggroups then
		for g, gdef in pairs(tdef._vlc_diggroups) do
			if ndef.groups[g] then
...
local function get_tool_capabilities(tdef)
	...
	local hand_toolcaps = vlc_meshhand.survival_hand_tool_caps
	return {
...
--
-- Parameters:
-- toolname - Name of the tool being enchanted (like "vlc_tools:diamond_pickaxe")
...
-- would have to add _vlc_autogroup as a dependency which would break the mod
-- loading order.
function vlc_autogroup.get_groupcaps(toolname, efficiency)
	local tdef = minetest.registered_items[toolname]
	local groupcaps = table.copy(get_tool_capabilities(tdef).groupcaps or {})
	add_groupcaps(toolname, groupcaps, tdef._vlc_diggroups, efficiency)
	return groupcaps
end
...
-- would have to add _vlc_autogroup as a dependency which would break the mod
-- loading order.
function vlc_autogroup.get_wear(toolname, diggroup)
	local tdef = minetest.registered_tools[toolname]
	local uses = tdef._vlc_diggroups[diggroup].uses
...
local function overwrite()
	for nname, ndef in pairs(minetest.registered_nodes) do
		...
		if (nname ~= "ignore" and ndef.diggable) then
			...
			for g, gdef in pairs(vlc_autogroup.registered_diggroups) do
				local index = hardness_lookup[g][ndef._vlc_hardness] or newgroups[g] or hardness_lookup[g][0]
				if newgroups[g] then
					if gdef.levels then
	...
	for tname, tdef in pairs(minetest.registered_items) do
		...
		-- depending on the _vlc_diggroups in the tool definition
		if tdef._vlc_diggroups then
			local toolcaps = table.copy(get_tool_capabilities(tdef))
			toolcaps.groupcaps = vlc_autogroup.get_groupcaps(tname)
