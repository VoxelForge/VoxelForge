...
local function place_schematic(pos, file_name, rotation, rotation_origin)
  ...
  --[[  if rotation == 0 then  -- No rotation
        minp = vector.add(pos, vector.new(0, 0, 0))
        maxp = vector.add(pos, vector.subtract(schematic_size, vector.new(1, 1, 1)))

    elseif rotation == 90 then  -- 90 degrees rotation
        minp = vector.add(pos, vector.new(-schematic_size.z + 1, 0, 0))
        maxp = vector.add(pos, vector.new(0, schematic_size.y - 1, schematic_size.x - 1))

    elseif rotation == 180 then  -- 180 degrees rotation
        minp = vector.add(pos, vector.new(-schematic_size.x + 1, 0, -schematic_size.z + 1))
        maxp = vector.add(pos, vector.subtract(schematic_size, vector.new(1, 1, 1)))

    elseif rotation == 270 then  -- 270 degrees rotation
        minp = vector.add(pos, vector.new(0, 0, -schematic_size.x + 1))
        maxp = vector.add(pos, vector.new(schematic_size.z - 1, schematic_size.y - 1, 0))
    end]]
    
--[[    if rotation == 0 then
    minp = vector.add(pos, vector.multiply(schematic_size, 0.0))
    maxp = vector.add(pos, vector.subtract(vector.multiply(schematic_size, 1.0), {x = 1, y = 1, z = 1}))

elseif rotation == 90 then
    minp = vector.add(pos, {x = -schematic_size.z + 2, y = 0, z = 0})
    maxp = vector.add(pos, {x = -1, y = schematic_size.y - 2, z = schematic_size.x - 2})

elseif rotation == 180 then
    minp = vector.add(pos, vector.multiply(schematic_size, 1.0))
    maxp = vector.add(pos, vector.subtract(vector.multiply(schematic_size, 0.0), {x = 1, y = 1, z = 1}))

elseif rotation == 270 then
    minp = vector.add(pos, {x = 0, y = 0, z = -schematic_size.x + 2})
    maxp = vector.add(pos, {x = schematic_size.z - 2, y = schematic_size.y - 2, z = -1})
end]]
    local schemsize
    if rotation == 0 then
...
--[[local function place_schematic(pos, file_name, rotation, rotation_origin)
    ...
    -- Determine the area to be loaded based on schematic size
    local schematic_size = schematic.size
    local minp = vector.add(pos, vector.multiply(schematic_size, -0.5))
    local maxp = vector.add(pos, vector.multiply(schematic_size, 0.5))
...
--[[local function get_bounding_box(schematic_pos, schematic_size, rotation)
    ...
    local schemsize
    if rotation == 0 then
    minp = vector.subtract(schematic_pos, vector.multiply(schematic_size, 0.0))
    maxp = vector.add(schematic_pos, vector.multiply(schematic_size, 1.0))
    elseif rotation == 90 then
    schem_size = {x=schematic_size.z, y=schematic_size.y, z=schematic_size.x}
    minp = vector.add(schematic_pos, vector.multiply(schem_size, 0.0))
    maxp = vector.subtract(schematic_pos, vector.multiply(schematic_size, 1.0))
    elseif rotation == 180 then
    minp = vector.subtract(schematic_pos, vector.multiply(schematic_size, 1.0))
    maxp = vector.add(schematic_pos, vector.multiply(schematic_size, 0.0))
    elseif rotation == 270 then
    schem_size = {x=schematic_size.z, y=schematic_size.y, z=schematic_size.x}
    minp = vector.add(schematic_pos, vector.multiply(schem_size, 0.5))
    maxp = vector.subtract(schematic_pos, vector.multiply(schematic_size, 1.5))
    else
    minp = vector.subtract(schematic_pos, vector.multiply(schematic_size, 0.0))
    maxp = vector.add(schematic_pos, vector.multiply(schematic_size, 1.0))
    end
...
--[[local function get_bounding_box(schematic_pos, schematic_size, rotation)
    ...
    elseif rotation == 90 then
        ...
        minp = vector.new(schematic_pos)
        maxp = vector.add(schematic_pos, vector.subtract({x = schematic_size.z, y = schematic_size.y, z = schematic_size.x}, {x = 1, y = 1, z = 1}))
    elseif rotation == 180 then
        -- Rotate 180 degrees
        minp = vector.subtract(schematic_pos, vector.subtract(schematic_size, {x = 1, y = 1, z = 1}))
        maxp = vector.new(schematic_pos)
        minp, maxp = maxp, minp -- Swap to ensure minp is the lower corner and maxp is the upper corner
    elseif rotation == 270 then
        -- Rotate 270 degrees (x and z dimensions swapped)
        minp = vector.subtract(schematic_pos, vector.subtract({x = schematic_size.z, y = schematic_size.y, z = schematic_size.x}, {x = 1, y = 1, z = 1}))
        maxp = vector.new(schematic_pos)
        minp, maxp = maxp, minp -- Swap to ensure minp is the lower corner and maxp is the upper corner
    else
        -- If no valid rotation is given, use default bounding box
        minp = vector.new(schematic_pos)
        maxp = vector.add(schematic_pos, vector.subtract(schematic_size, {x = 1, y = 1, z = 1}))
    end
...
--[[local function get_bounding_box(schematic_pos, schematic_size, rotation)
    ...
    if rotation == 0 then
        -- Rotation 0: No change, correct as is
        minp = vector.new(schematic_pos)
        maxp = vector.add(schematic_pos, vector.subtract(schematic_size, {x = 1, y = 1, z = 1}))
    elseif rotation == 90 then
        -- Rotate 90 degrees (x and z dimensions swapped)
        ...
        minp = vector.new(schematic_pos)
        maxp = vector.add(schematic_pos, vector.subtract(rotated_size, {x = 1, y = 1, z = 1}))
    elseif rotation == 180 then
        -- Rotate 180 degrees (reverse both x and z dimensions)
        minp = vector.subtract(schematic_pos, vector.subtract(schematic_size, {x = 1, y = 1, z = 1}))
        maxp = vector.new(schematic_pos)
        minp, maxp = maxp, minp -- Swap to ensure minp is the lower corner and maxp is the upper corner
    elseif rotation == 270 then
        -- Rotate 270 degrees (x and z dimensions swapped)
        local rotated_size = {x = schematic_size.z, y = schematic_size.y, z = schematic_size.x}
        minp = vector.subtract(schematic_pos, vector.subtract(rotated_size, {x = 1, y = 1, z = 1}))
        maxp = vector.new(schematic_pos)
        minp, maxp = maxp, minp -- Swap to ensure minp is the lower corner and maxp is the upper corner
    else
...
--[[local function get_bounding_box(schematic_pos, schematic_size, rotation)
    ...
    elseif rotation == 90 then
        ...
        maxp = vector.add(schematic_pos, vector.subtract(rotated_size, {x = 1, y = 1, z = 1}))
    elseif rotation == 180 then
        -- Rotate 180 degrees (flip both x and z dimensions)
        minp = vector.new(schematic_pos)
        maxp = vector.subtract(schematic_pos, vector.add(schematic_size, {x = 1, y = 1, z = 1}))
    elseif rotation == 270 then
        -- Rotate 270 degrees (x and z dimensions swapped)
        local rotated_size = {x = schematic_size.z, y = schematic_size.y, z = schematic_size.x}
        minp = vector.subtract(schematic_pos, vector.subtract(rotated_size, {x = 1, y = 1, z = 1}))
...
--[[local function get_bounding_box(schematic_pos, schematic_size, rotation)
    -- Only rotations 0, 180, and 270 work.
    local minp, maxp

    if rotation == 0 then
        -- Rotation 0: Reminder: Don't change, correct as is.
        minp = vector.new(schematic_pos)
        maxp = vector.add(schematic_pos, vector.subtract(schematic_size, {x = 1, y = 1, z = 1}))
    elseif rotation == 90 then
        -- Rotate 90 degrees (x and z dimensions swapped)
        local rotated_size = {x = schematic_size.z, y = schematic_size.y, z = schematic_size.x}
        local schempos = {x = schematic_pos.x, y = schematic_pos.y, z = schematic_pos.z - schematic_size.x + schematic_size.x/2}
        minp = vector.new(schempos)
        maxp = vector.add(schempos, vector.subtract(rotated_size, {x = 1, y = 1, z = 1}))
    elseif rotation == 180 then
        -- Rotate 180 degrees: Reminder: Don't change, correct as is.
    ...
    elseif rotation == 270 then
        -- Rotate 270 degrees: Reminder: Don't change, correct as is.
        local rotated_size = {x = schematic_size.z, y = schematic_size.y, z = schematic_size.x}
        local schempos = {x = schematic_pos.x + schematic_size.z/2, y = schematic_pos.y, z = schematic_pos.z}
        minp = vector.new(schempos)
        maxp = vector.add(schempos, vector.subtract(rotated_size, {x = 1, y = 1, z = 1}))
    else
        -- Fallback case (shouldn't be needed)
...
--[[local function spawn_struct(pos)
    ...
    local blockseed = minetest.get_mapgen_setting("seed")
    local seed = pos_hash + blockseed + minetest.hash_node_position({x = pos.x * rng:next(1, 47), y = pos.y * rng:next(1, 49), z = pos.z * rng:next(1, 45)})
    ...
    for _, schematic_data in ipairs(valid_schematics) do
    ...
    local schematic_size = schematic_data.size
    local minp, maxp = get_bounding_box(placement_pos, schematic_size, rot)
    minetest.log("action", "Next Schematic's " .. selected_schematic .. " bounds: minp=" .. minetest.pos_to_string(minp) .. ", maxp=" .. minetest.pos_to_string(maxp) .. rot)
    --local new_bbox = BoundingBox.fromCorners(minp, maxp)
    ...
    table.insert(existing_bounding_boxes, {minp = minp, maxp = maxp})
...
--[[local function spawn_struct(pos)
    ...
    local function process_elements(elements)
        for _, element_entry in ipairs(elements) do
            ...
            local weight = element_entry.weight or 1
            if not element.location then
                --minetest.log("error", "Adding schematic without location with weight: " .. weight)
                table.insert(valid_schematics, {schematic = nil, weight = weight})
                total_weight = total_weight + weight
            else
                local location = element.location:gsub("minecraft:", "")
                ...
                if found_matching_node then
                    --minetest.log("error", "Adding schematic with location " .. location .. " and weight: " .. weight)
                    table.insert(valid_schematics, {schematic = selecting_schematic, weight = weight})
                    total_weight = total_weight + weight
                else
                    --minetest.log("error", "No matching nodes found in schematic: " .. location)
                end
            end
        end
    end
    ...
    if #valid_schematics == 0 then
        --minetest.log("error", "No matching schematics found with target: " .. target)
        if fallback_pool then
    ...
    for _, schematic_data in ipairs(valid_schematics) do
        cumulative_weight = cumulative_weight + schematic_data.weight
        --minetest.log("error", "Cumulative weight: " .. cumulative_weight .. ", Selected weight: " .. selected_weight)
        if selected_weight <= cumulative_weight then
            selected_schematic = schematic_data.schematic
            --minetest.log("error", "Selected schematic: " .. (selected_schematic or "None (no-location schematic)"))
            break
        end
    end

    if not selected_schematic then
        --minetest.log("action", "No-location schematic selected, skipping placement")
        minetest.set_node(pos, {name = final_state})
        return
    end
    ...
    --minetest.log("action", "Placing schematic at position: " .. minetest.pos_to_string(placement_pos))
    place_schematic(placement_pos, selected_schematic, rot, target_pos)
    ...
    local target_node_meta = minetest.get_meta(place_pos)
    local target_final_state = target_node_meta:get_string("final_state")
    --minetest.log("error", "final state " .. target_final_state .. " at Pos:" .. minetest.pos_to_string(place_pos) .. "")
    if target_final_state and target_final_state ~= "" then
        minetest.set_node(place_pos, {name = target_final_state})
        --minetest.log("action", "Updated target node to final_state: " .. target_final_state .. " at position: " .. minetest.pos_to_string(placement_pos))
    end
end]]
...
local function rotate_pos(pos, size, rotation)
    ...
    if rotation == 90 then
        new_pos.x = size.z - pos.z - 1
        new_pos.z = pos.x
    elseif rotation == 180 then
        new_pos.x = size.x - pos.x - 1
        new_pos.z = size.z - pos.z - 1
    elseif rotation == 270 then
        new_pos.x = pos.z
        new_pos.z = size.x - pos.x - 1
    end
...
local function check_overlap(placement_pos, schematic_data, rot)
    ...
    for _, node in ipairs(schematic_data.nodes) do
        ...
        local check_pos = vector.add(placement_pos, rotated_pos)
        local node_name = minetest.get_node(check_pos).name
        if node_name == "vlf_deepslate:tuff_bricks"--[[ and node_name ~= "ignore"]] then
...
--[[local function check_overlap(placement_pos, schematic_data, rot)
    ...
    for _, node in ipairs(schematic_data.nodes) do
        ...
        -- Get metadata of the node at check_pos
        local meta = minetest.get_meta(check_pos)
        local is_edge = meta:get_string("edge")

        -- Check if both metadata values are true
