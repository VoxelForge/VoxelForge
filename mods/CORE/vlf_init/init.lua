...
minetest.register_on_mods_loaded(function()
	...
	--minetest.settings:set("shader_path", modpath.."/vlf_shaders/")
end)
...
minetest.register_on_shutdown(function()
	...
	--minetest.settings:set("shader_path", "")
end)
...
local function create_chat_hud(player)
    ...
    player_chat_huds[player_name] = player:hud_add({
        hud_elem_type = "text",
        position = {x = 0.2, y = 0.9}, -- Bottom center
        offset = {x = 0, y = -50}, -- Adjust y offset as needed
        text = "",
        alignment = {x = 0, y = 0},
        scale = {x = 100, y = 100},
...
local function update_chat_hud(player)
    ...
    if hud_id then
        ...
        for _, message in ipairs(chat_history) do
            chat_text = chat_text .. message.text .. "\n"
        end
        player:hud_change(hud_id, "text", chat_text)
    end
end

-- Register the on_joinplayer event
minetest.register_on_joinplayer(function(player)
    -- Hide the default chat
    hide_default_chat(player)
    -- Create the chat HUD element for the player
    create_chat_hud(player)
    -- Update the HUD with the current chat history
    update_chat_hud(player)
end)

-- Register the on_chat_message event to intercept chat messages
minetest.register_on_chat_message(function(name, message)
    -- Append the new message to the chat history with a timestamp
    table.insert(chat_history, {text = "<" .. name .. "> " .. message, time = os.time()})
    -- Trim the chat history to the maximum number of lines
    while #chat_history > max_chat_lines do
        table.remove(chat_history, 1)
    end

    -- Update the chat HUD for all connected players
    for _, player in pairs(minetest.get_connected_players()) do
        update_chat_hud(player)
    end

    -- Prevent the default chat display
    return true
end)

-- Globalstep to remove old chat messages
...
local function update_chat_hud(player)
    ...
    if hud_id then
        ...
        -- Iterate through stored chat history and add them to the chat HUD
        local line_count = 0
        for i = #chat_history, 1, -1 do
            local message = chat_history[i]
            local message_time = message.time or current_time

            -- Only display messages within the message lifetime
            if current_time - message_time <= message_lifetime then
                -- Format the message with player name at the top
                local formatted_message = ""
              --[[  if message.name then
                    formatted_message = message.name .. ": " .. message.message
                else
                    formatted_message = message.message
              --  end

                -- Wrap the message to fit the HUD width
                local words = {}
                ...
                -- Concatenate wrapped lines to the right of existing text
        ...
        -- Update the HUD with the combined chat history
        player:hud_change(hud_id, "text", chat_text)
    end
end]]
    ...
    if hud_id then
        ...
        for i = #chat_history, 1, -1 do
            ...
            if current_time - message_time <= message_lifetime then
                ...
                for word in formatted_message:gmatch("%S+") do
                    local word_length = word:len()
                    local remaining_space = max_hud_width_chars - line_length

                    -- Check if the word is too long to fit on a line
                    if word_length > max_hud_width_chars then
                        local start = 1
                        while start <= word_length do
                            local end_pos = math.min(start + max_hud_width_chars - 1, word_length)
                            local split_word = word:sub(start, end_pos)
                            table.insert(wrapped_lines, split_word)
                            start = end_pos + 1
                        end
                        line = ""
                        line_length = 0
                    elseif line_length > 0 and line_length + word_length > max_hud_width_chars then
                        table.insert(wrapped_lines, line)
                        line = ""
                        line_length = 0
                    end
                    ...
                    -- If the line is too long, move it to wrapped_lines
                    if line_length > max_hud_width_chars then
                        table.insert(wrapped_lines, line)
                        line = ""
                        line_length = 0
                    end
...
local function add_chat_message(name, message)
    ...
    -- Regular chat message with <Player Name> prefix
    if name ~= "" then
...
-- Register the on_leaveplayer event
minetest.register_on_leaveplayer(function(player)
    local player_name = player:get_player_name()

    -- Add a system message for player leaving
...
-- Globalstep to remove old chat messages from the chat history
minetest.register_globalstep(function(dtime)
    local current_time = os.time()
    local updated = false
