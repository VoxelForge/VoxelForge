vlc_events = {}
vlc_events.registered_events = {}
local disabled_events = minetest.settings:get("vlc_disabled_events")
if disabled_events then	disabled_events = disabled_events:split(",")
else disabled_events = {} end
local DBG = minetest.settings:get_bool("vlc_logging_event_api",false)
local active_events = {}
...
local function vlc_log(m,l)
	if DBG then
		if not l then l = "action" end
		minetest.log(l,"[vlc_events] "..m)
	end
end

function vlc_events.register_event(name,def)
	if table.indexof(disabled_events,name) ~= -1 then return end
	vlc_events.registered_events[name] = def
	vlc_events.registered_events[name].name = name
end
...
local function addbars(self)
	...
	for _,player in pairs(minetest.get_connected_players()) do
		if vector.distance(self.pos,player:get_pos()) < 64 then
			local bar = vlc_bossbars.add_bar(player, {color = "red", text = self.readable_name .. ": Wave "..self.stage.." / "..self.max_stage, percentage = self.percent }, true,1)
...
local function start_event(p,e)
	vlc_log("[vlc_events] Event started: "..e.readable_name.." at "..minetest.pos_to_string(vector.round(p.pos)))
...
local function finish_event(self,idx)
	vlc_log("[vlc_events] Finished: "..self.readable_name.." at "..minetest.pos_to_string(vector.round(self.pos)))
	if self.on_complete then self:on_complete() end
	for _,b in pairs(self.bars) do
		vlc_bossbars.remove_bar(b)
	end
...
function check_events(dtime)
		...
		elseif not ae.finished and ae.cond_progress then
			...
			if p == true then
				...
				if ae:on_stage_begin() == true then
					vlc_log("[vlc_events] Event "..ae.readable_name.." at "..minetest.pos_to_string(vector.round(ae.pos)).." failed at stage_begin of stage "..ae.stage )
	...
	etime = 10
	for _,e in pairs(vlc_events.registered_events) do
		...
		if pp then
			for _,p in pairs(pp) do
				...
				if start then
					start_event(p,e)
				elseif DBG then
					vlc_log("[vlc_events] Event "..e.readable_name.." already active at "..minetest.pos_to_string(vector.round(p.pos)))
				end
			end
		end
	end
	for idx,ae in pairs(active_events) do
		...
		if ae.pos and not player_near then
			vlc_log("[vlc_events] Event "..ae.readable_name.." at "..minetest.pos_to_string(vector.round(ae.pos)).." aborted - no players near." )
			active_events[idx] = nil
		end
	end
end
...
vlc_info.register_debug_field("Active Events",{
	level = 4,
...
minetest.register_chatcommand("event_start",{
	...
	func = function(pname,param)
		...
		local evdef = vlc_events.registered_events[param]
