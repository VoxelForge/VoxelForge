...
function vlf_util.file_exists(name)
...
-- Returns true on success and false on failure.
function vlf_util.move_item_container(source_pos, destination_pos, source_list, source_stack_id, destination_list)
	...
	local function normalize_double_container(pos, node, ctype)
		if ctype == 6 then
			pos = vlf_util.get_double_container_neighbor_pos(pos, node.param2, "right")
			if not pos then
				return false
			end
	...
	if dctype ~= 0 then
		...
		if destination_list then
			...
			if not ok and dctype == 5 then
				...
				ok = vlf_util.move_item(sinv, source_list, source_stack_id, dinv, destination_list)
			end
...
vlf_util.drop_item_stack = drop_item_stack

function vlf_util.drop_items_from_meta_container(lists)
...
function vlf_util.generate_on_place_plant_function(condition)
...
function vlf_util.get_object_center(obj)
...
function vlf_util.get_color(colorstr)
	local mc_color = vlf_colors[colorstr:upper()]
	if mc_color then
		colorstr = mc_color
...
function vlf_util.call_on_rightclick(itemstack, player, pointed_thing)
	-- Call on_rightclick if the pointed node defines it
...
function vlf_util.calculate_durability(itemstack)
	local unbreaking_level = vlf_enchanting.get_enchantment(itemstack, "unbreaking")
	local armor_uses = minetest.get_item_group(itemstack:get_name(), "vlf_armor_uses")
	...
	if armor_uses > 0 then
		...
		if def then
			local fixed_uses = def._vlf_uses
			if fixed_uses then
				uses = fixed_uses
				if unbreaking_level > 0 then
...
function vlf_util.use_item_durability(itemstack, n)
	local uses = vlf_util.calculate_durability(itemstack)
	itemstack:add_wear(65535 / uses * n)
end

function vlf_util.deal_damage(target, damage, vlf_reason)
	if not vlf_reason.flags then
		vlf_damage.finish_reason(vlf_reason)
	end
	local luaentity = target:get_luaentity()
	if luaentity then
		damage = vlf_damage.run_modifiers(target, damage, vlf_reason or {type = "generic"})
...
function vlf_util.get_inventory(object, create)
...
function vlf_util.set_properties(obj, props)
...
function vlf_util.bypass_buildable_to(func)
	--------------------------
	-- MINETEST CODE: UTILS --
	--------------------------
...
function vlf_util.safe_place(pos, node, player, itemstack)
	local name = player and player:get_player_name() or ""
	local nnode = node or (itemstack and {name = itemstack:get_name()}) or nil
	if not nnode then return itemstack end
	if vlf_util.check_position_protection(pos,player) then return itemstack end
...
function vlf_util.replace_node_vm(pos1, pos2, mat_from, mat_to, is_group)
...
function vlf_util.bulk_set_node_vm(pos1, pos2, mat_to)
	local c_to = minetest.get_content_id(mat_to)
...
-- Will also set param2 on changed nodes if provided.
function vlf_util.circle_bulk_set_node_vm(radius, pos, y, mat_to, param2)
	local c_to = minetest.get_content_id(mat_to)
