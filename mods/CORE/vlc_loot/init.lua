...
function vlc_loot.get_loot(loot_definitions, pr)
	local items = {}
	...
	for s=1, stacks do
		...
		if item then
			...
			if itemstring then
				...
				minetest.log("error", "[vlc_loot] INTERNAL ERROR! Failed to select random loot item!")
			end
		end
	end
...
--[[
Repeat vlc_loot.get_loot multiple times for various loot_definitions.
Useful for filling chests.

* multi_loot_definitions: Table of loot_definitions (see vlc_loot.get_loot)
* pr: PseudoRandom object used for the randomness

Returns: Table of itemstrings ]]
function vlc_loot.get_multi_loot(multi_loot_definitions, pr)
	local items = {}
	for m=1, #multi_loot_definitions do
		local group = vlc_loot.get_loot(multi_loot_definitions[m], pr)
		for g=1, #group do
...
function vlc_loot.fill_inventory(inv, listname, items, pr)
