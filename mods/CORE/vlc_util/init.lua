local ground_padding = tonumber(minetest.settings:get("vlc_ground_padding")) or 1
...
function vlc_util.get_luaentity_by_id(id)
	for _, e in pairs(minetest.luaentities) do
		if e._id == id then return e end
	end
end

local LOGGING_ON = minetest.settings:get_bool("vlc_logging_default", false)
local LOG_MODULE = "[MCL2]"
function vlc_util.vlc_log(message, module, bypass_default_logger)
...
function vlc_util.file_exists(name)
...
function vlc_util.rotate_axis_and_place(itemstack, placer, pointed_thing, infinitestacks, invert_wall)
...
function vlc_util.rotate_axis(itemstack, placer, pointed_thing)
	if placer and placer:is_player() then
		vlc_util.rotate_axis_and_place(itemstack, placer, pointed_thing,
...
function vlc_util.is_pointing_above_middle(pointer, pointed_thing)
	if
		not pointer
...
function vlc_util.get_double_container_neighbor_pos(pos, param2, side)
	if side == "right" then
		if param2 == 0 then
...
function vlc_util.get_eligible_transfer_item_slot(src_inventory, src_list, dst_inventory, dst_list, condition)
...
function vlc_util.move_item(source_inventory, source_list, source_stack_id, destination_inventory, destination_list)
	if source_stack_id == -1 then
		source_stack_id = vlc_util.get_first_occupied_inventory_slot(source_inventory, source_list)
...
-- Returns true on success and false on failure.
function vlc_util.move_item_container(source_pos, destination_pos, source_list, source_stack_id, destination_list)
	...
	local function normalize_double_container(pos, node, ctype)
		if ctype == 6 then
			pos = vlc_util.get_double_container_neighbor_pos(pos, node.param2, "right")
			if not pos then
				return false
			end
	...
	if source_stack_id == -1 then
		...
		source_stack_id = vlc_util.get_eligible_transfer_item_slot(sinv, source_list, dinv, dpos, cond)
		if not source_stack_id then
			-- Try again if source is a double container
			if sctype == 5 then
				spos = vlc_util.get_double_container_neighbor_pos(spos, snode.param2, "left")
	...
	if dctype ~= 0 then
		...
		if destination_list then
			-- Move item
			local ok = vlc_util.move_item(sinv, source_list, source_stack_id, dinv, destination_list)

			-- Try transfer to neighbor node if transfer failed and double container
			if not ok and dctype == 5 then
				dpos = vlc_util.get_double_container_neighbor_pos(dpos, dnode.param2, "left")
				dmeta = minetest.get_meta(dpos)
				dinv = dmeta:get_inventory()

				ok = vlc_util.move_item(sinv, source_list, source_stack_id, dinv, destination_list)
			end
...
function vlc_util.get_first_occupied_inventory_slot(inventory, listname)
	return vlc_util.get_eligible_transfer_item_slot(inventory, listname)
end
...
vlc_util.drop_item_stack = drop_item_stack

function vlc_util.drop_items_from_meta_container(lists)
...
function vlc_util.is_fuel(item)
...
function vlc_util.generate_on_place_plant_function(condition)
...
function vlc_util.get_object_center(obj)
...
function vlc_util.get_color(colorstr)
	local mc_color = vlc_colors[colorstr:upper()]
	if mc_color then
		colorstr = mc_color
...
function vlc_util.call_on_rightclick(itemstack, player, pointed_thing)
	-- Call on_rightclick if the pointed node defines it
...
function vlc_util.calculate_durability(itemstack)
	local unbreaking_level = vlc_enchanting.get_enchantment(itemstack, "unbreaking")
	local armor_uses = minetest.get_item_group(itemstack:get_name(), "vlc_armor_uses")
	...
	if armor_uses > 0 then
		...
		if def then
			local fixed_uses = def._vlc_uses
			if fixed_uses then
				uses = fixed_uses
				if unbreaking_level > 0 then
...
function vlc_util.use_item_durability(itemstack, n)
	local uses = vlc_util.calculate_durability(itemstack)
	itemstack:add_wear(65535 / uses * n)
end

function vlc_util.deal_damage(target, damage, vlc_reason)
	if not vlc_reason.flags then
		vlc_damage.finish_reason(vlc_reason)
	end
	local luaentity = target:get_luaentity()
	if luaentity then
		damage = vlc_damage.run_modifiers(target, damage, vlc_reason or {type = "generic"})
		if luaentity.deal_damage then
			if luaentity:deal_damage(damage, vlc_reason or {type = "generic"}) ~= true then
				vlc_damage.run_damage_callbacks(target, damage, vlc_reason or {type = "generic"})
			end
			return
		elseif luaentity.is_mob then
			if vlc_reason.source and vlc_reason.source.is_player and vlc_reason.source:is_player() then
				luaentity.last_player_hit_time = minetest.get_gametime()
				luaentity.last_player_hit_name = vlc_reason.source:get_player_name()
			end
			if luaentity.health - damage > 0 then
				...
				luaentity.health = luaentity.health - damage
				vlc_damage.run_death_callbacks(target, vlc_reason or {type = "generic"})
			end
			return
		else
			local armorgroups = target:get_armor_groups()
			if armorgroups and not armorgroups.immortal then
				local puncher = vlc_reason and vlc_reason.direct or target
	...
	if hp > 0 and armorgroups and not armorgroups.immortal then
		target:set_hp(hp - damage, {_vlc_reason = vlc_reason})
	end
end

function vlc_util.get_hp(obj)
...
function vlc_util.get_inventory(object, create)
...
function vlc_util.get_wielded_item(object)
	if object:is_player() then
...
function vlc_util.get_object_name(object)
	if object:is_player() then
...
function vlc_util.replace_mob(obj, mob)
...
function vlc_util.get_pointed_thing(player, liquid)
...
function vlc_util.set_properties(obj, props)
...
function vlc_util.set_bone_position(obj, bone, pos, rot)
...
function vlc_util.bypass_buildable_to(func)
	--------------------------
	-- MINETEST CODE: UTILS --
	--------------------------
...
function vlc_util.check_area_protection(pos1, pos2, player, interval)
...
function vlc_util.check_position_protection(position, player)
	local name = player and player:get_player_name() or ""
...
function vlc_util.safe_place(pos, node, player, itemstack)
	local name = player and player:get_player_name() or ""
	local nnode = node or (itemstack and {name = itemstack:get_name()}) or nil
	if not nnode then return itemstack end
	if vlc_util.check_position_protection(pos,player) then return itemstack end
...
function vlc_util.in_cube(tpos, wpos1, wpos2)
	local xmax=wpos2.x
	local xmin=wpos1.x
...
function vlc_util.traverse_tower(pos, dir, callback)
	local node = minetest.get_node(pos)
	local i = 0
...
function vlc_util.replace_node_vm(pos1, pos2, mat_from, mat_to, is_group)
...
function vlc_util.circle_replace_node_vm(radius, pos, y, mat_from, mat_to, param2)
...
function vlc_util.bulk_set_node_vm(pos1, pos2, mat_to)
	local c_to = minetest.get_content_id(mat_to)
...
-- Will also set param2 on changed nodes if provided.
function vlc_util.circle_bulk_set_node_vm(radius, pos, y, mat_to, param2)
	local c_to = minetest.get_content_id(mat_to)
...
function vlc_util.create_ground_turnip(pos, fwidth, fdepth)

	local biome_data = minetest.get_biome_data(pos)
	local biome_name = minetest.get_biome_name(biome_data.biome)
	local reg_biome = minetest.registered_biomes[biome_name]

	local mat = "vlc_core:dirt"
	local filler = "vlc_core:dirt"
	local grass_idx = 0

	-- Use biome info if we have it
	if reg_biome and reg_biome.node_top then
		mat = reg_biome.node_top
		grass_idx = reg_biome._vlc_palette_index or 0
	...
	for count2 = 1, 2 do
		...
		vlc_util.circle_bulk_set_node_vm(radius, pos, y, mat, grass_idx)
		y = y - 1
	end
	...
	for count3 = 1, 5 do
		...
		vlc_util.circle_bulk_set_node_vm(radius, pos, y, filler)
		y = y - 1
	end
end
