...
local mod_fire = minetest.get_modpath("vlc_fire")
...
minetest.register_on_mods_loaded(function()
	...
	for name, def in pairs(minetest.registered_nodes) do
		...
		node_blastres[id] = def._vlc_blast_resistance or 0
...
local function trace_explode(pos, strength, raydirs, radius, info, direct, source)
	...
	for _, obj in pairs(objs) do
		...
		if (obj:is_player() or (ent and ent.name ~= "__builtin.item")) and obj:get_hp() > 0 then
			...
			if collisionbox then
				...
				vlc_util.deal_damage(obj, damage, { type = "explosion", direct = direct, source = source })
		...
		if ent and ent.name == "vlc_end:crystal" then
			if direct then
				local puncher = direct:get_luaentity()
				if puncher and puncher.name == "vlc_end:crystal" then
	...
	if #fires > 0 then
		minetest.bulk_set_node(fires, { name = "vlc_fire:fire" })
	end
...
function vlc_explosions.explode(pos, strength, info, direct, source)
	if info == nil then
		info = {}
	end
